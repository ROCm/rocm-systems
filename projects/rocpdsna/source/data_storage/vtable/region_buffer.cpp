...
constexpr std::array<const char*, region_buffer::k_total_columns> k_column_names = {
    "id", "nid", "pid", "tid", "start", "end", "name_id", "event_id", "extdata"
};
...
std::unordered_map<std::string, region_buffer*>&
registry()
{
    static std::unordered_map<std::string, region_buffer*> r;
    return r;
}
...
void
region_buffer::push_from_values(sqlite3_value** argv)
{
...
int
region_buffer::prepare_insert_stmt()
{
...
void
region_buffer::reserve(std::size_t expected_rows)
{
    for(auto& col : m_int_cols)
    {
...
void
region_buffer::register_instance(const std::string& real_table_name,
...
void
region_buffer::unregister_instance(const std::string& real_table_name)
{
    std::lock_guard<std::mutex> lock(registry_mutex());
...
region_buffer*
region_buffer::get_active_instance(const std::string& real_table_name)
{
    std::lock_guard<std::mutex> lock(registry_mutex());
