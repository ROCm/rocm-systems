...
#include "region_buffer_vtab.hpp"

#include "region_buffer.hpp"
...
struct region_vtab : public sqlite3_vtab
{
    sqlite3*                       db = nullptr;
    std::string                    real_table_name;
    std::unique_ptr<region_buffer> buffer;
};

struct region_cursor : public sqlite3_vtab_cursor
{
                 ...
                 char**             pzErr)
{
    if(argc < 4)
    {
        *pzErr = sqlite3_mprintf("region_buffer requires one argument: real table name");
        return SQLITE_ERROR;
    }
    ...
    std::string create_sql = "CREATE TABLE x("
                             ...
                             "start INTEGER, \"end\" INTEGER, name_id INTEGER, "
    ...
    auto* vtab = new(std::nothrow) region_vtab();
...
{
    auto* vtab = static_cast<region_vtab*>(p);
    if(vtab != nullptr)
    {
        region_buffer::unregister_instance(vtab->real_table_name);
        vtab->buffer.reset();
        delete vtab;
    }
    return SQLITE_OK;
...
{
    auto* cur = new(std::nothrow) region_cursor();
...
{
    delete static_cast<region_cursor*>(c);
    return SQLITE_OK;
}
...
{
    static_cast<region_cursor*>(c)->rowid++;
    return SQLITE_OK;
}
...
{
    *out = static_cast<region_cursor*>(c)->rowid;
    return SQLITE_OK;
}
...
xUpdate(sqlite3_vtab* p, int argc, sqlite3_value** argv, sqlite3_int64* out_rowid)
{
    ...
    constexpr int expected_argc = 2 + static_cast<int>(region_buffer::k_total_columns);
...
{
    int rc = sqlite3_create_module(db, "region_buffer", &k_module, nullptr);
    if(rc != SQLITE_OK)
    {
        LOG_ERROR("Failed to register region_buffer module: {}", sqlite3_errmsg(db));
    }
    return rc;
}
