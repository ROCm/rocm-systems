...
#include "memory_alloc_buffer_vtab.hpp"
...
struct memory_alloc_vtab : public sqlite3_vtab
{
    sqlite3*                             db = nullptr;
    std::string                          real_table_name;
    std::unique_ptr<memory_alloc_buffer> buffer;
};

struct memory_alloc_cursor : public sqlite3_vtab_cursor
{
                 ...
                 char**             pzErr)
{
    ...
    {
        *pzErr =
            sqlite3_mprintf("memory_alloc_buffer requires one argument: real table name");
        return SQLITE_ERROR;
    }
    ...
    std::string create_sql =
        ...
        "type TEXT, level TEXT, start INTEGER, \"end\" INTEGER, address INTEGER, "
        "size INTEGER, queue_id INTEGER, stream_id INTEGER, event_id INTEGER, "
        "extdata TEXT"
        ")";
    ...
    auto* vtab = new(std::nothrow) memory_alloc_vtab();
    ...
    vtab->buffer = std::make_unique<memory_alloc_buffer>(vtab->real_table_name, db_path);
    memory_alloc_buffer::register_instance(vtab->real_table_name, vtab->buffer.get());
...
{
    auto* vtab = static_cast<memory_alloc_vtab*>(p);
    if(vtab != nullptr)
    {
        memory_alloc_buffer::unregister_instance(vtab->real_table_name);
        vtab->buffer.reset();
        delete vtab;
    }
    return SQLITE_OK;
...
{
    auto* cur = new(std::nothrow) memory_alloc_cursor();
...
{
    delete static_cast<memory_alloc_cursor*>(c);
    return SQLITE_OK;
}
...
{
    static_cast<memory_alloc_cursor*>(c)->rowid++;
    return SQLITE_OK;
}
...
{
    *out = static_cast<memory_alloc_cursor*>(c)->rowid;
    return SQLITE_OK;
}
...
xUpdate(sqlite3_vtab* p, int argc, sqlite3_value** argv, sqlite3_int64* out_rowid)
{
    ...
    constexpr int expected_argc =
        2 + static_cast<int>(memory_alloc_buffer::k_total_columns);
...
{
    int rc = sqlite3_create_module(db, "memory_alloc_buffer", &k_module, nullptr);
    if(rc != SQLITE_OK)
    {
        LOG_ERROR("Failed to register memory_alloc_buffer module: {}",
                  sqlite3_errmsg(db));
    }
    return rc;
}
