...
struct event_detail_result
{
    ...
    std::string           call_stack;   // Empty in v4, use separate query
    std::string           line_info;    // Empty in v4, use separate query
    std::string           extdata;
};
...
struct event_id_result
{
    ...
    std::optional<size_t> parent_stack_id;
    std::optional<size_t> correlation_id;
    std::string           call_stack;      // Empty in v4, fetched from rocpd_call_stack table
    std::string           line_info;       // Empty in v4, fetched from rocpd_line_info table  
...
private:
    ...
    void initialize_detail_statements()
    {
        ...
        // v4 schema: call_stack and line_info are in separate tables, use empty strings
        auto ev_q = fmt::format(
            "SELECT id, category_id, stack_id, parent_stack_id, correlation_id, "
            "'' as call_stack, '' as line_info, extdata FROM rocpd_event_{} WHERE id = ?",
            m_uuid);
    ...
    void initialize_event_id_statements()
    {
        auto make_event_id_stmt = [&](const std::string& table) {
            // v4 schema: call_stack and line_info are in separate tables
            // Return empty strings for compatibility with v3 result structure
            auto q =
                fmt::format("SELECT E.id, E.category_id, E.stack_id, E.parent_stack_id, "
                            "E.correlation_id, '' as call_stack, '' as line_info, E.extdata "
