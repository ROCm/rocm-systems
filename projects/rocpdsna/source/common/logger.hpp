...
{
    logger_settings_t()
    : m_log_level(log_level_from_env(std::getenv("ROCPDSNA_LOG_LEVEL")))
    , m_log_file(std::getenv("ROCPDSNA_LOG_FILE"))
    {
        const char* rocpdsna_monochrome_env = std::getenv("ROCPDSNA_MONOCHROME");
        const char* monochrome_env          = std::getenv("MONOCHROME");
        if((rocpdsna_monochrome_env != nullptr) || (monochrome_env != nullptr))
        {
            m_monochrome = parse_boolean_env(rocpdsna_monochrome_env) ||
...
private:
    ...
    static constexpr const char* s_logger_name = "rocpdsna";
};
