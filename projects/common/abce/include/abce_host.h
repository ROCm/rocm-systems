...
/// @brief Registered engine matching a hardware SDMA engine id, or kMaxEngines.
/// Only the engine table needs this, and only while it is being formed.
...
struct SubmitResult {
  ...
  /// Ring the coordinator frame ran on, or kMaxEngines unless @c status is
  /// kSuccess. A client that keeps a stream on one ring needs the engine that
  /// actually ran, which for a fused MapSubmit the policy chose rather than the
  /// client.
  uint32_t coordinator_engine = kMaxEngines;
};
...
///
/// MapSubmit() fuses both, for a client content to run with ABCE's choice. The
/// phases stay separable because that choice is a recommendation, not a verdict:
/// the client states which engines are permitted in CopyMetadata::engine_mask,
/// ABCE best-fits within them and reports its pick per frame along with the
/// ranked alternatives, and the client may keep it, retarget PlanFrame::engine,
/// or inspect the plan before it is published.
class CopyOrchestrator {
 public:
  ...
  /// @brief The engine preference table selection reads. Formed from the part's
  /// host-copy profile plus whatever the driver said about individual links, and
  /// re-formed whenever either input changes, so it is always current for the
  /// engines registered so far.
  ///
  /// Exposed because a client driving device-side submission has to publish the
  /// same table to its kernels: both paths must answer "which engine for this
  /// agent pair" identically or a stream would migrate rings mid-flight.
  const EngineTable& engine_table() const { return engine_table_; }

  /// Install a caller-supplied table, for a client that would rather not use
  /// the built-in profiles at all. Replaces everything formation would produce.
  void SetEngineTable(const EngineTable& table) { engine_table_ = table; }

  // ---- Device init ----

  /// @brief Record the device's total SDMA engine count (host + xGMI), which
  /// keys the host-copy profile: the same IP ships with different counts and
  /// genuinely different measured orders. A fixed device property, so this is
  /// said once. LoadTopologyFromKfd() reads the same count from the driver, so
  /// call this only when KFD topology is unavailable.
  void InitDeviceProfile(uint32_t total_sdma_engines) {
    total_sdma_ = total_sdma_engines;
    FormEngineTable();
  }

  /// @brief Harvest the driver's per-link engine recommendations, which is the
  /// whole of what ABCE wants from topology now: KFD's
  /// recommended_sdma_engine_id_mask names the engines with affinity to each
  /// link, already indexed by the node ids the client uses as device ids. Nothing
  /// here derives an engine class, a hive or a physical slot.
  ///
  /// Returns false where KFD sysfs does not exist, leaving the table as it was;
  /// device-to-device cells then stay empty and selection load-balances.
  bool LoadTopologyFromKfd(const std::string& base_path = "/sys/class/kfd/kfd/topology/nodes") {
    ...
    if (result.total_sdma != 0) total_sdma_ = result.total_sdma;
    FormEngineTable();
    return true;
#else
    (void)base_path;
  ...
  /// Register an engine's ring and re-form the engine table, so the table always
  /// describes exactly the engines registered so far. Formation is a few hundred
  /// cell writes, and this runs once per ring at init.
  bool RegisterEngine(uint32_t index, RingBuffer* ring, const EngineAffinity& affinity = {}) {
    ...
    // A hardware id outside the table's range would leave the engine registered
    // but unrankable, so refuse it here rather than half-register it.
  ...
  SubmitResult Submit(Plan& plan) {
    ...
    result.coordinator_engine = plan.frames[0].engine;
    return result;
  }

  /// @brief Map and submit in one call, leaving engine choice to the policy.
  ///
  /// The two-phase form exists for clients that either arbitrate engines using
  /// state ABCE does not model — stream affinity, cross-stream exclusion — or
  /// inspect the plan before it is published. A client with neither need wants
  /// this one: MapCopy already ranks by measured bandwidth and xGMI affinity and
  /// balances a fan-out across rings. The ring the coordinator ended up on comes
  /// back in SubmitResult::coordinator_engine.
  SubmitResult MapSubmit(const CopyOp* copies, uint32_t num_copies,
 ...
 private:
  ...
  /// Agent id for an endpoint, in the space the engine table is indexed in.
  static int32_t AgentOf(const CopyEndpoint& endpoint) {
    if (endpoint.kind == EndpointKind::kHost) return kAgentHost;
    return endpoint.device_id == kAnyDevice ? kAgentUnknown
                                            : static_cast<int32_t>(endpoint.device_id);
  }

  /// Expand this copy's table cell into a ranked list over @p candidates: the
  /// cell's preferred prefix first, then every other permitted engine ascending,
  /// because a prefix ranks rather than restricts. Forbidden engines are dropped.
  ///
  /// kNoPreference is reported only for a genuinely empty cell, where the
  /// permitted set is all of @p candidates and a caller's ascending fallback is
  /// therefore still correct.
  PolicyResult InvokePolicy(const CopyOp& op, uint64_t candidates,
                            uint32_t (&ranked)[kRankMax]) const {
    const EngineOrder& order = engine_table_.Cell(AgentOf(op.src_end), AgentOf(op.dst_end));
    const uint64_t permitted = candidates & ~order.forbidden_mask;
    if (permitted == 0) return {PolicyDisposition::kNoLegalEngine, 0};

    uint32_t count = 0;
    uint64_t emitted = 0;
    for (uint32_t rank_idx = 0; rank_idx < order.num_ranked && count < kRankMax; ++rank_idx) {
      const uint32_t engine = order.ranked[rank_idx];
      if (engine >= kMaxEngines) continue;
      const uint64_t engine_bit = uint64_t{1} << engine;
      if (!(permitted & engine_bit) || (emitted & engine_bit)) continue;
      emitted |= engine_bit;
      ranked[count++] = engine;
    }
    const bool expressed_preference = count != 0;
    for (uint32_t engine = 0; engine < kMaxEngines && count < kRankMax; ++engine) {
      const uint64_t engine_bit = uint64_t{1} << engine;
      if (!(permitted & engine_bit) || (emitted & engine_bit)) continue;
      ranked[count++] = engine;
    }
    if (count == 0) return {PolicyDisposition::kNoLegalEngine, 0};
    const bool restricted = permitted != candidates;
    return {(expressed_preference || restricted) ? PolicyDisposition::kRanked
                                                 : PolicyDisposition::kNoPreference,
            count};
  }
  ...
  uint32_t BackToBackEngine(const CopyOp& first_copy, uint64_t candidates) const {
    ...
    if (isa.major == 9 && isa.minor >= 4) {
      const uint32_t reversed_host_engine = RegisteredEngineForHwId(1);
  ...
  //
  // Placement itself is shared with the device planner and lives in
  // abce_engine_table.h (FillOrder / PlacePiece): spread across the permitted
  // rings first and balance after, or fill one ring at a time for a
  // back-to-back batch, capped at kMaxPiecesPerEngine either way.
  //
  // What stays here is the part-specific notion of which rings are LEGAL for a
  // copy, which PlaceOp hands the policy as a forbidden mask:
  //
  ...
  //     candidate ring and there is no recommended ring, so placement reduces to
  //     spread-then-balance, independent of src/dst.
  ...
  /// @brief Assigns @p op a ring through the shared placement policy and records
  /// the ranked choices HwQueueTracker needs to elide a preceding signal.
  ///
  /// @p preferred_override anchors a back-to-back batch on the ring
  /// BackToBackEngine chose; kMaxEngines leaves the pair's own affinity in
  /// charge. Leaves @c op.engine at kMaxEngines when the pair has no legal ring,
  /// which every caller turns into kNoLegalEngine.
  void PlaceOp(EngineOp& op, const CopyOp& copy, uint64_t candidates,
               EnginePlacement& placement, uint32_t preferred_override = kMaxEngines) const {
    uint32_t recommended = kMaxEngines;
    bool has_engine_preference = false;
    const uint64_t legal_mask = LegalEngineSet(copy, candidates, recommended, op.ranked_engines,
                                               op.num_ranked_engines, has_engine_preference);
    op.has_engine_preference = has_engine_preference;
    op.engine = PlacePiece(
        placement, preferred_override < kMaxEngines ? preferred_override : recommended,
        candidates & ~legal_mask);
    if (op.engine < kMaxEngines && op.num_ranked_engines == 0) {
      op.ranked_engines[0] = op.engine;
  ...
  bool Decompose(const CopyOp* copies, uint32_t num_copies, const CopyMetadata& metadata,
                 ...
                 uint32_t& failed_op) const {
    // A batch spread over several rings is several frames, and a multi-frame
    // plan needs the fan-in atomic to retire its completion — so without atomic
    // support every batch stays on one ring whatever its shape.
    const bool may_use_many_rings = PopCount(candidates) > 1 && caps_.device_atomic_support;
    EnginePlacement placement;
    placement.candidates = candidates;
    // A back-to-back batch fills one ring at a time, so a short batch stays on a
    // single ring and therefore in a single frame with no coordinator to pay
    // for. It spills only once a ring is kMaxPiecesPerEngine deep. Everything
    // else spreads across the permitted rings first and balances after.
    placement.order =
        force_back_to_back ? FillOrder::kDepthFirst : FillOrder::kBreadthFirst;
    placement.max_participants = may_use_many_rings ? metadata.max_engines : 1;
    ...
    for (uint32_t op_idx = 0; op_idx < num_copies; ++op_idx) {
      ...
      if (cop.kind == OpKind::kMulticast) {
        if (!DecomposeMultiDst(cop, metadata, candidates, placement, force_back_to_back,
                               fixed_engine, ops, num_ops, capacity))
      ...
      PlaceOp(engine_op, cop, candidates, placement,
              force_back_to_back ? fixed_engine : kMaxEngines);
  ...
  bool DecomposeMultiDst(const CopyOp& op, const CopyMetadata& metadata, uint64_t candidates,
                         EnginePlacement& placement, bool force_back_to_back,
                         uint32_t fixed_engine, EngineOp* ops, uint32_t& num_ops,
                         uint32_t capacity) const {
    const uint32_t anchor = force_back_to_back ? fixed_engine : kMaxEngines;

    // gfx125plus multicast: one packet handles up to kMulticastMaxDsts
    // destinations. If num_dsts exceeds that, chunk and place each chunk.
    ...
    if (is_gfx125plus_ && use_multicast) {
      for (uint32_t base = 0; base < op.num_dsts; base += kMulticastMaxDsts) {
        ...
        PlaceOp(engine_op, op, candidates, placement, anchor);
    ...
    // The two-destination broadcast packet is the pre-OSS7 equivalent, so a wide
    // broadcast becomes ceil(N/2) packets with a linear copy for an odd tail.
    if (!is_gfx125plus_ && op.size < BroadcastMaxSize()) {
      for (uint32_t base = 0; base < op.num_dsts; base += kBroadcastMaxDsts) {
        const bool whole_pair = op.num_dsts - base >= kBroadcastMaxDsts;
        EngineOp engine_op{};
        engine_op.src = op.src;
        engine_op.size = op.size;
        if (whole_pair) {
    ...
    // Fan out: each destination goes to the ring its own pair prefers, and the
    // shared policy spreads before it stacks.
  ...
  /// Registered index carrying hardware SDMA engine id @p hw_engine_id, or
  /// kMaxEngines. Only formation and the back-to-back host-engine pick need the
  /// hardware numbering; nothing on the selection path does.
  uint32_t RegisteredEngineForHwId(uint32_t hw_engine_id) const {
    for (uint32_t engine = 0; engine < kMaxEngines; ++engine)
      if ((registered_mask_ & (uint64_t{1} << engine)) &&
          affinity_[engine].hw_engine_id == hw_engine_id)
        return engine;
    return kMaxEngines;
  }

  /// Rebuild the table from the part profile, the registered engines and the
  /// driver's per-link recommendations. Cheap and idempotent, so every input
  /// change just calls it again rather than tracking what became stale.
  void FormEngineTable() {
    EngineTableBuilder table_builder(builder_.Isa(), total_sdma_);
    for (uint32_t engine = 0; engine < kMaxEngines; ++engine)
      if (registered_mask_ & (uint64_t{1} << engine))
        table_builder.RegisterEngine(engine, affinity_[engine].hw_engine_id);
    for (int32_t src_agent = 0; src_agent < static_cast<int32_t>(kMaxTableAgents); ++src_agent)
      for (int32_t dst_agent = 0; dst_agent < static_cast<int32_t>(kMaxTableAgents); ++dst_agent) {
        const uint64_t recommended = topology_.RecommendedMask(
            static_cast<uint32_t>(src_agent), static_cast<uint32_t>(dst_agent));
        if (recommended != 0)
          table_builder.SetPeerRecommendation(src_agent, dst_agent, recommended);
      }
    engine_table_ = table_builder.Build();
  }

  uint32_t total_sdma_ = 0;   ///< keys the host-copy profile.
  TopologyData topology_;     ///< driver per-link recommendations only.
  EngineTable engine_table_;  ///< what selection actually reads.
};
