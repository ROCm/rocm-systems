...
class FrameComposer {
 ...
 private:
  ...
  static constexpr uint32_t kPoll64 = sizeof(SDMA_PKT_POLL_MEM_64B_OSS70);
  static constexpr uint32_t kFence32 = sizeof(SDMA_PKT_FENCE);
  static constexpr uint32_t kFence64 = sizeof(SDMA_PKT_FENCE_64B_OSS70);
  ...
  uint32_t GcrBytes() const {
    ...
    return is_gfx125plus_ ? sizeof(SDMA_PKT_GCR_OSS70) : sizeof(SDMA_PKT_GCR);
  }
  ...
  size_t PayloadBytes(const EngineOp& engine_op) const {
    switch (engine_op.kind) {
      ...
      case OpKind::kIndirect:
        return sizeof(SDMA_PKT_COPY_LINEAR_WAITSIGNAL_INDIRECT_OSS70);
      case OpKind::kFill:
