 ...
 public:
  ...
  ABCE_HD uint32_t NumSwapPackets(size_t size) const {
    const size_t max = (scope_fields_ && is_gfx125plus_)
                           ? SDMA_PKT_COPY_LINEAR_SWAP_OSS70::kMaxSize_
  ...
  ABCE_HD uint32_t NumWaitSignalSwapPackets(size_t size_a, size_t size_b) const {
    ...
    const size_t max = SDMA_PKT_COPY_LINEAR_SWAP_WAITSIGNAL_OSS70::kMaxSize_;
  ...
  ABCE_HD void BuildMulticastCopyCommand(char* cmd_addr, void* const* dsts, uint32_t num_dsts,
    ...
    while (cur_size < size) {
      ...
      auto* pkt = reinterpret_cast<SDMA_PKT_COPY_LINEAR_MULTICAST_OSS70*>(cmd_addr);
      pkt->HEADER_UNION.op = SDMA_OP_COPY;
  ...
  ABCE_HD void BuildMulticastWaitSignalCopyCommand(char* cmd_addr, void* const* dsts,
    ...
    {
      SDMA_PKT_COPY_LINEAR_MULTICAST_WAITSIGNAL_OSS70 tmpl;
    ...
    while (cur_size < size) {
      ...
      SDMA_PKT_COPY_LINEAR_MULTICAST_WAITSIGNAL_OSS70 header{};
      ...
      SDMA_PKT_COPY_LINEAR_MULTICAST_WAITSIGNAL_OSS70 chunk;
  ...
  ABCE_HD void BuildSwapCopyCommand(char* cmd_addr, void* addr_a, void* addr_b, size_t size) const {
    static_assert(sizeof(SDMA_PKT_COPY_LINEAR_SWAP_OSS70) == sizeof(SDMA_PKT_COPY_LINEAR_SWAP),
                  "OSS7.0 swap packet must match legacy swap packet size for shared stride");
    const bool use_gfx125plus = scope_fields_ && is_gfx125plus_;

    // Only read by the asserts, which device compilation compiles out.
    [[maybe_unused]] const size_t alignment =
        use_gfx125plus ? SDMA_PKT_COPY_LINEAR_SWAP_OSS70::kAlignment_
                       : SDMA_PKT_COPY_LINEAR_SWAP::kAlignment_;
    ABCE_ASSERT((reinterpret_cast<uintptr_t>(addr_a) & (alignment - 1)) == 0);
    ABCE_ASSERT((reinterpret_cast<uintptr_t>(addr_b) & (alignment - 1)) == 0);

    const size_t max_copy_size = use_gfx125plus ? SDMA_PKT_COPY_LINEAR_SWAP_OSS70::kMaxSize_
                                                : SDMA_PKT_COPY_LINEAR_SWAP::kMaxSize_;
    size_t cur_size = 0;
    while (cur_size < size) {
      ...
      if (use_gfx125plus) {
        auto* pkt = reinterpret_cast<SDMA_PKT_COPY_LINEAR_SWAP_OSS70*>(cmd_addr);
        pkt->HEADER_UNION.op = SDMA_OP_COPY;
  ...
  ABCE_HD void BuildPoll64bCommand(char* cmd_addr, void* addr, uint64_t reference,
    ...
    auto* pkt = reinterpret_cast<SDMA_PKT_POLL_MEM_64B_OSS70*>(cmd_addr);
    pkt->HEADER_UNION.op = SDMA_OP_POLL_REGMEM;
  ...
  ABCE_HD void BuildFence64bCommand(char* cmd_addr, void* fence_addr, uint64_t fence_value) const {
    auto* pkt = reinterpret_cast<SDMA_PKT_FENCE_64B_OSS70*>(cmd_addr);
    pkt->HEADER_UNION.op = SDMA_OP_FENCE;
  ...
  ABCE_HD void BuildWaitSignalCopyCommand(char* cmd_addr, void* dst, const void* src, size_t size,
    ...
    uint32_t signal_dws[5] = {};
    {
      SDMA_PKT_COPY_LINEAR_WAITSIGNAL_OSS70 tmpl;
      memset(&tmpl, 0, sizeof(tmpl));
      tmpl.HEADER_UNION.op = SDMA_OP_COPY;
    ...
    while (cur_size < size) {
      ...
      SDMA_PKT_COPY_LINEAR_WAITSIGNAL_OSS70 header{};
      header.HEADER_UNION.op = SDMA_OP_COPY;
      ...
      SDMA_PKT_COPY_LINEAR_WAITSIGNAL_OSS70 body;
  ...
  ABCE_HD void BuildWaitSignalIndirectCopyCommand(char* cmd_addr, void* dst, const void* src,
    ...
    SDMA_PKT_COPY_LINEAR_WAITSIGNAL_INDIRECT_OSS70 tmpl;
    memset(&tmpl, 0, sizeof(tmpl));
    tmpl.HEADER_UNION.op = SDMA_OP_COPY;
  ...
  ABCE_HD void BuildWaitSignalSwapCommand(char* cmd_addr, void* addr_a, void* addr_b, size_t size_a,
    ...
    const bool do_wait = (wait_addr != nullptr);
    const bool do_signal = (signal_addr != nullptr);
    const size_t max_copy_size = SDMA_PKT_COPY_LINEAR_SWAP_WAITSIGNAL_OSS70::kMaxSize_;

    uint32_t wait_dws[7] = {};
    uint32_t signal_dws[5] = {};
    {
      SDMA_PKT_COPY_LINEAR_SWAP_WAITSIGNAL_OSS70 tmpl;
      memset(&tmpl, 0, sizeof(tmpl));
      tmpl.HEADER_UNION.op = SDMA_OP_COPY;
    ...
    while (cur_a < size_a || cur_b < size_b) {
      ...
      SDMA_PKT_COPY_LINEAR_SWAP_WAITSIGNAL_OSS70 header{};
      header.HEADER_UNION.op = SDMA_OP_COPY;
      ...
      SDMA_PKT_COPY_LINEAR_SWAP_WAITSIGNAL_OSS70 body;
  ...
  ABCE_HD void BuildGCRCommand(char* cmd_addr, bool invalidate) const {
    ...
    if (is_gfx125plus_) {
      auto* pkt = reinterpret_cast<SDMA_PKT_GCR_OSS70*>(cmd_addr);
