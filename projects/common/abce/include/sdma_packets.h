 ...
 */
 ...
 #ifndef SDMA_PACKETS_H_
 #define SDMA_PACKETS_H_

 #include <stddef.h>
 #include <stdint.h>

// These names mirror ROCr's sdma_registers.h, which declares them inside
// rocr::AMD. Keeping ABCE's copy in namespace abce keeps unqualified lookups
// unambiguous in translation units that see both.
namespace abce {
 ...
 // Compare functions, for the POLL packets' `func` and the fused wait/signal
 // packets' `wait_function`: proceed once (value & mask) == reference.
 const unsigned int SDMA_FUNC_EQUAL = 3;

 // Signal operations, for the fused wait/signal packets' `signal_operation`.
 const unsigned int SDMA_SIGNAL_OP_SUB64 = 112;
 ...
 typedef struct SDMA_PKT_CONSTANT_FILL_TAG {
   union {
     struct {
       unsigned int op           : 8;
       unsigned int sub_op       : 8;
       unsigned int mtype        : 2;  /* gfx125plus*/
       unsigned int reserved_0   : 2;
       unsigned int sys          : 1;  /* gfx125plus*/
       unsigned int reserved_1   : 1;
       unsigned int snp          : 1;  /* gfx125plus*/
       unsigned int gpa          : 1;  /* gfx125plus*/
       unsigned int scope        : 2;  /* gfx125plus*/
       unsigned int temporal_hint: 3;  /* gfx125plus*/
       unsigned int npd          : 1;  /* gfx125plus*/
 ...
 typedef struct SDMA_PKT_POLL_MEM_64B_TAG_GFX125PLUS {
   union {
     struct {
       unsigned int op             :  8;
       unsigned int sub_op         :  8;
 ...
 } SDMA_PKT_POLL_MEM_64B_GFX125PLUS;
 ...
 typedef struct SDMA_PKT_GCR_TAG_GFX125PLUS {
   union {
     struct {
       unsigned int op : 8;
       unsigned int sub_op : 8;
 ...
 } SDMA_PKT_GCR_GFX125PLUS;

 // Linear Swap Copy (GFX125PLUS)
 // Atomically swaps data between Address A and Address B.
 // Addresses must be 32-byte aligned.
 typedef struct SDMA_PKT_COPY_LINEAR_SWAP_TAG_GFX125PLUS {
   union {
     struct {
       unsigned int op          :  8;
       unsigned int sub_op      :  8;
 ...
 } SDMA_PKT_COPY_LINEAR_SWAP_GFX125PLUS;

 // Linear Multicast Copy (GFX125PLUS)
 ...
 typedef struct SDMA_PKT_COPY_LINEAR_MULTICAST_TAG_GFX125PLUS {
   union {
     struct {
       unsigned int op          :  8;
       unsigned int sub_op      :  8;
 ...
 } SDMA_PKT_COPY_LINEAR_MULTICAST_GFX125PLUS;

 // Linear Copy with Wait/Signal (GFX125PLUS)
 ...
 typedef struct SDMA_PKT_COPY_LINEAR_WAITSIGNAL_TAG_GFX125PLUS {
   union {
     struct {
       unsigned int op          :  8;
       unsigned int sub_op      :  8;
 ...
 } SDMA_PKT_COPY_LINEAR_WAITSIGNAL_GFX125PLUS;

 // Linear Copy with Wait/Signal - Indirect Memory Address (GFX125PLUS)
 // Like Linear Copy with Wait/Signal but source/destination addresses can be indirect pointers.
 // DW layout below assumes both WAIT=1 and SIGNAL=1 (maximum packet size).
 typedef struct SDMA_PKT_COPY_LINEAR_WAITSIGNAL_INDIRECT_TAG_GFX125PLUS {
   union {
     struct {
       unsigned int op           :  8;
       unsigned int sub_op       :  8;
 ...
 } SDMA_PKT_COPY_LINEAR_WAITSIGNAL_INDIRECT_GFX125PLUS;

 // Linear Multicast Copy with Wait/Signal (GFX125PLUS)
 ...
 typedef struct SDMA_PKT_COPY_LINEAR_MULTICAST_WAITSIGNAL_TAG_GFX125PLUS {
   union {
     struct {
       unsigned int op          :  8;
       unsigned int sub_op      :  8;
 ...
 } SDMA_PKT_COPY_LINEAR_MULTICAST_WAITSIGNAL_GFX125PLUS;

 // Linear Swap Copy with Wait/Signal (GFX125PLUS)
 // Macro command combining optional wait, swap copy, and optional signal.
 // DW layout below assumes both WAIT=1 and SIGNAL=1 (maximum packet size).
 // Swap addresses must be 32-byte aligned.
 typedef struct SDMA_PKT_COPY_LINEAR_SWAP_WAITSIGNAL_TAG_GFX125PLUS {
   union {
     struct {
       unsigned int op          :  8;
       unsigned int sub_op      :  8;
   ...
   static const size_t kMaxSize_ = 0x3fffffe0;
   static const size_t kAlignment_ = 32;
 } SDMA_PKT_COPY_LINEAR_SWAP_WAITSIGNAL_GFX125PLUS;

 // clang-format on

// Packet ABI checks. These types are copied directly to an SDMA queue, so a
// compiler/layout change must fail compilation rather than silently alter the
// command stream.
static_assert(sizeof(SDMA_PKT_COPY_LINEAR) == 28);
static_assert(sizeof(SDMA_PKT_COPY_LINEAR_BROADCAST) == 36);
static_assert(sizeof(SDMA_PKT_COPY_LINEAR_SWAP) == 28);
static_assert(sizeof(SDMA_PKT_COPY_LINEAR_RECT) == 52);
static_assert(sizeof(SDMA_PKT_COPY_LINEAR_RECT_GFX12) == 52);
static_assert(sizeof(SDMA_PKT_CONSTANT_FILL) == 20);
static_assert(sizeof(SDMA_PKT_FENCE) == 16);
static_assert(sizeof(SDMA_PKT_FENCE_GFX12) == 16);
static_assert(sizeof(SDMA_PKT_FENCE_64B_GFX125PLUS) == 20);
static_assert(sizeof(SDMA_PKT_POLL_REGMEM) == 24);
static_assert(sizeof(SDMA_PKT_POLL_MEM_64B_GFX125PLUS) == 32);
static_assert(sizeof(SDMA_PKT_ATOMIC) == 32);
static_assert(sizeof(SDMA_PKT_TIMESTAMP) == 12);
static_assert(sizeof(SDMA_PKT_TRAP) == 8);
static_assert(sizeof(SDMA_PKT_HDP_FLUSH) == 24);
static_assert(sizeof(SDMA_PKT_GCR) == 20);
static_assert(sizeof(SDMA_PKT_GCR_GFX125PLUS) == 24);
static_assert(sizeof(SDMA_PKT_COPY_LINEAR_SWAP_GFX125PLUS) == 28);
static_assert(sizeof(SDMA_PKT_COPY_LINEAR_WAITSIGNAL_GFX125PLUS) == 76);
static_assert(sizeof(SDMA_PKT_COPY_LINEAR_WAITSIGNAL_INDIRECT_GFX125PLUS) == 76);
static_assert(sizeof(SDMA_PKT_COPY_LINEAR_MULTICAST_WAITSIGNAL_GFX125PLUS) == 76);
static_assert(sizeof(SDMA_PKT_COPY_LINEAR_SWAP_WAITSIGNAL_GFX125PLUS) == 76);
static_assert(alignof(SDMA_PKT_COPY_LINEAR) == alignof(uint32_t));

}  // namespace abce

#endif  // SDMA_PACKETS_H_
