...
- **No device-initiated path.** SDMA packet construction is host-only code, so a kernel cannot
  enqueue a copy without a round trip to the CPU. This is the gap `abce_device.h` closes.
...
- **Host and device from one source.** The packet builders, the placement policy and the ring
  reserve/commit protocol are `__host__ __device__`, so the device-initiated path in
  `abce_device.h` shares the implementation rather than reimplementing the packet layouts. Its
  planner is host-callable for the same reason: every placement and sizing decision can be
  tested without a GPU queue.
...
```
                 client runtime (CLR / HRX-systems / ROCr / test / benchmark)
                                          │
                 ┌────────────────────────┴────────────────────────┐
            host producer                                device producer (a kernel)
                 │                                                 │
  abce_hsa.h     optional adapter:                  abce_device.h  DeviceSubmitLinear:
                 hsa_queue_t -> RingBuffer,                        one copy -> choose ring ->
                 amd_signal_t -> SignalRef                         reserve -> emit ->
                 │                                                 ring doorbell -> token
  abce_host.h    CopyOrchestrator: validate ->                     │
                 select engines -> decompose ->                    │
                 plan -> submit                                    │
                 SdmaEnginePolicy: engine ranking                  │
                 └────────────────────────┬────────────────────────┘
                                          │
  abce_engine_table.h  the (src agent, dst agent) affinity table both producers read;
                       the multi-piece placement policy over it is the host planner's
  abce_frame.h      FrameComposer (host), and the OpKind / DepSignal vocabulary both share
  abce_builder.h    ABCE: one method per SDMA packet, ISA-aware layouts
  abce_ring_host.h  RingBuffer        abce_ring_device.h  DeviceRing: the same protocol,
                                                          driven from a shader
...
A host client normally touches only `abce_host.h` (plus `abce_hsa.h` when submitting to
...
A kernel touches only `abce_device.h`, and its surface is deliberately one function. It
receives a `DeviceEngineSetHandle` the runtime published and calls `DeviceSubmitLinear()` with
one source, one destination, a size and a pre-armed signal; it gets back a token to poll with
`DeviceTokenQuery()` or `DeviceTokenWait()`. No descriptor to fill, no LDS workspace, no
scratch.

Submitting is a **system-scope release point**, and owning that is the API's job rather than
the caller's. The doorbell store orders the packet dwords, and the calling thread's prior
stores to the buffers being copied, ahead of the engine being told to run — so SDMA cannot read
a source before the stores that filled it land, and an in-flight store to the destination
cannot arrive after the copy and overwrite it. A caller therefore neither needs nor should add
a release of its own, which would only pay the cache writeback twice for one guarantee. The
release covers the calling thread; stores made by other threads are covered once the caller has
synchronised with them by its own means first.

The asymmetry between the two producers is intentional. The host planner decomposes a batch
across rings, collapses small multi-destination copies into multicast or broadcast packets,
gates on dependencies and coordinates the resulting frames. The device path does none of that:
one copy is one frame on one ring, so there is nothing to decompose, no surplus arming and no
coordinator — see §6.1. What the two still share is ring choice, through the same
`abce_engine_table.h` affinity data, and publication, through the same `abce_ring_core.h`
reserve/commit protocol. A kernel that wants batching builds it from repeated calls or hands
...
`MapSubmit()` fuses both for callers content to run with the mapping decision unmodified. It
reports the ring it selected in `SubmitResult::coordinator_engine`, since a fused caller has no
`Plan` left to read it from. Keep the seam to override that decision or to inspect the plan
before publication.
...
### 6.1 The device path sidesteps coordination entirely

None of the above applies to `abce_device.h`, which has no coordination word, no start gate and
no fan-in counter. It avoids them by construction rather than by cleverness: one call is one
linear copy, which is one frame on one ring. With a single frame there is no ordering between
frames to establish and no fan-in to count, so the caller's arming is exact — the signal is
armed to `reference + 1` and the frame's single decrement retires it. The submitter never adds
to the signal.

That is the whole reason the device surface stays small. Everything §6 exists to solve —
gating, counting, nominating a coordinator to carry the once-per-batch work — is a consequence
of spreading one logical batch across rings, and the device path simply does not do that. The
single-shot work itself (HDP flush, GCR, profiling timestamps, completion interrupts) stays
host-side for the same reason: each has to happen exactly once around a batch, and a submitter
that only ever emits one self-contained frame has nowhere sensible to put it.

A kernel that needs a batch spread over rings issues several copies and waits on several
tokens, or hands the work to the host planner, which is built for exactly that.
...
multi-producer, with the protocol in `abce_ring_core.h` shared verbatim with the device ring
(`abce_ring_device.h`), so a kernel and the host runtime can produce into one ring
concurrently:
...
- **The device path is undocumented here.** `abce_device.h` and `abce_ring_device.h` carry the
  device-submission surface — `DeviceSubmitLinear()` plus the two token pollers — but this
  document still describes only the host interface. §2 sketches the layering and §6.1 says why
  it needs no coordination; there is no equivalent of §3 for the device yet.
- **The device path does one thing.** A single linear copy per call: no batching, no
  multi-destination fan-out, no dependency gating, no interrupt, and no rect/fill/swap. That is
  a deliberate starting point, not an oversight — the host planner covers the wide cases — but
  a client that expected the device side to mirror `CopyOrchestrator` will not find it here.
- **The device path has no runtime on this branch.** Nothing here constructs a
  `DeviceEngineSet`: the CLR integration that creates the rings and publishes the view lives
  on the device branch, so `abce_device.h` compiles but cannot be exercised from this tree.
  The two halves need reconciling before either lands.
