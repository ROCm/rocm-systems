...
```c++
// raiser.cpp — Phase 1: branch target collection
if (di.isBranch) {
...
```c++
// raiser.cpp — Phase 3: basic blocks from branch targets
...
```c++
// raiser.cpp — AllocaRegFile: the full GPU register file as allocas
...
```c++
// raiser.cpp — Phase 6: alloca → SSA in one call
DominatorTree DT(*F);
...
```c++
// raiser.cpp — Phase 1: implicit defs from MCInstrDesc
...
```c++
// raiser.cpp — Phase 4: ABI-defined register initialization
...
```c++
// raiser.cpp — s_and_saveexec: save old EXEC, EXEC = EXEC & src
...
cross-function calls.  GPU compute kernels have **none of these**.  Our raiser
exploits every one of these structural properties to reduce what is normally a
research-grade problem to a ~5,500-line single-pass translator with 100% raise
rate on 27 production kernels.
