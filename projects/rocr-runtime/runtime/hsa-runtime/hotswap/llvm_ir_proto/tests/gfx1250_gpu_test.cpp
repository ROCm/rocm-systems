...
static bool testVecadd() {
  ...
  auto data = ir_proto::readFile(path);
  if (data.empty()) { fprintf(stderr, "Cannot read %s\n", path.c_str()); return false; }

  auto result = ir_proto::runPipeline(data, "gfx1250", "gfx942", "vecadd_kernel");
  ...
  auto meta = ir_proto::extractKernelMeta(data, "vecadd_kernel");
...
static bool testMatmul(const char *hsacoFile, int M, int N, int K, const char *label) {
  ...
  auto result = ir_proto::runPipeline(data, "gfx1250", "gfx942", "matmul_kernel");
  if (!result.success) { fprintf(stderr, "Pipeline failed\n"); return false; }
  printf("  Pipeline: raised %d/%d insts, HSACO=%zu bytes\n",
         result.liftedCount, result.totalCount, result.hsaco.size());
  ...
  auto meta = ir_proto::extractKernelMeta(data, "matmul_kernel");
...
static bool testSoftmax() {
  ...
  auto result = ir_proto::runPipeline(data, "gfx1250", "gfx942", "softmax_kernel");
  if (!result.success) { fprintf(stderr, "Pipeline failed\n"); return false; }
  printf("  Pipeline: raised %d/%d insts, HSACO=%zu bytes\n",
         result.liftedCount, result.totalCount, result.hsaco.size());
  ...
  auto meta = ir_proto::extractKernelMeta(data, "softmax_kernel");
