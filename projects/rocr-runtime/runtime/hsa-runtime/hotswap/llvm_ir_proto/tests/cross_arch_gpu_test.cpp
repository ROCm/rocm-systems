...
static int getPostGSUScalarBase(const ir_proto::KernelMeta &meta) {
  std::vector<ir_proto::KernelArgMeta> explicitArgs;
...
buildPostGSUKernargs(std::vector<uint8_t> &kernargBuf,
                     const ir_proto::KernelMeta &meta,
...
testPostGSUKernel(const std::string &kernelName,
  ...
  auto pipeResult =
      ir_proto::runPipeline(sourceData, sourceISA, targetISA, kernelName);
  ...
  auto meta = ir_proto::extractKernelMeta(nativeData, kernelName);
  if (meta.args.empty()) {
...
static int testVecaddCrossArch() {
  ...
  auto vecaddData = ir_proto::readFile(vecaddPath);
  if (vecaddData.empty()) {
  ...
  auto pipeResult = ir_proto::runPipeline(vecaddData, "gfx1250", "gfx942",
                                          "_Z6vecaddPfS_S_i");
  if (!pipeResult.success) {
...
int main(int argc, char **argv) {
  ...
  auto nativeData = ir_proto::readFile(nativePath);
  auto sourceData = ir_proto::readFile(sourcePath);
  if (nativeData.empty() || sourceData.empty()) {
    fprintf(stderr, "ERROR: Failed to read hsaco files\n");
    return 1;
  }

  auto kernelNames = ir_proto::listKernelNames(sourceData);
  ...
  for (auto &kName : kernelNames) {
    ...
    if (isPostGSU && !pInfo.isGB) {
      ...
      auto pipeResult =
          ir_proto::runPipeline(sourceData, sourceISA, targetISA, kName);
      ktr.name = kName;
      ktr.liftedCount = pipeResult.liftedCount;
      ktr.totalCount = pipeResult.totalCount;
      ktr.codeBytes = (int)pipeResult.hsaco.size();
