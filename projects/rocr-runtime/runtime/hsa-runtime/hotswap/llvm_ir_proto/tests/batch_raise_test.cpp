...
int main(int argc, char **argv) {
  ...
  for (auto &coPath : coFiles) {
    auto coData = ir_proto::readFile(coPath);
    if (coData.empty()) {
      fprintf(stderr, "WARNING: Cannot read '%s', skipping\n", coPath.c_str());
      continue;
    }

    auto kernelNames = ir_proto::listKernelNames(coData);
    ...
    auto text = ir_proto::extractTextSection(coData);
    if (!text.valid) {
    ...
    for (auto &kName : kernelNames) {
      totalKernels++;
      auto meta = ir_proto::extractKernelMeta(coData, kName);

      auto raised = ir_proto::raiseToIR(text.bytes, isa, kName, meta);
