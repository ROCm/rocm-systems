...
# be safely invoked with a generic test placeholder.
#
...
RUNTIME_SMOKE_APIS = frozenset(PLACEHOLDER_OVERRIDES)
...
print()
print('  // Per-API curated calls -- one per runtime-smoke YAML API.')
for api in parse_yaml_file(yaml_path):
    name = api['api']
    if name not in RUNTIME_SMOKE_APIS:
        continue
...
# Enable each selected <api>_args event.
...
events = []
runtime_smoke_apis = {
    'hsa_queue_create', 'hsa_queue_destroy', 'hsa_amd_queue_intercept_create',
    'hsa_signal_create', 'hsa_signal_destroy', 'hsa_amd_signal_create',
...
for a in parse_yaml_file(sys.argv[1]):
    if a['api'] in runtime_smoke_apis:
...
# Assert each selected <api>_args event appears at least once.
...
if [ "$MISSING" -gt 0 ]; then
    echo "FAIL: $MISSING / $TOTAL HSA curated smoke _args events missing ($SKIPPED skipped)"
    exit 1
fi
echo "PASS: all $((TOTAL-SKIPPED)) HSA curated smoke _args events fired ($SKIPPED skipped)"
