 ...
 *  - The prev->next.store(..., release) publishes the new node after its value is fully
 * constructed.
...
template <typename T, typename Alloc = std::allocator<T>> class MPSCQueue {
 private:
  struct Node {
 ...
 public:
  ...
  explicit MPSCQueue(const Alloc& alloc) : alloc_(alloc) {
    Node* stub = create_node_stub();
    head_.ptr.store(stub, std::memory_order_relaxed);
    tail_.ptr.store(stub, std::memory_order_relaxed);
    qsize_.store(0, std::memory_order_relaxed);
  ...
  inline void enqueue(const T& value) { emplace(value); }

  // Enqueue by rvalue
  inline void enqueue(T&& value) { emplace(std::move(value)); }
  ...
  inline bool empty() const { return qsize_.load(std::memory_order_acquire) == 0; }
  ...
  inline size_t size() const { return qsize_.load(std::memory_order_acquire); }
 ...
 private:
  ...
  template <typename... Args> Node* allocate_node(Args&&... args) {
    Node* n = NodeAllocTraits::allocate(alloc_, 1);
    try {
