...
// On Linux, returns the versioned name (librocdxg.so.X).
const char* GetDxgLibraryName();

// Returns an unversioned fallback DXG library name (empty if not applicable)
// Used when the versioned library fails to load.
