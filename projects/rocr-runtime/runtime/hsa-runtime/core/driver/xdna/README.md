...
found on Ryzen AI parts. It builds one of two firmware command shapes per dispatch. This document
covers why the second one exists.
...
control code before submission. Both dispatch shapes are unified behind one opaque kernel object,
obtained the ordinary HSA way via `HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_OBJECT` after loading an
hsaco with `hsa_executable_load_agent_code_object`. The application never sees a PDI, an
instruction sequence, or a control code directly; it dispatches a kernel object with kernel
arguments, exactly as it would on any other HSA agent. Responsibility splits as follows:

- **The loader** parses the hsaco's AIE section (`core/inc/amd_aie_section.h`,
  `core/runtime/amd_aie_code.cpp`) once per kernel at load time, decides the payload kind
  (`PdiInsts` or `FullElf`) from the kernel table entry, and -- for `FullElf` -- parses the nested
  ELF (`core/inc/amd_aie_elf.h`) to find its control code, its PDI section, and its argument patch
  sites. The resulting descriptor (`AieKernelDescriptor`) is what the kernel object handle points
  to.
- **The driver** (this directory) reads that descriptor at dispatch time. It never parses an ELF
  or a hsaco; it only knows PDIs, instruction sequences, control code buffers, and patch sites,
  all already resolved by the loader.
...
For `FullElf`, the pristine control code the loader parsed lives in host memory
(`AieKernelDescriptor::ctrl_code`) and is never itself submitted to the device. Each dispatch
copies it into a fresh, runtime-allocated per-dispatch device buffer and patches the PDI address
and argument addresses into that copy, leaving the pristine host copy untouched for the next
dispatch. Allocating and freeing that buffer on every dispatch is a deliberate simplicity choice,
not an oversight: pooling these buffers is a real optimization opportunity, but it is being left
for a follow-up gated on measuring whether allocation cost actually shows up in a real workload,
rather than added speculatively now.

### Packet ABI

There is **no new packet opcode, and no shape distinction visible in the packet at all**. A
`hsa_amd_aie_kernel_dispatch_packet_t` carries a kernel object (`kernel_object_low` /
`kernel_object_high`), exactly like a GPU AQL dispatch packet. The driver resolves that handle to
an `AieKernelDescriptor` and reads `AieKernelDescriptor::kind` to decide which firmware command to
build:

| `kind` | firmware command | what travels in the command |
|---|---|---|
| `PdiInsts` | `ERT_START_CU` | the instruction sequence, plus the PDI resolved via the 32-entry CU cache |
| `FullElf` | `ERT_START_NPU_PREEMPT_ELF` (opcode 22) | a per-dispatch copy of the control code, PDI-patched and argument-patched |
...
- Full-ELF command construction (`ERT_START_NPU_PREEMPT_ELF`) alongside the unchanged
  `ERT_START_CU` path, selected per dispatch from the kernel object's `AieKernelDescriptor::kind`
  rather than from any packet field.
...
- The application-side ELF reader that used to live in this test suite is gone: the loader now
  owns ELF parsing (`core/inc/amd_aie_elf.h`, `core/runtime/amd_aie_code.cpp`), so the tests
  dispatch through the ordinary hsaco-load-and-freeze path and assert on the resulting kernel
  object and on load/dispatch outcomes, not on a test-local parser.
- Coverage includes: parsing and its rejections (`elf_parse_test.cc`), single and batched
  dispatch, a full queue, re-patching one control code with different arguments, both buffers at
  a non-zero offset into their allocation, dispatching more than 32 distinct PDIs (which the PDI
  path cannot do), rejection of an unknown kernel name, and rejection of a hsaco whose kernel
  table declares two entries sharing one blob under the same kernel name.
