...
Lib* Lib::GetLib(ADDR_HANDLE hLib)  ///< [in] handle of ADDR_HANDLE
{
  ...
  if ((pAddrLib != NULL) && (pAddrLib->GetChipFamily() <= ADDR_CHIP_FAMILY_VI)) {
...
ADDR_E_RETURNCODE Lib::ComputeSurfaceInfo(
  ...
  if (returnCode == ADDR_OK) {
    ...
    if (localIn.format != ADDR_FMT_INVALID) {
      ...
      localIn.bpp = GetElemLib()->GetBitsPerPixel(localIn.format, &elemMode, &expandX, &expandY);
      ...
      UINT_32 basePitch = 0;
      GetElemLib()->AdjustSurfaceInfo(elemMode, expandX, expandY, &localIn.bpp, &basePitch,
                                      &localIn.width, &localIn.height);
  ...
  if (returnCode == ADDR_OK) {
    ...
    if (returnCode == ADDR_OK) {
      ...
      if (localIn.format != ADDR_FMT_INVALID) {
        ...
        GetElemLib()->RestoreSurfaceInfo(elemMode, expandX, expandY, &pOut->pixelBits,
                                         &pOut->pixelPitch, &pOut->pixelHeight);

        GetElemLib()->RestoreSurfaceInfo(elemMode, expandX, expandY, &pixelBits,
                                         &pOut->pixelMipChainPitch, &pOut->pixelMipChainHeight);
      ...
      if (localIn.flags.needEquation && (Log2(localIn.numFrags) == 0)) {
        ...
        if ((localIn.flags.allowExtEquation == 0) &&
            ...
            (m_equationTable[pOut->equationIndex].numBitComponents >
...
ADDR_E_RETURNCODE Lib::ComputeSurfaceAddrFromCoord(
  ...
  if ((localIn.bpp < 8) || (localIn.bpp > 128) || ((localIn.bpp % 8) != 0) ||
      (localIn.sample >= localIn.numSamples) || (localIn.slice >= localIn.numSlices) ||
...
ADDR_E_RETURNCODE Lib::ComputeSurfaceCoordFromAddr(
  ...
  if ((pIn->bpp < 8) || (pIn->bpp > 128) || ((pIn->bpp % 8) != 0) || (pIn->bitPosition >= 8)) {
    returnCode = ADDR_INVALIDPARAMS;
  }
...
ADDR_E_RETURNCODE Lib::ComputeFmaskInfo(
  ...
  BOOL_32 valid =
      (IsZOrderSwizzle(pIn->swizzleMode) == TRUE) && ((pIn->numSamples > 0) || (pIn->numFrags > 0));
...
************************************************************************************************************************
*/
ADDR_E_RETURNCODE Lib::ComputePipeBankXor(const ADDR2_COMPUTE_PIPEBANKXOR_INPUT* pIn,
...
************************************************************************************************************************
*/
ADDR_E_RETURNCODE Lib::ComputeSlicePipeBankXor(const ADDR2_COMPUTE_SLICE_PIPEBANKXOR_INPUT* pIn,
  ...
  if ((GetFillSizeFieldsFlags() == TRUE) &&
    ...
    returnCode = ADDR_INVALIDPARAMS;
             ...
             (IsNonPrtXor(pIn->swizzleMode) == FALSE) || (pIn->numSamples > 1)) {
    returnCode = ADDR_NOTSUPPORTED;
  } else if ((pIn->bpe != 0) && (pIn->bpe != 8) && (pIn->bpe != 16) && (pIn->bpe != 32) &&
             (pIn->bpe != 64) && (pIn->bpe != 128)) {
    returnCode = ADDR_INVALIDPARAMS;
  } else {
...
************************************************************************************************************************
*/
ADDR_E_RETURNCODE Lib::ExtractPipeBankXor(UINT_32 pipeBankXor, UINT_32 bankBits, UINT_32 pipeBits,
                                          UINT_32* pBankX, UINT_32* pPipeX) {
  ADDR_E_RETURNCODE returnCode;
...
ADDR_E_RETURNCODE Lib::ComputeSurfaceAddrFromCoordLinear(
  ...
  if (valid) {
    ...
    if (returnCode == ADDR_OK) {
      pOut->addr = (localOut.sliceSize * pIn->slice) + mipInfo[pIn->mipId].offset +
    ...
    if (returnCode == ADDR_OK) {
      ...
      for (; mipId < pIn->numMipLevels; mipId++) {
        ...
        if (mipSize == 0) {
          ...
          if ((mipId == (pIn->numMipLevels - 1)) || (mipOffsetInSlice >= localOut.sliceSize)) {
            valid = FALSE;
          }
        }
      }
      ...
      if (valid) {
        ...
        if ((pOut->slice >= pIn->numSlices) || (pOut->mipId >= pIn->numMipLevels) ||
            (pOut->x >= Max((pIn->unalignedWidth >> pOut->mipId), 1u)) ||
            (pOut->y >= Max((pIn->unalignedHeight >> pOut->mipId), 1u)) ||
            (IsTex3d(pIn->resourceType) &&
             (FALSE == Valid3DMipSliceIdConstraint(pIn->numSlices, pOut->mipId, pOut->slice)))) {
          valid = FALSE;
        }
      }
    } else {
...
************************************************************************************************************************
*/
ADDR_E_RETURNCODE Lib::ComputeBlockDimensionForSurf(UINT_32* pWidth, UINT_32* pHeight,
                                                    UINT_32* pDepth, UINT_32 bpp,
                                                    UINT_32 numSamples,
...
************************************************************************************************************************
*/
VOID Lib::ComputeThinBlockDimension(UINT_32* pWidth, UINT_32* pHeight, UINT_32* pDepth, UINT_32 bpp,
                                    UINT_32 numSamples, AddrResourceType resourceType,
                                    AddrSwizzleMode swizzleMode) const {
  ADDR_ASSERT(IsThin(resourceType, swizzleMode));

  // GFX9/GFX10 use different dimension amplifying logic: say for 128KB block + 1xAA + 1BPE, the
  // dimension of thin swizzle mode will be [256W * 512H] on GFX9 ASICs and [512W * 256H] on GFX10
  // ASICs. Since GFX10 is newer HWL so we make its implementation into base class (in order to save
  // future change on new HWLs)
  ...
  // For "1xAA/4xAA cases" or "2xAA/8xAA + odd log2BlkSize cases", width == height or width == 2 *
  // height; For other cases, height == width or height == 2 * width
...
************************************************************************************************************************
*/
ADDR_E_RETURNCODE Lib::ComputeBlockDimension(UINT_32* pWidth, UINT_32* pHeight, UINT_32* pDepth,
                                             UINT_32 bpp, AddrResourceType resourceType,
                                             AddrSwizzleMode swizzleMode) const {
  ADDR_E_RETURNCODE returnCode = ADDR_OK;
...
************************************************************************************************************************
*/
VOID Lib::ComputeThickBlockDimension(UINT_32* pWidth, UINT_32* pHeight, UINT_32* pDepth,
                                     UINT_32 bpp, AddrResourceType resourceType,
                                     AddrSwizzleMode swizzleMode) const {
...
************************************************************************************************************************
*/
Dim3d Lib::GetMipTailDim(AddrResourceType resourceType, AddrSwizzleMode swizzleMode,
                         UINT_32 blockWidth, UINT_32 blockHeight, UINT_32 blockDepth) const {
...
#if DEBUG
    // GFX9/GFX10 use different dimension shrinking logic for mipmap tail: say for 128KB block +
    // 2BPE, the maximum dimension of mipmap tail level will be [256W * 128H] on GFX9 ASICs and
    // [128W * 256H] on GFX10 ASICs. Since GFX10 is newer HWL so we make its implementation into
    // base class, in order to save future change on new HWLs. And assert log2BlkSize will always be
    // an even value on GFX9, so we never need the logic wrapped by DEBUG...
...
UINT_32 Lib::ComputeSurface2DMicroBlockOffset(
  ...
  if (IsStandardSwizzle(pIn->resourceType, pIn->swizzleMode)) {
    ...
    if (log2ElementBytes == 4) {
      microBlockOffset = (GetBit(pIn->x, 0) << 4) | (GetBit(pIn->y, 0) << 5) |
          (GetBit(pIn->x, 1) << 6) | (GetBit(pIn->y, 1) << 7);
    } else {
      microBlockOffset = GetBits(pIn->x, 0, 3, log2ElementBytes) |
          GetBits(pIn->y, 1, 2, 3 + log2ElementBytes) |
          GetBits(pIn->x, 3, 1, 5 + log2ElementBytes) | GetBits(pIn->y, 3, 1, 6 + log2ElementBytes);
      microBlockOffset = GetBits(microBlockOffset, 0, 4, 0) | (GetBit(pIn->y, 0) << 4) |
          GetBits(microBlockOffset, 4, 3, 5);
    }
  } else if (IsRotateSwizzle(pIn->swizzleMode)) {
    microBlockOffset = GetBits(pIn->y, 0, 3, log2ElementBytes) |
        GetBits(pIn->x, 1, 2, 3 + log2ElementBytes) | GetBits(pIn->x, 3, 1, 5 + log2ElementBytes) |
        GetBits(pIn->y, 3, 1, 6 + log2ElementBytes);
    microBlockOffset = GetBits(microBlockOffset, 0, 4, 0) | (GetBit(pIn->x, 0) << 4) |
        GetBits(microBlockOffset, 4, 3, 5);
    if (log2ElementBytes == 3) {
      microBlockOffset = GetBits(microBlockOffset, 0, 6, 0) | GetBits(pIn->x, 1, 2, 6);
    }
  }
...
UINT_32 Lib::ComputeSurface3DMicroBlockOffset(
  ...
  if (IsStandardSwizzle(pIn->resourceType, pIn->swizzleMode)) {
    ...
    if (log2ElementBytes == 0) {
      ...
      microBlockOffset = (((pIn->x & 1) | ((pIn->y & 1) << 1) | ((pIn->slice & 1) << 2)) << 4);
...
************************************************************************************************************************
*/
UINT_32 Lib::GetPipeXorBits(UINT_32 macroBlockBits) const {
...
************************************************************************************************************************
*/
ADDR_E_RETURNCODE Lib::GetAllowedBlockSet(ADDR2_SWMODE_SET allowedSwModeSet,
...
************************************************************************************************************************
*/
ADDR_E_RETURNCODE Lib::GetAllowedSwSet(ADDR2_SWMODE_SET allowedSwModeSet,
...
************************************************************************************************************************
*/
ADDR_E_RETURNCODE Lib::ComputeBlock256Equation(AddrResourceType rsrcType, AddrSwizzleMode swMode,
...
************************************************************************************************************************
*/
ADDR_E_RETURNCODE Lib::ComputeThinEquation(AddrResourceType rsrcType, AddrSwizzleMode swMode,
                                           UINT_32 elementBytesLog2,
                                           ADDR_EQUATION* pEquation) const {
  ADDR_E_RETURNCODE ret;
...
************************************************************************************************************************
*/
ADDR_E_RETURNCODE Lib::ComputeThickEquation(AddrResourceType rsrcType, AddrSwizzleMode swMode,
                                            UINT_32 elementBytesLog2,
                                            ADDR_EQUATION* pEquation) const {
  ADDR_E_RETURNCODE ret;
...
************************************************************************************************************************
*/
VOID Lib::FilterInvalidEqSwizzleMode(ADDR2_SWMODE_SET& allowedSwModeSet,
                                     AddrResourceType resourceType, UINT_32 elemLog2,
    ...
    if (TestCoord[xIdx] < pIn->width) {
      ...
      for (UINT_32 yIdx = 0; yIdx < sizeof(TestCoord) / sizeof(TestCoord[0]); yIdx++) {
        if (TestCoord[yIdx] < pIn->height) {
          ...
          ADDR_ASSERT(rightEyeOffsetFromBase ==
                      rightEyeOffsetRelative + pOut->pStereoInfo->rightOffset);
        }
      }
    }
  }
