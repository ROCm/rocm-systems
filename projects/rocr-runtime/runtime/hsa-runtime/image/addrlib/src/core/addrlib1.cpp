...
****************************************************************************************************
*/
Lib::Lib(const Client* pClient) : Addr::Lib(pClient) {}
...
ADDR_E_RETURNCODE Lib::ComputeSurfaceInfo(
  ...
  if (returnCode == ADDR_OK) {
    ...
    if (localIn.format != ADDR_FMT_INVALID)  // Set format to INVALID will skip this conversion
    {
      ...
      GetElemLib()->AdjustSurfaceInfo(elemMode, expandX, expandY, &localIn.bpp, &localIn.basePitch,
    ...
    if (returnCode == ADDR_OK) {
      if (UseTileIndex(localIn.tileIndex)) {
        ...
        if (localIn.tileIndex != TileIndexLinearGeneral) {
          ...
          macroModeIndex =
              HwlComputeMacroModeIndex(localIn.tileIndex, localIn.flags, localIn.bpp, numSamples,
                                       localIn.pTileInfo, &localIn.tileMode, &localIn.tileType);
        }

        // If macroModeIndex is not needed, then call HwlSetupTileCfg to get tile info
        if (macroModeIndex == TileIndexNoMacroIndex) {
          returnCode = HwlSetupTileCfg(localIn.bpp, localIn.tileIndex, macroModeIndex,
                                       localIn.pTileInfo, &localIn.tileMode, &localIn.tileType);
        }
    ...
    if (returnCode == ADDR_OK) {
      ...
      if (localIn.format != ADDR_FMT_INVALID) {
        ...
        //
        GetElemLib()->RestoreSurfaceInfo(elemMode, expandX, expandY, &localIn.bpp,
  ...
  if (returnCode == ADDR_OK) {
    ...
    if (UseTileIndex(pIn->tileIndex)) {
      ...
      INT_32 macroModeIndex =
          HwlComputeMacroModeIndex(input.tileIndex, flags, input.bpp, numSamples, input.pTileInfo,
                                   &input.tileMode, &input.tileType);

      // If macroModeIndex is not needed, then call HwlSetupTileCfg to get tile info
      if (macroModeIndex == TileIndexNoMacroIndex) {
        returnCode = HwlSetupTileCfg(input.bpp, input.tileIndex, macroModeIndex, input.pTileInfo,
                                     &input.tileMode, &input.tileType);
      }
      // If macroModeIndex is invalid, then assert this is not macro tiled
      else if (macroModeIndex == TileIndexInvalid) {
...
ADDR_E_RETURNCODE Lib::CombineBankPipeSwizzle(
  ...
  if (returnCode == ADDR_OK) {
    ...
    if (returnCode == ADDR_OK) {
      returnCode = HwlCombineBankPipeSwizzle(pIn->bankSwizzle, pIn->pipeSwizzle, pIn->pTileInfo,
                                             pIn->baseAddr, &pOut->tileSwizzle);
    }
  }
...
****************************************************************************************************
*/
ADDR_E_RETURNCODE Lib::ComputeBaseSwizzle(const ADDR_COMPUTE_BASE_SWIZZLE_INPUT* pIn,
...
ADDR_E_RETURNCODE Lib::ComputeFmaskInfo(
  ...
  if (returnCode == ADDR_OK) {
    ...
    if (UseTileIndex(pIn->tileIndex)) {
      ...
      // Try finding a macroModeIndex
      INT_32 macroModeIndex =
          HwlComputeMacroModeIndex(pIn->tileIndex, flags, HwlComputeFmaskBits(pIn, NULL),
                                   pIn->numSamples, input.pTileInfo, &input.tileMode);

      // If macroModeIndex is not needed, then call HwlSetupTileCfg to get tile info
      if (macroModeIndex == TileIndexNoMacroIndex) {
        returnCode =
            HwlSetupTileCfg(0, input.tileIndex, macroModeIndex, input.pTileInfo, &input.tileMode);
      }
...
ADDR_E_RETURNCODE Lib::ConvertTileInfoToHW(
  ...
  if (returnCode == ADDR_OK) {
    ...
    if (pIn->reverse == FALSE && UseTileIndex(pIn->tileIndex)) {
      ...
      returnCode =
          HwlSetupTileCfg(input.bpp, input.tileIndex, input.macroModeIndex, input.pTileInfo);
...
ADDR_E_RETURNCODE Lib::ConvertTileIndex(
  ...
  if (returnCode == ADDR_OK) {
    returnCode = HwlSetupTileCfg(pIn->bpp, pIn->tileIndex, pIn->macroModeIndex, pOut->pTileInfo,
                                 &pOut->tileMode, &pOut->tileType);
...
ADDR_E_RETURNCODE Lib::GetMacroModeIndex(
  ...
  if (returnCode == ADDR_OK) {
    ...
    pOut->macroModeIndex =
        HwlComputeMacroModeIndex(pIn->tileIndex, pIn->flags, pIn->bpp, pIn->numFrags, &tileInfo);
  }
...
ADDR_E_RETURNCODE Lib::ConvertTileIndex1(
  ...
  if (returnCode == ADDR_OK) {
    ...
    HwlComputeMacroModeIndex(pIn->tileIndex, flags, pIn->bpp, pIn->numSamples, pOut->pTileInfo,
                             &pOut->tileMode, &pOut->tileType);
...
****************************************************************************************************
*/
ADDR_E_RETURNCODE Lib::GetTileIndex(const ADDR_GET_TILEINDEX_INPUT* pIn,  ///< [in] input structure
...
****************************************************************************************************
*/
UINT_32 Lib::Thickness(AddrTileMode tileMode)  ///< [in] tile mode
{
  ...
  if (returnCode == ADDR_OK) {
    ...
    if (returnCode == ADDR_OK) {
      if (pIn->flags.tcCompatible) {
        ...
        const UINT_32 align =
            HwlGetPipes(pIn->pTileInfo) * pIn->pTileInfo->banks * m_pipeInterleaveBytes;
        ...
        if (pIn->numSlices > 1) {
          ...
          pOut->htileBytes =
              pIn->flags.skipTcCompatSizeAlign ? surfBytes : PowTwoAlign(surfBytes, align);
          pOut->sliceInterleaved = ((sliceSize % align) != 0) ? TRUE : FALSE;
        } else {
          pOut->sliceSize =
              pIn->flags.skipTcCompatSizeAlign ? sliceSize : PowTwoAlign(sliceSize, align);
        ...
        pOut->bpp = ComputeHtileInfo(
            pIn->flags, pIn->pitch, pIn->height, pIn->numSlices, pIn->isLinear, isWidth8, isHeight8,
            pIn->pTileInfo, &pOut->pitch, &pOut->height, &pOut->htileBytes, &pOut->macroWidth,
            &pOut->macroHeight, &pOut->sliceSize, &pOut->baseAlign);
      }
    }
  }
...
ADDR_E_RETURNCODE Lib::ComputeCmaskInfo(
  ...
  if (returnCode == ADDR_OK) {
    ...
    if (returnCode == ADDR_OK) {
      returnCode = ComputeCmaskInfo(pIn->flags, pIn->pitch, pIn->height, pIn->numSlices,
                                    pIn->isLinear, pIn->pTileInfo, &pOut->pitch, &pOut->height,
                                    &pOut->cmaskBytes, &pOut->macroWidth, &pOut->macroHeight,
                                    &pOut->sliceSize, &pOut->baseAlign, &pOut->blockMax);
    }
  }
...
ADDR_E_RETURNCODE Lib::ComputeDccInfo(
  ...
  if (ret == ADDR_OK) {
    ...
    if (UseTileIndex(pIn->tileIndex)) {
      ...
      ret = HwlSetupTileCfg(input.bpp, input.tileIndex, input.macroModeIndex, &input.tileInfo,
                            &input.tileMode);
  ...
  if (returnCode == ADDR_OK) {
    ...
    if (returnCode == ADDR_OK) {
      if (pIn->flags.tcCompatible) {
        ...
        pOut->addr = HwlComputeXmaskAddrFromCoord(
            pIn->pitch, pIn->height, pIn->x, pIn->y, pIn->slice, pIn->numSlices, 1, pIn->isLinear,
            isWidth8, isHeight8, pIn->pTileInfo, &pOut->bitPosition);
      }
    }
  }
...
ADDR_E_RETURNCODE Lib::ComputeHtileCoordFromAddr(
  ...
  if (returnCode == ADDR_OK) {
    ...
    if (returnCode == ADDR_OK) {
      HwlComputeXmaskCoordFromAddr(pIn->addr, pIn->bitPosition, pIn->pitch, pIn->height,
                                   pIn->numSlices, 1, pIn->isLinear, isWidth8, isHeight8,
                                   pIn->pTileInfo, &pOut->x, &pOut->y, &pOut->slice);
    }
  }
  ...
  if (returnCode == ADDR_OK) {
    ...
    if (returnCode == ADDR_OK) {
      if (pIn->flags.tcCompatible == TRUE) {
        ...
        pOut->addr = HwlComputeXmaskAddrFromCoord(pIn->pitch, pIn->height, pIn->x, pIn->y,
                                                  pIn->slice, pIn->numSlices, 2, pIn->isLinear,
                                                  FALSE,  // this is cmask, isWidth8 is not needed
                                                  FALSE,  // this is cmask, isHeight8 is not needed
                                                  pIn->pTileInfo, &pOut->bitPosition);
      }
    }
  }
...
ADDR_E_RETURNCODE Lib::ComputeCmaskCoordFromAddr(
  ...
  if (returnCode == ADDR_OK) {
    ...
    if (returnCode == ADDR_OK) {
      HwlComputeXmaskCoordFromAddr(pIn->addr, pIn->bitPosition, pIn->pitch, pIn->height,
                                   pIn->numSlices, 2, pIn->isLinear, FALSE, FALSE, pIn->pTileInfo,
                                   &pOut->x, &pOut->y, &pOut->slice);
    }
  }
...
****************************************************************************************************
*/
VOID Lib::ComputeTileDataWidthAndHeight(UINT_32 bpp,               ///< [in] bits per pixel
...
****************************************************************************************************
*/
UINT_32 Lib::ComputeHtileInfo(ADDR_HTILE_FLAGS flags,    ///< [in] htile flags
  ...
  if (isLinear) {
    ...
    ComputeTileDataWidthAndHeight(bpp, cacheBits, pTileInfo, &macroWidth, &macroHeight);
  }
  ...
  *pHeightOut = PowTwoAlign(heightIn, macroHeight);
  ...
  surfBytes = HwlComputeHtileBytes(*pPitchOut, *pHeightOut, bpp, isLinear, numSlices, &sliceBytes,
                                   baseAlign);
...
****************************************************************************************************
*/
UINT_32 Lib::ComputeCmaskBaseAlign(ADDR_CMASK_FLAGS flags,   ///< [in] Cmask flags
                                   ADDR_TILEINFO* pTileInfo  ///< [in] Tile info
) const {
...
****************************************************************************************************
*/
UINT_64 Lib::ComputeCmaskBytes(UINT_32 pitch,     ///< [in] pitch
...
ADDR_E_RETURNCODE Lib::ComputeCmaskInfo(
  ...
  *pHeightOut = (heightIn + macroHeight - 1) & ~(macroHeight - 1);
  ...
  sliceBytes = ComputeCmaskBytes(*pPitchOut, *pHeightOut, 1);
...
****************************************************************************************************
*/
UINT_32 Lib::ComputeXmaskCoordYFromPipe(UINT_32 pipe,  ///< [in] pipe number
...
VOID Lib::HwlComputeXmaskCoordFromAddr(
  ...
  if (factor == 2)  // CMASK
  {
    ...
    ComputeCmaskInfo(flags, pitch, height, numSlices, isLinear, pTileInfo, &pitchAligned,
                     &heightAligned, &totalBytes, &macroTilePitch, &macroTileHeight);
  } else  // HTILE
  {
    ...
    ComputeHtileInfo(flags, pitch, height, numSlices, isLinear, isWidth8, isHeight8, pTileInfo,
                     &pitchAligned, &heightAligned, &totalBytes, &macroTilePitch, &macroTileHeight);
  }
  ...
  *pSlice = macroZ;
  ...
  microTileCoordY = ComputeXmaskCoordYFromPipe(pipe, *pX / MicroTileWidth);
...
UINT_64 Lib::HwlComputeXmaskAddrFromCoord(
  ...
  if (factor == 2)  // CMASK
  {
    ...
    ComputeCmaskInfo(flags, pitch, height, numSlices, isLinear, pTileInfo, &newPitch, &newHeight,
                     &totalBytes, &macroTileWidth, &macroTileHeight);
  ...
  } else  // HTILE
  {
    ...
    ComputeHtileInfo(flags, pitch, height, numSlices, isLinear, isWidth8, isHeight8, pTileInfo,
                     &newPitch, &newHeight, &totalBytes, &macroTileWidth, &macroTileHeight,
                     &sliceBytes);
  }
  ...
  //
  pipe = ComputePipeFromCoord(x, y, 0, ADDR_TM_2D_TILED_THIN1, 0, FALSE, pTileInfo);
...
****************************************************************************************************
*/
VOID Lib::ComputeSurfaceCoordFromAddrLinear(UINT_64 addr,         ///< [in] address
                                            UINT_32 bitPosition,  ///< [in] bitPosition in a byte
                                            UINT_32 bpp,          ///< [in] bits per pixel
                                            UINT_32 pitch,        ///< [in] pitch
                                            UINT_32 height,       ///< [in] height
...
VOID Lib::ComputeSurfaceCoordFromAddrMicroTiled(
  ...
  //
  HwlComputePixelCoordFromOffset(pixelOffset, bpp, numSamples, tileMode, tileBase, compBits,
                                 &pixelCoordX, &pixelCoordY, &pixelCoordZ, &pixelCoordS,
                                 microTileType, isDepthSampleOrder);
...
****************************************************************************************************
*/
UINT_32 Lib::ComputePipeFromAddr(UINT_64 addr,     ///< [in] address
...
****************************************************************************************************
*/
VOID Lib::AdjustPitchAlignment(ADDR_SURFACE_FLAGS flags,  ///< [in] Surface flags
...
VOID Lib::PadDimensions(
  ...
  HwlPadDimensions(tileMode, bpp, flags, numSamples, pTileInfo, mipLevel, pPitch, pPitchAlign,
                   *pHeight, heightAlign);
}
...
****************************************************************************************************
*/
UINT_32 Lib::HwlPreHandleBaseLvl3xPitch(const ADDR_COMPUTE_SURFACE_INFO_INPUT* pIn,  ///< [in] input
  ...
  //
  if (ElemLib::IsExpand3x(pIn->format) && pIn->mipLevel == 0 &&
...
****************************************************************************************************
*/
BOOL_32 Lib::IsMacroTiled(AddrTileMode tileMode)  ///< [in] tile mode
{
...
****************************************************************************************************
*/
BOOL_32 Lib::IsMacro3dTiled(AddrTileMode tileMode)  ///< [in] tile mode
{
...
****************************************************************************************************
*/
BOOL_32 Lib::IsMicroTiled(AddrTileMode tileMode)  ///< [in] tile mode
{
...
****************************************************************************************************
*/
BOOL_32 Lib::IsLinear(AddrTileMode tileMode)  ///< [in] tile mode
{
...
****************************************************************************************************
*/
BOOL_32 Lib::IsPrtNoRotationTileMode(AddrTileMode tileMode) {
...
****************************************************************************************************
*/
BOOL_32 Lib::IsPrtTileMode(AddrTileMode tileMode) { return ModeFlags[tileMode].isPrt; }
...
****************************************************************************************************
*/
VOID Lib::ComputeMipLevel(ADDR_COMPUTE_SURFACE_INFO_INPUT* pIn  ///< [in,out] Input structure
) const {
...
****************************************************************************************************
*/
BOOL_32 Lib::DegradeTo1D(UINT_32 width,                ///< surface width
...
VOID Lib::OptimizeTileMode(
  ...
  BOOL_32 doOpt = (pInOut->flags.opt4Space == TRUE) || (pInOut->flags.minimizeAlignment == TRUE) ||
      (pInOut->maxBaseAlign != 0);

  BOOL_32 convertToPrt = FALSE;

  // Optimization can only be done on level 0 and samples <= 1
  if ((doOpt == TRUE) && (pInOut->mipLevel == 0) && (IsPrtTileMode(tileMode) == FALSE) &&
    ...
    if (IsMacroTiled(tileMode)) {
      macroTiledOK = HwlGetAlignmentInfoMacroTiled(pInOut, &macroWidthAlign, &macroHeightAlign,
                                                   &macroSizeAlign);
    }

    if (macroTiledOK) {
      if ((pInOut->flags.display == FALSE) && (pInOut->flags.opt4Space == TRUE) &&
          (pInOut->numSamples <= 1)) {
        // Check if linear mode is optimal
        if ((pInOut->height == 1) && (IsLinear(tileMode) == FALSE) &&
            (ElemLib::IsBlockCompressed(pInOut->format) == FALSE) &&
            (pInOut->flags.depth == FALSE) && (pInOut->flags.stencil == FALSE) &&
          ...
          if (DegradeTo1D(width, height, macroWidthAlign, macroHeightAlign)) {
            ...
            if (tileMode != pInOut->tileMode) {
              ...
              macroTiledOK = HwlGetAlignmentInfoMacroTiled(&input, &macroWidthAlign,
                                                           &macroHeightAlign, &macroSizeAlign);

              if (macroTiledOK && DegradeTo1D(width, height, macroWidthAlign, macroHeightAlign)) {
      ...
      if (macroTiledOK) {
        if ((pInOut->flags.minimizeAlignment == TRUE) && (pInOut->numSamples <= 1) &&
            (IsMacroTiled(tileMode) == TRUE)) {
          UINT_32 macroSize =
              PowTwoAlign(width, macroWidthAlign) * PowTwoAlign(height, macroHeightAlign);
          UINT_32 microSize =
              PowTwoAlign(width, MicroTileWidth) * PowTwoAlign(height, MicroTileHeight);
        ...
        if ((pInOut->maxBaseAlign != 0) && (IsMacroTiled(tileMode) == TRUE)) {
...
****************************************************************************************************
*/
AddrTileMode Lib::DegradeLargeThickTile(AddrTileMode tileMode, UINT_32 bpp) const {
  // Override tilemode
...
****************************************************************************************************
*/
UINT_32 Lib::HwlGetPipes(const ADDR_TILEINFO* pTileInfo  ///< [in] Tile info
) const {
...
****************************************************************************************************
*/
ADDR_E_RETURNCODE Lib::ComputePrtInfo(const ADDR_PRT_INFO_INPUT* pIn,
  ...
  UINT_32 bpp = GetElemLib()->GetBitsPerPixel(pIn->format, &elemMode, &expandX, &expandY);
