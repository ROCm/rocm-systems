...
class Lib : public Object {
 public:
  ...
  static ADDR_E_RETURNCODE Create(const ADDR_CREATE_INPUT* pCreateInfo,
                                  ADDR_CREATE_OUTPUT* pCreateOut);
  ...
  UINT_32 GetVersion() { return m_version; }

  /// Returns asic chip family name defined by AddrLib
  ChipFamily GetChipFamily() const { return m_chipFamily; }

  ADDR_E_RETURNCODE Flt32ToDepthPixel(const ELEM_FLT32TODEPTHPIXEL_INPUT* pIn,
                                      ELEM_FLT32TODEPTHPIXEL_OUTPUT* pOut) const;

  ADDR_E_RETURNCODE Flt32ToColorPixel(const ELEM_FLT32TOCOLORPIXEL_INPUT* pIn,
 ...
 protected:
  ...
  ElemLib* GetElemLib() const { return m_pElemLib; }

  /// Returns fillSizeFields flag
  UINT_32 GetFillSizeFieldsFlags() const { return m_configFlags.fillSizeFields; }
