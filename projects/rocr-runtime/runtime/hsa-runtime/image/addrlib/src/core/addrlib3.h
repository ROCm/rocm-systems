...
************************************************************************************************************************
*/
const UINT_32 Gfx12Blk256KBSwModeMask = (1u << ADDR3_256KB_2D) | (1u << ADDR3_256KB_3D);

const UINT_32 Gfx12Blk64KBSwModeMask = (1u << ADDR3_64KB_2D) | (1u << ADDR3_64KB_3D);

const UINT_32 Gfx12Blk4KBSwModeMask = (1u << ADDR3_4KB_2D) | (1u << ADDR3_4KB_3D);
...
// contains the indices which map to 2D arrays SW_PATTERN_NIBBLE[1-4] which contain sections of an
// index equation.
...
class Lib : public Addr::Lib {
 public:
  ...
  // For data surface
  ADDR_E_RETURNCODE ComputeSurfaceInfo(const ADDR3_COMPUTE_SURFACE_INFO_INPUT* pIn,
                                       ADDR3_COMPUTE_SURFACE_INFO_OUTPUT* pOut) const;

  ADDR_E_RETURNCODE GetPossibleSwizzleModes(const ADDR3_GET_POSSIBLE_SWIZZLE_MODE_INPUT* pIn,
  ...
  // Misc
  ADDR_E_RETURNCODE ComputePipeBankXor(const ADDR3_COMPUTE_PIPEBANKXOR_INPUT* pIn,
  ...
  ADDR_E_RETURNCODE ComputeSlicePipeBankXor(const ADDR3_COMPUTE_SLICE_PIPEBANKXOR_INPUT* pIn,
 ...
 protected:
  ...
  void SetEquationTableEntry(Addr3SwizzleMode addrType, UINT_32 msaaLog2, UINT_32 elementLog2,
                             UINT_32 value) {
    m_equationLookupTable[addrType - 1][msaaLog2][elementLog2] = value;
  }

  const UINT_32 GetEquationTableEntry(Addr3SwizzleMode addrType, UINT_32 msaaLog2,
                                      UINT_32 elementLog2) const {
    return m_equationLookupTable[addrType - 1][msaaLog2][elementLog2];
  }

  static BOOL_32 Valid3DMipSliceIdConstraint(UINT_32 numSlices, UINT_32 mipId, UINT_32 slice) {
    return (Max((numSlices >> mipId), 1u) > slice);
  }

  UINT_32 GetBlockSize(Addr3SwizzleMode swizzleMode, BOOL_32 forPitch = FALSE) const;

  UINT_32 GetBlockSizeLog2(Addr3SwizzleMode swizzleMode, BOOL_32 forPitch = FALSE) const;
  ...
  virtual BOOL_32 HwlInitGlobalParams(const ADDR_CREATE_INPUT* pCreateIn) {
    ...
    // Although GFX12 addressing should be consistent regardless of the configuration, we still need
    // to call some initialization for member variables.
    return TRUE;
  }

  virtual ChipFamily HwlConvertChipFamily(UINT_32 chipFamily, UINT_32 chipRevision);

  virtual UINT_32 HwlComputeMaxMetaBaseAlignments() const { return 0; }

  virtual ADDR_E_RETURNCODE HwlComputeSurfaceInfo(const ADDR3_COMPUTE_SURFACE_INFO_INPUT* pIn,
                                                  ADDR3_COMPUTE_SURFACE_INFO_OUTPUT* pOut) const {
    ADDR_NOT_IMPLEMENTED();
    return ADDR_NOTSUPPORTED;
  }

  virtual ADDR_E_RETURNCODE HwlComputePipeBankXor(const ADDR3_COMPUTE_PIPEBANKXOR_INPUT* pIn,
                                                  ADDR3_COMPUTE_PIPEBANKXOR_OUTPUT* pOut) const {
    ADDR_NOT_IMPLEMENTED();
    return ADDR_NOTSUPPORTED;
  }

  VOID ComputeBlockDimensionForSurf(ADDR_EXTENT3D* pExtent, UINT_32 bpp, UINT_32 numSamples,
                                    Addr3SwizzleMode swizzleMode) const;

  ADDR_EXTENT3D GetMipTailDim(Addr3SwizzleMode swizzleMode, const ADDR_EXTENT3D& blockDims) const;
  ...
  ADDR_E_RETURNCODE ApplyCustomizedPitchHeight(const ADDR3_COMPUTE_SURFACE_INFO_INPUT* pIn,
                                               ADDR3_COMPUTE_SURFACE_INFO_OUTPUT* pOut) const;
