...
************************************************************************************************************************
*/
Lib::Lib() : Addr::Lib(), m_pipesLog2(0), m_pipeInterleaveLog2(0), m_numEquations(0) { Init(); }
...
Lib::Lib(const Client* pClient)
    : Addr::Lib(pClient), m_pipesLog2(0), m_pipeInterleaveLog2(0), m_numEquations(0) {
  Init();
}
...
void Lib::Init() {
  ...
  for (UINT_32 swizzleModeIdx = ADDR3_LINEAR + 1; swizzleModeIdx < ADDR3_MAX_TYPE;
       swizzleModeIdx++) {
    for (UINT_32 msaaRateIdx = 0; msaaRateIdx < MaxMsaaRateLog2; msaaRateIdx++) {
      for (UINT_32 log2BytesIdx = 0; log2BytesIdx < MaxElementBytesLog2; log2BytesIdx++) {
        SetEquationTableEntry(static_cast<Addr3SwizzleMode>(swizzleModeIdx), msaaRateIdx,
                              log2BytesIdx, ADDR_INVALID_EQUATION_INDEX);
      }
    }
  }
}
...
************************************************************************************************************************
*/
UINT_32 Lib::GetBlockSize(Addr3SwizzleMode swizzleMode, BOOL_32 forPitch) const {
...
************************************************************************************************************************
*/
UINT_32 Lib::GetBlockSizeLog2(Addr3SwizzleMode swizzleMode, BOOL_32 forPitch) const {
  UINT_32 blockSize = 0;
...
ADDR_E_RETURNCODE Lib::GetPossibleSwizzleModes(
  ...
  if (returnCode == ADDR_OK) {
    ...
    // The organization of elements in the hierarchical surface is the same as any other surface,
    // and it can support any 2D swizzle mode (SW_256_2D, SW_4KB_2D, SW_64KB_2D, or SW_256KB_2D).
    // The swizzle mode can be selected orthogonally to the underlying z or stencil surface.
    ...
    // Block-compressed images need to be either using 2D or linear swizzle modes.
      ...
      // An eventual determination would be based on pal setting of height_watermark and
      // depth_watermark. However, we just adopt the simpler logic currently. For 3D images w/
      // view3dAs2dArray = 0, SW_3D is preferred. For 3D images w/ view3dAs2dArray = 1, it should go
      // to 2D path above. Enable linear since client may force linear tiling for 3D texture that
      // does not set view3dAs2dArray.
      pOut->validModes.swLinear = 1;
...
************************************************************************************************************************
*/
VOID Lib::ComputeBlockDimensionForSurf(ADDR_EXTENT3D* pExtent, UINT_32 bpp, UINT_32 numSamples,
  ...
  if (IsLinear(swizzleMode)) {
    ...
    const UINT_32 log2Width = (log2BlkSize >> 1) - (log2EleBytes >> 1) - (log2Samples >> 1) -
        (log2EleBytes & log2Samples & 1);
    const UINT_32 log2Height = (log2BlkSize >> 1) - (log2EleBytes >> 1) - (log2Samples >> 1) -
        ((log2EleBytes | log2Samples) & 1);
...
************************************************************************************************************************
*/
ADDR_EXTENT3D Lib::GetMipTailDim(Addr3SwizzleMode swizzleMode,
...
ADDR_E_RETURNCODE Lib::ComputeSurfaceAddrFromCoord(
  ...
  if ((localIn.bpp < 8) || (localIn.bpp > 128) || ((localIn.bpp % 8) != 0) ||
      (localIn.sample >= localIn.numSamples) || (localIn.slice >= localIn.unAlignedDims.depth) ||
      (localIn.mipId >= localIn.numMipLevels) ||
      (IsTex3d(localIn.resourceType) &&
       (Valid3DMipSliceIdConstraint(localIn.unAlignedDims.depth, localIn.mipId, localIn.slice) ==
        FALSE))) {
    returnCode = ADDR_INVALIDPARAMS;
  }
...
************************************************************************************************************************
*/
ADDR_E_RETURNCODE Lib::ComputePipeBankXor(const ADDR3_COMPUTE_PIPEBANKXOR_INPUT* pIn,
...
************************************************************************************************************************
*/
ADDR_E_RETURNCODE Lib::ComputeSlicePipeBankXor(const ADDR3_COMPUTE_SLICE_PIPEBANKXOR_INPUT* pIn,
  ...
  if ((pIn->bpe != 0) && (pIn->bpe != 8) && (pIn->bpe != 16) && (pIn->bpe != 32) &&
...
*
*   @brief
*       Determines if the calculations for this surface should use minimal HW values or
...
************************************************************************************************************************
*/
BOOL_32 Lib::UseCustomHeight(const ADDR3_COMPUTE_SURFACE_INFO_INPUT* pIn) const {
  return ((pIn->numMipLevels <= 1) && IsLinear(pIn->swizzleMode) && (pIn->sliceAlign > 0));
}
...
************************************************************************************************************************
*/
BOOL_32 Lib::UseCustomPitch(const ADDR3_COMPUTE_SURFACE_INFO_INPUT* pIn) const {
  return ((pIn->numMipLevels <= 1) && IsLinear(pIn->swizzleMode) && (pIn->pitchInElement > 0));
}
...
*
*   @brief
*       Determines if the calculations for this surface can omit extra trailing padding for linear
*surfaces.
*
*   @return
...
************************************************************************************************************************
*/
BOOL_32 Lib::CanTrimLinearPadding(const ADDR3_COMPUTE_SURFACE_INFO_INPUT* pIn) const {
  return ((IsTex3d(pIn->resourceType) == FALSE) && (pIn->numSlices <= 1) &&
