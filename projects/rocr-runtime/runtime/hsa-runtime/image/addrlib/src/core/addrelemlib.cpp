...
****************************************************************************************************
*/
ElemLib::ElemLib(Lib* pAddrLib)  ///< [in] Parent addrlib instance pointer
    : Object(pAddrLib->GetClient()), m_pAddrLib(pAddrLib) {
...
****************************************************************************************************
*/
ElemLib* ElemLib::Create(const Lib* pAddrLib)  ///< [in] Pointer of parent AddrLib instance
{
 ...
 ****************************************************************************************************
 */
VOID ElemLib::Flt32sToInt32s(ADDR_FLT_32 value,      ///< [in] ADDR_FLT_32 value
          ...
          *pResult = (1U << bits) - 1;
          ...
          if ((value.i | 0x87FFFFFF) == 0xFFFFFFFF) {
            *pResult = 0;  // NaN, so force to 0
            ...
            UINT_64 temp = mant24 - (mant24 >> bits) - static_cast<INT_32>((mant24 & mask) > half);
...
****************************************************************************************************
*/
VOID ElemLib::Flt32ToDepthPixel(AddrDepthFormat format,      ///< [in] Depth format
...
****************************************************************************************************
*/
VOID ElemLib::Flt32ToColorPixel(AddrColorFormat format,      ///< [in] Color format
  ...
  for (i = 0; i < 4; i++) {
    ...
    // Mark if there are any floating point components
    if ((pixelInfo.numType[i] == ADDR_U4FLOATC) || (pixelInfo.numType[i] >= ADDR_S8FLOAT)) {
      properties.floatComp = TRUE;
    }
  }
  ...
  Int32sToPixel(4, values, &pixelInfo.compBit[0], &pixelInfo.compStart[0], properties, resultBits,
                pPixel);
}
...
****************************************************************************************************
*/
VOID ElemLib::GetCompType(AddrColorFormat format,     ///< [in] surface format
...
****************************************************************************************************
*/
VOID ElemLib::GetCompSwap(AddrSurfaceSwap swap,    ///< [in] swap mode
...
****************************************************************************************************
*/
VOID ElemLib::SwapComps(UINT_32 c0,              ///< [in] component index 0
  ...
  for (UINT_32 c = 0; c < 4; c++) {
    if (m_fp16ExportNorm) {
      if (((formatInfo.compBit[c] > 11) || (formatInfo.numType[c] > ADDR_USCALED)) &&
          (formatInfo.numType[c] != ADDR_U4FLOATC) && (formatInfo.numType[c] != ADDR_S5FLOAT) &&
          (formatInfo.numType[c] != ADDR_S5FLOATM) && (formatInfo.numType[c] != ADDR_U5FLOAT) &&
...
****************************************************************************************************
*/
VOID ElemLib::AdjustSurfaceInfo(ElemMode elemMode,    ///< [in] element mode
...
****************************************************************************************************
*/
VOID ElemLib::RestoreSurfaceInfo(ElemMode elemMode,  ///< [in] element mode
                                 UINT_32 expandX,    ///< [in] decompression expansion factor in X
...
****************************************************************************************************
*/
UINT_32 ElemLib::GetBitsPerPixel(AddrFormat format,    ///< [in] surface format code
...
****************************************************************************************************
*/
VOID ElemLib::GetCompBits(UINT_32 c0,              ///< [in] bits of component 0
...
****************************************************************************************************
*/
VOID ElemLib::SetClearComps(ADDR_FLT_32 comps[4],  ///< [in,out] components
                            ...
                            BOOL_32 float32)     ///< [in] TRUE if float32 component (BLEND_FLOAT32)
{
  ...
  // Otherwise use the (modified) clear value
    ...
    for (i = 0; i < 4; i++) {  // If full precision, use clear value unchanged
      if (float32) {
        // Do nothing
        // comps[i] = comps[i];
      }
...
****************************************************************************************************
*/
BOOL_32 ElemLib::IsBlockCompressed(AddrFormat format)  ///< [in] Format
{
...
****************************************************************************************************
*/
BOOL_32 ElemLib::IsCompressed(AddrFormat format)  ///< [in] Format
{
...
****************************************************************************************************
*/
BOOL_32 ElemLib::IsExpand3x(AddrFormat format)  ///< [in] Format
{
  BOOL_32 is3x = FALSE;
...
****************************************************************************************************
*/
BOOL_32 ElemLib::IsMacroPixelPacked(AddrFormat format)  ///< [in] Format
{
