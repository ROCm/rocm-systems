...
************************************************************************************************************************
*/
Addr::Lib* Gfx12HwlInit(const Client* pClient) { return V3::Gfx12Lib::CreateObj(pClient); }
...
const ADDR_EXTENT3D Gfx12Lib::Block4K_Log2_3d[] = {
    {4, 4, 4}, {3, 4, 4}, {3, 4, 3}, {3, 3, 3}, {2, 3, 3}};
const ADDR_EXTENT3D Gfx12Lib::Block64K_Log2_3d[] = {
    {6, 5, 5}, {5, 5, 5}, {5, 5, 4}, {5, 4, 4}, {4, 4, 4}};
const ADDR_EXTENT3D Gfx12Lib::Block256K_Log2_3d[] = {
    {6, 6, 6}, {5, 6, 6}, {5, 6, 5}, {5, 5, 5}, {4, 5, 5}};
...
************************************************************************************************************************
*/
Gfx12Lib::Gfx12Lib(const Client* pClient) : Lib(pClient), m_numSwizzleBits(0) {
...
VOID Gfx12Lib::InitEquationTable() {
  ...
  for (UINT_32 swModeIdx = 0; swModeIdx < ADDR3_MAX_TYPE; swModeIdx++) {
    ...
    if (IsLinear(swMode)) {
      // Skip linear equation (data table is not useful for 2D/3D images-- only contains
      // x-coordinate bits)
      continue;
    }
...
************************************************************************************************************************
*/
ADDR_EXTENT3D Gfx12Lib::GetBlockPixelDimensions(Addr3SwizzleMode swizzleMode,
...
************************************************************************************************************************
*/
VOID Gfx12Lib::GetMipOrigin(const ADDR3_COMPUTE_SURFACE_INFO_INPUT* pIn,  ///< [in] input structure
  ...
  for (UINT_32 i = pOut->firstMipIdInTail; i < pIn->numMipLevels; i++) {
    ...
    if (IsLinear(pIn->swizzleMode)) {
      ...
      UINT_32 mipX = ((mipOffset >> 9) & 1) | ((mipOffset >> 10) & 2) | ((mipOffset >> 11) & 4) |
          ((mipOffset >> 12) & 8) | ((mipOffset >> 13) & 16) | ((mipOffset >> 14) & 32);
      UINT_32 mipY = ((mipOffset >> 8) & 1) | ((mipOffset >> 9) & 2) | ((mipOffset >> 10) & 4) |
          ((mipOffset >> 11) & 8) | ((mipOffset >> 12) & 16) | ((mipOffset >> 13) & 32);
...
************************************************************************************************************************
*/
VOID Gfx12Lib::GetMipOffset(const ADDR3_COMPUTE_SURFACE_INFO_INPUT* pIn,  ///< [in] input structure
                            ADDR3_COMPUTE_SURFACE_INFO_OUTPUT* pOut  ///< [out] output structure
) const {
  const UINT_32 bytesPerPixel = pIn->bpp >> 3;
  const UINT_32 log2Bpp = Log2(bytesPerPixel);
  ...
  for (UINT_32 mipIdx = 0; mipIdx < pIn->numMipLevels; mipIdx++) {
    ...
    if (SupportsMipTail(pIn->swizzleMode) &&
      ...
      const UINT_32 pitch = UseCustomPitch(pIn) ? pOut->pitch
      ...
      // sizes.  We aligned our pitch and height to those sizes, which means we need to multiply the
      // various factors back together to get back to the slice size.
      const UINT_64 sliceSize =
          static_cast<UINT_64>(pitch) * height * pIn->numSamples * (pIn->bpp >> 3);
      ...
      if (pOut->pMipInfo != NULL) {
        ...
        // The slice size of a linear image was calculated above as if the "pitch" is 256 byte
        // aligned. However, the rendering pitch is aligned to 128 bytes, and that is what needs to
        // be reported to our clients.
        if (IsLinear(pIn->swizzleMode)) {
  ...
  if (pOut->pMipInfo != NULL) {
    if (IsLinear(pIn->swizzleMode)) {
      ...
      // 2. The organization of linear 3D mipmap resource is same as GFX11, we should use mip slice
      // size to caculate mip offset.
      ...
      if (firstMipInTail != pIn->numMipLevels) {
        ...
        offset = blockSize * PowTwoAlign(mipExtentFirstInTail.depth, pOut->blockExtent.depth) /
            pOut->blockExtent.depth;
...
ADDR_E_RETURNCODE Gfx12Lib::HwlComputeSurfaceInfo(
  ...
  ComputeBlockDimensionForSurf(&pOut->blockExtent, pIn->bpp, pIn->numSamples, pIn->swizzleMode);
  ...
  if (returnCode == ADDR_OK) {
    ...
    // - with 3D images, one block will contain multiple slices, so that needs to be taken into
    // account.
    // - with linear images that have only once slice, we may trim and use the pitch alignment for
    // size.
...
************************************************************************************************************************
*/
ADDR_EXTENT3D Gfx12Lib::GetBaseMipExtents(const ADDR3_COMPUTE_SURFACE_INFO_INPUT* pIn) const {
...
************************************************************************************************************************
*/
UINT_32 Gfx12Lib::GetMaxNumMipsInTail(Addr3SwizzleMode swizzleMode,
...
ADDR_E_RETURNCODE Gfx12Lib::HwlComputeSurfaceAddrFromCoordTiled(
  ...
  if (ret == ADDR_OK) {
    ...
    const UINT_32 eqIndex =
        GetEquationTableEntry(pIn->swizzleMode, Log2(localIn.numSamples), elemLog2);

    if (eqIndex != ADDR_INVALID_EQUATION_INDEX) {
      const BOOL_32 inTail =
          ((mipInfo[pIn->mipId].mipTailOffset != 0) && (blkSizeLog2 != Log2Size256));
      const BOOL_32 is3dNoMsaa =
          ((IsTex3d(pIn->resourceType) == TRUE) && (localIn.numSamples == 1));
      const UINT_64 sliceSize =
          is3dNoMsaa ? (localOut.sliceSize * localOut.blockExtent.depth) : localOut.sliceSize;
      ...
      const UINT_32 blkOffset =
          ComputeOffsetFromEquation(&m_equationTable[eqIndex], x << elemLog2, y, z, pIn->sample);
      pOut->addr = sliceSize * sliceId + mipInfo[pIn->mipId].macroBlockOffset +
          (blkIdx << blkSizeLog2) + blkOffset;
    } else {
...
ADDR_E_RETURNCODE Gfx12Lib::HwlComputePipeBankXor(
  ...
  if ((m_numSwizzleBits !=
       0) &&  // does this configuration support swizzling
              //         base address XOR in GFX12 will be applied to all blk_size = 4KB, 64KB, or
              //         256KB swizzle modes, Note that Linear and 256B are excluded.
...
************************************************************************************************************************
*/
UINT_32 Gfx12Lib::ComputeOffsetFromEquation(const ADDR_EQUATION* pEq,  ///< Equation
...
************************************************************************************************************************
*/
BOOL_32 Gfx12Lib::HwlInitGlobalParams(const ADDR_CREATE_INPUT* pCreateIn)  ///< [in] create input
{
  BOOL_32 valid = TRUE;
  ...
  if (((pIn->format < ADDR_FMT_ASTC_4x4) || (pIn->format > ADDR_FMT_ETC2_128BPP)) &&
    ...
    if (returnCode == ADDR_OK) {
      ...
      if (inTail) {
        ...
        // threshold
        pOut->unAlignedDims.width =
            Min(requestMipWidth << pOut->mipId, infoOut.blockExtent.width / 2);

        // - (mip0) height = requestMipHeight << mipId, the value can't exceed mip tail dimension
        // threshold
        pOut->unAlignedDims.height =
            Min(requestMipHeight << pOut->mipId, infoOut.blockExtent.height);
      }
  ...
  if ((IsLinear(pIn->swizzleMode) == FALSE) && (IsBlock256b(pIn->swizzleMode) == FALSE)) {
    if (pIn->bpe == 0) {
      ...
      const ADDR_SW_PATINFO* pPatInfo =
          GetSwizzlePatternInfo(pIn->swizzleMode, Log2(pIn->bpe >> 3), 1);

      if (pPatInfo != NULL) {
        const UINT_32 elemLog2 = Log2(pIn->bpe >> 3);
        const UINT_32 eqIndex =
            GetEquationTableEntry(pIn->swizzleMode, Log2(pIn->numSamples), elemLog2);

        const UINT_32 pipeBankXorOffset =
            ComputeOffsetFromEquation(&m_equationTable[eqIndex], 0, 0, pIn->slice, 0);
...
************************************************************************************************************************
*/
void Gfx12Lib::SanityCheckSurfSize(const ADDR3_COMPUTE_SURFACE_INFO_INPUT* pIn,
  ...
  // implicit assumptions about the surface dimensions being less than "MaxImageDim"; otherwise, it
  // can't calculate "firstMipInTail" accurately and the below assertion will trip incorrectly.
  //
  // Surfaces destined for use only on the SDMA engine can exceed the gfx-engine-imposed limitations
  // of the "maximum" image dimensions.
  if ((pIn->width <= MaxImageDim) && (pIn->height <= MaxImageDim) &&
      (pIn->numMipLevels <= MaxMipLevels) && (UseCustomPitch(pIn) == FALSE) &&
      (UseCustomHeight(pIn) == FALSE) &&
      // HiZS surfaces have a reduced image size (i.e,. each pixel represents an 8x8 region of the
      // parent image, at least for single samples) but they still have the same number of mip
      // levels as the parent image.  This disconnect produces false assertions below as the image
      // size doesn't apparently support the specified number of mip levels.
      ((pIn->flags.hiZHiS == 0) || (pIn->numMipLevels == 1)) && !(pIn->flags.view3dAs2dArray)) {
    ...
    for (INT_32 mipIdx = MaxMipLevels - 1; mipIdx >= 0; mipIdx--) {
      ...
      if ((mipExtents.width <= tailMaxDim.width) && (mipExtents.height <= tailMaxDim.height) &&
          ((static_cast<INT_32>(pIn->numMipLevels) - mipIdx) <
           static_cast<INT_32>(maxMipsInTail))) {
        firstMipInTail = mipIdx;
      }
    }
    ...
    for (INT_32 mipIdx = firstMipInTail - 1; mipIdx >= -1; mipIdx--) {
      ...
      if (mipIdx >= 0) {
        lastMipSize = 4 * lastMipSize - ((mipBlockWidth & 1) ? mipBlockHeight : 0) -
