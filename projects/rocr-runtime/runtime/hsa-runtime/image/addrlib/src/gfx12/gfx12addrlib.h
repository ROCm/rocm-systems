...
class Gfx12Lib : public Lib {
 ...
 protected:
  ...
  UINT_32 GetMaxNumMipsInTail(Addr3SwizzleMode swizzleMode, UINT_32 blockSizeLog2) const;

  BOOL_32 IsInMipTail(const ADDR_EXTENT3D& mipTailDim, const ADDR_EXTENT3D& mipDims,
                      UINT_32 maxNumMipsInTail, UINT_32 numMipsToTheEnd) const {
    BOOL_32 inTail =
        ((mipDims.width <= mipTailDim.width) && (mipDims.height <= mipTailDim.height) &&
 ...
 private:
  ...
  void SanityCheckSurfSize(const ADDR3_COMPUTE_SURFACE_INFO_INPUT* pIn,
  ...
  VOID GetSwizzlePatternFromPatternInfo(const ADDR_SW_PATINFO* pPatInfo,
    ...
    memcpy(pSwizzle, GFX12_SW_PATTERN_NIBBLE1[pPatInfo->nibble1Idx],
           sizeof(GFX12_SW_PATTERN_NIBBLE1[pPatInfo->nibble1Idx]));

    memcpy(&pSwizzle[8], GFX12_SW_PATTERN_NIBBLE2[pPatInfo->nibble2Idx],
           sizeof(GFX12_SW_PATTERN_NIBBLE2[pPatInfo->nibble2Idx]));

    memcpy(&pSwizzle[12], GFX12_SW_PATTERN_NIBBLE3[pPatInfo->nibble3Idx],
           sizeof(GFX12_SW_PATTERN_NIBBLE3[pPatInfo->nibble3Idx]));

    memcpy(&pSwizzle[16], GFX12_SW_PATTERN_NIBBLE4[pPatInfo->nibble4Idx],
           sizeof(GFX12_SW_PATTERN_NIBBLE4[pPatInfo->nibble4Idx]));
  }

  VOID ConvertSwizzlePatternToEquation(UINT_32 elemLog2, Addr3SwizzleMode swMode,
                                       const ADDR_SW_PATINFO* pPatInfo,
                                       ADDR_EQUATION* pEquation) const;

  ADDR_EXTENT3D GetBaseMipExtents(const ADDR3_COMPUTE_SURFACE_INFO_INPUT* pIn) const;

  ADDR_EXTENT3D GetBlockPixelDimensions(Addr3SwizzleMode swizzleMode,
  ...
  static ADDR_EXTENT3D GetMipExtent(const ADDR_EXTENT3D& mip0, UINT_32 mipId) {
    return {ShiftCeil(Max(mip0.width, 1u), mipId), ShiftCeil(Max(mip0.height, 1u), mipId),
            ShiftCeil(Max(mip0.depth, 1u), mipId)};
  }

  // # See 6.3 in //gfxip/gfx10/doc/architecture/ImageAddressing/gfx10_image_addressing.docx
  //  miptail is applied to only larger block size (4kb, 64kb, 256kb), so there is no miptail in
  //  linear and 256b_2d addressing since they are both 256b block.
  BOOL_32 SupportsMipTail(Addr3SwizzleMode swizzleMode) const {
    return GetBlockSize(swizzleMode) > 256u;
  }

  UINT_32 ComputeOffsetFromEquation(const ADDR_EQUATION* pEq, UINT_32 x, UINT_32 y, UINT_32 z,
                                    UINT_32 s) const;

  const ADDR_SW_PATINFO* GetSwizzlePatternInfo(Addr3SwizzleMode swizzleMode, UINT_32 log2Elem,
                                               UINT_32 numFrag) const;

  VOID GetMipOffset(const ADDR3_COMPUTE_SURFACE_INFO_INPUT* pIn,
                    ADDR3_COMPUTE_SURFACE_INFO_OUTPUT* pOut) const;

  VOID GetMipOrigin(const ADDR3_COMPUTE_SURFACE_INFO_INPUT* pIn,
