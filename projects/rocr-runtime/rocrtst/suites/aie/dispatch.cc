...
// The firmware's chain slot carries at most 26 argument dwords (MAX_NPU_ARGS_SIZE in the driver),
// and the fill path a PDI-path command takes does not check it: it copies whatever the command
// declares into that fixed-size structure. Such a command declares 6 + 2 * num_kernargs dwords, so
// 11 arguments is the first count that overruns it.
//
// The runtime refuses it up front rather than at the point of chaining. Whether a packet is
// chained depends on how many others share its batch, so the alternative is a packet that works
// alone and corrupts firmware state when batched.
//
// Every argument below is a real allocation, so what is being tested is the argument budget and
// not a failure to resolve one of them -- the status code tells the two apart.
TEST_F(DispatchTest, PdiTooManyKernargsRejected) {
  constexpr std::uint32_t too_many_kernargs = 11;
  ...
  ASSERT_EQ(output.allocate(data_pool, aie_vector_scalar_kernel::element_bytes),
            HSA_STATUS_SUCCESS);
  // Pointers followed by sizes, so the buffer has to hold two entries per argument -- otherwise
  // the packet is refused for overrunning its kernarg buffer instead.
  ASSERT_EQ(kernargs.allocate(kernarg_pool, 2 * too_many_kernargs * sizeof(std::uint64_t)),
            HSA_STATUS_SUCCESS);

  auto* args = kernargs.as<std::uint64_t>();
  for (std::uint32_t i = 0; i < too_many_kernargs; ++i) {
    // Alternating real buffers: every one of these resolves to a registered BO.
    args[i] = reinterpret_cast<std::uint64_t>((i % 2 == 0) ? input.get() : output.get());
    args[too_many_kernargs + i] = aie_vector_scalar_kernel::element_bytes;
  }
  ...
  auto pkt = make_aie_packet(signal);
  pkt.pdi_addr = add.pdi.get();
  pkt.insts_addr_low = reinterpret_cast<std::uintptr_t>(add.insts.get()) & 0xFFFFFFFF;
  pkt.insts_addr_high = reinterpret_cast<std::uintptr_t>(add.insts.get()) >> 32;
  pkt.insts_size = static_cast<std::uint32_t>(add.insts_size);
  pkt.num_kernargs = too_many_kernargs;
  pkt.kernarg_address = args;
  const auto wr_idx = enqueue_aie_packet(queue, pkt);
  hsa_signal_store_screlease(queue->doorbell_signal, wr_idx);

  ExpectRejected(err, queue, signal, 1);
  // Specifically the argument budget: a kernarg that failed to resolve reports a different status.
  EXPECT_EQ(err.status, HSA_STATUS_ERROR_INVALID_ARGUMENT);
