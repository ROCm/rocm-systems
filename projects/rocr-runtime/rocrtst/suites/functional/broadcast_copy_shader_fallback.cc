...
class BroadcastCopyShaderFallback : public ::testing::Test {
 protected:
  ...
  // - Large buffers (>=4KB) use SDMA on capable HW or shader on older HW
};
...
TEST_F(BroadcastCopyShaderFallback, SmallCopyAutoFallback) {
  ...
  std::cout << "  Note: Copies <4KB automatically use shader path on capable HW hardware" << std::endl;
  std::cout << "  This provides optimal performance by avoiding SDMA overhead" << std::endl;

  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";

  // Use small size to trigger shader fallback even on capable HW
...
TEST_F(BroadcastCopyShaderFallback, AutoFallback_NonMulticastHW) {
  ...
  std::cout << "  Runtime will select best path: SDMA (capable HW,size>=4KB) or Shader (fallback)"
            << std::endl;
...
TEST_F(BroadcastCopyShaderFallback, SmallVsLarge_PathPerformance) {
  ...
  // Test large size (SDMA on capable HW, shader on older HW)
  ...
  std::cout << "\n  Testing LARGE copies (" << LARGE_SIZE
            << " bytes, SDMA on capable HW or shader):" << std::endl;
