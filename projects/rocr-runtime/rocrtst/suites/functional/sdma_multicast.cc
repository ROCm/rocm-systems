...
SdmaMulticastTest::SdmaMulticastTest()
  ...
  set_description(
      ...
      "Validates decision tree, chunking, and data integrity.");
}
...
uint32_t SdmaMulticastTest::GetGfxMajorVersion() {
  ...
  // Parse gfxXXXX from name (e.g., "amdgcn-amd-amdhsa--gfx942")
  const char* gfx_str = strstr(isa_name, "gfx");
  if (gfx_str) {
    uint32_t version = 0;
    if (sscanf(gfx_str, "gfx%u", &version) == 1) {
      return version / 100;  // gfx942 -> 9, gfx1200 -> 12
    }
  }
  return 0;
}
...
bool SdmaMulticastTest::IsMulticastSupported() {
  // Query multicast capability from runtime instead of hardcoding ISA version
  uint32_t max_dests = 0;
  hsa_status_t status = hsa_amd_memory_broadcast_capability(gpu_agent_, &max_dests);
  return (status == HSA_STATUS_SUCCESS && max_dests > 1);
}
