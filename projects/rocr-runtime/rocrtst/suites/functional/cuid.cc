...
void CuidTest::ValidateCpuCuidTest() {
  // Find all CPU agents
  ...
  for(auto cpu : cpus) {
    uint8_t cpu_cuid[16];
    ASSERT_SUCCESS(hsa_agent_get_info(cpu, (hsa_agent_info_t)HSA_AMD_AGENT_INFO_CUID, &cpu_cuid));

    // Verify that CUID is not all zeroes
    bool all_zeros = true;
    for (int i = 0; i < 16; i++) {
      if (cpu_cuid[i] != 0) {
        all_zeros = false;
    ...
    EXPECT_FALSE(all_zeros) << "CPU CUID should not be all zeros";
    // Print CUID for debugging
    printf(
        "[Debug] CPU CUID: %02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%02x%02x\n",
        cpu_cuid[0], cpu_cuid[1], cpu_cuid[2], cpu_cuid[3], cpu_cuid[4], cpu_cuid[5],
        cpu_cuid[6], cpu_cuid[7], cpu_cuid[8], cpu_cuid[9], cpu_cuid[10], cpu_cuid[11],
        cpu_cuid[12], cpu_cuid[13], cpu_cuid[14], cpu_cuid[15]);
  }
}