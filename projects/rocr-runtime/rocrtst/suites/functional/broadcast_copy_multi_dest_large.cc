...
// Broadcast copy tests: large multi-destination (100-1024)
...
class BroadcastCopyMultiDestLarge : public ::testing::Test {
 protected:
  void SetUp() override {}
};
...
TEST_F(BroadcastCopyMultiDestLarge, Broadcast_100_Destinations) {
  HsaTestContext ctx;
...
TEST_F(BroadcastCopyMultiDestLarge, Broadcast_256_Destinations) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
...
TEST_F(BroadcastCopyMultiDestLarge, Broadcast_1023_Destinations) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
...
TEST_F(BroadcastCopyMultiDestLarge, Broadcast_1024_Destinations_MAX) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
...
TEST_F(BroadcastCopyMultiDestLarge, Broadcast_1025_Destinations_ShouldFail) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
