...
// Broadcast copy tests: small multi-destination (2-10)
...
class BroadcastCopyMultiDestSmall : public ::testing::Test {
 protected:
  void SetUp() override {}
};
...
TEST_F(BroadcastCopyMultiDestSmall, TwoDests_4KB) {
  HsaTestContext ctx;
...
TEST_F(BroadcastCopyMultiDestSmall, TenDests_1MB_Performance) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
...
TEST_F(BroadcastCopyMultiDestSmall, ProgressiveDestCounts) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
...
TEST_F(BroadcastCopyMultiDestSmall, MultipleSequentialBroadcasts) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
