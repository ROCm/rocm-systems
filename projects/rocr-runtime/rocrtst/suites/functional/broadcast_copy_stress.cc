...
// Broadcast copy tests: stress & concurrent
...
class BroadcastCopyStress : public ::testing::Test {
 protected:
  void SetUp() override {}
};
...
TEST_F(BroadcastCopyStress, StabilityTest_1000_Iterations) {
  HsaTestContext ctx;
...
void BroadcastWorkerThread(int thread_id, hsa_agent_t gpu_agent, hsa_amd_memory_pool_t pool,
                           std::atomic<int>& pass_count, std::atomic<int>& fail_count) {
  ...
  for (int iter = 0; iter < ITERS_PER_THREAD; iter++) {
    ...
    hsa_status_t status = hsa_amd_memory_pool_allocate(pool, SIZE, 0, &src);
    ...
    for (int i = 0; i < NUM_DESTS; i++) {
      status = hsa_amd_memory_pool_allocate(pool, SIZE, 0, &dsts[i]);
      if (status != HSA_STATUS_SUCCESS) {
        alloc_success = false;
        break;
      }
    ...
    if (!alloc_success) {
      ...
      for (auto dst : dsts)
        if (dst) hsa_amd_memory_pool_free(dst);
      fail_count++;
      continue;
    }
    ...
    hsa_amd_memory_pool_free(src);
    for (auto dst : dsts) hsa_amd_memory_pool_free(dst);
  }
}

TEST_F(BroadcastCopyStress, ConcurrentBroadcasts_4_Threads) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
  ...
  for (int i = 0; i < NUM_THREADS; i++) {
    threads.emplace_back(BroadcastWorkerThread, i, ctx.gpu_agent, ctx.cpu_pool,
                         std::ref(pass_count), std::ref(fail_count));
  }
...
TEST_F(BroadcastCopyStress, MemoryPressureTest) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
  ...
  for (int i = 0; i < 100; i++) {  // Max 100MB pressure
    void* buf = nullptr;
    hsa_status_t status = hsa_amd_memory_pool_allocate(ctx.cpu_pool, BUFFER_SIZE, 0, &buf);
  ...
  if (!alloc_success) {
    std::cout << "  WARNING: Could not allocate test buffers under pressure (expected)" << std::endl;
    // Clean up pressure
    for (auto buf : pressure_buffers) hsa_amd_memory_pool_free(buf);
...
TEST_F(BroadcastCopyStress, RapidSequentialBroadcasts) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
