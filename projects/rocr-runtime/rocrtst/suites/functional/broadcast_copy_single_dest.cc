...
// Broadcast copy tests: single destination end-to-end
...
class BroadcastCopySingleDest : public ::testing::Test {
 protected:
  void SetUp() override {}
};
...
TEST_F(BroadcastCopySingleDest, SingleDest_4KB_Minimal) {
  HsaTestContext ctx;
...
TEST_F(BroadcastCopySingleDest, SingleDest_VariousSizes) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
...
TEST_F(BroadcastCopySingleDest, SingleDest_WithDependencySignal) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
...
TEST_F(BroadcastCopySingleDest, SingleDest_BasicCopy) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
...
TEST_F(BroadcastCopySingleDest, SingleDest_DataPatterns) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
