...
// Broadcast copy tests: parameter validation
...
class BroadcastCopyParamValidation : public ::testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(BroadcastCopyParamValidation, NullSourcePointer) {
  HsaTestContext ctx;
...
TEST_F(BroadcastCopyParamValidation, NullDestinationList) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
...
TEST_F(BroadcastCopyParamValidation, NullDestinationAgents) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
...
TEST_F(BroadcastCopyParamValidation, ZeroDestinations) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
...
TEST_F(BroadcastCopyParamValidation, ExceedsMaxDestinations) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
...
TEST_F(BroadcastCopyParamValidation, ZeroSize) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
...
TEST_F(BroadcastCopyParamValidation, InvalidSourceAgent) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
...
TEST_F(BroadcastCopyParamValidation, MismatchedSignalCount) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
...
TEST_F(BroadcastCopyParamValidation, NullPointerInDestList) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
...
TEST_F(BroadcastCopyParamValidation, CapabilityQueryNullOutput) {
  HsaTestContext ctx;
  if (!ctx.HasGPUAgent()) GTEST_SKIP() << "No GPU agent";
