...
//
// Note: This could be moved to common/helper_funcs.h for broader reuse across
// rocrtst. For now, it's defined here to minimize changes to shared headers.
// =============================================================================
...
static inline hsa_status_t hsa_amd_memory_broadcast_capability(hsa_agent_t agent,
  ...
  // GFX12+ has native SDMA multicast packet support (up to 1024 destinations)
  // The LINEAR MULTICAST packet is only available on GFX12 and later.
  // Pre-GFX12 hardware does not support broadcast copy natively.
  if (ver.major >= 12) {
...
static inline hsa_status_t hsa_amd_memory_broadcast_copy(
  ...
  // Zero-size copy is a no-op that succeeds immediately.
  // This matches the runtime behavior of hsa_amd_memory_async_batch_copy
  // which treats size==0 as a valid (empty) operation.
  if (size == 0) {
    return HSA_STATUS_SUCCESS;
  }
...
class HsaTestContext {
 public:
  ...
  // Allocate a buffer for GPU operations that is also CPU-accessible.
  //
  // This allocates from the CPU memory pool (system memory) and grants GPU
  // access. This approach ensures the buffer can be:
  //   1. Written by CPU (to fill test patterns)
  //   2. Read by CPU (to verify results)
  //   3. Accessed by GPU (source/destination for SDMA operations)
  //
  // Note: For tests that specifically require device-local (VRAM) memory,
  // use AllocateDeviceLocalBuffer() instead.
  //
  ...
  // Allocate device-local (VRAM) memory for GPU operations.
  //
  // This allocates from the GPU memory pool (device-local VRAM). The memory
  // may not be directly CPU-accessible on all hardware. Use this for tests
  // that specifically require device-local memory paths.
  //
  // Returns nullptr on allocation failure or if GPU pool is not available.
  void* AllocateDeviceLocalBuffer(size_t size) {
    if (gpu_pool.handle == 0) {
      return nullptr;
    }
    void* ptr = nullptr;
    hsa_status_t status = hsa_amd_memory_pool_allocate(gpu_pool, size, 0, &ptr);
    if (status != HSA_STATUS_SUCCESS) {
      return nullptr;
    }
    return ptr;
  ...
  // Multicast support is detected via HSA capability query, not ISA version
  bool IsMulticastSupported() const {
    uint32_t max_dests = 0;
    hsa_amd_memory_broadcast_capability(gpu_agent, &max_dests);
    return max_dests > 1;  // Multicast = more than 1 destination supported
  }
