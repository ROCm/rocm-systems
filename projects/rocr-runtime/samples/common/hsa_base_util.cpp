...
void HSA_UTIL::GetHsailNameAndKernelName(char* file_name_full, char* file_name_base,
                                         char* kernel_name) {
...
bool HSA_UTIL::HsaInit() {
  ...
  if (err == HSA_STATUS_ERROR) return false;
   ...
   */
  err =
      hsa_queue_create(device, queue_size, HSA_QUEUE_TYPE_MULTI, NULL, NULL, 0, 0, &command_queue);
  ...
  err = hsa_ext_program_create(HSA_MACHINE_MODEL_LARGE, profile,
                               HSA_DEFAULT_FLOAT_ROUNDING_MODE_ZERO, NULL, &hsa_program);
  ...
  err = hsa_ext_program_finalize(hsa_program, isa, 0, control_directives,
                                 NULL,  //"-g -O0 -dump-isa",
  ...
  err = hsa_executable_symbol_get_info(kernelSymbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_OBJECT,
                                       &codeHandle);
...
double HSA_UTIL::Run(int dim, int group_x, int group_y, int group_z, int s_size, int grid_x,
                     int grid_y, int grid_z, void* kernel_args, int kernel_args_size) {
#ifdef TIME
  ...
  hsa_executable_symbol_get_info(
      kernelSymbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_PRIVATE_SEGMENT_SIZE, &private_mem_size);
  ...
  */
   ...
   */
  while (hsa_signal_wait_acquire(local_signal, HSA_SIGNAL_CONDITION_EQ, 0, UINT64_MAX,
...
double HSA_UTIL::GetKernelTime() { return base_timer.ReadTimer(base_kernel_time_idx); }

double HSA_UTIL::GetSetupTime() { return base_timer.ReadTimer(base_setup_time_idx); }
