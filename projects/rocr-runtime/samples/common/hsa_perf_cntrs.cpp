...
hsa_status_t RocrPerfCntrApp::Init(hsa_agent_t agent) {
  ...
  if (cntrChoice == NULL) {
    ...
    info = new CntrInfo(0x4, "Iommu_Cntr_4", NULL, 0x63, NULL, 0x00, 0xFFFFFFFF, CntrValCnf_None);
    cntrList_.push_back(info);

    // Program to collect event number 6
    info = new CntrInfo(0x6, "Iommu_Cntr_6", NULL, 0x63, NULL, 0x00, 0xFFFFFFFF, CntrValCnf_None);
    cntrList_.push_back(info);
  }
