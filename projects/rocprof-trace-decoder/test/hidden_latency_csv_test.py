...
from rocprof_trace_decoder import AnalysisFlags, CodeIndex, Decoder, Pc
...
def _compute(args: argparse.Namespace) -> tuple[dict[Pc, Row], int]:
    ...
    with Decoder(args.lib) as decoder:
        ...
        for att_path in att_paths:
            ...
            # The decoder reports one record per (SIMD, program counter) and one parse covers
            # one capture, so summing every record aggregates the run by program counter.
            for record in records.hidden_latency:
                totals = hidden.setdefault(record.pc, [0, 0, 0])
    ...
    rows: dict[Pc, Row] = {}
    for pc, (idle, stall, issue) in hidden.items():
        if not idle + stall + issue:
            continue
...
def main() -> int:
    ...
    parser.add_argument(
        "--smoke",
        action="store_true",
        help="Require the analysis to run over the captures, without checking any values",
    )
    parser.add_argument(
        "--self-test",
        action="store_true",
    ...
    if args.expected is None and not (args.expect_empty or args.smoke):
        parser.error("--expected is required unless --expect-empty or --smoke is given")
    ...
    if args.smoke:
        # Deliberately checks no values. The control CSVs pin one trace per architecture; this
        # only asserts the analysis survives the captures they do not reach, which is where a
        # crash or a sanitizer failure would show up.
        print(f"analysed {decoded_wave_count} wave(s), {len(actual)} program counter(s) hidden")
        return 0
