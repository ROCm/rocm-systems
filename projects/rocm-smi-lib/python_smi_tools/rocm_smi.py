...
def getTargetGfxVersion(device, silent=False):
    ...
    gfx_ver_ret = "N/A"
    ret = rocmsmi.rsmi_dev_target_graphics_version_get(
        device, byref(target_graphics_version)
    )
...
def getComputePartition(device, silent=True):
    ...
    ret = rocmsmi.rsmi_dev_compute_partition_get(
        device, currentComputePartition, MAX_BUFF_SIZE
    )
    if (
...
def getMemoryPartition(device, silent=True):
    ...
    ret = rocmsmi.rsmi_dev_memory_partition_get(
        device, currentMemoryPartition, MAX_BUFF_SIZE
    )
    if (
...
def printEventList(device, delay, eventList):
    ...
    while (
        not stop_threads
    ):  # Exit condition from user keyboard input of 'q' or 'ctrl + c'
...
def printLog(
    device, metricName, value=None, extraSpace=False, useItalics=False, xcp=None
):
...
def setClockExtremum(deviceList, level, clkType, clkValue, autoRespond):
    ...
    if clkType not in {"sclk", "mclk"}:
        printLog(
            None,
            "Invalid clock type identifier %s, use sclk or mclk " % (clkType),
            None,
        )
    ...
    for device in deviceList:
        ...
        if rsmi_ret_ok(ret, device, silent=True):
            printLog(
                device,
                "Successfully set %s %s to %s(MHz)" % (level, clkType, clkValue),
                None,
            )
        else:
            if ret == rsmi_status_t.RSMI_STATUS_NOT_SUPPORTED:
                printLog(
                    device,
                    "Setting %s %s clock is not supported for this device."
                    % (level, clkType),
                    None,
                )
            else:
...
def setComputePartition(deviceList, computePartitionType):
    ...
    """
    printLogSpacer(
        " Set compute partition to %s " % (str(computePartitionType).upper())
    )
    for device in deviceList:
        ...
        if rsmi_ret_ok(ret, device, "set_compute_partition", silent=True):
            ...
            printLog(
                device,
                "Requested setting (%s) is unavailable for current device"
...
def setMemoryPartition(deviceList, memoryPartition, autoRespond):
    ...
    for device in deviceList:
        ...
        if rsmi_ret_ok(ret, device, "set_memory_partition", silent=True):
            ...
            printLog(
                device,
                "Available memory partition modes: "
                + str(memory_partition_caps).upper(),
                None,
                addExtraLine,
            )
        elif ret == rsmi_status_t.RSMI_STATUS_BUSY:
            printLog(
                device, "Device is currently busy, try again later", None, addExtraLine
            )
        elif ret == rsmi_status_t.RSMI_STATUS_AMDGPU_RESTART_ERR:
            printLog(
                device,
                "Issue reloading driver, please check dmsg for errors",
                None,
                addExtraLine,
            )
        else:
            printErrLog(
                device,
                "Failed to set memory partition, even though device supports it.",
            )
    printLogSpacer()
...
def showAllConcise(deviceList):
    ...
    subheader_output = "".join(
        ...
        for col, word in zip(range(len(max_widths)), subheader)
    )
    ...
    for device in deviceList:
        if clk_defined:
            if (
                ...
                logging.debug(
                    "{} clock is unsupported on device[{}]".format(clk_defined, device)
                )
            ...
            if rocmsmi.rsmi_dev_pci_bandwidth_get(device, None) == 1:
                ...
                logging.debug(
                    "{} clock is unsupported on device[{}]".format("PCIe", device)
                )
    if not concise:
        printLogSpacer()
    ...
    for device in deviceList:
        ...
        if retCode == rsmi_status_t.RSMI_STATUS_NOT_SUPPORTED:
            ...
            if fanLevel == 0 or fanSpeed == 0:
                printLog(
                    device, "Unable to detect fan speed for GPU %d" % (device), None
                )
                logging.debug(
            ...
            if PRINT_JSON:
                ...
                printLog(
                    device, "Fan Level", str(fanLevel) + " (%s%%)" % (str(fanSpeed))
                )
...
def showFwInfo(deviceList, fwType):
    ...
    for device in deviceList:
        ...
        for fw_name in firmware_blocks:
            ...
            if rsmi_ret_ok(
                ret, device, "get_firmware_version_" + str(fw_name), silent=True
            ):
...
def showMemUse(deviceList):
    ...
    for device in deviceList:
        allocated_mem_percent = getAllocatedMemoryPercent(device)
        printLog(
            device, "GPU Memory Allocated (VRAM%)", int(allocated_mem_percent["value"])
        )
            ...
            if header.format_revision >= 1 and header.content_revision >= 5:
                ...
                if rsmi_ret_ok(ret, device, "get_gpu_metrics", True):
                    ...
                    if metric_bw != ctypes.c_uint64(-1).value and metric_bw > 0:
                        ...
                        printLog(
                            device, "GPU metrics pcie_bandwidth_inst is invalid", None
                        )
                else:
    ...
    for pid in pidList:
        ...
        if verbose == "details":
            for dv_ind in dv_indices:
                ret = rocmsmi.rsmi_compute_process_info_by_device_get(
                    int(pid), dv_ind, byref(proc)
                )
...
def showPowerPlayTable(deviceList):
    ...
    for device in deviceList:
        ...
        if rsmi_ret_ok(ret, device, "get_od_volt"):
            ...
            printLog(
                device,
                "0: %sMhz" % (int(odvf.curr_mclk_range.lower_bound / 1000000)),
                None,
            )
            printLog(
                device,
            ...
            if (
                odvf.mclk_freq_limits.lower_bound > 0
                or odvf.mclk_freq_limits.upper_bound > 0
            ):
                printLog(
                    device,
        ...
        if rsmi_ret_ok(ret, device, "get_od_volt", silent=False):
            ...
            if rangeType == "voltage":
                if odvf.num_regions == 0:
                    printErrLog(
                        device, "Voltage curve regions unsupported.", is_warning=True
                    )
                    continue
...
def showEvents(deviceList, eventTypes):
        ...
        for device in deviceList:
            try:
                thread = threading.Thread(
                    target=printEventList, args=(device, 1000, eventTypeList)
                )
    ...
    while (
        1
    ):  # Exit condition from user keyboard input of 'q' or 'ctrl + c' and then 'Enter'
        user_input = input()
...
def showTempGraph(deviceList):
    ...
    try:
        ...
        thread = threading.Thread(
            target=printTempGraph, args=(deviceList, 150, temp_type)
        )
        thread.start()
        thread_list.append(thread)
    except Exception as e:
...
def showVoltageCurve(deviceList):
    ...
    for device in deviceList:
        ret = rocmsmi.rsmi_dev_od_volt_info_get(device, byref(odvf))
        if (
            rsmi_ret_ok(ret, device, "get_od_volt_info", silent=False)
            and odvf.num_regions > 0
        ):
...
def showComputePartition(deviceList):
    ...
    for device in deviceList:
        ret = rocmsmi.rsmi_dev_compute_partition_get(
            device, currentComputePartition, 256
        )
        if (
            ...
            printLog(
                device, "Compute Partition", currentComputePartition.value.decode()
            )
        elif ret == rsmi_status_t.RSMI_STATUS_NOT_SUPPORTED:
            printLog(device, "Not supported on the given system", None)
        else:
    ...
    for device in deviceList:
        ...
        if rsmi_ret_ok(ret, device, "rsmi_dev_gpu_metrics_info_get", silent=True):
            metrics = {
                ...
                "temperature_mem": {
                    "value": validateIfMaxUint(
                        gpu_metrics.temperature_mem, UIntegerTypes.UINT16_T
                    ),
                    "unit": temp_unit,
                },
                "temperature_vrgfx": {
                ...
                "current_gfxclk": {
                    "value": validateIfMaxUint(
                        gpu_metrics.current_gfxclk, UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "current_socclk": {
                    "value": validateIfMaxUint(
                        gpu_metrics.current_socclk, UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "current_uclk": {
                    "value": validateIfMaxUint(
                        gpu_metrics.current_uclk, UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "current_vclk0": {
                    "value": validateIfMaxUint(
                        gpu_metrics.current_vclk0, UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "current_dclk0": {
                    "value": validateIfMaxUint(
                        gpu_metrics.current_dclk0, UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "current_vclk1": {
                    "value": validateIfMaxUint(
                        gpu_metrics.current_vclk1, UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "current_dclk1": {
                    "value": validateIfMaxUint(
                        gpu_metrics.current_dclk1, UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "throttle_status": {
                    "value": validateIfMaxUint(
                        gpu_metrics.throttle_status, UIntegerTypes.UINT32_T
                    ),
                    "unit": no_unit,
                },
                "current_fan_speed": {
                ...
                "pcie_link_width": {
                    "value": validateIfMaxUint(
                        gpu_metrics.pcie_link_width, UIntegerTypes.UINT16_T
                    ),
                    "unit": pcie_lanes_unit,
                },
                "pcie_link_speed": {
                    "value": validateIfMaxUint(
                        gpu_metrics.pcie_link_speed, UIntegerTypes.UINT16_T
                    ),
                ...
                "voltage_soc": {
                    "value": validateIfMaxUint(
                        gpu_metrics.voltage_soc, UIntegerTypes.UINT16_T
                    ),
                    "unit": volt_unit,
                },
                "voltage_gfx": {
                    "value": validateIfMaxUint(
                        gpu_metrics.voltage_gfx, UIntegerTypes.UINT16_T
                    ),
                    "unit": volt_unit,
                },
                "voltage_mem": {
                    "value": validateIfMaxUint(
                        gpu_metrics.voltage_mem, UIntegerTypes.UINT16_T
                    ),
                    "unit": volt_unit,
                },
                ...
                "xgmi_link_width": {
                    "value": validateIfMaxUint(
                        gpu_metrics.xgmi_link_width, UIntegerTypes.UINT16_T
                    ),
                    "unit": no_unit,
                },
                "xgmi_link_speed": {
                    "value": validateIfMaxUint(
                        gpu_metrics.xgmi_link_speed, UIntegerTypes.UINT16_T
                    ),
                    "unit": xgmi_speed,
                },
                "pcie_bandwidth_acc": {
                ...
                "num_partition": {
                    "value": validateIfMaxUint(
                        gpu_metrics.num_partition, UIntegerTypes.UINT16_T
                    ),
                    "unit": no_unit,
                },
            ...
            for k, v in metrics.items():
                if k != "common_header" and "xcp_stats" not in k:
                    if v["unit"] != None:
                        printLog(
                            device, k + " (" + str(v["unit"]) + ")", str(v["value"])
                        )
                    elif v["unit"] == None:
                ...
                if "xcp_stats.gfx_busy_inst" in k:
                    for curr_xcp, item in enumerate(v["value"]):
                        ...
                        for _, val in enumerate(item.gfx_busy_inst):
                            print_xcp_detail.append(
                                validateIfMaxUint(val, UIntegerTypes.UINT32_T)
                            )
                        printLog(
                            device,
                            k + " (" + str(v["unit"]) + ")",
                ...
                if "xcp_stats.jpeg_busy" in k:
                    for curr_xcp, item in enumerate(v["value"]):
                        ...
                        for _, val in enumerate(item.jpeg_busy):
                            print_xcp_detail.append(
                                validateIfMaxUint(val, UIntegerTypes.UINT16_T)
                            )
                        printLog(
                            device,
                            k + " (" + str(v["unit"]) + ")",
                ...
                if "xcp_stats.gfx_busy_acc" in k:
                    for curr_xcp, item in enumerate(v["value"]):
                        ...
                        for _, val in enumerate(item.gfx_busy_acc):
                            print_xcp_detail.append(
                                validateIfMaxUint(val, UIntegerTypes.UINT64_T)
                            )
                        printLog(
                            device,
                            k + " (" + str(v["unit"]) + ")",
...
def check_runtime_status() -> bool:
    ...
    for device in os.listdir(base_path):
        if os.path.isdir(os.path.join(base_path, device)):
            runtime_status_path = os.path.join(
                base_path, device, "power", "runtime_status"
            )
            try:
        ...
        if not PRINT_JSON:
            ...
            if not silent:
                ...
                if my_ret in rsmi_status_verbose_err_out:
                    printLog(
                        device,
                        metric + ", " + rsmi_status_verbose_err_out[my_ret],
                        None,
                    )
        RETCODE = my_ret
        return False
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="AMD ROCm System Management Interface  |  ROCM-SMI version: %s"
        % __version__,
    ...
    groupDisplayTop.add_argument(
        "-i", "--showid", help="Show DEVICE IDs", action="store_true"
    )
    groupDisplayTop.add_argument(
    ...
    groupAction.add_argument(
        ...
        choices=compute_partition_type_l
        + [x.lower() for x in compute_partition_type_l],
        type=str,
        nargs=1,
    )
    groupAction.add_argument(
