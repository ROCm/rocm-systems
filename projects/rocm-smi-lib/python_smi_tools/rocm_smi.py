...
try:
    ...
    try:
        ...
        print(
            f"Still couldn't import 'rsmiBindings'. Make sure it's installed in {additional_path}"
        )
        sys.exit(1)
...
def getMemoryPartitionCapabilities(device, silent=True):
    ...
    ret = rocmsmi.rsmi_dev_memory_partition_capabilities_get(
        device, memoryPartitionCapabilities, MAX_BUFF_SIZE
    )
    if (
        rsmi_ret_ok(ret, device, "get_compute_partition", silent)
        and memoryPartitionCapabilities.value.decode()
    ):
...
def setClockRange(deviceList, clkType, minvalue, maxvalue, autoRespond):
    ...
    for device in deviceList:
        ...
        if rsmi_ret_ok(ret, device, silent=True):
            printLog(
                device,
                "Successfully set %s from %s(MHz) to %s(MHz)" % (clkType, minvalue, maxvalue),
                None,
            )
        else:
            if ret == rsmi_status_t.RSMI_STATUS_NOT_SUPPORTED:
                printLog(
                    device, "Setting %s range is not supported for this device." % (clkType), None
                )
            else:
                RETCODE = 1
...
def setClockExtremum(deviceList, level, clkType, clkValue, autoRespond):
    ...
    for device in deviceList:
        ret = rocmsmi.rsmi_dev_clk_extremum_set(
            device, rsmi_freq_ind_t(int(point)), int(clkValue), rsmi_clk_names_dict[clkType]
        )
        if rsmi_ret_ok(ret, device, silent=True):
            ...
            if ret == rsmi_status_t.RSMI_STATUS_NOT_SUPPORTED:
                printLog(
                    device,
                    "Setting %s %s clock is not supported for this device." % (level, clkType),
                    None,
                )
            else:
                RETCODE = 1
...
def setVoltageCurve(deviceList, point, clk, volt, autoRespond):
    ...
    for device in deviceList:
        ...
        if rsmi_ret_ok(ret, device, "set_voltage_curve"):
            printLog(
                device,
                "Successfully set voltage point %s to %s(MHz) %s(mV)" % (point, clk, volt),
                None,
            )
    ...
    for device in deviceList:
        if clkType == "sclk":
            ret = rocmsmi.rsmi_dev_od_clk_info_set(
                device, rsmi_freq_ind_t(int(point)), int(clk), rsmi_clk_names_dict[clkType]
            )
    ...
    for device in deviceList:
        ...
        if clktype != "pcie":
            ...
            ret = rocmsmi.rsmi_dev_gpu_clk_freq_get(
                device, rsmi_clk_names_dict[clktype], byref(freq)
            )
            ...
            if rsmi_ret_ok(ret, device, "set_PCIe_bandwidth"):
                printLog(
                    device, "Successfully set %s to level bitmask" % (clktype), hex(freq_bitmask)
                )
            else:
                RETCODE = 1
            continue
...
def setFanSpeed(deviceList, fan):
    ...
    for device in deviceList:
        if str(fan):
            ...
            if rsmi_ret_ok(ret, device, silent=True):
                printLog(
                    device, "Successfully set fan speed to level %s" % (str(int(fanLevel))), None
                )
            else:
    ...
    for device in deviceList:
        ...
        if rsmi_ret_ok(ret, device, "get_power_profile"):
            ...
            if str(profile).isnumeric() and int(profile) > 0 and int(profile) < 8:
                ...
                printErrLog(
                    device, "Unable to set profile to: %s (UNKNOWN profile)" % (str(profile))
                )
                return
...
def setComputePartition(deviceList, computePartitionType):
    ...
    for device in deviceList:
        ...
        ret = rocmsmi.rsmi_dev_compute_partition_set(
            device, rsmi_compute_partition_type_dict[computePartitionType]
        )
        if rsmi_ret_ok(ret, device, "set_compute_partition", silent=True):
            printLog(
                device, "Successfully set compute partition to %s" % (computePartitionType), None
            )
            ...
            printErrLog(
                device, "Failed to retrieve compute partition, even though device supports it."
            )
    printLogSpacer()
...
def setMemoryPartition(deviceList, memoryPartition, autoRespond):
    ...
    for device in deviceList:
        ...
        t1 = multiprocessing.Process(
            target=showProgressbar, args=("Updating memory partition", kTimeWait)
        )
        t1.start()
        addExtraLine = True
        start = time.time()
        ...
        if rsmi_ret_ok(ret, device, "set_memory_partition", silent=True):
            ...
            printLog(
                device,
                "Device does not support setting to " + str(memoryPartition).upper(),
                None,
                addExtraLine,
            )
            ...
            printLog(
                device,
                "Available memory partition modes: " + str(memory_partition_caps).upper(),
                None,
                addExtraLine,
            )
        elif ret == rsmi_status_t.RSMI_STATUS_BUSY:
            ...
            printLog(
                device, "Issue reloading driver, please check dmsg for errors", None, addExtraLine
            )
        else:
        ...
        for clk_type in sorted(rsmi_clk_names_dict):
            if rocmsmi.rsmi_dev_gpu_clk_freq_get(device, rsmi_clk_names_dict[clk_type], None) == 1:
                ...
                if not rsmi_ret_ok(ret, device, "get_clk_freq_" + clk_type, True):
                    continue
                printLog(
                    device, "Supported %s frequencies on GPU%s" % (clk_type, str(device)), None
                )
        ...
        if rocmsmi.rsmi_dev_pci_bandwidth_get(device, None) == 1:
            ...
            if rsmi_ret_ok(ret, device, "get_PCIe_bandwidth", True):
                ...
                for i in range(bw.transfer_rate.num_supported):
                    freq_string = "{:>.1f}GT/s x{}".format(
                        bw.transfer_rate.frequency[i] / 1000000000, bw.lanes[i]
                    )
                ...
                if (
                    ...
                    if rsmi_ret_ok(ret, device, "get_clk_freq_" + str(clk_type), True):
                        ...
                        if PRINT_JSON:
                            ...
                            printLog(
                                device,
                                "%s clock level: %s" % (clk_type, freq_index),
                                "(%sMhz)" % (str(fr)[:-2]),
                            )
                elif not concise:
            ...
            if rocmsmi.rsmi_dev_pci_bandwidth_get(device, None) == 1:
                ...
                if rsmi_ret_ok(ret, device, "get_PCIe_bandwidth", True):
                    ...
                    if current_f >= bw.transfer_rate.num_supported:
                        ...
                        continue
                    fr = "{:.1f}GT/s x{}".format(
                        bw.transfer_rate.frequency[current_f] / 1000000000, bw.lanes[current_f]
                    )
        ...
        for fw_name in firmware_blocks:
            ...
            if rsmi_ret_ok(ret, device, "get_firmware_version_" + str(fw_name), silent=True):
                ...
                # The ME, MC, and CE firmware names are only 2 characters, so they need an additional tab
                    ...
                    printLog(
                        device, "%s firmware version" % (fw_name), "\t\t%s" % (str(fw_ver.value))
                    )
                else:
                    printLog(
                        device, "%s firmware version" % (fw_name), "\t%s" % (str(fw_ver.value))
...
def showEnergy(deviceList):
    ...
    for device in deviceList:
        ...
        if rsmi_ret_ok(ret, device, "% Energy Counter"):
            ...
            printLog(
                device, "Accumulated Energy (uJ)", round(power.value * counter_resolution.value, 2)
            )
    printLogSpacer()
...
def showMemUse(deviceList):
    ...
    for device in deviceList:
        allocated_mem_percent = getAllocatedMemoryPercent(device)
        printLog(device, "GPU Memory Allocated (VRAM%)", int(allocated_mem_percent["value"]))
...
def showRange(deviceList, rangeType):
    ...
    for device in deviceList:
        ...
        if rsmi_ret_ok(ret, device, "get_od_volt", silent=False):
            if rangeType == "sclk":
                if (
                    odvf.curr_sclk_range.lower_bound == uint64_max
                    or odvf.curr_sclk_range.upper_bound == uint64_max
                ):
            ...
            if rangeType == "mclk":
                if (
                    odvf.curr_mclk_range.lower_bound == uint64_max
                    or odvf.curr_mclk_range.upper_bound == uint64_max
                ):
                    printLog(device, "Unable to display %s range" % (rangeType), None)
                    continue
                printLog(
...
def showWeightTopology(deviceList):
    ...
    if PRINT_JSON:
        formatMatrixToJSON(
            deviceList, gpu_links_weight, "(Topology) Weight between DRM devices {} and {}"
        )
        return
...
def showTypeTopology(deviceList):
    ...
    if PRINT_JSON:
        formatMatrixToJSON(
            deviceList, gpu_links_type, "(Topology) Link type between DRM devices {} and {}"
        )
        return
    ...
    for srcdevice in deviceList:
        for destdevice in deviceList:
            if srcdevice != destdevice:
                ...
                if rsmi_ret_ok(
                    ret2, " {} to {}".format(srcdevice, destdevice), "get_link_topology_type", True
                ):
                    if linktype.value != 2:
                        nonXgmi = True
                        silent = True
                ...
                if rsmi_ret_ok(
                    ret,
                    " {}  to {}".format(srcdevice, destdevice),
...
def showGPUMetrics(deviceList):
    ...
    for device in deviceList:
        ret = rocmsmi.rsmi_dev_gpu_metrics_info_get(device, byref(gpu_metrics))
        metrics = {"common_header": "N/A"}
        if rsmi_ret_ok(ret, device, "rsmi_dev_gpu_metrics_info_get", silent=True):
            metrics = {
                "common_header": {
                    "version": float(
                        str(gpu_metrics.common_header.format_revision)
                        + "."
                ...
                "temperature_edge": {
                    "value": validateIfMaxUint(
                        gpu_metrics.temperature_edge, UIntegerTypes.UINT16_T
                    ),
                    "unit": temp_unit,
                },
                "temperature_hotspot": {
                    "value": validateIfMaxUint(
                        gpu_metrics.temperature_hotspot, UIntegerTypes.UINT16_T
                    ),
                    "unit": temp_unit,
                },
                "temperature_mem": {
                ...
                "temperature_vrgfx": {
                    "value": validateIfMaxUint(
                        gpu_metrics.temperature_vrgfx, UIntegerTypes.UINT16_T
                    ),
                    "unit": temp_unit,
                },
                "temperature_vrsoc": {
                    "value": validateIfMaxUint(
                        gpu_metrics.temperature_vrsoc, UIntegerTypes.UINT16_T
                    ),
                    "unit": temp_unit,
                },
                "temperature_vrmem": {
                    "value": validateIfMaxUint(
                        gpu_metrics.temperature_vrmem, UIntegerTypes.UINT16_T
                    ),
                    "unit": temp_unit,
                },
                "average_gfx_activity": {
                    "value": validateIfMaxUint(
                        gpu_metrics.average_gfx_activity, UIntegerTypes.UINT16_T
                    ),
                    "unit": percent_unit,
                },
                "average_umc_activity": {
                    "value": validateIfMaxUint(
                        gpu_metrics.average_umc_activity, UIntegerTypes.UINT16_T
                    ),
                    "unit": percent_unit,
                },
                "average_mm_activity": {
                    "value": validateIfMaxUint(
                        gpu_metrics.average_mm_activity, UIntegerTypes.UINT16_T
                    ),
                    "unit": percent_unit,
                },
                "average_socket_power": {
                    "value": validateIfMaxUint(
                        gpu_metrics.average_socket_power, UIntegerTypes.UINT16_T
                    ),
                    "unit": power_unit,
                },
                "energy_accumulator": {
                    "value": validateIfMaxUint(
                        gpu_metrics.energy_accumulator, UIntegerTypes.UINT64_T
                    ),
                    "unit": energy_unit,
                },
                "system_clock_counter": {
                    "value": validateIfMaxUint(
                        gpu_metrics.system_clock_counter, UIntegerTypes.UINT64_T
                    ),
                    "unit": time_unit,
                },
                "average_gfxclk_frequency": {
                    "value": validateIfMaxUint(
                        gpu_metrics.average_gfxclk_frequency, UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "average_socclk_frequency": {
                    "value": validateIfMaxUint(
                        gpu_metrics.average_socclk_frequency, UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "average_uclk_frequency": {
                    "value": validateIfMaxUint(
                        gpu_metrics.average_uclk_frequency, UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "average_vclk0_frequency": {
                    "value": validateIfMaxUint(
                        gpu_metrics.average_vclk0_frequency, UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "average_dclk0_frequency": {
                    "value": validateIfMaxUint(
                        gpu_metrics.average_dclk0_frequency, UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "average_vclk1_frequency": {
                    "value": validateIfMaxUint(
                        gpu_metrics.average_vclk1_frequency, UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "average_dclk1_frequency": {
                    "value": validateIfMaxUint(
                        gpu_metrics.average_dclk1_frequency, UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "current_gfxclk": {
                ...
                "current_fan_speed": {
                    "value": validateIfMaxUint(
                        gpu_metrics.current_fan_speed, UIntegerTypes.UINT16_T
                    ),
                    "unit": fan_speed,
                },
                "pcie_link_width": {
                ...
                "gfx_activity_acc": {
                    "value": validateIfMaxUint(
                        gpu_metrics.gfx_activity_acc, UIntegerTypes.UINT32_T
                    ),
                    "unit": percent_unit,
                },
                "mem_activity_acc": {
                    "value": validateIfMaxUint(
                        gpu_metrics.mem_activity_acc, UIntegerTypes.UINT32_T
                    ),
                    "unit": percent_unit,
                },
                "temperature_hbm": {
                    "value": validateIfMaxUint(
                        list(gpu_metrics.temperature_hbm), UIntegerTypes.UINT16_T
                    ),
                    "unit": temp_unit,
                },
                "firmware_timestamp": {
                    "value": validateIfMaxUint(
                        gpu_metrics.firmware_timestamp, UIntegerTypes.UINT64_T
                    ),
                    "unit": time_unit_10,
                },
                "voltage_soc": {
                ...
                "indep_throttle_status": {
                    "value": validateIfMaxUint(
                        gpu_metrics.indep_throttle_status, UIntegerTypes.UINT64_T
                    ),
                    "unit": no_unit,
                },
                "current_socket_power": {
                    "value": validateIfMaxUint(
                        gpu_metrics.current_socket_power, UIntegerTypes.UINT16_T
                    ),
                    "unit": power_unit,
                },
                "vcn_activity": {
                    "value": validateIfMaxUint(
                        list(gpu_metrics.vcn_activity), UIntegerTypes.UINT16_T
                    ),
                    "unit": percent_unit,
                },
                "gfxclk_lock_status": {
                    "value": validateIfMaxUint(
                        gpu_metrics.gfxclk_lock_status, UIntegerTypes.UINT32_T
                    ),
                    "unit": no_unit,
                },
                "xgmi_link_width": {
                ...
                "pcie_bandwidth_acc": {
                    "value": validateIfMaxUint(
                        gpu_metrics.pcie_bandwidth_acc, UIntegerTypes.UINT64_T
                    ),
                    "unit": pcie_acc_unit,
                },
                "pcie_bandwidth_inst": {
                    "value": validateIfMaxUint(
                        gpu_metrics.pcie_bandwidth_inst, UIntegerTypes.UINT64_T
                    ),
                    "unit": pcie_acc_unit,
                },
                "pcie_l0_to_recov_count_acc": {
                    "value": validateIfMaxUint(
                        gpu_metrics.pcie_l0_to_recov_count_acc, UIntegerTypes.UINT64_T
                    ),
                    "unit": count,
                },
                "pcie_replay_count_acc": {
                    "value": validateIfMaxUint(
                        gpu_metrics.pcie_replay_count_acc, UIntegerTypes.UINT64_T
                    ),
                    "unit": count,
                },
                "pcie_replay_rover_count_acc": {
                    "value": validateIfMaxUint(
                        gpu_metrics.pcie_replay_rover_count_acc, UIntegerTypes.UINT64_T
                    ),
                    "unit": count,
                },
                "xgmi_read_data_acc": {
                    "value": validateIfMaxUint(
                        list(gpu_metrics.xgmi_read_data_acc), UIntegerTypes.UINT64_T
                    ),
                    "unit": xgmi_data_sz,
                },
                "xgmi_write_data_acc": {
                    "value": validateIfMaxUint(
                        list(gpu_metrics.xgmi_write_data_acc), UIntegerTypes.UINT64_T
                    ),
                    "unit": xgmi_data_sz,
                },
                "current_gfxclks": {
                    "value": validateIfMaxUint(
                        list(gpu_metrics.current_gfxclks), UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "current_socclks": {
                    "value": validateIfMaxUint(
                        list(gpu_metrics.current_socclks), UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "current_vclk0s": {
                    "value": validateIfMaxUint(
                        list(gpu_metrics.current_vclk0s), UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "current_dclk0s": {
                    "value": validateIfMaxUint(
                        list(gpu_metrics.current_dclk0s), UIntegerTypes.UINT16_T
                    ),
                    "unit": clock_unit,
                },
                "jpeg_activity": {
                    "value": validateIfMaxUint(
                        list(gpu_metrics.jpeg_activity), UIntegerTypes.UINT16_T
                    ),
                    "unit": percent_unit,
                },
                "pcie_nak_sent_count_acc": {
                    "value": validateIfMaxUint(
                        gpu_metrics.pcie_nak_sent_count_acc, UIntegerTypes.UINT32_T
                    ),
                    "unit": count,
                },
                "pcie_nak_rcvd_count_acc": {
                    "value": validateIfMaxUint(
                        gpu_metrics.pcie_nak_rcvd_count_acc, UIntegerTypes.UINT32_T
                    ),
                    "unit": count,
                },
                "accumulation_counter": {
                    "value": validateIfMaxUint(
                        gpu_metrics.accumulation_counter, UIntegerTypes.UINT64_T
                    ),
                    "unit": count,
                },
                "prochot_residency_acc": {
                    "value": validateIfMaxUint(
                        gpu_metrics.prochot_residency_acc, UIntegerTypes.UINT64_T
                    ),
                    "unit": count,
                },
                "ppt_residency_acc": {
                    "value": validateIfMaxUint(
                        gpu_metrics.ppt_residency_acc, UIntegerTypes.UINT64_T
                    ),
                    "unit": count,
                },
                "socket_thm_residency_acc": {
                    "value": validateIfMaxUint(
                        gpu_metrics.socket_thm_residency_acc, UIntegerTypes.UINT64_T
                    ),
                    "unit": count,
                },
                "vr_thm_residency_acc": {
                    "value": validateIfMaxUint(
                        gpu_metrics.vr_thm_residency_acc, UIntegerTypes.UINT64_T
                    ),
                    "unit": count,
                },
                "hbm_thm_residency_acc": {
                    "value": validateIfMaxUint(
                        gpu_metrics.hbm_thm_residency_acc, UIntegerTypes.UINT64_T
                    ),
                    "unit": count,
                },
                "pcie_lc_perf_other_end_recovery": {
                    "value": validateIfMaxUint(
                        gpu_metrics.pcie_lc_perf_other_end_recovery, UIntegerTypes.UINT32_T
                    ),
                    "unit": count,
                },
                "vram_max_bandwidth": {
                    "value": validateIfMaxUint(
                        gpu_metrics.vram_max_bandwidth, UIntegerTypes.UINT64_T
                    ),
                    "unit": vram_max_bw,
                },
                "xgmi_link_status": {
                    "value": validateIfMaxUint(
                        list(gpu_metrics.xgmi_link_status), UIntegerTypes.UINT16_T
                    ),
                    "unit": link_status,
                },
                "num_partition": {
                ...
                "xcp_stats.gfx_busy_inst": {"value": gpu_metrics.xcp_stats, "unit": percent_unit},
                "xcp_stats.jpeg_busy": {"value": gpu_metrics.xcp_stats, "unit": percent_unit},
                "xcp_stats.vcn_busy": {"value": gpu_metrics.xcp_stats, "unit": percent_unit},
                "xcp_stats.gfx_busy_acc": {"value": gpu_metrics.xcp_stats, "unit": count},
            ...
            printLog(
                device,
                "Metric Version and Size (Bytes)",
                str(metrics["common_header"]["version"])
                + " "
                + str(metrics["common_header"]["size"]),
            )
            ...
            printLog(
                device,
                "Failed to retrieve GPU metrics, metric version may not be supported for this device.",
            )
    printLogSpacer()
    ...
    for device in os.listdir(base_path):
        if os.path.isdir(os.path.join(base_path, device)):
            ...
            try:
                ...
                logging.debug(
                    f"Permission denied while accessing {runtime_status_path} \nError: {e}"
                )
                continue
        else:
            pass
...
if __name__ == "__main__":
    ...
    groupDev.add_argument(
        "-d", "--device", help="Execute command on specified device", type=int, nargs="+"
    )
    ...
    groupDisplayOpt.add_argument(
        "-a", "--showallinfo", help="Show Temperature, Fan and Clock values", action="store_true"
    )
    ...
    groupDisplayTop.add_argument(
        "-e", "--showevents", help="Show event list", metavar="EVENT", type=str, nargs="*"
    )
    groupDisplayTop.add_argument(
        "--showdriverversion", help="Show kernel driver version", action="store_true"
    )
    groupDisplayTop.add_argument(
        "--showtempgraph", help="Show Temperature Graph", action="store_true"
    )
    groupDisplayTop.add_argument(
    ...
    groupDisplayTop.add_argument(
        "--showproductname", help="Show product details", action="store_true"
    )
    groupDisplayTop.add_argument(
    ...
    groupDisplayPages.add_argument(
        "--showpagesinfo", help="Show retired, pending and unreservable pages", action="store_true"
    )
    groupDisplayPages.add_argument(
    ...
    groupDisplayHw.add_argument(
        "-P",
        "--showpower",
        help="Show current average or instant socket graphics package power consumption",
        action="store_true",
    )
    groupDisplayHw.add_argument(
    ...
    groupDisplay.add_argument(
        "-g", "--showgpuclocks", help="Show current GPU clock frequencies", action="store_true"
    )
    groupDisplay.add_argument(
        "-l", "--showprofile", help="Show Compute Profile attributes", action="store_true"
    )
    groupDisplay.add_argument(
        "-M",
        "--showmaxpower",
    ...
    groupDisplay.add_argument(
        "-o", "--showoverdrive", help="Show current GPU Clock OverDrive level", action="store_true"
    )
    groupDisplay.add_argument(
        "-p", "--showperflevel", help="Show current DPM Performance Level", action="store_true"
    )
    groupDisplay.add_argument(
        "-S",
        "--showclkvolt",
    ...
    groupDisplay.add_argument(
        "-s", "--showclkfrq", help="Show supported GPU and Memory Clock", action="store_true"
    )
    groupDisplay.add_argument(
        "--showmeminfo",
    ...
    groupDisplay.add_argument(
        "--showxgmierr", help="Show XGMI error information since last read", action="store_true"
    )
    groupDisplay.add_argument(
        "--showtopo", help="Show hardware topology information", action="store_true"
    )
    groupDisplay.add_argument(
        "--showtopoaccess", help="Shows the link accessibility between GPUs ", action="store_true"
    )
    groupDisplay.add_argument(
        "--showtopoweight", help="Shows the relative weight between GPUs ", action="store_true"
    )
    groupDisplay.add_argument(
        "--showtopohops", help="Shows the number of hops between GPUs ", action="store_true"
    )
    groupDisplay.add_argument(
    ...
    groupDisplay.add_argument(
        "--showcomputepartition", help="Shows current compute partitioning ", action="store_true"
    )
    groupDisplay.add_argument(
        "--showmemorypartition", help="Shows current memory partition ", action="store_true"
    )
    groupDisplay.add_argument(
        "--showmetrics", help="Show current gpu metric data ", action="store_true"
    )

    groupActionReset.add_argument(
        "-r", "--resetclocks", help="Reset clocks and OverDrive to default", action="store_true"
    )
    groupActionReset.add_argument(
        "--resetfans", help="Reset fans to automatic (driver) control", action="store_true"
    )
    groupActionReset.add_argument(
        "--resetprofile", help="Reset Power Profile back to default", action="store_true"
    )
    groupActionReset.add_argument(
        "--resetpoweroverdrive",
    ...
    groupActionReset.add_argument(
        "--resetperfdeterminism", help="Disable performance determinism", action="store_true"
    )
    ...
    groupAction.add_argument(
        "--setsrange", help="Set min and max SCLK speed", metavar=("SCLKMIN", "SCLKMAX"), nargs=2
    )
    groupAction.add_argument(
        "--setextremum",
    ...
    groupAction.add_argument(
        "--setmrange", help="Set min and max MCLK speed", metavar=("MCLKMIN", "MCLKMAX"), nargs=2
    )
    ...
    groupActionGpuReset.add_argument(
        "--gpureset", help="Reset specified GPU (One GPU must be specified)", action="store_true"
    )

    groupFile.add_argument(
        "--load", help="Load Clock, Fan, Performance and Profile settings from FILE", metavar="FILE"
    )
    groupFile.add_argument(
        "--save", help="Save Clock, Fan, Performance and Profile settings to FILE", metavar="FILE"
    )
    ...
    if not check_runtime_status():
        ...
        if wake_device_failed:
            logging.warning(
                "AMD GPU device(s) is/are in a low-power state. Check power control/runtime_status\n"
            )
