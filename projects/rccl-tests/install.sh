...
getopt -T
if [[ $? -eq 4 ]]; then
  GETOPT_PARSE=$(getopt --name "${0}" --longoptions help,mpi,test,rocm_home:,rccl_home:,mpi_home:,hip_compiler:,gpu_targets:,enable-device-coverage --options hmt -- "$@")
else
...
if ($mpi_enabled); then
  if [[ ${mpi_dir} == "" ]]; then
    ...
    echo
    make NCCL_HOME=${rccl_dir} CUSTOM_RCCL_LIB=${rccl_dir}/lib/librccl.so MPI=1 MPI_HOME=${mpi_dir} HIPCC=${hip_compiler} ${GPU_TARGETS} ${COVERAGE_MAKE_ARG} -j$(nproc)
  fi
else
  echo "[INFO] Compiling without MPI support (MPI support requires -m and --mpi_home)"
  echo
  make NCCL_HOME=${rccl_dir} CUSTOM_RCCL_LIB=${rccl_dir}/lib/librccl.so HIPCC=${hip_compiler} ${GPU_TARGETS} ${COVERAGE_MAKE_ARG} -j$(nproc)
fi
check_exit_code "$?"
