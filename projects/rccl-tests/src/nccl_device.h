#ifndef RCCL_TESTS_LOCAL_NCCL_DEVICE_H_
#define RCCL_TESTS_LOCAL_NCCL_DEVICE_H_

#if !defined(__HIP_PLATFORM_AMD__)
#include_next "nccl_device.h"
#else
...
ncclResult_t ncclDevCommCreate(ncclComm_t comm, ncclDevCommRequirements_t const* reqs, ncclDevComm_t* outDevComm);
...
NCCL_DEVICE_INLINE void* ncclGetLsaPointer(ncclWindow_t w, size_t offset, int peer) {
  return static_cast<void*>(ncclAdd4G(w->lsaFlatBase, peer * w->stride4G) + offset);
}
...
struct ncclLsaBarrierSession {
  ...
  NCCL_DEVICE_INLINE ncclLsaBarrierSession(
      ...
      uint32_t index_,
      bool multimem_ = false)
      : ncclLsaBarrierSession(coop_, comm_, ncclTeamLsa(comm_), comm_.lsaBarrier, index_, multimem_, comm_.lsaMultimem) {}
