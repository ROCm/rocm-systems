 ...
 ************************************************************************/
 ...
 static void decodePerChannelValue(float val, int nCh, int nRanks, int fillMode,
                                   int* outRank, int* outCh) {
 ...
 testResult_t CheckData(struct threadArgs* args, ncclDataType_t type, ncclRedOp_t op, int root, int in_place, int64_t *wrongElts) {
   ...
   for (int i=0; i<args->nGpus; i++) {
     ...
     // ---- Per-channel error reporting (runtime, always compiled) ----
     if (args->reportErrors && wrongPerGpu[i] != 0) {
       static int perChannelInit = -1;
       static int nChannelsEnv   = -1;
       static int fillModeEnv    = -1;
       if (perChannelInit == -1) {
         const char* env  = getenv("NCCL_TESTS_PERCHANNEL_INIT");
         perChannelInit   = (env && atoi(env) > 0) ? 1 : 0;
         const char* chEnv = getenv("NCCL_MIN_NCHANNELS");
         nChannelsEnv     = (chEnv && atoi(chEnv) > 0) ? atoi(chEnv) : 16;
         const char* fmEnv = getenv("NCCL_TESTS_FILL_MODE");
         fillModeEnv      = (fmEnv) ? atoi(fmEnv) : 1;
         if (fillModeEnv < 1 || fillModeEnv > 3) fillModeEnv = 1;
       }
 
       if (perChannelInit && type == ncclFloat) {
         // Per-channel AllGather mismatch report
         int nCh = nChannelsEnv;
         size_t sendcount   = args->sendBytes / wordSize(type);
         size_t floats_per_ch = (nCh > 0 && sendcount % nCh == 0)
                                  ? sendcount / nCh : sendcount;
         size_t totalFloats = args->expectedBytes / sizeof(float);
 
         char hostname[1024];
         getHostName(hostname, 1024);
 
         printf("\n===== Per-Channel AllGather Errors [%s rank %d]: "
                "%lld wrong elements =====\n",
                hostname, rank, (long long)wrongPerGpu[i]);
         printf("  nranks=%d  nChannels=%d  sendcount=%zu  floats_per_ch=%zu  fillMode=%d\n",
                nranks, nCh, sendcount, floats_per_ch, fillModeEnv);
 
         float *expectedHost = (float*)malloc(args->expectedBytes);
         float *dataHost     = (float*)malloc(args->expectedBytes);
         if (expectedHost && dataHost) {
           cudaMemcpy(expectedHost, args->expected[i],
                      args->expectedBytes, cudaMemcpyDeviceToHost);
           cudaMemcpy(dataHost, data,
                      args->expectedBytes, cudaMemcpyDeviceToHost);
 
           int errCount = 0;
           // Default 50; set NCCL_TESTS_MAX_ERRORS=0 for unlimited
           static int maxErrors = -1;
           if (maxErrors == -1) {
             const char* meEnv = getenv("NCCL_TESTS_MAX_ERRORS");
             maxErrors = (meEnv) ? atoi(meEnv) : 50;
           }
 
           for (size_t j = 0; j < totalFloats && (maxErrors == 0 || errCount < maxErrors); j++) {
             float gotVal = dataHost[j];
             float expVal = expectedHost[j];
             // Bitwise compare — catches NaN vs valid and any mismatch
             uint32_t gotBits, expBits;
             memcpy(&gotBits, &gotVal, sizeof(uint32_t));
             memcpy(&expBits, &expVal, sizeof(uint32_t));
             if (gotBits != expBits) {
               // Decode position in recv buffer
               int srcRank    = (sendcount > 0) ? (int)(j / sendcount) : 0;
               size_t offInRk = (sendcount > 0) ? j % sendcount : j;
               int channel    = (floats_per_ch > 0) ? (int)(offInRk / floats_per_ch) : 0;
               size_t offInCh = (floats_per_ch > 0) ? offInRk % floats_per_ch : offInRk;
 
               printf("  recvbuf[%zu]: srcRank=%d ch=%d chOff=%zu | got=",
                      j, srcRank, channel, offInCh);
 
               if (gotVal != gotVal) {  // NaN
                 printf("NaN(0x%08x,unwritten)", gotBits);
               } else {
                 int gr, gc;
                 decodePerChannelValue(gotVal, nCh, nranks, fillModeEnv, &gr, &gc);
                 printf("%.0f(r%d,ch%d)", gotVal, gr, gc);
               }
 
               int er, ec;
               decodePerChannelValue(expVal, nCh, nranks, fillModeEnv, &er, &ec);
               printf(" | exp=%.0f(r%d,ch%d)\n", expVal, er, ec);
 
               errCount++;
             }
           }
 
          if (maxErrors > 0 && (long long)wrongPerGpu[i] > maxErrors)
            printf("  ... and %lld more errors (showing first %d)\n",
                   (long long)wrongPerGpu[i] - maxErrors, maxErrors);
          else
          ...
          if (hist) {
            for (size_t j = 0; j < totalFloats; j++) {
              uint32_t gb, eb;
              memcpy(&gb, &dataHost[j], sizeof(uint32_t));
              memcpy(&eb, &expectedHost[j], sizeof(uint32_t));
              if (gb != eb) {
                int sr = (sendcount > 0) ? (int)(j / sendcount) : 0;
                size_t offInRk = (sendcount > 0) ? j % sendcount : j;
                int ch = (floats_per_ch > 0) ? (int)(offInRk / floats_per_ch) : 0;
            ...
            for (int sr = 0; sr < nranks; sr++) {
              for (int ch = 0; ch < nCh; ch++) {
                int64_t cnt = hist[sr * nCh + ch];
                if (cnt > 0) {
                  double pct = 100.0 * (double)cnt / (double)floats_per_ch;
                  printf("    src_rank=%d  ch=%d: %lld errors (%.1f%% of segment)\n",
                         sr, ch, (long long)cnt, pct);
                }
              }
            }
            free(hist);
        ...
        fflush(stdout);
         ...
         // Original debug print path (compile-time, non-per-channel types)
 ...
 testResult_t BenchTime(struct threadArgs* args, ncclDataType_t type, ncclRedOp_t op, int root, int in_place) {
   ...
   // --- Per-iteration validation mode (NCCL_TESTS_VALIDATE_EACH_ITER=1) ---
   static int validateEachIter = -1;
   if (validateEachIter == -1) {
     const char* env = getenv("NCCL_TESTS_VALIDATE_EACH_ITER");
     validateEachIter = (env && atoi(env) > 0) ? 1 : 0;
   }
 
   double cputimeSec = 0, deltaSec = 0;
   double algBw, busBw;
   int64_t wrongElts = 0;
 
   if (validateEachIter && datacheck) {
     static __thread int rep_v = 0;
     timer tim;
     ...
     for (int iter = 0; iter < iters; iter++) {
       ...
       int64_t iterWrong = 0;
       TESTCHECK(CheckData(args, type, op, root, in_place, &iterWrong));
       long long iterWrong1 = iterWrong;
   ...
   // --- Original performance benchmark path ---
   ...
   cputimeSec = tim.elapsed()/(iters*agg_iters);
   TESTCHECK(completeColl(args));
 
   deltaSec = tim.elapsed();
   ...
   } // end else (original benchmark path)
