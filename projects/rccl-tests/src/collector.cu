...
NicType NetCounterDetectNicType(const std::string& ib_device) {
  ...
  // The prefix is the reliable signal that this is a bnxt_re device
...
static void CollectEthtoolCounters(const std::string& nic,
  ...
  for (const auto& d : selected) {
    ...
    const char* ethtool_key = d.name.c_str(); // default: use canonical name
    if (nic_type == NIC_IONIC && e && e->ionic_key &&
        e->ionic_source == COUNTER_SRC_ETHTOOL) {
      ethtool_key = e->ionic_key;             // ionic: use ionic-specific key
    } else if (nic_type == NIC_BNXT_RE && e && e->bnxt_key) {
      ethtool_key = e->bnxt_key;              // bnxt_re: use bnxt-specific key
    }
...
static void CollectIbHwCounters(const std::string& ib_device,
  ...
  for (const auto& d : selected) {
    ...
    if (nic_type == NIC_IONIC) {
      ...
      // Try device-level if port-level not found (some counters are at device level on ionic)
...
void NetCounterPrintTable(
  ...
  printf("\nNET_COUNTER_TABLE: node=%s  rank=%d  duration_sec=%.3f  nic_type=%s\n",
         hostname, rank, dur_sec[0], NicTypeStr(nic_type));
  printf("%s\n", sep.c_str());
