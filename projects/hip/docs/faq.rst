...
What NVIDIA CUDA features can be ported to HIP?
-----------------------------------------------
...
pages describe which NVIDIA CUDA APIs can be automatically converted to HIP equivalents.
The :ref:`HIP runtime API reference<runtime_api_reference>` describes each HIP API and
its limitations, if any, compared with the equivalent NVIDIA CUDA API.
...
How easy is it to port NVIDIA CUDA code to HIP?
-----------------------------------------------

The `HIPIFY <https://github.com/ROCm/HIPIFY>`_ tools can automatically convert
almost all NVIDIA CUDA runtime code to HIP. Most device code needs no additional
conversion because HIP and NVIDIA CUDA have the same signatures for math and built-in
...
queries or NVIDIA CUDA capabilities that HIP doesn't support.

To better understand the syntax differences, see :doc:`NVIDIA CUDA to HIP API Function Comparison <reference/api_syntax>`
