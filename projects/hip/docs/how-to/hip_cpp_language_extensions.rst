...
``void __syncwarp()`` simply synchronizes all the threads in a warp.

``void __syncwarp(unsigned long long mask)`` simply synchronizes the threads
of a warp. If a mask is passed, then only threads which correspond to an on
bit lane will be synchronized. On AMD targets the mask is ignored and all
threads in the warp are synchronized.
