...
#endif
#define RC_PATH_MAX                                                            \
  ((PATH_MAX > 1024 && FILENAME_MAX > 1024                                     \
        ? 1024                                                                 \
        : (PATH_MAX < FILENAME_MAX ? PATH_MAX : FILENAME_MAX)) +               \
   1)
#endif
...
#define NULL_CHECK(ptr)                                                        \
  if (!ptr)                                                                    \
    return PathIncorrecPararmeters;
...
//  Argument1 (out) :  InstallPath (char** pointer which will return InstallPath
found)
//  Argument2 (out) :  InstallPathLen (Pointer to integer (size of InstallPath)
returned)
//  Usage :
...
*/
PathErrors_t getROCmInstallPath(char **InstallPath,
                                unsigned int *InstallPathLen) {
...
static int getROCmBase(char *buf) {
  ...
  if ((envStr = getenv("ROCM_PATH"))) {
    /* User space override, essentially just copied through as long as it is not
     * too long */
    len = strlen(envStr);
    if (len > 0) {
      if (envStr[len] == '/') {
