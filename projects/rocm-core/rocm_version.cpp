...
#define NULL_CHECK(ptr)                                                        \
  if (!ptr)                                                                    \
    return VerIncorrecPararmeters;

#define CHECK_AND_REPORT_API_RESULT(val)                                       \
  do {                                                                         \
    if (VerSuccess != val) {                                                   \
      const char *ErrStrings[VerErrorMAX] = {                                  \
          "VerSuccess", "VerIncorrecPararmeters", "VerValuesNotDefined"};      \
...
static VerErrors getBuildInfoLen(int *InfoStrlen) {
  ...
  *InfoStrlen =
      1 + strlen(ROCM_BUILD_INFO); // additional char for null termination
#else
