...
class TestSelectTags:
    ...
    def _tags(self, **kwargs):
        from rocm_ai_analysis.llm_analyzer import AnalysisContext, _select_tags
...
class TestFilterGuide:
    ...
    def _filter(self, guide, tags):
        from rocm_ai_analysis.llm_analyzer import _filter_guide
...
class TestAnalyzeWithLLMContextParam:
    ...
    def test_analyze_source_with_llm_context_filters_guide(self):
        from rocm_ai_analysis.llm_analyzer import AnalysisContext, LLMAnalyzer
        from unittest.mock import patch
...
class TestPublicExport:
    ...
    def test_analysis_context_importable_from_package(self):
        from rocm_ai_analysis import AnalysisContext

        ctx = AnalysisContext(tier=2)
        assert ctx.tier == 2

    def test_analysis_context_in_all(self):
        import rocm_ai_analysis as pkg
...
class TestEndToEndWithRealGuide:
    ...
    def _build_prompt(self, **ctx_kwargs):
        from rocm_ai_analysis.llm_analyzer import LLMAnalyzer, AnalysisContext
        from unittest.mock import patch

        guide = (
            LLMAnalyzer.__module__
            and __import__(
                "rocm_ai_analysis.llm_analyzer", fromlist=["get_reference_guide_path"]
            )
...
class TestGuideIntegrity:
    ...
    def _sections(cls):
        ...
        import re
        from rocm_ai_analysis.llm_analyzer import get_reference_guide_path
