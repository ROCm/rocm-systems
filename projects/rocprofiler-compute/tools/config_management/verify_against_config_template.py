...
- CDNA gfx9 configs use ``gfx9_config_template.yaml``; RDNA 3.5 gfx11** configs use
  ``gfx11_config_template.yaml`` (see ``--gfx11-template``).
...
Usage:
  python verify_against_config_template.py \\
      <analysis_configs_dir> <gfx9_template_yaml> \\
...
_GFX11_IP_RE = re.compile(r"^gfx11[0-9a-f]{2}$", re.IGNORECASE)


def is_gfx11_ip_variant(arch_name: str) -> bool:
    """True for RDNA 3.5 style IP dirs: ``gfx1100`` … ``gfx11ff``."""
    return bool(_GFX11_IP_RE.fullmatch(arch_name))
...
def main() -> None:
    ...
    parser.add_argument(
        "template_yaml",
        help="CDNA / gfx9 line template YAML (e.g. gfx9_config_template.yaml)",
    )
    parser.add_argument(
        "--gfx11-template",
        default=None,
    ...
    arch_names = sorted(
        p.name for p in configs_dir.iterdir() if p.is_dir() and p.name.startswith("gfx")
    )
    needs_gfx11 = any(is_gfx11_ip_variant(a) for a in arch_names)
    gfx11_panels: list[TemplatePanel] = []
    gfx11_by_id: dict[int, TemplatePanel] = {}
    if needs_gfx11:
        if not gfx11_template_path.is_file():
            print(
                f"Error: gfx11** architecture(s) present but template missing: "
