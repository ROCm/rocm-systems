...
function(configure_pkg PACKAGE_NAME_T COMPONENT_NAME_T PACKAGE_VERSION_T
  ...
  if(DEBIAN)
    set(BUILD_DEBIAN_PKGING_FLAG
        ON
        CACHE BOOL "Internal Status Flag to indicate Debian Packaging Build"
              FORCE)
    ...
    configure_file("${CMAKE_SOURCE_DIR}/DEBIAN/copyright.in"
                   "${CMAKE_BINARY_DIR}/DEBIAN/copyright" @ONLY)
    ...
    configure_file("${CMAKE_SOURCE_DIR}/CHANGELOG.md"
                   "${CMAKE_BINARY_DIR}/DEBIAN/CHANGELOG.md" @ONLY)

    # Install Change Log
    find_program(DEB_GZIP_EXEC gzip)
    if(EXISTS "${CMAKE_BINARY_DIR}/DEBIAN/CHANGELOG.md")
      execute_process(
        COMMAND ${DEB_GZIP_EXEC} -f -n -9
...
# Set variables for changelog and copyright For Debian specific Packages
function(set_debian_pkg_cmake_flags DEB_PACKAGE_NAME_T DEB_PACKAGE_VERSION_T
         DEB_MAINTAINER_NM_T DEB_MAINTAINER_EMAIL_T)
  # Setting configure flags
  set(DEB_PACKAGE_NAME
      "${DEB_PACKAGE_NAME_T}"
      CACHE STRING "Debian Package Name")
  set(DEB_PACKAGE_VERSION
      "${DEB_PACKAGE_VERSION_T}"
      CACHE STRING "Debian Package Version String")
  set(DEB_MAINTAINER_NAME
      "${DEB_MAINTAINER_NM_T}"
      CACHE STRING "Debian Package Maintainer Name")
  set(DEB_MAINTAINER_EMAIL
      "${DEB_MAINTAINER_EMAIL_T}"
      CACHE STRING "Debian Package Maintainer Email")
  set(DEB_COPYRIGHT_YEAR
      "2025"
      CACHE STRING "Debian Package Copyright Year")
  set(DEB_LICENSE
      "MIT"
      CACHE STRING "Debian Package License Type")
  set(DEB_CHANGELOG_INSTALL_FILENM
      "CHANGELOG.md.gz"
      CACHE STRING "Debian Package ChangeLog File Name")
  ...
  set(DEB_TIMESTAMP
      "${TIMESTAMP_T}"
      CACHE STRING "Current Time Stamp for Copyright/Changelog")
  ...
  message(
    STATUS "DEB_CHANGELOG_INSTALL_FILENM : ${DEB_CHANGELOG_INSTALL_FILENM}")
endfunction()
