...
def instruction_pipeline(instruction: Optional[str]) -> Optional[str]:
    ...
    instruction itself, so it applies to every disassembled line, not only the
    offsets PC sampling landed on. Returns None for an instruction the table
    does not hold, leaving the type unset rather than guessing.
    """
    if not instruction:
        return None
    return _instruction_pipelines().get(instruction.split(maxsplit=1)[0])
...
def _instruction_pipelines() -> dict[str, str]:
    ...
    17,000 unordered entries; the lookup wants the inverse. It is generated by
