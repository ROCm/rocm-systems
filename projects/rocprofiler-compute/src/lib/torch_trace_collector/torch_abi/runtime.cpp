...
extern "C" std::uint64_t aoti_torch_abi_version();
...
constexpr const char* kAotiAbiVersionSymbol = "aoti_torch_abi_version";
constexpr const char* kThreadLocalDebugInfoGetSymbol =
    "_ZN3c1020ThreadLocalDebugInfo3getENS_13DebugInfoKindE";
// GNU build-ID note records use four-byte padding. Only file-backed PT_NOTE
// bytes that also lie inside PT_LOAD are parsed; malformed or differently
// aligned notes fail closed instead of being accepted as a supported runtime.
...
bool consume_segment_bytes(std::size_t amount, std::size_t segment_size, std::size_t& offset) noexcept
{
...
std::optional<GnuBuildId> mapped_build_id(const dl_phdr_info& object) noexcept
{
    ...
    for (std::size_t index = 0; index < object.dlpi_phnum; ++index)
    {
        ...
        // dl_iterate_phdr exposes mapped addresses as integer ElfW(Addr) values.
...
{
    const auto torch_cpu_build_id = provider_build_id(kAotiAbiVersionSymbol);
    ...
    const auto found = std::find_if(kSupportedRuntimes.begin(),
    ...
    return found != kSupportedRuntimes.end() && aoti_torch_abi_version() == found->aoti_abi;
}
