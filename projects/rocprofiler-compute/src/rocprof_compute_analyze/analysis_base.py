...
class OmniAnalyze_Base:
    ...
    def join_prof(
        ...
        reader.materialize_pmc_perf(workload_dir, output_file)
        return None
    ...
    def join_workload_csvs(self, workload_dir: Path) -> None:
        ...
        if args.nodes is not None or args.spatial_multiplexing:
            ...
            for subdir in workload_dir.iterdir():
                if subdir.is_dir():
                    self.prepare_profile_data_directory(subdir)
        else:
            # Regular single-node case: CSV files are in workload_dir directly
            self.prepare_profile_data_directory(workload_dir)
    ...
    def prepare_profile_data_directory(self, directory: Path) -> None:
        ...
        elif not pmc_perf.exists():
            console_log(f"Joining results_*.csv for {directory}...")
            reader.materialize_pmc_perf(directory, pmc_perf)
