...
# Data type value supports bitwise checks:
# 1 - only Peak VALU Ops supported
# 2 - only Matrix Ops supported
# 3 - VALU and Matrix operations supported

SUPPORTED_DATATYPES: dict[str, dict[str, int]] = {
    "gfx90a": {
        "FP16": 3,
        "BF16": 2,
        "FP32": 3,
...
def calc_ceilings(
    ...
    peak_ops = 0.0
    if SUPPORTED_DATATYPES[mspec.gpu_arch][dtype] & 1:
        try:
            peak_ops = float(
    ...
    for cache_level in cache_hierarchy:
        ...
        peak_matrix = 0.0
        if SUPPORTED_DATATYPES[mspec.gpu_arch][dtype] & 2:
    ...
    #                                     Plot computing roof
    # ----------------------------------------------------------------------------------
    if (SUPPORTED_DATATYPES[mspec.gpu_arch][dtype] & 1) and x2 > 0:
        # Plot FMA roof
    ...
    if (SUPPORTED_DATATYPES[mspec.gpu_arch][dtype] & 2) and x2_matrix > 0:
        x0_matrix = (
