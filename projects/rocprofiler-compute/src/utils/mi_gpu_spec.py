...
class MIGPUSpecs:
    ...
    def get_num_dies(cls, gpu_arch: str, gpu_model: str) -> int:
        """
        Some gpu models' architecture has larger physical AIDs but through software
        divides the physical AIDs into distinct logical AID partitions.
        """
        if cls.get_gpu_series(gpu_arch) == "navi3":
