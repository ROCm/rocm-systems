...
def _make_script_path(tmp_path: Path) -> Path:
...
def _create_prebuilt_library(base_path: Path) -> Path:
...
class TestNativeTool:
    def test_uses_prebuilt_library_when_found(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Pre-built .so found via glob; no build command is issued."""
        fake_script = _make_script_path(tmp_path)
        monkeypatch.setattr(sys, "argv", [str(fake_script)])
        _create_prebuilt_library(tmp_path)
    ...
    def test_build_command_references_sdk_library_path(
        ...
        with (
            ...
            NativeTool().get_collector_library_path(
                rocprofiler_sdk_tool_path=sdk_tool_path
            )
    ...
    def test_uses_current_directory_as_base_when_script_path_is_shallow(
        ...
        monkeypatch.setattr(sys, "argv", ["/rocprof-compute"])
