...
def profile_artifact_files(output_dir):
    """Return text-searchable profile artifact paths under ``out/``.

    Deliberately excludes the rocpd ``.db`` files: callers scan these for counter
    names as UTF-8 text, and a binary SQLite file raises UnicodeDecodeError. The
    counter names live in the native counter CSV lane anyway.
    """
