 ...
 *   -  (/)
 * 
 * Build: hipcc -o _test main.cpp _mega_kernel.hip --offload-arch=<arch>
 *
 */
...
void print_usage(const char* prog_name) {
    ...
    printf("                         Must be power of 2, 64-1024\n");
...
TestConfig parse_args(int argc, char** argv) {
    ...
    int opt;
    int option_index = 0;
    while ((opt = getopt_long(argc, argv, "b:t:m:vh", long_options, &option_index)) != -1) {
        switch (opt) {
            case 'b':
            ...
            case 't':
                ...
                // Validate block size (must be power of 2, 64-1024)
                if (config.block_size < 64 || config.block_size > 1024 || 
                    (config.block_size & (config.block_size - 1)) != 0) {
                    fprintf(stderr, "Warning: invalid block_size, using default: %d\n", DEFAULT_BLOCK_SIZE);
                    config.block_size = DEFAULT_BLOCK_SIZE;
                }
                break;
            case 'm':
...
struct TestResults {
    ...
    // Cluster group operations ( specific)
    ...
    // Cooperative Atomic operations ( specific)
    int coop_atomic_load_passed;   // cooperative_atomic_load
    int coop_atomic_store_passed;  // cooperative_atomic_store
...
// External declaration of the kernel
extern "C" __global__ void _mega_kernel(
    TestResults* results,
    float* global_float,
...
void print_test_result(const char* test_name, int passed, int expected = 1) {
    // Width of 48 to accommodate longest test name: "Async LDS Store (global_store_async_from_lds)"
    if (passed < 0) {
...
static int g_arch_type = 0;  // 0=unknown, 1=gfx90a, 2=gfx942, 3=gfx950, 4=gfx1200, 5=gfx1201, 6=, 7=gfx1150/gfx1151

void detect_architecture(const char* arch) {
    strncpy(g_arch_name, arch, sizeof(g_arch_name) - 1);
    
    if (strstr(arch, "") || strstr(arch, "")) {
        g_arch_type = 6;  // /
        ...
        g_arch_type = 0;  // Unknown
    }
}

const char* get_arch_description() {
    switch (g_arch_type) {
...
int main(int argc, char** argv) {
    ...
    // Buffer sizes for atomic tests - must accommodate kernel indexing patterns
    // Kernel accesses: global_int[tid % 64 + offset] where offset can be up to 192
    // Maximum index: 64 + 192 = 256
    const int GLOBAL_INT_SIZE = 256;    // For atomic tests (was 16, too small!)
    const int GLOBAL_FLOAT_SIZE = 256;  // For atomic tests (was 8, too small!)
    const int GLOBAL_DOUBLE_SIZE = 64;  // For atomic tests (was 8, too small!)
    ...
    const char* mfma_mode_str = (config.mfma_mode == MFMA_MODE_ASM) ? "asm" :
                                 (config.mfma_mode == MFMA_MODE_BUILTIN) ? "builtin" : "both";
    ...
    HIP_CHECK(hipMemset(d_global_float, 0, GLOBAL_FLOAT_SIZE * sizeof(float)));
    HIP_CHECK(hipMemset(d_global_double, 0, GLOBAL_DOUBLE_SIZE * sizeof(double)));
    HIP_CHECK(hipMemset(d_global_int, 0, GLOBAL_INT_SIZE * sizeof(int)));
    HIP_CHECK(hipMemcpy(d_input, h_input, BUFFER_SIZE * sizeof(float), hipMemcpyHostToDevice));
    ...
    print_test_result("Warp Shuffle (shfl/shfl_xor/shfl_up/down)", h_results.warp_shuffle_passed);
    print_test_result("Warp Ballot (__ballot/__any/__all)", h_results.warp_ballot_passed);
    print_test_result("Warp Permute (ds_bpermute/ds_permute)", h_results.warp_permute_passed);
    print_test_result("Warp Reduce (shuffle-based reduction)", h_results.warp_reduce_passed);
    ...
    print_test_result("FP32 Arithmetic (add/sub/mul/div/fma)", h_results.fp32_arith_passed);
    print_test_result("FP64 Arithmetic (add/sub/mul/div/fma)", h_results.fp64_arith_passed);
    ...
    print_test_result("Atomic Add FP64 (HW atomic on CDNA)", h_results.atomic_add_f64_passed);
    ...
    print_test_result("Dual-Issue Patterns (VOPD/cross-wave)", h_results.dual_issue_passed);
    ...
    print_test_result("Async LDS Load (global_load_async_to_lds)", h_results.async_lds_load_passed);
    print_test_result("Async LDS Store (global_store_async_from_lds)", h_results.async_lds_store_passed);
    
    
    printf("\n[Category: Cluster Group Operations ( only)]\n");
    
    printf("\n[Category: WMMA Operations (RDNA4/gfx12 - Wave32)]\n");
    print_test_result("WMMA FP16 → FP32 (wmma_f32_16x16x16_f16)", h_results.wmma_f16_passed);
    print_test_result("WMMA BF16 → FP32 (wmma_f32_16x16x16_bf16)", h_results.wmma_bf16_passed);
    print_test_result("WMMA INT8 → INT32 (wmma_i32_16x16x16_iu8)", h_results.wmma_i8_passed);
    
    printf("\n[Category: Cooperative Atomic Operations ( only)]\n");
    print_test_result("Cooperative Atomic Load (32x4B/16x8B/8x16B)", h_results.coop_atomic_load_passed);
    print_test_result("Cooperative Atomic Store (32x4B/16x8B/8x16B)", h_results.coop_atomic_store_passed);
    
    printf("\n[Category: VMEM (Vector Memory) Operations - Inline ASM]\n");
    print_test_result("Flat Load/Store (flat_load_b32/flat_store_b32)", h_results.vmem_flat_passed);
    print_test_result("Global Load/Store (global_load_b32/global_store_b32)", h_results.vmem_global_passed);
    ...
    print_test_result("Global FP32 Atomics (global_atomic_add_f32)", h_results.atomic_global_f32_passed);
    print_test_result("Global FP64 Atomics (global_atomic_add_f64/min/max)", h_results.atomic_global_f64_passed);
    print_test_result("Flat INT Atomics (flat_atomic_add)", h_results.atomic_flat_int_passed);
    print_test_result("DS INT Atomics (ds_add_u32/min_i32/max_u32)", h_results.atomic_ds_int_passed);
    print_test_result("DS FP32 Atomics (ds_add_f32/min_f32)", h_results.atomic_ds_f32_passed);
    print_test_result("DS FP64 Atomics (ds_add_f64)", h_results.atomic_ds_f64_passed);
    print_test_result("Packed FP16 Atomics (global_atomic_pk_add_f16)", h_results.atomic_pk_f16_passed);
    print_test_result("Packed BF16 Atomics (global_atomic_pk_add_bf16)", h_results.atomic_pk_bf16_passed);
    ...
    #define COUNT_TEST(result) \
        if ((result) > 0) passed_categories++; \
        else if ((result) < 0) bypassed_categories++; \
        else failed_categories++;
    ...
    if (memory_verified) passed_categories++; else failed_categories++;
    
    #undef COUNT_TEST
    
    int total_tests = passed_categories + failed_categories;  // Exclude bypassed from total
    ...
    switch (g_arch_type) {
        ...
        case 3:  // MI350/CDNA4
            ...
            printf("  - Async LDS (global_load_async_to_lds, global_store_async_from_lds)\n");
        ...
        case 5:  // RX 9070 XT/RDNA4
            ...
            printf("  - Ray Tracing 2.0 hardware\n");
        ...
        case 6:  // /
            ...
            printf("      - FP16/BF16 → FP32, INT8 → INT32\n");
            printf("      - 2x throughput vs RDNA3 (1024 FLOPS/clock for FP16/BF16)\n");
            printf("      - TENSOR_LOAD_TO_LDS / TENSOR_STORE_FROM_LDS\n");
            printf("  - Async LDS (global_load_async_to_lds, global_store_async_from_lds)\n");
            printf("  - Dual-Issue VALU: 2 instructions/cycle\n");
            printf("  - Workgroup Clusters: Up to 12 workgroups per cluster, 15 clusters per SE\n");
            printf("      - Cluster barrier: s_barrier_signal/wait(-3)\n");
            printf("      - Cluster broadcast loads: CLUSTER_LOAD_B{32,64,128}\n");
            printf("  - Wave32 optimized operations\n");
            printf("  - Ray Tracing 2.0 hardware\n");
            break;
            
        case 7:  // Strix/Strix Halo (RDNA 3.5)
            printf("  - RDNA 3.5 (Navi3-based) iGPU: Strix Point (gfx1150) / Strix Halo (gfx1151)\n");
