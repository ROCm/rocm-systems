...
### RDNA 3.5 (Strix / Strix Halo APU iGPU)

| GPU Series | Architecture | GFXIP | Key Features |
|------------|--------------|-------|--------------|
| **Strix Halo** | RDNA 3.5 | gfx1151 | 40 CUs, WMMA, Wave32, Async LDS, 32MB Infinity Cache |
| **Strix Point** | RDNA 3.5 | gfx1150 | 16 CUs, same feature set as gfx1151 |
...
| Feature | MI250 (gfx90a) | MI300 (gfx942) | MI350 (gfx950) | Strix (gfx1150/51) | RX 9070 XT (gfx1201) |  () |
|---------|:--------------:|:--------------:|:--------------:|:-------------------:|:--------------------:|:---------------:|
| FP32 Arithmetic | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
...
| Workgroup Clusters | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ |
| Ray Tracing (RT) | ❌ | ❌ | ❌ | ✅ | ✅ (RT 2.0) | ✅ (RT 2.0) |
| AI Inference Accel. | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ |
| MFMA Matrix Ops | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ |
| WMMA Matrix Ops | ❌ | ❌ | ❌ | ✅ (gfx12) | ✅ (gfx12) | ✅ (gfx12) |
| Packed FP16 Atomics | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| Packed BF16 Atomics | ❌ | ✅ | ✅ | ✅ | ✅ | ✅ |
| VMEM Inline ASM | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
...
| 12 | Cluster Operations | 2 | Cluster barrier, info ( only) |
...
- **FP8/BF8 Conversions** ( specific scaled conversions)
...
- Atomic Add FP64 (`unsafeAtomicAdd` - CDNA/ only)
...
| **DS FP64** | `ds_add_f64/min_f64/max_f64` | LDS FP64 atomics (CDNA/ only) |
...
- `global_load_async_to_lds_b8/b32/b64/b128` - Asynchronous load from global to LDS
- `global_store_async_from_lds_b8/b32/b64/b128` - Asynchronous store from LDS to global
- Uses `__builtin_amdgcn_global_load_async_to_lds_b*` and `__builtin_amdgcn_global_store_async_from_lds_b*`

- `TENSOR_LOAD_TO_LDS` - Load N-dimensional tensor data from global to LDS
- `TENSOR_STORE_FROM_LDS` - Store N-dimensional tensor data from LDS to global
- Uses `__builtin_amdgcn_tensor_load_to_lds_d2` and `__builtin_amdgcn_tensor_store_from_lds_d2`
- Supports up to 5-dimensional tensor operations
- **Only available on  ()** - bypassed on other architectures
...
**Cross-Wave Dual-Issue** - MI350 (gfx950) and  () only:
MI350 and  can issue 2 VALU instructions from different waves per cycle under certain conditions:
...
### Workgroup Cluster Operations ( specific)
Multiple workgroups launched to same shader engine for cooperative execution ( only):

**Cluster Configuration:**
- Up to 15 clusters per SE, up to 12 workgroups per cluster
- ClusterID: 0 = not in cluster, 1-15 = cluster ID

**Cluster Synchronization:**
- `S_BARRIER_SIGNAL(-3)` / `S_BARRIER_WAIT(-3)`: Cluster barrier
- Uses `__builtin_amdgcn_s_barrier_signal(-3)` and `__builtin_amdgcn_s_barrier_wait(-3)`

**Cluster Broadcast Loads:**
- `CLUSTER_LOAD_B{32,64,128}`: Shared memory loads across cluster
- Multiple workgroups request same data, coalesced into single request
...
##  (/) Specific Features
 ...
 is the datacenter variant (), combining the best of RDNA4 with datacenter-specific enhancements:

1. **All RDNA4 Consumer Features**
   - FP8/BF8 OCP format support
   - Async LDS operations
   - Wave32 native execution
   - Ray Tracing 2.0
   - WMMA 16x16x16 matrix operations (FP16/BF16/INT8)

   - Dedicated DMA unit for tensor operations
   - Supports 5D tensor data movement
   - `TENSOR_LOAD_TO_LDS` / `TENSOR_STORE_FROM_LDS` instructions
   - Accelerates VMEM<->LDS/VGPR transfers

3. **Dual-Issue VALU**
   - 2 VALU instructions per cycle (same as MI350)
   - FP64 + FP32 co-execution patterns
   - 1024 VGPRs per Wave32

4. **Workgroup Clusters**
   - Up to 15 clusters per SE, up to 12 workgroups per cluster
...
5. **Full FP64 Support**
   ...
   - Full rate FP64 arithmetic (unlike consumer RDNA4)

6. **FP6/FP4 Support**
   - Extended low-precision format support
   - Same as MI350 (gfx950)

## Strix / Strix Halo (RDNA 3.5 / gfx1150 / gfx1151) Specific Features

RDNA 3.5 is the integrated GPU in Strix Point (gfx1150, 16 CUs) and Strix Halo (gfx1151, up to 40 CUs) APUs (Navi3-based):
...
- ROCm 6.0+ with HIP
- hipcc compiler
- AMD GPU (MI350/MI300/MI250 series)

### Using Make

```bash
# Build for MI350 (gfx950 - default)
make
...
# For  () - requires ROCm 7.x+
hipcc -O2 -std=c++17 --offload-arch= -o mega_kernel_test main.cpp mega_kernel.hip

# For Strix Halo (RDNA 3.5)
hipcc -O2 -std=c++17 --offload-arch=gfx1151 -o mega_kernel_test main.cpp mega_kernel.hip
```
...
| `-t, --block-size <N>` | Set thread block size (power of 2, 64-1024) | 256 |
...
### Sample Output (gfx1201 - RX 9070 XT)
...
Device Information:
  ...
  ...
================================================================================
Detected Platform:       RX 9070 XT/9070 (RDNA4/gfx1201)
================================================================================

Running GPU Mega Kernel for RX 9070 XT/9070 (RDNA4/gfx1201)...
...
| FP6/FP4 Support | No | No | Yes | No | No | Yes |
| WMMA Wave64 | Yes | Yes | No (Wave32) | No | No | No |
| VGPRs per Wave32 | 512 | 512 | 1024 | 512 | 512 | 1024 |
| Hardware FP64 Atomics | Add | Add, Min, Max | Add, Min, Max | No | No | Add, Min, Max |
...
| Async LDS | No | Limited | Yes | Yes | Yes | Yes |
| Workgroup Clusters | No | No | No | No | No | Yes |
| Ray Tracing | No | No | No | Yes | RT 2.0 | RT 2.0 |
| MFMA Matrix Ops | Yes | Yes | Yes | No | No | No |
| WMMA Matrix Ops | No | No | No | Yes | Yes | Yes |
