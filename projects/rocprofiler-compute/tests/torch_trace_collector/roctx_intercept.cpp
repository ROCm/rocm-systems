...
{
    // The collector callback remains registered until process exit.
...
extern "C" TORCH_TRACE_TEST_EXPORT void torch_trace_test_fail_next_push()
{
    auto&                             state = capture_state();
    const std::lock_guard<std::mutex> lock{state.mutex};
    state.fail_next_push = true;
}
