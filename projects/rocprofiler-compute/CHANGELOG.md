...
* Stale rocprofiler-sdk output is now removed before a PC sampling-only run (`--block 21`), so re-profiling into an existing workload directory no longer mixes in results from a previous run
