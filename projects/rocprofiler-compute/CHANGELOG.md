...
* Removed the `--join-type` profile mode option. It only ever selected between the `kernel` and `grid` variants of the CSV wide merge, which is gone, so both values had already become no-ops.
