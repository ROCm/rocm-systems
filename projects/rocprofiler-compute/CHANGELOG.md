...
* kokkos is currently supported only in rocprofv3 and not in rocprofiler-sdk. 
* Kokkos integration for `GFX950` uses GFX942 architecture as Kokkos upstream does not yet support this architecture.
* Real Kokkos builds will automatically fall back to mock implementation when `rocminfo` or `wmic` are not available.
...
* Added `--kokkos-trace` option to profiling  
...
* Fallback to mock Kokkos  when `rocminfo` or `wmic` are unavailable
