...
node_types = [
    ...
    {'label': 'Document', 'value': 'document', 'color': '#9b59b6', 'shape': 'rectangle'},
    ...
    {'label': 'Cloud', 'value': 'cloud', 'color': '#96ceb4', 'shape': 'round-rectangle'},
    {'label': 'Database', 'value': 'database', 'color': '#74b9ff', 'shape': 'barrel'},
    {'label': 'Actor', 'value': 'actor', 'color': '#fd79a8', 'shape': 'round-rectangle'}
]
...
app.layout = html.Div([
    # Header
    html.Div([
        html.H1("CyEditor - Dash/Cytoscape Flow Chart Editor", className="header-title"),
        
        # Main toolbar
        html.Div([
            html.Button("Add Node", id="add-node-btn", className="btn btn-primary"),
            html.Button("Add Edge", id="add-edge-btn", className="btn btn-secondary"),
            html.Button("Delete Selected", id="delete-btn", className="btn btn-danger"),
            html.Button("Clear All", id="clear-btn", className="btn btn-warning"),
            html.Button("Fit Graph", id="fit-btn", className="btn btn-info"),
            html.Button("Undo", id="undo-btn", className="btn btn-secondary", disabled=True),
            html.Button("Redo", id="redo-btn", className="btn btn-secondary", disabled=True),
        ], className="toolbar"),
        ...
        html.Div([
            ...
            html.Button("Export PNG", id="export-png-btn", className="btn btn-secondary"),
    ...
    html.Div([
        ...
        html.Div([
            ...
            html.Div([
                html.Div([
                    html.Div(
                        ...
                        id={'type': 'node-type', 'index': node_type['value']},
                        style={
            ...
            html.Div(id="properties-panel", children=[
                html.P("Select an element to edit properties")
            ])
        ...
        html.Div([
            ...
            html.Div(id="navigator-info", children=[
                html.P("Graph overview and navigation")
            ]),
...
def update_elements(add_clicks, delete_clicks, clear_clicks, upload_contents, 
                   tap_node_data, edge_mode, current_elements, selected_nodes, 
                   selected_edges, selected_node_type, current_state, undo_stack, 
                   redo_stack, edge_source):
    ctx = callback_context
    ...
    if trigger_id == 'cytoscape-graph' and tap_node_data and edge_mode:
        ...
        if edge_source is None:
            ...
            return current_elements, current_state, undo_stack, redo_stack, tapped_node_id
        else:
        ...
        # Add a new node
        node_type_info = next((nt for nt in node_types if nt['value'] == selected_node_type), node_types[1])
        ...
        new_node = {
            ...
            'position': {'x': 200 + (add_clicks * 50) % 400, 'y': 200 + (add_clicks * 50) % 400}
        }
        ...
        for element in current_elements:
            ...
            if 'source' in element['data'] and (
                element['data']['source'] in selected_ids or 
                element['data']['target'] in selected_ids
            ):
                continue
            filtered_elements.append(element)
        
        return filtered_elements, filtered_elements, new_undo_stack, new_redo_stack, edge_source
...
def display_selected_info(selected_nodes, selected_edges):
    ...
    if selected_nodes:
        ...
        for node in selected_nodes:
            info.append(html.P(f"ID: {node['id']}, Label: {node.get('label', 'No label')}, Type: {node.get('type', 'unknown')}"))
    ...
    if selected_edges:
        ...
        for edge in selected_edges:
            info.append(html.P(f"ID: {edge['id']}, From: {edge['source']} To: {edge['target']}"))
...
@app.callback(
    ...
    [Input('undo-stack', 'data'),
     Input('redo-stack', 'data')]
)
...
def handle_undo_redo(undo_clicks, redo_clicks, current_elements, undo_stack, redo_stack):
    ctx = callback_context
    if not ctx.triggered:
