...
PUBLIC_API hsa_status_t aqlprofile_spm_decode_accumulate_v0(
	...
	counter.resize(desc->num_events);
	...
	while (datain < data_end)
	{
		...
		for (int i = 0; i < desc->num_events; i++)
		{
			...
			if (is_global)
			{
				...
				if (bufvalue != 0xFFFF) counter_value += bufvalue;
			}
			else
			{
			...
			counter.at(i) = counter_value;
		}

		decode_cb(timestamp, counter.data(), counter.size(), userdata);
		datain += seg_elem;
	}
...
PUBLIC_API hsa_status_t aqlprofile_spm_decode_get_counter_map_v0(
    aqlprofile_spm_buffer_desc_t desc_bin,
    uint16_t**                   counter_map,
    size_t*                      counter_map_size
) {
	SpmBufferDesc* desc = (SpmBufferDesc*)desc_bin.data;
	if (desc->version != 1) return HSA_STATUS_ERROR_INVALID_ARGUMENT;

	*counter_map_size = desc->num_events;
	*counter_map = desc->get_counter_map();
