...
static void consumer(std::shared_ptr<class spm_state_t> s, aqlprofile_spm_data_callback_t callback,
                     void* userdata);
...
inline static hsa_status_t HsaSpmSetDestBuffer(spm_set_dest_buffer_args& args) {
  ...
  return hsa_amd_spm_set_dest_buffer(args.hsa_agent, args.buf_size, &args.timeout,
                                     &args.size_copied, args.dest_buf, &args.is_data_loss);
}
...
counter_des_t GetCounter(aql_profile::Pm4Factory* pm4_factory, const aqlprofile_pmc_event_t& event,
...
pm4_builder::counters_vector CountersVec(const aqlprofile_pmc_event_t* events, size_t num_events,
...
hsa_status_t _internal_aqlprofile_spm_create_packets(aqlprofile_handle_t* handle,
  ...
  s->memory = std::make_unique<SPMMemoryManager>(
      profile.aql_agent, profile.hsa_agent, profile.alloc_cb, profile.dealloc_cb, profile.userdata);
...
PUBLIC_API hsa_status_t aqlprofile_spm_create_packets(aqlprofile_handle_t* handle,
                                                      aqlprofile_spm_buffer_desc_t* out_desc,
                                                      aqlprofile_spm_aql_packets_t* packets,
                                                      aqlprofile_spm_profile_t profile,
                                                      size_t flags) {
  try {
    return aqlprofile::spm::_internal_aqlprofile_spm_create_packets(handle, out_desc, packets,
                                                                    profile, flags);
  } catch (...) {
...
PUBLIC_API hsa_status_t aqlprofile_spm_start(aqlprofile_handle_t handle,
...
static void consumer(std::shared_ptr<spm_state_t> s, aqlprofile_spm_data_callback_t callback,
                     void* userdata) {
  while (true) {
    std::unique_lock<std::mutex> lock(s->work_mutex);
...
PUBLIC_API bool aqlprofile_spm_is_event_supported(aqlprofile_agent_handle_t agent,
