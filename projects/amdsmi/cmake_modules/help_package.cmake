...
function(generic_package)
    ...
    set(SHARE_INSTALL_PREFIX
        ...
        CACHE STRING
        "Tests and Example install directory"
    )

    if(
        CMAKE_COMPILER_IS_GNUCC
        AND CMAKE_CXX_COMPILER_VERSION VERSION_LESS 5.4.0
    )
    ...
    if(${ADDRESS_SANITIZER})
        ...
        set(CMAKE_C_FLAGS
            "${CMAKE_C_FLAGS} ${ASAN_COMPILER_FLAGS}"
            PARENT_SCOPE
        )
        set(CMAKE_CXX_FLAGS
            "${CMAKE_CXX_FLAGS} ${ASAN_COMPILER_FLAGS}"
            PARENT_SCOPE
        )
        set(CMAKE_EXE_LINKER_FLAGS
            "${CMAKE_EXE_LINKER_FLAGS} ${ASAN_LINKER_FLAGS}"
            PARENT_SCOPE
        )
        ...
        set(CMAKE_EXE_LINKER_FLAGS
            "${CMAKE_EXE_LINKER_FLAGS} ${HARDENING_LDFLAGS}"
            PARENT_SCOPE
        )
        set(CMAKE_SHARED_LINKER_FLAGS
            "${CMAKE_SHARED_LINKER_FLAGS} ${HARDENING_LDFLAGS}"
            PARENT_SCOPE
        )
    ...
    if(CMAKE_CXX_COMPILER_ID STREQUAL "Clang")
        ...
        set(CMAKE_SHARED_LINKER_FLAGS
            "${CMAKE_SHARED_LINKER_FLAGS} -Wl,--build-id=sha1"
            PARENT_SCOPE
        )
    endif()
    ...
    set(CPACK_VERBATIM_VARIABLES
        ON
        CACHE BOOL
        "Escape strings passed to CPACK."
    )
    ...
    message(
        "Using CPACK_DEBIAN_PACKAGE_RELEASE ${CPACK_DEBIAN_PACKAGE_RELEASE}"
    )
...
function(generic_package_post)
    ...
    cpack_add_component(
        dev
        GROUP runtime
        DESCRIPTION "Development components of the library"
    )
    ...
    cpack_add_component(
        runtime
        GROUP runtime
        DESCRIPTION "Runtime components of the library"
    )
    ...
    cpack_add_component(
        tests
        GROUP tests
        DESCRIPTION "Test components of the library"
    )
endfunction()
