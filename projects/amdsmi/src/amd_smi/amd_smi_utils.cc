...
auto smi_amdgpu_get_active_dkms_version(const smi_amdgpu_dkms_packages_t& packages,
                                        std::string_view loaded_version,
  ...
  if (loaded_version.empty() || loaded_version == "N/A") {
    return AMDSMI_STATUS_NOT_SUPPORTED;
  }

  const auto package = packages.find(std::string{loaded_version});
