...
__device__ __forceinline__ uint32_t QueuePairIONIC::reserve_sq(uint32_t num_wqes) {
  ...
  uint32_t my_sq_prod = __scoped_atomic_fetch_add(&sq.pos, num_wqes,
...
__device__ __forceinline__ void QueuePairIONIC::ring_doorbell(uint32_t pos) {
  ...
  uint64_t *lane_sq_dbreg = &sq.dbreg[__lane_id() * num_qtypes];
  uint64_t dbval = sq.dbval | (pos & sq.mask);
  __scoped_atomic_store_n(lane_sq_dbreg, dbval, __ATOMIC_RELEASE, __MEMORY_SCOPE_SYSTEM);
}
