...
function(catch_discover_tests_compile_time_detection TARGET TEST_SET)
    ...
    set(ctest_include_file
        "${CMAKE_CURRENT_BINARY_DIR}/${TEST_SET}_include-${args_hash}.cmake"
    )
    set(ctest_tests_file
        "${CMAKE_CURRENT_BINARY_DIR}/${TEST_SET}_tests-${args_hash}.cmake"
    )
    ...
    foreach(EXE_NAME ${TARGET})
        add_custom_command(
            ...
            POST_BUILD
            COMMAND
                "${CMAKE_COMMAND}" -D "TEST_TARGET=${EXE_NAME}" -D
                "TEST_EXECUTABLE=$<TARGET_FILE:${EXE_NAME}>" -D
                ...
                "TEST_PREFIX=${_TEST_PREFIX}" -D "TEST_SUFFIX=${_TEST_SUFFIX}"
                -D "TEST_LIST=${_TEST_LIST}" -D "TEST_REPORTER=${_REPORTER}" -D
                "TEST_OUTPUT_DIR=${_OUTPUT_DIR}" -D
                "TEST_OUTPUT_PREFIX=${_OUTPUT_PREFIX}" -D
                "TEST_OUTPUT_SUFFIX=${_OUTPUT_SUFFIX}" -D
    ...
    if(NOT ${CMAKE_VERSION} VERSION_LESS "3.10.0")
        ...
        set_property(
            DIRECTORY
            APPEND
            PROPERTY TEST_INCLUDE_FILES "${ctestincludepath}"
        )
    else()
...
function(catch_discover_tests TARGET)
    ...
    string(SUBSTRING ${args_hash} 0 7 args_hash)
    # Define rule to generate test list for aforementioned test executable
    set(ctest_include_file_build
        "${CMAKE_CURRENT_BINARY_DIR}/${TARGET}_include_build-${args_hash}.cmake"
    )
    set(ctest_include_file_install
        "${CMAKE_CURRENT_BINARY_DIR}/${TARGET}_include_install-${args_hash}.cmake"
    )
    ...
    file(
        RELATIVE_PATH
        ctest_include_rel_path
        ${CMAKE_CURRENT_BINARY_DIR}
        ${ctest_include_file_build}
    )
    file(
        RELATIVE_PATH
        ctest_file_rel_path
    ...
    if(NOT ${CMAKE_VERSION} VERSION_LESS "3.10.0")
        ...
        file(
            APPEND
            ${ctest_include_file_build}
            "set(ctestfilepath ${ctest_file_rel_path})\n"
        )
        file(
            APPEND
            ${ctest_include_file_build}
            "set(_CATCH_ADD_TEST_SCRIPT ${_CATCH_ADD_TEST_SCRIPT})\n"
        )
        file(
            APPEND
            ${ctest_include_file_build}
            "set(crosscompiling_emulator ${crosscompiling_emulator})\n"
        )
        file(
            APPEND
            ${ctest_include_file_build}
            "set(_PROPERTIES ${_PROPERTIES})\n"
        )
        file(
            APPEND
            ${ctest_include_file_build}
            "include(${CATCH_INCLUDE_PATH})\n"
        )
        # Add discovered tests to directory TEST_INCLUDE_FILES
        set_property(
            DIRECTORY
            APPEND
            PROPERTY TEST_INCLUDE_FILES "${ctest_include_rel_path}"
        )

        # write install time include file
        file(
            WRITE
            ${ctest_include_file_install}
            "set(_TARGET_EXECUTABLE ${TARGET})\n"
        )
        ...
        file(
            APPEND
            ${ctest_include_file_install}
            "set(ctestfilepath script/${ctest_tests_file_name})\n"
        )
        file(
            APPEND
            ${ctest_include_file_install}
            "set(_CATCH_ADD_TEST_SCRIPT script/CatchAddTests.cmake)\n"
        )
        file(
            APPEND
            ${ctest_include_file_install}
        ...
        file(
            APPEND
            ${ctest_include_file_install}
            "include(script/catch_include.cmake)\n"
        )
        ...
        set_property(
            ...
            APPEND
            PROPERTY
                G_INSTALL_CTEST_INCLUDE_FILES "${ctest_include_file_install}"
        )
    endif()
endfunction()
