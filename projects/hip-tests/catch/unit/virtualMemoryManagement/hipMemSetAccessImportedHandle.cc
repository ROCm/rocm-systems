 ...
 * allocation on the last device, seeds it and exports it to the Child Process over a
 * socket. The Child Process imports the handle, maps it into its own reserved address
 * range and grants *CPU* access to the imported mapping. The Child then reads the Parent's
 * data and writes back through the CPU mapping, and the Parent verifies the result.
 * Exporting from the last device rather than device 0 keeps a wrong pick of the exporting
 * GPU from being masked by everything agreeing.
 * ------------------------
