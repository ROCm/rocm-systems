...
HIP_TEST_CASE(Unit_HipTest_StreamCallbackLatch) {
  // <REVIEW HELPER> Covers normal completion and enforces the latch's
  // single-shot contract.
  StreamGuard stream(Streams::created);
  HipTest::StreamCallbackLatch latch(stream.stream());

  HIP_CHECK(latch.enqueue());
  REQUIRE(latch.wait_for(std::chrono::seconds(5)));
  REQUIRE(latch.complete());
  HIP_CHECK_ERROR(latch.enqueue(), hipErrorInvalidValue);
}

HIP_TEST_CASE(Unit_HipTest_StreamCallbackLatch_EnqueueFailure) {
  // <REVIEW HELPER> Capture mode provides a safe, deterministic enqueue error.
  // A fabricated invalid stream was tried first but the runtime dereferenced it.
  StreamGuard stream(Streams::created);
  HipTest::StreamCallbackLatch latch(stream.stream());
  HIP_CHECK(hipStreamBeginCapture(stream.stream(), hipStreamCaptureModeGlobal));

  HIP_CHECK_ERROR(latch.enqueue(), hipErrorStreamCaptureUnsupported);
  REQUIRE_FALSE(latch.complete());
  HIP_CHECK_ERROR(hipStreamEndCapture(stream.stream(), nullptr), hipErrorStreamCaptureInvalidated);
}

HIP_TEST_CASE(Unit_HipTest_BlockingContext_DestructorUnblocks) {
  // <REVIEW HELPER> Exercises assertion-unwind behavior: destruction must
  // release and join the callback even without an explicit unblock call.
  StreamGuard stream(Streams::created);

  {
    HipTest::BlockingContext context(stream.stream());
    HIP_CHECK(context.block_stream());
    REQUIRE(context.is_blocked());
  }

  HIP_CHECK(hipStreamSynchronize(stream.stream()));
}

HIP_TEST_CASE(Unit_HipTest_BlockingContext_RejectsSecondUse) {
  // <REVIEW HELPER> Multiple callbacks cannot share one blocker state safely,
  // so the helper explicitly rejects a second arm.
  StreamGuard stream(Streams::created);
  HipTest::BlockingContext context(stream.stream());

  HIP_CHECK(context.block_stream());
  HIP_CHECK_ERROR(context.block_stream(), hipErrorInvalidValue);
  context.unblock_stream();
  HIP_CHECK(hipStreamSynchronize(stream.stream()));
}

HIP_TEST_CASE(Unit_HipTest_BlockingContext_DoesNotBlockUnrelatedStream) {
  // <REVIEW HELPER> Use GPU event work rather than a second host callback;
  // callback dispatch itself may be serialized while the first callback blocks.
  StreamGuard blocked_stream(Streams::withFlags, hipStreamNonBlocking);
  StreamGuard independent_stream(Streams::withFlags, hipStreamNonBlocking);
  EventsGuard independent_event(1);
  HipTest::BlockingContext context(blocked_stream.stream());

  HIP_CHECK(context.block_stream());
  REQUIRE(context.is_blocked());
  HIP_CHECK(hipEventRecord(independent_event[0], independent_stream.stream()));
  HIP_CHECK(hipEventSynchronize(independent_event[0]));
  HIP_CHECK(hipEventQuery(independent_event[0]));

  context.unblock_stream();
  HIP_CHECK(hipStreamSynchronize(blocked_stream.stream()));
}
