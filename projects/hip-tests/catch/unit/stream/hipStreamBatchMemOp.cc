...
HIP_TEST_CASE(Unit_hipStreamBatchMemOp_SequentialOrdering_MultiGPU) {
  ...
  // Pop contexts in reverse creation order — hipCtxCreate pushes onto the per-thread
  // context stack, so the last-created context is on top and must be popped first.
