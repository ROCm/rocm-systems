...
HIP_TEST_CASE(Unit_hipGetProcAddress_MemoryApisMemset2D3D) {
  ...
  // Validating hipMemset2DAsync API
  {
    ...
    HIP_CHECK(dyn_hipMemset2DAsync_ptr(devMem, pitch, 5, width, height, nullptr))
  ...
  // Validating hipMemset3DAsync API
  {
    ...
    HIP_CHECK(dyn_hipMemset3DAsync_ptr(devMem, value, extent3d, nullptr))
...
HIP_TEST_CASE(Unit_hipGetProcAddress_MemoryApisManagedMemory) {
  ...
  // Validating hipMemAdvise and hipMemRangeGetAttribute APIs
  {
    ...
    hipDevice_t device = hipCpuDeviceId;
    HIP_CHECK(hipMemPrefetchAsync(memPtr, Nbytes, device, nullptr))
