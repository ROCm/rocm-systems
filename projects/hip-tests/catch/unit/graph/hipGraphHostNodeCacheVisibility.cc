 ...
 * Test Description
 * ------------------------
 * - Regression test for cache visibility of an upstream GPU node's writes to a
 * downstream graph host node's CPU callback. A host node's callback runs on the
 * CPU and may read host memory produced by an upstream GPU node. The runtime
 * must ensure the producer's writes are flushed past L2 and are visible to the
 * CPU before the callback executes.
 *
 ...
 * |
 * v (dependency)
 * copyNode      devProduced -> hostProduced (DtoH memcpy)
 * |
 * v (dependency)
 ...
 * and the visibility bug is deterministic on the fence scope, not on timing
 * (a NOP-scope release leaves the DtoH write invisible to the CPU regardless
 * of how long the callback waits).
 *
 * With a system-scope fence: the DtoH write is CPU-visible, so the callback
 * observes kProducedValue -> hostObserved == kProducedValue.
 * With a NOP-scope fence: the callback reads un-flushed/stale memory on GPUs
 * whose producer release is not host-visible -> hostObserved != kProducedValue.
 *
 * hostProduced is set to 0 on the host before launch, so if the DtoH write is
 * not flushed the callback reads back the host's own cached 0
...
HIP_TEST_CASE(Unit_hipGraphAddHostNode_CallbackSeesUpstreamDtoH) {
  ...
  // copyNode: DtoH devProduced -> hostProduced, depends on producerNode.
  ...
  // callbackNode: host callback reads the GPU-produced hostProduced.
