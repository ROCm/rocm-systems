...
static VulkanMipmapImage CreateVulkanMipmapImage(
  ...
  // Treat each texel as a single 32-bit element (matches the test's packed-uint32 PixelValue and
  // surf2Dread/write<uint32_t>). A 4x8 channel format would apply RGBA channel selection in the
...
HIP_TEST_CASE(Unit_hipVulkanImageDataVerification_Positive_SurfaceWrite_VulkanRead) {
  CHECK_IMAGE_SUPPORT
  auto err = hipGetLastError();  // Remove error of previously failed tests
  VulkanTest vkt(enable_validation);
  auto img = CreateVulkanMipmapImage(vkt);
  REQUIRE(img.valid());
...
// ── Single-level LINEAR image interop ───────────────────────────────────────────────────────────
// VK_IMAGE_TILING_LINEAR requires mipLevels == 1 (linear images cannot be mipmapped). This exercises
// the linear import path: Vulkan writes the whole image, HIP reads it back and round-trips via HIP.
HIP_TEST_CASE(Unit_hipVulkanImageDataVerification_Positive_Linear_RoundTrip) {
  ...
  std::vector<uint32_t> hip_pixels(N, 0);
  HIP_CHECK(hipMemcpyFromArray(hip_pixels.data(), hip_level, 0, 0, N * sizeof(uint32_t),
                               hipMemcpyDeviceToHost));
  for (uint32_t y = 0; y < H; ++y)
    for (uint32_t x = 0; x < W; ++x)
      REQUIRE(hip_pixels[y * W + x] == PixelValue(x, y, 0));

  // HIP inverts RGB of every pixel and writes back; Vulkan reads and verifies.
  for (auto& px : hip_pixels) {
    const uint8_t r = 255u - (px & 0xFFu), g = 255u - ((px >> 8) & 0xFFu),
                  b = 255u - ((px >> 16) & 0xFFu), a = (px >> 24) & 0xFFu;
    px = pack_rgba(r, g, b, a);
  }
  ...
  for (uint32_t y = 0; y < H; ++y)
    for (uint32_t x = 0; x < W; ++x) {
      ...
      const uint32_t expected = pack_rgba(static_cast<uint8_t>(255u - (orig & 0xFFu)),
                                          static_cast<uint8_t>(255u - ((orig >> 8) & 0xFFu)),
                                          static_cast<uint8_t>(255u - ((orig >> 16) & 0xFFu)),
                                          static_cast<uint8_t>((orig >> 24) & 0xFFu));
