...
Addition::Addition() {
  ...
  hipSynchronizationPolicy syncPolicy = hipSynchronizationPolicy::hipSyncPolicyYield;
  ...
  std::cout << __FUNCTION__ << " : Stream Sync Policy: " << valueOut.syncPolicy << std::endl;
}
...
void Addition::LaunchKernelForAddition(float *x, float *y, float *z, int N){
  ...
  //CHECK_HIP_ERROR(hipGetKernel(&function, VecAddKernel));

  hipFunction_t function;
  //CHECK_HIP_ERROR(hipGetFuncBySymbol(&function, "VecAddKernel"));
  CHECK_HIP_ERROR(hipGetFuncBySymbol(&function, reinterpret_cast<void *>(VecAddKernel)));

  ExecuteKernel(reinterpret_cast<hipKernel_t>(function), d_x, d_y, d_z, N, stream);
...
void Addition::getStream(hipStream_t &strm) {
  strm = stream;
}
