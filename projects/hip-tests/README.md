...
When building through cmake, levels work automatically. The build system generates `hip_test_parameters.hh` from `definitions.yaml`. The level is resolved from a strict priority order: an explicit `[level_N]` command-line filter (e.g., `./test "[level_0]"`), then the `HIP_TEST_LEVEL` environment variable, and finally the hardcoded default `level_2`. The resolved level's parameters are loaded into `TestParameterStore` by the event listener in `hip_test_level.cc`'s companion `hip_test_listener.cc`.

Only an *explicitly requested* level also narrows which test cases run, and that narrowing happens in `main()` rather than the listener, because Catch2 has already parsed the test spec by the time any listener event fires. The hardcoded default never narrows: a bare `./test` and plain `ctest` run every test case in the binary and simply use `level_2` parameters.

For standalone builds levels remain opt-in, and require compiling additional source files from `catch/hipTestMain/`. Add `hip_test_features.cc` as well if the test under build needs feature detection:

```bash
amdclang++                                                                           \
  -I $ROCM_SYSTEMS_DIR/projects/hip-tests/catch/external/picojson                    \
  -I $ROCM_SYSTEMS_DIR/projects/hip-tests/catch/include                              \
  -I <build_dir>/catch_tests/include                                                 \
  -x hip $ROCM_SYSTEMS_DIR/projects/hip-tests/catch/hipTestMain/main.cc              \
  -x hip $ROCM_SYSTEMS_DIR/projects/hip-tests/catch/hipTestMain/hip_test_context.cc  \
  ...
  -x hip $ROCM_SYSTEMS_DIR/projects/hip-tests/catch/hipTestMain/hip_test_level.cc    \
  -L /usr/local/lib                                                                  \
  --offload-arch=native                                                              \
  -lCatch2                                                                           \
  -x hip <path_to_test>
...
This requires `hip_test_parameters.hh` to have been generated by a prior cmake build (it lives in the build directory, under `catch_tests/include`). Without these additional files, standalone builds still work but all tests use hardcoded default parameters regardless of level.

Note that level-based *selection* additionally needs `-DENABLE_YAML_TAGS`, since it is compiled out of `main()` otherwise and the test cases carry no tags to match against. Parameters work without it.
