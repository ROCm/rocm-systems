...
kernel_replay_cb(rocprofiler_callback_tracing_record_t record, rocprofiler_user_data_t*, void*)
{
    ...
    // Counters on 0..N-2; PC sampling on the last pass only. Never both on one pass:
    // dispatch counting re-enables clock gating around the kernel, which can hang
    // MI2xx/MI3xx if PC sampling is also live.
...
configure_pcs()
{
    ...
    // Leave the context globally stopped. Kernel replay starts only the replaying agent
    // on the last pass, then restores the stopped state.
    return any;
}
