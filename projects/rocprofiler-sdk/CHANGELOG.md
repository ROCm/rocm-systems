...
- Bump rocpd schema to version 3.0.4 for PC sampling support (`rocpd_gpu_pc_sample`, blob tables `rocpd_info_blob_schema`/`rocpd_info_blob_field`/`rocpd_blob_event`, `rocpd_disassembly_data`, and `rocpd_event.parent_id`). The pre-PC-sampling 3.0.3 schema is now frozen under `versions/3.0.3/` per the rocpd schema versioning scheme. `rocpd_gpu_pc_sample.exec_mask` is stored as a `0x`-prefixed, 16-digit zero-padded hex string. The `Exec_Mask` column of the rocpd CSV export, along with the `--output-format csv` and `--output-format json` outputs, continues to report the mask as a decimal integer.
