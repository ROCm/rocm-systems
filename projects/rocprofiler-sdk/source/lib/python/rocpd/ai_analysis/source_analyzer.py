...
_SKIP_DIRS = frozenset({
    "build", "_build", ".build", "__pycache__", ".git", ".svn",
    "node_modules", "vendor", ".cache", "dist", "site-packages",
    ".tox", ".mypy_cache", ".pytest_cache", "CMakeFiles",
    "CMakeCache", ".deps", "third_party", "thirdparty", "extern",
})
...
def _pmc_commands(
    ...
    if derived and (len(derived) > 1 or regular):
        ...
        all_cmds = []
        if regular:
            all_cmds.extend(_pmc_commands(
                regular, flags, output_dir, output_prefix, description, app_placeholder
            ))
        for dc in derived:
...
_PATTERN_DEFS: List[_PatternDef] = [
    ...
    _PatternDef(
        "HIP_KERNEL_LAUNCH",
        re.compile(r'\bhipLaunchKernelGGL\s*\(\s*(?:\(void\s*\*\)\s*)?(\w+)', re.MULTILINE),
        "info", "GPU Kernels",
    ...
    _PatternDef(
        "RCCL",
        re.compile(r'\bncclAll\w+\s*\(|\brcclAll\w+\s*\(|\bncclBcast\s*\(|\brcclBcast\s*\(', re.MULTILINE),
        "info", "Multi-GPU",
    ...
    _PatternDef(
        "TORCH_CUDA_DEVICE",
        re.compile(r'\.cuda\(\)|\.to\(["\']cuda["\']|\.to\(device\s*=\s*["\']cuda["\']', re.MULTILINE),
        "info", "PyTorch",
...
_KERNEL_PATTERN_IDS = frozenset({
    "GLOBAL_KERNEL_DEF", "HIP_KERNEL_LAUNCH",
    "TRIPLE_ANGLE_LAUNCH", "HIP_KERNEL_NAME",
})

# IDs of ROCTx instrumentation patterns
_ROCTX_PATTERN_IDS = frozenset({
    "ROCTX_RANGE_PUSH", "ROCTX_RANGE_POP", "ROCTX_MARK",
})
...
class SourceAnalyzer:
    ...
    def _scan_file(self, path: Path) -> None:
        ...
        for pdef in _PATTERN_DEFS:
            ...
            self._pattern_counts[pdef.pattern_id] = (
                self._pattern_counts.get(pdef.pattern_id, 0) + len(matches)
            )
        ...
        for m in re.finditer(
            ...
            if name not in seen:
                ...
                self._detected_kernels.append(
                    DetectedKernel(name=name, file=rel_path, line=lineno,
    ...
    def _apply_synthetic_patterns(self) -> None:
        ...
        has_kernels = any(
            ...
            for pid in _KERNEL_PATTERN_IDS
        )
        ...
        if dev_sync_count > 5:
            ...
            self._pattern_locations["LOOP_DEVICE_SYNC"] = (
                self._pattern_locations.get("DEVICE_SYNC", [])[:5]
            )
    ...
    def _generate_profiling_commands(self, programming_model: str) -> List[Dict[str, Any]]:
        """
        ...
        kernel_names = [k.name for k in self._detected_kernels[:5]]
        ...
        if loop_sync > 0:
            ...
            locations_str = "; ".join(sync_locations) if sync_locations else "multiple locations"
            recommendations.append({
                "priority": "HIGH",
        ...
        if has_libs:
            lib_names = []
            if self._pattern_counts.get("ROCBLAS", 0) or self._pattern_counts.get("HIPBLAS", 0):
        ...
        if programming_model in ("PyTorch_HIP", "JAX_HIP", "Python_GPU"):
            fw = "PyTorch" if "Torch" in programming_model or programming_model == "PyTorch_HIP" else "JAX"
            recommendations.append({
                "priority": "HIGH",
                "category": "PyTorch",
                "issue": (
                ...
                "actions": [
                    f"Wrap your training/inference loop with torch.profiler.profile(activities=[ProfilerActivity.CUDA])",
                ...
                "commands": [
                    {
                        ...
                        "flags": ["--trace"],
                        "args": [],
                        "full_command": f"rocprof-sys --trace -- python ./train.py",
                    },
                    {
                        "tool": "rocprofv3",
