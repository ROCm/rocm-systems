...
- ``Exec_Mask``: Active SIMD lanes when sampled. Stored as a decimal integer in the
CSV and JSON outputs, and as "0x"-prefixed hex text in the ``rocpd`` output.
...
- ``rocpd_gpu_pc_sample``: One row per sample. Key columns:
  ...
  - ``exec_mask``: Active SIMD lanes at sample time, stored as TEXT holding a
    ``0x``-prefixed hex mask zero-padded to 16 digits
...
.. _pc_sampling_exec_mask_sql:

.. warning::

  ``exec_mask`` cannot be used numerically in SQL. SQLite parses leading decimal
  digits out of text but has no notion of the ``0x`` prefix, so
  ``CAST(exec_mask AS INTEGER)``, ``exec_mask + 0``, ``SUM(exec_mask)``, and
  ``WHERE exec_mask = 65535`` all return ``0`` or match nothing -- silently, with
  no error or warning. No portable workaround exists in pure SQL either, as
  ``unhex()`` requires SQLite 3.41 or later and returns a BLOB rather than an
  integer. Select the column as text and convert it in the consuming application,
  for example ``int(row["exec_mask"], 16)`` in Python, then perform lane-count or
  bit-test analysis there.
