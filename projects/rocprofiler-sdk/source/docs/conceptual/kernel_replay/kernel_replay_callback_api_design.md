...
Counter collection, SPM, and ATT consult the override at dispatch time. PC sampling consults it
when kernel replay starts or stops the **replaying agent** around each pass. Kernel dispatch
tracing drops disabled contexts from the pass's tracing data. Dispatch counters and PC sampling
must not collect on the same pass.
