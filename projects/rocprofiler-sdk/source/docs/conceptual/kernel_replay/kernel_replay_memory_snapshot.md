...
capture fails, and the limitations that follow from the capture mechanism.
...
HSA table so it uses the original, un-wrapped function. There is no hashing, no dirty-page diffing,
no chunking and no disk backing: the tracked footprint is duplicated in host RAM for the duration of
the replay, and every region is written back on every restore.
