...
| `printf` and hostcall buffers | Buffers the runtime allocates and names in hidden arguments; the host drains them when the application's dispatch completes | **Not handled.** Replayed passes execute the calls again. Output written to a buffer in device-local memory is discarded when the snapshot is restored; output in host memory is not, and can appear in a later drain |
