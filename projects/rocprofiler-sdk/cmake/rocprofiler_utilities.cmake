...
function(ROCPROFILER_ADD_NOLINK_INTERFACE_LIBRARY _TARGET _DESCRIPT)
    ...
    target_link_libraries(
        ...
        INTERFACE $<BUILD_INTERFACE:${_TARGET}>
    )
endfunction()
...
function(rocprofiler_custom_compilation)
    ...
    if(NOT COMP_COMPILER)
        message(
            FATAL_ERROR
            "rocprofiler_custom_compilation not provided COMPILER argument"
        )
    endif()
...
function(ROCPROFILER_PYTHON_CONSOLE_SCRIPT SCRIPT_NAME SCRIPT_SUBMODULE)
    ...
    if(ARG_VERSION AND ARG_ROOT_DIR)
        ...
        execute_process(
            COMMAND ${Python3_EXECUTABLE} -m pip install pandas
        )
        configure_file(
    ...
    if(EXISTS "${HSA_API_TRACE_HEADER}")
        ...
        foreach(_LINE ${HSA_API_TRACE_HEADER_CONTENTS})
            if("${_LINE}" MATCHES ".*define HSA_.*_(MAJOR|STEP)_VERSION .*")
                ...
                if(_VAR AND _VAL)
                    set(ROCPROFILER_${_VAR} "${_VAL}" PARENT_SCOPE)
                endif()
            endif()
        endforeach()
    endif()
...
function(rocprofiler_install_env_setup_files)
    ...
    cmake_parse_arguments(
        RIEF
        ""
        "NAME;VERSION;SOURCE_DIR;BINARY_DIR;INSTALL_DIR;COMPONENT"
        ""
        ${ARGN}
    )
    ...
    configure_file(
        ...
        ${RIEF_BINARY_DIR}/${RIEF_INSTALL_DIR}/${PACKAGE_NAME}/setup-env.sh
        @ONLY
    )
...
function(rocprofiler_add_unit_test)
    ...
    foreach(_VAR TARGET SOURCES)
        if(NOT RAUT_${_VAR})
            message(
                FATAL_ERROR
                "rocprofiler_add_unit_test missing required argument: ${_VAR}"
            )
        endif()
    endforeach()
    ...
    set_tests_properties(
        ${${RAUT_TEST_LIST}}
        PROPERTIES
            TIMEOUT ${RAUT_TIMEOUT}
            LABELS "${RAUT_LABELS}"
            FAIL_REGULAR_EXPRESSION "${RAUT_FAIL_REGULAR_EXPRESSION}"
            PASS_REGULAR_EXPRESSION "${RAUT_PASS_REGULAR_EXPRESSION}"
            SKIP_REGULAR_EXPRESSION "${RAUT_SKIP_REGULAR_EXPRESSION}"
            ENVIRONMENT "${RAUT_ENVIRONMENT}"
            DISABLED ${RAUT_DISABLED}
    )
    ...
    install(
        FILES
            ${CMAKE_CURRENT_BINARY_DIR}/${CMAKE_INSTALL_DATAROOTDIR}/${PACKAGE_NAME}/tests/unit-tests/${RAUT_TARGET}.cmake
        DESTINATION
            ${CMAKE_INSTALL_DATAROOTDIR}/${PACKAGE_NAME}/tests/unit-tests
        COMPONENT tests
    )
