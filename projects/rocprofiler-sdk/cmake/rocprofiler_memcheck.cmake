...
function(rocprofiler_add_memcheck_flags _TYPE _LIB_BASE _FLAG)
    ...
    target_link_options(
        rocprofiler-sdk-memcheck
        INTERFACE $<BUILD_INTERFACE:-fsanitize=${_FLAG} -Wl,--no-undefined>
    )
    ...
    execute_process(
        COMMAND
            ${PROJECT_SOURCE_DIR}/source/scripts/deduce-sanitizer-lib.sh
    ...
    if(_DEDUCE_RET EQUAL 0 AND EXISTS "${_DEDUCE_OUT}")
        set(${_TYPE}_LIBRARY
            "${_DEDUCE_OUT}"
            CACHE FILEPATH
            "Linked library when compiled with -fsanitize=${_FLAG}"
        )
    endif()
endfunction()
...
function(rocprofiler_set_memcheck_env _TYPE _LIB_BASE)
    ...
    if(${_TYPE}_LIBRARY)
        ...
        set(ROCPROFILER_MEMCHECK_PRELOAD_ENV_VALUE
            "${${_TYPE}_LIBRARY}"
            CACHE INTERNAL
            "Library to LD_PRELOAD for tests "
            FORCE
        )
    endif()
endfunction()
...
elseif(NOT ROCPROFILER_MEMCHECK STREQUAL "")
    message(
        FATAL_ERROR
        "Unsupported ROCPROFILER_MEMCHECK type: ${ROCPROFILER_MEMCHECK}"
    )
endif()
