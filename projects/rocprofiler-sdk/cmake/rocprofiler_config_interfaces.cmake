...
target_compile_definitions(
    rocprofiler-sdk-headers
    INTERFACE
        $<BUILD_INTERFACE:AMD_INTERNAL_BUILD=1>
...
if(EXISTS ${ROCPROFILER_DEFAULT_ROCM_PATH})
    ...
    if(
        NOT "${_ROCPROFILER_DEFAULT_ROCM_PATH}"
            STREQUAL
            "${ROCPROFILER_DEFAULT_ROCM_PATH}"
    )
        set(ROCPROFILER_DEFAULT_ROCM_PATH
...
if(pthread_LIBRARY)
    target_link_libraries(
        rocprofiler-sdk-threading
        INTERFACE ${pthread_LIBRARY}
    )
else()
    find_package(
        Threads
        ${rocprofiler_FIND_QUIETLY}
        ${rocprofiler_FIND_REQUIREMENT}
    )
...
if(rocm_version_FOUND)
    list(
        ...
        CMAKE_PREFIX_PATH
        "${rocm_version_DIR}"
        "${rocm_version_DIR}/llvm"
    )
    list(
        APPEND
        CMAKE_MODULE_PATH
        "${rocm_version_DIR}/hip/cmake"
...
if(PkgConfig_FOUND)
    ...
    target_link_libraries(
        rocprofiler-sdk-drm
        INTERFACE PkgConfig::DRM PkgConfig::DRM_AMDGPU
    )
else()
    find_path(
        drm_INCLUDE_DIR
        ...
        PATH_SUFFIXES include/drm include/libdrm include
        REQUIRED
    )
    ...
    target_link_libraries(
        rocprofiler-sdk-drm
        INTERFACE ${drm_LIBRARY} ${drm_amdgpu_LIBRARY}
    )
endif()
...
find_package(
    ...
    HINTS ${rocm_version_DIR} ${ROCM_PATH}
    PATHS ${rocm_version_DIR} ${ROCM_PATH}
    PATH_SUFFIXES lib/cmake/rccl
)

if(
    NOT ROCPROFILER_INTERNAL_RCCL_API_TRACE
    AND rccl_FOUND
...
if(
    ...
    target_compile_definitions(
        rocprofiler-sdk-rocjpeg-nolink
...
# ----------------------------------------------------------------------------------------#
#
# rocpdsna: next-generation rocpd writer/reader API
#
# ----------------------------------------------------------------------------------------#
