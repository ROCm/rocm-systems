.. meta::
   ...
   :keywords: hipfile, driver, lifecycle, reference counting, hipFileDriverOpen, hipFileDriverClose, hipFileUseCount, ROCm, GPU IO
...
- A reference count of **zero** means the driver is uninitialized. API calls that require an initialized driver return ``hipFileDriverNotInitialized``.
- A **positive** reference count means the driver is initialized and ready for normal use.
- When the reference count transitions from one to zero, the library destroys its internal state, clearing the ``FileMap``, ``BufferMap``, ``StreamMap``, and ``BatchContextMap``.
...
Internally, the ``DriverState::ensureInitialized()`` method sets the reference count to one if it is currently zero, guaranteeing that the driver is ready for use.
...
For the complete function signatures, see the GPU IO driver API group in :doc:`/reference/api-reference`.
...
Example: balanced open and close
********************************

The following example from the ``aiscp`` sample program demonstrates a typical pattern. Although ``aiscp`` relies on implicit initialization through ``hipFileHandleRegister()``, a fully explicit approach looks like this:
...
.. code-block:: cpp
   ...
   int main() {
       ...
       // Register files and buffers, perform IO...
...
``FileMap``
   Stores all registered file handles created by ``hipFileHandleRegister()``. Each entry maps a file descriptor and opaque handle to an internal ``File`` object containing file metadata (file descriptors, filesystem type, alignment requirements).
...
The ``BufferMap`` is cleared before the ``FileMap`` to ensure correct teardown ordering.
