...
Before building hipfile, ensure you have the following software installed:
...
.. note::
   ...
   Verify the exact package name for hipfile with the latest ROCm release documentation, as the group name may vary between releases.
...
hipfile requires the following libraries, which CMake locates during configuration:

- ``hsa-runtime64`` — HSA runtime (found via CMake config)
- ``hip`` — HIP runtime (found via CMake config)
- ``libmount`` — mount information parsing (from ``util-linux``)
...
.. code-block:: shell
   ...
   cd rocm-libraries/projects/hipfile
   cmake -B build \
...
For an **NVIDIA platform** build, set ``CMAKE_HIP_PLATFORM`` to ``nvidia`` and ensure
...
.. list-table::
   ...
   * - ``AIS_INSTALL_TOOLS``
     - ``ON``
     - Install tool programs (such as ``ais-stats``). Only applies to the AMD platform.
   * - ``AIS_BUILD_DOCS``
