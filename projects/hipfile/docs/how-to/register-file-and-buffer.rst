.. meta::
   :description: Step-by-step instructions for registering a file and GPU buffer for direct GPU IO using hipfile.
   :keywords: hipfile, GPU IO, hipFileHandleRegister, hipFileBufRegister, O_DIRECT, ROCm, register file, GPU buffer

=========================================
Register a file and GPU buffer for GPU IO
=========================================

This guide walks you through registering a file and a GPU memory buffer for direct GPU IO with hipfile. After registration, you can perform read and write operations directly between storage and GPU memory.
...
- You have a file to register for GPU IO.

.. note::

   Using ``O_DIRECT`` when opening files is recommended for optimal performance. hipfile reports the error ``hipFileDIONotSet`` (error code 5020) if ``O_DIRECT`` is not set.

Open a file with ``O_DIRECT``
*****************************

Use the POSIX ``open(2)`` system call to open the file. Include the ``O_DIRECT`` flag to enable direct IO:
...
2. Call ``hipFileHandleRegister()`` to register the file for GPU IO:
   ...
   On success, ``handle`` contains an opaque file handle for use with hipfile IO APIs.
...
Call ``hipFileBufRegister()`` to register the GPU memory region for use with GPU IO:
...
After both the file handle and buffer are registered, you can perform GPU IO operations such as ``hipFileRead()`` and ``hipFileWrite()``. For a complete worked example, see :doc:`/tutorial/copy-a-file`.
...
- ``err`` — a ``hipFileOpError_t`` value indicating a hipfile-specific or GPU IO driver error.
...
When you are finished with GPU IO, release all resources in the following order:
...
.. code-block:: c
   ...
   int main(void)
   {
       ...
       /* Perform GPU IO operations here (hipFileRead, hipFileWrite, etc.) */
...
- :doc:`/tutorial/copy-a-file` — A complete tutorial that uses registered files and buffers to copy a file through GPU memory.
