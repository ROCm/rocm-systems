...
::
    ...
    typedef struct __HIPFILE_NODISCARD hipFileError {
        hipFileOpError_t err;         //!< Errors related to hipFile or the GPU I/O driver
...
Unlike the POSIX call, however, which only returns -1 on errors, the hipFile I/O
...
When hipFile I/O calls that return a ``size_t`` fail, the returned value is the
