...
.. code-block:: python
   ...
   with FileHandle(src_path, os.O_RDONLY | os.O_DIRECT) as src, \
        FileHandle(dst_path, os.O_WRONLY | os.O_DIRECT | os.O_CREAT) as dst:
       ...
...
pointer for registration with hipFile. ``Buffer`` does not free the allocation;
...
``FileHandle.write`` may also raise ``RuntimeError`` (handle not open) or
``OSError`` (system errno). ``hipMalloc`` and ``hipFree`` raise ``RuntimeError``
on HIP allocation failures.
