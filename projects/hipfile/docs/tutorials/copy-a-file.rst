.. meta::
   ...
   :keywords: hipfile, ROCm, GPU IO, direct-to-GPU, file copy, hipFileRead, hipFileWrite, tutorial, example
...
essential step of a hipfile workflow:

1. Opening files with ``O_DIRECT`` and registering them for GPU IO
...
Run the resulting binary just like the Linux ``cp`` command:
...
The listing below is the complete ``aiscp.cpp`` file. Subsequent sections
explain each piece.
...
Open a file and register it for GPU IO
--------------------------------------
...
1. **Opened** with the standard POSIX ``open()`` call. The ``O_DIRECT`` flag is
   included to enable direct IO, which avoids the kernel page cache.
...
3. **Registered** with ``hipFileHandleRegister()``, which returns an opaque
   ``hipFileHandle_t`` for use with all subsequent hipfile IO calls.
...
filesystem block size. The block size is critical because writes performed with
...
filesystem block size. This ensures every IO operation meets alignment
requirements.
...
1. **Maximum IO size per call**: Because each ``hipFileRead`` or
...
2. **Block-size alignment on writes**: The write size is rounded up to the
   filesystem block size with ``align_up()``. This is required because the file
...
falls in the hipfile error range (above ``HIPFILE_BASE_ERR``, which is 5000).
...
- Explore asynchronous and batch IO operations for overlapping transfers with
