 ...
 *
 * Usage: ./batch-roundtrip READ_FILE WRITE_FILE FILE_SIZE BATCH_SIZE [GPUID]
 *
 ...
 *   BATCH_SIZE  Number of requests kept in flight, 1 to 128.
 *   GPUID       GPU device index (optional, default 0).
 *
 * The GPU buffer holds the whole file rounded up to a 4 KiB boundary. Each
 * phase starts with up to BATCH_SIZE requests, waits for at least one
 * completion, then submits one replacement for each returned event until all
 * 4 KiB requests have completed. All reads finish before any writes are
 * submitted. The final padded write is truncated to FILE_SIZE.
 */
    ...
    while (!requests.empty()) {
        ...
        for (unsigned i = 0; i < nr; ++i) {
            ...
            if (next_chunk < operation_count) {
                ...
                if (auto back{std::prev(requests.end())}; request != back)
                    std::iter_swap(request, back);
                requests.pop_back();
            }
        }
        in_flight -= nr;
...
{
    if (argc != 5 && argc != 6) {
        fprintf(stderr, "Usage: %s READ_FILE WRITE_FILE FILE_SIZE BATCH_SIZE [GPUID]\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    try {
        ...
        fprintf(stderr, "Invalid BATCH_SIZE: %s\n", argv[4]);
        return EXIT_FAILURE;
    }
    if (batch_capacity == 0 || batch_capacity > MAX_BATCH_SIZE) {
        fprintf(stderr, "BATCH_SIZE must be between 1 and %u\n", MAX_BATCH_SIZE);
        return EXIT_FAILURE;
    }
