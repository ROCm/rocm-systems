 ...
 *
 * Usage: ./roundtrip-async-multi-stream READ_FILE WRITE_FILE [GPUID]
 *
...
#define TOTAL_SIZE (static_cast<size_t>(NUM_STREAMS) * static_cast<size_t>(SLICE_SIZE))
...
main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < NUM_STREAMS; ++i) {
        ...
        if (hipSuccess != hip_err) {
            fprintf(stderr, "Could not allocate %zu bytes on GPU %d for slice %d (%d)\n",
                    static_cast<size_t>(SLICE_SIZE), gpu_id, i, hip_err);
            goto cleanup_slices;
        }
...

    {
        ...
        printf("OK  %s == %s  (%zu bytes across %d streams, hash 0x%016" PRIx64 ")\n", read_path, write_path,
               static_cast<size_t>(TOTAL_SIZE), NUM_STREAMS, hash);
    }
