 ...
 *  When both phases match, the device owns the packet. When they
 *  differ, the host owns it.
 *
 *  A separate occupied bitfield (in device-local memory) tracks
 *  which packets are currently claimed by a wave. This prevents
 *  multiple waves from using the same packet.
 */
 ...
 *  one device queue.
 *
 *  Packet ownership is determined by comparing per-packet phase values.
 *  The device_phase array is written only by the device; the host_phase
 *  array is written only by the host. When both match, the device owns
 *  the packet. When they differ, the host owns it.
 *
 *  The occupied bitfield lives in device-local memory and tracks which
 *  packets are currently in use by a wave. It is referenced by device
 *  virtual address from this struct.
 */
class HostcallBuffer {
  ...
  // --- Host-only fields (not part of shared prefix) ---

  /** Device pointer for service dispatch */
  const amd::Device *device_;
  /** Owner of the device-local occupied allocation (for cleanup) */
  amd::Memory* occupied_mem_;
  /** Adaptive upper bound on how many packets to scan */
  uint32_t scan_limit_;

 public:
  void processPackets(MessageHandler& messages);
  void initialize(uint32_t num_packets, amd::Memory* occupied_mem);
