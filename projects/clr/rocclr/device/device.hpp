...
class Device : public RuntimeObject {
 ...
 public:
  ...
  //! Removes queues from the set of active command queues
  template <typename IteratorT>
  void removeFromActiveQueues(IteratorT commandQueueBegin, IteratorT commandQueueEnd) {
    amd::ScopedLock lock(activeQueuesLock_);
    for (auto it = commandQueueBegin; it != commandQueueEnd; ++it) {
