...
// time.
//
// A dedicated ready byte per ring slot (not the AQL header) lets the CPU-side "is
// slot N ready?" test avoid racing the CP reading the header. The byte holds a lap
// generation so a stale ready flag from a prior lap does not count after ring
// wrap-around.
