...
hsa_queue_t* Device::acquireQueue(uint32_t queue_size_hint, bool coop_queue,
  ...
  // DEBUG_CLR_DEV_QUEUE controls queue ring buffer placement:
  //   0 - system memory (default)
  //  >0 - ring buffer in device memory if Large BAR is available.
  const bool device_mem_ring_buf =
      (DEBUG_CLR_DEV_QUEUE > 0) && info_.largeBar_ && Hsa::amd_queue_create_available();
  hsa_amd_queue_create_desc_t desc = {};
  desc.version = HSA_AMD_QUEUE_CREATE_DESC_VERSION;
  desc.queue_size = queue_size;
  ...
  if (!final_mask.empty()) {
    desc.cu_mask_count = static_cast<uint32_t>(final_mask.size() * 32);
    desc.cu_mask = final_mask.data();
  }
