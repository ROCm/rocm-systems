...
bool Device::CreateHwEvents(int count, std::vector<void*>& hw_events) const {
  ...
  // The completion role's host accesses are loads and plain stores, which this placement
  // supports; ROCr aborts on a host read-modify-write of such a word, so that has to stay true.
