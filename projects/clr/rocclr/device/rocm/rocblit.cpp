...
bool KernelBlitManager::WriteBufferBatch(
  ...
  for (const amd::BatchWriteMemoryOp& op : write_ops) {
    ...
    while (remaining_size > 0) {
      ...
      // REMOVED: Premature flush was creating artificial sync points that regressed
      // MPI-heavy workloads like Laghos (15% FOM loss on 4-GPU MI300A). The staging
      // pool naturally handles rotation, and getBuffer() will handle resource
      // exhaustion if needed. We only flush at the end of the full batch, matching
      // the behavior of the original non-batch writeBuffer() path.
...
bool KernelBlitManager::ReadBufferBatch(const std::vector<amd::BatchReadMemoryOp>& read_ops) const {
  ...
  for (const amd::BatchReadMemoryOp& op : read_ops) {
    ...
    while (remaining_size > 0) {
      ...
      // pool naturally handles rotation, and getBuffer() will handle resource
      // exhaustion if needed. We only flush at the end of the full batch, matching
      // the behavior of the original non-batch readBuffer() path.
