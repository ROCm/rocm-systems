...
inline bool DmaBlitManager::rocrCopyBuffer(address dst, hsa_agent_t& dstAgent, const_address src,
  ...
  // ROC_SDMA_ENGINE_SELECT: 2,3,4 all disable CLR selection -> let ROCr pick the engine.
  // (Mode 3 = ROCr round-robin via HSA_ENABLE_SDMA_RR; Mode 4 = ROCr preferred+spill
  //  round-robin via HSA_ENABLE_SDMA_RR_SPILL.)
  // requireSDMA (e.g. P2P) still needs the on_engine path, so it is honored here.
  bool kUseRegularCopyApi =
      (ROC_SDMA_ENGINE_SELECT == 2 || ROC_SDMA_ENGINE_SELECT == 3 ||
       ROC_SDMA_ENGINE_SELECT == 4) && !requireSDMA;
