...
// packet executed by this queue's own command processor - a barrier's dep_signal[], a dispatch's
// dep_signal, or the signal a barrier-value packet watches.  Every other caller hands the same
...
void VirtualGPU::profilingEnd(bool clearHwEvent, bool publishOrderingEdge) {
  // Here rather than in each submit method: this is the one point every command passes on its
  // way out, with its packets already in the ring and its HwEvent still set.  The edge is a
  // separate packet, so it has to sit behind the point it denotes.
