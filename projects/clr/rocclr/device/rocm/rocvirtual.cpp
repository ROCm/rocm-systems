  ...
  *aql_loc = *packet;
  ...
  if (ring_doorbell) {
    ...
    if (metadata_preloader_.HasDynDataPrefetch()) {
      ...
      const amd::DynDataPrefetchRegion* regions = metadata_preloader_.GetDynDataPrefetchRegions();
      for (uint32_t i = 0; i < numRegions && i < amd::kDynDataPrefetchMaxRegions; i++) {
        ClPrint(amd::LOG_EXTRA_DEBUG, amd::LOG_AQL,
                "  Region[%u]: addr=%p, burstSize=%zu, numBursts=%u, stride=%zu",
                i, regions[i].baseAddress, regions[i].burstSize, regions[i].numBursts, regions[i].stride);
      }
    }
      ...
      for (size_t i = chunkStart; i < chunkEnd; ++i) {
        ...
        if ((IsLogEnabled(amd::LOG_DETAIL_DEBUG, amd::LOG_KERN2) ||
          ...
          ClPrint(amd::LOG_DETAIL_DEBUG, amd::LOG_KERN2, "Graph ShaderName : %s, device id : %u",
                  ...
                  dev().index());
          // Select the layout per-packet: ext kernel-dispatch packets can be
