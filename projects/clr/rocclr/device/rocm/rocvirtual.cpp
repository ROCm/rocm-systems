...
//
// MOVDIR64B + device memory path (1 sfence):
//   metadata MOVDIR64B → sfence → AQL MOVDIR64B → doorbell
//
...
void VirtualGPU::ringQueueDoorbell(uint64_t index) {
  if (doorbell_ptr_) {
    amd::ringDoorbell(doorbell_ptr_, index, roc_device_.info().movdir64b_ && device_mem_ring_buf_);
  } else {
