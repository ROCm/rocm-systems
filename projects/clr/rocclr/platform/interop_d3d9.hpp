...
class D3D9Object : public InteropObject {
 ...
 public:
  ...
  D3D9Object() : releaseResources_(false), pQuery_(nullptr) {
    // @todo Incorrect initialization!!!
    memset(this, 0, sizeof(D3D9Object));
  }
  // copy constructor
  D3D9Object(D3D9Object& d3d9obj) : pQuery_(nullptr) {
    *this = d3d9obj;
    ...
    if (releaseResources_) {
      ...
      if (resources_.size()) {
        for (auto it = resources_.cbegin(); it != resources_.cend(); it++) {
          if (surfInfo_.resource && ((*it).first.surfInfo.resource == surfInfo_.resource) &&
