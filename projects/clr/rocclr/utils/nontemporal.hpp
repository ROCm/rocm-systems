...
//
 ...
 //  (3) sfence before the doorbell ring (step #11).  In the default path
 //      this is inside ROCr's AqlQueue::StoreRelease; in the direct path
 //      it is inside ringDoorbell().  This ensures the valid dispatch
 ...
 // switches to an optimised flow that reduces 3 sfences to 1:
 //
 ...
 // guaranteed atomically, eliminating sfences #1 and #2.  The WC buffer
 // close after the dispatch MOVDIR64B also covers the doorbell ordering,
 // eliminating sfence #3 (for the direct doorbell path).  The only
 // remaining sfence is before the first MOVDIR64B, ensuring kernarg
 // NT stores and metadata MOVDIR64B writes are globally ordered before
 // the dispatch packet becomes visible (#1a / #9):
 //
 //   1. MOVDIR64B metadata segments (body + headers atomic per 64B, WC closed)
 //   2. nontemporalStoreFence — ONE sfence (#1a / #7 / #9)
 //   3. MOVDIR64B dispatch packet   (body + header atomic, WC closed)
 //   4. doorbell ring               (UC write, no sfence needed)
 //
 // This reduces the dispatch hot path from 3 sfences to 1.
 //
...
//     HSA_AMD_QUEUE_INFO_DOORBELL_ID.  This eliminates ~5 levels of
//     indirection (dynamic dispatch → hsa.cpp → Signal::Convert →
//     virtual StoreRelease → StoreRelaxed).
//
// In both paths the UC (uncacheable) MMIO doorbell write implicitly flushes
// any remaining WC data, so no additional SFENCE is needed after the
// doorbell store.
...
// Ordering: sfence flushes the WC buffer so that the packet header written by
// packet_store_release (step #10) is visible before the UC doorbell write
// reaches the GPU (steps #11 / #12).  The UC write itself also implicitly
// flushes any remaining WC data.
//
// This collapses what ROCr does internally (atomic_thread_fence + sfence +
// UC store) into a single inline call, removing ~5 levels of indirection
