...
// File Desc abstraction between OS
  ...
  // If mapped_size_ptr is non-null, it also receives the number of safely
  // readable bytes from `image` to the end of its file-backed mapping (0 if
  // unknown, e.g. heap memory).
  static bool FindFileNameFromAddress(const void* image, std::string* fname_ptr,
                                      size_t* foffset_ptr, size_t* mapped_size_ptr = nullptr);
