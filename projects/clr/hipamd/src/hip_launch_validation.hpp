...
struct LaunchErrorRule {
  ...
  hipError_t error;      //!< code to return when any of them is set
};

//! Return the error for the FIRST rule matching any set bit; hipSuccess if none.
//! Rules are listed in each caller's existing check order — that order is observable API.
template <size_t N>
...
//! Shared config-bit mask matching LaunchParams::IsValidConfig()'s legacy semantics.
//! NOTE: deliberately excludes amd::kZeroBlock — that bit is conditional on !hipParams_ in
//! IsValidConfig(), so callers that need the zero-block check must test amd::kZeroBlock
//! explicitly rather than folding it into this shared mask.
