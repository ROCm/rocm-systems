...
Twenty-eight APIs have hand-written playback handlers (`MANUAL_PLAYBACK_APIS`): everything in the capture manual set plus all malloc/free variants, stream/event create/destroy, `hipModuleGetFunction`, host memory registration (`hipHostRegister`, `hipHostUnregister`), and the full HIP graph chain (`hipStreamBeginCapture` → `hipStreamEndCapture` → `hipGraphInstantiate` → `hipGraphLaunch`). All others are code-generated.
...
hipamd/src/hrr/
  ...
  hip_capture.cpp                 — 14 manual shims (kernel launch, memcpy H2D+D2H,
                                    module load, fat binary, hipHostRegister/Unregister)
  ...
  playback/
    ...
    hip_playback.cpp              — 28 manual playback shims (all graph APIs + hipHostRegister/Unregister)
...
test/
  hip_raw_trace.cpp               — multi-threaded integration test workload
                                    4 threadFunc + graphFunc + pinnedFunc (hipHostMalloc)
                                    + hostRegisterFunc (hipHostRegister)
                                    capture: HIP_HRR_CAPTURE_OUTPUT=out.hrr hip_raw_trace.exe
                                    replay:  hrr-playback out.hrr   (expects 11/11 D2H pass)
...
- **`MANUAL_CAPTURE_APIS`** (14): kernel launches ×4, memcpy H2D ×4, module load ×3, `__hipRegisterFatBinary`, `hipHostRegister`, `hipHostUnregister`
- **`MANUAL_PLAYBACK_APIS`** (28): above plus malloc ×4, free ×2, stream create/destroy ×4,
