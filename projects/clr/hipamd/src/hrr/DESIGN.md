...
| **Multi-threaded replay**      | No                                        | One CPU thread per captured thread                 |
...
Twenty-six APIs have hand-written playback handlers (`MANUAL_PLAYBACK_APIS`): everything in the capture manual set plus all malloc/free variants, stream/event create/destroy, `hipModuleGetFunction`, and the full HIP graph chain (`hipStreamBeginCapture` → `hipStreamEndCapture` → `hipGraphInstantiate` → `hipGraphLaunch`). All others are code-generated.
...
hipamd/src/hrr/
  DESIGN.md                       — this file
  CLAUDE.md                       — agent instructions with full implementation context
  ...
  hip_capture.cpp                 — 12 manual shims (kernel launch, memcpy H2D+D2H,
                                    module load, fat binary) + dispatch table install/uninstall
  ...
  playback/
    ...
    hip_playback.h                — PlaybackContext (11 handle maps), PlaybackFn typedef
    hip_playback.cpp              — 26 manual playback shims (all graph APIs included)
    hip_playback_generated.cpp    — AUTO-GENERATED: ~504 playback shims + dispatch table
...
test/
  ...
  hrr_validate.cpp                — kernel-count + NaN/Inf validation tool (hrr-validate)
```
...
| `hrr/hip_capture_generated.cpp` | Capture shims for ~517 APIs not in `MANUAL_CAPTURE_APIS` |
| `hrr/playback/hip_playback_generated.cpp` | Playback dispatch table for ~504 APIs not in `MANUAL_PLAYBACK_APIS` |
...
- **`MANUAL_CAPTURE_APIS`** (12): kernel launches ×4, memcpy H2D ×4, module load ×3, `__hipRegisterFatBinary`
- **`MANUAL_PLAYBACK_APIS`** (26): above plus malloc ×4, free ×2, stream create/destroy ×4,
  event create/destroy ×3, `hipModuleGetFunction`, `hipStreamBeginCapture`,
  `hipStreamEndCapture`, `hipGraphInstantiate`, `hipGraphLaunch`
- **`NOOP_PLAYBACK_APIS`**: APIs with type mismatches or missing in ROCm SDK 6.4 — emit `return hipSuccess;`
...
**CRITICAL playback offset rule:** `payload` in playback functions points to bytes
AFTER the 32-byte header. So `payload[0]` = `ret`, `payload[4]` = first field.
Use `rd32(payload, 0)` / `rd64(payload, 4)` — NOT `reinterpret_cast<hrr_args_*>(payload)`
(which reads at struct offsets 32+ — wrong). Generated shims use reinterpret_cast and
happen to work for APIs that don't check ret or output handles, but all manual playback
functions MUST use rd32/rd64.
...
| `hipModuleLoadData`, `hipModuleLoadDataEx` | `co_hash_lo u64`, `co_hash_hi u64`, `module_id u32` |
...
hrr-playback <capture.hrr> [options]
  ...
  --single-thread       Force single-threaded CPU submission
