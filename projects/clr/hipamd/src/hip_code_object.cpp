...
hip::Var* DynCO::getVar(const std::string& var_name) {
  std::scoped_lock lock(dclock_);
  // Populate variables on demand for lazy loading
  if (populateDynGlobalVars() != hipSuccess) {
    return nullptr;
  }
  auto it = vars_.find(var_name);
...
bool DynCO::isValidDynFunc(const void* hfunc) {
  std::scoped_lock lock(dclock_);
  // Populate functions on demand for lazy loading
  if (populateDynGlobalFuncs() != hipSuccess) {
    return false;
  }
