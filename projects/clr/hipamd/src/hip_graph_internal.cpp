      ...
      *out_attach_signal = *out_attach_signal && (i == (segment.nodes.size() - 1));
      ...
      if (segBatch && batchIndex < segBatch->packet_batches.size()) {
        ...
        *out_attach_signal = *out_attach_signal && (i == (segment.nodes.size() - 1));
        ...
        // Fast path: use the pre-built flat buffer when available.
        //   - All nodes enabled: use flatPacketData (built at instantiation / param update).
        //   - Some nodes disabled: use filteredFlatPacketData.
        // Falls back to per-pointer dispatchAqlPacketBatch if the flat path declines.
        if (!packetsToDispatch->empty()) {
          bool batchStatus = false;
          if (packetBatch.disabledNodeCount == 0 && !packetBatch.flatPacketData.empty()) {
            batchStatus = stream->vdev()->dispatchAqlPacketBatchFlat(
                packetBatch.flatPacketData, packetBatch.validPacketFullHeaders,
                accumulate, *out_attach_signal);
          } else if (packetBatch.disabledNodeCount > 0 &&
                     !packetBatch.filteredFlatPacketData.empty()) {
            batchStatus = stream->vdev()->dispatchAqlPacketBatchFlat(
                packetBatch.filteredFlatPacketData, packetBatch.filteredValidPacketFullHeaders,
                accumulate, *out_attach_signal);
          }
          if (!batchStatus) {
            batchStatus = stream->vdev()->dispatchAqlPacketBatch(
