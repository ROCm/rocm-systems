...
static hipError_t hipExtLaunchKernelLayer(const void* function_address, dim3 numBlocks,
  ...
  if (args) {
    ...
    if (ihipGetFuncBySymbol(&hfunc, function_address) == hipSuccess)
  ...
  for (auto* node : nodes) {
    ...
    switch (node->GetType()) {
      case hipGraphNodeTypeKernel: {
        ...
        if (!kp.func) continue;
        hipFunction_t hfunc = nullptr;
        if (ihipGetFuncBySymbol(&hfunc, kp.func) != hipSuccess || !hfunc) {
...
static hipError_t hipLaunchCooperativeKernelLayer(const void* f, dim3 gridDim, dim3 blockDimX,
  ...
  if (kernelParams) {
    hipFunction_t hfunc = nullptr;
    if (ihipGetFuncBySymbol(&hfunc, f) == hipSuccess)
