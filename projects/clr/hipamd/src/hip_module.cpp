...
hipError_t ihipLaunchKernelCommand(amd::Command*& command, hipFunction_t f,
  ...
  if (devKernel != nullptr &&
      !ndrange.UpdateNumClusters(static_cast<uint32_t>(devKernel->getClusterSize(0)),
                                 static_cast<uint32_t>(devKernel->getClusterSize(1)),
                                 static_cast<uint32_t>(devKernel->getClusterSize(2)))) {
...
amd::NDRangeContainer makeHipLaunchNDRange(uint32_t gridX, uint32_t gridY, uint32_t gridZ,
  ...
  // Checks are ordered as the old amd::LaunchParams ctor ran them: first failure is reported.
