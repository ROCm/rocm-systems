...
# 2. Enables rocm_hip:hipMemcpyAsync_args, rocm_hip:hipMalloc_args, and
...
# 6. Asserts the channel-context vpid + vtid attach to every event,
...
# __amd_streamOps* externs. Reclassify the missing hipMemcpyAsync_args event
# as WARN when the program itself failed so the other typed-event assertions
# can still validate the LTTng infrastructure.
HIPMA_CRASH=0
if [ "$APP_RC" -ne 0 ] && \
   ! grep -q 'rocm_hip:hipMemcpyAsync_args' "$DUMP"; then
    HIPMA_CRASH=1
fi
...
# D. vpid + vtid context propagation: every event must carry a vpid and
