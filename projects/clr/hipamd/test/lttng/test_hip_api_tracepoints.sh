...
#    uniquely named typed API tracepoint we expected appears.
#
# Usage:
...
lttng enable-event --userspace --channel=default \
    'rocm_hip:hipMalloc_args,rocm_hip:hipFree_args,rocm_hip:hipGetDevice_args,rocm_hip:hip_kernel_dispatch_enqueue'
...
for api in hipMalloc hipFree hipGetDevice; do
    if grep -q "rocm_hip:${api}_args" "$LOG"; then
        echo "  OK: ${api}_args"
    else
        echo "  FAIL: missing ${api}_args" >&2
        FAIL=1
    fi
done
