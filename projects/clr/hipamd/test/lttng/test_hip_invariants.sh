...
lttng enable-event --userspace -c default 'rocm_hip:*,rocm_hsa:*' >/dev/null
...
if [ -z "$LAUNCH_VTID" ]; then
    ...
    if [ "$ENQUEUE_ON_LAUNCH_VTID" -lt 1 ]; then
        ...
        HSA_ON_LAUNCH_VTID=$(grep -E 'rocm_hsa:.*_enter:' "$LOG" \
            | grep -cE "vtid = $LAUNCH_VTID[ ,}]" || true)
        echo "  I4 INFO: $HSA_ON_LAUNCH_VTID HSA curated enter events also fired on launching vtid (cross-runtime same-thread parent recovery via LIFO walk)"
    fi
fi
