#!/usr/bin/env bash
# Runtime smoke test for selected curated `_args` events. Full inventory
# coverage is enforced statically by lttng_coverage_gate.sh; executing every
# exported API with fabricated resources is neither safe nor meaningful.
set -euo pipefail
...
WORK="$(mktemp -d)"
SMOKE_YAML="$WORK/curated_smoke.yaml"
...
# Keep runtime validation to a representative safe subset. The full YAML also
# covers implementation-only compatibility wrappers and APIs requiring real
# descriptors, code objects, callbacks, or multiple devices; those use the
# mandatory static sentinel/helper gate instead.
python3 - "$YAML" "$SMOKE_YAML" <<'PY'
import sys
import yaml

source, output = sys.argv[1:]
wanted = {'hipDeviceSynchronize', 'hipGetDevice', 'hipMalloc', 'hipMemcpyAsync'}
with open(source) as file:
    apis = yaml.safe_load(file)
with open(output, 'w') as file:
    yaml.safe_dump([api for api in apis if api['api'] in wanted], file,
                   sort_keys=False)
PY
...
PLACEHOLDER_OVERRIDES = {
    # The full-coverage runtime smoke includes this newly auto-curated API.
    # A null typed output pointer returns an error but still reaches the
    # all-IN typed emit safely after the implementation call.
    'hipGetDevice':                    {'device': 'nullptr'},
...
apis = sorted(parse_yaml_file('$SMOKE_YAML'), key=sort_key)
...
# Enable the runtime-smoke curated _args events. lttng enable-event takes a single
# comma-separated event-name list, not multiple positional args.
EVENTS=$(python3 -c "
import sys
sys.path.insert(0, 'shared/lttng/scripts')
from lttng_curated_lib import parse_yaml_file
 print(','.join(f'rocm_hip:{a[\"api\"]}_args' for a in parse_yaml_file('$SMOKE_YAML')))
")
...
sys.path.insert(0, 'shared/lttng/scripts')
from lttng_curated_lib import parse_yaml_file
for a in parse_yaml_file('$SMOKE_YAML'):
    print(a['api'])
")

if [ "$MISSING" -gt 0 ]; then
    echo "FAIL: $MISSING runtime-smoke _args events missing ($SKIP skipped, $WARN env warning(s))"
    exit 1
fi
if [ "$WARN" -gt 0 ]; then
    echo "PASS: all runtime-smoke _args events fired ($SKIP skipped, $WARN env warning(s) -- see above)"
else
    echo "PASS: all runtime-smoke _args events fired ($SKIP skipped, runtime; static-only)"
fi
