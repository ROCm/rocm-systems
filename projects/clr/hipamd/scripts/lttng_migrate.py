...
def classify(return_type_spelling: str) -> str:
    """Map a return type spelling to STATUS / PTR / VOID, or raise."""
    s = return_type_spelling.strip()
    if s in VOID_TYPES:
        return 'VOID'
    if s in STATUS_TYPES:
        return 'STATUS'
    # Pointer types (any depth, any cv-qualifier): * or **
    # Examples: 'const char *', 'void **', 'hipChannelFormatDesc *',
    # 'const HipDispatchTable *' (defensively).
    if s.endswith('*') or '*' in s:
        return 'PTR'
    # struct/class returns by value (e.g. hipChannelFormatDesc) - we cannot
    # generally extract a usable id from these; fail-loudly so a human
    # decides.
    raise SystemExit(
        f'Unhandled return type: {s!r}. Add to STATUS_TYPES, VOID_TYPES, or '
        f'extend classify() to handle this case.'
    )
...
MIGRATION_MARKER = '__rocm_corr'  # presence in body => already migrated
...
def migrate_file(source_path: str, include_path: str, extra_args: list,
    ...
    # Diagnose hard errors but tolerate warnings (HIP headers may emit
    # warnings under random configs that are not our concern).
    fatal = [d for d in tu.diagnostics if d.severity >= cindex.Diagnostic.Error]
    if fatal:
        for d in fatal:
            print(f'PARSE ERROR: {d.spelling} at {d.location}', file=sys.stderr)
        raise SystemExit(1)
