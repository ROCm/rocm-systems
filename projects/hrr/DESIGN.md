...
### Capture Layer (`projects/clr/hipamd/src/hrr/`)
...
### Build
```bash
# 1. Capture runtime: compiled into amdhip64 as part of the CLR/HIP build tree
cmake -S projects/clr -B build -GNinja -DCLR_BUILD_HIP=ON -DHIP_PLATFORM=amd \
      -DCMAKE_BUILD_TYPE=Release
ninja -C build amdhip64
# Output: build/hipamd/lib/libamdhip64.so*

# 2. Playback tool: built standalone from projects/hrr against a capture-enabled prefix
cmake -S projects/hrr -B hrr-build -DROCM_PATH=<prefix> -DCMAKE_PREFIX_PATH=<prefix> \
      -DCMAKE_BUILD_TYPE=Release
cmake --build hrr-build --target hrr-playback
# Output: hrr-build/playback/hrr-playback  (installs to <prefix>/bin/hrr-playback)
```
...
```
projects/hrr/                     — standalone HRR project (portable layer)
  DESIGN.md                       — this file
  README.md                       — user-facing quick start
  tools/
    gen_hrr_api_args.py           — generator: produces hrr_api_args.h and
                                    playback/hip_playback_generated.cpp under
                                    projects/hrr, plus hip_capture_generated.cpp
                                    under projects/clr/hipamd/src/hrr
  include/hrr/
  ...
  playback/
    ...
    hip_playback_generated.cpp    — AUTO-GENERATED: ~245 playback shims + dispatch table
...
projects/clr/hipamd/src/hrr/      — in-tree capture layer (compiled into amdhip64)
  ...
  hip_capture_metadata.h/.cpp     — capture-side metadata helpers
  hip_capture_writer.h/.cpp       — streaming event serialization to events.bin / blobs/
  hip_capture_generated.cpp       — AUTO-GENERATED: ~517 capture shims + build_table()
  CMakeLists.txt                  — adds capture sources to the amdhip64 target; the shared
                                    hrr_api_args.h wire format is included from projects/hrr/include
```
...
| `projects/clr/hipamd/src/hrr/hip_capture_generated.cpp` | ~517 capture shims for APIs not in `MANUAL_CAPTURE_APIS` |
| `projects/hrr/playback/hip_playback_generated.cpp` | ~245 playback shims + dispatch table for APIs not in `MANUAL_PLAYBACK_APIS` |

Script location: `projects/hrr/tools/gen_hrr_api_args.py`

Normal builds generate temporary files and verify them against the checked-in compilation inputs. Run `python3 projects/hrr/tools/gen_hrr_api_args.py --check-hrr-coverage` from the repository root only when intentionally refreshing those checked-in files; use the temporary-output command above for inspection.
...
`projects/clr/hipamd/src/hrr/CMakeLists.txt`. The playback tools build from the
standalone `projects/hrr` project (`projects/hrr/playback/CMakeLists.txt`, wired in
from `projects/hrr/CMakeLists.txt`) against a capture-enabled ROCm/HIP install prefix.
Builds are Linux/Ninja (on Windows, build inside WSL — there is no native Windows build).

### Capture runtime (amdhip64)
```bash
...
ninja -C build amdhip64 -j"$(nproc)"
# Runtime: build/hipamd/lib/libamdhip64.so*
```

### Playback tool (standalone HRR project)
```bash
# Build hrr-playback from projects/hrr against a capture-enabled ROCm/HIP prefix
# (the prefix that provides hip::host / libamdhip64).
...
# Tool: hrr-build/playback/hrr-playback  (locate via: find hrr-build -name hrr-playback -type f;
