...
class CheckReplayCompatTests(unittest.TestCase):
    ...
    @mock.patch(
        "check_replay_compat.shutil.which",
        side_effect=lambda name: "/usr/bin/rocm-smi" if name == "rocm-smi" else None,
    )
    def test_probe_host_reads_comgr(
        self, _which_mock: mock.Mock, run_mock: mock.Mock, _comgr_mock: mock.Mock
    ) -> None:
    ...
    def test_resolve_replay_mounts_skips_without_overlay(self) -> None:
        clr, rocr, extra = crc.resolve_replay_mounts(
            ...
            clr_build="/build",
            hrr_playback="/opt/rocm/bin/hrr-playback",
            mount_clr=False,
        )
    ...
    @mock.patch("check_replay_compat._run")
    @mock.patch("check_replay_compat.shutil.which", return_value="/usr/bin/docker")
    def test_probe_docker_mounts_clr_lib(
        self, _which_mock: mock.Mock, run_mock: mock.Mock, _docker_mock: mock.Mock
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            lib_dir = Path(tmp)
            versioned = lib_dir / "libamdhip64.so.7.15.26291-2ed46569ce"
