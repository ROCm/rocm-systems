...
```bash
<rocm-systems>/projects/hrr/skills/decode-and-triage/scripts/triage_archive.sh \
...
```powershell
<rocm-systems>\projects\hrr\skills\decode-and-triage\scripts\triage_archive.ps1 `
  --archive <path-to>\capture.hrr\pid-<pid>
```

Scripts locate the colocated `projects/hrr` project from their install path; you do
not need to `cd` into `rocm-systems` first.
...
| Locate / build `hrr-playback.exe` | **Yes** — `ensure_playback.ps1` searches `HRR_PLAYBACK`, `HIP_PATH\bin`, `ROCM_PATH\bin`, `PATH`; `--build` builds `hrr-playback` from `projects/hrr` |
...
| Auto-build `hrr-playback.exe` | **Yes** — `ensure_playback.ps1 --build` builds `hrr-playback` from `projects/hrr` into a dedicated HRR build dir |
...
.\ensure_playback.ps1          # find existing binary (checks HRR_PLAYBACK, HIP_PATH\bin, ROCM_PATH\bin, PATH)
.\ensure_playback.ps1 --build  # also build hrr-playback from projects/hrr if not found
```
...
```
projects\hrr\skills\decode-and-triage\scripts\
  triage_archive.ps1
  ensure_playback.ps1
...
1. Resolve `capture.hrr/pid-*` yourself: no script does this, `--archive` takes one
   process directory. Given a root, run `../enable-recording/scripts/inspect_archive.py
   --archive <root> --no-playback` and pick from its per-process table. Prefer the
   process whose state is `incomplete`, since that is the one that died; fall back to
   the largest `events.bin` only when every process shut down cleanly. Size is the
   wrong rule for a crash, because the process that died is the one cut short.
   Triage each process separately; the archive still travels whole.
...
| Situation | Action |
|-----------|--------|
| `--build` fails | Install `hrr-playback` under `$ROCM_PATH/bin` or set `HRR_PLAYBACK`; ensure a capture-enabled ROCm/HIP prefix (see `../../README.md`). |
...
| `--verbose` | Per-event trace. `triage_archive.sh` never passes it, so the analyser's last-launch-before-the-fault fallback cannot fire through the script. When a fault names no kernel, rerun by hand: `hrr-playback <pid-dir> --sync-after-launch --verbose` |
