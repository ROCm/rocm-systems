...
| `ROCR_LIB` | Directory holding the `libhsa-runtime64.so.1` that matches the reader, put in front of `$ROCM_PATH/lib` |
| `ROCM_PATH` | Its `lib` goes last on the replay library path, after the reader's own and after your `LD_LIBRARY_PATH` |
...
| `undefined symbol` from the reader itself (Linux) | Its own libraries are not in front. A packaged build ships `bin/`, `lib/` and `runtime-lib/` as siblings and the script adds all three; a build tree may not, so point `ROCR_LIB` at the directory holding the matching `libhsa-runtime64.so.1`. This is a one-line fix and comes before trying Docker. |
