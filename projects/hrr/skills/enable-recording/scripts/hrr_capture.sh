...
resolved_runtime() {
  ...
  # Only the mapping lines, `soname => /path`. A binary built against a newer
  # ROCm than the host's makes ldd print symbol-version errors first, and those
  # lines also mention libamdhip64: taking the third field off one of them
  # yielded the literal word "version", resolution then failed silently, and
  # preflight blamed an interpreter for a compiled binary.
  path="$(ldd "$bin" 2>/dev/null |
            awk '/libamdhip64\.so/ && /=>/ { print $3; exit }')"
