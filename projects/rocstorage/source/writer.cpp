...
struct writer::impl
{
    ...
    private:
        entitiy_utility<std::unordered_set<writer_api::node_id_t>> node_info_utility{};
        entitiy_utility<std::unordered_map<writer_api::process_id_t, primary_key>>
            process_info_utility{};
        entitiy_utility<std::unordered_map<writer_api::agent_unique_id_t,
                                           primary_key,
                                           agent_unique_id_hash>>
            agent_info_utility{};
        entitiy_utility<std::unordered_map<writer_api::pmc_info_unique_id_t,
                                           primary_key,
                                           pmc_unique_id_hash>>
            pmc_info_utility{};
        entitiy_utility<std::unordered_map<writer_api::thread_id_t, primary_key>>
            thread_info_utility{};
        entitiy_utility<std::unordered_map<writer_api::stream_id_t, primary_key>>
            stream_info_utility{};
        entitiy_utility<std::unordered_map<writer_api::queue_id_t, primary_key>>
            queue_info_utility{};
        entitiy_utility<std::unordered_set<writer_api::code_object_id_t>>
            code_object_info_utility{};
        entitiy_utility<std::unordered_set<writer_api::kernel_symbol_id_t>>
            kernel_symbol_info_utility{};
        entitiy_utility<
            std::unordered_map<writer_api::track_info_t, primary_key, track_info_hash>>
            track_info_utility{};
        entitiy_utility<std::unordered_map<std::string, primary_key>>
            string_info_utility{};
...
public:
    ...
    , m_entity_identifiers(std::make_unique<data_identifiers>())
    {
        if(!m_database)
        {
    ...
    void register_process_info(const writer_api::process_info_t& process_info)
    {
        ...
        const auto is_node_registered =
            node_info_utility.is_entry_registered(process_info.node_id);
        const auto is_process_registered =
            process_info_utility.is_entry_registered(process_info.pid);

        if(is_process_registered)
        {
    ...
    void register_agent_info(const writer_api::agent_info_t& agent_info)
    {
        ...
        const auto is_agent_registered =
            agent_info_utility.is_entry_registered(agent_info.unique_id);
        const auto is_node_registered =
            node_info_utility.is_entry_registered(agent_info.node_id);
        const auto is_process_registered =
            process_info_utility.is_entry_registered(agent_info.process_id);
        ...
        const auto process_primary_key =
            process_info_utility.get_primary_key_value_for_entity(agent_info.process_id);
    ...
    void register_pmc_info(const writer_api::pmc_info_t& pmc_info)
    {
        ...
        const auto is_pmc_info_registered =
            pmc_info_utility.is_entry_registered(pmc_info.unique_id);
        const auto is_node_registered =
            node_info_utility.is_entry_registered(pmc_info.node_id);
        const auto is_process_registered =
            process_info_utility.is_entry_registered(pmc_info.process_id);
        const auto is_agent_registered =
            agent_info_utility.is_entry_registered(*pmc_info.unique_id.agent_id);

        if(is_pmc_info_registered)
        {
        ...
        {
            throw std::runtime_error(
                fmt::format("Agent not registered for PMC Info: agent_id [agent_type={}, "
                            "type_index={}]",
                            pmc_info.unique_id.agent_id->agent_type,
                            pmc_info.unique_id.agent_id->type_index));
        }

        const auto process_primary_key =
            process_info_utility.get_primary_key_value_for_entity(pmc_info.process_id);
        const auto agent_foreign_key =
            agent_info_utility.get_primary_key_value_for_entity(
                *pmc_info.unique_id.agent_id);
    ...
    void register_thread_info(const writer_api::thread_info_t& thread_info)
    {
        ...
        const auto is_thread_info_registered =
            thread_info_utility.is_entry_registered(thread_info.thread_id);
        const auto is_node_registered =
            node_info_utility.is_entry_registered(thread_info.node_id);
        const auto is_process_registered =
            process_info_utility.is_entry_registered(thread_info.process_id);

        if(is_thread_info_registered)
        {
        ...
        const auto process_primary_key =
            process_info_utility.get_primary_key_value_for_entity(thread_info.process_id);
    ...
    void register_stream_info(const writer_api::stream_info_t& stream_info)
    {
        ...
        const auto is_stream_info_registered =
            stream_info_utility.is_entry_registered(stream_info.stream_id);
        const auto is_node_registered =
            node_info_utility.is_entry_registered(stream_info.node_id);
        const auto is_process_registered =
            process_info_utility.is_entry_registered(stream_info.process_id);

        if(is_stream_info_registered)
        {
        ...
        const auto process_primary_key =
            process_info_utility.get_primary_key_value_for_entity(stream_info.process_id);
    ...
    void register_queue_info(const writer_api::queue_info_t& queue_info)
    {
        ...
        const auto is_queue_info_registered =
            queue_info_utility.is_entry_registered(queue_info.queue_id);
        const auto is_node_registered =
            node_info_utility.is_entry_registered(queue_info.node_id);
        const auto is_process_registered =
            process_info_utility.is_entry_registered(queue_info.process_id);

        if(is_queue_info_registered)
        {
        ...
        const auto process_primary_key =
            process_info_utility.get_primary_key_value_for_entity(queue_info.process_id);
    ...
    void register_code_object_info(const writer_api::code_object_info_t& code_object_info)
    {
        ...
        const auto is_code_object_info_registered =
            code_object_info_utility.is_entry_registered(code_object_info.id);
        const auto is_node_registered =
            node_info_utility.is_entry_registered(code_object_info.node_id);
        const auto is_process_registered =
            process_info_utility.is_entry_registered(code_object_info.process_id);

        if(is_code_object_info_registered)
        {
        ...
        if(code_object_info.agent_id.has_value() &&
           !agent_info_utility.is_entry_registered(code_object_info.agent_id.value()))
        {
            throw std::runtime_error(
        ...
        const auto process_primary_key =
            process_info_utility.get_primary_key_value_for_entity(
                code_object_info.process_id);

        std::optional<primary_key> agent_foreign_key = std::nullopt;
        if(code_object_info.agent_id.has_value())
        {
            agent_foreign_key = agent_info_utility.get_primary_key_value_for_entity(
                code_object_info.agent_id.value());
        }
        ...
        const writer_api::kernel_symbol_info_t& kernel_symbol_info)
    {
        ...
        const auto is_kernel_symbol_info_registered =
            kernel_symbol_info_utility.is_entry_registered(kernel_symbol_info.id);
        const auto is_node_registered =
            node_info_utility.is_entry_registered(kernel_symbol_info.node_id);
        const auto is_process_registered =
            process_info_utility.is_entry_registered(kernel_symbol_info.process_id);
        const auto is_code_object_info_registered =
            code_object_info_utility.is_entry_registered(kernel_symbol_info.code_obj_id);

        if(is_kernel_symbol_info_registered)
        {
    ...
    void register_track_info(const writer_api::track_info_t& track)
    {
        ...
        const auto is_track_info_registered =
            track_info_utility.is_entry_registered(track);
        const auto is_node_registered =
            node_info_utility.is_entry_registered(track.node_id);

        if(is_track_info_registered)
        {
        ...
        if(track.process_id.has_value() &&
           !process_info_utility.is_entry_registered(track.process_id.value()))
        {
            throw std::runtime_error(
        ...
        if(track.thread_id.has_value() &&
           !thread_info_utility.is_entry_registered(track.thread_id.value()))
        {
            throw std::runtime_error(
        ...
        if(track.name.has_value() &&
           !string_info_utility.is_entry_registered(track.name.value()))
        {
        ...
        if(track.process_id.has_value())
        {
            process_primary_key = process_info_utility.get_primary_key_value_for_entity(
                track.process_id.value());
        }
        ...
        if(track.thread_id.has_value())
        {
            thread_primary_key = thread_info_utility.get_primary_key_value_for_entity(
                track.thread_id.value());
        }
        ...
        {
            string_primary_key =
                string_info_utility.get_primary_key_value_for_entity(track.name.value());
        }
...
public:
...

    {
        ...
        if(!trace_environment.node_id.has_value() ||
           !node_info_utility.is_entry_registered(trace_environment.node_id.value()))
        {
        ...
        if(!trace_environment.process_id.has_value() ||
           !process_info_utility.is_entry_registered(
        ...
        if(!trace_environment.thread_id.has_value() ||
           !thread_info_utility.is_entry_registered(trace_environment.thread_id.value()))
        {
        ...
        const auto thread_primary_key =
            thread_info_utility.get_primary_key_value_for_entity(
        ...
        if(trace_environment.track_name.has_value() && event_primary_key.has_value())
        {
            ...
            const writer_api::sample_data_t sample_data = { region_data.start_timestamp,
                                                            track_info,
                                                            "{}" };
            insert_sample(sample_data, event_primary_key.value());
        }
    }
                               ...
                               const writer_api::pmc_info_unique_id_t& pmc_unique_id)
    {
        ...
        const auto is_pmc_info_registered =
            pmc_info_utility.is_entry_registered(pmc_unique_id);
        ...
        const auto pmc_info_primary_key =
            pmc_info_utility.get_primary_key_value_for_entity(pmc_unique_id);
        ...
        const writer_api::trace_environment_t&    trace_environment)
    {
        ...
        if(!trace_environment.agent_id.has_value() ||
           !agent_info_utility.is_entry_registered(trace_environment.agent_id.value()))
        {
        ...
        if(!trace_environment.queue_id.has_value() ||
           !queue_info_utility.is_entry_registered(trace_environment.queue_id.value()))
        {
        ...
        if(!trace_environment.stream_id.has_value() ||
           !stream_info_utility.is_entry_registered(trace_environment.stream_id.value()))
        {
        ...
        if(!kernel_symbol_info_utility.is_entry_registered(
        ...
        const auto agent_primary_key =
            agent_info_utility.get_primary_key_value_for_entity(
                trace_environment.agent_id.value());
        const auto queue_primary_key =
            queue_info_utility.get_primary_key_value_for_entity(
                trace_environment.queue_id.value());
        const auto stream_primary_key =
            stream_info_utility.get_primary_key_value_for_entity(
                                 ...
                                 const writer_api::trace_environment_t& trace_environment)
    {
        ...
        if(memory_copy_data.src_agent_id.has_value() &&
           !agent_info_utility.is_entry_registered(memory_copy_data.src_agent_id.value()))
        {
            throw std::runtime_error(
        ...
        if(memory_copy_data.dst_agent_id.has_value() &&
           !agent_info_utility.is_entry_registered(memory_copy_data.dst_agent_id.value()))
        {
            throw std::runtime_error(
        ...
        if(memory_copy_data.src_agent_id.has_value())
        {
            src_agent_primary_key = agent_info_utility.get_primary_key_value_for_entity(
        ...
        if(memory_copy_data.dst_agent_id.has_value())
        {
            dst_agent_primary_key = agent_info_utility.get_primary_key_value_for_entity(
        ...
        if(trace_environment.queue_id.has_value())
        {
            queue_primary_key = queue_info_utility.get_primary_key_value_for_entity(
                trace_environment.queue_id.value());
        }
        ...
        if(trace_environment.stream_id.has_value())
        {
            stream_primary_key = stream_info_utility.get_primary_key_value_for_entity(
                trace_environment.stream_id.value());
        }
        ...
        const writer_api::trace_environment_t& trace_environment)
    {
        ...
        if(trace_environment.agent_id.has_value())
        {
            agent_primary_key = agent_info_utility.get_primary_key_value_for_entity(
                trace_environment.agent_id.value());
        }
    ...
    std::unique_ptr<data_identifiers>                           m_entity_identifiers;
};
