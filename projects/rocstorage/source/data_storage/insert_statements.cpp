...
insert_statements::insert_statements(std::shared_ptr<sqlite_backend> backend,
                                     std::string                     uuid)
...
insert_statements::initialize_arg_statement()
{
    ...
    auto                                            query =
        query_builder.set_table_name(fmt::format("rocpd_arg_{}", m_uuid))
            ...
            .set_values('?', '?', '?', '?', '?', '?', '?')
            .get_query_string();
    m_arg_statement = m_backend->create_write_statement_executor<integer_primary_key_t,
                                                                 integer_foreign_key_t,
                                                                 size_t,
                                                                 const char*,
                                                                 const char*,
