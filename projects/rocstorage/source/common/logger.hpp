...
inline __attribute__((always_inline)) auto to_lower(std::string_view s) {
  std::string result;
...
std::string include_process_id_in_filename(std::string_view filename) {
  if (filename.empty()) {
    return std::string{};
  }
...
inline bool parse_boolean_env(const char *env) {
  if (!env) {
    return false;
  }
  constexpr std::array<const char *, 4> true_values = {"1", "on", "true",
                                                       "yes"};
...
struct logger_settings_t {
  logger_settings_t()
      : m_log_level(log_level_from_env(std::getenv("ROCSTORAGE_LOG_LEVEL"))),
        m_log_file(std::getenv("ROCSTORAGE_LOG_FILE")) {
    const char *rocstorage_monochrome_env =
        std::getenv("ROCSTORAGE_MONOCHROME");
...
protected:
  ...
  const char *m_log_pattern_monochrome{
      "[%H:%M:%S.%e][P:%P T:%t][%s:%# %!][%l] %v"};
};
...
private:
  static std::shared_ptr<spdlog::logger> create_logger() {
    ...
    auto log = std::make_shared<spdlog::logger>(s_logger_name, sinks.begin(),
                                                sinks.end());
  ...
  static constexpr const char *s_logger_name = "rocstorage";
};
