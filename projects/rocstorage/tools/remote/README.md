...
Run a local HTTP server that proxies queries over SSH:

```bash
# Start the proxy (runs locally)
...
**Pros:** Single SSH authentication, connection reuse, caching
...
Local HTTP server that proxies SQL queries to remote SQLite via SSH.
