...
class RdcUtil:
    ...
    def create_field_group(self, rdc_handle, field_group_name, field_ids):
        ...
        result = rdc.rdc_group_field_get_all_ids(
            rdc_handle, field_group_id_list, field_group_count
        )
        if rdc_status_t(result) != rdc_status_t.RDC_ST_OK:
        ...
        for index in range(field_group_count.value):
            ...
            if rdc_status_t(result) != rdc_status_t.RDC_ST_OK:
                raise Exception(
                    "Fail to get field group "
                    + str(field_group_id_list[index])
                    + " info"
                )
            if group_info.group_name.decode("utf-8") == field_group_name:
                field_ids_ori = [
                    e.value for e in group_info.field_ids[: group_info.count]
                ]
                # reuse the old field group
                if field_ids == field_ids_ori:
                    return field_group_id_list[index], False
                else:
                    result = rdc.rdc_group_field_destroy(
                        rdc_handle, field_group_id_list[index]
                    )
                    if rdc_status_t(result) != rdc_status_t.RDC_ST_OK:
                        raise Exception(
                            "Fail to delete field group "
                            + str(field_group_id_list[index])
                        )
