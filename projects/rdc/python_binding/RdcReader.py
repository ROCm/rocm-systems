...
class RdcReader:
    ...
    def __init__(
        ...
        field_group_name="rdc_reader_field_group",
        gpu_group_name="rdc_reader_gpu_group",
        gpu_indexes=None,
        root_ca="/etc/rdc/client/certs/rdc_cacert.pem",
        ...
        if not ip_port:  # embedded
            ...
            result = rdc.rdc_start_embedded(
                rdc_operation_mode_t.RDC_OPERATION_MODE_AUTO, self.rdc_handle
            )
            if rdc_status_t(result) != rdc_status_t.RDC_ST_OK:
            ...
            result = rdc.rdc_connect(
                ip_port.encode("utf-8"),
                self.rdc_handle,
                root_ca_str,
                client_cert_str,
                client_key_str,
        ...
        if gpu_indexes == None:
            ...
            for idx in gpu_indexes:
                ...
                if rdc.rdc_is_partition_string(encoded):
                    rc = rdc.rdc_parse_partition_string(
                        encoded, ctypes.byref(phys_gpu), ctypes.byref(part_idx)
                    )
                    if not rc:
        ...
        self.gpu_group_id, gpu_group_created = self.rdc_util.create_gpu_group(
            self.rdc_handle, self.gpu_group_name, self.gpu_indexes
        )

        # Create the field group
        self.field_ids = field_ids
        self.field_group_name = field_group_name.encode()
        self.field_group_id, field_group_created = self.rdc_util.create_field_group(
            self.rdc_handle, self.field_group_name, self.field_ids
        )
        ...
        result = rdc.rdc_field_watch(
            self.rdc_handle,
            self.gpu_group_id,
            self.field_group_id,
            self.update_freq,
            self.max_keep_age,
            self.max_keep_samples,
        )

        if rdc_status_t(result) != rdc_status_t.RDC_ST_OK:
            raise Exception(
                "RdcReader fail to watch group "
                + str(self.gpu_group_id)
                + ", field group "
                + str(self.field_group_id)
                + ":"
                + str(result)
            )
    ...
    def process(self):
        ...
        for gindex in self.gpu_indexes:
            for fid in self.field_ids:
                ...
                result = rdc.rdc_field_get_latest_value(
                    self.rdc_handle, gindex, fid, value
                )
                ...
                if rdc_status_t(result) == rdc_status_t.RDC_ST_OK:
                    ...
                    if self.unit_converter != None and fid in self.unit_converter:
                        if value.type.value == rdc_field_type_t.INTEGER:
                            value.value.l_int = int(
                                value.value.l_int * self.unit_converter[fid]
                            )
                        if value.type.value == rdc_field_type_t.DOUBLE:
                            value.value.dbl = int(
                                value.value.dbl * self.unit_converter[fid]
                            )
        ...
        if (
            len(self.gpu_indexes) != 0
            and len(self.field_ids) != 0
            and has_succeed == False
        ):
            self.try_reconnect()
    ...
    def handle_field(self, gpu_index, value):
        ...
        print(
            "%d %s:%d %s:%d"
            % (value.ts, gpu_str, value.field_id.value, field_name, value.value.l_int)
        )
