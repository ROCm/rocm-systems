...
class PrometheusReader(RdcReader):
    def __init__(
        ...
        for fid in self.field_ids:
            ...
            rdc_metric_type = rdc_field_t.get_rdc_metric_type(
                rdc_field_t.get_field_name(fid)
            )

            field_name = field_name.lower()

            if rdc_metric_type == 1:
                self.gauges[fid] = Gauge(
                    field_name, field_name, labelnames=["gpu_index"]
                )
            elif rdc_metric_type == 2:
                self.counters[fid] = Counter(
                    field_name, field_name, labelnames=["gpu_index"]
                )
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rdc_update_freq",
        default=10,
        help="The fields update frequency in seconds (default: 10)",
    )
    parser.add_argument(
        "--rdc_max_keep_age",
        default=3600,
