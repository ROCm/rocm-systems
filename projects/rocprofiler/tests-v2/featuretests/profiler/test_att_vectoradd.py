...
def test_addr(csv):
    ...
    assert (
        addrs.max() - addrs.min() < 2**24
    )  # Kernels are not anywhere near that large
...
def test_memory_list(csv):
    ...
    assert (
        "global_load" in inst_list
        or "buffer_load" in inst_list
        or "flat_load" in inst_list
    )


def test_mean_cycles(csv):
    cycles = np.array(
        [c / float(h) for c, h in zip(csv["Cycles"], csv["Hitcount"]) if c != 0]
    )
    print("cycles", cycles)
...
def test_memory_cycles(csv):
    ...
    memory_ops = [
        c for s, c in zip(csv["Instruction"], csv["Cycles"]) if is_memory_op(s)
    ]
    print(
        "memory_ops", memory_ops
    )  # Memory ops should be more than half the total cycles
