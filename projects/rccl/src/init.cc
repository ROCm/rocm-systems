...
static ncclResult_t commFree(ncclComm_t comm) {
  ...
  // NOTE: allocated with hipExtMallocWithFlags (not CUmem), so use plain hipFree.
  if (comm->tempBuff) {
    CUDACHECK(hipFree(comm->tempBuff));
...
// Same process as transport/p2p.cc P2P_SAME_PID, for direct pointers when IPC is unnecessary.
#define RCCL_P2P_SAME_PID_PTR(a, b) \
...
static void rcclTeardownTwoShotSyncFence(struct ncclComm* comm) {
  if (comm->peerTwoShotSyncImported) {
    for (int r = 0; r < comm->nRanks; r++) {
      if (comm->peerTwoShotSyncIpcOpened[r]) {
        CUDACHECKIGNORE(cudaIpcCloseMemHandle(comm->peerTwoShotSyncImported[r]));
  ...
  if (comm->twoShotSyncBuff) {
    // allocated with hipExtMallocWithFlags, so use plain hipFree
    (void)hipFree(comm->twoShotSyncBuff);
...
static ncclResult_t rcclSetupTwoShotSyncFence(struct ncclComm* comm) {
  ...
  cudaError_t gh = cudaSuccess;
  // Must use plain hipMalloc (not CUmem) so cudaIpcGetMemHandle succeeds below.
  // Must use plain hipMalloc (not CUmem) so cudaIpcGetMemHandle succeeds below.
  CUDACHECKGOTO(hipExtMallocWithFlags((void**)&syncDev, (size_t)nranks * sizeof(uint32_t), hipDeviceMallocDefault), nret, fail_sync_early);
  ...
  gh = cudaIpcGetMemHandle(&syncTable[rank].ipcHandle, comm->twoShotSyncBuff);
  if (gh != cudaSuccess) {
    INFO(NCCL_INIT, "RCCL two-shot: sync IpcGetMemHandle failed (%s)", cudaGetErrorString(gh));
    hipFree(syncDev);  // allocated with hipExtMallocWithFlags
    comm->twoShotSyncBuff = nullptr;
    free(syncTable);
    return ncclSuccess;
  }
  ...
  for (int r = 0; r < nranks; r++) {
    ...
    if (RCCL_P2P_SAME_PID_PTR(myInfo, peerInfo)) {
...
fail_sync_malloc:
  (void)hipFree(syncDev);  // allocated with hipExtMallocWithFlags
  comm->twoShotSyncBuff = nullptr;
...
static ncclResult_t ncclCommInitRankFunc(struct ncclAsyncJob* job_) {
  ...
  // NOTE: must use plain hipExtMallocWithFlags (not ncclCuMemAlloc/ncclMemAlloc) so that
  // cudaIpcGetMemHandle works in rcclSetupPeerTempStaging.  CUmem-backed allocations
  // cannot be shared via IPC handles.
  if (IsArchMatch(archName,"gfx950") || rcclParamTwoShotAllReduce()) {
    CUDACHECK(hipExtMallocWithFlags((void**)&(comm->tempBuff), TEMP_BUFF_SIZE, hipDeviceMallocDefault));
  }
