...
static ncclResult_t commAlloc(struct ncclComm* comm, struct ncclComm* parent, int ndev, int rank) {
  ...
  // [RCCL] doneEvent / lastStream initialization (cudaGetDevice + compCap now done
  // earlier per upstream v2.30 refactor; dropped redundant call here).
...
#endif
  // [RCCL] Upstream-side context/busId/nvml init dropped: identical work already
  // happens in non-conflict context above (lines ~875-887).
...
static ncclResult_t initTransportsRank(struct ncclComm* comm, struct ncclComm* parent, uint64_t timers[TIMERS_INIT_COUNT]) {
  ...
  // [RCCL] Topology-aware nChannels selection: only force min when GPU/rank count diverge
  // and NET nodes exist (preserves RCCL bandwidth for symmetric configs).
