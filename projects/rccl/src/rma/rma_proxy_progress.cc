...
static ncclResult_t ncclRmaProxyIssuePutSignal(ncclRma_t* ncclRma, struct ncclRmaProxyCtx* ctx,
  ...
  if (ps->signal.op == 0) {
    NCCLCHECK(ncclRma->iput(ctx->ginCtx, 0, ps->srcOff, ps->srcHandle, ps->size, ps->dstOff, ps->dstHandle,
                            ps->targetRank, &ps->request));
  } else {
    NCCLCHECK(ncclRma->iputSignal(ctx->ginCtx, 0, ps->srcOff, ps->srcHandle, ps->size, ps->dstOff, ps->dstHandle,
...
static ncclResult_t ncclRmaProxyTestPutSignal(ncclRma_t* ncclRma, struct ncclRmaProxyCtx* ctx,
  ...
  NCCLCHECK(ncclRma->test(ctx->ginCollComm, ps->request, done));
  if (*done) {
...
static ncclResult_t ncclRmaProxyFlushNicGpuPath(ncclRma_t* ncclRma, struct ncclRmaProxyCtx* ctx) {
  ...
  NCCLCHECK(ncclRma->iput(ctx->ginCtx, 0, flushOff, ctx->flushBufMhandle, sizeof(uint64_t), flushOff,
                          ctx->flushBufMhandle, ctx->comm->rank, &request));
  int done = 0;
  while (!done) {
    NCCLCHECK(ncclRma->test(ctx->ginCollComm, request, &done));
  }
  return ncclSuccess;
}
...
ncclResult_t ncclRmaProxyProgress(ncclRma_t* ncclRma, void* rmaProxyCtx) {
  ...
  if (ncclRma->rmaProgress) NCCLCHECK(ncclRma->rmaProgress(ctx->ginCtx));
