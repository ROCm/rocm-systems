...
// [RCCL] RCCL-only RMA host launch functions follow. Upstream v2.30 has no
// counterpart in this file; they live elsewhere in the upstream tree. Kept
...
ncclResult_t ncclRmaPutProxy(struct ncclComm* comm, struct ncclKernelPlan* plan, cudaStream_t stream){
  ...
  for (int i = 0; i < nRmaTasksProxy; i++) {
    ...
    // [RCCL] v2.30 sync: ncclRmaProxyDesc network fields moved into the `putSignal` union variant.
    desc->putSignal.srcOff = task->srcWinOffset;
    desc->putSignal.srcHandle = ncclDevrGetRmaDevWin(task->srcWinHost, ctx);
    desc->putSignal.dstOff = task->peerWinOffset;
    desc->putSignal.dstHandle = ncclDevrGetRmaDevWin(task->peerWinHost, ctx);
    ...
    desc->rmaDescState = ncclRmaDescStateInit;  // [RCCL] v2.30 sync: ncclRmaDescStatePending renamed Init.
    ...
    // If the signal mode is NCCL_SIGNAL, we use the per-rank signal for the target rank
      ...
      desc->putSignal.signal.offset = comm->rank * sizeof(uint64_t); // Write to our rank slot in peer's buffer
      desc->putSignal.signal.signalMhandle = rmaProxyCtx->signalsMhandle;
    ...
    INFO(NCCL_COLL, "ncclRmaPutProxy enqueued Desc: rank=%d peer=%d ctx=%d size=%ld signalMode=%d readySeq=%lu doneSeq=%lu",
      comm->rank, task->peer, ctx, task->count * ncclTypeSize(task->datatype), task->signalMode, (uint64_t)desc->opSeq, (uint64_t)desc->opSeq);

    // Write descriptor to queue
    uint32_t idx = pi & (rmaProxyCtx->queueSize - 1);
    rmaProxyCtx->circularBuffers[peer * rmaProxyCtx->queueSize + idx] = desc;  // [RCCL] v2.30 sync: pendingQueues renamed circularBuffers.
