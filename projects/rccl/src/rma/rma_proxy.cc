  ...
  if (type == NCCL_PTR_HOST) {
    ...
    if (ncclParamDmaBufEnable() && (props.ptrSupport & NCCL_PTR_DMABUF)) {
      ...
      if (registrationResult != ncclSuccess) {
        ...
        if (dmabufResult == ncclSuccess) {
          NCCLCHECK(ginComm->regMrSymDmaBuf(ginCollComm, addr, size, type, 0, dmabufFd, mr_flags, mhandle, ginHandle));
          close(dmabufFd);
        }
      }
    }
...
static ncclResult_t ncclRmaProxyCtxAllocGraph(struct ncclComm* comm, ncclGin_t* ginComm,
  ...
  NCCLCHECK(ncclRmaProxyRegMrSym(ginComm, rmaProxyCtx->ginCollComm, rmaProxyCtx->props,
                                 ...
                                 &rmaProxyCtx->cpuAccessSignalsGinHandle));
  // Allocate the host buffer to track the expected values of the signals
...
void* ncclRmaProxyProgressThread(struct ncclRmaProxyState* rmaProxyState_) {
  ...
  while (1) {
    ...
    if (rmaProxyState->rmaProgress == 1) {
      ...
      INFO_LOC(NCCL_ALL, "[RMA Proxy Progress Thread] state unknown %d", rmaProxyState->ginProgress);
      rmaProxyState->ginProgress = -2;
      return NULL;
    }
  }
...
ncclResult_t ncclRmaProxyConnectOnce(struct ncclComm* comm) {
  ...
  for (int n = 0; n < rmaCommCount; n++) {
    void* listenComm;
    NCCLCHECKGOTO(rmaProxyState->ncclGin->listen(rmaProxyState->ginInstance, localRmaDevs[n],
    ...
    NCCLCHECKGOTO(rmaProxyState->ncclGin->connect(comm->netContext, handles, comm->nRanks, comm->rank, listenComm,
                  ...
                  ret, fail);
    NCCLCHECKGOTO(rmaProxyState->ncclGin->getProperties(localRmaDevs[n], &rmaProxyState->props[n]), ret, fail);
  ...
  INFO(NCCL_INIT, "Rank %d ncclRmaProxyConnectOnce: ginCommCount %d rmaProxyCtxCount:%d needsProxyProgress %d",
       comm->rank, rmaProxyState->ginCommCount, rmaProxyState->rmaProxyCtxCount, rmaProxyState->needsProxyProgress);
