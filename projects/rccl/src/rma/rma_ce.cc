...
// [RCCL] rocmwrap.h (HIP shim) instead of cudawrap.h — cudawrap's `pfn_`-prefixed CUCHECKGOTO
// (expecting CUDA driver symbol pointers) doesn't apply on the ROCm build. The few
// `ncclCuStreamBatchMemOp` call sites are rewritten below to use the direct hipStreamBatchMemOp.
#include "rocmwrap.h"
#include "rma/rma.h"
...
ncclResult_t ncclRmaCePutLaunch(struct ncclComm* comm, struct ncclKernelPlan* plan, cudaStream_t stream){
  ...
  for (int i = 0; i < nRmaTasksCe; i++) {
    ...
    if (persistent) {
      ...
      ackOps[1].writeValue.flags = /*[RCCL HIP-shim]*/0;
      // [RCCL] direct HIP call; ncclCuStreamBatchMemOp wrapper lives in cudawrap.h which we don't pull in.
    ...
    if (task->signalMode != NCCL_SIGNAL_NONE) {
      ...
      if (!persistent) {
        ...
        CUCHECKGOTO(cuStreamWriteValue64(stream, (CUdeviceptr)ceCtx->signalOpSeqsDev, ceCtx->signalOpSeqs[task->peer], /*[RCCL HIP-shim]*/0), ret, fail);
    ...
    if (!persistent) {
      ...
      for (int i = 0; i < task->npeers; i++) {
        ...
        for (int s = 0; s < task->nsignals[i]; s++) {
          ...
          signalOps[1].writeValue.flags = /*[RCCL HIP-shim]*/0;
          // [RCCL] direct HIP call.
