...
// [RCCL] ncclGinIbProxy is gone — the proxy backend is now ncclRma_t ncclRmaIbProxy.
...
try_proxy:
  ...
  if (!gdrSupport) return ncclInternalError;
  // [RCCL] Proxy is now ncclRma_t; no struct morph needed — ncclGinIb is
  // pre-wired to the RMA proxy ops via wrappers in the static initialiser.
...
// Forward declarations for functions referenced in ncclGinIb dispatcher below.
...
// [RCCL] ncclGinIb (the "GIN_IB" built-in) is defined near the end of this
// file, after the RMA proxy ops. It is a ncclGin_v14_t (no iput/iget slots)
// pre-wired to the RMA proxy ops via wrappers for the compatible slots.
...
// [RCCL] ncclGinIb: RCCL-only dispatcher that picks between GDAKI and Proxy at
// runtime. ncclGin_t is now v14 (GDAKI-only device API; no iput/iget slots).
// On AMD the GDAKI path is disabled; the proxy ops that fit the gin_v14_t
// signature are wired in directly. regMrSym/regMrSymDmaBuf have an extra
// ginHandle** out-param in gin_v14_t that the RMA proxy doesn't use -- thin
// wrappers ignore it.
static ncclResult_t ncclGinIbProxyRegMrSymWrapper(void* collComm, void* data, size_t size, int type,
                                                   uint64_t mrFlags, void** mhandle, void** /*ginHandle*/) {
  return ncclRmaIbProxyRegMrSym(collComm, data, size, type, mrFlags, mhandle);
}
static ncclResult_t ncclGinIbProxyRegMrSymDmaBufWrapper(void* collComm, void* data, size_t size, int type,
                                                         uint64_t offset, int fd, uint64_t mrFlags,
                                                         void** mhandle, void** /*ginHandle*/) {
  return ncclRmaIbProxyRegMrSymDmaBuf(collComm, data, size, type, offset, fd, mrFlags, mhandle);
}
// Dispatcher init: selects GIN backend based on NCCL_GIN_TYPE env-var.
static ncclResult_t ncclGinIbInit(void** ctx, uint64_t commId, ncclDebugLogger_t logFunction) {
...
ncclGin_t ncclGinIb = {"GIN_IB",
                       ...
                       ncclIbDevices,
                       NULL,                                    // getGinProperties (proxy has none)
                       ncclRmaIbProxyGetProperties,
                       ncclIbListen,
                       ncclGinIbConnect,
                       NULL,                                    // createContext (GDAKI-only in gin_v14)
                       ncclGinIbProxyRegMrSymWrapper,
                       ncclGinIbProxyRegMrSymDmaBufWrapper,
                       ncclRmaIbProxyDeregMrSym,
                       ncclRmaIbProxyDestroyContext,
                       ncclGinIbCloseColl,
                       ncclIbCloseListen,
