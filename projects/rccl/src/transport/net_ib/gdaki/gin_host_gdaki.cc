...
ncclResult_t ncclGinGdakiCreateContext(void *collComm, int nSignals, int nCounters, int nContexts, int queueDepth,
  ...
  // [RCCL] Use the IB context/PD owned by the shared ncclGinIbCollComm (`cComm->ib.{context,pd}`),
  // populated by gin_ib_open_collcomm. The RCCL-owned `gdaki_ctx->ib_{dev,ctx,pd}` allocation
  // pre-dated cComm->ib and is now removed; all downstream code already reads cComm->ib.*.
  ...
  // [RCCL] keep RCCL's `props.port` lookup over upstream's hard-coded `1` (correct for non-mlx5 NICs);
  // query via the shared cComm->ib.context.
...
out:
  if (status != ncclSuccess) {
    ...
    if (gdaki_ctx) {
      // [RCCL] ib_pd / ib_ctx are owned by cComm->ib now; nothing to deallocate here.
