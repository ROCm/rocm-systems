...
ncclResult_t ncclRmaSocketProxyInit(void** ctx, uint64_t commId, ncclDebugLogger_t logFunction) {
  ...
  // [RCCL] RMA/Socket pins and CPU-maps arbitrary user device buffers through GDRCopy. On AMD, ncclGdrCopy
  // only marks RCCL's own fine-grained allocations as host-visible, so this plugin is unavailable.
  INFO(NCCL_INIT | NCCL_NET, "RMA/Socket : not supported on AMD GPUs (no GDRCopy buffer mapping)");
