...
static ncclResult_t recvSetup(struct ncclComm* comm, struct ncclTopoGraph* graph, struct ncclPeerInfo* myInfo, struct ncclPeerInfo* peerInfo, struct ncclConnect* connectInfo, struct ncclConnector* recv, int channelId, int connIndex) {
  ...
  // [RCCL] Use myInfo->rank as the receiver uses its own NIC; P2P_NET takes the intra-net path first.
  int proxyRank = myInfo->rank;
  int64_t netId;
  ...
  if (req.useGdr) {
    ...
    // [RCCL] PR #2766: caller-side managed→ncclTopoFlushNone short-circuit (was previously a
    // `bool netManaged` param to ncclTopoNeedFlush, moved here when graph.h adopted the
    // enum-typed `ncclTopoFlushType` API. See paths.cc:ncclTopoNeedFlush HIP branch.)
    if (managed) {
      req.needFlush = ncclTopoFlushNone;
    } else {
