  ...
  if (args->state == ncclProxyOpReady) {
    for (int s = 0; s < args->nsubs; s++) {
      ...
      if (proxyState->proxyTrace) {
        proxyState->proxyTrace->addNewProxyOp(sub->traceKey, sub->traceInfo, facebook_rccl::ProxyOpType::SEND,
  ...
  if (args->state == ncclProxyOpProgress) {
    ...
    for (int s = 0; s < args->nsubs; s++) {
      ...
      if (sub->posted < sub->nsteps && sub->posted < sub->done + maxDepth) {
        ...
        if (proxyState->proxyTrace) {
          proxyState->proxyTrace->updateProxyOpCounter(sub->traceKey, facebook_rccl::ProxyCounterTypes::POSTED,
                                                       sub->posted);
      ...
      if (sub->transmitted < sub->posted && sub->transmitted < sub->done + NCCL_STEPS) {
        ...
        if (proxyState->proxyTrace) {
          proxyState->proxyTrace->updateProxyOpCounter(sub->traceKey, facebook_rccl::ProxyCounterTypes::RECV_TAIL,
                                                       *recvTail);
          proxyState->proxyTrace->updateProxyOpCounter(sub->traceKey, facebook_rccl::ProxyCounterTypes::TAIL_OR_HEAD,
                                                       tail);
          proxyState->proxyTrace->updateProxyOpCounter(
            sub->traceKey, facebook_rccl::ProxyCounterTypes::FIFO_SZ_OR_HEAD_CACHE, connFifo[buffSlot].size);
        }
          ...
          if (ready) {
            ...
            if (proxyState->proxyTrace) {
              proxyState->proxyTrace->updateProxyOpCounter(sub->traceKey,
                                                           ...
                                                           sub->reg ? 1 : sub->transmitted + args->sliceSteps);
            }
            ...
            if (*requestPtr != NULL) {
              if (proxyState->proxyTrace) {
                proxyState->proxyTrace->setProxyOpTimestamp(sub->traceKey,
                                                            facebook_rccl::ProxyCounterTypes::KERNEL_COPY_READY);
              }
              TRACE(NCCL_NET,
              ...
              if (proxyState->proxyTrace) {
                proxyState->proxyTrace->updateProxyOpCounter(
                  sub->traceKey, facebook_rccl::ProxyCounterTypes::TRANSMITTED, sub->transmitted);
              }
              args->idle = 0;
              continue;
            }
      ...
      if (sub->done < sub->transmitted) {
        ...
        if (done) {
          ...
          if (proxyState->proxyTrace) {
            proxyState->proxyTrace->updateProxyOpCounter(sub->traceKey, facebook_rccl::ProxyCounterTypes::DONE,
                                                         sub->done);
          }
          if (resources->shared == 0) {
            volatile uint64_t* sendHead = resources->gdcSync ? resources->gdcSync : &resources->sendMem->head;
...
static ncclResult_t recvProxyProgress(struct ncclProxyState* proxyState, struct ncclProxyArgs* args) {
  ...
  if (args->state == ncclProxyOpReady) {
    ...
    for (int s = 0; s < args->nsubs; s++) {
      ...
      ncclProfilerRecordProxyOpEventState(s, args, ncclProfilerProxyOpInProgress_v4);
      if (proxyState->proxyTrace) {
        proxyState->proxyTrace->addNewProxyOp(sub->traceKey, sub->traceInfo, facebook_rccl::ProxyOpType::RECV,
                                              sub->channelId, sub->nsteps, sub->nbytes, sub->peer);
      }
      ...
      if (subGroup->posted > subGroup->received) {
        ...
        if (done) {
          ...
          for (int i = 0; i < subGroup->groupSize; i++) {
            ...
            if (proxyState->proxyTrace) {
              proxyState->proxyTrace->updateProxyOpCounter(sub->traceKey, facebook_rccl::ProxyCounterTypes::RECEIVED,
                                                           sub->received);
            }
            if (step < sub->nsteps) {
              struct recvNetResources* resources = (struct recvNetResources*)(sub->connection->transportResources);
            ...
            if (resources->gdcFlush) {
              ...
              if (subGroup->requests[step % NCCL_STEPS]) {
                for (int i = 0; i < subGroup->groupSize; i++) {
                  ...
                  if (proxyState->proxyTrace) {
                    proxyState->proxyTrace->updateProxyOpCounter(
                      sub->traceKey, facebook_rccl::ProxyCounterTypes::FLUSHED, sub->received);
                  }
                }
              }
              if (once) {
      ...
      if (subGroup->received > subGroup->transmitted) {
        ...
        if (done) {
          for (int i = 0; i < subGroup->groupSize; i++) {
            ...
            if (proxyState->proxyTrace) {
              proxyState->proxyTrace->updateProxyOpCounter(sub->traceKey, facebook_rccl::ProxyCounterTypes::TRANSMITTED,
                                                           sub->transmitted);
            }
            if (step < sub->nsteps) {
              std::atomic_thread_fence(std::memory_order_seq_cst);
      ...
      for (int i = 0; i < subGroup->groupSize; i++) {
        ...
        if (sub->transmitted > sub->done) {
          ...
          if (proxyState->proxyTrace) {
            ...
            proxyState->proxyTrace->updateProxyOpCounter(
              sub->traceKey, facebook_rccl::ProxyCounterTypes::FIFO_SZ_OR_HEAD_CACHE, sub->base + sub->done);
          }
          while (
