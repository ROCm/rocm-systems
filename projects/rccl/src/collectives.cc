...
ncclResult_t ncclAlltoAll_impl(const void* sendbuff, void* recvbuff, size_t count, ncclDataType_t datatype,
  ...
  if (!forceP2pSendRecv && comm->topo->pivotA2AEnabled &&
      comm->nChannels >= comm->topo->pivotA2ANumBiRings * 2 &&
    ...
    if (!forceP2pSendRecv && rcclUseAlltoAllGda(comm) && msgSize <= comm->rocshmemThreshold) {
    ...
    if (!forceP2pSendRecv && ncclAllToAllGinSdmaEligible(comm, sendbuff, recvbuff, count, datatype)) {
    ...
    if (!forceP2pSendRecv && (comm->config.CTAPolicy & NCCL_CTA_POLICY_ZERO) &&
    ...
    if (!forceP2pSendRecv && rcclAlltoAllShouldTakeDdaPath(comm, totalBytes, ceAlltoAllAllowed)) {
