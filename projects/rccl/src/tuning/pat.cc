...
int ncclPatEnable(struct ncclComm* comm) {
  ...
  // [RCCL] init may force PAT on (comm->forcePatEnable); honour it here too, as graph/tuning.cc does,
  // so ncclTransportPatConnect sets up the connections the tuner will then select PAT for.
  if (patEnable == 0 && comm->forcePatEnable) patEnable = 1;
