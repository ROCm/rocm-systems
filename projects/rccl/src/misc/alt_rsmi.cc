    ...
    *fw_version = 0;
    for (uint32_t card = 0; card < maxCards; card++) {
        ...
        snprintf(path, sizeof(path),
                 "/sys/class/drm/card%u/device/fw_version/mec_fw_version", card);
...
static int findCardForBdf(uint32_t domain, uint8_t bus, uint8_t device, uint8_t function)
{
    for (uint32_t card = 0; card < 128; card++) {
        char path[256];
        snprintf(path, sizeof(path), "/sys/class/drm/card%u/device/uevent", card);
        FILE *fp = fopen(path, "r");
...
int ARSMI_get_fabric_info(uint32_t dv_ind, struct ARSMI_fabricInfo *info)
{
    ...
    char dir[256];
    snprintf(dir, sizeof(dir), "/sys/class/drm/card%d/device/ualink", card);
    ...
    if (readUalinkStr(dir, "accel_state", buf, sizeof(buf)) == 0) {
        if      (strcmp(buf, "active")       == 0) info->accel_state = ARSMI_FABRIC_STATE_ACTIVE;
        else if (strcmp(buf, "ready")        == 0) info->accel_state = ARSMI_FABRIC_STATE_READY;
        else if (strcmp(buf, "configured")   == 0) info->accel_state = ARSMI_FABRIC_STATE_CONFIGURED;
        else if (strcmp(buf, "error")        == 0) info->accel_state = ARSMI_FABRIC_STATE_ERROR;
        else                                       info->accel_state = ARSMI_FABRIC_STATE_UNCONFIGURED;
    }

    info->supported = (info->fabric_type == ARSMI_FABRIC_TYPE_UALOE ||
                       info->fabric_type == ARSMI_FABRIC_TYPE_UALLINK) &&
                      (info->accel_state == ARSMI_FABRIC_STATE_ACTIVE ||
                       info->accel_state == ARSMI_FABRIC_STATE_READY);
    ...
    if (readUalinkStr(dir, "addr_mode", buf, sizeof(buf)) == 0)
        info->addr_mode = (strcmp(buf, "source-identification") == 0)
                          ? ARSMI_FABRIC_ADDR_MODE_SOURCE_IDENTIFICATION
                          : ARSMI_FABRIC_ADDR_MODE_SOURCE_ALIASING;
