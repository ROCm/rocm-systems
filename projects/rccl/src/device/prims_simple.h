...
class Primitives<
  ...
  int repeat;
  bool skip_fence = 0;
  LDSPtr<ncclShmemData> ncclShmem;
...
private:
  ...
  inline __device__ void barrier() {
    if (nthreads == WARP_SIZE)
      __syncwarp();
        ...
        barrier_generic(__threadfence_block(), nworkers, barrier_next, &ncclShmem->groups[group].barrier, *ncclShmem);
      #else
        barrier_generic(__threadfence(), nworkers, barrier_next, &ncclShmem->groups[group].barrier, *ncclShmem);
      #endif
  }
  ...
  inline __device__ void patBarrier() {
    #if defined(__gfx942__) || defined(__gfx950__)
      barrier_generic(__threadfence_block(), NCCL_PAT_NWORKERS, barrier_next_pat, &ncclShmem->barrier_pat, *ncclShmem);
    #else
      barrier_generic(__threadfence(), NCCL_PAT_NWORKERS, barrier_next_pat, &ncclShmem->barrier_pat, *ncclShmem);
    #endif
  }
  ...
  inline __device__ void postPeer(bool dataStored) {
    if (skip_fence){
      ...
      barrier_generic(asm volatile("s_waitcnt lgkmcnt(0) vmcnt(0)"), nworkers, barrier_next, &ncclShmem->groups[group].barrier, *ncclShmem);
      __atomic_signal_fence(__ATOMIC_SEQ_CST);
    }
...
public:
  static inline __device__ void sendPeerNotify(int peer, int connIndex, int steps, __shared__ ncclShmemData& ncclShmem) {
  ...
  static inline __device__ void recvPeerNotify(int peer, int connIndex, int steps, __shared__ ncclShmemData& ncclShmem) {
    int spins = 0;
#ifdef ENABLE_WARP_SPEED
    ncclDevChannelPeer* peerPtr = ncclShmem.warpChannel[threadIdx.x/WARP_SIZE].peers[peer];
#else
...
public:
  __forceinline__ __device__ Primitives(
      ...
      __shared__ ncclShmemData& shmem, ncclShmemPerWarpPtr ncclShmemPerWarp_,
...
#endif
    ncclShmem(LDSPtr<ncclShmemData>(&shmem)), ncclShmemPerWarp(ncclShmemPerWarp_){
