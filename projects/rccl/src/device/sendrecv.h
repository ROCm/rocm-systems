...
struct RunWorkBatch<ncclFuncSendRecv, T, RedOp, NCCL_ALGO_RING, NCCL_PROTO_SIMPLE> {
  ...
  if (isCopy) {
    ...
    rccl::reduceCopyOrTdm<COLL_UNROLL * 2, 0, RedOp, T, 0, 1, 1, 0, 1, 1, /*PreOpSrcs=*/0>(
      subtid, subtn, 0, false, 1, work->sendAddr, work->recvAddr, (ssize_t)work->sendBytes);
#else
      rccl::reduceCopyOrTdm<COLL_UNROLL, 0, RedOp, T, 0, 1, 1, 0, 1, 1, /*PreOpSrcs=*/0>(
        subtid, subtn, 0, false, 1, work->sendAddr, work->recvAddr, (ssize_t)work->sendBytes);
#endif
  } else if (isSend) {
    if (work->sendProtoLL) {
