...
if not (unroll_override_unroll.isdigit()
        and 0 < int(unroll_override_unroll) <= 32
        and int(unroll_override_unroll) % 2 == 0):
  sys.exit("generate.py: %s must be an even integer in 2..32, got %r"
           % (unroll_override_env, os.environ[unroll_override_env]))
...
# unroll_override_kernels caps it. Kernels already at the override value are left
# alone, which is what makes the default of 32 a no-op for this unroll-32 set.
def compiled_unroll(fn):
  if unroll_override_unroll == fn.unroll:
    return fn.unroll
  sym = fn_sym(fn).lower()
