...
all_protos    = ["LL","LL128","SIMPLE","NAN"]
# NaN-flag protocol: readiness is "no element is NaN", so it only has meaning for
# floating-point types. fp8 is left out because gfx942 uses the _fnuz variants,
# where the all-ones sentinel is a finite value rather than NaN (rccl_float8.h).
nan_tys       = ["f16","f32","f64","bf16"]
...
# Unrolls generated by a multi-arch build. Derived, so pinning another unroll generates it.
default_unrolls = ["1", "2", "4"] + list(unroll_arch_requirement)

# Kernels whose body is compiled at a lower unroll factor than their symbol names.
# Only the Unroll template argument of DEFINE_ncclDevFunc changes: the symbol, the
# device-table slot it occupies and its arch guard all stay tied to the advertised
# unroll, so the host still reaches these through that unroll's table. Keyed by the
# lowercased symbol, which is the specialized_*.cpp stem build timings report.
#
# Membership is every kernel that took longer than 60s to compile in a gfx1250
# multi-arch build, listed slowest first (435s down to 60s). All 33 are unroll-32
# SIMPLE AllReduce reductions and together were 4171s of that build's compile time,
# including its critical path. 24 stays even, which the Unroll*(16/sizeof(T))/2
# expansion in reduceCopy() needs to divide exactly.
unroll_override_unroll = "2"
unroll_override_kernels = {
  "allreduce_ring_simple_minmax_f8e4m3_1_0_32",
  "allreduce_ring_simple_minmax_f8e5m2_1_0_32",
  "allreduce_ring_simple_minmax_u8_1_0_32",
  "allreduce_tree_simple_premulsum_f8e5m2_1_0_32",
  "allreduce_tree_simple_sum_f8e5m2_1_0_32",
  "allreduce_tree_simple_sum_f8e4m3_1_0_32",
  "allreduce_tree_simple_premulsum_f8e4m3_1_0_32",
  "allreduce_ring_simple_prod_f8e5m2_1_0_32",
  "allreduce_ring_simple_prod_f8e4m3_1_0_32",
  "allreduce_tree_simple_minmax_f16_1_0_32",
  "allreduce_tree_simple_minmax_f8e5m2_1_0_32",
  "allreduce_tree_simple_minmax_f8e4m3_1_0_32",
  "allreduce_tree_simple_prod_f8e5m2_1_0_32",
  "allreduce_tree_simple_prod_f8e4m3_1_0_32",
  "allreduce_ring_simple_premulsum_f8e4m3_1_0_32",
  "allreduce_ring_simple_premulsum_f8e5m2_1_0_32",
  "allreduce_ring_simple_sum_f8e5m2_1_0_32",
  "allreduce_ring_simple_sum_f8e4m3_1_0_32",
  "allreduce_tree_simple_premulsum_f8e4m3_0_0_32",
  "allreduce_tree_simple_sum_f8e5m2_0_0_32",
  "allreduce_tree_simple_sum_f8e4m3_0_0_32",
  "allreduce_tree_simple_premulsum_f8e5m2_0_0_32",
  "allreduce_tree_simple_minmax_f8e5m2_0_0_32",
  "allreduce_tree_simple_prod_f8e4m3_0_0_32",
  "allreduce_tree_simple_minmax_f8e4m3_0_0_32",
  "allreduce_tree_simple_minmax_f16_0_0_32",
  "allreduce_tree_simple_prod_f8e5m2_0_0_32",
  "allreduce_tree_simple_minmax_u8_1_0_32",
  "allreduce_tree_simple_sumpostdiv_u8_1_0_32",
  "allreduce_ring_simple_minmax_f16_1_0_32",
  "allreduce_tree_simple_prod_u8_1_0_32",
  "allreduce_tree_simple_premulsum_bf16_0_1_32",
  "allreduce_tree_simple_premulsum_u8_1_0_32",
}
unroll_override_applied = set()
...
def calc_unroll_and_pipeline_for_local_arch():
  ...
  if not is_local_arch_only:
    return (default_unrolls, all_pipelines)
...
# The unroll the kernel body is compiled with, which is fn.unroll unless
# unroll_override_kernels caps it.
def compiled_unroll(fn):
  sym = fn_sym(fn).lower()
  if sym in unroll_override_kernels:
    unroll_override_applied.add(sym)
    return unroll_override_unroll
  return fn.unroll
...
with open(os.path.join(gensrc, "host_table.cpp"), "w") as f:
  ...
  for fn in [fn for fn in func_rows if fn.unroll == local_unroll[0]]:
    fn_id = -1
    if fn is not None:
      guard = get_arch_guard(fn)
    ...
    for fn in fns:
      ...
      out(
        ...
        .format(sym=sym, coll=fn.coll, redop_cxx=redop_to_cxx[fn.redop], ty_cxx=ty_to_cxx[fn.ty],
                algo=(fn.algo or "RING"), proto=(fn.proto or "SIMPLE"), acc=fn.acc, pipeline=fn.pipeline, unroll=compiled_unroll(fn), reg=fn.reg)
      )
      if guard: 
        out("#endif\n")
  ...
  with open(filepath, "w") as f:
    ...
    out(
      ...
      .format(sym=sym, coll=fn.coll, redop_cxx=redop_to_cxx[fn.redop],
              ...
              unroll=compiled_unroll(fn), reg=fn.reg, func_name=func_name)
    )
    if guard:
      out("#endif\n")
...
_proto_cost = {"SIMPLE": 0, "LL": 1, "LL128": 2, "NAN": 2}
...
with open(os.path.join(gensrc, "specialized_files.txt"), "w") as f:
  for filename, func_name, guard, _ in specialized_filelist:
    # Must stay byte-identical to the #if get_arch_guard() emits: the device linker
    # filters on this, so any drift means an undefined symbol or a compiled-out body.
    # That is why get_arch_guard() carries the ENABLE_LL128 term itself rather than
    # having it added only here, which would make this predicate the stricter of the two.
    f.write("%s %s %s\n" % (filename, func_name, guard or ""))

print("-- Generated %d specialized kernel files in %s" % (len(specialized_filelist), specialized_dir))

if unroll_override_applied:
  print("-- Unroll override: %d kernels compiled at unroll %s"
        % (len(unroll_override_applied), unroll_override_unroll))
