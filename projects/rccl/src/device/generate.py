...
if is_msccl_kernels:
  for redop in all_redops:
    if redop in ("Sum", "Prod", "MinMax"):
      for ty in all_tys:
        msccl_fname = f"msccl_kernel_{redop}_{ty}.cpp"
        msccl_device_files.append(msccl_fname)
        with open(os.path.join(gensrc, msccl_fname), "w") as f:
          print("-- Generating %s" % os.path.join(gensrc, msccl_fname))
...
################################################################################
# Generate <gensrc>/device_unity.cpp
#
# This unity file includes all device code in a single translation unit,
# enabling compilation without -fgpu-rdc. The collective headers have include
# guards and named namespaces (instead of anonymous) to prevent symbol
# collisions when combined.
#
# The unity file uses relative includes (just filenames) since gensrc/ is
# already on the include path. The two main device files (common.cu.cpp and
# onerank.cu.cpp) use a placeholder path that CMake will replace with the
# actual hipified path at build time.

device_impl_files = sorted(name_to_funcs.keys())

with open(os.path.join(gensrc, "device_unity.cpp"), "w") as f:
  print("-- Generating %s" % os.path.join(gensrc, "device_unity.cpp"))
  out = f.write

  out("// ============================================================================\n")
  out("// RCCL Unity Device Build - Auto-generated by generate.py\n")
  out("// ============================================================================\n")
  out("//\n")
  out("// This file includes all RCCL device code in a single translation unit,\n")
  out("// enabling compilation without -fgpu-rdc.\n")
  out("//\n")
  out("// Total collective function files: %d\n" % len(device_impl_files))
  out("// MSCCL kernel files: %d\n" % len(msccl_device_files))
  out("// ============================================================================\n")
  out("\n")

  # Include all generated collective function definition files FIRST.
  # Each file includes common.h (which has an include guard) and its
  # collective header (which now also has an include guard).
  # These define the ncclDevFunc_* functions via DEFINE_ncclDevFunc macro.
  out("// ---- Generated collective function definitions ----\n")
  for impl_file in device_impl_files:
    out('#include "%s"\n' % impl_file)
  out("\n")

  # Include MSCCL kernel files if enabled
  if msccl_device_files:
    out("// ---- MSCCL kernel definitions ----\n")
    for msccl_file in msccl_device_files:
      out('#include "%s"\n' % msccl_file)
    out("\n")

  # Include main device kernel files LAST.
  # common.cu.cpp contains the __global__ ncclDevKernel_* functions which
  # dispatch to the ncclDevFunc_* functions defined above.
  # These paths are relative to the hipify/src/device/ directory; CMake adds
  # this to the include path when unity build is enabled.
  out("// ---- Main device kernel implementations ----\n")
  out("// common.cu.cpp defines __global__ kernels that dispatch to ncclDevFunc_*\n")
  out('#include "common.cu.cpp"\n')
  out('#include "onerank.cu.cpp"\n')
  out("\n")
