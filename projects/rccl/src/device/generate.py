...
# Unrolls generated by a multi-arch build.
default_unrolls = ["1", "2", "4", "32"]
...
# table the host checks before selecting an unroll. 8 and 16 are absent because no build
# generates them by default; --all_unrolls adds them back, unpinned.
...
# --all_unrolls overrides only the unroll set; the arch's pipeline decision stands.
...
def get_arch_guard(fn):
  ...
  elif fn.unroll in unroll_arch_requirement and not build_all_unrolls:
...
with open(os.path.join(gensrc, "host_table.cpp"), "w") as f:
  ...
  for u in all_unrolls:
    # --all_unrolls drops the arch guard in get_arch_guard(), so the pin must drop here
    # too or the host would reject an unroll this build did compile for the running GPU.
    arch = None if build_all_unrolls else unroll_arch_requirement.get(u)
