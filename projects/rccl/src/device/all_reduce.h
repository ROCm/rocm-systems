...
struct RunWorkColl<ncclFuncAllReduce, T, RedOp, NCCL_ALGO_TREE, NCCL_PROTO_SIMPLE> {
  __device__ __forceinline__ void run(int tid, int nthreads, struct ncclDevWorkColl* work, struct ncclShmemData* ncclShmem, void* ncclShmemPerWarp) {
    ...
    //   runTreeUpDown<T, RedOp, ProtoSimple<1, 1>>(tid, nthreads, work, nullptr, 0, primsModeDefault, ncclShmem, ncclShmemPerWarp);
    // #else
    //   runTreeSplit<T, RedOp, ProtoSimple<1, 1>>(tid, nthreads, work, nullptr, 0, primsModeDefault, ncclShmem, ncclShmemPerWarp);
    // #endif
  }
};
