...
typedef enum {
    ARSMI_FABRIC_STATE_UNCONFIGURED = 0,
    ARSMI_FABRIC_STATE_CONFIGURED   = 1,
    ARSMI_FABRIC_STATE_READY        = 2,
    ARSMI_FABRIC_STATE_ACTIVE       = 3,
    ARSMI_FABRIC_STATE_ERROR        = 4,
    ARSMI_FABRIC_STATE_UNKNOWN      = 5
} ARSMI_fabric_state_t;

typedef enum {
    ARSMI_FABRIC_ADDR_MODE_SOURCE_ALIASING       = 0,
    ARSMI_FABRIC_ADDR_MODE_SOURCE_IDENTIFICATION = 1,
    ARSMI_FABRIC_ADDR_MODE_UNKNOWN               = 2
} ARSMI_fabric_addr_mode_t;

struct ARSMI_fabricInfo {
    int                      supported;   /* 1 if UALoE or UALLink and state is ready/active */
    ARSMI_fabric_type_t      fabric_type;
    ARSMI_fabric_state_t     accel_state;
    ARSMI_fabric_addr_mode_t addr_mode;
    uint32_t  accel_id;
