...
NCCL_DEVICE_INLINE void ncclGin_BackendMask<beMask>::increaseSignalShadow(ncclGinSignal_t signal, uint64_t delta) const {
  ...
  // HIP equivalent of upstream's `red.relaxed.cta.add.u64`: relaxed,
  // workgroup-scoped atomic add on the shadow slot. PTX inline asm is not
  // valid on the AMD device path even though NCCL_CHECK_CUDACC is true
  // (HIP defines __CUDACC__).
  __atomic_fetch_add(this->_signalShadows + signal, delta, __ATOMIC_RELAXED);
#else
