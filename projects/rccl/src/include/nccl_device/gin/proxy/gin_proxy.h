...
NCCL_DEVICE_INLINE void postGfd(Coop coop, ncclGinProxyGpuCtx_t* proxyCtx, ncclGinProxyGfd_t* gfd,
  ...
  if (coop.thread_rank() == 0) {
    ...
    idx &= queueSize - 1;
// 16-byte stores into the host-mapped queue.
// Cache-bypassing: __stwt on NVIDIA, system-scope dwordx4 (or NT-store fallback)
// on HIP - avoids a costly __threadfence_system().
#pragma unroll
