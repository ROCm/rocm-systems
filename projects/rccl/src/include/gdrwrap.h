...
// [RCCL v2.30 sync] Two overloads on the HIP path so callers can pass either an `ncclMemType_t`
// (RCCL internal callers, e.g. `init.cc:1036`) or a `uint32_t pinFlags` (call sites that took
// upstream v2.30's new `ncclGdcPinFlag` form, e.g. `transport/net.cc:1453`, `coll_net.cc:597`).
// The pinFlags overload is currently a thin shim that ignores the flag and forwards to memType;
// AMD doesn't have a corresponding GDRCopy pin-flag concept today. SME REVIEW: revisit if AMD
// ever surfaces a flush-vs-no-flush GDR pin distinction.
template <typename T>
static ncclResult_t ncclGdrCudaCalloc(T** ptr, T** devPtr, size_t nelem, void** gdrHandle,
                                      struct ncclMemManager* manager,
                                      uint32_t /*pinFlags*/) {
  return ncclGdrCudaCalloc(ptr, devPtr, nelem, gdrHandle, manager, ncclMemPersist);
}
