...
struct ncclRmaProxyState {
  ...
  // Physical GIN communicator contexts (RCCL GIN path)
  int ginCommCount;
  void* ginComms[NCCL_GIN_MAX_CONNECTIONS];
  // Physical RMA communicator contexts (upstream RMA path)
  int rmaCommCount;
...
// [RCCL] Wrapper around hipStreamBatchMemOp / cuStreamBatchMemOp, defined in
// rma/rma_proxy_launch.cc on AMD and misc/cudawrap.cc on NVIDIA.
