...
static ncclResult_t symMemoryImportAndMapSegmentsForRank(
  ...
  ncclResult_t ret = ncclSuccess;
  struct ncclDevrState* devr = &comm->devrState;
  CUdeviceptr addr = (CUdeviceptr)((char*)devr->lsaFlatBase + r * devr->bigSize + bigOffset);
  for (int segment = 0; segment < numSegments; segment++) {
    ...
    addr = (CUdeviceptr)((char*)addr + msg->segmentSize);
  }
fail:
  return ret;
}
...
static void symMemoryDropRef(
  ...
  if (mem != nullptr && 0 == --mem->refCount) {
    ...
    for (int r = 0; r < devr->lsaSize; r++) {
      CUdeviceptr addr = (CUdeviceptr)((char*)devr->lsaFlatBase + r*devr->bigSize + mem->bigOffset);
      for (int idx = 0; idx < mem->segmentCounts[r]; idx++) {
        ...
        addr = (CUdeviceptr)((char*)addr + tmpBaseSize);
      }
    }
...
ncclResult_t ncclDevrWindowRegisterInGroup(
  ...
  for (int segment = 0; segment < numSegments; segment++) {
    size_t baseSendSize;
    CUCHECK(cuMemGetAddressRange(nullptr, &baseSendSize, (CUdeviceptr)((char*)memAddr + offset)));
    CUCHECKGOTO(cuMemRetainAllocationHandle(&memHandles[segment], (void *) ((char*)memAddr + offset)), ret, fail_locReg);
    offset += baseSendSize;
  }
