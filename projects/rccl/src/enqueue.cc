...
void ncclAddWorkBatchToPlan(
    ...
    int p2pEpoch, int p2pRound, bool batchP2P, bool newBatch
  ) {
  ...
  if (workType == ncclDevWorkTypeP2p) {
    ...
    // [RCCL] If batching is enabled (RCCL_P2P_BATCH_ENABLE=1),
...
static ncclResult_t addP2pToPlan(
  ...
  for (int part=0; part < nChannelsMax; part++) {
    ...
    ncclAddWorkBatchToPlan(comm, plan, channelId, ncclDevWorkTypeP2p, funcIdx, workOffset, p2pEpoch, p2pRound, batchP2P);
    if (funcIdx < 0) {
      WARN("%s: unsupported collective. Please ensure the collective has been enabled in build.", __func__);
      return ncclInvalidUsage;
    }
  ...
  while (comm->planner.nTasksP2p != 0) {
    ...
    for (int round=0; round < nRanks; round++) {
      ...
      if (sendRank == comm->rank && send->buff == recv->buff && rcclEffectiveP2pBatchEnable(comm) == 0) {
        ...
        NCCLCHECK(addP2pToPlan(comm, plan, nChannelsMin, nChannelsMax, *p2pEpoch, round, sendRank, sendBuff, sendBytes, recvRank, recvBuff, recvBytes, send ? send->opCount : 0, recv ? recv->opCount : 0, planTotalTasks, p2pTasks));
        if (send != nullptr) {
          ncclIntruQueueDequeue(&peers[sendRank].sendQueue);
...
rccl_static ncclResult_t ncclGetAlgoInfo(
