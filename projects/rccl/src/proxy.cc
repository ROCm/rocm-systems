...
void* ncclProxyService(void* _args) {
  ...
  while (stop == PROXY_RUNNING || npeers > 0) {
    ...
    if (pollfds[maxProxyConnections].revents) {
      ...
      // [RCCL] PR #6023: keep `retryOnBadMagic=false` so a stray garbage connection doesn't
      // wedge the proxy accept loop; upstream's default-arg `true` retries indefinitely.
