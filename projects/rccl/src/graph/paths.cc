...
// Worst case over the GPUs of the best path each of them has to a NIC of its own. A PXN relay
// counts as one, since it reaches the NIC through the GPU that owns it.
ncclResult_t ncclTopoGetGpuMaxLocalNetPath(struct ncclTopoSystem* system, int* max) {
  ...
  for (int i = 0; i < system->nodes[GPU].count; i++) {
    ...
    // A GPU that reaches no NIC constrains nothing, and would otherwise push the result to PATH_DIS.
