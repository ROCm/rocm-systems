...
ncclResult_t ncclTopoCheckP2p(struct ncclComm* comm, struct ncclTopoSystem* system, int rank1, int rank2,
  ...
  *p2p = 0;
  ...
  // In general, use P2P whenever we can. (RCCL: keep permissive default; CPU-vendor-specific tightenings below.)
...
// NOTE: RCCL pre-merge had a `bool netManaged` parameter (PR #2766) that short-circuited the flush
// when the net plugin reported the buffer as net-managed. That parameter no longer exists upstream;
// the equivalent short-circuit must now happen in the caller (transport/net.cc) before calling this
// function. Until that caller-side fix lands, the AMD/HIP path conservatively flushes always.
// TODO(AICOMRCCL-1123 follow-up): move the `managed` short-circuit into transport/net.cc:546.
ncclResult_t ncclTopoNeedFlush(struct ncclComm* comm, int64_t netId, int netDev, int rank, enum ncclTopoFlushType* flush) {
  *flush = ncclTopoFlushAlways;
  ...
  NCCLCHECK(ncclTopoRankToIndex(system, rank, &g, /*showWarn=*/true));
#if defined(__HIP_PLATFORM_AMD__) || defined(__HIPCC__)
  // [RCCL] PR #2766: the `netManaged` short-circuit is now applied at the caller in
  // transport/net.cc; HIP path conservatively returns ncclTopoFlushAlways (over-flushes;
  // correctness-safe).
  *flush = ncclTopoFlushAlways;
#else
