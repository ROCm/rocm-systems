...
ncclResult_t ncclTopoPostset(struct ncclComm* comm, int* firstRanks, int* treePatterns,
...
#else
  // Skip the 64-channel cap for gfx1250 and for MNNVL transports.
  ...
  // Otherwise retain the 64-channel cap for all other arches.
  {
    int userMax = (int)ncclParamMaxNchannels();
    bool userOptedHigher = (userMax != -2 && userMax > 64);
    if (!userOptedHigher && !comm->MNNVL && !isGfx1250 &&
