...
static ncclResult_t getPciPath(const char* busId, char** path) {
  ...
  // [RCCL] override PCIe device function ID in CPX mode
...
ncclResult_t ncclTopoGetXmlFromSys(struct ncclXmlNode* pciNode, struct ncclXml* xml) {
  ...
  if (index == -1) {
    if (path) {
      ...
      // [RCCL] fall back to device speed when port speed is unavailable (empty sysfs read)
      if (portSpeedStr[0])
  ...
  if (index == -1) {
    if (path) {
      ...
      NCCLCHECKGOTO(ncclTopoGetStrFromSys(path, "../max_link_width", strValue), ret, exit);
      // [RCCL] fall back to device width when port width is unavailable (empty sysfs read)
      int portWidth;
      if (strValue[0])
  ...
  if (parent == NULL) {
    if (path) {
      ...
      // [RCCL] Workaround kernel bug for now: -1 means NUMA-less, treat as node 0
