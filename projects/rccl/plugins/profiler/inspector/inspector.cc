...
inspectorDumpThread::inspectorDumpThread(const char* _outputRoot, int64_t _sampleIntervalUsecs)
  ...
  // Fixes teardown hang where stopThread()'s run=false could sit unobserved
  // for up to sampleIntervalUsecs, deadlocking ncclCommDestroy.
  ...
  if (useMonotonic) {
    ...
    sleepCondClock = CLOCK_MONOTONIC;
  } else {
    pthread_condattr_destroy(&condAttr);
...
inspectorDumpThread::~inspectorDumpThread() {
  ...
  // RCCL: tear down condvar sleep machinery (see ctor).
...
void inspectorDumpThread::stopThread() {
  ...
  run = false;
  inspectorUnlockRWLock(&guard);
  // RCCL: wake the dump thread immediately so teardown does not block for
  // up to sampleIntervalUsecs waiting on the next sleep timeout.
...
inspectorResult_t inspectorDumpThread::inspectorStateDumpProm(const char* output_root) {
  ...
  // RCCL: re-read ncomms under lock to avoid TOCTOU after the dump above.
  inspectorLockRd(&g_state.deletedComms.guard);
  bool hasDeleted = (g_state.deletedComms.ncomms > 0);
  inspectorUnlockRWLock(&g_state.deletedComms.guard);
  if (hasDeleted) {
...
void* inspectorDumpThread::dumpMain(void* arg) {
  ...
  while (dumper->run) {
    ...
    // RCCL: condvar-based sleep replaces std::this_thread::sleep_for so that
    // stopThread() can wake us immediately (fixes teardown hang).
    if (dumper->sampleIntervalUsecs > 0) {
      struct timespec deadline;
      clock_gettime(dumper->sleepCondClock, &deadline);
      uint64_t nsec = (uint64_t)deadline.tv_nsec
                    + (uint64_t)dumper->sampleIntervalUsecs * 1000ULL;
      deadline.tv_sec += nsec / 1000000000ULL;
      deadline.tv_nsec = nsec % 1000000000ULL;
...
static uint64_t calculateMaxKernelExecTimeUsecsP2p(struct inspectorP2pInfo *p2pInfo,
  ...
  // RCCL: cap iteration to MAX_CHANNELS (see above).
  for (uint32_t i = 0; i < nCh; i++) {
    struct inspectorKernelChInfo *kernelCh = &p2pInfo->kernelCh[i];
    if (kernelCh->tsCompletedUsec > kernelCh->tsStartUsec) {
      uint64_t cpuExecTimeUsecs = kernelCh->tsCompletedUsec - kernelCh->tsStartUsec;
