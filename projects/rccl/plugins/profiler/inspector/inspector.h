...
struct inspectorDumpThread {
  ...
  pthread_t pthread;
  pthread_rwlock_t guard;
  // RCCL: condvar-based sleep replaces std::this_thread::sleep_for in dumpMain
  // so that stopThread() can wake the dump thread immediately. Without this,
  // ncclCommDestroy hangs for up to sampleIntervalUsecs at teardown.
