...
class DockerRuntime(ContainerRuntime):
    ...
    def _wait_for_entrypoint(self, timeout=600):
        ...
        Implementation notes:
          * ``docker logs --follow`` is read by a background thread so the
            main thread can enforce a real wall-clock deadline even when
            the container goes silent (a blocking ``readline()`` would
            otherwise hang indefinitely on quiet containers).
          * The container state is also probed periodically; if the
            container has exited before printing Ready, we surface that
        ...
        try:
            while True:
                ...
                if elapsed - last_state_check > 10.0:
                    ...
                    state = self._inspect_container_state()
