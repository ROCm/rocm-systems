.. meta::
   :description: Build and run an RCCL AllReduce program on AMD GPUs — from installation through single-node and multi-node execution with correctness checks.
   :keywords: RCCL, tutorial, AllReduce, ncclCommInitAll, ncclCommInitRank, ncclGroupStart, ncclCommDestroy, HIP, ROCm, getting started, multi-GPU, multi-node, bandwidth, MPI

.. _create-a-rccl-program:

**********************
Create a RCCL program
**********************
...
- At least two AMD Instinct™ GPUs on the same node for the single-process
  ...
  example.
- For multi-process: Message Passing Interface (MPI) is installed (Open MPI or MPICH) and passwordless SSH
...
1. Initialize the ROCm and HIP runtime and choose a GPU for each rank.
...
the correct output at every index is ``numRanks × 1.0f = numRanks`` (where
``numRanks`` is the total number of GPUs). This makes correctness checking
trivial without requiring a reference computation.
...
.. code-block:: cpp
   ...
   // nullptr uses GPU indices 0, 1, 2, ... nranks-1
   NCCL_CALL(ncclCommInitAll(comm.data(), nranks, nullptr));
...
.. code-block:: cpp
   ...
   HIP_CALL(hipMalloc((void **)&recvBuf, N * sizeof(float)));  // written by ncclAllReduce; no init needed
...
warming, and connection establishment between ranks. Timing any of these first
...
.. note::
   ...
   This snippet shows single-process, single-rank usage. When managing multiple
   ranks in one process (the common case in this tutorial), wrap every
   ``ncclAllReduce`` call in ``ncclGroupStart`` and ``ncclGroupEnd`` — otherwise
   the call deadlocks waiting for ranks that haven't submitted yet. See
   `Step 7`_ for the full explanation and pattern.
...
.. note::
   ...
   The ``if (rank == 0)`` guard on the ``printf`` only applies in
   multi-process mode, where each process has its own ``rank`` variable
   set from the command-line argument. In single-process mode, there is no
   single ``rank`` value — the complete example uses ``intraRankStartId == 0``
   instead to print output once per run.
...
you must wrap per-rank collective calls in ``ncclGroupStart`` and
...
.. code-block:: cpp
   ...
   // numIntraRank, intraRankStartId, sendBuf[], recvBuf[], comm[], and
   // stream[] are parameters and locals from ExecuteTest() in the complete
   // example below — they are not global variables.
...
.. warning::
   ...
   Always call ``hipStreamSynchronize`` before ``ncclCommDestroy``. Destroying
   the communicator while GPU work is still in flight produces undefined
   behavior and is a common source of difficult-to-reproduce crashes.
...
.. code-block:: cpp
   ...
   int main(int argc, char **argv)
   {
     if (getenv("NCCL_COMM_ID") && argc == 3)
     {
       ...
       ncclUniqueId commId;
       // When NCCL_COMM_ID is set, RCCL's bootstrap overrides the generated
       // commId with the address encoded in the variable, so every rank can
       // safely call ncclGetUniqueId independently. For MPI-based launches
       // without NCCL_COMM_ID, call ncclGetUniqueId only on rank 0 and use
       // MPI_Bcast to share the result before calling ncclCommInitRank.
...
1. Find a valid address for rank 0. Run the binary with no arguments
...
2. Set ``NCCL_COMM_ID`` and launch all ranks. On a single node with
...
3. Use multi-node with MPI. For a two-node run with 8 GPUs per node
   (16 ranks total):
   ...
   .. code-block:: bash
      ...
      mpirun -np 16 --hostfile hostfile.txt \
        --bind-to numa \
        -x NCCL_COMM_ID \
        bash -c './HelloRccl 16 $OMPI_COMM_WORLD_RANK'
...
You have built a complete RCCL program that covers these key topics:
...
- Runs warmup and timed AllReduce iterations using ``ncclGroupStart`` and
