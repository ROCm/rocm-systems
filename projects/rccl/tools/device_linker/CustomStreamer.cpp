// Copied from LLVM and modified to allow passing in "cpu" as required
// by AMDGCN back end
//===- CustomStreamer.cpp --------------------------------------------------===//
//
...
#define DwarfStreamer CustomStreamer
...
Expected<std::unique_ptr<CustomStreamer>> CustomStreamer::createStreamer(
    const Triple &TheTriple, const char * CPU, const char * FeatureStr,
    DWARFLinkerBase::OutputFileType FileType,
    raw_pwrite_stream &OutFile, DWARFLinkerBase::MessageHandlerTy Warning) {
  std::unique_ptr<CustomStreamer> Streamer =
      std::make_unique<CustomStreamer>(FileType, OutFile, Warning);
  if (Error Err = Streamer->init(TheTriple, CPU, FeatureStr, "__DWARF"))
    return std::move(Err);

  return std::move(Streamer);
}

Error CustomStreamer::init(Triple TheTriple,
                           const char * CPU,
...
void CustomStreamer::finish() { MS->finish(); }

void CustomStreamer::switchToDebugInfoSection(unsigned DwarfVersion) {
...
void CustomStreamer::emitCompileUnitHeader(CompileUnit &Unit,
...
void CustomStreamer::emitAbbrevs(
...
void CustomStreamer::emitDIE(DIE &Die) {
  MS->switchSection(MOFI->getDwarfInfoSection());
...
void CustomStreamer::emitSectionContents(StringRef SecData,
...
MCSection *CustomStreamer::getMCSection(DebugSectionKind SecKind) {
  switch (SecKind) {
...
void CustomStreamer::emitStrings(const NonRelocatableStringpool &Pool) {
...
void CustomStreamer::emitStringOffsets(
...
void CustomStreamer::emitLineStrings(const NonRelocatableStringpool &Pool) {
...
void CustomStreamer::emitDebugNames(DWARF5AccelTable &Table) {
...
void CustomStreamer::emitAppleNamespaces(
...
void CustomStreamer::emitAppleNames(
    AccelTable<AppleAccelTableStaticOffsetData> &Table) {
...
void CustomStreamer::emitAppleObjc(
    AccelTable<AppleAccelTableStaticOffsetData> &Table) {
...
void CustomStreamer::emitAppleTypes(
...
void CustomStreamer::emitSwiftAST(StringRef Buffer) {
...
void CustomStreamer::emitSwiftReflectionSection(
...
void CustomStreamer::emitDwarfDebugArangesTable(
...
void CustomStreamer::emitDwarfDebugRangesTableFragment(
    const CompileUnit &Unit, const AddressRanges &LinkedRanges,
    PatchLocation Patch) {
...
MCSymbol *
CustomStreamer::emitDwarfDebugRangeListHeader(const CompileUnit &Unit) {
...
void CustomStreamer::emitDwarfDebugRangeListFragment(
    const CompileUnit &Unit, const AddressRanges &LinkedRanges,
...
void CustomStreamer::emitDwarfDebugRangeListFooter(const CompileUnit &Unit,
                                                  MCSymbol *EndLabel) {
  if (Unit.getOrigUnit().getVersion() < 5)
    return;
...
void CustomStreamer::emitDwarfDebugRngListsTableFragment(
    const CompileUnit &Unit, const AddressRanges &LinkedRanges,
    PatchLocation Patch, DebugDieValuePool &AddrPool) {
...
MCSymbol *CustomStreamer::emitDwarfDebugLocListHeader(const CompileUnit &Unit) {
  if (Unit.getOrigUnit().getVersion() < 5)
    return nullptr;
...
void CustomStreamer::emitDwarfDebugLocListFragment(
...
void CustomStreamer::emitDwarfDebugLocListFooter(const CompileUnit &Unit,
                                                MCSymbol *EndLabel) {
  if (Unit.getOrigUnit().getVersion() < 5)
    return;
...
void CustomStreamer::emitDwarfDebugLocTableFragment(
    const CompileUnit &Unit,
    const DWARFLocationExpressionsVector &LinkedLocationExpression,
    PatchLocation Patch) {
...
MCSymbol *CustomStreamer::emitDwarfDebugAddrsHeader(const CompileUnit &Unit) {
...
void CustomStreamer::emitDwarfDebugAddrs(const SmallVector<uint64_t> &Addrs,
                                        uint8_t AddrSize) {
...
void CustomStreamer::emitDwarfDebugAddrsFooter(const CompileUnit &Unit,
                                              MCSymbol *EndLabel) {
...
void CustomStreamer::emitDwarfDebugLocListsTableFragment(
    const CompileUnit &Unit,
    const DWARFLocationExpressionsVector &LinkedLocationExpression,
    PatchLocation Patch, DebugDieValuePool &AddrPool) {
...
void CustomStreamer::emitLineTableForUnit(
...
void CustomStreamer::emitLineTablePrologue(const DWARFDebugLine::Prologue &P,
...
void CustomStreamer::emitLineTablePrologueV2IncludeAndFileTable(
...
void CustomStreamer::emitLineTablePrologueV5IncludeAndFileTable(
    const DWARFDebugLine::Prologue &P, OffsetsStringPool &DebugStrPool,
    OffsetsStringPool &DebugLineStrPool) {
...
void CustomStreamer::emitLineTableString(const DWARFDebugLine::Prologue &P,
...
void CustomStreamer::emitLineTableProloguePayload(
    const DWARFDebugLine::Prologue &P, OffsetsStringPool &DebugStrPool,
    OffsetsStringPool &DebugLineStrPool) {
...
void CustomStreamer::emitLineTableRows(
...
void CustomStreamer::emitIntOffset(uint64_t Offset, dwarf::DwarfFormat Format,
...
void CustomStreamer::emitLabelDifference(const MCSymbol *Hi, const MCSymbol *Lo,
...
void CustomStreamer::emitPubSectionForUnit(
...
void CustomStreamer::emitPubNamesForUnit(const CompileUnit &Unit) {
...
void CustomStreamer::emitFDE(uint32_t CIEOffset, uint32_t AddrSize,
...
void CustomStreamer::emitMacroTables(DWARFContext *Context,
...
void CustomStreamer::emitMacroTableImpl(const DWARFDebugMacro *MacroTable,
                                       const Offset2UnitMap &UnitMacroMap,
                                       OffsetsStringPool &StringPool,
                                       uint64_t &OutOffset) {
