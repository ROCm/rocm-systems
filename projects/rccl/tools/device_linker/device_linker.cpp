...
private:
    ...
    // Debug info chunk tracking - only stores what DWARFLinker needs for address remapping
    ...
    bool collectSections() {
        ...
        if (!omit_dwarf_) {
        {
            ...
            // Build debug_info_chunks_ for DWARFLinker address remapping
            // Only stores original->new text address mapping; DWARFLinker does the actual DWARF merging
            {
                auto* disp_debug_info = disp_->find(".debug_info");
                auto* disp_text = disp_->find(".text");

                // Add dispatcher chunk if it has debug info
                ...
                printf("Built %zu debug info chunks for DWARFLinker\n", debug_info_chunks_.size());
            }

            // Note: mergeDebugInfoWithDWARFLinker() is called later from link() after
