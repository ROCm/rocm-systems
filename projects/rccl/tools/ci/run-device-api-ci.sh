...
# After the JSON matrix, runs pytest devtime smoke (GIN -D 3 in-kernel
...
# GIN in-kernel device-timing smoke (wall_clock64 / *TimedKernel).
# Requires ENABLE_DEVICE_API=ON rccl-tests build (perf binaries with -B flags).
# Failures are recorded in FAILED_RUNS; the caller swallows the return status so
# the summary below still prints under `set -e`.
run_devtime_smoke() {
  ...
  local a2a_bin="${PERF_DIR}/alltoall_perf"
  ...
  # device-api.sbatch builds librccl with ENABLE_ROCSHMEM_GIN, so Anvil-SDMA
  # (type 6) is in this build. Default matches gin-tests.json, not the proxy
  # AllToAll smoke above.
  local rs_bin="${PERF_DIR}/reduce_scatter_perf"
