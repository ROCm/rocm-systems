...
class TestExecutor:
    ...
    def run_test(self, test_config, suite_config):
        ...
        #   1. caller environment – LD_LIBRARY_PATH already set in the shell wins
        #                           so users can point at their own librccl.so (or a
        #                           custom libamdhip64.so.7) without any special var:
        #                             LD_LIBRARY_PATH=/path/to/rccl python3 test_runner.py ...
        #   2. test JSON value    – per-test custom lib dir (e.g. a backport HIP stack)
        #   3. build_dir          – the test-built librccl.so (default fallback when
        #                           neither the caller nor the JSON config override it)
        ...
        # merged_env loop below) so its position in the priority order is honored.
        mpi_path  = self.paths.get("mpi_path", "")
        rocm_path = self._rocm_root()
        ...
        ld_library_path_parts.append(self.build_dir)
        ...
        if num_ranks == 1:
            ...
            # LD_LIBRARY_PATH is skipped here and forwarded once below using the
            # fully-merged value so the caller/JSON/build_dir priority order is
            # preserved on remote ranks (avoids emitting a duplicate -x with only
            # the JSON value).
    ...
    def _run_pytest_test(self, test_config, merged_env):
        ...
        # Env: caller's shell LD_LIBRARY_PATH wins (lets users point at their own
        # librccl.so), then a per-test LD_LIBRARY_PATH from the JSON env, then
        # build_dir (default fallback), then rocm/mpi libs (mirrors the gtest/MPI
        # path). RCCL_BUILD defaults to build_dir; remaining config env is applied
        # as-is.
        env = os.environ.copy()
        rocm_path = self._rocm_root()
        mpi_path = self.paths.get("mpi_path", "")
        ...
        ld_parts.append(self.build_dir)
