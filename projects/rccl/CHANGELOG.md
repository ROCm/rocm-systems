...
* Added an opt-in init-parallel / execute-serial mode to the unit-test runner (`--exec-mode=init-pipeline`, default stays `serial`). Test binaries warm their RCCL device code up front, publish a READY token, and block until the runner issues GO, so one entry's initialization overlaps another entry's execution while the measured executions stay strictly serialized (`EXECUTING <= 1`). The runner drives a single run-wide scheduler across all suites — a loader bounded by `--init-pool`, one watcher per in-flight entry, and one executor — with `--release-order`, `--loader-policy`, `--fork-sweep-policy`, `--init-timeout`, `--go-timeout`, and `--phase-timings` controlling release order, perf-run quiescence, legacy per-parent sibling ordering and fail-fast, and timing reports. Fork sweeps can be split into pinned per-configuration sub-entries that roll back up into a parent summary, and `--expand-sweeps` produces the matching per-configuration serial baseline. See `tools/scripts/test_runner/INIT_PIPELINE.md`.
* Added a fail-loud planning pre-pass to the unit-test runner. Before any process is spawned, every test across every enabled suite is resolved into a manifest recording its stable identity, resolved binary and role, warmup profile and provenance, expansion selectors, capture and rendezvous paths, and serial/pipeline/rejected disposition; `--emit-manifest` prints it and the resolved manifest is persisted to `<log_dir>/init_pipeline_manifest.json`. Any rejected entry aborts the whole run before the first spawn. A binary-backed `--gtest_list_tests` preflight (cached per binary identity) resolves each configured `test_filter` to fully qualified test identities, so a filter that matches zero tests is a planning error instead of a process that exits 0 having run nothing. Duplicate stable ids, duplicate capture or rendezvous paths, wildcard filters on pipeline entries, binary/profile mismatches, and suite-supplied overrides of runner-owned pipeline environment variables are all fatal.
* Added automatic execution-serialization and timing validation to the unit-test runner. At the end of an init-pipeline run the combined serial (`[spawn, reap]`) and pipeline (`[go, reap]`) execution intervals are checked in production, not just in unit tests: overlapping intervals, an executed entry without a confirmed process-tree reap, or phase timestamps that are not ordered `launch <= ready <= go <= exit` invalidate the run, and the result is stored in `interval_validation`. Missing events stay absent rather than being zero-filled, and a failed or timed-out entry's rendezvous directory is retained for postmortem. Added `tools/compare_results.py`, which diffs a serial baseline against a pipeline run per configuration and exits non-zero on any regression or dropped coverage.
...
* Refactored the unit-test runner's monolithic `TestExecutor.run_test` into three composable stages — `_build_launch_spec` (pure command and environment assembly), `_spawn`/`_spawn_captured` (non-blocking launch), and `_wait_and_infer` (wait plus result inference) — so a scheduler can own the process handle across a READY/GO boundary. Serial behavior is unchanged.
