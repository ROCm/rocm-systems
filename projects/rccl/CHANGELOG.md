...
* Bidirectional socket bootstrap AllGather now drives Ring1 over a *dedicated* reverse TCP pair when set up via `bootstrapInit`. The reverse-listen address is piggybacked through the existing root rendezvous, so no extra handle exchange is added. Eliminates the per-FD lock contention of the previous shared-pair multiplex and stabilises the bootstrap tail. The shared-pair multiplex remains as the fallback for `bootstrapSplit`/`bootstrapIntraNodeAllGather`, which only have one pair available.
