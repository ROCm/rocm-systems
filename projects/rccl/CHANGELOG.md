...
* Added the `--enable-device-coverage` install flag (`ENABLE_DEVICE_COVERAGE=AUTO|ON|OFF`) for unified host + device LLVM source-based code coverage. It requires `--debug` and ROCm 7.15 or newer; under `AUTO` it probes the selected compiler and its packaged amdgcn profile runtime and falls back to host-only coverage when device instrumentation is unavailable.
