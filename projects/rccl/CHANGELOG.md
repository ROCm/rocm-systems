...
* Retagged the RCCL-only `COLLTRACE` destroy-time log line from `NCCL_INIT` to `NCCL_DESTROY` and documented the `DESTROY` `NCCL_DEBUG_SUBSYS` subsystem. The NCCL 2.30.3 sync added `NCCL_DESTROY` and retagged the shared comm-destroy/plugin-unload log lines, but missed this RCCL-specific line since `COLLTRACE` has no upstream equivalent; it is now excluded from `NCCL_DEBUG=INFO` output by default, consistent with the other destroy-time lines.
