...
* Fixed a crash in the IB-CAST transport's memory registration and deregistration: `regMr`, `regMrDmaBuf` and `deregMr` dereferenced a null communicator instead of rejecting it, and `regMr` accepted a zero size. They now return `ncclInvalidArgument`.
