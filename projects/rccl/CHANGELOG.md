...
* Fixed DDA fabric barrier publication race causing sporadic validation errors by moving the final barrier acquire before the kernel's data publication.
* Gated DDA LL128 eligibility on `RCCL_DDA_LL128` (default off) across all collectives for consistent protocol selection.
