...
* Enabled hierarchical Copy Engine `ncclAllGather` and `ncclAlltoAll` on multi-node communicators: the intra-node phase runs over SDMA/CE across the LSA team and the inter-node phase over one-sided RMA through the CPU (GIN) proxy. Selected when `NCCL_CTA_POLICY=2` with symmetric send and receive windows, `NCCL_LOCAL_REGISTER=0`, `NCCL_CUMEM_ENABLE=1`, `NCCL_NUM_RMA_CTX` greater than zero, and a GIN backend available. Validated for correctness and path selection from 1 to 32 nodes.
...
### Fixed
* Copy Engine collectives are no longer selected when the LSA team does not cover the communicator. The LSA-local CE routines address peers by LSA rank, so on a single node with `NCCL_LSA_TEAM_SIZE` below the local rank count they gathered only `lsaSize` slices and placed them at LSA rather than communicator indices. Such communicators now fall back to the kernel path.
...
* `RCCL_UNROLL_FACTOR` is now rejected when the requested unroll factor's device functions were not compiled for the running architecture, instead of being accepted and then dispatching into an empty device function table. Unroll factors 8, 16 and 32 are compiled for gfx1250 only, but a multi-arch build reported every unroll factor as available, so requesting one of them on another GPU crashed on the device. Those values now fail communicator initialization with a warning naming the architecture, and the default selection falls back to the highest unroll factor actually compiled for the GPU rather than trusting the heuristic's choice. The WarpSpeed auto-tuner's preference for unroll factor 2 is subject to the same check, so it no longer replaces a validated unroll factor with one this build cannot dispatch. gfx1250 is unaffected and still defaults to 32.
