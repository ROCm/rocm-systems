...
* Wired the init-pipeline into `TestExecutor.run_test_suite`: `--exec-mode=init-pipeline` routes a suite through `_run_suite_init_pipeline`, which plans entries, builds a per-sub-entry `LaunchSpec` with the pinned sweep env plus (for pipeline entries) `RCCL_TEST_READY_GO` and a per-entry `RCCL_TEST_RENDEZVOUS_DIR`, drives them through the `PipelineScheduler` via the capture-fd launch primitive, and emits `record_type`-tagged results. The serial path is untouched. Covered by host tests (`tests/test_executor_pipeline.py`), including an end-to-end run through the real scheduler + rendezvous.
