...
### Changed
* Vendored `{fmt}` 10.2.1 as a header-only tree under `external/fmt` so RCCL and its tests no longer FetchContent-download fmt when a system package is missing.
