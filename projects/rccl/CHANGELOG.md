...
* Completed the init-pipeline execution-serialization evidence and gate artifacts (v10 review amendment 1 + §7/§9). Each entry record now carries `entry_kind`, `exec_start_reason` (`go_issued`/`serial_spawned`), and `exec_start`/`exec_end` monotonic stamps, so `EXECUTING ≤ 1` is verified from the **combined** interval set of pipeline (`[go, reap]`) and serial (`[spawn, reap]`) entries via `max_concurrent_execution` — not just pipeline entries. A failed/timed-out/cancelled entry's rendezvous directory is now **retained** (cleanup only on a clean pass/skip) and its path recorded for postmortem. Added the committed 3-entry warm-gate config (`configs/init_pipeline_warmgate.json`) and the exact gate-run commands (`INIT_PIPELINE_WARMGATE_COMMANDS.md`). Covered by host tests (combined-interval non-overlap with a serial control ordered between two pipeline entries).
* Added the init-pipeline whole-run planner pre-pass with guardrails, provenance, and a classification manifest (v10 §7). Before spawning anything, the runner resolves every test across all suites (`resolve_test`): resolved profile, **provenance** (`entry`/`suite`/`fallback`), resolved binary, disposition (`pipeline`/`serial`/`rejected`), effective env disposition, expanded sub-entry count, and an exclusion reason. It prints a planning summary (counting executable sub-entries, not parent descriptors) and enforces two run-wide guards: a **zero-pipeline guard** (`--exec-mode init-pipeline` resolving to no pipeline entries is a hard error unless `--allow-serial-only`) and an **overbroad-default guard** (a pipeline entry from an implicit non-`none` fallback is rejected) — together catching both the all-serial-void and all-pipeline-broken footguns. `--emit-manifest` prints the manifest and exits. Covered by host tests.
* Added the canonical `RCCL_TEST_WARMUP_PROFILE` contract with two-layer validation (v10 §5.1). The runner sets exactly one profile (`fork_coll`/`mpi_coll`) on pipeline entries; serial entries get neither the profile nor rendezvous vars. **Planner layer:** `classify_errors` rejects, before spawn, unknown profiles, `netib_plugin` (until its boundary exists, §5.4), and binary/profile mismatches (`mpi_coll` needs an MPI binary; `fork_coll` needs a fork `rccl-UnitTests` binary) — recorded as `INFRA_ERROR`/`configuration_error`, never launched. **Binary layer:** `MPIEnvironment` and `TestBed` validate the profile against their compiled role before any HIP/RCCL warmup (and before the fork) and, on mismatch, exit a distinct configuration-error code (`RCCL_TEST_CONFIG_ERROR=42`, mapped by the runner to `INFRA_ERROR`, not a test `FAILED`). This is the guard that would have caught the `phase0j` blanket-`mpi_coll` mis-routing. Covered by planner host tests; the direct-binary negative path runs at the hardware gate.
* Made the init-pipeline manifest an authoritative persisted artifact with a stable per-entry identity and duplicate guard (v11 CR-7), and corrected the warm-gate runbook (v11 CR-10). Each resolved entry gets a `stable_id` (suite + label + binary + test_filter); a duplicate stable id is a fatal planning error, and the resolved manifest is written to `<log_dir>/init_pipeline_manifest.json` (not just printed). `INIT_PIPELINE_WARMGATE_COMMANDS.md` now drives the schema-valid config with a positive `--go-timeout`, a list-tests preflight step, the two-suite mixed run, no broad `--exclude`, and a required artifact bundle.
* Added an automatic execution-serialization + timing validator and a log-isolation test (v11 CR-8/CR-9). `validate_execution_intervals` runs in production at the end of each init-pipeline suite (not only in unit tests): it fails the run's validity if any two combined serial/pipeline execution intervals overlap (`max EXECUTING > 1`), if an executed entry lacks a confirmed reap endpoint, or if a pipeline entry's phase timestamps are not ordered `launch ≤ ready ≤ go ≤ exit` (missing events stay absent, never zero-filled). The per-suite result is printed and stored in `interval_validation`. Added a scheduler log-isolation host test proving concurrent entries never leak markers across per-entry capture files.
* Enforced the complete init-pipeline environment contract and a positive GO timeout (v11 CR-3/CR-5). The planner rejects a test/suite that sets any runner-owned pipeline var (`RCCL_TEST_WARMUP_PROFILE`/`RCCL_TEST_READY_GO`/`RCCL_TEST_RENDEZVOUS_DIR`/`RCCL_TEST_GO_TIMEOUT_SEC`) in `env_variables`. The C++ (`MPIEnvironment` and `TestBed`) now validates the whole combination before any warmup/fork and exits `RCCL_TEST_CONFIG_ERROR` (42) on any inconsistency — READY/GO without profile, profile without READY/GO, pipeline profile without a rendezvous dir, wrong profile for the binary role, or an invalid timeout — so the warmup-only path (no rendezvous) is gone. `--go-timeout` must be positive for init-pipeline runs unless `--allow-indefinite-go-wait` is passed. Covered by planner + parser host tests; direct-binary negatives run at the hardware gate.
* Hardened init-pipeline gate validity and made planner rejection globally fatal (v11 CR-1/CR-2). The committed warm-gate config (`configs/init_pipeline_warmgate.json`) is now schema-valid (`test_configurations` + `test_suites{name,config}`, no `_comment`) with an explicit per-test `test_filter`, split across two suites. Pipeline gtest entries with a missing/wildcard (`*`/`ALL`) `test_filter` are rejected — `name` is the runner label, never the gtest filter, and fork expansion preserves the parent filter. Added a binary-backed `--gtest_list_tests` preflight (`lib/gtest_preflight.py`, cached by binary identity, no pipeline env) that resolves each filter to fully-qualified identities and requires exactly one match for the curated gate (zero or many → fatal); this catches the job-216209 "exact-looking label matched zero tests" failure. Any resolved entry with `disposition==rejected` now aborts the whole run before any spawn (`run_guards`), deduped but identity-preserving. Covered by host tests.
* Switched the init-pipeline to one run-wide scheduler across all suites (v11 CR-6). `--exec-mode=init-pipeline` now resolves and expands every enabled/filtered suite into a single ordered plan driven by one `PipelineScheduler` (suite-qualified entry labels/parents), so `EXECUTING ≤ 1` holds across suite boundaries and a later suite's init can overlap an earlier suite's execution — while per-suite reporting, suite-specific binary/ranks/env/timeout/log, and legacy per-parent sibling ordering are preserved. Interval validation runs run-wide. Serial and `--expand-sweeps` runs stay per-suite. Covered by a two-suite host test proving one scheduler with no cross-suite execution overlap.
* Converged MPI device setup with the warmup/release state machine and added injected-failure hooks (v11 CR-4). A new setup-status `MPI_Allreduce` step runs before warmup, and `initialize_devices()` skips its final `MPI_Barrier` in init-pipeline mode so a rank-local device-setup failure can no longer strand healthy ranks at that barrier — every rank (healthy or failed) converges at the setup Allreduce and takes one common path. A rank that never set its device is also caught (not just `retCode`). Test-only injection hooks (`RCCL_TEST_INJECT_SETUP_FAIL_RANK` / `WARMUP_FAIL_RANK` / `READY_FAIL` / `GO_TIMEOUT`) exercise each convergence/failure path at the hardware gate.
* Reworked the MPI init-pipeline rendezvous into one uniform convergence state machine (`MPIEnvironment::warmup_device_code`), fixing a verified hang: previously a rank-0 `PublishReady` failure (or runner death during `WaitForGo`) left ranks 1..N blocked forever at `MPI_Bcast`. Now every rank runs the same steps on success and failure — local warmup → `MPI_Allreduce(MIN)` warmup status → rank 0 publishes READY only if all warmed → broadcast the READY-publish status → on any failure all ranks take the same cleanup path → rank 0 polls GO with a bounded, runner-derived deadline and broadcasts a release status (`GO`/`CANCEL`/`GO_TIMEOUT`/`LIVENESS_LOST`) → all ranks return from `SetUp` together or clean up together. `Rendezvous::WaitForGo` now returns a `ReleaseStatus` with a bounded timeout (no more `_exit`), honoring an optional `CANCEL` token and the liveness pipe; the fork path handles the status too. The runner passes the deadline via `--go-timeout` (`RCCL_TEST_GO_TIMEOUT_SEC`, 0 = indefinite) and sets the canonical `RCCL_TEST_WARMUP_PROFILE` on pipeline entries.
...
* Added the same opt-in warmup to the fork-collective unit-test harness: `TestBed::InitComms` sends a new `CHILD_WARMUP` command to each already-forked execution child (never the fork parent), which warms every unique device it owns. Also added the `UT_RANKS_PER_GPU` exact selector (`EnvVars`, honored by `TestBed::RunSimpleSweep`) that pins the ranks-per-GPU sweep dimension to a single value (0 = unset = original `1..UT_MAX_RANKS_PER_GPU` sweep, byte-identical), plus a defense-in-depth single-generation assertion under `RCCL_TEST_READY_GO`. All no-ops unless enabled.
* Added a file-based READY/GO rendezvous (`test/common/Rendezvous.{hpp,cpp}`) for the init-pipeline: after device-code warmup the binary atomically publishes a `ready` token and blocks until the runner writes `go` (indefinitely — the runner owns all phase timeouts; it aborts early only on runner death via an optional inherited liveness fd). Wired into `MPIEnvironment::SetUp` (rank 0 publishes/waits then `MPI_Bcast`es GO) and `TestBed::InitComms` (the parent publishes/waits after all children are warm). No-op unless `RCCL_TEST_RENDEZVOUS_DIR` is set.
* Added the runner-side two-entry overlap orchestration (`tools/scripts/test_runner/lib/pipeline.py`): a Python `Rendezvous` mirroring the C++ token protocol and `run_overlap_batch`, which launches entries concurrently (device-code init overlaps), waits for each to reach READY, then releases them one `go` at a time and waits for full exit before the next — so no two tested executions overlap. Records per-entry runner-observed phase timings. Covered by host tests (`tests/test_pipeline.py`) proving concurrent init, serial non-overlapping execution, exit-before-READY handling, and log isolation. Minimal simple-stream orchestration (the full mixed-mode scheduler lands later).
* Added the mixed-mode `PipelineScheduler` (`lib/pipeline.py`): a loader thread (launches pipeline entries under a bounded init-pool semaphore; serial entries go straight to the executor), one watcher per in-flight entry, and a single executor thread, enforcing `INITIALIZING+READY ≤ init_pool` and `EXECUTING ≤ 1`. Idempotent terminal transitions with a claim/settle split so run completion is only signalled after teardown, an atomic READY→EXECUTING handoff, a per-unit executor exception boundary (`INFRA_ERROR` distinct from a test failure), and a `shutdown()`/cancellation path where every entry reaches exactly one terminal state. Covered by host tests (`tests/test_scheduler.py`) that verify the pool/execution bounds from runner-observed stamps plus launch-fault and cancellation races.
* Added the pure sweep enumerator + fork eligibility model (`lib/sweep.py`): `enumerate_sweep` is a side-effect-free mirror of the C++ `RunSimpleSweep` outer loops (same coverage and nested-loop order), so one enumerator drives both `--dry-run`/manifest output and real sub-entry expansion and the Python/C++ sweep shapes cannot drift. `expand_fork_entry` pins each sweep point to one child generation (`UT_MIN_GPUS`/`UT_MAX_GPUS`/`UT_POW2_GPUS`, `UT_PROCESS_MASK`, `UT_RANKS_PER_GPU`), and bad selectors are rejected exactly as the C++ harness rejects them. Covered by host tests (`tests/test_sweep.py`).
* Added init-pipeline CLI flags (`--exec-mode`, `--init-pool`, `--loader-policy`, `--init-timeout`, `--release-order`, `--fork-sweep-policy`, `--phase-timings`, `--queue-wait-warn`, `--emit-manifest`) with validation, and a per-test/suite `warmup_profile` + `fork_expand` config schema. Added entry planning + result roll-up (`lib/pipeline_runner.py`): `plan_entries` expands each test into scheduler units (serial / single pipeline entry / per-generation fork sub-entries) and `assemble_records` folds sub-entry results into a `parent_summary` with `record_type`/`counts_toward_topline` so totals never double-count a split sweep. Default execution stays serial. Covered by host tests (`tests/test_pipeline_runner.py`).
* Added `--loader-policy=quiescent_exec` to the init-pipeline scheduler: while a `perf_sensitive` entry (config flag) executes, the loader pauses new device warmups so the measured numbers are not distorted by concurrent init (best effort — already-in-flight warmups still complete); the default `continuous` keeps refilling. Covered by host tests (quiescent defers a later launch until the perf entry finishes; continuous does not), including a fix for the acquire/pause race.
* Added cross-entry `--release-order=config` to the init-pipeline scheduler: entries execute strictly in config (seq) order across the whole suite (deterministic, with intentional head-of-line blocking on the next seq), composing with legacy fork fail-fast; the default `ready` executes entries as they reach READY. Both keep executions non-overlapping. Covered by host tests (config order holds despite out-of-order READY; ready-order contrast; single-executor invariant).
* Added `--expand-sweeps` for serial mode: `--exec-mode=serial --expand-sweeps` splits fork sweeps into the same pinned per-configuration sub-entries the init-pipeline uses (run serially through the normal path — no warmup, rendezvous, or overlap) and emits per-config records with a `parent_summary`. This produces the true per-configuration serial baseline the plan §12 correctness gate diffs an init-pipeline run against (plain serial emits one monolithic row per sweep, which only supports a parent-level diff). Covered by a host test.
* Added a per-configuration results-diff tool for the init-pipeline correctness gate (`lib/results_diff.py` + `tools/compare_results.py`): it indexes each run's top-line records by (suite, test) and reports regressions / fixes / dropped-coverage / new configs between a serial baseline and an init-pipeline candidate, with glob excludes for known pre-existing failures. The CLI consumes the `tests.jsonl` emitted under `--emit-results` and exits non-zero on any regression or dropped coverage — the plan §12 "identical per-config pass/fail/skip" gate. Covered by host tests.
* Added `--rerun-failed` support to init-pipeline mode: after the main pass, the failed/cancelled sub-entries are re-run (with `rerun_env_variables` merged) as a fresh legacy-ordered batch, prior-passed siblings keep their results, and the parent roll-up is recomputed — so a split sweep converges to the same coverage a clean serial rerun would. The pipeline execution core was factored into `_execute_pipeline_plan` (shared by the main and rerun passes). Covered by an end-to-end host test (a fork sweep whose first sub-entry fails, then passes on rerun, rolls up to PASSED).
* Added init-pipeline phase-timing reporting: per-entry init / queue-wait / execution / total durations are collected unconditionally from runner-observed monotonic stamps; `--phase-timings` prints a per-suite count/min/median/p95/max/sum aggregate (splitting warmup from post-GO time to refine the ledger), and `--queue-wait-warn SECS` flags any entry parked longer than the threshold between READY and GO. Covered by host tests (`aggregate_phase_timings`).
* Added legacy per-parent sub-entry ordering + fail-fast to the init-pipeline scheduler (`--fork-sweep-policy=legacy`, the default): a split fork sweep's sub-entries execute in their original nested-loop order even when a later sibling reaches READY first, and the first sub-entry failure cancels that parent's remaining siblings (matching the serial `isCorrect` short-circuit), so `legacy` is serial-equivalent per configuration. `independent` keeps the max-overlap READY-order behavior with no fail-fast. Covered by host tests (out-of-order READY still executes in order; exec- and init-phase failures fail-fast; `independent` runs all). Cross-entry `--release-order` remains a simple stream for now.
* Wired the init-pipeline into `TestExecutor.run_test_suite`: `--exec-mode=init-pipeline` routes a suite through `_run_suite_init_pipeline`, which plans entries, builds a per-sub-entry `LaunchSpec` with the pinned sweep env plus (for pipeline entries) `RCCL_TEST_READY_GO` and a per-entry **absolute** `RCCL_TEST_RENDEZVOUS_DIR` (the binary and MPI ranks run with a different cwd than the runner, so a relative path would put the READY token where the runner never looks — a deadlock), drives them through the `PipelineScheduler` via the capture-fd launch primitive, and emits `record_type`-tagged results. `--init-timeout` now defaults to a finite 600 s so a stuck/mis-configured READY handshake fails that one entry (bounded, no orphans) instead of hanging the whole run. The serial path is untouched. Covered by host tests (`tests/test_executor_pipeline.py`), including an end-to-end run through the real scheduler + rendezvous.
...
* Refactored the unit-test runner's monolithic `TestExecutor.run_test` into three composable stages — `_build_launch_spec` (pure command/env assembly), `_spawn`/`_spawn_captured` (non-blocking launch), and `_wait_and_infer` (wait + result inference) — so a scheduler can own the process handle across a READY/GO boundary. Serial behavior is byte-identical; added host-side launch-API regression tests and `test/common/ForkSafetyInvariant.md` documenting the no-HIP-before-fork invariant. Groundwork for the opt-in init-pipeline execution mode (default stays serial).
