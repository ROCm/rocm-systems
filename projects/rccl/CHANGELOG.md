...
* Added `RCCL_DDA_NRANKS_RELAX` (default `0`, off). When set to `1`, the low-latency DDA IPC AllReduce, AllGather, ReduceScatter and AllToAll paths accept any single-node communicator of 2 to 8 ranks on gfx942/gfx950, instead of only the full 8-rank clique. Largest gains are at odd and non-power-of-two rank counts, where the ring is least efficient; 8 ranks is unchanged. The variable is read per process and must be set identically on every rank of a communicator. With it enabled below 8 ranks, AllReduce and ReduceScatter results are not bit-identical to the ring path because the DDA kernels reduce in strict rank order, and every admitted communicator allocates the full DDA IPC scratch buffer.
