...
* Added an opt-in gfx1151 full-mesh AllReduce fast path, gated by `RCCL_DIRECT_A2A_ENABLE` (default off). Two ranks use one-shot through 4 MiB. At 3–4 ranks, `RCCL_DIRECT_A2A_ONESHOT_THRESHOLD` (default 64 KiB) and `RCCL_DIRECT_A2A_MAX_BYTES` (default 16 MiB, cap 32 MiB) tune message size. The data path is hand-validated on gfx1151; host eligibility tests run in CI.
