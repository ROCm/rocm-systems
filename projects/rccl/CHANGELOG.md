...
* Added MSCCL support for AllGather multinode on gfx942 and gfx950 (for example, on 16, 32, and 64 GPUs). To enable this feature, set the environment variable `RCCL_MSCCL_FORCE_ENABLE=1`. The max message size for MSCCL AllGather usage is `12292 * sizeof(datatype) * nGPUs`.
