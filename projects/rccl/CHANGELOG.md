...
* Fixed crashes and registration failures when running against the ROCm 7.0.x HIP stack, whose partial cuMem VMM backport expects a pointer to the file descriptor in `cuMemImportFromShareableHandle` and writes both `cuMemGetAddressRange` outputs unconditionally. Behavior on HIP 7.1 and newer is unchanged.
