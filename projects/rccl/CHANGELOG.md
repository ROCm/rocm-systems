...
* Fixed a crash in the IB-CAST transport's memory registration and data path: `regMr`, `regMrDmaBuf`, `deregMr`, `isend`, `irecv` and `iflush` dereferenced a null communicator instead of rejecting it, and `regMr` accepted a zero size. They now return `ncclInvalidArgument`.
