...
### Optimized
* Removed the standalone done-event record that closed every DDA, GIN SDMA and hierarchical AllGather collective. The event that orders a following collective on another stream now rides the collective's last kernel as its stop event. The removed record cost about 3.9 microseconds of queue time per call on gfx942, gfx950 and gfx1250, and the measured net saving is about 3.6 microseconds per call. Small-message AllReduce on the DDA path benefits most, since a whole collective there takes about 6 microseconds. A collective captured into a HIP graph keeps the separate record, because a stop event fused into a captured launch is not bound.
