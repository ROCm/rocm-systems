...
* Fixed crashes and registration failures when running against the ROCm 7.0.x HIP stack, whose partial cuMem VMM backport expects a pointer to the file descriptor in `cuMemImportFromShareableHandle`, writes both `cuMemGetAddressRange` outputs unconditionally, and does not implement `CU_POINTER_ATTRIBUTE_IS_LEGACY_CUDA_IPC_CAPABLE`. Behavior on HIP 7.1 and newer is unchanged.
