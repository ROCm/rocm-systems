...
* Fixed Anvil SDMA GIN small-message AllToAll latency after `ncclGinFenceLevel` support: a sub-threshold `gin.put`, a `gin.putValue`, or a standalone `gin.signal` no longer `quiet()`s an idle peer queue, whether it carries a `SignalInc` or a completion counter. The expensive fence runs only when the call itself issued SDMA or the peer/channel dirty bit is set. Clean-queue IPC `SignalInc` skips a duplicate `__threadfence_system` in `fenceBeforeSignal`; `signalPeer` emits one system-scope release immediately before the remote atomic so payload stores stay ordered. `Flush` broadcasts a dirty snapshot on typed coops; `ncclCoopAny` with size > 1 quiets the snapshot on rank 0. `epoch0` stays rank-local. `fetch_and` is skipped if a concurrent `markSdmaDirty` bumped `sdmaEpoch`.
