...
### Fixed
* Fixed AllToAll ignoring `NCCL_CTA_POLICY_ZERO` on gfx942 when the DDA IPC path was eligible. `ncclAlltoAll_impl` dispatched DDA and returned before `ncclEnqueueCheck`, where Copy-Engine (CE) selection happens, so a CE-eligible AllToAll took DDA instead of CE. The DDA fast path now yields when `ncclCeAlltoAllEligible()` reports CE will service the call, mirroring the existing CE-vs-DDA guard for AllReduce. Latent since the DDA IPC init was made reliable on 8×gfx942.
