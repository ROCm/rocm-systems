...
* Fixed the DDA, Copy-Engine 2-shot and GIN SDMA backends not making the communicator's device current and not ordering a new stream against the previous collective. A collective issued while another device was current failed with `ncclUnhandledCudaError`, and consecutive collectives on different streams could overlap.
