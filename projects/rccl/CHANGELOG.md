...
* Fixed Anvil SDMA GIN small-message AllToAll latency after `ncclGinFenceLevel` support: `gin.put` with `SignalInc` below the SDMA threshold no longer `quiet()`s an idle peer queue. The expensive fence runs only when this put issued SDMA or the peer/channel dirty bit is set.
