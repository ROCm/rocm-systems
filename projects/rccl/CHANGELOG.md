...
### Changed
* Vendored `{fmt}` 10.2.1 as a header-only tree under `external/fmt`. RCCL and its tests now always build against this copy instead of looking for a system `fmt` package or downloading one through `FetchContent`.
