...
function display_help()
{
    ...
    echo "    -u|--unity                 Per-collective unity build (-fno-gpu-rdc), fast link"
...
getopt -T
if [[ "$?" -eq 4 ]]; then
    GETOPT_PARSE=$(getopt --name "${0}" --options cdfhij:lprtqu --longoptions address-sanitizer,dependencies,debug,debug-fast,dump-asm,enable-code-coverage,enable_backtrace,disable-colltrace,disable-msccl-kernel,enable-mscclpp,enable-mpi-tests,fast,help,install,jobs:,kernel-resource-use,local_gpu_only,unity,amdgpu_targets:,no_clean,npkit-enable,log-trace,openmp-test-enable,roctx-enable,package_build,prefix:,rm-legacy-include-dir,run_tests_all,run_tests_quick,static,tests_build,time-trace,force-reduce-pipeline,generate-sym-kernels,quiet-warnings,disable-warp-speed,verbose,rocshmem,cmake-options: -- "$@")
else
...
while true; do
    ...
    -u | --unity)                    unity_device_build=true;                                                                          shift ;;
...
# Unity device build (per-collective -fno-gpu-rdc)
if [[ "${unity_device_build}" == true ]]; then
    cmake_common_options="${cmake_common_options} -DENABLE_UNITY_DEVICE_BUILD=ON"
fi
