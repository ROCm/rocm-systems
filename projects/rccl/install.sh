...
function display_help()
{
    ...
    echo "       --disable-uncached-memory  Disable HIP_UNCACHED_MEMORY compile-time flag (default: enabled; runtime: RCCL_UNCACHED_MEMORY=0)"
...
while true; do
    ...
    -d | --dependencies)             install_dependencies=true;                                                                        shift ;;
         ...
         --disable-uncached-memory) disable_uncached_memory=true;                                                                        shift ;;
...
# Disable HIP_UNCACHED_MEMORY compile-time flag
if [[ "${disable_uncached_memory}" == true ]]; then
    cmake_common_options="${cmake_common_options} -DDISABLE_UNCACHED_MEMORY=ON"
fi
