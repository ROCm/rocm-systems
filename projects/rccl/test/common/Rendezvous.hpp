...
{
  /// Parse RCCL_TEST_GO_TIMEOUT_SEC; false on garbage or negative. atof("abc") is 0.0, which would read back as "0 = wait indefinitely".
  inline bool ParseGoTimeoutSec(const char* s, double& out)
  {
    if (!s || !s[0])
    {
      return false;
  ...
  constexpr int RCCL_TEST_CONFIG_ERROR = 42;
   ...
   * @note WaitForGo() blocks until `timeout_sec` (0 = INDEFINITELY) -- the runner
   *       owns every phase timeout, so there is no C++ init-scale timeout that
   *       could collide with the runner's queue-wait. RCCL_TEST_LIVENESS_FD is
   *       read but no runner sets it yet, so process-group teardown from the
   *       runner remains the only orphan-avoidance mechanism.
   */
  class Rendezvous
  {
  public:
