...
//
// On the GIN proxy backend, ginQueueDepth sizes the per-peer GFD queue, which
// drives the proxy's automatic credit/flow-control (the device waits for the
// host consumer to advance the CI before claiming a slot). The other advanced
// queue-control knobs in the 2.29.7 notes -- manual credit management
// (ncclGinOptFlagsMaySkipCreditCheck) and request aggregation
// (ncclGinOptFlagsAggregateRequests) -- are GDAKI-only device-API opt flags;
// the proxy backend accepts them for API parity but does not act on them, so
// they are intentionally out of scope for this proxy-path test.
