...
TEST_F(GinMultiSegmentMPITest, RegisterAsymmetricSegmentCountRejected)
{
    if (!SetUpFixture(2, 2)) return;
...
TEST_F(GinMultiSegmentMPITest, IPutOutOfRangeRejectedNoCorruption)
{
    if (!SetUpFixture(2, 2)) return;
...
TEST_F(GinMultiSegmentMPITest, IGetOutOfRangeRejected)
{
    if (!SetUpFixture(2, 2)) return;
    ...
    if (worldRank_ == 0)
    {
        void* req = nullptr;
        EXPECT_EQ(ncclInvalidArgument,
                  gin_->iget(ginCtx_, 0, /*remoteOff=*/0, mh, total + 1, 0, mh, 1, &req))
            << "oversized iget must be rejected";
        EXPECT_EQ(ncclInvalidArgument,
                  gin_->iget(ginCtx_, 0, /*remoteOff=*/1, mh, total, 0, mh, 1, &req))
            << "remote offset overrun must be rejected";
        EXPECT_EQ(ncclInvalidArgument,
                  gin_->iget(ginCtx_, 0, 0, mh, total, /*localOff=*/1, mh, 1, &req))
...
TEST_F(GinMultiSegmentMPITest, IPutSignalOutOfRangeRejectedNoCorruption)
{
    if (!SetUpFixture(2, 2)) return;
    ...
    if (worldRank_ == 0)
    {
        ...
        EXPECT_EQ(ncclInvalidArgument,
                  gin_->iputSignal(ginCtx_, 0, 0, sendMh, total + 1, 0, recvMh, 1,
        ...
        EXPECT_EQ(ncclInvalidArgument,
                  gin_->iputSignal(ginCtx_, 0, 0, sendMh, total, 0, recvMh, 1,
...
TEST_F(GinMultiSegmentMPITest, BoundaryStressNoCorruption)
{
    if (!SetUpFixture(2, 2)) return;
    ...
    for (int k = 1; k < nSeg; ++k)
    {
        ...
        if (worldRank_ == 0)
        {
            void* req = nullptr;
            ASSERT_EQ(ncclSuccess,
                      gin_->iput(ginCtx_, 0, off, sendMh, len, off, recvMh, 1, &req))
    ...
    if (worldRank_ == 0)
    {
        void* req = nullptr;
        ASSERT_EQ(ncclSuccess,
                  gin_->iput(ginCtx_, 0, 0, sendMh, total, 0, recvMh, 1, &req));
        ASSERT_TRUE(PollUntilDone(req));
    }
    Barrier();
