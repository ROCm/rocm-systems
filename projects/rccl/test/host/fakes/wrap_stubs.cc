...
// Fail-loud stub floor for src/rccl_wrap.cc's host-only microtest
// (rccl-UnitTestsMicroWrap). #include-ing the whole 1771-line rccl_wrap.cc
// (via WRAP_CC_PATH) pulls in the DDA / CE / symmetric-kernel / hierarchical
// backend-selection machinery even though the current test batch only
// exercises a handful of low-dependency helpers at the top of the file (see
// wrap-test.cc). Everything below satisfies the link closure; entries the
...
// / transport_stubs.cc here: those are curated for OTHER units-under-test
// (init.cc, p2p.cc) and each already stubs commSetUnrollFactor and
// rcclCommSetP2pShiftSize as abort() floors for init.cc's benefit -- both are
// REAL functions defined by rccl_wrap.cc itself, so linking those files
// together with this one would be a duplicate-symbol error.
//
...
// on that text via gtest's stderr capture. Self-contained here rather than
// linking nccl_fakes.cc, which would pull in the proxy/shm/topology fake
// surface this lean binary doesn't otherwise need.
// ---------------------------------------------------------------------------
int              ncclDebugLevel  = 0;
uint64_t         ncclDebugMask   = ~0ULL;
...
// ---------------------------------------------------------------------------
int64_t ncclParamMinNchannels() { return 1; }
int64_t ncclParamMaxNchannels() { return MAXCHANNELS; }
int64_t rcclParamForceCe() { return 0; }
int64_t ncclParamLaunchOrderImplicit() { return 0; }
...
// rcclOverrideAlgorithm / rcclUseAllGatherDirect are later rungs); default to
// "unset" so RCCL_OVERRIDE_PROTO-style static-once guards stay on their
// no-override arm until a future rung adds a controllable seam here.
...
// later rungs (rcclGetAlgoInfo reads it via NCCLCHECK(ncclCommCount(...))).
...
// faithful copies of collectives.cc's trivial switch tables (verified against
// src/collectives.cc at the time this was written), NOT abort-floors --
// rcclGetAlgoName/rcclGetProtocolName (Tier-1, tested below) delegate to the
// first two for native algo/protocol values, and rcclOverrideProtocol/
// rcclOverrideAlgorithm (later rung) use all four in WARN messages. Real
...
// file header comment). If collectives.cc's tables are ever extended, this
// copy drifts silently -- there is no compile-time link between the two; a
// mismatch would only surface as a test asserting the wrong string.
// ---------------------------------------------------------------------------
...
// rcclSymKGetInfo -- none exercised by the current test batch (a later rung
// per MICROTEST_README's incremental-unit convention). Kept as abort() so a
// test that accidentally reaches one of these fails immediately at the exact
// call site instead of silently taking whatever default a stub returned.
// ---------------------------------------------------------------------------
...
// included), so no stub is needed here -- an earlier build attempt hit a
// redefinition error from stubbing it anyway.
...
// just a comm/task struct -- a later rung's seam, not an abort-floor gap).
