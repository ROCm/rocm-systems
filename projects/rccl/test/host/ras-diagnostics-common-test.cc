...
// Host-only microtests for src/ras/diagnostics_checks_common.cc. The file's
// communicator registry is owned by ras.cc, which is compiled into this same
// binary by ras-test.cc. Allocation is redirected only in this TU so failure
// paths can be exercised without changing the shared process-wide libc seams.
...
TEST_F(RasDiagnosticsCommonMicrotest, CommSnapshotCopiesEveryDiagnosticField) {
  OwnedComm owned(0x11, 0x22, 0x33, 5);
  ...
  EXPECT_EQ(0x11u, snapshot.rank.commId.commHash);
  EXPECT_EQ(0x22u, snapshot.rank.commId.hostHash);
  EXPECT_EQ(0x33u, snapshot.rank.commId.pidHash);
...
TEST_F(RasDiagnosticsCommonMicrotest, CommIdCompareOrdersEveryIdentityComponent) {
  const rasCommId base{10, 20, 30};
  const std::vector<rasCommId> less = {{9, 999, 999}, {10, 19, 999}, {10, 20, 29}};
  const std::vector<rasCommId> greater = {{11, 0, 0}, {10, 21, 0}, {10, 20, 31}};

  EXPECT_EQ(0, rasDiagnosticsCommIdCompare(&base, &base));
  for (const rasCommId& value : less) {
    EXPECT_LT(rasDiagnosticsCommIdCompare(&value, &base), 0);
    EXPECT_GT(rasDiagnosticsCommIdCompare(&base, &value), 0);
  }
  for (const rasCommId& value : greater) {
    EXPECT_GT(rasDiagnosticsCommIdCompare(&value, &base), 0);
    EXPECT_LT(rasDiagnosticsCommIdCompare(&base, &value), 0);
  }
}
...
TEST_F(RasDiagnosticsCommonMicrotest, CollectFiltersCommsAndBuildsAlignedRecordsAfterUnlockingRegistry) {
  ...
  // Mutation residual: replacing the snapshot loop's `&&` with `||` is
  // equivalent while ncclCommsMutex is held. The first pass counts every
  // matching entry with the same filters, so reaching nRecords proves every
  // later slot is nonmatching. The trailing null still exercises the real
  // loop's recordIdx < nRecords == false short-circuit arm.
  // Removing the registry lock is another non-injectable residual: the only
  // callback seam intentionally runs after the registry scan has unlocked.
...
TEST_F(RasDiagnosticsCommonMicrotest, ReportIncompleteIncludesCountsAndFullCommIdentity) {
  ReporterState state;
  const rasDiagnosticsReporter reporter{CaptureReport, nullptr, &state};
  const rasDiagnosticsRankHeader rank{{0x11, 0x22, 0x33}, 4, 16};

  EXPECT_EQ(ncclSuccess, rasDiagnosticsReportIncomplete(&reporter, "GPU model", &rank, 12));
  ASSERT_EQ(1u, state.lines.size());
  EXPECT_EQ("[INFO] GPU model: diagnostics incomplete, gathered 12/16 ranks in comm 0x11/0x22/0x33 "
