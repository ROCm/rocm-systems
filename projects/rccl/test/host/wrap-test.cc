...
// RCCL_GIN_SDMA is the newest member of rcclAddonAlgos_t and the last one
// before RCCL_ALGO_COUNT, so it is also the boundary case for the bounds
// guard the two tests above probe from the outside: if its `case` label were
// dropped, this value would fall to the `default:` arm and return
// ncclInvalidArgument rather than a name. Nothing else in this file names it.
TEST(WrapMicrotest, GetAlgoName_GinSdmaHasItsOwnName) {
  const char* name = nullptr;
  ASSERT_EQ(ncclSuccess, rcclGetAlgoName(rcclAddonAlgos_t::RCCL_GIN_SDMA, &name));
  EXPECT_STREQ("GIN-SDMA", name);
}
