...
constexpr int kUnknownTaskKind = 0x5F;
...
// Chosen so the raw send pointer looks 16B-aligned against it while the window-relative offset does not.
constexpr uintptr_t kRawAlignedRecvAddr = 0x50024ULL;
...
struct TaskPreTuning_TuningInfoQueue : ncclTaskTuningInfoQueue {
  TaskPreTuning_TuningInfoQueue() { ncclIntruQueueConstruct(&queue); }
};
...
TEST_F(TaskPreTuningMicrotest,
       FillCollTuningInput_UnscaledCollRegistrationShorterThanTheQueriedSize_ClearsRegBuff) {
...
// Built at two ranks, so a literal 4 in place of comm->nRanks oversizes one lookup and clears regBuff.
TEST_F(TaskPreTuningMicrotest, FillCollTuningInput_RegistrationsSizedFromTheCommsRankCount_SetsRegBuff) {
  struct Case {
    ncclFunc_t func;
    size_t sendBytes;
    size_t recvBytes;
  };
  const std::vector<Case> cases = {{ncclFuncAllGather, kCount, kTwoRanks * kCount},
                                   {ncclFuncReduceScatter, kTwoRanks * kCount * kFloatSize, kCollSendBytes}};
  for (const Case& c : cases) {
    TaskPrepScene scene(kTwoRanks, 0);
    struct ncclRawTask* raw = scene.NewColl(c.func);
    RegisteredRanges registered(&scene, {{raw->coll.sendbuff, c.sendBytes},
                                         {raw->coll.recvbuff, c.recvBytes}});
    TaskPreTuning_LocallyValidRegistrations isValid;
    ncclTuningInput_t in = TaskPrep_Poisoned<ncclTuningInput_t>();

    ASSERT_EQ(ncclSuccess, fillCollTuningInput(scene.comm(), &raw->coll, &in));

    EXPECT_EQ(1, in.regBuff) << "func = " << c.func;
  }
}
...
TEST_F(TaskPreTuningMicrotest, FillSendRecvTuningInput_UnregisteredBuffer_CopiesTheRawFieldsAndClearsRegBuff) {
  ...
  // A lowered AlltoAll carries func = Send alongside collAPI = AlltoAll, so the two must not be confused.
  ...
  EXPECT_EQ(ncclFuncAlltoAll, raw->sendRecv.collAPI) << "the raw task must not be rewritten";
}
...
TEST_F(TaskPreTuningMicrotest, FillSendRecvTuningInput_RegistrationCoversTheBuffer_SetsRegBuff) {
  ...
  EXPECT_EQ(1, in.regBuff);
  ASSERT_EQ(1u, isValid.probed().size());
  EXPECT_NE(nullptr, isValid.probed().front());
}
...
TEST_F(TaskPreTuningMicrotest, FillRmaTuningInput_SignalAndWaitSignal_CarryNoPayload) {
  for (ncclFunc_t func : {ncclFuncSignal, ncclFuncWaitSignal}) {
    ...
    // These four are written before the func ladder, so moving them inside the PutSignal arm must turn red.
...
TEST_F(TaskPreTuningMicrotest, PreTuningMergeBcastQueue_EmptyQueue_LeavesTheTuningQueueEmpty) {
  TaskPreTuning_TuningInfoQueue tiq;
...
TEST_F(TaskPreTuningMicrotest, TaskPreTuning_UnknownRawTaskKind_ReturnsInternalError) {
  struct ncclRawTask* raw = scene_.NewColl(ncclFuncAllReduce);
  raw->kind = static_cast<ncclTaskKind>(kUnknownTaskKind);
...
struct ncclRawTaskColl* TaskPreTuning_NewBcast(TaskPrepScene* scene, int root, size_t count,
  ...
  raw->coll.count = count;
  raw->coll.stream = kStream;
  raw->coll.sendbuff = TaskPrep_Addr(kBcastSendAddr + root);
  raw->coll.recvbuff = TaskPrep_Addr(kBcastRecvAddr + root);
  return &raw->coll;
}
...
TEST_F(TaskPreTuningMicrotest, UnreachableMerge_AddBcastToAllGatherV_RootIsThisRank_TakesTheBroadcastSourceBuffer) {
  ...
  EXPECT_EQ(TaskPrep_Addr(kBcastSendAddr + kOwnRoot), agv->sendbuff);
  EXPECT_EQ(TaskPrep_Addr(kBcastRecvAddr + kOwnRoot), agv->recvbuff[kOwnRoot]);
}
...
TEST_F(TaskPreTuningMicrotest, UnreachableMerge_AddBcastToAllGatherV_RootIsAnotherRank_LeavesTheSourceBufferUnset) {
  ...
  EXPECT_EQ(nullptr, agv->sendbuff);
  EXPECT_EQ(TaskPrep_Addr(kBcastRecvAddr + kOtherRoot), agv->recvbuff[kOtherRoot]);
}
...
TEST_F(TaskPreTuningMicrotest,
  ...
  EXPECT_EQ(TaskPrep_Addr(kBcastRecvAddr + kSpareRoot), agv->recvbuff[kSpareRoot]);
  EXPECT_EQ(TaskPrep_Addr(kBcastRecvAddr + kOtherRoot), agv->recvbuff[kOtherRoot]);
