...
TEST_F(TaskClassifyMicrotest, CollToP2pTasks_GatherAtRootAtANarrowerDatatype_ScalesTheRecvOffsets) {
  TaskPrepScene scene(kRanks, kRoot);
  struct ncclTaskTuningInfo* tInfo = TaskClassify_NewCollInfo(&scene, ncclFuncGather, kRoot, ncclInt8);
  const void* sendBase = tInfo->raw->coll.sendbuff;
  const void* recvBase = tInfo->raw->coll.recvbuff;
  TaskClassify_Queue queue;
  ...
  EXPECT_EQ(expected, TaskClassify_P2pShape(&queue));
  TaskClassify_ExpectLoweredPayload(&queue, ncclFuncGather, ncclInt8, kCount);
}
...
TEST_F(TaskClassifyMicrotest, CollToP2pTasks_ScatterAtRootAtANarrowerDatatype_ScalesTheSendOffsets) {
  TaskPrepScene scene(kRanks, kRoot);
  struct ncclTaskTuningInfo* tInfo = TaskClassify_NewCollInfo(&scene, ncclFuncScatter, kRoot, ncclInt8);
  const void* sendBase = tInfo->raw->coll.sendbuff;
  const void* recvBase = tInfo->raw->coll.recvbuff;
  TaskClassify_Queue queue;
  ...
  expected.push_back({ncclFuncRecv, recvBase, kRoot});
  EXPECT_EQ(expected, TaskClassify_P2pShape(&queue));
  TaskClassify_ExpectLoweredPayload(&queue, ncclFuncScatter, ncclInt8, kCount);
}
