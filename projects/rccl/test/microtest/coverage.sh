...
# Allow nonzero exit: DISABLED_ tests that reproduce a known-bad behaviour
# will fail by design, but we still want to capture their coverage.
echo "==> Running ${BIN}"
LLVM_PROFILE_FILE="${PROFRAW}" "${BIN}" "$@" || true
