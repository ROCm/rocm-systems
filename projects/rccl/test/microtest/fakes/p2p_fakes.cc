...
// ---------------------------------------------------------------------------
// Bucket E: stubs for symbols added to p2p.cc after the exploration branch
// ---------------------------------------------------------------------------

// ncclMemTrack / ncclMemUntrack / ncclDynMemMarkExportToPeer:
// Memory-manager entry points called by the ncclCudaCallocAsync overload
// that takes a memManager argument (the 4-arg form). Our macro shim in
// p2p-test.cc absorbs the memManager arg with __VA_ARGS__ and never passes
// it to the fake, so these stubs are here only to satisfy the linker.
...
// ncclProxyClientBatchQueryFdBlocking: multi-segment cuMem FD exchange.
// Not exercised by the 1859 test; stub returns failure.
...
// ncclParamMultiSegmentRegister: generated by NCCL_PARAM in generic.cc
// (not in p2p.cc, so our NCCL_PARAM macro redirect doesn't cover it).
// Return the compile-time default (1) so the param behaves as enabled.
int64_t ncclParamMultiSegmentRegister() { return 1; }
