...
* timemory remains in `external/` and is still referenced by code that has not yet
  been migrated. Existing usage is not a licence to add more.
* When you remove the last call site of a `tim::` symbol, migrate it rather than
  leaving a wrapper behind.
