...
main(int argc, char** argv)
{
    ...
    for(int i = 1; i < argc; i++)
    {
        if(std::string_view{ argv[i] } == "--help" ||
           std::string_view{ argv[i] } == "-h" || std::string_view{ argv[i] } == "-?")
        {
            std::cout << "Usage: " << argv[0] << " [num_kernels (default=" << NUM_KERNELS
                      << ")] [num_iterations (default=" << NUM_ITERATIONS << ")]"
                      << std::endl;
            return 0;
        }
    }
    ...
    std::cout << "Creating HIP graph with " << NUM_KERNELS << " kernel launches"
              << std::endl;
    ...
    for(int iter = 0; iter < NUM_ITERATIONS; iter++)
    {
        ...
        // std::this_thread::sleep_for(std::chrono::milliseconds(100));  // Sleep to
        // simulate work between iterations
    }
    ...
    std::cout << "Average time per iteration: " << (elapsed.count() / NUM_ITERATIONS)
              << " seconds" << std::endl;
    std::cout << "======================" << std::endl;
