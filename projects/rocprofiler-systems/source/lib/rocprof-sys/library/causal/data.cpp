                ...
                sf::BINARY_FILTER, sf::SOURCE_FILTER, sf::FUNCTION_FILTER })
{
    ...
    if(_scopes.count(sf::BINARY_FILTER) > 0)
    {
        ...
        if(_use_default_excludes && _scopes.count(sf::FUNCTION_FILTER) > 0)
        {
            ...
            if(config::get_causal_mode() == state::process::CausalMode::function)
            {
                ...
                _filters.emplace_back(
                    sf{ sf::FILTER_EXCLUDE, sf::FUNCTION_FILTER, "::_M" });
            }
        }
        ...
        // include handling
        {
            ...
            auto _source_include = get_regex_or(config::get_causal_source_scope(),
                                                _source_end_converter, "");
            auto _function_include =
                get_regex_or(config::get_causal_function_scope(), "");
        ...
        // exclude handling
        {
            ...
            auto _source_exclude = get_regex_or(config::get_causal_source_exclude(),
                                                _source_end_converter, "");
            auto _function_exclude =
                get_regex_or(config::get_causal_function_exclude(), "");
    ...
    std::pair<binary_info_t, binary_info_t>& get_cached_binary_info()
    {
        static auto _v = []() {
    ...
    bool satisfies_filter(const binary::scope_filter::filter_scope& _scope,
    ...
    auto compute_eligible_lines_impl()
    {
    ...
    void save_maps_info_impl(std::ostream & _ofs)
    {
    ...
    void save_line_info_impl(std::ostream & _ofs,
    ...
    void perform_experiment_impl(std::shared_ptr<std::promise<void>> _started)  // NOLINT
    {
        ...
        auto _exceeded_duration = [&]() {
            if(_duration_sec > 1.0e-3)
            {
                ...
                {
                    LOG_DEBUG(
                        "[causal] stopping experimentation after {} seconds "
                        "(elapsed: {} seconds)...",
                        _duration_sec,
        ...
        while(state::process::get() < state::process::Finalized)
        {
            ...
            while(!_experim.start())
            {
                if(state::process::get() == state::process::Finalized)
                {
                    ...
                    {
                        _eligible_pc_hist.emplace_back(
                            std::make_pair(itr.first, itr.second));
                    }

                    std::sort(_eligible_pc_hist.begin(), _eligible_pc_hist.end(),
                              [](auto&& _lhs, auto&& _rhs) {
                    ...
                    if(get_verbose() >= 1)
                    {
                        LOG_WARNING("binary pcs: {} maps: {}", _binary.str(),
                                    _memory.str());
                    }
