...
on_kfd_page_fault(typename SdkBackend::kfd_page_fault_record* record, void* data)
{
    ...
    const auto* agent =
        try_get_agent<Externals>(record->agent_id.handle, "kfd_page_fault", "agent");

    record_thread<Externals>(tid);

    constexpr auto k_empty_event_metadata = "{}";

    auto track_name = fmt::format("KFD Page Fault [{}]", agent_label<Externals>(agent));
    ...
    const auto args_str      = get_args_string(function_args_t{
             { 0, "std::uint64_t", "address", fmt::format("{:#x}", record->address.value) },
             { 1, "string", "agent", agent_node_id },
    });
