...
TEST(kfd_queue_test, descriptor_reports_correct_metadata)
{
    ...
    constexpr const auto& domain = k_kfd_queue<mock_sdk, externals>;

    EXPECT_EQ(domain.meta.name, "kfd_queue");
    EXPECT_EQ(domain.meta.id, mock_sdk::BUFFER_TRACING_KFD_QUEUE);
     ...
     on_configure_registers_category_string_and_skips_pmc_info_without_gpu_agents)
{
    ...
    EXPECT_CALL(*g_externals_mock, add_string(Eq(externals::kfd_queue_category_name)))
        .Times(1);
...
TEST(kfd_queue_test, on_configure_registers_pmc_info_for_each_gpu_agent)
{
    ...
    auto gpu_agent = std::make_shared<agent_t>(
        agent_t{ .type = externals::AGENT_TYPE_GPU, .device_type_index = 5 });
    ...
    EXPECT_CALL(
        *g_externals_mock,
        add_pmc_info(
            AllOf(Field(&pmc_info_data_t::type, Eq(externals::AGENT_TYPE_GPU)),
                  ...
                  Field(&pmc_info_data_t::name,
                        Eq(std::string{ externals::kfd_queue_category_name })),
                  Field(&pmc_info_data_t::symbol, Eq(std::string{ "KFD Queue Events" })),
                  Field(&pmc_info_data_t::description,
                        Eq(std::string{ externals::kfd_queue_category_description })))))
        .Times(1);
