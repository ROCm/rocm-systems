...
TEST(kfd_page_migrate_test, descriptor_reports_correct_metadata)
{
    ...
    constexpr const auto& domain = k_kfd_page_migrate<mock_sdk, externals>;

    EXPECT_EQ(domain.meta.name, "kfd_page_migrate");
    EXPECT_EQ(domain.meta.id, mock_sdk::BUFFER_TRACING_KFD_PAGE_MIGRATE);
     ...
     on_configure_registers_category_string_and_skips_pmc_info_without_agents)
{
    ...
    EXPECT_CALL(*g_externals_mock,
                add_string(Eq(externals::kfd_page_migrate_category_name)))
        .Times(1);
    ...
    EXPECT_CALL(*g_externals_mock, get_agents_by_type(Eq(externals::AGENT_TYPE_CPU)))
        .Times(1)
        .WillOnce(Return(std::vector<std::shared_ptr<agent_t>>{}));
...
TEST(kfd_page_migrate_test, on_configure_registers_pmc_info_for_each_gpu_and_cpu_agent)
{
    ...
    auto cpu_agent = std::make_shared<agent_t>(
        agent_t{ .type = externals::AGENT_TYPE_CPU, .device_type_index = 0 });
        ...
        *g_externals_mock,
        add_pmc_info(AllOf(
            ...
            Field(&pmc_info_data_t::name,
                  Eq(std::string{ externals::kfd_page_migrate_category_name })),
            Field(&pmc_info_data_t::symbol,
                  Eq(std::string{ "KFD Page Migration Events" })),
            Field(&pmc_info_data_t::description,
                  Eq(std::string{ externals::kfd_page_migrate_category_description })))))
        .Times(1);
    EXPECT_CALL(
        *g_externals_mock,
        add_pmc_info(AllOf(
            Field(&pmc_info_data_t::type, Eq(externals::AGENT_TYPE_CPU)),
