...
TEST(kfd_event_page_fault_test, descriptor_reports_correct_metadata)
{
    using mock_dispatcher =
        buffered_callback_dispatcher<mock_sdk, mock_sdk::kfd_event_page_fault_record,
                                     on_kfd_event_page_fault<mock_sdk, externals>>;
    constexpr const auto& k_domain = k_kfd_event_page_fault<mock_sdk, externals>;

    EXPECT_EQ(k_domain.meta.name, "kfd_event_page_fault");
    EXPECT_EQ(k_domain.meta.id, mock_sdk::BUFFER_TRACING_KFD_EVENT_PAGE_FAULT);
...
TEST(kfd_event_page_fault_test, descriptor_uses_default_buffer_properties)
{
    constexpr const auto& k_domain = k_kfd_event_page_fault<mock_sdk, externals>;
...
TEST(kfd_event_page_fault_test,
     on_kfd_event_page_fault_handles_empty_record_batch_without_crashing)
{
    mock_sdk::kfd_event_page_fault_record record{};

    on_kfd_event_page_fault<mock_sdk, externals>(&record, nullptr);
}
     ...
     on_configure_registers_category_string_and_skips_pmc_info_without_gpu_agents)
{
    ...
    EXPECT_CALL(*g_externals_mock,
                add_string(Eq(externals::k_kfd_event_page_fault_category_name)))
        .Times(1);
    ...
    on_kfd_event_page_fault_configure<externals>();

    g_externals_mock.reset();
}

TEST(kfd_event_page_fault_test, on_configure_registers_pmc_info_for_each_gpu_agent)
{
    g_externals_mock = std::make_unique<StrictMock<gmock_externals>>();
        ...
        *g_externals_mock,
        add_pmc_info(AllOf(
            ...
            Field(&pmc_info_data_t::name,
                  Eq(std::string{ externals::k_kfd_event_page_fault_category_name })),
            Field(&pmc_info_data_t::symbol,
                  Eq(std::string{ "KFD Event Page Fault Events" })),
