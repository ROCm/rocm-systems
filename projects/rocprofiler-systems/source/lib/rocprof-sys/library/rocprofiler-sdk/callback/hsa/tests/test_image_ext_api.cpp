...
// Regression guard: hsa::image_ext_api must push/pop timemory and stamp
// buffer-storage records with "rocm_hsa_api", not "rocm_hip_api" or any other
// domain's category.
TEST(image_ext_api_test, uses_rocm_hsa_api_category)
{
    constexpr const auto& k_domain =
        k_image_ext_api<mock_sdk_with_tracing, externals_with_tracing>;
