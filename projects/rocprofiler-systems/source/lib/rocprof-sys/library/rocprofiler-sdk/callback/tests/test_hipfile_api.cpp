...
TEST(hipfile_api_test, descriptor_reports_correct_metadata)
{
    constexpr const auto& k_domain = k_hipfile_api<mock_sdk, externals>;

    EXPECT_EQ(k_domain.meta.name, "hipfile_api");
    EXPECT_EQ(k_domain.meta.id, mock_sdk::CALLBACK_TRACING_HIPFILE_API);
    EXPECT_EQ(k_domain.meta.mode, collection_mode::callback);
    ASSERT_TRUE(k_domain.meta.group.has_value());
    EXPECT_EQ(k_domain.meta.group->name, "hipfile_api");
}

TEST(hipfile_api_test, on_record_dispatches_by_phase_without_crashing)
{
    constexpr const auto& k_domain = k_hipfile_api<mock_sdk, externals>;
...
// Regression guard: hipfile_api must push/pop timemory and stamp buffer-storage
// records with "rocm_hipfile_api", not any other domain's category.
TEST(hipfile_api_test, uses_rocm_hipfile_api_category)
{
    constexpr const auto& k_domain =
        k_hipfile_api<mock_sdk_with_tracing, externals_with_tracing>;

    expect_domain_uses_category(k_domain, "rocm_hipfile_api");
}
