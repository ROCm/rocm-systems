...
using MockBackend     = rocprofsys::backends::procfs::testing::strict_mock_backend;
...
{
    EXPECT_CALL(*mock_backend, read_proc_stat()).Times(1);        // init probe
...
{
    ON_CALL(*mock_backend, read_proc_stat())
...
TEST_F(cpu_device_test, load_calculation_with_increasing_jiffies)
{
    ...
    EXPECT_CALL(*mock_backend, read_rusage()).Times(3);           // init + 2 samples
...
TEST_F(cpu_device_test, multiple_samples_accumulate_correctly)
{
    ...
    EXPECT_CALL(*mock_backend, read_cpu_frequencies()).Times(4);  // init + 3 samples
    EXPECT_CALL(*mock_backend, read_rusage()).Times(4);           // init + 3 samples
...
TEST_F(cpu_device_test, only_load_enabled_skips_frequency_and_process)
{
    ...
    EXPECT_CALL(*mock_backend, read_cpu_frequencies())
        .Times(1);  // only the init probe in constructor
