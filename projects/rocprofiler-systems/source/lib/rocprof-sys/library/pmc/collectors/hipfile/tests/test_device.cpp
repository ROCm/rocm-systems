...
constexpr std::uint64_t k_ts_3       = 3 * k_ns_per_sec;

namespace slot_bytes
{
constexpr std::uint64_t k_slot0 = 111;
constexpr std::uint64_t k_slot4 = 999;
constexpr std::uint64_t k_kb1   = 1000;
constexpr std::uint64_t k_kb2p5 = 2500;
constexpr std::uint64_t k_kb1p5 = 1500;
constexpr std::uint64_t k_kb5   = 5000;
constexpr std::uint64_t k_b100  = 100;
constexpr std::uint64_t k_b200  = 200;
constexpr std::uint64_t k_b300  = 300;
constexpr std::uint64_t k_kb4   = 4096;
constexpr std::uint64_t k_mb1   = 1'000'000;
constexpr std::uint64_t k_kb2   = 2000;
constexpr std::uint64_t k_mb2   = 2'000'000;
constexpr std::uint64_t k_kb9   = 9'000;
constexpr std::uint64_t k_kb4w  = 4'000;
constexpr std::uint64_t k_b500  = 500;
constexpr std::uint64_t k_kb10  = 10'000;
}  // namespace slot_bytes

namespace counter_values
{
constexpr std::uint64_t k_read_bytes       = 11;
constexpr std::uint64_t k_write_bytes      = 22;
constexpr std::uint64_t k_read_ops         = 33;
constexpr std::uint64_t k_write_ops        = 44;
constexpr std::uint64_t k_fastpath_reads   = 55;
constexpr std::uint64_t k_fastpath_writes  = 66;
constexpr std::uint64_t k_fallback_reads   = 77;
constexpr std::uint64_t k_fallback_writes  = 88;
constexpr std::uint64_t k_unaligned_reads  = 99;
constexpr std::uint64_t k_unaligned_writes = 110;
constexpr std::uint64_t k_read_errors      = 121;
constexpr std::uint64_t k_write_errors     = 132;
}  // namespace counter_values

namespace attribution
{
constexpr std::uint64_t k_read_ops         = 100;
constexpr std::uint64_t k_fastpath_reads   = 70;
constexpr std::uint64_t k_fallback_reads   = 30;
constexpr std::uint64_t k_read_ops_compat  = 40;
constexpr std::uint64_t k_write_ops_compat = 20;
}  // namespace attribution
...
{
    m_backend->gpu(0).read_bytes = slot_bytes::k_slot0;
    m_backend->gpu(4).read_bytes = slot_bytes::k_slot4;

    device_t remapped{ m_backend, 0, k_remapped_profiler_index };

    EXPECT_EQ(remapped.get_metrics(m_enabled, k_ts_1).read_bytes, slot_bytes::k_slot0);
}
...
TEST_F(HipFileDeviceTest, ordinal_beyond_snapshot_supports_nothing)
{
    ...
    EXPECT_TRUE(out_of_range.get_metrics(m_enabled, k_ts_1).query_failed);
}
...
{
    m_backend->gpu(0).read_bytes = slot_bytes::k_kb1;
    EXPECT_EQ(sample(k_ts_1).read_bytes, slot_bytes::k_kb1);

    m_backend->gpu(0).read_bytes = slot_bytes::k_kb2p5;
    const auto second            = sample(k_ts_2);
    ...
    EXPECT_EQ(second.read_bytes, slot_bytes::k_kb2p5);
    EXPECT_NE(second.read_bytes, slot_bytes::k_kb1p5);
}

TEST_F(HipFileDeviceTest, all_counters_pass_through_unmodified)
{
    auto& stats            = m_backend->gpu(0);
    stats.read_bytes       = counter_values::k_read_bytes;
    stats.write_bytes      = counter_values::k_write_bytes;
    stats.read_ops         = counter_values::k_read_ops;
    stats.write_ops        = counter_values::k_write_ops;
    stats.fastpath_reads   = counter_values::k_fastpath_reads;
    stats.fastpath_writes  = counter_values::k_fastpath_writes;
    stats.fallback_reads   = counter_values::k_fallback_reads;
    stats.fallback_writes  = counter_values::k_fallback_writes;
    stats.unaligned_reads  = counter_values::k_unaligned_reads;
    stats.unaligned_writes = counter_values::k_unaligned_writes;
    stats.read_errors      = counter_values::k_read_errors;
    stats.write_errors     = counter_values::k_write_errors;

    const auto out = sample(k_ts_1);

    EXPECT_EQ(out.read_bytes, counter_values::k_read_bytes);
    EXPECT_EQ(out.write_bytes, counter_values::k_write_bytes);
    EXPECT_EQ(out.read_ops, counter_values::k_read_ops);
    EXPECT_EQ(out.write_ops, counter_values::k_write_ops);
    EXPECT_EQ(out.fastpath_reads, counter_values::k_fastpath_reads);
    EXPECT_EQ(out.fastpath_writes, counter_values::k_fastpath_writes);
    EXPECT_EQ(out.fallback_reads, counter_values::k_fallback_reads);
    EXPECT_EQ(out.fallback_writes, counter_values::k_fallback_writes);
    EXPECT_EQ(out.unaligned_reads, counter_values::k_unaligned_reads);
    EXPECT_EQ(out.unaligned_writes, counter_values::k_unaligned_writes);
    EXPECT_EQ(out.read_errors, counter_values::k_read_errors);
    EXPECT_EQ(out.write_errors, counter_values::k_write_errors);
}

TEST_F(HipFileDeviceTest, counter_reset_is_not_clamped)
{
    m_backend->gpu(0).read_bytes = slot_bytes::k_kb5;
    discard_sample(k_ts_1);
    ...
    m_backend->gpu(0).read_bytes = slot_bytes::k_b100;
    EXPECT_EQ(sample(k_ts_2).read_bytes, slot_bytes::k_b100);
}
...
{
    m_backend->gpu(0).read_ops       = attribution::k_read_ops;
    m_backend->gpu(0).fastpath_reads = attribution::k_fastpath_reads;
    m_backend->gpu(0).fallback_reads = attribution::k_fallback_reads;

    const auto out = sample(k_ts_1);

    EXPECT_EQ(out.fastpath_reads, attribution::k_fastpath_reads);
    EXPECT_EQ(out.fallback_reads, attribution::k_fallback_reads);
...
TEST_F(HipFileDeviceTest, compat_mode_reports_only_fallback)
{
    ...
    m_backend->gpu(0).read_ops        = attribution::k_read_ops_compat;
    m_backend->gpu(0).fallback_reads  = attribution::k_read_ops_compat;
    m_backend->gpu(0).write_ops       = attribution::k_write_ops_compat;
    m_backend->gpu(0).fallback_writes = attribution::k_write_ops_compat;

    const auto out = sample(k_ts_1);

    EXPECT_EQ(out.fallback_reads, attribution::k_read_ops_compat);
    EXPECT_EQ(out.fallback_writes, attribution::k_write_ops_compat);
...
{
    m_backend->gpu(0).read_bytes = slot_bytes::k_mb1;
    ...
    EXPECT_DOUBLE_EQ(sample(k_ts_1).read_bandwidth, 0.0);
}
...
TEST_F(HipFileDeviceTest, bandwidth_normalised_to_wall_clock)
{
    ...
    m_backend->gpu(0).read_bytes = slot_bytes::k_kb2;

    EXPECT_DOUBLE_EQ(sample(k_ts_2).read_bandwidth, k_one_second_bandwidth);
}
...
TEST_F(HipFileDeviceTest, bandwidth_halves_when_interval_doubles)
{
    ...
    m_backend->gpu(0).read_bytes = slot_bytes::k_kb1;
    const auto one_second        = sample(k_ts_2).read_bandwidth;

    m_backend->gpu(0).read_bytes = slot_bytes::k_kb2;
    const auto also_one_second   = sample(k_ts_3).read_bandwidth;
    ...
    m_backend->gpu(0).read_bytes = 0;
    static_cast<void>(slow.get_metrics(m_enabled, k_ts_1));
    m_backend->gpu(0).read_bytes = slot_bytes::k_kb1;
    EXPECT_DOUBLE_EQ(slow.get_metrics(m_enabled, k_ts_3).read_bandwidth,
...
TEST_F(HipFileDeviceTest, write_bandwidth_uses_write_bytes)
{
    ...
    m_backend->gpu(0).read_bytes  = slot_bytes::k_kb9;
    m_backend->gpu(0).write_bytes = slot_bytes::k_kb4w;

    const auto out = sample(k_ts_2);
...
{
    m_backend->gpu(0).read_bytes = slot_bytes::k_kb4;
    discard_sample(k_ts_1);

    // Bytes unchanged: an idle interval reads as zero bandwidth, not as a repeat of
    // the previous rate.
    EXPECT_DOUBLE_EQ(sample(k_ts_2).read_bandwidth, 0.0);
}
...
{
    m_backend->gpu(0).read_bytes = slot_bytes::k_kb10;
    discard_sample(k_ts_1);

    m_backend->gpu(0).read_bytes = slot_bytes::k_b500;
...
TEST_F(HipFileDeviceTest, bandwidth_ignores_io_duration)
{
    ...
    m_backend->gpu(0).read_bytes = slot_bytes::k_mb2;

    EXPECT_DOUBLE_EQ(sample(k_ts_2).read_bandwidth, k_two_megabyte_bandwidth);
}
...
{
    EXPECT_FALSE(sample(k_ts_1).query_failed);
}
...
{
    m_backend->gpu(0).read_bytes = slot_bytes::k_b100;
    m_backend->gpu(1).read_bytes = slot_bytes::k_b200;
    m_backend->gpu(2).read_bytes = slot_bytes::k_b300;

    device_t gpu1{ m_backend, 1 };
    device_t gpu2{ m_backend, 2 };

    EXPECT_EQ(sample(k_ts_1).read_bytes, slot_bytes::k_b100);
    EXPECT_EQ(gpu1.get_metrics(m_enabled, k_ts_1).read_bytes, slot_bytes::k_b200);
    EXPECT_EQ(gpu2.get_metrics(m_enabled, k_ts_1).read_bytes, slot_bytes::k_b300);
}
...
TEST_F(HipFileDeviceTest, bandwidth_state_is_per_device)
{
    ...
    discard_sample(k_ts_1);
    static_cast<void>(gpu1.get_metrics(m_enabled, k_ts_1));

    m_backend->gpu(0).read_bytes = slot_bytes::k_kb1;
    m_backend->gpu(1).read_bytes = slot_bytes::k_kb5;

    EXPECT_DOUBLE_EQ(sample(k_ts_2).read_bandwidth, k_one_second_bandwidth);
    EXPECT_DOUBLE_EQ(gpu1.get_metrics(m_enabled, k_ts_2).read_bandwidth,
...
TEST_F(HipFileDeviceTest, inactive_gpu_reports_zeros)
{
    ...
    const auto out = idle.get_metrics(m_enabled, k_ts_1);
...
TEST_F(HipFileDeviceTest, both_devices_forward_the_same_timestamp)
{
    ...
    EXPECT_EQ(m_backend->queried_timestamps[0], k_ts_1);
    EXPECT_EQ(m_backend->queried_timestamps[1], k_ts_1);
}
