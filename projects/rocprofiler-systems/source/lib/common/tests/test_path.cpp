...
{
    const std::string file_path = create_file("existing_file.txt");
...
{
    const std::string target    = create_file("target.txt");
    const std::string link_path = create_symlink(target, "link_to_target");
    EXPECT_TRUE(exists(link_path));
}

TEST_F(PathTest, Exists_BrokenSymlink)
{
    const std::string link_path = create_symlink("/nonexistent/target", "broken_link");
    EXPECT_TRUE(exists(link_path));
}
...
TEST_F(PathTest, FindPath_AbsoluteExisting)
{
    ...
    const std::string result    = find_path(file_path, 0, "");
...
TEST_F(PathTest, FindPath_NonexistentReturnsOriginal)
{
    ...
    const std::string result      = find_path(nonexistent, 0, m_test_dir);
...
TEST_F(PathTest, FindPath_InSearchPath)
{
    ...
    const std::string result    = find_path("searchable.txt", 0, m_test_dir);
    EXPECT_EQ(result, file_path);
}
...
{
    const std::string file_path = create_file("file with spaces.txt");
    EXPECT_TRUE(exists(file_path));
}
