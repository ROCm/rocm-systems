...
{
    // backend.hpp unpacks record.id before forwarding to the SDK.
    counter_record rec{ counter_instance_id{ 42 }, 0.0 };
    counter_id     out_id{};
    EXPECT_CALL(*g_mock_sdk, query_record_counter_id(rec.id, &out_id))
        .WillOnce(gm::Return(mock_sdk::STATUS_SUCCESS));

    EXPECT_EQ(sut::query_record_counter_id(rec, &out_id), mock_sdk::STATUS_SUCCESS);
}
