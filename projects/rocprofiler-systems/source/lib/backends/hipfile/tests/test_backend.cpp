...
{
constexpr std::uint64_t k_read_bytes       = 4096;
constexpr std::uint64_t k_write_bytes      = 8192;
constexpr std::uint64_t k_total_reads      = 10;
constexpr std::uint64_t k_total_writes     = 20;
constexpr std::uint64_t k_nvfs_reads       = 7;
constexpr std::uint64_t k_nvfs_writes      = 13;
constexpr std::uint64_t k_posix_reads      = 3;
constexpr std::uint64_t k_posix_writes     = 7;
constexpr std::uint64_t k_unaligned_reads  = 1;
constexpr std::uint64_t k_unaligned_writes = 2;
constexpr std::uint64_t k_read_errors      = 5;
constexpr std::uint64_t k_write_errors     = 6;
}  // namespace field_mapping

namespace counts
{
constexpr std::uint64_t k_hundred = 100;
constexpr std::uint64_t k_fifty   = 50;
constexpr std::uint64_t k_sixty   = 60;
constexpr std::uint64_t k_forty   = 40;
}  // namespace counts

namespace bytes
{
constexpr std::uint64_t k_b256    = 256;
constexpr std::uint64_t k_b512    = 512;
constexpr std::uint64_t k_kb1     = 1024;
constexpr std::uint64_t k_kb2     = 2048;
constexpr std::uint64_t k_kb4     = 4096;
constexpr std::uint64_t k_b1000   = 1000;
constexpr std::uint64_t k_b2000   = 2000;
constexpr std::uint64_t k_b999999 = 999'999;
}  // namespace bytes
...
constexpr std::uint64_t k_ts_1 = 1'000'000'000;
constexpr std::uint64_t k_ts_2 = 2'000'000'000;

TEST_F(HipFileBackendTest, field_mapping_is_exhaustive)
{
    auto& src              = gpu(0);
    src.read_bytes         = field_mapping::k_read_bytes;
    src.write_bytes        = field_mapping::k_write_bytes;
    src.n_total_reads      = field_mapping::k_total_reads;
    src.n_total_writes     = field_mapping::k_total_writes;
    src.n_nvfs_reads       = field_mapping::k_nvfs_reads;
    src.n_nvfs_writes      = field_mapping::k_nvfs_writes;
    src.n_posix_reads      = field_mapping::k_posix_reads;
    src.n_posix_writes     = field_mapping::k_posix_writes;
    src.n_unaligned_reads  = field_mapping::k_unaligned_reads;
    src.n_unaligned_writes = field_mapping::k_unaligned_writes;
    src.n_reads_err        = field_mapping::k_read_errors;
    src.n_writes_err       = field_mapping::k_write_errors;

    mock_backend backend{};
    const auto&  out = backend.get_stats(k_ts_1).per_gpu[0];

    // Every field distinct so a transposed or duplicated mapping cannot pass.
    EXPECT_EQ(out.read_bytes, field_mapping::k_read_bytes);
    EXPECT_EQ(out.write_bytes, field_mapping::k_write_bytes);
    EXPECT_EQ(out.read_ops, field_mapping::k_total_reads);
    EXPECT_EQ(out.write_ops, field_mapping::k_total_writes);
    EXPECT_EQ(out.fastpath_reads, field_mapping::k_nvfs_reads);
    EXPECT_EQ(out.fastpath_writes, field_mapping::k_nvfs_writes);
    EXPECT_EQ(out.fallback_reads, field_mapping::k_posix_reads);
    EXPECT_EQ(out.fallback_writes, field_mapping::k_posix_writes);
    EXPECT_EQ(out.unaligned_reads, field_mapping::k_unaligned_reads);
    EXPECT_EQ(out.unaligned_writes, field_mapping::k_unaligned_writes);
    EXPECT_EQ(out.read_errors, field_mapping::k_read_errors);
    EXPECT_EQ(out.write_errors, field_mapping::k_write_errors);
}

TEST_F(HipFileBackendTest, fastpath_only_reads)
{
    gpu(0).n_total_reads = counts::k_hundred;
    gpu(0).n_nvfs_reads  = counts::k_hundred;
    gpu(0).n_posix_reads = 0;

    mock_backend backend{};
    const auto&  out = backend.get_stats(k_ts_1).per_gpu[0];

    EXPECT_EQ(out.fastpath_reads, counts::k_hundred);
...
TEST_F(HipFileBackendTest, fallback_only_reads)
{
    ...
    gpu(0).n_posix_reads = counts::k_hundred;

    mock_backend backend{};
    const auto&  out = backend.get_stats(k_ts_1).per_gpu[0];

    EXPECT_EQ(out.fastpath_reads, 0U);
    EXPECT_EQ(out.fallback_reads, counts::k_hundred);
}

TEST_F(HipFileBackendTest, fastpath_only_writes)
{
    gpu(0).n_total_writes = counts::k_fifty;
    gpu(0).n_nvfs_writes  = counts::k_fifty;
    gpu(0).n_posix_writes = 0;

    mock_backend backend{};
    const auto&  out = backend.get_stats(k_ts_1).per_gpu[0];

    EXPECT_EQ(out.fastpath_writes, counts::k_fifty);
...
TEST_F(HipFileBackendTest, fallback_only_writes)
{
    ...
    gpu(0).n_posix_writes = counts::k_fifty;

    mock_backend backend{};
    const auto&  out = backend.get_stats(k_ts_1).per_gpu[0];

    EXPECT_EQ(out.fastpath_writes, 0U);
    EXPECT_EQ(out.fallback_writes, counts::k_fifty);
}
...
TEST_F(HipFileBackendTest, mixed_fastpath_fallback_totals_match)
{
    ...
    gpu(0).n_total_reads = counts::k_hundred;
    gpu(0).n_nvfs_reads  = counts::k_sixty;
    gpu(0).n_posix_reads = counts::k_forty;
...
{
    gpu(0).read_bytes = bytes::k_kb1;

    mock_backend backend{};
    static_cast<void>(backend.get_stats(k_ts_1));
    static_cast<void>(backend.get_stats(k_ts_1));
    static_cast<void>(backend.get_stats(k_ts_1));
...
TEST_F(HipFileBackendTest, new_timestamp_triggers_new_query)
{
    ...
    gpu(0).read_bytes = bytes::k_kb1;
    EXPECT_EQ(backend.get_stats(k_ts_1).per_gpu[0].read_bytes, bytes::k_kb1);

    gpu(0).read_bytes = bytes::k_kb4;
    EXPECT_EQ(backend.get_stats(k_ts_2).per_gpu[0].read_bytes, bytes::k_kb4);
...
{
    gpu(0).read_bytes = bytes::k_b1000;
    gpu(1).read_bytes = bytes::k_b2000;

    mock_backend backend{};
    const auto&  first = backend.get_stats(k_ts_1);
    ...
    gpu(0).read_bytes = bytes::k_b999999;

    const auto& second = backend.get_stats(k_ts_1);

    EXPECT_EQ(first.per_gpu[0].read_bytes, bytes::k_b1000);
    EXPECT_EQ(second.per_gpu[0].read_bytes, bytes::k_b1000);
    EXPECT_EQ(second.per_gpu[1].read_bytes, bytes::k_b2000);
}
...
TEST_F(HipFileBackendTest, availability_recovers_after_failure)
{
    ...
    gpu(0).read_bytes            = bytes::k_b512;
    EXPECT_EQ(backend.get_stats(k_ts_2).per_gpu[0].read_bytes, bytes::k_b512);
    EXPECT_TRUE(backend.is_available());
}
...
TEST_F(HipFileBackendTest, old_runtime_never_calls_the_stats_api)
{
    ...
    mock_backend backend{};
    const auto&  first  = backend.get_stats(k_ts_1);
    const auto&  second = backend.get_stats(k_ts_2);
...
TEST_F(HipFileBackendTest, supported_runtime_queries_normally)
{
    ...
    gpu(0).read_bytes               = bytes::k_kb2;

    mock_backend backend{};

    EXPECT_EQ(backend.get_stats(k_ts_1).per_gpu[0].read_bytes, bytes::k_kb2);
    EXPECT_TRUE(backend.is_available());
    EXPECT_EQ(mock_wrapper::call_count, 1U);
}
...
{
    gpu(k_inactive_gpu_slot).read_bytes = bytes::k_kb4;
    ...
    EXPECT_EQ(snapshot.per_gpu[k_inactive_gpu_slot].read_bytes, bytes::k_kb4);
...
TEST_F(HipFileBackendTest, factory_produces_usable_backend)
{
    ...
    gpu(0).read_bytes = bytes::k_b256;
    EXPECT_EQ(backend->get_stats(k_ts_1).per_gpu[0].read_bytes, bytes::k_b256);
}
