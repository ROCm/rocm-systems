...
// `pid` is the actual owning process for per-pid cached output; left as
// nullopt for the combined/root-process output written by single_file_sink,
// which defers to output_summary's own getpid() default.
inline bool
write_proto_to(const std::string& filename, const char* data, std::size_t size,
               output_summary& registry, bool emit_status = true,
               std::optional<pid_t> pid = std::nullopt)
{
    ...
    registry.register_file(filename, output_format::perfetto, pid);
    return true;
}
}  // namespace rocprofsys::core::perfetto_sink_detail
