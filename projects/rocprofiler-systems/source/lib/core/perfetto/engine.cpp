...
perfetto_sdk_backend::init_sdk(const engine_config& cfg) const
{
    ...
    std::call_once(g_sdk_init_flag, [&cfg]() {
        ...
        // Registered before Initialize() so that on log-disabled ranks,
        // Initialize's own no-op log_message_callback (set above) has the
        // final word and this filter never overrides it into re-enabling
        // important/error output on a rank that should stay silent.
