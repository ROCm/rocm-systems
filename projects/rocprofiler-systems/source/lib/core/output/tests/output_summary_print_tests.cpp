...
{
std::string
render(rocprofsys::output_summary& summary)
{
...
TEST(output_summary_print, empty_summary_prints_nothing)
{
    rocprofsys::output_summary summary;
    EXPECT_TRUE(render(summary).empty());
}

TEST(output_summary_print, single_row_renders_header_full_path_and_legend)
{
    rocprofsys::output_summary summary;
    summary.register_file("/tmp/rocprofsys-test/perfetto-trace.proto",
                          rocprofsys::output_format::perfetto);

    const std::string out = render(summary);
...
TEST(output_summary_print, multiple_formats_render_names_and_legend_entries)
{
    rocprofsys::output_summary summary;
    summary.register_file("/tmp/rocprofsys-test/perfetto-trace.proto",
                          rocprofsys::output_format::perfetto);
    summary.register_file("/tmp/rocprofsys-test/wall_clock.txt",
...
TEST(output_summary_print, utilized_gpus_render_in_header_not_per_process)
{
    rocprofsys::output_summary summary;
    summary.register_file("/tmp/rocprofsys-test/perfetto-trace.proto",
    ...
    self.gpu_ids = { 0, 1 };
    summary.record_process(std::move(self));
...
TEST(output_summary_print, full_gpu_set_marks_all)
{
    rocprofsys::output_summary summary;
    summary.register_file("/tmp/rocprofsys-test/perfetto-trace.proto",
                          rocprofsys::output_format::perfetto, getpid());
    summary.set_node_gpu_count(2);
    rocprofsys::output::process_metadata self{};
    self.pid     = getpid();
    self.ppid    = -1;
    self.gpu_ids = { 0, 1 };
    ...
    EXPECT_NE(render(summary).find("GPUs: 0,1 (all)"), std::string::npos);
}

TEST(output_summary_print, peer_controlled_path_control_chars_are_stripped)
{
    rocprofsys::output_summary summary;
    // A traced binary could produce a file whose name carries a CSI escape.
    summary.register_file("/tmp/rocprofsys-test/\x1b[31mevil\x1b[0m.proto",
                          rocprofsys::output_format::perfetto);
...
TEST(output_summary_print, relative_path_renders_as_absolute)
{
    rocprofsys::output_summary summary;
    summary.register_file("relative-dir/perfetto-trace.proto",
                          rocprofsys::output_format::perfetto);
...
TEST(output_summary_print, deep_process_collapses_to_one_line_with_sizes)
{
    rocprofsys::output_summary summary;
    const pid_t                root  = getpid();
    constexpr pid_t            child = 700;
    constexpr pid_t            grand = 800;
    summary.register_file("/tmp/rocprofsys-test/root.proto",
                          rocprofsys::output_format::perfetto, root);
    summary.register_file("/tmp/rocprofsys-test/child.proto",
                          rocprofsys::output_format::perfetto, child);
    summary.register_file("/tmp/rocprofsys-test/grand.proto",
                          rocprofsys::output_format::perfetto, grand);
    auto record = [&](pid_t pid, pid_t ppid) {
        rocprofsys::output::process_metadata meta{};
        meta.pid  = pid;
        meta.ppid = ppid;
        summary.record_process(std::move(meta));
    };
    record(root, -1);
    record(child, root);
    record(grand, child);
