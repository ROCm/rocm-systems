...
TEST(output_summary, default_pid_resolves_to_getpid)
{
    rocprofsys::output_summary summary;
    summary.register_file("/tmp/rocprofsys-test/perfetto-trace.proto",
                          rocprofsys::output_format::perfetto);
    const auto rows = summary.rows();
    ASSERT_EQ(rows.size(), 1u);
    EXPECT_EQ(rows.front().pid, getpid());
}

TEST(output_summary, explicit_pid_is_preserved)
{
    rocprofsys::output_summary summary;
...
TEST(output_summary, bump_session_filters_prior_rows_from_view)
{
    rocprofsys::output_summary summary;

    summary.register_file("/tmp/rocprofsys-test/session1-a.proto",
                          rocprofsys::output_format::perfetto);
    EXPECT_EQ(summary.rows().size(), 1u);

    const auto session_two = summary.bump_session();
    EXPECT_EQ(session_two, 2u);

    // After bump, the prior-session row is filtered out of rows().
    EXPECT_TRUE(summary.rows().empty());

    summary.register_file("/tmp/rocprofsys-test/session2-a.proto",
                          rocprofsys::output_format::perfetto);
    summary.register_file("/tmp/rocprofsys-test/session2-b.proto",
                          rocprofsys::output_format::perfetto);
    const auto rows_v2 = summary.rows();
    EXPECT_EQ(rows_v2.size(), 2u);
    for(const auto& r : rows_v2)
        EXPECT_FALSE(r.path.empty());
}

TEST(output_summary, bump_session_is_race_safe_with_concurrent_register)
{
    ...
    std::thread       writer([&]() {
        ...
        {
            summary.register_file("/tmp/rocprofsys-test/stress-" + std::to_string(i++) +
                                            ".proto",
                                        rocprofsys::output_format::perfetto);
        }
    });
    ...
    for(int round = 0; round < 5; ++round)
    {
        ...
        const auto sid = summary.bump_session();
        EXPECT_GE(sid, 2u);
    }
...
TEST(output_summary, missing_file_yields_nullopt_size)
{
    ...
    rocprofsys::output_summary summary;
    summary.register_file(missing.string(), rocprofsys::output_format::perfetto);
    const auto rows = summary.rows();
    ASSERT_EQ(rows.size(), 1u);
    EXPECT_FALSE(rows.front().size_bytes.has_value());
}

TEST(output_summary, existing_file_size_is_captured)
{
    namespace fs        = std::filesystem;
    const auto base_dir = fs::temp_directory_path() / "rocprofsys-summary-test";
    ...
    rocprofsys::output_summary summary;
    summary.register_file(path.string(), rocprofsys::output_format::perfetto);
    const auto rows = summary.rows();
    ASSERT_EQ(rows.size(), 1u);
...
TEST(output_summary, concurrent_register_is_thread_safe)
{
    rocprofsys::output_summary summary;
    ...
    for(int t = 0; t < THREAD_COUNT; ++t)
    {
        threads.emplace_back([&summary, t]() {
            ...
            {
                summary.register_file("/tmp/rocprofsys-test/concurrent-" +
    ...
    EXPECT_EQ(summary.rows().size(), static_cast<std::size_t>(THREAD_COUNT * PER_THREAD));
}

TEST(output_summary, record_process_sparse_upsert_preserves_gpu_ids)
{
    // Mirrors the finalize ordering: the post-processor records the main
    // process with its GPU ids and ppid, then output_summary::print re-records the
    ...
    summary.record_process(sparse);

    const auto procs = summary.processes();
    ASSERT_EQ(procs.size(), 1u);
    EXPECT_EQ(procs.front().ppid, 1);
    EXPECT_EQ(procs.front().gpu_ids, (std::vector<int>{ 0, 1 }));
}

TEST(output_summary, record_process_non_empty_fields_win_on_upsert)
{
