...
to_json(const metadata_registry& registry)
{
    ...
    auto process_info = registry.get_process_info();
    result["process"] = to_json(process_info);

    auto           pmc_list  = registry.get_pmc_info_list();
    ...
    auto           thread_list  = registry.get_thread_info_list();
    ...
    auto           track_list  = registry.get_track_info_list();
    ...
    auto           string_list  = registry.get_string_list();
    nlohmann::json string_array = nlohmann::json::array();
...
#if ROCPROFSYS_USE_ROCM
    auto           code_object_list  = registry.get_code_object_list();
    ...
    // Serialize kernel symbols
    auto           kernel_symbol_list  = registry.get_kernel_symbol_list();
...
void
from_json(metadata_registry& registry, const nlohmann::json& _json)
{
    // Deserialize process info
...
// Public methods for metadata_registry class
bool
metadata_registry::save_to_file(const std::string& filepath) const
{
    try
    {
...
std::string
metadata_registry::to_json_string() const
{
    auto json = to_json(*this);
    return json.dump();
}
...
bool
metadata_registry::from_json_string(const std::string& json_string)
{
    try
    {
