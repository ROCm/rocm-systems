...
struct processor_view_t
{
    ...
    using wall_clock_span_begin_fn_t =
        void (*)(void*, const wall_clock_span_begin_sample&) noexcept;
    using wall_clock_span_end_fn_t    = void (*)(void*,
                                              const wall_clock_span_end_sample&) noexcept;
    ...
    struct vtable_t
    {
        ...
        wall_clock_span_begin_fn_t  handle_wall_clock_span_begin;
        wall_clock_span_end_fn_t    handle_wall_clock_span_end;
    ...
    ROCPROFSYS_INLINE void handle(
        const wall_clock_span_begin_sample& sample) const noexcept
    {
        m_vtable->handle_wall_clock_span_begin(m_object, sample);
    }

    ROCPROFSYS_INLINE void handle(const wall_clock_span_end_sample& sample) const noexcept
    {
        m_vtable->handle_wall_clock_span_end(m_object, sample);
    }
...
private:
    ...
    static inline const vtable_t& get_vtable_for_type() noexcept
    {
        static const vtable_t vtable{
            ...
            +[](void* obj, const wall_clock_span_begin_sample& sample) noexcept {
                static_cast<T*>(obj)->handle(sample);
            },
            +[](void* obj, const wall_clock_span_end_sample& sample) noexcept {
                static_cast<T*>(obj)->handle(sample);
            },
        ...
        type_identifier_t type_identifier, const trace_cache::cacheable_t& sample) const
    {
        switch(type_identifier)
        {
            ...
            case type_identifier_t::kfd_sample:
                ...
                break;
            case type_identifier_t::wall_clock_span_begin:
                handle_sample(static_cast<const wall_clock_span_begin_sample&>(sample));
                break;
            case type_identifier_t::wall_clock_span_end:
                handle_sample(static_cast<const wall_clock_span_end_sample&>(sample));
                break;
