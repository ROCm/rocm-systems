...
namespace
{
constexpr auto gpu_counter_id_0    = std::uint64_t{ 10 };
constexpr auto gpu_counter_id_1    = std::uint64_t{ 20 };
constexpr auto missing_gpu_counter = std::uint64_t{ 99 };
constexpr auto spm_counter_id_0    = std::uint64_t{ 1001 };
constexpr auto spm_counter_id_1    = std::uint64_t{ 1002 };
constexpr auto missing_spm_counter = std::uint64_t{ 9999 };
...
TEST(metadata_registry_test, gpu_perf_counter_lookup_uses_device_and_counter_id)
{
    ...
    registry.set_gpu_perf_counter_counter_names(
        0, {
               { .counter_id    = gpu_counter_id_0,
                 .pmc_info_name = "SQ_WAVES",
                 .track_name    = "GPU [0] SQ_WAVES (S)" },
               { .counter_id    = gpu_counter_id_1,
    ...
    auto device0_counter20 = registry.find_gpu_perf_counter_by_id(0, gpu_counter_id_1);
    ...
    auto device1_counter10 = registry.find_gpu_perf_counter_by_id(1, gpu_counter_id_0);
    ASSERT_TRUE(device1_counter10.has_value());
    EXPECT_EQ(device1_counter10->get().track_name, "GPU [1] SQ_WAVES (S)");

    EXPECT_FALSE(
        registry.find_gpu_perf_counter_by_id(0, missing_gpu_counter).has_value());
    EXPECT_FALSE(registry.find_gpu_perf_counter_by_id(2, gpu_counter_id_0).has_value());
}
...
TEST(metadata_registry_test, spm_counter_lookup_reuses_gpu_counter_entry)
{
    ...
    auto device0_counter1002 = registry.find_spm_counter_by_id(0, spm_counter_id_1);
    ASSERT_TRUE(device0_counter1002.has_value());
    EXPECT_EQ(device0_counter1002->get().counter_id, spm_counter_id_1);
    ...
    auto device1_counter1001 = registry.find_spm_counter_by_id(1, spm_counter_id_0);
    ASSERT_TRUE(device1_counter1001.has_value());
    EXPECT_EQ(device1_counter1001->get().track_name, "GPU SPM SQ_WAVES[XCC=0] [1]");

    EXPECT_FALSE(registry.find_spm_counter_by_id(0, missing_spm_counter).has_value());
    EXPECT_FALSE(registry.find_spm_counter_by_id(2, spm_counter_id_0).has_value());
}
