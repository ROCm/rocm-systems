...
//
// Pattern (same as rocpdsna's own writer_test.cpp):
...
protected:
    ...
    void SetUp() override
    {
        ...
        m_uuid       = "test123";
        auto storage = std::make_unique<rocpdsna::storage_t>(m_db_path, m_uuid);
    ...
    void register_base_metadata()
    {
        ...
        rocpdsna::writer_types::thread_info_t thread{};
    ...
    {
        rocpdsna::writer_types::agent_info_t info{};
    ...
    {
        rocpdsna::writer_types::queue_info_t queue{};
        ...
        rocpdsna::writer_types::stream_info_t stream{};
    ...
    void flush_and_open_reader()
    {
        ...
        auto read_storage = std::make_unique<rocpdsna::storage_t>(m_db_path, m_uuid);
        m_reader          = std::make_unique<rocpdsna::reader_t>(std::move(read_storage));
    }
    ...
    std::unique_ptr<rocpdsna::reader_t> m_reader;
...
TEST_F(rocpd_write_read_test, agents_round_trip_all_types)
{
    ...
    // rocpdsna only supports CPU and GPU agent types; NIC is rejected
...
TEST_F(rocpd_write_read_test, kernel_dispatch_values_persisted)
{
    ...
    rocpdsna::writer_types::kernel_dispatch_data_t kd_write{};
    ...
    for(const auto& tl_event : events)
    {
        if(tl_event.unique_identifier.type !=
           rocpdsna::reader_types::event_type_t::kernel_dispatch)
            continue;
...
TEST_F(rocpd_write_read_test, region_with_args_values_persisted)
{
    ...
    rocpdsna::writer_types::arg_data_t arg0{};
    arg0.position = 0;
    arg0.type     = "void*";
    arg0.name     = "dst";
    ...
    rocpdsna::writer_types::arg_data_t arg1{};
    arg1.position = 1;
    arg1.type     = "size_t";
    arg1.name     = "sizeBytes";
    arg1.value    = "4096";
    ...
    {
        if(tl_event.unique_identifier.type !=
           rocpdsna::reader_types::event_type_t::region)
            continue;
...
TEST_F(rocpd_write_read_test, memory_copy_values_persisted)
{
    ...
    rocpdsna::writer_types::memory_copy_data_t mc_write{};
    ...
    {
        if(tl_event.unique_identifier.type !=
           rocpdsna::reader_types::event_type_t::memory_copy)
            continue;
...
TEST_F(rocpd_write_read_test, memory_alloc_values_persisted)
{
    ...
    rocpdsna::writer_types::memory_alloc_data_t ma_write{};
    ...
    {
        if(tl_event.unique_identifier.type !=
           rocpdsna::reader_types::event_type_t::memory_allocate)
            continue;
...
TEST_F(rocpd_write_read_test, pmc_event_value_persisted)
{
    ...
    rocpdsna::writer_types::agent_unique_id_t agent_uid = make_agent_uid(gpu_agent());

    rocpdsna::writer_types::pmc_info_t pmc_desc{};
    ...
    rocpdsna::writer_types::pmc_event_data_t pmc_write{};
...
TEST_F(rocpd_write_read_test, event_counts_match_inserted_data)
{
    ...
    auto region_it = counts.find(rocpdsna::reader_types::event_type_t::region);
    ASSERT_NE(region_it, counts.end());
    EXPECT_EQ(region_it->second, 2U);

    auto kd_it = counts.find(rocpdsna::reader_types::event_type_t::kernel_dispatch);
