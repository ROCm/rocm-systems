...
// timemory_processor_adapter_t — processor_t<>-derived wrapper around
// timemory_processor_t for registration with sample_processor_t.
//
// Kept in a separate header from timemory_processor.hpp to isolate AMD-SMI / PMC
// collector includes (sample_processor.hpp transitively requires amd_smi/amdsmi.h).
...
class timemory_processor_adapter_t : public processor_t<timemory_processor_adapter_t>
{
public:
    explicit timemory_processor_adapter_t(std::string output_dir)
