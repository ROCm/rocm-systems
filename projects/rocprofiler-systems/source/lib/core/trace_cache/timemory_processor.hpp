...
class timemory_processor_t
{
    ...
    struct tree_node
    {
        ...
        tree_node* find_or_create_child(const std::string& child_name,
                                        std::uint64_t child_hash, int child_depth)
        {
            for(auto& c : children)
            {
...
public:
    ...
    void handle(const backtrace_region_sample& sample)
    {
        ...
        try
        {
            ...
            if(ext.contains("cpu_ns")) cpu_ns = ext["cpu_ns"].get<std::int64_t>();
        } catch(...)
        {
            return;
        }
...
private:
    ...
    void group_and_process_()
    {
        ...
        std::map<std::uint64_t, std::uint64_t> thread_frame_counts;
        ...
        for(auto& [sk, frames] : samples)
        {
            ...
            for(auto* fp : frames)
            {
                std::uint64_t h     = hash_label_(fp->name);
            ...
            std::uint64_t total_frames = thread_frame_counts[sk.thread_id];
    ...
    void flatten_tree_(std::uint64_t tid, const tree_node& node,
    ...
    void parse_seq_from_track_(const std::string& track_name, std::uint64_t tid)
    {
        ...
        if(pos != std::string::npos)
        {
            ...
            if(end != std::string::npos)
            {
                try
                {
                    std::uint64_t seq = std::stoull(track_name.substr(pos, end - pos));
    ...
    std::uint64_t seq_for_(std::uint64_t tid) const
    {
    ...
    std::string make_prefix_(std::uint64_t tid, int depth, const std::string& label) const
    {
    ...
    static std::uint64_t hash_label_(const std::string& label)
    {
    ...
    static double variance_(std::uint64_t n, double sum, double sum_sq)
    {
        if(n < 2) return 0.0;
        double v =
    ...
    void emit_metric_files_(const std::string& metric_name,
                            const std::string& description, std::uint64_t unit_value,
                            ...
                            const std::vector<flat_node>& all_nodes, bool is_cpu)
    {
        ...
        std::uint64_t  thread_count = m_thread_ids.size();
    ...
    void emit_trip_count_files_(const std::vector<flat_node>& all_nodes)
    {
        ...
        for(auto& n : all_nodes)
        {
            ...
            entry["value"]                = static_cast<std::int64_t>(n.trip_count);
            entry["repr_data"]            = static_cast<std::int64_t>(n.trip_count);
            entry["repr_display"]         = static_cast<std::int64_t>(n.trip_count);
    ...
    {
        std::map<std::uint64_t, double> thread_totals;
    ...
    std::map<std::uint64_t, std::unique_ptr<tree_node>> m_trees;

    std::map<std::pair<std::uint64_t, std::string>, pct_entry> m_pct_agg;

    std::set<std::uint64_t>                m_thread_ids;
    std::map<std::uint64_t, std::uint64_t> m_tid_to_seq;
