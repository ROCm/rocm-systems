...
class session_scope_test_interface : public ::testing::Test
{
protected:
    session m_ses;
};
...
TEST_F(session_scope_test_interface,
       subscriber_not_resumed_while_a_listened_scope_is_still_paused)
{
    ...
    const mock_trigger global_trigger{ m_ses, "global_trigger", scope::global,
                                       action::pause };
    const mock_trigger sampling_trigger{ m_ses, "sampling_trigger", scope::sampling,
                                         action::pause };

    int resume_count = 0;
    int pause_count  = 0;
    m_ses.subscribe({ .on_pause  = [&pause_count]() { ++pause_count; },
...
TEST_F(session_scope_test_interface,
       subscriber_paused_immediately_when_any_listened_scope_pauses)
{
    const mock_trigger global_trigger{ m_ses, "global_trigger", scope::global,
                                       action::trace };
    const mock_trigger sampling_trigger{ m_ses, "sampling_trigger", scope::sampling,
...
TEST_F(session_scope_test_interface, single_scope_subscriber_unaffected_by_other_scope)
{
...
TEST_F(session_scope_test_interface, is_active_is_tracked_independently_per_scope)
{
    const mock_trigger global_trigger{ m_ses, "global_trigger", scope::global,
                                       action::trace };
    const mock_trigger sampling_trigger{ m_ses, "sampling_trigger", scope::sampling,
    ...
    EXPECT_TRUE(m_ses.is_active(scope::global));
    EXPECT_FALSE(m_ses.is_active(scope::sampling));
}
       ...
       same_named_triggers_in_different_scopes_do_not_clobber_each_other)
{
    ...
    // overwrite the first.
    const mock_trigger global_window{ m_ses, "time_window", scope::global,
                                      action::pause };
    const mock_trigger sampling_window{ m_ses, "time_window", scope::sampling,
                                        action::pause };

    EXPECT_FALSE(m_ses.is_active(scope::global));
    EXPECT_FALSE(m_ses.is_active(scope::sampling));

    sampling_window.set_action(action::trace);
    EXPECT_TRUE(m_ses.is_active(scope::sampling));
    ...
    {
        const mock_trigger probe{ m_ses, "probe", scope::global, action::trace };
    }
    EXPECT_FALSE(m_ses.is_active(scope::global))
        << "global_window's pause must survive the sampling window's registration";
}

TEST_F(session_scope_test_interface, unregister_removes_only_the_matching_scope)
{
    const mock_trigger sampling_window{ m_ses, "time_window", scope::sampling,
                                        action::pause };
    {
    ...
    {
        const mock_trigger probe{ m_ses, "probe", scope::sampling, action::trace };
    }
    EXPECT_FALSE(m_ses.is_active(scope::sampling))
