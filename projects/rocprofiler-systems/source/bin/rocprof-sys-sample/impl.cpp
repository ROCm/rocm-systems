...
parse_args(int argc, char** argv, std::vector<char*>& _env)
{
    ...
    auto parser = parser_t(argv[0], rocprofsys::sample::descriptions::main);
...
%{INDENT}%    1     do not modify how ROCm is notified about kernel completion)";
    ...
    for(const auto& preset : rocprofsys::sample::PRESETS)
    {
        ...
        parser
            ...
            .max_count(1)
            .dtype("bool")
            .action([&, preset_name, env_settings = preset.env_settings,
                     has_gpu_check = preset.has_gpu_devices_check](parser_t& p) {
                    ...
                    for(const auto& [key, value] : env_settings)
                    {
                        std::visit(
                            [&](auto&& val) {
                                ...
                                {
                                    rocprofsys::common::update_env(
                                        _env, std::string(key), std::string(val),
                                        update_mode::REPLACE, ":", updated_envs,
                                        original_envs);
                                }
                                else
                                {
                                    rocprofsys::common::update_env(
                                        _env, std::string(key), val, update_mode::REPLACE,
                                        ":", updated_envs, original_envs);
                                }
                            },
                            value);
