...
: function_signature(_ret, _name, _file, _row, _col, _loop, _info_beg, _info_end)
{
    ...
    if(!_params.empty())
    {
        m_params.resize(m_params.length() - 2);
    }
    m_params.push_back(')');
}
...
{
    if(!_all && _save && !m_signature.empty())
    {
        return m_signature;
    }

    std::stringstream ss;
    if((_all || use_return_info) && !m_return.empty())
    {
        ss << m_return << " ";
    }
    ss << m_name;
    if(_all || use_args_info)
    {
        ss << m_params;
    }
    if(m_loop)
    ...
    if((_all || use_line_info) && m_row.first > 0)
    {
        ss << ":" << m_row.first;
    }
    if((_all || use_file_info) && m_file.length() > 0)
