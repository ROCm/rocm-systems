...
| `@pytest.mark.run_if_gpu_category("expr")` | Run test only if GPU category expression is true. Supports boolean expressions with `and`, `or`, `not`. Categories: `instinct`, `radeon`, `apu`. Examples: `"instinct"`, `"not apu"`, `"apu and not instinct"`, `"instinct or radeon"`. |
