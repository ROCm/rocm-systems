...
.. note::
   ...
   ROCpd database output is optional and is generated only when ``rocpd`` is
   included in ``--output-format`` (for example, ``--output-format rocpd``).
   In that case, the output directory also contains ``rocpd-<pid>.db``.
