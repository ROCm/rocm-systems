...
struct arg_vtab : public sqlite3_vtab
{
    sqlite3*                    db = nullptr;
    std::string                 real_table_name;
    std::unique_ptr<arg_buffer> buffer;
};

struct arg_cursor : public sqlite3_vtab_cursor
{
                 ...
                 char**             pzErr)
{
    if(argc < 4)
    {
        *pzErr = sqlite3_mprintf("arg_buffer requires one argument: real table name");
        return SQLITE_ERROR;
    }
    ...
    std::string create_sql = "CREATE TABLE x("
                             "id INTEGER, event_id INTEGER, position INTEGER, "
                             "type TEXT, name TEXT, value TEXT, extdata TEXT"
                             ")";
    ...
    auto* vtab = new(std::nothrow) arg_vtab();
    ...
    vtab->buffer = std::make_unique<arg_buffer>(vtab->real_table_name, db);
    arg_buffer::register_instance(vtab->real_table_name, vtab->buffer.get());
...
{
    auto* vtab = static_cast<arg_vtab*>(p);
    if(vtab != nullptr)
    {
        arg_buffer::unregister_instance(vtab->real_table_name);
        vtab->buffer.reset();
        delete vtab;
    }
    return SQLITE_OK;
...
{
    auto* cur = new(std::nothrow) arg_cursor();
...
{
    delete static_cast<arg_cursor*>(c);
    return SQLITE_OK;
}
...
{
    static_cast<arg_cursor*>(c)->rowid++;
    return SQLITE_OK;
}
...
{
    *out = static_cast<arg_cursor*>(c)->rowid;
    return SQLITE_OK;
}
...
xUpdate(sqlite3_vtab* p, int argc, sqlite3_value** argv, sqlite3_int64* out_rowid)
{
    ...
    constexpr int expected_argc = 2 + static_cast<int>(arg_buffer::k_total_columns);
...
{
    int rc = sqlite3_create_module(db, "arg_buffer", &k_module, nullptr);
    if(rc != SQLITE_OK)
    {
        LOG_ERROR("Failed to register arg_buffer module: {}", sqlite3_errmsg(db));
    }
    return rc;
}
