// Copyright (c) Advanced Micro Devices, Inc.
// SPDX-License-Identifier:  MIT
//
// Per-table buffer for the sample writer.

#pragma once

#include <sqlite3.h>

#include <array>
#include <cstdint>
#include <optional>
#include <string>
#include <string_view>
#include <vector>

namespace profiler_hub::data_storage::vtable
{

// Named-field row for sample_buffer::push. Field order matches the SQL
// column list: id, track_id, timestamp, event_id, extdata.
struct sample_row
{
    int64_t                id;
    int64_t                track_id;
    int64_t                timestamp;
    std::optional<int64_t> event_id;  // nullable
    std::string_view       extdata;
};

class sample_buffer
{
public:
    static constexpr size_t k_int_column_count  = 4;
    static constexpr size_t k_text_column_count = 1;
    static constexpr size_t k_total_columns     = 5;

    // The writer_conn is owned by sqlite_backend and outlives this buffer.
    sample_buffer(std::string real_table_name, sqlite3* writer_conn);
    ~sample_buffer();

    sample_buffer(const sample_buffer&)            = delete;
    sample_buffer& operator=(const sample_buffer&) = delete;
    sample_buffer(sample_buffer&&)                 = delete;
    sample_buffer& operator=(sample_buffer&&)      = delete;

    void push(const sample_row& row);

    void push_from_values(sqlite3_value** argv);

    int flush();

    void reserve(std::size_t expected_rows);

    [[nodiscard]] size_t row_count() const noexcept { return m_row_count; }

    [[nodiscard]] sqlite3* writer_connection() const noexcept { return m_writer_conn; }

    static void register_instance(const std::string& real_table_name,
                                  sample_buffer*     buffer);
    static void unregister_instance(const std::string& real_table_name);
    [[nodiscard]] static sample_buffer* get_active_instance(
        const std::string& real_table_name);

private:
    int prepare_insert_stmt();

    struct int_column_t
    {
        std::vector<int64_t> values;
        std::vector<uint8_t> is_null;
    };

    struct text_column_t
    {
        std::vector<std::string> values;
        std::vector<uint8_t>     is_null;
    };

    std::string                                    m_real_table_name;
    std::string                                    m_insert_sql;
    sqlite3*                                       m_writer_conn = nullptr;
    sqlite3_stmt*                                  m_insert_stmt = nullptr;
    std::array<int_column_t, k_int_column_count>   m_int_cols{};
    std::array<text_column_t, k_text_column_count> m_text_cols{};
    size_t                                         m_row_count = 0;
};

}  // namespace profiler_hub::data_storage::vtable
