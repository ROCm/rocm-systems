...
#include "sample_buffer_vtab.hpp"
...
struct sample_vtab : public sqlite3_vtab
{
    sqlite3*                       db = nullptr;
...
struct sample_cursor : public sqlite3_vtab_cursor
{
                 ...
                 char**             pzErr)
{
    if(argc < 4)
    {
        *pzErr = sqlite3_mprintf("sample_buffer requires one argument: real table name");
        return SQLITE_ERROR;
    }
    ...
    std::string create_sql = "CREATE TABLE x("
                             "id INTEGER, track_id INTEGER, timestamp INTEGER, "
    ...
    auto* vtab = new(std::nothrow) sample_vtab();
    ...
    vtab->buffer = std::make_unique<sample_buffer>(vtab->real_table_name, db);
    sample_buffer::register_instance(vtab->real_table_name, vtab->buffer.get());
...
{
    auto* vtab = static_cast<sample_vtab*>(p);
    if(vtab != nullptr)
    {
        sample_buffer::unregister_instance(vtab->real_table_name);
        vtab->buffer.reset();
        delete vtab;
    }
    return SQLITE_OK;
...
{
    auto* cur = new(std::nothrow) sample_cursor();
...
{
    delete static_cast<sample_cursor*>(c);
    return SQLITE_OK;
}
...
{
    static_cast<sample_cursor*>(c)->rowid++;
    return SQLITE_OK;
}
...
{
    *out = static_cast<sample_cursor*>(c)->rowid;
    return SQLITE_OK;
}
...
xUpdate(sqlite3_vtab* p, int argc, sqlite3_value** argv, sqlite3_int64* out_rowid)
{
    ...
    constexpr int expected_argc = 2 + static_cast<int>(sample_buffer::k_total_columns);
...
{
    int rc = sqlite3_create_module(db, "sample_buffer", &k_module, nullptr);
    if(rc != SQLITE_OK)
    {
        LOG_ERROR("Failed to register sample_buffer module: {}", sqlite3_errmsg(db));
    }
    return rc;
}
