...
#define REQUIRE(expression) do { \
  if (!(expression)) { \
    fprintf(stderr, "PM4 LOCAL: line %d: %s\n", __LINE__, #expression); \
    return 1; \
  } \
} while (0)
#define CALL(expression) do { \
  ...
  if (call_status != AMDF_STATUS_OK) { \
    fprintf(stderr, "PM4 LOCAL: line %d: %s: status=0x%016" PRIx64 "\n", \
            __LINE__, #expression, call_status); \
    return 1; \
  } \
} while (0)

enum { BUFFER_BYTES = 65536, SLOT_WORDS = 256, ITERATIONS = 16 };
...
static int create_queue(struct workload *w) {
  ...
  REQUIRE(w->transport.command_type == AMDF_QUEUE_COMMAND_TYPE_GPU_PM4);
  REQUIRE(w->transport.format_version == AMDF_GPU_PM4_QUEUE_FORMAT_VERSION_1);
  REQUIRE(w->transport.format_features == w->format_features);
  REQUIRE(w->transport.ring_byte_length == 4096);
...
static uint32_t pm4_header(uint32_t opcode, uint32_t words) {
  return (3u << 30) | (opcode << 8) | ((words - 2) << 16);
}

static void pm4_barrier(uint32_t *words, size_t *used) {
  const uint32_t gcr = (3u << 0) | (1u << 4) | (1u << 5) |
      (1u << 7) | (1u << 8) | (1u << 9) | (1u << 14) | (1u << 15);
  words[(*used)++] = pm4_header(0x46, 2);
  words[(*used)++] = 7 | (4 << 8);
  words[(*used)++] = pm4_header(0x58, 8);
  words[(*used)++] = 0;
  words[(*used)++] = UINT32_MAX;
  words[(*used)++] = 0xff;
  words[(*used)++] = 0;
  words[(*used)++] = 0;
  words[(*used)++] = 0x0a;
...
static void pm4_copy32(uint32_t *words, size_t *used, uint64_t source,
                       uint64_t target) {
  words[(*used)++] = pm4_header(0x40, 6);
  words[(*used)++] = (2u << 0) | (2u << 8) | (1u << 20);
  words[(*used)++] = (uint32_t)source;
  words[(*used)++] = (uint32_t)(source >> 32);
  words[(*used)++] = (uint32_t)target;
  words[(*used)++] = (uint32_t)(target >> 32);
}

static void pm4_write32(uint32_t *words, size_t *used, uint64_t target,
                        uint32_t value) {
  words[(*used)++] = pm4_header(0x37, 5);
  words[(*used)++] = (2u << 8) | (1u << 20);
  words[(*used)++] = (uint32_t)target;
  words[(*used)++] = (uint32_t)(target >> 32);
  words[(*used)++] = value;
}
...
static void encode(uint32_t words[SLOT_WORDS], uint64_t source,
                   uint64_t local, uint64_t target,
                   uint64_t fence, uint32_t token) {
  size_t used = 0;
  pm4_barrier(words, &used);
  for (uint32_t i = 0; i < 16; ++i)
    pm4_copy32(words, &used, source + i * 4, local + i * 4);
  pm4_barrier(words, &used);
  for (uint32_t i = 0; i < 16; ++i)
    pm4_copy32(words, &used, local + i * 4, target + i * 4);
  pm4_barrier(words, &used);
  pm4_write32(words, &used, fence, token);
  const size_t padding = SLOT_WORDS - used;
  if (used >= SLOT_WORDS || padding < 2) abort();
  words[used++] = pm4_header(0x10, (uint32_t)padding);
  while (used < SLOT_WORDS) words[used++] = 0;
}

static int publish_and_wait(struct workload *w, uint64_t *producer,
                            const uint32_t words[SLOT_WORDS], uint32_t token) {
  ...
  const uint64_t ring_words = w->transport.ring_byte_length / 4;
  uint32_t *slot = (uint32_t *)(uintptr_t)w->transport.ring_address +
      (*producer & (ring_words - 1));
  memcpy(slot, words, sizeof(uint32_t[SLOT_WORDS]));
  *producer += SLOT_WORDS;
  w->in_flight = 1;
  ...
  while (__atomic_load_n(completion, __ATOMIC_ACQUIRE) != token) {
    if ((++polls & 255u) == 0 && now_ns() - begin >= timeout_ns) {
      fprintf(stderr, "PM4 LOCAL timeout: token=%u observed=%u producer=%" PRIu64
              "\n", token,
              __atomic_load_n(completion, __ATOMIC_ACQUIRE), *producer);
      return 1;
    }
  ...
  CALL(w->api->user_queue_wait_consumed(w->queue, *producer,
      timeout_ns, UINT64_C(10000000)));
  w->in_flight = 0;
  return 0;
}
...
static int run(struct workload *w) {
  ...
  uint32_t *completion = (uint32_t *)w->completion.host.pointer;
  uint32_t *local = w->public_local ? (uint32_t *)w->local.host.pointer : NULL;
  for (unsigned iteration = 0; iteration < ITERATIONS; ++iteration) {
    for (uint32_t i = 0; i < 16; ++i) {
      source[i] = 0x13570000u + i * 0x00110101u + iteration;
      target[i] = 0xdeadbeefu;
    }
    __atomic_store_n(completion, 0, __ATOMIC_RELAXED);
    CALL(w->api->host_mapping_cache_control(w->source.mapping,
        AMDF_HOST_CACHE_OPERATION_FLUSH, 0, 64));
    CALL(w->api->host_mapping_cache_control(w->target.mapping,
        AMDF_HOST_CACHE_OPERATION_FLUSH, 0, 64));
    CALL(w->api->host_mapping_cache_control(w->completion.mapping,
        AMDF_HOST_CACHE_OPERATION_FLUSH, 0, 64));
    ...
    REQUIRE(publish_and_wait(w, &producer, commands, iteration + 1) == 0);
    CALL(w->api->host_mapping_cache_control(w->target.mapping,
        AMDF_HOST_CACHE_OPERATION_INVALIDATE, 0, 64));
    for (uint32_t i = 0; i < 16; ++i)
      REQUIRE(target[i] == source[i]);
    if (w->public_local) {
      CALL(w->api->host_mapping_cache_control(w->local.mapping,
          AMDF_HOST_CACHE_OPERATION_INVALIDATE, 0, 64));
      for (uint32_t i = 0; i < 16; ++i)
        REQUIRE(local[i] == source[i]);
      for (uint32_t i = 0; i < 16; ++i) {
        local[i] = 0xace00000u + i * 0x101u + iteration;
        target[i] = 0xdeadbeefu;
      }
      __atomic_store_n(completion, 0, __ATOMIC_RELAXED);
      CALL(w->api->host_mapping_cache_control(w->local.mapping,
      ...
      encode(commands, w->local.address, w->target.address, w->local.address,
             w->completion.address, token);
      REQUIRE(publish_and_wait(w, &producer, commands, token) == 0);
      CALL(w->api->host_mapping_cache_control(w->target.mapping,
          AMDF_HOST_CACHE_OPERATION_INVALIDATE, 0, 64));
      for (uint32_t i = 0; i < 16; ++i)
        REQUIRE(target[i] == 0xace00000u + i * 0x101u + iteration);
    }
  }
  ...
  printf("Verified %u PM4 %s LOCAL copies%s and %" PRIu64
         " ring wraps\n", ITERATIONS,
         w->public_local ? "public" : "private",
         w->public_local ? " with CPU access" : "",
         producer / (w->transport.ring_byte_length / 4));
  return 0;
}
