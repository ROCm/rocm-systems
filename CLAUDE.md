...
**Recommendations** (rules in priority order):
   - **Tier 3 ATT**: `[ATT VMEM Latency/LDS Conflict/Dependency Chain/Divergence]` — stall_ratio ≥ 0.40 AND hitcount ≥ 6400
...
**Collection**: Decoder is `librocprof-trace-decoder.so` (loaded at collection time via `--att-library-path`). No standalone `rocprof-trace-decoder` binary. No separate decode step.
```bash
rocprofv3 --att --att-library-path /opt/rocm/lib --att-target-cu 0 \
    -d ./att_output -o trace -- ./my_app
```

**Real CSV format** (produced by `librocprof-trace-decoder.so`):
```
"CodeObj","Vaddr","Instruction","Hitcount","Latency","Stall","Idle","Source"
```
First data row per CSV has `Hitcount=0, Latency=0`, `Instruction="; _Zmangled..."`, `Source="demangled_kernel_name(args)"` — kernel name extracted from `Source` column of this comment row.

New in `analyze.py`:
- `analyze_thread_trace(att_dir)` — parses `stats_*.csv`; normalizes column names (strip quotes + lowercase); extracts kernel name from comment row Source column; computes weighted stall ratio; classifies: `att_vmem_latency`, `att_lds_conflict`, `att_dependency_chain`, `att_divergence`
- `generate_recommendations()` now accepts `att_analysis=` kwarg; generates HIGH/MEDIUM `[ATT ...]` recs when `stall_ratio >= 0.40` and `hitcount >= 6400`
- `--att-dir` CLI flag; `att_dir=` parameter in `analyze_database()` API
- JSON output includes `att_trace` key; `schema_version` bumps to `"0.4.0"`
- Webview includes full ATT section card (KPI cards + per-kernel stall table) injected at `<!-- ATT_SECTION_PLACEHOLDER -->` inside `.wrap` div

**HTML section order**: Overview → Execution Breakdown → Hotspots → Memory Transfer → Hardware Counters (Tier 2) → Thread Trace (Tier 3) → Optimization Recommendations
