// Simple HSA application that creates a queue, signal, and submits a barrier packet
...
int main(int argc, char** argv) {
    LOG_INFO("Starting HSA barrier packet test");
    ...
    // Create completion signal (initialize to 1, will be decremented to 0 on completion)
    LOG_INFO("Creating completion signal with initial value 1");
    hsa_signal_t completion_signal;
    CHECK_HSA(hsa_signal_create(1, 0, nullptr, &completion_signal));

    // Verify initial signal value
    hsa_signal_value_t initial_value = hsa_signal_load_relaxed(completion_signal);
    LOG_INFO("Completion signal created with initial value: %ld", initial_value);

    // Create a barrier packet
    LOG_INFO("Creating barrier packet");
    ...
    // Submit the barrier packet
    LOG_INFO("Submitting barrier packet to queue");

    app_debugger_block();  // Block before packet submission
    submit_packet(queue, &barrier_packet);
    app_debugger_block();  // Block after packet submission

    // Wait for completion signal to reach 0
    LOG_INFO("Waiting for packet completion (signal to reach 0)");
    ...
    if(final_value == 0) {
        LOG_INFO("Barrier packet completed successfully!");
    } else {
        LOG_INFO("Barrier packet timed out, final signal value: %ld", final_value);
    }
    ...
    LOG_INFO("Barrier packet test completed successfully");
    return 0;
}