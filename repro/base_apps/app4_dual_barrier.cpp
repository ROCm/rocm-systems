// HSA Dual Barrier Application
// Creates queue, signal, and 2 barrier packets with dependencies
...
int main(int argc, char** argv) {
    LOG_INFO("Starting HSA dual barrier application");
    ...
    // Create first barrier packet with signal as completion signal
    LOG_INFO("Creating first barrier packet");
    hsa_barrier_and_packet_t barrier_packet1 = {};
    barrier_packet1.header = create_packet_header(HSA_PACKET_TYPE_BARRIER_AND);
    barrier_packet1.completion_signal = completion_signal;

    // Submit first barrier packet
    LOG_INFO("Submitting first barrier packet");
    submit_packet(queue, &barrier_packet1);

    // Create second barrier packet that depends on first (using dep_signal array)
    // and also uses same signal as completion
    LOG_INFO("Creating second barrier packet with dependency");
    hsa_barrier_and_packet_t barrier_packet2 = {};
    barrier_packet2.header = create_packet_header(HSA_PACKET_TYPE_BARRIER_AND);
    barrier_packet2.dep_signal[0] = completion_signal;  // Depends on first barrier
    barrier_packet2.completion_signal = completion_signal;  // Same completion signal

    // Submit second barrier packet
    LOG_INFO("Submitting second barrier packet");

    app_debugger_block();  // Block before second packet submission
    submit_packet(queue, &barrier_packet2);
    app_debugger_block();  // Block after second packet submission
    ...
    if(final_value == 0) {
        LOG_INFO("Both barrier packets completed successfully!");
    } else {
        LOG_INFO("Barrier packets timed out, final signal value: %ld", final_value);
    }
    ...
    LOG_INFO("Dual barrier application completed successfully");
