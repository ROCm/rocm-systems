// HSA AQL Read Packet Application
// Creates queue and submits AQL profiling start and read packets with debugger blocks around read
...
int main(int argc, char** argv) {
    LOG_INFO("Starting HSA AQL read packet application");
    ...
    // Submit read packet with debugger blocks
    LOG_INFO("Submitting AQL read packet");

    app_debugger_block();  // Block before read packet submission
    uint64_t read_write_idx = submit_packet(queue, &packets.read_packet);
    app_debugger_block();  // Block after read packet submission
    ...
    LOG_INFO("AQL start and read packets have been submitted to the queue");
    LOG_INFO("Profiling counters are active and data has been copied for reading");
    ...
    LOG_INFO("AQL read packet application completed successfully");
