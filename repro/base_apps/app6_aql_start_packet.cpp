// HSA AQL Start Packet Application
// Creates queue and submits AQL profiling start packet with debugger blocks
...
int main(int argc, char** argv) {
    LOG_INFO("Starting HSA AQL start packet application");
    ...
    // Submit start packet with debugger blocks
    LOG_INFO("Submitting AQL start packet");

    app_debugger_block();  // Block before start packet submission
    uint64_t start_write_idx = submit_packet(queue, &packets.start_packet);
    app_debugger_block();  // Block after start packet submission
    ...
    LOG_INFO("AQL start packet has been submitted to the queue");
    LOG_INFO("Profiling counters are now active and collecting data");
    ...
    LOG_INFO("AQL start packet application completed successfully");
