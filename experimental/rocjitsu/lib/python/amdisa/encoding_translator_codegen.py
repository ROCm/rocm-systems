...
# CDNA4 -> CDNA3 is structurally close to an in-family copy for the encodings
# used by ordinary kernels. Keeping it as an explicit pair still routes DBT
# through the generated decode/re-encode path, which lets tests catch future
# CDNA4-only field drift instead of accidentally relying on raw word copies.
