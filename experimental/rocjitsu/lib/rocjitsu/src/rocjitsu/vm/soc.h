...
public:
  ...
  /// @brief Set the execution plugin group (takes ownership).
  void set_plugin_group(std::unique_ptr<amdgpu::ExecutionPluginGroup> plugin_group);
...
private:
  ...
  std::unique_ptr<amdgpu::ExecutionPluginGroup> plugin_group_;
};
