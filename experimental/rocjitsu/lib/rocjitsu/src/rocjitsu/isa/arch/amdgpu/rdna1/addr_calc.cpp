...
uint64_t smem_calculate_address(const SmemMachineInst &inst, amdgpu::Wavefront &wf) {
  (void)inst;
  (void)wf;
  // RDNA1 SmemMachineInst lacks soffset_en/imm fields required by shared template.
...
void flat_calculate_addresses(const FlatMachineInst &inst, amdgpu::Wavefront &wf,
  ...
  (void)addrs;
  (void)lane_mask;
  // RDNA1 FlatMachineInst lacks pad_12 field required by shared flat template.
