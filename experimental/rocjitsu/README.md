...
The instruction files under `lib/rocjitsu/src/rocjitsu/isa/arch/amdgpu/` are
autogenerated from the MR ISA XML specifications. To fetch and unpack the latest
archive directly into the repo, run:

```bash
python scripts/download_machine_readable_isa.py --force
```

This extracts the XML files under `third_party/machine-readable-isa/`.
To regenerate all architectures (CDNA1–CDNA4, RDNA1–RDNA4) in one step:

```bash
bash scripts/regenerate_isa.sh
```

The script invokes `amdisa --multi` with `--gen-all --gen-shared-execute` across
all nine architecture XMLs, writes the output to the `amdgpu/` directory, and
runs clang-format automatically.
