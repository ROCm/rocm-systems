...
- **Classification rules** (which `Action` each instruction gets) — auto-generated by `amdisa/legalization.py` from the ISA XML specs, stored as sorted `LegEntry[]` arrays in `isa/legalization/legalize_*.cpp`. Each entry maps a `(src_encoding_id, src_opcode)` pair to an `Action` value and, for `Action::Substitute`, a `target_opcode`. Adding a new rule means updating the mnemonic rename map or domain-specific rules in `legalization.py` and regenerating.
...
// isa/legalization/legalization_tables.h.  The types Action, LegEntry,
// and lookup() are defined ONCE in the generated header — InstructionMapper
// does NOT duplicate them.

#include "rocjitsu/isa/legalization/legalization_tables.h"

namespace rocjitsu {

/// @brief Legalization rule table for a (src_arch, dst_arch) pair.
///
/// Wraps a std::span<const LegEntry> from the auto-generated tables.
/// The Action enum (Identity, Substitute, Lower, Expand, Illegal) and
/// LegEntry struct are defined in isa/legalization/legalization_tables.h
...
public:
  explicit InstructionMapper(std::span<const LegEntry> table) : table_(table) {}
  ...
  const LegEntry *legalize(uint16_t encoding_id, uint16_t opcode) const {
    return lookup(table_, encoding_id, opcode);
  }

private:
  std::span<const LegEntry> table_;
};
...
std::vector<uint32_t> BinaryTranslator::translate_instruction(
  ...
  const LegEntry *entry = mapper_->legalize(encoding_id_of(inst), opcode_of(inst));
  if (!entry) {
...
|---|---|---|
| **Identity** | `Action::Identity` | Instruction encoding is identical on both ISAs; copy word(s) verbatim. |
| **Substitute** | `Action::Substitute` | Same semantics, different opcode; `BinaryTranslator` swaps the opcode field via `LegEntry::target_opcode`. |
| **Lower** | `Action::Lower` | Instruction must be lowered to a target-native sequence (e.g., waitcnt re-encoding, flat→global, decode-and-re-encode). Lowering logic is in the matching `lower_*()` method on `BinaryTranslator`. |
...
> **Type system note:** The `Action` enum and `LegEntry` struct are defined once in the auto-generated header `isa/legalization/legalization_tables.h`. `InstructionMapper` in `code/dbt/instruction_mapper.h` includes this header and uses these types directly — no parallel `LegalizationAction` or `InstructionMapping` types exist.
...
```cpp
// isa/legalization/legalization_tables.h — AUTO-GENERATED by amdisa
...
// Per-pair tables (e.g., kTable_cdna3_cdna4, kTable_cdna3_rdna4, ...)
...
std::unique_ptr<InstructionMapper> InstructionMapper::create(
    ...
    // Select the correct kTable_* array for this (src, dst) pair.
...
└── legalization_codegen.py  # NEW — emit C++ LegEntry tables (~150 lines)
```
...
```cpp
std::vector<uint32_t> BinaryTranslator::lower_encoding(
    const Instruction &inst, const InstrRecord &src_rec, const InstrRecord &dst_rec) {
    // 1. Decode all fields from source instruction using src ISA bitfields
    auto fields = decode_fields(inst, src_arch_);
    // 2. Construct target instruction using dst ISA bitfields with same field values
    auto target_word = encode_fields(fields, dst_arch_, dst_rec.enc_name);
    return {target_word};
}
```

This is the most common lowering case (~60-80% of all LOWER entries) and produces exactly the same number of target instructions as source instructions (1:1 mapping).
...
Covered in §3.6. All MFMA and SMFMAC instructions on non-CDNA targets are expanded using a **tiered strategy** (cheapest first):

**Tier 0 — MFMA→WMMA hardware mapping (CDNA→RDNA3/4 only):**
When the target has WMMA hardware (RDNA3/4), MFMA instructions with compatible shapes and data types are lowered to tiled WMMA sequences rather than software fallback. The MFMA output matrix is decomposed into WMMA-sized tiles (16×16 per WMMA instruction), and AccVGPR-resident accumulators are staged through regular VGPRs. The WMMA instructions run at hardware speed (~10-50x faster than software stubs). Not all MFMA shapes map cleanly to WMMA tiles — unsupported shapes fall through to Tier 1.
...
| MFMA → WMMA (CDNA→RDNA3/4) | EXPAND | Tier 0: tiled WMMA hardware mapping | Yes (1:4–1:8) | 1:4 |
| MFMA (no WMMA target) | EXPAND | Tier 1: software stub via trampoline | Yes (1:~100+) | 1:~100+ |
| WMMA → MFMA (RDNA3/4→CDNA) | EXPAND | Tier 0: MFMA hardware mapping + AccVGPR staging | Yes (1:3–1:6) | 1:4 |
...
### Phase 8.5: Legalization Table Generator — `amdisa/legalization.py` (3–5 days) [parallel with Phases 1–8]
...
| C++ codegen: emit sorted `LegEntry[]` tables per (src, dst) pair + shared `legalization_tables.h` header | `lib/python/amdisa/legalization_codegen.py` (new) |
| CLI integration: `--gen-legalization`, `--legalization-output`, `--legalization-pairs` flags | `lib/python/amdisa/__main__.py` (modified) |
| CMake integration: invoke `python -m amdisa --multi ... --gen-legalization` and add generated `.cpp` files to `librocjitsu` | `CMakeLists.txt` (modified) |
...
- Never-empty test: for every supported (src, dst) pair with all 9 ISAs loaded, assert the generated table is non-empty and contains at least one IDENTITY entry (there are always shared instructions).
...
- `lib/rocjitsu/src/rocjitsu/isa/legalization/legalization_tables.h` — auto-generated legalization tables
- `lib/python/amdisa/legalization.py` — legalization table generator
- `lib/python/amdisa/legalization_codegen.py` — C++20 table codegen
