...
def warmup_perfxpert_mcp(
    ...
    * Idempotent — safe to call multiple times; each call is a
    ...
    # Best-effort read — we don't care about the response content,
    # only that the subprocess got far enough to open sqlite + emit
    # a response.
    if error is None and proc.stdout is not None:
        proc.stdout.readline()  # may return '' if subprocess exited early
    ...
    success = (proc.returncode is not None) and error is None
    return WarmupReport(
        success=success,
        duration_s=time.monotonic() - start,
        error=error,
