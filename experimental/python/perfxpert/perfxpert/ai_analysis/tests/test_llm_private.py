...
from perfxpert.ai_analysis.exceptions import LLMAuthenticationError, LLMRateLimitError
from perfxpert.ai_analysis.llm_analyzer import LLMAnalyzer
from perfxpert.ai_analysis.llm_conversation import LLMConversation, _build_private_client
...
def private_env(monkeypatch):
    ...
    monkeypatch.setenv("PERFXPERT_LLM_PRIVATE_URL", "https://private.example.com/v1")
    monkeypatch.setenv("PERFXPERT_LLM_PRIVATE_MODEL", "my-private-model")
    monkeypatch.setenv("PERFXPERT_LLM_PRIVATE_API_KEY", "test-key-123")
    monkeypatch.delenv("PERFXPERT_LLM_PRIVATE_HEADERS", raising=False)
...
class TestBuildPrivateClient:
    def test_requires_private_url_env(self, monkeypatch):
        ...
        with patch.dict("sys.modules", {"openai": MagicMock()}):
            ...
            with pytest.raises(ValueError, match="PERFXPERT_LLM_PRIVATE_URL"):
                _build_private_client(None, None)

    def test_merges_json_headers_from_env(self, monkeypatch):
        monkeypatch.setenv("PERFXPERT_LLM_PRIVATE_URL", "https://example.com/v1")
        monkeypatch.setenv(
    ...
    def test_auto_sets_user_header(self, monkeypatch):
        ...
        with patch("perfxpert.ai_analysis.llm_conversation.os.getlogin", return_value="testuser"):
            with patch.dict("sys.modules", {"openai": MagicMock(OpenAI=mock_openai_cls)}):
                _build_private_client("dummy-key", None)
...
class TestLLMAnalyzerPrivate:
    def test_call_private_uses_correct_url(self, private_env, ref_guide_path):
        """LLMAnalyzer._call_private must build a client pointing at PERFXPERT_LLM_PRIVATE_URL."""
    ...
    def test_private_headers_include_auto_user(self, private_env, ref_guide_path, monkeypatch):
        """PERFXPERT_LLM_PRIVATE_HEADERS merges with the auto-set user header."""
        monkeypatch.setenv(
            "PERFXPERT_LLM_PRIVATE_HEADERS",
        ...
        with patch(
            "perfxpert.ai_analysis.llm_analyzer.os.getlogin", return_value="devuser"
        ):
            with patch.dict("sys.modules", {"openai": mock_openai_mod}):
                analyzer._call_private("sys", "user")
...
class TestLLMConversationPrivate:
    def test_send_private_uses_private_url(self, private_env):
        ...
        with patch(
            "perfxpert.ai_analysis.llm_conversation._build_private_client",
    ...
    def test_send_local_uses_ollama_endpoint(self, monkeypatch):
        monkeypatch.setenv("PERFXPERT_LLM_LOCAL_URL", "http://localhost:11434/v1")
