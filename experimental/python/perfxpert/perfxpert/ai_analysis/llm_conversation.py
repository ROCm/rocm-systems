...
def _build_private_client(api_key: Optional[str], model_override: Optional[str]):
    ...
    Reads configuration from environment variables:
        PERFXPERT_LLM_PRIVATE_URL        Base URL of the private server (required)
        PERFXPERT_LLM_PRIVATE_MODEL      Model name to use
        PERFXPERT_LLM_PRIVATE_API_KEY    API key (default: "dummy" for header-auth servers)
        PERFXPERT_LLM_PRIVATE_HEADERS    JSON object of extra request headers, e.g.
        ...
        PERFXPERT_LLM_PRIVATE_VERIFY_SSL Set to "0" or "false" to disable SSL certificate
    ...
    if not base_url:
        raise ValueError(
            ...
            "  export PERFXPERT_LLM_PRIVATE_URL=https://my-apim.example.com/openai/deployments/gpt4"
        )
    key = api_key or os.environ.get("PERFXPERT_LLM_PRIVATE_API_KEY", "dummy")
    model = model_override or os.environ.get("PERFXPERT_LLM_PRIVATE_MODEL", "")
    ...
    if raw_headers:
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    f"PERFXPERT_LLM_PRIVATE_HEADERS is not valid JSON: {e}\n"
                    f'Use double-quoted JSON: \'{{"Ocp-Apim-Subscription-Key": "abc123"}}\'\n'
                    f"Value was: {raw_headers!r}"
                )
        if not isinstance(parsed_headers, dict):
            raise ValueError(
                f"PERFXPERT_LLM_PRIVATE_HEADERS must be a JSON object, got "
    ...
    # SSL verification — disabled when PERFXPERT_LLM_PRIVATE_VERIFY_SSL=0/false
    ...
    if not verify_ssl:
        warnings.warn(
            ...
            "PERFXPERT_LLM_PRIVATE_VERIFY_SSL — LLM traffic is exposed to "
        ...
        try:
            ...
            warnings.warn(
                "[LLMConversation] PERFXPERT_LLM_PRIVATE_VERIFY_SSL=0 requested but httpx is "
...
class LLMConversation:
    ...
    def _stream_anthropic(
        ...
        model = (
            self._model or os.environ.get("PERFXPERT_LLM_MODEL") or DEFAULT_ANTHROPIC_MODEL
        )
    ...
    def _stream_claude_code(
        ...
        """
        model = self._model or os.environ.get("PERFXPERT_LLM_MODEL") or "sonnet"
    ...
    def _stream_openai(
        ...
        if self._provider == "local":
            base_url = os.environ.get("PERFXPERT_LLM_LOCAL_URL", _DEFAULT_LOCAL_URL)
            model = self._model or os.environ.get(
                "PERFXPERT_LLM_LOCAL_MODEL", _DEFAULT_LOCAL_MODEL
            )
            ...
            model = (
                self._model or os.environ.get("PERFXPERT_LLM_MODEL") or DEFAULT_OPENAI_MODEL
            )
