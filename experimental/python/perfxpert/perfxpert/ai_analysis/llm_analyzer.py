...
# Regex to match perfxpert-context tag comments in the reference guide
_TAG_RE = re.compile(r"<!--\s*perfxpert-context:\s*([^-]+?)\s*-->")
...
# Users can override with PERFXPERT_LLM_REFERENCE_GUIDE environment variable
DEFAULT_REFERENCE_GUIDE_NAME = "llm-reference-guide.md"

# Default model names — override at runtime with PERFXPERT_LLM_MODEL env var
...
PROVIDER_REGISTRY: Dict[str, str] = {
    ...
    "private": "Private/enterprise OpenAI-compatible endpoint (requires PERFXPERT_LLM_PRIVATE_URL)",
    "local": "Local Ollama or OpenAI-compatible server (PERFXPERT_LLM_LOCAL_URL)",
...
def get_reference_guide_path() -> Path:
    ...
    Priority order:
    1. PERFXPERT_LLM_REFERENCE_GUIDE environment variable
    ...
    env_path = os.environ.get("PERFXPERT_LLM_REFERENCE_GUIDE")
    if env_path:
...
def load_reference_guide() -> str:
    ...
    PERFXPERT_LLM_REFERENCE_GUIDE env var → module share/ dir → /opt/rocm/share/...
...
def _filter_guide(guide_text: str, tags: set) -> str:
    ...
    ``<!-- perfxpert-context: TAG [, TAG ...] -->`` comment matches one of the
    requested *tags*.
...
class LLMAnalyzer:
    ...
    def __init__(
        ...
        Args:
            ...
            thinking_budget_tokens: Enable extended thinking with this token budget.
                ...
                Can also be set via PERFXPERT_LLM_THINKING environment variable.
        """
        ...
        if thinking_budget_tokens is not None:
            ...
            _env_thinking = os.environ.get("PERFXPERT_LLM_THINKING")
            if _env_thinking:
                try:
                    ...
                    warnings.warn(
                        f"PERFXPERT_LLM_THINKING={_env_thinking!r} is not a valid integer; "
    ...
    def _get_api_key_from_env(self, raise_if_missing: bool = True) -> str:
        ...
        if self.provider == "anthropic":
            ...
            return os.environ.get("PERFXPERT_LLM_LOCAL_API_KEY", "ignored")
        elif self.provider == "private":
            return os.environ.get("PERFXPERT_LLM_PRIVATE_API_KEY", "dummy")
...
Follow the reference guide strictly for analysis methodology and output format."""
    ...
    def _call_anthropic(
        ...
        try:
            ...
            model = (
                self.model or os.environ.get("PERFXPERT_LLM_MODEL") or DEFAULT_ANTHROPIC_MODEL
            )
    ...
    def _call_openai(
        ...
        try:
            ...
            model = (
                self.model or os.environ.get("PERFXPERT_LLM_MODEL") or DEFAULT_OPENAI_MODEL
            )
            _messages = [
    ...
    def _call_local(self, system_prompt: str, user_prompt: str) -> str:
        ...
        base_url = os.environ.get("PERFXPERT_LLM_LOCAL_URL", "http://localhost:11434/v1")
        client = openai.OpenAI(base_url=base_url, api_key="ignored")
        model = self.model or os.environ.get("PERFXPERT_LLM_LOCAL_MODEL", "codellama:13b")
        try:
            ...
            raise RuntimeError(
                ...
                f"Is Ollama running? Set PERFXPERT_LLM_LOCAL_URL to override endpoint. "
                f"Error: {exc}"
            ) from exc
    ...
    def _call_private(self, system_prompt: str, user_prompt: str) -> str:
        ...
        Configuration via environment variables:
            PERFXPERT_LLM_PRIVATE_URL        Base URL (required)
            PERFXPERT_LLM_PRIVATE_MODEL      Model name (required)
            PERFXPERT_LLM_PRIVATE_API_KEY    API key (default: "dummy")
            PERFXPERT_LLM_PRIVATE_HEADERS    JSON object of extra request headers
                                         (the "user" header defaults to os.getlogin())
            PERFXPERT_LLM_PRIVATE_VERIFY_SSL Set to "0" or "false" to disable SSL
        ...
        base_url = os.environ.get("PERFXPERT_LLM_PRIVATE_URL", "")
        if not base_url:
            raise ValueError(
                "PERFXPERT_LLM_PRIVATE_URL is not set. "
                "Export it to point at your private LLM server."
            )
        model = self.model or os.environ.get("PERFXPERT_LLM_PRIVATE_MODEL", "")
        if not model:
            raise ValueError(
                "No model specified for private provider. "
                "Set PERFXPERT_LLM_PRIVATE_MODEL or pass --llm-private-model."
            )
        key = self.api_key or os.environ.get("PERFXPERT_LLM_PRIVATE_API_KEY", "dummy")
        ...
        try:
            ...
            pass
        raw_headers = os.environ.get("PERFXPERT_LLM_PRIVATE_HEADERS", "")
        if raw_headers:
            ...
            try:
                ...
                try:
                    ...
                    raise ValueError(f"PERFXPERT_LLM_PRIVATE_HEADERS is not valid JSON: {e}")
            if not isinstance(parsed_h, dict):
                raise ValueError(
                    "PERFXPERT_LLM_PRIVATE_HEADERS must be a JSON object of header "
                    'key/value pairs (e.g. {"X-My-Header": "value"}), '
                    f"got {type(parsed_h).__name__}"
                )
            headers.update(parsed_h)

        verify_ssl_env = os.environ.get("PERFXPERT_LLM_PRIVATE_VERIFY_SSL", "1").lower()
        ...
        if not verify_ssl:
            ...
            warnings.warn(
                ...
                "PERFXPERT_LLM_PRIVATE_VERIFY_SSL — LLM traffic is exposed "
            ...
            try:
                ...
                warnings.warn(
                    "PERFXPERT_LLM_PRIVATE_VERIFY_SSL=0 requested but httpx is not installed. "
        ...
        try:
            try:
                ...
                raise RuntimeError(
                    ...
                    f"Check PERFXPERT_LLM_PRIVATE_URL, PERFXPERT_LLM_PRIVATE_HEADERS. "
                    f"Error: {exc}"
                ) from exc
        finally:
            if http_client is not None:
    ...
    def _call_claude_code(self, system_prompt: str, user_prompt: str) -> str:
        ...
        """
        model = self.model or os.environ.get("PERFXPERT_LLM_MODEL") or "sonnet"
    ...
    def _call_claude_cli_subprocess(
        ...
        with _tempfile.TemporaryDirectory(prefix="perfxpert_claude_") as _tmpdir:
    ...
    def _call_claude_code_api_fallback(
        ...
        try:
            ...
            raise AnalysisError(
                ...
                "Install one: pip install 'perfxpert[llm]'  or  pip install claude-agent-sdk"
            )
