...
def _classify_verdict(diff_result: Dict[str, Any]) -> str:
    ...
    Thresholds mirror ``trace_diff._airgap_narrative`` (0.5% wall band
    for the headline) so the narrative and verdict agree on tone.
    """
...
def _airgap_narrative(diff_result: Dict[str, Any]) -> str:
    """Deterministic one-liner summary used under ``PERFXPERT_AIRGAP=1``.

    Matches the template documented in the agent task brief — one short
    line that surfaces the wall-time delta, regression/improvement
    counts, and the single largest regression (by |delta_pct|). The
    trace_diff tool already synthesises a longer multi-line narrative;
    the specialist purposely returns a one-liner so calling agents can
    quote it inline without re-formatting.
    """
    wall = float(diff_result.get("wall_delta_pct", 0.0) or 0.0)
    regressions = diff_result.get("primary_regressions", []) or []
...
def run_diff_specialist(
    ...
    *,
    ...
    The deterministic ``trace_diff.diff_runs`` call happens in both
    airgap and live-provider modes — the LLM only rewrites the
    narrative; the numeric verdict is always derived from the same
    arithmetic so air-gap parity is preserved (spec §5).
    """
    ...
    if raw.get("_mode") == "airgap":
        ...
        # Allow the LLM to refine verdict wording only within the
        # documented enum. Fall back to the deterministic classification
        # if the provider returns anything off-piste.
        llm_verdict = so.get("verdict")
        if llm_verdict in {"improved", "regressed", "neutral"}:
            verdict = llm_verdict
        # Accept LLM-supplied overrides for kernel lists when present;
        # otherwise the deterministic ranking stands.
