# XXX(docs): tool count is now 41, agent tools available
...
Correctness, Compute-Specialist, Memory-Specialist, Latency-Specialist) is
exposed here as a READ_ONLY MCP tool. The MCP auto-discovery registry
walks this sub-package and registers the 7 callables so backend TUIs
(opencode / claude / codex / gemini) can invoke any agent directly —
the backend LLM picks which agent to call based on the user's intent.

The same 7 functions are mirrored 1:1 at :mod:`perfxpert.api` so Python
