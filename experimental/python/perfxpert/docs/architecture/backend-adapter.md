...
|---------|------------------------|--------------|-------------------|---------------|
| opencode (patched artifact + packaged config) | `$XDG_CACHE_HOME/perfxpert/opencode/opencode.json`, or `~/.cache/perfxpert/opencode/opencode.json` when `XDG_CACHE_HOME` is unset | Patched system prompt (AMD fork patches 0010, 0012-0017, 0020) | Prompt-layer tool-priority guidance; no mechanical runtime block hook today | N/A — state carried in opencode session |
