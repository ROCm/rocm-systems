...
# when the submodule is at the pinned tag with no local diff. If patches
# are already applied (tree is dirty), we skip silently.
cd "${SUBMODULE}"
...
if [ ! -d "node_modules" ] || [ "${PERFXPERT_FORCE_INSTALL:-0}" = "1" ]; then
  echo "build-bundled-opencode: running 'bun install' at ${SUBMODULE}"
  bun install
else
...
echo "build-bundled-opencode: built ${BUILT} ($(stat -c%s "${BUILT}") bytes)"
...
echo "build-bundled-opencode: sha256=$(sha256sum "${OUTPUT}" | awk '{print $1}')"
