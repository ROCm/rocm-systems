...
# while suppressing monorepo submodule initialization for the Python package.
#
# Usage:
...
readonly _SUBMODULE_SCOPE="__perfxpert_no_submodules__"
...
OS prerequisite handling:
  If curl, git, or a supported Python/pip pair is missing, the wrapper
...
Patched opencode source:
  The wrapper does not initialize monorepo submodules and does not build or
  package a generated opencode binary during pip install. Source/editable users
  can initialize the pinned PerfXpert opencode submodule and build it
  explicitly with scripts/build-patched-opencode.sh or the perfxpert-code
  install-patches alias.
...
If you are working from a source checkout and need the patched opencode TUI,
initialize the pinned submodule and run:
...
_install_os_prereqs_if_needed() {
  local -a missing
  missing=()
  local need_curl need_git need_python
  need_curl=0
  need_git=0
  need_python=0
...
if ! command -v curl >/dev/null 2>&1; then
  {
    cat <<'EOF'
pip-install-from-git: `curl` is required for the GitHub install path.
EOF
    echo
    _print_python_prereqs
  } >&2
...
echo "pip-install-from-git: perfxpert install complete; monorepo submodules were not initialized and the generated opencode binary was not built or packaged" >&2
