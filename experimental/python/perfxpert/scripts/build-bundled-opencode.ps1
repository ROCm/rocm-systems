...
function Get-NormalizedTextSha256Lower([string]$Path) {
...
function New-NormalizedPatchFile([string]$Path) {
    $Bytes = [System.IO.File]::ReadAllBytes($Path)
    $Text = [System.Text.Encoding]::UTF8.GetString($Bytes)
    $Text = $Text.Replace("`r`n", "`n").Replace("`r", "`n")
...
function Invoke-OpencodePostinstall([string]$Submodule) {
...
function Invoke-OpencodeBuild([string]$OpencodePackage) {
...
function Apply-OpencodePatches([string]$PerfXpertRoot, [string]$OpencodeDir) {
    ...
    foreach ($Entry in $ManifestEntries) {
        ...
        $Actual = Get-NormalizedTextSha256Lower $PatchPath
    ...
    foreach ($Patch in $Patches) {
        ...
        $NormalizedPatch = New-NormalizedPatchFile $Patch.FullName
        try {
            & git -C $OpencodeDir apply --check $NormalizedPatch
            if ($LASTEXITCODE -ne 0) {
            ...
            & git -C $OpencodeDir apply $NormalizedPatch
            if ($LASTEXITCODE -ne 0) {
                & git -C $OpencodeDir checkout HEAD -- . | Out-Null
            ...
            Remove-Item -Path $NormalizedPatch -Force -ErrorAction SilentlyContinue
        }
        $Applied += $Patch.Name
    }
...
if ((-not (Test-Path -Path $NodeModules -PathType Container)) -or $env:PERFXPERT_FORCE_INSTALL -eq "1") {
    ...
    try {
        ...
        Invoke-OpencodePostinstall $Submodule
    }
    finally {
        Pop-Location
    }
...
$BuildExit = Invoke-OpencodeBuild $OpencodePackage
if ($BuildExit -ne 0) {
    [Console]::Error.WriteLine("build-bundled-opencode: build failed; running 'bun install --force --ignore-scripts' once and retrying")
    Push-Location $Submodule
    try {
        & bun install --force --ignore-scripts
        if ($LASTEXITCODE -ne 0) {
            Write-Fail "'bun install --force --ignore-scripts' exited $LASTEXITCODE" $LASTEXITCODE
        }
        Invoke-OpencodePostinstall $Submodule
    }
    finally {
    ...
    $BuildExit = Invoke-OpencodeBuild $OpencodePackage
    if ($BuildExit -ne 0) {
        Write-Fail "'bun run build --single' exited $BuildExit after force reinstall retry" $BuildExit
    }
}
