...
def _ensure_opencode_checkout() -> bool:
    ...
    if repo_root is not None:
        rel_opencode = _OPENCODE_DIR.relative_to(repo_root).as_posix()
    ...
    rel_hint = (
        _OPENCODE_DIR.relative_to(repo_root).as_posix()
        if repo_root is not None
...
def _copy_bundled_binary_to_build_lib(build_lib: str) -> None:
    """Ensure a freshly built bundle lands in the wheel build tree.

    ``build_py`` discovers package data before our build hook creates the
    opencode binary on some pip/setuptools combinations. Copying the
    platform-specific binary into ``build_lib`` after ``super().run()`` keeps
    the final wheel from losing the generated executable.
    """
    src = _bundle_path()
    if not src.is_file():
        return
    dst = Path(build_lib) / "perfxpert" / "_bundled" / src.name
...
class _BuildPyWithOpencode(_build_py):
    def run(self) -> None:
        _run_opencode_build()
        super().run()
        _copy_bundled_binary_to_build_lib(self.build_lib)
