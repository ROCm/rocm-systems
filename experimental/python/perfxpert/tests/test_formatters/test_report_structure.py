...
_FIXTURE_DB = Path(__file__).resolve().parent.parent / "fixtures" / "compute_bound.db"
...
def test_webview_summary_at_top_and_uses_scard(airgap, tiny_hip_src, tmp_path):
    ...
    assert out.count("<!DOCTYPE html>") == 1, f"expected 1 <!DOCTYPE html>, got {out.count('<!DOCTYPE html>')}"
    ...
    assert i_summary < i_exec, f"Summary (at {i_summary}) must come before Execution Breakdown " f"(at {i_exec})"
    ...
    assert 'class="scard"' in preamble, "Summary h2 must be inside a `.scard` container, not an ad-hoc `.card`"

    # No legacy `.card` markup remains.
    assert 'class="card"' not in out, (
        "webview still contains ad-hoc `.card` markup — Summary/Tier-0 " "splice should standardize on `.scard`"
    )
...
def test_markdown_summary_has_horizontal_rule_before_metadata(airgap, tiny_hip_src, tmp_path):
    ...
    # H1 first.
    assert out.lstrip().startswith("# PerfXpert AI Performance Analysis"), "markdown must start with canonical H1"
...
def test_text_summary_precedes_time_breakdown(airgap, tiny_hip_src, tmp_path):
    ...
    assert i_summary < i_tb, f"SUMMARY (at {i_summary}) must precede TIME BREAKDOWN (at {i_tb})"
    # Between the two there must be at least one '\n\n' pair.
    between = out[i_summary:i_tb]
    assert "\n\n" in between, "expected a blank line between SUMMARY and TIME BREAKDOWN"
...
def test_json_has_all_required_keys_and_bumped_schema_version(airgap, tiny_hip_src, tmp_path):
    ...
    assert sv.startswith("0.3."), f"expected schema_version 0.3.x, got {doc['schema_version']!r}"
    assert sv >= "0.3.0"
    ...
    if "llm_enhanced_explanation" in doc:
        assert (
            doc["llm_enhanced_explanation"] == doc["narrative"] or doc["llm_enhanced_explanation"] is None
