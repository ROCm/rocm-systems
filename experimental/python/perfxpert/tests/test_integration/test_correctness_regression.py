...
def test_regressed_verdict_flows_to_revert_action(regression_db_pair, monkeypatch):
    ...
    monkeypatch.setattr(gate_cascade, "_run_compile_gate", lambda *a, **kw: {"ok": True})
    monkeypatch.setattr(gate_cascade, "_run_sol_gate", lambda *a, **kw: {"ok": True, "peak_ratio": 0.5})
    monkeypatch.setattr(gate_cascade, "_run_bitwise_gate", lambda *a, **kw: {"ok": True})
    monkeypatch.setattr(
        gate_cascade,
    ...
    monkeypatch.setattr(gate_cascade, "_run_anchors_gate", lambda *a, **kw: {"ok": True})
