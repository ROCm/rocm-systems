...
def test_root_routing_identical_airgap_vs_llm(user_query, expected_route, monkeypatch):
    ...
    airgap_out = airgap_session.run_root(schemas.RootInput(user_query=user_query, database_path=None))
    ...
    llm_out = llm_session.run_root(schemas.RootInput(user_query=user_query, database_path=None))
...
def test_classification_verdict_identical_in_both_modes(memory_bound_db, monkeypatch):
    ...
    airgap_session = build_session(airgap=True)
    airgap_out = airgap_session.run_analysis(schemas.AnalysisInput(database_path=str(memory_bound_db)))
    ...
    llm_session = build_session(provider="anthropic")
    llm_out = llm_session.run_analysis(schemas.AnalysisInput(database_path=str(memory_bound_db)))
