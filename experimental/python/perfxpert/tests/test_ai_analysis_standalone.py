...
class TestBugFixes:
    ...
    def test_analyze_source_passes_analysis_context_to_llm(self, tmp_path):
        ...
        from perfxpert.ai_analysis.llm_analyzer import AnalysisContext
        ...
        with patch("perfxpert.ai_analysis.api.LLMAnalyzer", return_value=mock_analyzer):
            analyze_source(
    ...
    def test_llm_analyzer_construction_without_api_key_does_not_raise(self):
        ...
        from perfxpert.ai_analysis.exceptions import LLMAuthenticationError
...
class TestLLMThinking:
    ...
    def test_llm_thinking_defaults_to_none(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("PERFXPERT_LLM_THINKING", None)
    ...
    def test_llm_thinking_env_var(self):
        """PERFXPERT_LLM_THINKING env var must set thinking_budget_tokens on construction."""
        import os
        from unittest.mock import patch
        from perfxpert.ai_analysis.llm_analyzer import LLMAnalyzer

        with patch.dict(os.environ, {"PERFXPERT_LLM_THINKING": "5000"}):
            analyzer = LLMAnalyzer(provider="anthropic")
