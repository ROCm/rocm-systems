...
def _init_schema(conn: sqlite3.Connection, uid: str) -> None:
    ...
    """)
    cur.execute("INSERT OR REPLACE INTO rocpd_metadata(key,value) VALUES (?,?)", ("active_uuid", uid))
    cur.execute("INSERT OR REPLACE INTO rocpd_metadata(key,value) VALUES (?,?)", ("schema_version", "rocpd_v3"))
    cur.execute(f"INSERT INTO rocpd_agent_{uid}(id,gfx_id,name,cu_count) VALUES (0,'gfx942','MI300X',304)")
    conn.commit()
...
def _write_fixture(
    ...
    for a in api:
        cur.execute(f"INSERT INTO rocpd_api_{uid}(name,duration_ns) VALUES (?,?)", (a["name"], a["duration_ns"]))
    for cnt in counters:
        cur.execute(
...
def case_vgpr_reduction_compute_bound():
    ...
    baseline = dict(
        kernels=[{"name": "matmul_kernel", "duration_ns": 1_000_000_000, "vgpr": 128, "lds": 0}],
        counters=[
    ...
    optimized = dict(
        kernels=[{"name": "matmul_kernel", "duration_ns": 700_000_000, "vgpr": 64, "lds": 0}],
        counters=[
...
def case_fast_math_compiler_flag():
    baseline = dict(kernels=[{"name": "transcendental_heavy", "duration_ns": 1_000_000_000, "vgpr": 64}])
    optimized = dict(kernels=[{"name": "transcendental_heavy", "duration_ns": 800_000_000, "vgpr": 64}])
    return baseline, optimized


def case_lds_tiling_matmul():
    baseline = dict(
        kernels=[{"name": "matmul_global", "duration_ns": 3_000_000_000, "vgpr": 96, "lds": 0}],
        counters=[{"counter_name": "FETCH_SIZE", "kernel_id": 1, "value": 5_000_000}],
    )
    optimized = dict(
        kernels=[{"name": "matmul_lds", "duration_ns": 900_000_000, "vgpr": 96, "lds": 16384}],
...
def case_kernel_fusion_small_launches():
    baseline = dict(
        kernels=[{"name": f"pw_op_{i}", "duration_ns": 8_000} for i in range(2000)],
        api=[{"name": "hipLaunchKernel", "duration_ns": 2_000} for _ in range(2000)],
    )
    optimized = dict(
        kernels=[{"name": "fused_pw", "duration_ns": 8_000_000}],
...
def case_warp_primitives_reduction():
    baseline = dict(
        kernels=[{"name": "lds_reduction", "duration_ns": 1_000_000_000, "lds": 4096}],
        counters=[{"counter_name": "SQ_LDS_BANK_CONFLICT", "kernel_id": 1, "value": 1_000_000}],
    )
    optimized = dict(
        kernels=[{"name": "shfl_reduction", "duration_ns": 600_000_000, "lds": 0}],
        counters=[{"counter_name": "SQ_LDS_BANK_CONFLICT", "kernel_id": 1, "value": 0}],
    )
    return baseline, optimized
...
def case_cache_blocking_kernel():
    baseline = dict(
        kernels=[{"name": "stencil_bad_block", "duration_ns": 1_500_000_000}],
        counters=[
    ...
    optimized = dict(
        kernels=[{"name": "stencil_tuned_block", "duration_ns": 900_000_000}],
        counters=[
