...
Run with:
    PERFXPERT_SYS=$(python3 -c "import site; print(site.getsitepackages()[-1])")
    PYTHONPATH="${PERFXPERT_SYS}" pytest --noconftest test_guide_filter_standalone.py -v
"""
...
class TestSelectTags:
    ...
    def _tags(self, **kwargs):
        from perfxpert.ai_analysis.llm_analyzer import AnalysisContext, _select_tags
...
class TestFilterGuide:
    ...
    def _filter(self, guide, tags):
        from perfxpert.ai_analysis.llm_analyzer import _filter_guide
    ...
    def _make_guide(self, *sections):
        ...
        for title, tag, content in sections:
            tag_line = f"<!-- perfxpert-context: {tag} -->\n" if tag else ""
...
class TestAnalyzeWithLLMContextParam:
    ...
    def test_analyze_source_with_llm_context_filters_guide(self):
        from perfxpert.ai_analysis.llm_analyzer import AnalysisContext, LLMAnalyzer
        from unittest.mock import patch
...
class TestPublicExport:
    ...
    def test_analysis_context_importable_from_package(self):
        from perfxpert.ai_analysis import AnalysisContext

        ctx = AnalysisContext(tier=2)
        assert ctx.tier == 2

    def test_analysis_context_in_all(self):
        import perfxpert.ai_analysis as pkg
...
class TestEndToEndWithRealGuide:
    ...
    def _build_prompt(self, **ctx_kwargs):
        from perfxpert.ai_analysis.llm_analyzer import LLMAnalyzer, AnalysisContext
        from unittest.mock import patch

        guide = (
            LLMAnalyzer.__module__
            and __import__(
                "perfxpert.ai_analysis.llm_analyzer", fromlist=["get_reference_guide_path"]
            )
...
class TestGuideIntegrity:
    ...
    def _sections(cls):
        ...
        import re
        from perfxpert.ai_analysis.llm_analyzer import get_reference_guide_path

        text = get_reference_guide_path().read_text()
        tag_re = re.compile(r"<!--\s*perfxpert-context:\s*([^-]+?)\s*-->")
        results = []
    ...
    def test_every_section_has_a_tag(self):
        """No ## section should be accidentally left without a perfxpert-context tag."""
        untagged = [
            title
        ...
        assert untagged == [], f"Sections missing perfxpert-context tag: {untagged}"
