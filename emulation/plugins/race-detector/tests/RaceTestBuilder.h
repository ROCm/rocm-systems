...
#include "race-detector/RaceDetector.h"
...
public:
  RaceTestBuilder(int numWaves, int vgprs, int sgprs, int waveSize = 64,
                  Dim3d wgId = Dim3d(0))
      : waveSize_(waveSize),
        defaultExec_(waveSize == 64 ? ~0ULL : (1ULL << waveSize) - 1) {
    detector_ = std::make_unique<RaceDetector>(
        numWaves, vgprs, sgprs, wgId,
        [this](RaceViolation v) { violations_.push_back(v); });
  ...
  void globalLoad(int wave, int vgprBase, int numRegs, uint64_t exec = 0) {
    ...
    waves_[wave]->registerEvent(pc_++, MemoryEventType::GLOBAL_TO_VGPR,
                                std::move(regs), exec);
  }

  /// Register a Direct-to-LDS global load (tracked by vmcnt).
  /// ldsAddrs is per-active-lane; padded to waveSize internally.
  void globalToLds(int wave, std::vector<uint32_t> ldsAddrs, int bytesPerLane,
                   uint64_t exec = 0) {
    if (!exec) {
      exec = defaultExec_;
    }
    ...
    waves_[wave]->registerLdsEvent(pc_++, MemoryEventType::GLOBAL_TO_LDS,
                                   /*registers=*/{}, exec, waveSize_, ldsAddrs,
                                   bytesPerLane);
  }
  ...
  void scalarLoad(int wave, int sgprBase, int numRegs) {
    ...
    waves_[wave]->registerEvent(pc_++, MemoryEventType::GLOBAL_TO_SGPR,
                                std::move(regs), defaultExec_);
  }
  ...
  void ldsRead(int wave, int lane, int addr, int bytes, int vgprDst,
               uint8_t byteMask = 0xF, uint64_t exec = 0) {
    if (!exec) {
      exec = defaultExec_;
    }
    ...
    waves_[wave]->registerLdsEvent(pc_++, MemoryEventType::LDS_TO_VGPR,
                                   std::move(regs), laneMask, waveSize_,
                                   ldsAddrs, bytes, byteMask);
  }
