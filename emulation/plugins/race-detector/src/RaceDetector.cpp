...
RaceDetector::RaceDetector(int nWaves, int vgprCount, int sgprCount,
                           Dim3d workgroupId,
...
EventId RaceDetector::allocateEventId(WaveId waveId, uint64_t pc,
...
std::string RaceDetector::decorateException(
    const RaceViolation &e, uint64_t wavePc, WaveRaceState *waveRaceState,
    int numSourceLines,
    std::function<std::string_view(int)> getSourceLine) const {

  auto printCodeBlock = [&](std::ostringstream &oss, int64_t startLine,
                            int64_t endLine,
