...
def gen_pk_binop_f32(
    ...
    for var, src in [("s0", s0), ("s1", s1)]:
        ...
        L.append(
            f"    uint32_t {var}_hi_w = ({src}.encoding_value_ >= 256 && {src}.encoding_value_ <= 511)"
        )
        L.append(
            f"        ? wf.cu().read_vgpr(wf.vgpr_alloc().base + static_cast<uint32_t>({src}.encoding_value_ - 256) + 1, lane)"
        )
...
def gen_pk_mov_b32(
    dst: list[str], src: list[str], opsel_exprs: tuple[str, str] = ("", "")
) -> str:
