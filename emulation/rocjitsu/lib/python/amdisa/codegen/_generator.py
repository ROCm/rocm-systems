...
class CodeGenerator:
    ...
    def _output_operand_is_also_source(self, inst: Instruction, opnd: Operand) -> bool:
        ...
        if opnd.name in ('vdata', 'sdata') and (
            (
                sem is not None
                and sem.semantic_class in ('buffer_atomic', 'image_atomic')
            )
    ...
    def _gen_global_load_lds(
        ...
        self._append_wait_counter_type(lines, sem, 'flat_load')
        lines.extend([
    ...
    def _atomic_op_enum(cls, sem: InstructionSemantics) -> str:
        ...
        if '_PK_ADD_BF16' in name:
            return 'amdgpu::AtomicOp::PK_ADD_BF16'
        if '_PK_ADD_F16' in name:
            return 'amdgpu::AtomicOp::PK_ADD_F16'
            ...
            if all_insts and not profile.is_alt_encoding(enc.enc_name):
                ...
                for inst in all_insts:
                    ...
                    for opnd in inst.operands:
                        ...
                        # Some ISA XMLs describe an atomic's vdata only as an
                        # output even though it always supplies the atomic
                        # payload. Keep the source dependency independent of
                        # whether the old memory value is returned.
                        _is_atomic_vdata_payload = (
                            inst_sem is not None
                            and inst_sem.semantic_class
                            in ('buffer_atomic', 'image_atomic')
                            and opnd.name == 'vdata'
                        )
                        ...
                        if (opnd.is_input or _is_atomic_vdata_payload) and not (
...
"""
    ...
    @staticmethod
    def _named_reg_ref(name: str) -> tuple[str, int] | None:
            ...
            for pattern in opnd_sel.name_patterns:
                if pattern.kind == OperandNamePattern.REG_RANGE:
                    ...
                    if pattern.is_register:
                        ...
                        special_ref = self._named_reg_ref(pattern.operand_name)
