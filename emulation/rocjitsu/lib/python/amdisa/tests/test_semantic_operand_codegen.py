...
def test_gfx12_flat_vaddr_helper_uses_typed_generated_null_selector():
    helper = CodeGenerator._emit_vflat_helpers()
    assert 'inst->saddr == OPR_SREG_NULL' in helper
    assert 'static_cast<uint32_t>(OPR_SREG_NULL)' not in helper
