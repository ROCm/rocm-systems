...
@pytest.mark.parametrize(
    ('name', 'expected'),
    [
        ('DS_PK_ADD_F16', 'amdgpu::AtomicOp::PK_ADD_F16'),
        ('FLAT_ATOMIC_PK_ADD_F16', 'amdgpu::AtomicOp::PK_ADD_F16'),
        ('BUFFER_ATOMIC_PK_ADD_BF16', 'amdgpu::AtomicOp::PK_ADD_BF16'),
        ('GLOBAL_ATOMIC_PK_ADD_BF16', 'amdgpu::AtomicOp::PK_ADD_BF16'),
...
def test_output_operand_read_facts_cover_liveness_sensitive_families():
    ...
    codegen.semantics = SimpleNamespace(
        instructions={
            ...
            'IMAGE_ATOMIC_ADD': InstructionSemantics(
                'IMAGE_ATOMIC_ADD', 'image_atomic'
            ),
    ...
    assert codegen._output_operand_is_also_source(
        Instruction('IMAGE_ATOMIC_ADD', 'ENC_MIMG', 0, [vdata]), vdata
    )
    assert codegen._output_operand_is_also_source(
...
def test_generated_rdna4_image_atomic_payload_is_a_hidden_readwrite_operand(
    rdna4_generated_root: Path,
):
    vimage = (rdna4_generated_root / 'vimage.cpp').read_text()
    constructor = _generated_constructor_body(vimage, 'ImageAtomicAddUintVimage')

    assert 'src_operands_[0] = &vdata;' in constructor
    assert 'dst_operands_[0] = &vdata;' in constructor
    assert 'omit_repeated_destination_sources_ = true;' in constructor
