...
def test_gfx1250_model_sources_do_not_include_execution_headers(
    ...
    model_sources = [
        path
        for path in gfx1250_generated_root.glob('*.cpp')
...
def test_fixed_wave32_split_k_sparse_i32_uses_signed_extractors(input_type, suffix):
    inst = Instruction(f'V_SWMMAC_I32_16X16X128_{input_type}', 'ENC_VOP3P', 0, [])
    body = _gen_mfma(inst, 'renamed_split_k', _fixed_wave32_split_k_profile())

    assert f'(inst_.neg & 0x1u) ? amdgpu::extract_i{suffix}(cu, base, loc)' in body
    assert f'(inst_.neg & 0x2u) ? amdgpu::extract_i{suffix}(cu, base, loc)' in body
    assert 'amdgpu::exec_swmmac_i32(cu, 16, 16, 128,' in body
    assert ', index_base, 32, index_key,' in body
    assert 'wf.wf_size()' not in body


def test_fixed_wave32_split_k_dense_i32_iu8_uses_specialized_executor():
    inst = Instruction('V_WMMA_I32_16X16X64_IU8', 'ENC_VOP3P', 0, [])
    body = _gen_mfma(inst, 'renamed_split_k', _fixed_wave32_split_k_profile())

    assert 'amdgpu::exec_wmma_i32_16x16x64_iu8(' in body
    assert '/*a_signed=*/(inst_.neg & 0x1u) != 0' in body
    assert '/*b_signed=*/(inst_.neg & 0x2u) != 0' in body
    assert 'amdgpu::exec_wmma_i32(cu,' not in body


def test_fixed_wave32_split_k_dense_i32_iu4_uses_generic_executor():
    inst = Instruction('V_WMMA_I32_16X16X32_IU4', 'ENC_VOP3P', 0, [])
    body = _gen_mfma(inst, 'renamed_split_k', _fixed_wave32_split_k_profile())

    assert '(inst_.neg & 0x1u) ? amdgpu::extract_i4(cu, base, loc)' in body
    assert '(inst_.neg & 0x2u) ? amdgpu::extract_i4(cu, base, loc)' in body
    assert 'amdgpu::exec_wmma_i32(cu, 16, 16, 32, 4, dst, src0_base,' in body


def test_fixed_wave32_split_k_dense_i32_i8_uses_generic_executor():
    inst = Instruction('V_WMMA_I32_16X16X32_I8', 'ENC_VOP3P', 0, [])
    body = _gen_mfma(inst, 'renamed_split_k', _fixed_wave32_split_k_profile())

    assert 'auto extract_a = amdgpu::extract_i8;' in body
    assert 'auto extract_b = amdgpu::extract_i8;' in body
    assert 'amdgpu::exec_wmma_i32(cu, 16, 16, 32, 8, dst, src0_base,' in body


def test_runtime_wave_split_k_i32_does_not_use_fixed_wave32_executor():
    inst = Instruction('V_WMMA_I32_16X16X64_IU8', 'ENC_VOP3P', 0, [])
    body = _gen_mfma(inst, 'rdna4', Rdna4Profile())

    assert 'amdgpu::exec_wmma_i32_16x16x64_iu8(' not in body
    assert 'amdgpu::exec_wmma_i32(cu, 16, 16, 64, 8, dst,' in body
...
def test_gfx1250_generated_vop3_auxiliary_masks_are_wave32(
    ...
    source = (gfx1250_generated_root / 'vop3_alu.cpp').read_text()
...
def test_gfx1250_generated_dpp_execution_needs_no_destination_cleanup(
    gfx1250_generated_root: Path,
):
    vop1 = (gfx1250_generated_root / 'vop1_exec.cpp').read_text()
    vop2 = (gfx1250_generated_root / 'vop2_exec.cpp').read_text()
    vop3 = (gfx1250_generated_root / 'vop3_exec_alu.cpp').read_text()
    vop3p = (gfx1250_generated_root / 'vop3p_exec.cpp').read_text()
    ...
    for body in bodies:
        assert body.count('inst_.src0 == amdgpu::SRC_DPP') == 1
        assert 'write_operand_storage' not in body
        assert 'write_vgpr_storage' not in body

    cvt_f64 = _generated_method_body(vop1, 'VCvtF64I32Vop1', 'VCvtF32I32Vop1')
    assert 'write_operand_storage64' not in cvt_f64
    assert cvt_f64.count('inst_.src0 == amdgpu::SRC_DPP') == 1


def test_generated_64bit_dpp_execution_needs_no_physical_cleanup(
    amdgpu_generated_root: Path,
):
    for arch in ('cdna2', 'cdna3', 'cdna4'):
        ...
        assert 'write_vgpr_storage64(vb + inst_.vdst, ln,' not in cvt_f64, arch
        assert cvt_f64.count('inst_.src0 == amdgpu::SRC_DPP') == 1, arch
...
def test_generated_vop_execution_has_no_instruction_storage_bypass(
    ...
    assert 'sdwa::write_lane<' in shared_execute
    assert 'sdwa::write_lane64<' in shared_execute


def test_generated_dpp_execution_uses_effective_lane_mask_for_dpp16(
    amdgpu_generated_root: Path,
):
    vop1_arches = (
        'cdna1',
    ...
    shared_execute = (amdgpu_generated_root / 'shared' / 'execute_shared.h').read_text()
    assert 'uint64_t exec = dpp::execution_lane_mask(inst, wf);' in shared_execute
...
def test_generated_flat_saddr_null_selector_follows_encoding(
    ...
    gfx1250_vflat = (amdgpu_generated_root / 'gfx1250' / 'vflat.cpp').read_text()
    gfx1250_vglobal = (amdgpu_generated_root / 'gfx1250' / 'vglobal.cpp').read_text()
