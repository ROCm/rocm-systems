...
private:
  ...
  /// @details Serializes the empty-queue recheck with monitor startup so a queue
  /// registered concurrently cannot be left without a polling thread.
  ...
  // keeps a concurrent registration from losing its monitor. The loop retains its
  // empty-queue self-exit as a backstop, and the next lifecycle operation reaps a
  // monitor that returned before it could be joined.
