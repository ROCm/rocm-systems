...
void CommandProcessor::configure_for_arch(rj_code_arch_t arch) {
  ...
  if (arch == ROCJITSU_CODE_ARCH_CDNA5)
    ...
    sdma_packet_dialect_ = SdmaPacketDialect::Oss7;
...
void CommandProcessor::init_wavefront_regs(ComputeUnitCore *cu, Wavefront *wf,
  ...
  if (pkt.private_segment_fixed_size > 0) {
    ...
    // A fanned-out entry's total_wgs is its XCD-local share, but local_wg_id is
    // the workgroup's grid-wide ordinal (the shard walk is strided). Scratch is
    // likewise indexed and sized over the whole grid.
    if (local_wg_id >= pkt.grid_total_wgs())
    ...
    if (scratch_slots == 0 || scratch_slots > std::numeric_limits<uint64_t>::max() / per_wave_size)
      throw std::runtime_error(
    ...
    const bool should_provision = scratch_allocator_ && memory_ &&
  ...
  while (rpos < wpos) {
    ...
    case sdma::OP_COPY: {
      if (sdma_caps.wait_signal_copy && sub_op == sdma::SUBOP_COPY_LINEAR &&
        ...
        // Completion metadata is part of the packet transaction. An address
        // that is not mapped yet must leave both the copy and signal pending;
        // otherwise a retry would copy twice. A permanently faulted address
        // halts the queue before the destination can falsely appear complete.
        if (signal_addr > 0x1000 && signal_update != SignalUpdate::None) {
          uint64_t signal_value = 0;
          const amdgpu::CopyOutcome signal_outcome = memory_->atomic_load(
              signal_addr, sizeof(signal_value), signal_value, queue.process_id);
          if (signal_outcome == amdgpu::CopyOutcome::Unavailable)
            return stop_and_retry_current_packet();
          if (signal_outcome == amdgpu::CopyOutcome::Faulted) {
            fault_sdma_queue(queue);
            return stop_current_packet();
          }
        }
        ...
        auto resolve_copy_address = [&](uint64_t packet_va,
                                        bool indirect) -> std::optional<uint64_t> {
          if (!indirect)
            return packet_va;
          if (!resolve(packet_va, sizeof(uint64_t)))
            return std::nullopt;
          return read_gpu_u64(packet_va, queue.process_id);
        };
        ...
        const auto resolved_src_va = resolve_copy_address(src_va, indirect_src);
        const auto resolved_dst_va = resolve_copy_address(dst_va, indirect_dst);
        if (!resolved_src_va || !resolved_dst_va) {
          return stop_and_retry_current_packet();
        }
        src_va = *resolved_src_va;
        dst_va = *resolved_dst_va;
        ...
        if (signal_addr > 0x1000 && signal_update != SignalUpdate::None &&
            copy_outcome == amdgpu::CopyOutcome::Complete) {
          const amdgpu::AccessOutcome signal_outcome =
              signal_update == SignalUpdate::Add
                  ? memory_->atomic_fetch_add64(signal_addr, signal_data, queue.process_id)
                  : memory_->atomic_fetch_sub64(signal_addr, static_cast<int64_t>(signal_data),
                                                queue.process_id);
          if (signal_outcome == amdgpu::AccessOutcome::Faulted) {
            rpos += packet_dwords;
            fault_sdma_queue(queue);
            return stop_current_packet();
          }
