...
public:
  ...
  ///
  /// @details The shipping driver enables 32- and 64-bit requests together and
...
private:
  ...
  struct DoorbellRetry {
    DoorbellNotification notification;
    IpBlockModel *model = nullptr;
    simdojo::Tick backoff = 1;
    simdojo::Tick ready_tick = 0;
  };
  ...
  [[nodiscard]] bool begin_doorbell_observer(const DoorbellNotification &notification);
  void finish_doorbell_observer();
  ...
  struct OwnedBlockModel {
    ...
    /// benefit.
    std::unique_ptr<IpBlockModel> model;
  ...
  uint64_t active_doorbell_observers_ = 0;
