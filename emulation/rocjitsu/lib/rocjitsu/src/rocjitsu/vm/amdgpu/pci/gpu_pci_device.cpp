...
bool GpuPciDevice::shutdown_frontend() {
  ...
  for (const auto &[model, required] : models_) {
    (void)required;
    if (auto *hub = dynamic_cast<MemoryHubBlockModel *>(model.get()); hub != nullptr)
      hub->attach_soc(nullptr);
  }
  if (mes_ != nullptr) {
    mes_->attach_interrupt_sink({});
    mes_->attach_sdma(nullptr);
    mes_->attach_soc(nullptr);
  }
  if (sdma_ != nullptr)
...
void GpuPciDevice::build_block_models() {
  ...
  MesBlockModel *mes_model = nullptr;
  SdmaBlockModel *sdma_model = nullptr;
  ...
  for (const BlockModelBinding &binding : kBlockModels) {
    ...
    if (auto *mes = dynamic_cast<MesBlockModel *>(model.get()); mes != nullptr) {
      mes->attach_soc(soc_);
      mes->attach_interrupt_sink(interrupt_subscription_.sink());
      mes_model = mes;
      mes_ = mes;
    }
    if (auto *hub = dynamic_cast<MemoryHubBlockModel *>(model.get()); hub != nullptr) {
    ...
    models_.push_back({std::move(model), binding.required});
  }

  if (mes_model != nullptr) {
    mes_model->attach_sdma(sdma_model);
  }
...
bool GpuPciDevice::teardown_frontend_queues() {
  // MES owns every QueueService registration created through this frontend.
  // Destroy those first so QueueService drains admitted backend callbacks and
  // releases their GpuVm references. SDMA then discards register-backed and
  // fallback execution snapshots before the PCI transport can be revoked.
  const bool complete = mes_ == nullptr || mes_->teardown_queues();
  if (complete && sdma_ != nullptr)
...
int64_t GpuPciDevice::access_registers(std::span<std::byte> buf, uint64_t offset, bool write) {
  ...
  if (write) {
    ...
    if (modelled && !registers_.read_only(offset)) {
      ...
      for (const auto &[model, required] : models_) {
        (void)required;
        model->observe_register_write(offset, *this);
      }
    }
    return 4;
  }
...
void GpuPciDevice::enqueue_doorbell_notification(uint64_t byte_offset, uint64_t value,
  ...
  bool wake = false;
  {
    ...
    if (!doorbell_admission_open_)
      return;
    doorbell_inbox_.push_back({.session = session,
                               .generation = session != nullptr ? session->generation() : 0,
...
bool GpuPciDevice::begin_doorbell_observer(const DoorbellNotification &notification) {
  const std::lock_guard lock(doorbell_inbox_mutex_);
  if (!doorbell_admission_open_ || notification.reset_epoch != doorbell_reset_epoch_)
    return false;
  ++active_doorbell_observers_;
  return true;
}

void GpuPciDevice::finish_doorbell_observer() {
  const std::lock_guard lock(doorbell_inbox_mutex_);
  if (active_doorbell_observers_ == 0)
    return;
  --active_doorbell_observers_;
  if (active_doorbell_observers_ == 0)
...
bool GpuPciDevice::same_doorbell_retry_target(const DoorbellRetry &retry,
  ...
  return retry.model == model && retry.notification.generation == notification.generation &&
         retry.notification.reset_epoch == notification.reset_epoch &&
         retry.notification.byte_offset == notification.byte_offset;
}
...
void GpuPciDevice::enqueue_doorbell_retry(const DoorbellNotification &notification,
  ...
  bool wake = false;
  {
    ...
    if (!doorbell_admission_open_ || notification.reset_epoch != doorbell_reset_epoch_)
      return;
    const auto duplicate = std::ranges::find_if(doorbell_retry_inbox_, [&](const auto &retry) {
    ...
    if (duplicate != doorbell_retry_inbox_.end()) {
      duplicate->notification.value = std::max(duplicate->notification.value, notification.value);
      duplicate->notification.width = notification.width;
      duplicate->backoff = std::min(duplicate->backoff, bounded_backoff);
      duplicate->ready_tick = std::min(duplicate->ready_tick, ready_tick);
      candidate_tick = duplicate->ready_tick;
    } else {
      doorbell_retry_inbox_.push_back({.notification = notification,
                                       .model = model,
...
void GpuPciDevice::drain_doorbell_inbox(simdojo::Tick now) {
  ...
  for (const DoorbellNotification &notification : notifications) {
    const std::shared_ptr<simdojo::PciTransportSession> session = notification.session.lock();
    if (session == nullptr || session->generation() != notification.generation)
      continue;
    simdojo::PciTransportSession::OperationLease lease = session->acquire();
    if (!lease || transport_session() != session || !begin_doorbell_observer(notification))
      continue;

    try {
      for (const auto &[model, required] : models_) {
        (void)required;
        const DoorbellDisposition disposition = model->observe_doorbell_write(
            notification.byte_offset, notification.value, notification.width, *this);
        if (disposition == DoorbellDisposition::Retry) {
          enqueue_doorbell_retry(notification, model.get(), now, 1);
        } else if (disposition != DoorbellDisposition::Ignored) {
          discard_doorbell_retry(notification, model.get());
        }
      }
...
void GpuPciDevice::drain_doorbell_retry_inbox(simdojo::Tick now, bool ignore_ready) {
  ...
  for (DoorbellRetry &retry : retries) {
    if (!ignore_ready && retry.ready_tick > now) {
      deferred.push_back(std::move(retry));
      continue;
    }

    const DoorbellNotification &notification = retry.notification;
    const std::shared_ptr<simdojo::PciTransportSession> session = notification.session.lock();
    if (session == nullptr || session->generation() != notification.generation)
      continue;
    simdojo::PciTransportSession::OperationLease lease = session->acquire();
    ...
    try {
      disposition = retry.model->observe_doorbell_write(
          notification.byte_offset, notification.value, notification.width, *this);
    } catch (const std::exception &error) {
      util::Logger::warn(
    ...
    if (disposition == DoorbellDisposition::Retry) {
      if (now == simdojo::TICK_MAX)
        continue;
      retry.backoff = std::min(retry.backoff * 2, kMaximumDoorbellRetryBackoff);
      retry.ready_tick = add_ticks_saturated(now, retry.backoff);
      deferred.push_back(std::move(retry));
    }
  }
  ...
  bool wake = false;
  {
    ...
    for (DoorbellRetry &retry : deferred) {
      if (retry.notification.reset_epoch != doorbell_reset_epoch_)
        continue;
      const auto duplicate =
          std::ranges::find_if(doorbell_retry_inbox_, [&](const auto &candidate) {
            return same_doorbell_retry_target(candidate, retry.notification, retry.model);
          });
      if (duplicate == doorbell_retry_inbox_.end()) {
        next_tick = std::min(next_tick, retry.ready_tick);
        doorbell_retry_inbox_.push_back(std::move(retry));
        continue;
      }
      duplicate->notification.value =
          std::max(duplicate->notification.value, retry.notification.value);
      duplicate->notification.width = retry.notification.width;
      duplicate->backoff = std::min(duplicate->backoff, retry.backoff);
      duplicate->ready_tick = std::min(duplicate->ready_tick, retry.ready_tick);
      next_tick = std::min(next_tick, duplicate->ready_tick);
    }
...
void GpuPciDevice::close_doorbell_admission_for_reset() {
  ...
  doorbell_retry_wake_tick_.reset();
  doorbell_wake_pending_ = false;
  doorbell_idle_.wait(lock, [this]() { return active_doorbell_observers_ == 0; });
}
...
bool GpuPciDevice::write_register(uint64_t byte_offset, uint32_t value) {
  ...
  if (registers_.modelled(byte_offset) && !registers_.read_only(byte_offset)) {
    ...
    for (const auto &[model, required] : models_) {
      (void)required;
      model->observe_register_write(byte_offset, *this);
    }
  }
  return true;
}
