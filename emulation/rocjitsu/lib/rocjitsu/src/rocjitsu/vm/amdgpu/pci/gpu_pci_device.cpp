...
bool GpuPciDevice::enqueue_interrupt(const InterruptEntry &entry, InterruptRequestKind kind) {
  ...
  }
  {
    ...
    if (!sdma_effect_admission_open_)
      return false;
    interrupt_inbox_.push_back({.session = session,
...
bool GpuPciDevice::request_interrupt(const InterruptEntry &entry) {
  return enqueue_interrupt(entry, InterruptRequestKind::Diagnostic);
}
...
void GpuPciDevice::enqueue_cp_interrupt(uint32_t process_id, uint32_t event_id) {
  ...
  (void)enqueue_interrupt({.client_id = kGraphicsInterruptClient,
                           .source_id = kCpEndOfPipeSource,
                           .vmid = kFirstComputeVmid,
                           .pasid = static_cast<uint16_t>(process_id),
                           .node_id = kFirstXccInterruptNode,
                           .data = {event_id}},
                          InterruptRequestKind::Completion);
}

void GpuPciDevice::drain_interrupts() {
  std::deque<InterruptRequest> requests;
  {
    const std::lock_guard lock(sdma_effect_mutex_);
    requests.swap(interrupt_inbox_);
  }
  for (const InterruptRequest &request : requests) {
    ...
    if (!lease || transport_session() != session || !frontend)
      continue;
    if (request.kind == InterruptRequestKind::Completion) {
      const std::lock_guard lock(sdma_effect_mutex_);
      last_cp_interrupt_thread_ = std::this_thread::get_id();
    }
    const bool delivered = deliver_interrupt(request.entry);
    if (request.kind == InterruptRequestKind::Completion) {
      if (!delivered) {
        util::Logger::warn(std::format("{}: could not deliver CP event {} for PASID {}", name(),
                                       request.entry.data[0], request.entry.pasid));
      }
    } else if (delivered) {
      util::Logger::warn(interrupt_ring().raises_messages
                             ? "vfu: delivered an interrupt on request"
                             : "vfu: put an entry in the ring on request, with messages switched "
                               "off by the driver");
    } else {
      util::Logger::warn(
          std::format("vfu: cannot deliver an interrupt: {}", describe(interrupt_ring())));
    }
  }
}
...
bool GpuPciDevice::synchronize_sdma_queue_for_test(uint32_t engine) {
  if (engine >= 2 || soc_ == nullptr)
    return false;
  return soc_->sdma_queue_scheduler().synchronize_queue_for_test(
      0, 0xffff0000u | engine, [this]() { drain_sdma_pci_effects(); });
}
...
void GpuPciDevice::close_doorbell_admission_for_reset() {
  ...
  doorbell_retry_wake_tick_.reset();
  doorbell_wake_pending_ = false;
  // SDMA cancellation takes sdma_effect_mutex_. Drop the doorbell lock first:
  // admitted SDMA work can finish a frontend lease while cancellation closes
  // its mailbox, and releasing that lease takes doorbell_inbox_mutex_.
  lock.unlock();
