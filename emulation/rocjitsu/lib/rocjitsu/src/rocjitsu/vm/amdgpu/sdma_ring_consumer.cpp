...
bool valid_sdma_ring_config(const SdmaRingConfig &config) {
  return config.address_space && config.ring_bytes != 0 &&
         (config.ring_base % sizeof(uint32_t)) == 0 &&
         (config.ring_bytes % sizeof(uint32_t)) == 0 && config.read_pointer_address != 0 &&
         (config.read_pointer_address % sizeof(uint64_t)) == 0 &&
         config.ring_base <= std::numeric_limits<uint64_t>::max() - (config.ring_bytes - 1) &&
         (!config.initial_cursor || (*config.initial_cursor % sizeof(uint32_t)) == 0);
}
...
SdmaRingStatus SdmaRingConsumer::service(uint64_t producer, std::size_t packet_budget) {
  ...
  if (!valid_sdma_ring_config(config_))
    return latch(SdmaRingStatus::Malformed);
  if (!access_) {
    access_ = gpu_vm_->snapshot_pinned(config_.address_space);
    if (!access_)
  ...
  for (;;) {
    ...
    if (packet_result.retirement == PacketRetirement::Retire) {
      ...
      cursor_journal_.retire(retired_cursor, retired_cursor, *access_);
...
void SdmaRingConsumer::latch_callback_failure() noexcept {
  terminal_ = SdmaRingStatus::Faulted;
  clear_in_flight();
}
