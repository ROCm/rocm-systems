...
public:
  ...
  std::optional<WorkgroupPlacement> allocate_workgroup(const DispatchEntry &entry,
    ...
    for (size_t attempt = 0; attempt < wgps_.size(); ++attempt) {
      ...
      resident_wgp_workgroups_[wg_key(entry.dispatch_id, global_wg_id)] = wgp_index;
  ...
  bool release_wgp_workgroup(uint32_t dispatch_id, uint32_t global_wg_id) {
    ...
    if (resident == resident_wgp_workgroups_.end())
      return false;
    const size_t wgp_index = resident->second;
  ...
  uint32_t max_cu_lds_bytes() const {
    uint64_t max_bytes = 0;
    for (auto *cu : cus_)
      max_bytes = std::max(max_bytes, static_cast<uint64_t>(cu->config().lds_size_kb) * 1024u);
...
private:
  ...
  std::unordered_map<uint64_t, size_t> resident_wgp_workgroups_;
