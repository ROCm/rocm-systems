...
public:
  ...
  /// @brief Return whether a GPU VA has an actual VMID page-table entry.
  /// @details Unlike resolve_host_ptr(), this does not treat an arbitrary
  /// userspace-looking address as an identity-mapped pointer. Resource
  /// provisioning must use this predicate so a reserved but inaccessible host
  /// range cannot masquerade as mapped GPU storage.
