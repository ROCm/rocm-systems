...
GpuGenerationRegistry::validate(std::span<const GpuGenerationDescriptor> descriptors) {
  for (std::size_t i = 0; i < descriptors.size(); ++i) {
    const GpuGenerationDescriptor &descriptor = descriptors[i];
    if (descriptor.id.empty()) {
      return std::format("generation {} has no name", i);
    }
    ...
    for (std::size_t j = i + 1; j < descriptors.size(); ++j) {
      const GpuGenerationDescriptor &other = descriptors[j];
...
const GpuGenerationDescriptor *GpuGenerationRegistry::find(std::string_view id) const {
  ...
  const auto found = std::ranges::find(descriptors_, id, &GpuGenerationDescriptor::id);
