...
void ComputeUnitCore::issue_instruction(Wavefront *active, const FetchedInstruction &words) {
  ...
  DecodeResult decoded = decoder_->decode(words.data(), decode_error.emitter());
  if (decoded.failed()) {
...

  {
    ...
    if (mn == "s_setpc_b64" || mn == "s_swappc_b64" || mn == "s_set_pc_i64" ||
        mn == "s_swap_pc_i64") {
      const Operand *target_operand = inst->src_operand(0);
      assert(target_operand && "indirect branch must have a target operand");
...
bool ComputeUnitCore::step() {
  ...
  for (auto &wf : wfs_) {
    ...
    if (!entry.valid || entry.vmid != vmid || entry.pc != pc) {
      static_assert(sizeof(entry.words) == InstructionCache::kFetchBytes,
                    "the I$ fetch width must match the issue window");
      if (debug_active()) {
        // A debugger writes breakpoints straight into code memory with none of
        // the maintenance that invalidates the I$, so bypass it while attached.
        for (size_t i = 0; i < entry.words.size(); ++i)
          entry.words[i] = memory_->fetch32(pc + i * sizeof(entry.words[i]), vmid);
      } else {
        // Invalidate lines that may have been patched during an attach/detach
        // transition before sharing this fetch among same-PC waves.
        sync_inst_cache_debug_epoch();
        inst_cache_.fetch(*memory_, pc, vmid, reinterpret_cast<uint8_t *>(entry.words.data()));
      }
