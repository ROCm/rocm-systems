...
void ComputeUnitCore::route_memory_inst(Instruction *inst, Wavefront &wf) {
  ...
  if (inst->data()->tag() == GLOBAL_MEM && shared_aperture_base_ != 0) {
    ...
    if (probe >= shared_aperture_base_ && probe <= shared_aperture_limit_) {
      ...
      local_mem_pipeline_.issue_concrete(owned_inst.release(), wf);
      return;
    }
  }
  ...
  switch (route_tag) {
  case SCALAR_MEM:
    scalar_mem_pipeline_.issue_concrete(owned_inst.release(), wf);
    break;
  case LOCAL_MEM:
    local_mem_pipeline_.issue_concrete(owned_inst.release(), wf);
    break;
  case GLOBAL_MEM:
    global_mem_pipeline_.issue_concrete(owned_inst.release(), wf);
    break;
  default:
    break;
  }
