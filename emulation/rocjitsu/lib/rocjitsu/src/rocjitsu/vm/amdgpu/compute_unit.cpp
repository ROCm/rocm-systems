...
Wavefront *ComputeUnitCore::dispatch_wf(uint32_t wg_id, uint64_t pc, uint32_t num_sgprs,
  ...
  return dispatch_wf(wg_id, pc, num_sgprs,
                     ...
                     dispatch_id, scratch_wave_limit_per_se);
}
...
Wavefront *ComputeUnitCore::dispatch_wf(uint32_t wg_id, uint64_t pc, uint32_t num_sgprs,
  ...
  for (size_t word = 0; word < free_wf_slot_bits_.size(); ++word) {
    const uint64_t base = static_cast<uint64_t>(scratch_scoreboard_base_) + word * 64u;
    if (base >= scratch_wave_limit_per_se)
      break;
    const uint64_t eligible = std::min<uint64_t>(64u, scratch_wave_limit_per_se - base);
    const uint64_t mask = eligible == 64u ? ~uint64_t{0} : (uint64_t{1} << eligible) - 1u;
    const uint64_t free = free_wf_slot_bits_[word] & mask;
    if (free != 0) {
      slot = word * 64u + std::countr_zero(free);
      break;
    }
  }
...
bool ComputeUnitCore::can_accept_workgroup(uint32_t num_wfs, uint32_t num_sgprs, uint32_t num_vgprs,
  ...
  for (size_t word = 0; word < free_wf_slot_bits_.size(); ++word) {
    ...
    const uint64_t eligible = std::min<uint64_t>(64u, scratch_wave_limit_per_se - base);
    const uint64_t mask = eligible == 64u ? ~uint64_t{0} : (uint64_t{1} << eligible) - 1u;
    free_slots += std::popcount(free_wf_slot_bits_[word] & mask);
  }
