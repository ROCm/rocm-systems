    ...
    if (color_enabled_ && color_metadata_)
      for (uint32_t y = 0; y < height_; y += 8)
        for (uint32_t x = 0; x < width_; x += 8)
          materialize_gfx11_dcc(memory, process_id, color_base_, *color_metadata_, x, y, width_,
                                height_, 4, swizzle_, attrib3 & (1u << 30));
    if ((depth_control_ & 2) && depth_metadata_) {
      ...
      for (uint32_t y = 0; y < depth_height_; y += 8)
        for (uint32_t x = 0; x < depth_width_; x += 8)
          materialize_gfx11_htile(memory, process_id, depth_base_, *depth_metadata_, x, y,
                                  depth_width_, depth_height_, depth_bytes_, depth_swizzle_, bits);
    }
