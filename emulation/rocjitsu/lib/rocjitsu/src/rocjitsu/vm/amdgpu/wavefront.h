...
public:
  ...
  // state is later serialized into the queue's context-save area in the exact
  // layout rocm-dbgapi parses.
  ...
  /// @brief Whether this wave should execute exactly one instruction on resume
  /// and then stop again (rocm-dbgapi single-step, MODE.DEBUG_EN=1).
  ...
  /// the saved PC points just after the trap — rocm-dbgapi applies a -4 adjust
  /// to report the breakpoint address.
