...
public:
  ...
  /// @brief Raw gfx12.5-only exception flags that have no legacy TRAPSTS slot.
  uint32_t gfx1250_excp_flag_user_extra_raw() const { return gfx1250_excp_flag_user_extra_raw_; }

  /// @brief Set gfx12.5-only exception flags that have no legacy TRAPSTS slot.
  void set_gfx1250_excp_flag_user_extra_raw(uint32_t val) {
    gfx1250_excp_flag_user_extra_raw_ = val;
  }

  /// @brief Whether this wave uses gfx12.5's dedicated TRAP_CTRL register.
  bool uses_separate_trap_ctrl() const;

  /// @brief Raw gfx12.5 trap enables, architecturally separate from MODE.
  uint32_t gfx1250_trap_ctrl_raw() const { return gfx1250_trap_ctrl_raw_; }

  /// @brief Set the raw gfx12.5 trap enables without changing MODE.VGPR_MSB.
  void set_gfx1250_trap_ctrl_raw(uint32_t val) { gfx1250_trap_ctrl_raw_ = val; }
  ...
  struct DebugStopState {
    uint32_t trapsts = 0;
    uint32_t mode_raw = 0;
    uint32_t gfx1250_trap_ctrl_raw = 0;
    uint32_t trap_id = 0;
  ...
  DebugStopState debug_stop_state() const {
    return DebugStopState{trapsts_,      mode_raw_,    gfx1250_trap_ctrl_raw_,  trap_id_,
  ...
  void restore_debug_stop_state(const DebugStopState &saved) {
    ...
    gfx1250_trap_ctrl_raw_ = saved.gfx1250_trap_ctrl_raw;
  ...
  void reset() {
    ...
    gfx1250_excp_flag_user_extra_raw_ = 0;
    gfx1250_trap_ctrl_raw_ = 0;
...
private:
  ...
  uint32_t gfx1250_excp_flag_user_extra_raw_ = 0;
  uint32_t gfx1250_trap_ctrl_raw_ = 0;
