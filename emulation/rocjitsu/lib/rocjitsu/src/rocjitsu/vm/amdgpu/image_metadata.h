...
inline void read_image_bytes(GpuMemory &memory, uint32_t process_id, uint64_t address,
                             std::span<uint8_t> bytes) {
  if (memory.read_block_exact(address, bytes, process_id) != AccessOutcome::Complete)
    throw std::runtime_error("image read failed");
}

inline void write_image_bytes(GpuMemory &memory, uint32_t process_id, uint64_t address,
                              std::span<const uint8_t> bytes) {
  if (memory.write_block(address, bytes, process_id) != AccessOutcome::Complete)
...
inline void materialize_gfx11_dcc(GpuMemory &memory, uint32_t process_id, uint64_t base,
                                  uint64_t metadata, uint32_t x, uint32_t y, uint32_t width,
  ...
  uint8_t key;
  read_image_bytes(memory, process_id, *address, {&key, 1});
  if (key == 0xff)
    return;
  ...
  if (key == 1) {
    ...
    read_image_bytes(memory, process_id, *clear, {value.data(), bytes});
  } else if (key == 2) {
    value.fill(0xff);
  } else if (key == 4 || key == 6) {
  ...
  for (uint32_t py = y; py < std::min(y + bh, height); ++py)
    for (uint32_t px = x; px < std::min(x + bw, width); ++px)
      write_image_bytes(memory, process_id,
                        *gfx11_image_address(base, px, py, width, bytes, swizzle),
                        {value.data(), bytes});
  key = 0xff;
  write_image_bytes(memory, process_id, *address, {&key, 1});
}

/// HTILE ZMask zero references DB_DEPTH_CLEAR; ZMask fifteen is uncompressed.
inline void materialize_gfx11_htile(GpuMemory &memory, uint32_t process_id, uint64_t base,
                                    uint64_t metadata, uint32_t x, uint32_t y, uint32_t width,
                                    uint32_t height, uint32_t bytes, uint32_t swizzle,
  ...
  uint32_t key;
  read_image_bytes(memory, process_id, *address, {reinterpret_cast<uint8_t *>(&key), 4});
  if ((key & 15) == 15)
    return;
  if (key & 15)
  ...
  key = 0xfffc000f;
  write_image_bytes(memory, process_id, *address, {reinterpret_cast<const uint8_t *>(&key), 4});
}
