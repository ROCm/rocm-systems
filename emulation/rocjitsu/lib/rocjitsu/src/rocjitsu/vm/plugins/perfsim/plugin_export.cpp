...
ROCJITSU_DEFINE_PLUGIN(
    ...
    R"({"library_path":{"type":"string","description":"Absolute path to the separately supplied Perfsim backend shared object, such as libgpucsim_ffm_plugin.so"},"dispatch_name":{"type":"string","description":"Optional exact kernel name identified by dispatch ID after replay; every supported dispatch is still forwarded","default":""},"max_staged_bytes":{"type":"number","description":"Maximum charged bytes for staged event records","default":268435456},"max_observed_wgps":{"type":"number","description":"Optional diagnostic cap on distinct workgroups staged per dispatch","optional":true}})")
