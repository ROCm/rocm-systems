...
/// rocjitsu SimulatedKfd's doorbell dispatch path, which triggers the
...
/// backed by a SealedMemfd so the guest can mmap it and write doorbells
/// without socket round-trips on supported platforms.
///
/// VfuServer registers BAR2 with a lambda trampoline (same pattern as BAR5)
/// that calls handle_access(); this class owns no libvfio-user handles.
class Bar2Doorbell {
public:
  ...
  /// @brief Returns 0 if the doorbell memfd was initialized successfully.
  int valid() const { return memfd_.valid() ? 0 : -1; }

  /// @brief Handle a BAR2 access (called from the VfuServer lambda trampoline).
  ...
  /// @brief BAR size in bytes.
  uint64_t size() const { return static_cast<uint64_t>(memfd_.size()); }
