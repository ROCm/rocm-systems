...
public:
  ...
  const std::string &disassemble() const {
    if (disassembly_.empty()) {
      ...
      bool first = true;
      // TODO: Include explicit field-less operands (and/or implicit ones too).
      for (uint8_t i = 0; i < num_dst_; ++i) {
        if (dst_operands_[i]->is_field_less())
          continue;
        disassembly_ += (first ? " " : ", ");
      ...
      for (uint8_t i = 0; i < num_src_; ++i) {
        if (src_operands_[i]->size_bits() == 0 || src_operands_[i]->is_field_less())
          continue;
        disassembly_ += (first ? " " : ", ");
