...
void VCmpLtF16Vop3::execute_impl(amdgpu::Wavefront &wf) {
  ...
  auto &inst = *this;
  ROCJITSU_TRY_SIMD_VOPC_VOP3_TRUE16_FP16_RESULT(commit_result, [](auto a, auto b) { return a < b; });
...
void VCmpLgF16Vop3::execute_impl(amdgpu::Wavefront &wf) {
  ...
  auto &inst = *this;
  ROCJITSU_TRY_SIMD_VOPC_VOP3_TRUE16_FP16_RESULT(commit_result, [](auto a, auto b) { return (a < b) || (a > b); });
  uint64_t exec = wf.exec();
  [[maybe_unused]] uint32_t opsel = amdgpu::vop3_opsel(inst_);
  uint64_t vcc = 0;
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
...
void VCmpOF16Vop3::execute_impl(amdgpu::Wavefront &wf) {
  ...
  auto &inst = *this;
  ROCJITSU_TRY_SIMD_VOPC_VOP3_TRUE16_FP16_RESULT(commit_result, [](auto a, auto b) { return !util::stdx::isnan(a) && !util::stdx::isnan(b); });
  uint64_t exec = wf.exec();
  [[maybe_unused]] uint32_t opsel = amdgpu::vop3_opsel(inst_);
  uint64_t vcc = 0;
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
