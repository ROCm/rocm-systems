...
inline void execute_v_add_co_ci_u32_vop3([[maybe_unused]] Inst &inst,
                                         [[maybe_unused]] Wavefront &wf) {
  ROCJITSU_TRY_SIMD_VOP3_CARRY_SRC2([](auto a, auto b, auto cin) {
    auto t1 = a + b;
    auto c1 = t1 < a;
    auto t2 = t1 + cin;
    auto c2 = t2 < t1;
...
inline void execute_v_add_co_u32_vop3([[maybe_unused]] Inst &inst, [[maybe_unused]] Wavefront &wf) {
  ROCJITSU_TRY_SIMD_VOP3_CARRY([](auto a, auto b, auto) {
    auto s = a + b;
    return make_simd_carry(s, s < a);
  });
  uint64_t exec = wf.exec();
...
inline void execute_v_med3_i16_vop3([[maybe_unused]] Inst &inst, [[maybe_unused]] Wavefront &wf) {
  ROCJITSU_TRY_SIMD_VOP3_TERNARY_INT(uint32_t, [](auto a, auto b, auto c) {
    ...
    auto sb = (util::stdx::static_simd_cast<I>(b & 0xFFFFu) << 16) >> 16;
    auto sc = (util::stdx::static_simd_cast<I>(c & 0xFFFFu) << 16) >> 16;
    auto r = util::stdx::max(util::stdx::min(util::stdx::max(sa, sb), sc), util::stdx::min(sa, sb));
    return util::stdx::static_simd_cast<util::native<uint32_t>>(r) & 0xFFFFu;
  });
  uint64_t exec = wf.exec();
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
...
inline void execute_v_med3_u16_vop3([[maybe_unused]] Inst &inst, [[maybe_unused]] Wavefront &wf) {
  ROCJITSU_TRY_SIMD_VOP3_TERNARY_INT(uint32_t, [](auto a, auto b, auto c) {
    ...
    auto ub = b & 0xFFFFu;
    auto uc = c & 0xFFFFu;
    return util::stdx::max(util::stdx::min(util::stdx::max(ua, ub), uc), util::stdx::min(ua, ub));
  });
  uint64_t exec = wf.exec();
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
...
inline void execute_v_mov_b16_vop3([[maybe_unused]] Inst &inst, [[maybe_unused]] Wavefront &wf) {
  ROCJITSU_TRY_SIMD_VOP3_MOV_B16();
  uint64_t exec = wf.exec();
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
      continue;
...
inline void execute_v_sub_co_u32_vop3([[maybe_unused]] Inst &inst, [[maybe_unused]] Wavefront &wf) {
  ROCJITSU_TRY_SIMD_VOP3_CARRY([](auto a, auto b, auto) { return make_simd_carry(a - b, a < b); });
  uint64_t exec = wf.exec();
  uint64_t vcc = wf.vcc();
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
...
inline void execute_v_subrev_co_u32_vop3([[maybe_unused]] Inst &inst,
                                         [[maybe_unused]] Wavefront &wf) {
  ROCJITSU_TRY_SIMD_VOP3_CARRY([](auto a, auto b, auto) { return make_simd_carry(b - a, b < a); });
  uint64_t exec = wf.exec();
  uint64_t vcc = wf.vcc();
  for (uint32_t lane = 0; lane < wf.wf_size(); ++lane) {
    if (!(exec & (1ULL << lane)))
