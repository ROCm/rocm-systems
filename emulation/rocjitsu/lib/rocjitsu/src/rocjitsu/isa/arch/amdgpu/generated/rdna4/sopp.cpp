...
namespace detail {
DecodeResult decodeSWaitSamplecntSopp(const MachineInst *opcode,
                                      const DecodeErrorEmitter &emit_error) {
  Result validation = Sopp::validate_encoding(
      "s_wait_samplecnt", reinterpret_cast<const Sopp::OpEncoding *>(opcode), emit_error);
  if (validation.failed()) [[unlikely]]
    return Result::failure();
...
namespace detail {
DecodeResult decodeSWaitBvhcntSopp(const MachineInst *opcode,
                                   const DecodeErrorEmitter &emit_error) {
  Result validation = Sopp::validate_encoding(
      "s_wait_bvhcnt", reinterpret_cast<const Sopp::OpEncoding *>(opcode), emit_error);
  if (validation.failed()) [[unlikely]]
    return Result::failure();
...
namespace detail {
DecodeResult decodeSWaitExpcntSopp(const MachineInst *opcode,
                                   const DecodeErrorEmitter &emit_error) {
  Result validation = Sopp::validate_encoding(
      "s_wait_expcnt", reinterpret_cast<const Sopp::OpEncoding *>(opcode), emit_error);
  if (validation.failed()) [[unlikely]]
    return Result::failure();
