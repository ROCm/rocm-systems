...
[[nodiscard]] inline bool try_execute_ternary_vop2_f64_simd(Inst &inst, Wavefront &wf,
  ...
  const uint64_t exec = wf.exec();
  RegisterAccess regs(wf.cu());
  auto src0 = regs.read_operand64(inst.src0, wf, exec);
  auto src1 = regs.read_operand64(inst.vsrc1, wf, exec);
...
[[nodiscard]] inline bool try_execute_binary_vop2_f64_simd(Inst &inst, Wavefront &wf,
  ...
  auto src0 = regs.read_operand64(inst.src0, wf, exec);
  auto src1 = regs.read_operand64(inst.vsrc1, wf, exec);
  auto dst = regs.write_operand64(inst.vdst, wf, exec);
  for (uint32_t base = 0; base < wf.wf_size(); base += static_cast<uint32_t>(W)) {
    const uint64_t chunk = (exec >> base) & chunk_full;
    if (chunk == 0)
      continue;
...
[[nodiscard]] inline bool try_execute_cndmask_vop3_simd(Inst &inst, Wavefront &wf) {
  ...
  RegisterAccess regs(wf.cu());
  auto src0 = regs.read_operand(inst.src0, wf, exec);
  auto src1 = regs.read_operand(inst.src1, wf, exec);
  auto dst = regs.write_operand(inst.vdst, wf, exec);
  for (uint32_t base = 0; base < wf.wf_size(); base += static_cast<uint32_t>(W)) {
    const uint64_t chunk = (exec >> base) & chunk_full;
    if (chunk == 0)
...
[[nodiscard]] inline bool try_execute_vopc_class_f64_simd(Inst &inst, Wavefront &wf, CmpOp cmp_op) {
  ...
  RegisterAccess regs(wf.cu());
  auto src0 = regs.read_operand64(inst.src0, wf, exec);
  auto mask_src = regs.read_operand(inst.vsrc1, wf, exec);
  for (uint32_t base = 0; base < wf.wf_size(); base += static_cast<uint32_t>(W)) {
    const uint64_t chunk = (exec >> base) & chunk_full;
    if (chunk == 0)
      continue;
...
[[nodiscard]] inline bool try_execute_vop3_class_f64_simd(Inst &inst, Wavefront &wf,
  ...
  RegisterAccess regs(wf.cu());
  auto src0 = regs.read_operand64(inst.src0, wf, exec);
  auto mask_src = regs.read_operand(inst.src1, wf, exec);
  for (uint32_t base = 0; base < wf.wf_size(); base += static_cast<uint32_t>(W)) {
    const uint64_t chunk = (exec >> base) & chunk_full;
    if (chunk == 0)
      continue;
...
[[nodiscard]] inline bool try_execute_vopc64_vop3_int_simd(Inst &inst, Wavefront &wf,
  ...
  RegisterAccess regs(wf.cu());
  auto src0 = regs.read_operand64(inst.src0, wf, exec);
  auto src1 = regs.read_operand64(inst.src1, wf, exec);
  for (uint32_t base = 0; base < wf.wf_size(); base += static_cast<uint32_t>(W)) {
    const uint64_t chunk = (exec >> base) & chunk_full;
    if (chunk == 0)
      continue;
...
/// apply omod/clamp in f32, narrow via util::f32_to_f16_simd, and store through
/// a RegisterAccess write view.
...
[[nodiscard]] inline bool try_execute_unary_vop3_fp16_simd(Inst &inst, Wavefront &wf, UnOp un_op) {
  ...
  for (uint32_t base = 0; base < wf.wf_size(); base += static_cast<uint32_t>(W)) {
    ...
    if (chunk == 0)
      continue;
    const auto in = util::f16_to_f32_simd(src0.template load_native<T>(base));
...
[[nodiscard]] inline bool try_execute_ternary_vop3_simd(Inst &inst, Wavefront &wf, TernOp tern_op) {
  ...
  auto src0 = regs.read_operand(inst.src0, wf, exec);
  auto src1 = regs.read_operand(inst.src1, wf, exec);
  auto src2 = regs.read_operand(inst.src2, wf, exec);
  auto dst = regs.write_operand(inst.vdst, wf, exec);
  for (uint32_t base = 0; base < wf.wf_size(); base += static_cast<uint32_t>(W)) {
    const uint64_t chunk = (exec >> base) & chunk_full;
    if (chunk == 0)
...
[[nodiscard]] inline bool try_execute_ternary_vop3_fp16_simd(Inst &inst, Wavefront &wf,
  ...
  for (uint32_t base = 0; base < wf.wf_size(); base += static_cast<uint32_t>(W)) {
    ...
    const auto a = apply_vop3_src_mod_f32<0>(
        util::f16_to_f32_simd(src0.template load_native<T>(base)), abs, neg);
    const auto b = apply_vop3_src_mod_f32<1>(
        util::f16_to_f32_simd(src1.template load_native<T>(base)), abs, neg);
    const auto c = apply_vop3_src_mod_f32<2>(
        util::f16_to_f32_simd(src2.template load_native<T>(base)), abs, neg);
    const auto r = apply_vop3_dst_mod_f32(tern_op(a, b, c), omod, clamp);
    const auto out = util::f32_to_f16_simd(r);
    dst.template store_native<T>(base, out, chunk);
  }
...
[[nodiscard]] inline bool try_execute_ternary_vop3_fp64_simd(Inst &inst, Wavefront &wf,
  ...
  auto src0 = regs.read_operand64(inst.src0, wf, exec);
  auto src1 = regs.read_operand64(inst.src1, wf, exec);
  auto src2 = regs.read_operand64(inst.src2, wf, exec);
  auto dst = regs.write_operand64(inst.vdst, wf, exec);
  for (uint32_t base = 0; base < wf.wf_size(); base += static_cast<uint32_t>(W)) {
    const uint64_t chunk = (exec >> base) & chunk_full;
    if (chunk == 0)
...
[[nodiscard]] inline bool try_execute_ldexp_vop3_fp32_simd(Inst &inst, Wavefront &wf, Op op) {
  ...
  RegisterAccess regs(wf.cu());
  auto src0 = regs.read_operand(inst.src0, wf, exec);
  auto exp_src = regs.read_operand(inst.src1, wf, exec);
  auto dst = regs.write_operand(inst.vdst, wf, exec);
  for (uint32_t base = 0; base < wf.wf_size(); base += static_cast<uint32_t>(W)) {
    const uint64_t chunk = (exec >> base) & chunk_full;
    if (chunk == 0)
...
[[nodiscard]] inline bool try_execute_shift64_vop3_simd(Inst &inst, Wavefront &wf,
  ...
  RegisterAccess regs(wf.cu());
  auto shift_src = regs.read_operand(inst.src0, wf, exec);
  auto value_src = regs.read_operand64(inst.src1, wf, exec);
  auto dst = regs.write_operand64(inst.vdst, wf, exec);
  for (uint32_t base = 0; base < wf.wf_size(); base += static_cast<uint32_t>(W)) {
    const uint64_t chunk = (exec >> base) & chunk_full;
    if (chunk == 0)
...
[[nodiscard]] inline bool try_execute_lshl_add_u64_simd(Inst &inst, Wavefront &wf) {
  ...
  RegisterAccess regs(wf.cu());
  auto value_src = regs.read_operand64(inst.src0, wf, exec);
  auto shift_src = regs.read_operand(inst.src1, wf, exec);
  auto addend_src = regs.read_operand64(inst.src2, wf, exec);
  auto dst = regs.write_operand64(inst.vdst, wf, exec);
  for (uint32_t base = 0; base < wf.wf_size(); base += static_cast<uint32_t>(W)) {
    const uint64_t chunk = (exec >> base) & chunk_full;
    if (chunk == 0)
...
[[nodiscard]] inline bool try_execute_vop3p_fma_mix_simd(Inst &inst, Wavefront &wf) {
  ...
  if constexpr (DstMode == FmaMixDst::F32) {
    ...
    auto dst = regs.readwrite_operand(inst.vdst, wf, exec);
    for (uint32_t base = 0; base < wf.wf_size(); base += static_cast<uint32_t>(W)) {
      const uint64_t chunk = (exec >> base) & chunk_full;
      if (chunk == 0)
        continue;
...
[[nodiscard]] inline bool try_execute_vop3p_pk_binary_f32_simd(Inst &inst, Wavefront &wf, Op op) {
  ...
  const bool neg1_hi = inst.inst_.neg_hi & 2u;
  RegisterAccess regs(wf.cu());
  auto src0 = regs.read_operand_pair32(inst.src0, wf, exec);
  auto src1 = regs.read_operand_pair32(inst.src1, wf, exec);
  auto dst = regs.write_operand_pair32(inst.vdst, wf, exec);
  for (uint32_t base = 0; base < wf.wf_size(); base += static_cast<uint32_t>(W)) {
    const uint64_t chunk = (exec >> base) & chunk_full;
    if (chunk == 0)
      continue;
...
[[nodiscard]] inline bool try_execute_vop3p_pk_ternary_f32_simd(Inst &inst, Wavefront &wf, Op op) {
  ...
  auto src0 = regs.read_operand_pair32(inst.src0, wf, exec);
  auto src1 = regs.read_operand_pair32(inst.src1, wf, exec);
  auto src2 = regs.read_operand_pair32(inst.src2, wf, exec);
  auto dst = regs.write_operand_pair32(inst.vdst, wf, exec);
  for (uint32_t base = 0; base < wf.wf_size(); base += static_cast<uint32_t>(W)) {
    const uint64_t chunk = (exec >> base) & chunk_full;
    if (chunk == 0)
...
/// when the encoding points at the VGPR range. SGPR / literal sources broadcast
/// both halves identically through the RegisterAccess 64-bit scalar fallback.
/// Reads each src as a 64-bit pair, picks the low half of src0 and the high
/// half of src1 via mask, packs, and writes through a 64-bit RegisterAccess
/// write view. Functorless / fixed-op.
template <typename Inst>
  requires(util::has_stdx_simd)
...
[[nodiscard]] inline bool try_execute_dotc_int_simd(Inst &inst, Wavefront &wf) {
  ...
  auto src1 = [&]() {
    if constexpr (Vop3)
      return regs.read_operand(inst.src1, wf, exec);
    else
      return regs.read_operand(inst.vsrc1, wf, exec);
  }();
  auto acc = regs.readwrite_operand(inst.vdst, wf, exec);
  for (uint32_t base = 0; base < wf.wf_size(); base += static_cast<uint32_t>(W)) {
    const uint64_t chunk = (exec >> base) & chunk_full;
