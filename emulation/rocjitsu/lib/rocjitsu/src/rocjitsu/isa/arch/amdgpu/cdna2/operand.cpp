...
std::optional<RegisterRef> Operand::to_register_ref() const {
  if (size_bits_ == 0)
    return std::nullopt;
  // A field-less operand (no MR ISA encoding field: a hardwired
  // register/side effect like VCC/EXEC/SCC, or the field-less image
  ...
  // Making this explicit keeps every field-less operand inert by
