...
public:
  ...
  /// @brief Retain the compatibility service across a complete caller operation.
  [[nodiscard]] std::shared_ptr<LegacyAddressSpace> address_space_snapshot(uint32_t vmid);
  [[nodiscard]] std::shared_ptr<const LegacyAddressSpace>
  address_space_snapshot(uint32_t vmid) const;
