...
constexpr uint32_t kGfx94VgprLaneBytes = 64 * sizeof(uint32_t);
constexpr uint32_t kGfx1250VgprLaneBytes = 32 * sizeof(uint32_t);
...
constexpr uint32_t kGfx94MaxVgprs = 256;
constexpr uint32_t kGfx1250MaxVgprs = 1024;
...
constexpr bool is_gfx1250(rj_code_arch_t arch) { return arch == ROCJITSU_CODE_ARCH_CDNA5; }
...
struct CwsrGeometry {
  ...
  uint32_t vgpr_lane_bytes = kGfx94VgprLaneBytes;
...
CwsrGeometry compute_geometry(uint32_t area_size, const std::vector<CwsrWaveState> &waves,
  ...
  const bool gfx1250 = is_gfx1250(arch);
  geometry.sgpr_count = gfx1250 ? kCwsrGfx1250SavedSgprSlots : kCwsrSavedSgprSlots;
  geometry.hwreg_bytes = (gfx1250 ? kGfx1250HwregCount : kGfx94HwregCount) * sizeof(uint32_t);
  geometry.vgpr_lane_bytes = gfx1250 ? kGfx1250VgprLaneBytes : kGfx94VgprLaneBytes;
  geometry.lane_count = gfx1250 ? 32 : 64;
  geometry.record_prefix_bytes = gfx1250 ? 0 : 64;
  geometry.state_words = gfx1250 ? 2 : 1;

  uint32_t max_vgprs = 0;
  uint32_t max_lds = 0;
  for (const auto &wave : waves) {
    const uint32_t max_saved_vgprs = gfx1250 ? kGfx1250MaxVgprs : kGfx94MaxVgprs;
    if (wave.num_vgprs > max_saved_vgprs || wave.num_sgprs > geometry.sgpr_count ||
        wave.trap_id > 0xFu || wave.wave_in_group > (gfx1250 ? 0x1Fu : 0x3Fu) ||
        wave.queue_packet_id > 0x1FFFFFFu ||
        (gfx1250 && (wave.group_ids[1] > 0xFFFFu || wave.group_ids[2] > 0xFFFFu)))
      return geometry;
  ...
  const uint32_t lds_granule = gfx1250 ? 1024 : 1280;
  geometry.lds_bytes = round_up(max_lds, lds_granule);
  if (geometry.lds_bytes / lds_granule > (gfx1250 ? 0x1FFu : 0xFFu))
    return geometry;

  const uint32_t vgpr_granule = gfx1250 ? 16 : 8;
...
CwsrLayout serialize_queue_cwsr(uint64_t ctx_base, uint32_t area_size,
  ...
  write32(cs_base + 8, gfx1250 ? encode_gfx1250_state_word(vgpr_count, lds_bytes)
  ...
  for (size_t i = 0; i < num_waves; ++i) {
    ...
    write32(cs_base + wave_words_offset + i * 4,
            gfx1250 ? encode_gfx1250_wave_word(waves[i])
  ...
  for (size_t i = 0; i < num_waves; ++i) {
    ...
    const uint64_t ttmps_addr = gfx1250 ? sgprs_addr + (sgpr_count - kTtmpCount) * sizeof(uint32_t)
    ...
    ttmp[6] = encode_ttmp6(w) & (gfx1250 ? ~((1u << 31) | (0xFu << 25)) : UINT32_MAX);
    if (gfx1250) {
...
bool deserialize_queue_cwsr(uint64_t ctx_base, uint32_t area_size,
  ...
  for (uint32_t i = 0; i < num_waves; ++i) {
    ...
    const uint64_t ttmps_addr =
        gfx1250 ? sgprs_addr + (geometry.sgpr_count - kTtmpCount) * sizeof(uint32_t)
                : register_area_end - kTtmpCount * sizeof(uint32_t);
