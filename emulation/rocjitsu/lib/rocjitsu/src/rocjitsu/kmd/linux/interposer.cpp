...
public:
  ...
  struct SyncobjFence {
    ...
    bool is_signaled() const {
      ...
      return std::all_of(dependencies.begin(), dependencies.end(),
                         [](const auto &fence) { return fence->is_signaled(); });
    }
    bool is_failed() const {
      return failed.load(std::memory_order_acquire) ||
             std::any_of(dependencies.begin(), dependencies.end(),
                         [](const auto &fence) { return fence->is_failed(); });
    }
  };
  ...
  struct SyncFile {
    ...
    ~SyncFile() {
      if (event_fd >= 0)
        InterposerContext::real().close(event_fd);
    }
    void refresh() {
      if (notified || !fence || (!fence->is_signaled() && !fence->is_failed()))
        return;
      uint64_t value = 1;
      if (syscall(SYS_write, event_fd, &value, sizeof(value)) == sizeof(value))
        notified = true;
    }
  };
  ...
  struct DrmFileState {
    ...
    std::unordered_map<uint64_t, uint64_t>
        pm4_queue_keys;           ///< Context/ring queues until VM teardown.
  ...
  int drm_context(const DrmFileToken &file, drm_amdgpu_ctx *argument) {
    ...
    case AMDGPU_CTX_OP_FREE_CTX:
      return file->contexts.erase(request.ctx_id) ? 0 : -EINVAL;
  ...
  static void refresh_syncobj_locked(SyncobjEntry &entry) {
    ...
    for (const auto &[point, fence] : entry.timeline_fences)
  ...
  int transfer_syncobj(const DrmFileToken &file, const drm_syncobj_transfer &request) {
      ...
      return -EINVAL;
    {
      ...
      if (request.dst_point) {
        target->timeline_fences[request.dst_point] = fence;
        target->submitted_point = std::max<uint64_t>(target->submitted_point, request.dst_point);
      } else {
  ...
  int create_sync_file_locked(const std::shared_ptr<SyncobjFence> &fence) {
    ...
    if (fd < 0)
      return -errno;
    object->event_fd = real().fcntl(fd, F_DUPFD_CLOEXEC, 0);
  ...
  static void signal_syncobj_locked(const std::shared_ptr<SyncobjEntry> &entry, uint64_t point) {
    ...
    // Imported/submitted fences may be shared with another handle. A host
    // signal replaces this timeline's point; it must never complete the job.
    auto fence = std::make_shared<SyncobjFence>();
    fence->signaled.store(true, std::memory_order_release);
    auto [position, inserted] = entry->timeline_fences.insert_or_assign(point, fence);
    entry->timeline_fences.erase(entry->timeline_fences.begin(), position);
    if (point >= entry->submitted_point)
      entry->binary_fence = fence;
  ...
  int submit_drm_cs(const DrmFileToken &file, drm_amdgpu_cs *argument) {
    ...
    try {
      ...
      uint64_t sequence, queue_key;
      {
        ...
        if (user_fence) {
          ...
          if (it == gem_entries_.end() || it->second.handle_closed ||
            ...
            return -EINVAL;
          int fd = real().fcntl(it->second.dmabuf_fd, F_DUPFD_CLOEXEC, 0);
          if (fd < 0)
            return -errno;
          fence_fd = std::shared_ptr<int>(new int(fd), [](int *fd) {
            real().close(*fd);
            delete fd;
          });
        }
        ...
        for (const auto &output : outputs) {
          ...
          if (output.point) {
            entry->timeline_fences[output.point] = finished;
            entry->submitted_point = std::max<uint64_t>(entry->submitted_point, output.point);
          } else {
            entry->timeline_fences.clear();
            entry->submitted_point = entry->signaled_point = 0;
          }
      ...
      submission.complete = [this, file, finished, fence_fd, user_fence, sequence, retained,
                             driver](bool success) {
        if (success && fence_fd)
          success = pwrite(*fence_fd, &sequence, sizeof(sequence), user_fence->offset) ==
                    sizeof(sequence);
  ...
  void *mmap_gem(const DrmFileToken &file, void *address, size_t size, int prot, int flags,
    ...
    for (const auto &[handle, entry] : gem_entries_) {
      ...
      return real().mmap(address, size, prot, flags, entry.dmabuf_fd, relative);
    }
    errno = EINVAL;
    return MAP_FAILED;
  }
