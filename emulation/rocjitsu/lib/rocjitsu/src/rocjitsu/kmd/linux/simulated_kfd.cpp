...

  {
    ...
    for (auto &[handle, alloc] : proc.allocations_) {
      ...
      if (alloc.host_ptr && alloc.host_ptr_owned) {
        ...
        safe_munmap(alloc.host_ptr, alloc.mapped_size);
...
bool SimulatedKfd::allocate_scratch_backing(uint32_t process_id, uint64_t gpu_va, size_t size) {
  ...
  // Multiple XCDs can race to provision the same process-wide pool. Serialize
  // both the lookup/growth path and initial allocation so no dispatch can
  // repoint the pool underneath waves launched by another XCD.
  ...
  // only the initial extent.
  {
    ...
    if (existing != proc->allocations_.end()) {
      ...
      if (alloc.scratch_backing && alloc.host_ptr != nullptr && aligned_size <= alloc.mapped_size) {
        proc->map_pages(gpu_va, alloc.host_ptr, aligned_size, amdgpu::Mtype::RW,
                        alloc.memfd >= 0 ? KfdProcess::HostExtentOwner::Driver
      ...
      proc->map_pages(gpu_va, grown, aligned_size, amdgpu::Mtype::RW,
                      KfdProcess::HostExtentOwner::Driver);
