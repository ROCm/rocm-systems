...
#include "rj_hsa_dbi_moi_record_replay_report_analyzer.h"
...
uint64_t record_replay_bank_saturation_count(const ConSanMoiReportHeader &header) {
  return (header.flags & kConSanMoiReportFlagRecordReplayBankSaturated) != 0u ? 1u : 0u;
}
...
RecordReplayPressureTelemetry record_replay_pressure_telemetry(
  ...
  result.saturated = record_replay_bank_saturation_count(header) != 0;
...
void accumulate_auto_moi_record_replay_analysis(AutoMoiReportSummary &summary,
  ...
  summary.record_replay_bank_saturation_count = pressure.saturated ? 1u : 0u;
  summary.record_replay_invalid_site_token_count = pressure.invalid_site_token_count;
  summary.replay_conflict_count = analysis.effective_conflict ? 1u : 0u;
  summary.replay_diagnostic_count = analysis.effective_diagnostic_count;
  summary.replay_dropped_access_count = analysis.replay.dropped_access_count;
  summary.replay_dropped_barrier_count = analysis.replay.dropped_barrier_count;
  summary.replay_unsupported_access_count = analysis.replay.unsupported_access_count;
  summary.replay_unsupported_atomic_count = analysis.replay.unsupported_atomic_count;
  summary.replay_unsupported_fence_count = analysis.replay.unsupported_fence_count;
  summary.replay_metadata_full_count = analysis.replay.metadata_full ? 1u : 0u;
  summary.replay_diagnostic_capacity_exhausted_count =
      analysis.replay.diagnostic_capacity_exhausted ? 1u : 0u;
}
