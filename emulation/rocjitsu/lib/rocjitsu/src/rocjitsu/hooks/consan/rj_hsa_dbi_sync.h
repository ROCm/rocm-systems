...
struct PendingJoinResult {
  PendingAcquireState state = PendingAcquireState::Empty;
...
[[nodiscard]] inline PendingJoinResult join_pending_acquire(const PendingAcquireView &pending,
                                                            const CausalWindow &window,
                                                            uint64_t packed_watchpoint,
                                                            uint32_t selected_slot) {
  const WatchpointEntry watchpoint = decode_watchpoint_entry(packed_watchpoint);
  const PendingAcquireKey key{
  ...
  const PendingAcquireState state = classify_pending_acquire(pending, key, window.epoch);
  if (state != PendingAcquireState::Ready)
    return {.state = state, .sync = {}};
  if (!atomic_attachment_matches(window, packed_watchpoint, selected_slot,
                                 {.generation = window.generation,
                                  .dispatch_id = window.dispatch_id,
                                  .workgroup_x = window.workgroup_x,
                                  .workgroup_y = window.workgroup_y,
                                  ...
                                  .epoch = window.epoch,
                                  .owner_id = watchpoint.owner_id}))
    return {.state = PendingAcquireState::IdentityMismatch, .sync = {}};

  SyncDecodeResult decoded = decode_sync_metadata(pending.slot.metadata);
...
[[nodiscard]] inline PendingJoinResult
join_pending_release(const PendingAcquireView &pending, const CausalWindow &window,
  ...
  if (state != PendingAcquireState::Ready)
    return {.state = state, .sync = {}};
  if (!atomic_attachment_matches(window, packed_watchpoint, release_slot,
                                 {.generation = window.generation,
                                  .dispatch_id = window.dispatch_id,
                                  .workgroup_x = window.workgroup_x,
                                  .workgroup_y = window.workgroup_y,
  ...
  SyncDecodeResult decoded = decode_sync_metadata(pending.slot.metadata);
  constexpr uint32_t release_bit = static_cast<uint32_t>(SyncRole::Release);
  if ((static_cast<uint32_t>(decoded.metadata.role) & release_bit) == 0u)
    return {.state = PendingAcquireState::Malformed, .sync = {}};
  decoded.metadata.epoch_before = window.epoch;
  decoded.metadata.epoch_after = window.epoch;
  return {.state = state, .sync = decoded};
}
...
[[nodiscard]] inline bool sync_report_is_complete(uint32_t dropped_window_count,
                                                  uint32_t unsupported_sync_count,
                                                  uint32_t malformed_sync_count,
                                                  uint32_t pending_acquire_collision_count = 0u,
                                                  uint32_t pending_acquire_malformed_count = 0u) {
...
///
/// Wavefront scope is intentionally rejected: ConSan ABI v1 does not retain
...
[[nodiscard]] inline bool atomic_pair_orders_same_workgroup(const SyncDecodeResult &first,
                                                            const SyncDecodeResult &second) {
  using Classification = SyncClassification;
  using Kind = SyncMetadataKind;
  using Outcome = SyncOutcome;
  using Role = SyncRole;
  using Scope = SyncScope;

  if (first.classification != Classification::Valid ||
      second.classification != Classification::Valid)
    return false;
  const SyncMetadata &first_metadata = first.metadata;
  const SyncMetadata &second_metadata = second.metadata;
  ...
  const auto has_release = [&](const SyncMetadata &metadata) {
  ...
  const auto has_acquire = [&](const SyncMetadata &metadata) {
