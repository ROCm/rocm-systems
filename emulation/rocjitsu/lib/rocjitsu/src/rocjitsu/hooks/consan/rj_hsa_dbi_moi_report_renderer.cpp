...
render_auto_moi_report(const AutoMoiReportRenderInput &render_input) {
  ...
  const auto log_message = [&rendered](AutoMoiReportDiagnosticKind kind, const char *format,
                                       auto... args) {
    ...
    rendered.push_back({kind, std::string(buffer.data(), static_cast<size_t>(size))});
  };
  constexpr AutoMoiReportDiagnosticKind kLogFailure = AutoMoiReportDiagnosticKind::Failure;
  constexpr AutoMoiReportDiagnosticKind kLogEvidence = AutoMoiReportDiagnosticKind::Evidence;
  constexpr AutoMoiReportDiagnosticKind kLogSummary = AutoMoiReportDiagnosticKind::Summary;
  constexpr AutoMoiReportDiagnosticKind kLogInfo = AutoMoiReportDiagnosticKind::Detail;
  ...
  if (!decoded.complete()) {
    ...
    if (decoded.failure == AutoMoiReportDecodeFailure::InvalidHeader) {
      ...
      log_message(kLogFailure, "ConSan MOI auto report reader=%llu has inconsistent ABI-v%u layout",
                  static_cast<unsigned long long>(input.reader), kConSanMoiReportAbiVersion);
    } else {
      log_message(kLogFailure, "ConSan MOI auto report reader=%llu has undersized host snapshot",
  ...
  log_message(
      ...
      input.layout.fence_record_capacity, effective_diagnostic_count, visible_diagnostics,
      dropped_diagnostics, header->diagnostic_capacity, header->exact_shadow_entry_capacity,
