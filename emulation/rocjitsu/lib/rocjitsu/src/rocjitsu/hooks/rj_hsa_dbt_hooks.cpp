...
parse_virtual_lds_metadata_section(std::span<const uint8_t> image) {
  ...
  if (header.e_shstrndx == SHN_UNDEF || header.e_shstrndx >= header.e_shnum ||
      !elf_range_in_bounds(image.size(), header.e_shoff,
  ...
  if (shstrtab.sh_type != SHT_STRTAB ||
    ...
    return std::nullopt;
  const char *names = reinterpret_cast<const char *>(image.data() + shstrtab.sh_offset);
  for (const Elf64_Shdr &section : sections) {
    ...
    const std::string_view name(names + section.sh_name,
                                strnlen(names + section.sh_name, max_name));
...
[[nodiscard]] std::string normalize_kernel_symbol_name(std::string_view name) {
  if (name.ends_with(".kd"))
    name.remove_suffix(3);
  return std::string(name);
}

/// Resolve load-relative metadata records to the kernel-object addresses used by AQL.
...
public:
  ...
  void record_load(hsa_executable_t executable, std::vector<VirtualLdsKernelMetadata> metadata) {
    if (metadata.empty())
      return;
    std::lock_guard lock(mutex_);
    if (g_log_level.load(std::memory_order_relaxed) >= kLogDebug)
      std::fprintf(stderr, "[rocjitsu-hooks] virtual LDS registry load exec=%llu kernels=%zu\n",
                   static_cast<unsigned long long>(executable.handle), metadata.size());
    loads_[executable.handle] = std::move(metadata);
  }
  ...
  void record_symbol(hsa_executable_t executable, std::string_view symbol_name,
    ...
    std::lock_guard lock(mutex_);
    auto load = loads_.find(executable.handle);
    if (load == loads_.end())
      return;
    auto metadata =
        std::ranges::find(load->second, kernel_name, &VirtualLdsKernelMetadata::kernel_name);
    if (metadata != load->second.end()) {
      symbols_[symbol.handle] = *metadata;
      if (g_log_level.load(std::memory_order_relaxed) >= kLogDebug)
        std::fprintf(stderr,
                     "[rocjitsu-hooks] virtual LDS registry symbol=%llu exec=%llu name=%s\n",
                     static_cast<unsigned long long>(symbol.handle),
                     static_cast<unsigned long long>(executable.handle), kernel_name.c_str());
    }
  }
  ...
  void note_kernel_object(hsa_executable_symbol_t symbol, uint64_t kernel_object) {
    if (symbol.handle == 0 || kernel_object == 0)
      return;
    std::lock_guard lock(mutex_);
    auto metadata = symbols_.find(symbol.handle);
    if (metadata == symbols_.end() || metadata->second.normal_descriptor_vaddr == 0 ||
        kernel_object < metadata->second.normal_descriptor_vaddr)
      return;
    ResolvedKernel resolved{.metadata = metadata->second, .kernel_object = kernel_object};
    const uint64_t load_base = kernel_object - metadata->second.normal_descriptor_vaddr;
    resolved.metadata.virtual_descriptor_vaddr += load_base;
    resolved.metadata.normal_descriptor_vaddr = kernel_object;
    kernels_[kernel_object] = std::move(resolved);
    if (g_log_level.load(std::memory_order_relaxed) >= kLogDebug)
      std::fprintf(stderr, "[rocjitsu-hooks] virtual LDS registry kernel_object=%llu symbol=%llu\n",
                   static_cast<unsigned long long>(kernel_object),
                   static_cast<unsigned long long>(symbol.handle));
  }

  [[nodiscard]] std::optional<ResolvedKernel> lookup(uint64_t kernel_object) const {
    std::lock_guard lock(mutex_);
    auto it = kernels_.find(kernel_object);
    return it == kernels_.end() ? std::nullopt : std::optional<ResolvedKernel>(it->second);
  }

  void erase_executable(hsa_executable_t executable) {
    std::lock_guard lock(mutex_);
    loads_.erase(executable.handle);
  }
...
private:
  ...
  std::unordered_map<uint64_t, std::vector<VirtualLdsKernelMetadata>> loads_;
  std::unordered_map<uint64_t, VirtualLdsKernelMetadata> symbols_;
  std::unordered_map<uint64_t, ResolvedKernel> kernels_;
};
...
void clear_virtual_lds_runtime();
...
struct VirtualLdsAllocations {
  void *backing = nullptr;
  void *kernarg = nullptr;
  hsa_signal_t completion{};
  bool completion_was_pending = false;
  bool owns_completion = false;
};

void free_virtual_lds_pointer(void *pointer) {
  if (pointer == nullptr)
    return;
  if (auto *free_fn = layer().amd_memory_pool_free())
    (void)free_fn(pointer);
}

void release_virtual_lds_allocations(VirtualLdsAllocations &allocations) {
  free_virtual_lds_pointer(allocations.kernarg);
  free_virtual_lds_pointer(allocations.backing);
  if (allocations.owns_completion && allocations.completion.handle != 0) {
    if (auto *destroy = layer().signal_destroy())
      (void)destroy(allocations.completion);
  }
  allocations = {};
}

class VirtualLdsAllocator {
public:
  static VirtualLdsAllocator &instance() {
    static VirtualLdsAllocator allocator;
    return allocator;
  }

  [[nodiscard]] bool allocate(hsa_agent_t host, size_t backing_bytes, size_t kernarg_bytes,
                              VirtualLdsAllocations &out) {
    const auto pools = pools_for(host);
    if (!pools || !pools->has_backing || !pools->has_kernarg)
      return false;
    auto *allocate_fn = layer().amd_memory_pool_allocate();
    auto *allow_fn = layer().amd_agents_allow_access();
    if (allocate_fn == nullptr || allow_fn == nullptr)
      return false;
    if (allocate_fn(pools->backing, backing_bytes, 0, &out.backing) != HSA_STATUS_SUCCESS ||
        out.backing == nullptr)
      return false;
    if (allow_fn(1, &host, nullptr, out.backing) != HSA_STATUS_SUCCESS) {
      release_virtual_lds_allocations(out);
      return false;
    }
    if (allocate_fn(pools->kernarg, kernarg_bytes, 0, &out.kernarg) != HSA_STATUS_SUCCESS ||
        out.kernarg == nullptr) {
      release_virtual_lds_allocations(out);
      return false;
    }
    if (allow_fn(1, &host, nullptr, out.kernarg) != HSA_STATUS_SUCCESS) {
      release_virtual_lds_allocations(out);
      return false;
    }
    return true;
...
private:
  struct Pools {
    hsa_amd_memory_pool_t backing{};
    hsa_amd_memory_pool_t kernarg{};
    bool has_backing = false;
    bool has_kernarg = false;
  };
  struct Search {
    hsa_amd_memory_pool_get_info_fn_t get_info = nullptr;
    Pools pools;
  ...
  static hsa_status_t collect_pool(hsa_amd_memory_pool_t pool, void *data) {
    ...
    (void)search->get_info(pool, HSA_AMD_MEMORY_POOL_INFO_RUNTIME_ALLOC_ALLOWED, &alloc_allowed);
    if (segment != kHsaAmdSegmentGlobal || !alloc_allowed)
      return HSA_STATUS_SUCCESS;
    if ((flags & kHsaAmdMemoryPoolGlobalFlagKernargInit) != 0) {
      if (!search->pools.has_kernarg) {
        search->pools.kernarg = pool;
        search->pools.has_kernarg = true;
      }
    } else if (!search->pools.has_backing) {
      search->pools.backing = pool;
      search->pools.has_backing = true;
    }
    return HSA_STATUS_SUCCESS;
  }

  struct GlobalSearch {
    hsa_amd_agent_iterate_memory_pools_fn_t iterate = nullptr;
    hsa_amd_memory_pool_get_info_fn_t get_info = nullptr;
    hsa_amd_memory_pool_t kernarg{};
    bool found = false;
  };
  ...
  static hsa_status_t collect_agent(hsa_agent_t agent, void *data) {
    auto *global = static_cast<GlobalSearch *>(data);
    if (global == nullptr || global->found)
      return HSA_STATUS_SUCCESS;
    Search search{};
    search.get_info = global->get_info;
    (void)global->iterate(agent, collect_pool, &search);
    if (search.pools.has_kernarg) {
      global->kernarg = search.pools.kernarg;
      global->found = true;
    }
    return HSA_STATUS_SUCCESS;
  }

  [[nodiscard]] std::optional<Pools> pools_for(hsa_agent_t host) {
    {
      std::lock_guard lock(mutex_);
      auto it = pools_.find(host.handle);
      if (it != pools_.end())
        return it->second;
    }
    auto *iterate = layer().amd_agent_iterate_memory_pools();
    auto *get_info = layer().amd_memory_pool_get_info();
    if (iterate == nullptr || get_info == nullptr)
      return std::nullopt;
    Search search{};
    search.get_info = get_info;
    if (iterate(host, collect_pool, &search) != HSA_STATUS_SUCCESS)
      return std::nullopt;
    if (!search.pools.has_kernarg) {
      GlobalSearch global{.iterate = iterate, .get_info = get_info};
      if (auto *iterate_agents = layer().iterate_agents())
        (void)iterate_agents(collect_agent, &global);
      if (global.found) {
        search.pools.kernarg = global.kernarg;
        search.pools.has_kernarg = true;
      }
    }
    if (!search.pools.has_backing || !search.pools.has_kernarg)
      return std::nullopt;
    std::lock_guard lock(mutex_);
    pools_[host.handle] = search.pools;
    return search.pools;
  }

  std::mutex mutex_;
  std::unordered_map<uint64_t, Pools> pools_;
};

class VirtualLdsDispatchRuntime {
public:
  static VirtualLdsDispatchRuntime &instance() {
    static VirtualLdsDispatchRuntime runtime;
    return runtime;
  }

  [[nodiscard]] bool rewrite(hsa_kernel_dispatch_packet_t &packet, hsa_agent_t host,
                             uint64_t packet_index) {
    const auto resolved = VirtualLdsRuntimeRegistry::instance().lookup(packet.kernel_object);
    if (!resolved) {
      if (g_log_level.load(std::memory_order_relaxed) >= kLogDebug)
        std::fprintf(stderr,
                     "[rocjitsu-hooks] virtual LDS lookup miss kernel_object=%llu packet=%llu\n",
    ...
    if (g_log_level.load(std::memory_order_relaxed) >= kLogDebug)
      std::fprintf(stderr,
                   "[rocjitsu-hooks] virtual LDS lookup hit kernel_object=%llu static=%u "
                   "packet_group=%u name=%s\n",
                   static_cast<unsigned long long>(packet.kernel_object), metadata.static_lds_bytes,
                   packet.group_segment_size, metadata.kernel_name.c_str());
    const uint64_t requested_lds =
        std::max<uint64_t>(metadata.static_lds_bytes, packet.group_segment_size);
    if (requested_lds == 0 || requested_lds > std::numeric_limits<uint32_t>::max())
      return false;
    const auto ceil_div = [](uint32_t grid, uint16_t group) -> std::optional<uint32_t> {
      if (group == 0)
        return std::nullopt;
      return (grid + group - 1u) / group;
    };
    const auto groups_x = ceil_div(packet.grid_size_x, packet.workgroup_size_x);
    const auto groups_y = ceil_div(packet.grid_size_y, packet.workgroup_size_y);
    const auto groups_z = ceil_div(packet.grid_size_z, packet.workgroup_size_z);
    if (!groups_x || !groups_y || !groups_z)
      return false;
    const uint64_t stride_x = requested_lds;
    const uint64_t stride_y = static_cast<uint64_t>(*groups_x) * stride_x;
    const uint64_t stride_z = static_cast<uint64_t>(*groups_y) * stride_y;
    const uint64_t backing_bytes = static_cast<uint64_t>(*groups_z) * stride_z;
    if (stride_y > std::numeric_limits<uint32_t>::max() ||
        stride_z > std::numeric_limits<uint32_t>::max() ||
        backing_bytes > std::numeric_limits<size_t>::max())
      return false;

    const KernargExtensionPayloadLayout payload{.size = sizeof(VirtualLdsDispatchState),
                                                .alignment = alignof(uint64_t)};
    const auto layout =
        make_kernarg_extension_layout(metadata.kernarg_size, std::span{&payload, 1});
    if (!layout || layout->payload_offsets.size() != 1 ||
        layout->payload_offsets[0] != metadata.backing_pointer_kernarg_offset ||
        (metadata.kernarg_size != 0 && packet.kernarg_address == nullptr))
      return false;

    VirtualLdsAllocations allocations;
    if (!VirtualLdsAllocator::instance().allocate(
            host, static_cast<size_t>(std::max<uint64_t>(backing_bytes, requested_lds)),
            layout->wrapper_size, allocations))
      return false;

    const VirtualLdsDispatchState state{
        .backing_base = reinterpret_cast<uintptr_t>(allocations.backing),
        .stride_x = *groups_x > 1 ? static_cast<uint32_t>(stride_x) : 0,
        .stride_y = *groups_y > 1 ? static_cast<uint32_t>(stride_y) : 0,
        .stride_z = *groups_z > 1 ? static_cast<uint32_t>(stride_z) : 0,
    };
    const KernargExtensionPayloadWrite write{.data = &state, .size = sizeof(state)};
    if (!write_kernarg_extension_wrapper(
            std::span<uint8_t>(static_cast<uint8_t *>(allocations.kernarg), layout->wrapper_size),
            *layout, packet.kernarg_address, reinterpret_cast<uintptr_t>(packet.kernarg_address),
            std::span{&write, 1})) {
      release_virtual_lds_allocations(allocations);
      return false;
    }

    if (packet.completion_signal.handle == 0) {
      if (auto *create = layer().signal_create()) {
        hsa_signal_t signal{};
        if (create(1, 0, nullptr, &signal) == HSA_STATUS_SUCCESS) {
          packet.completion_signal = signal;
          allocations.completion = signal;
          allocations.completion_was_pending = true;
          allocations.owns_completion = true;
        }
      }
    } else {
      allocations.completion = packet.completion_signal;
      if (auto *load = layer().signal_load_scacquire())
        allocations.completion_was_pending = load(packet.completion_signal) > 0;
    }

    packet.kernel_object = metadata.virtual_descriptor_vaddr;
    packet.kernarg_address = allocations.kernarg;
    packet.group_segment_size = 0;
    log_message(kLogDebug, "virtualized full LDS packet=%llu kernel=%s bytes=%llu groups=%u,%u,%u",
                static_cast<unsigned long long>(packet_index), metadata.kernel_name.c_str(),
                static_cast<unsigned long long>(requested_lds), *groups_x, *groups_y, *groups_z);
    std::lock_guard lock(mutex_);
    pending_.push_back(std::move(allocations));
    return true;
  }

  [[nodiscard]] bool rewrite(rocjitsu::amdgpu::AmdExtKernelDispatchPacket &packet, hsa_agent_t host,
                             uint64_t packet_index) {
    const auto grid_size = [](uint32_t clusters, uint8_t cluster_size,
                              uint16_t workgroup_size) -> std::optional<uint32_t> {
    ...
    const auto grid_x =
        grid_size(packet.cluster_count_x, packet.cluster_size_x, packet.workgroup_size_x);
    const auto grid_y =
        grid_size(packet.cluster_count_y, packet.cluster_size_y, packet.workgroup_size_y);
    const auto grid_z =
        grid_size(packet.cluster_count_z, packet.cluster_size_z, packet.workgroup_size_z);
    if (!grid_x || !grid_y || !grid_z)
      return false;

    // The simulator consumes the AMD extended packet verbatim. Present an
    // ordinary dispatch view to the virtual-LDS runtime, then copy back only
    // the fields it owns so cluster geometry and dependency signaling remain
    // intact.
    hsa_kernel_dispatch_packet_t dispatch{};
    dispatch.workgroup_size_x = packet.workgroup_size_x;
    dispatch.workgroup_size_y = packet.workgroup_size_y;
    dispatch.workgroup_size_z = packet.workgroup_size_z;
    dispatch.grid_size_x = *grid_x;
    dispatch.grid_size_y = *grid_y;
    dispatch.grid_size_z = *grid_z;
    dispatch.private_segment_size = packet.private_segment_size;
    dispatch.group_segment_size = packet.group_segment_size;
    dispatch.kernel_object = packet.kernel_object;
    dispatch.kernarg_address = packet.kernarg_address;
    dispatch.completion_signal = packet.completion_signal;
    if (!rewrite(dispatch, host, packet_index))
      return false;
    packet.private_segment_size = dispatch.private_segment_size;
    packet.group_segment_size = dispatch.group_segment_size;
    packet.kernel_object = dispatch.kernel_object;
    packet.kernarg_address = dispatch.kernarg_address;
    packet.completion_signal = dispatch.completion_signal;
    return true;
  }
  ...
  void reap() {
    ...
    for (auto it = pending_.begin(); it != pending_.end();) {
      if (it->completion.handle != 0 && it->completion_was_pending && load(it->completion) <= 0) {
        release_virtual_lds_allocations(*it);
  ...
  void clear() {
    std::lock_guard lock(mutex_);
    for (auto &allocations : pending_)
      release_virtual_lds_allocations(allocations);
    pending_.clear();
    VirtualLdsAllocator::instance().clear();
  }

private:
  std::mutex mutex_;
  std::vector<VirtualLdsAllocations> pending_;
};

void clear_virtual_lds_runtime() { VirtualLdsDispatchRuntime::instance().clear(); }
...
void rj_aql_packet_interceptor(const void *packets, uint64_t packet_count,
  ...
  for (uint64_t index = 0; index < packet_count; ++index) {
    ...
    auto normalized = rocjitsu::amdgpu::normalize_aql_packet(packet, backend, resources);
    for (uint32_t normalized_index = 0; normalized_index < normalized.packet_count;
         ++normalized_index) {
      auto &slot = normalized.packets[normalized_index];
      hsa_kernel_dispatch_packet_t dispatch{};
      std::memcpy(&dispatch, slot.bytes.data(), sizeof(dispatch));
      constexpr uint16_t kTypeMask = ((uint16_t{1} << HSA_PACKET_HEADER_WIDTH_TYPE) - 1u)
                                     << HSA_PACKET_HEADER_TYPE;
      const uint16_t type = (dispatch.header & kTypeMask) >> HSA_PACKET_HEADER_TYPE;
      if (type == HSA_PACKET_TYPE_KERNEL_DISPATCH) {
        if (!VirtualLdsDispatchRuntime::instance().rewrite(
                dispatch, AgentMapper::instance().host_for_guest(), user_packet_index + index)) {
          std::fprintf(stderr,
                       "[rocjitsu-hooks] virtual LDS dispatch preparation failed at packet %llu; "
                       "forwarding an invalid kernel object to the queue error path\n",
                       static_cast<unsigned long long>(user_packet_index + index));
          dispatch.kernel_object = 0;
        }
        std::memcpy(slot.bytes.data(), &dispatch, sizeof(dispatch));
        continue;
      }
      if (backend != rocjitsu::amdgpu::AqlExecutionBackend::Simulator ||
          type != HSA_PACKET_TYPE_VENDOR_SPECIFIC)
        continue;
      rocjitsu::amdgpu::AmdExtKernelDispatchPacket extended{};
      std::memcpy(&extended, slot.bytes.data(), sizeof(extended));
      if (extended.amd_format != rocjitsu::amdgpu::kHsaAmdPacketTypeExtKernelDispatch)
        continue;
      if (!VirtualLdsDispatchRuntime::instance().rewrite(
              extended, AgentMapper::instance().host_for_guest(), user_packet_index + index)) {
        std::fprintf(stderr,
                     "[rocjitsu-hooks] virtual LDS extended-dispatch preparation failed at "
                     "packet %llu; forwarding an invalid kernel object to the queue error path\n",
                     static_cast<unsigned long long>(user_packet_index + index));
        extended.kernel_object = 0;
      }
      std::memcpy(slot.bytes.data(), &extended, sizeof(extended));
    }
...
hsa_status_t HSA_API rj_executable_get_symbol(hsa_executable_t executable, const char *module_name,
  ...
  if (status == HSA_STATUS_SUCCESS && symbol != nullptr && symbol_name != nullptr)
...
hsa_status_t HSA_API rj_executable_get_symbol_by_name(hsa_executable_t executable,
  ...
  const hsa_status_t status = original(executable, symbol_name, mapped_ptr, symbol);
  if (status == HSA_STATUS_SUCCESS && symbol != nullptr && symbol_name != nullptr)
    VirtualLdsRuntimeRegistry::instance().record_symbol(executable, symbol_name, *symbol);
  return status;
}
...
hsa_status_t HSA_API rj_iterate_agent_symbols_callback(hsa_executable_t executable, hsa_agent_t,
  ...
  if (auto *get_info = layer().executable_symbol_get_info()) {
    uint32_t name_length = 0;
    if (get_info(symbol, HSA_EXECUTABLE_SYMBOL_INFO_NAME_LENGTH, &name_length) ==
            HSA_STATUS_SUCCESS &&
        name_length != 0) {
      std::vector<char> name(name_length + 1u, '\0');
      if (get_info(symbol, HSA_EXECUTABLE_SYMBOL_INFO_NAME, name.data()) == HSA_STATUS_SUCCESS) {
        VirtualLdsRuntimeRegistry::instance().record_symbol(
            executable, std::string_view(name.data(), strnlen(name.data(), name.size())), symbol);
      }
    }
  }
...
hsa_status_t HSA_API rj_executable_load_agent_code_object(
  ...
  if (!virtual_lds_metadata) {
    std::fprintf(stderr, "[rocjitsu-hooks] translated virtual LDS metadata is malformed\n");
  ...
  if (status == HSA_STATUS_SUCCESS && !virtual_lds_metadata->empty())
    VirtualLdsRuntimeRegistry::instance().record_load(executable, std::move(*virtual_lds_metadata));
  return status;
}
