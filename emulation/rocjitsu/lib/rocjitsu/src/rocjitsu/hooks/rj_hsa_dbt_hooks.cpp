...
/// without translating it (a non-gfx1250 object). Rather than hand ROCR the
/// caller's own reader -- whose backing the application can still mutate after
/// we inspected it -- build
...
/// than loading untranslated B0 on A0. Objects we forward rather than translate
/// (a non-gfx1250 object) are loaded from an authoritative hook-owned snapshot,
/// never the caller's mutable reader (see load_snapshot_for_auto_a0).
[[nodiscard]] hsa_status_t
