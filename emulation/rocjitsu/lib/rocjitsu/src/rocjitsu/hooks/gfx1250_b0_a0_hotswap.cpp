...
/// @brief Read the memo's byte ceiling from the environment, once.
///
/// @details This is process-lifetime memory that the workload does not get back,
/// so an operator who cannot afford it needs a way to say so, and one who is
/// watching the memo thrash needs a way to enlarge it. Zero disables the memo
/// outright, which is also the way to confirm that a fault is or is not the
/// memo's doing. Anything unparsable leaves the default rather than guessing.
size_t memo_capacity_from_environment() {
  const char *value = std::getenv("HSA_HOTSWAP_TRANSLATION_MEMO_BYTES");
  if (value == nullptr || value[0] == '\0')
    return kDefaultMemoCapacity;
  errno = 0;
  char *end = nullptr;
...
struct HookState {
  ...
  size_t memo_bytes = 0;
  size_t memo_capacity = memo_capacity_from_environment();
...
void emit_memo_summary(const MemoLogSummary &summary) noexcept {
  if (summary.refused)
    log("memo refused entry bytes=%zu capacity=%zu; raise "
        "HSA_HOTSWAP_TRANSLATION_MEMO_BYTES to retain it",
        summary.refused_bytes, summary.capacity);
...
MemoLookup memo_acquire(uint64_t fingerprint, const Blob &source, TranslationRecord &out,
  ...
  try {
    ...
    // claimed, and every load translates as it did before this existed.
    if (g_state.memo_capacity == 0)
  ...
  try {
    {
      ...
      // from the rule. Name the knob that would have kept it: the alternative to
      // holding it is paying the translation on every load, and only an operator
      // can weigh those against each other.
      bool admitted = false;
...
extern "C" RJ_HOOK_EXPORT void rj_test_clear_retained_storage() {
  ...
  g_state.memo_bytes = 0;
  g_state.memo_capacity = memo_capacity_from_environment();
...
// memo, exactly as the environment override does.
