...
void LivenessAnalysis::analyze(KernelBlockScope blocks, const LivenessAnalysisOptions &options,
                               std::span<const ScopedCfgEdge> extra_edges) {
  std::array<BasicBlock *, 1> single_entry{options.entry_block};
  const std::span<BasicBlock *const> entry_blocks =
      !options.entry_blocks.empty()
          ? options.entry_blocks
          : std::span<BasicBlock *const>(single_entry.data(),
                                         options.entry_block == nullptr ? 0u : 1u);
  if (options.arch == ROCJITSU_CODE_ARCH_GFX1250 && !entry_blocks.empty()) {
    gfx1250_vgpr_msb_ =
        std::make_unique<Gfx1250VgprMsbAnalysis>(blocks, entry_blocks, extra_edges, options.text);
  }
...
std::optional<uint16_t> LivenessAnalysis::find_free_run(const Instruction *inst, uint16_t count,
  ...
  if (!scratch_vgpr_range_limits_.empty()) {
    bool matched = false;
    for (const ScratchVgprRangeLimit &range : scratch_vgpr_range_limits_) {
      if (range.begin_offset <= inst->src_loc() && inst->src_loc() < range.end_offset) {
