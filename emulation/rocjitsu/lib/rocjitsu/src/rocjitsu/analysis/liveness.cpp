...
bool LivenessAnalysis::alias_live_before(const Instruction &alias, const Instruction &anchor) {
  auto it = live_before_.find(&anchor);
  if (it == live_before_.end())
    return false;
  live_before_[&alias] = it->second;
  return true;
}
...
void LivenessAnalysis::reserve_scratch_registers(RegisterRef ref) { scratch_reserved_.expand(ref); }
