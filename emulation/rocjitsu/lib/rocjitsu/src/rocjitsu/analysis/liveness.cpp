...
[[nodiscard]] size_t align_up(size_t value, uint16_t alignment) {
  assert(alignment > 0 && "Register tuple alignment must be non-zero");
...
std::optional<uint16_t> LivenessAnalysis::find_free_run(const Instruction *inst, uint16_t count,
  ...
  for (size_t base = align_up(first_candidate, base_alignment);
