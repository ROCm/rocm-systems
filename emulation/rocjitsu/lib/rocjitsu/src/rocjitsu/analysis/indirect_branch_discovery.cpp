  ...
  const auto scan_block = [&](const AnalysisBlock &block, VectorLaneFlowState state,
    ...
    for (size_t index = block.first_index; index <= block.last_index; ++index) {
      ...
      if (mnemonic == "v_writelane_b32") {
        ...
        const auto dst = transfer && transfer->kind == FixedLaneTransfer::Kind::Write
                             ? std::optional<uint16_t>{transfer->lane.vgpr}
                             : std::nullopt;
        const auto dst_phys = dst ? physical_vgpr(*dst, kDstBankShift) : std::nullopt;
        if (transfer && dst) {
          if (dst_phys) {
            const VectorLaneSlot written_slot{*dst_phys, transfer->lane.lane};
            state.slots.erase(written_slot);
            for (uint16_t pair_lo : builders.active_pairs()) {
              const PcValue *value = builders.builder(pair_lo);
              if (value == nullptr || (transfer->sgpr != pair_lo && transfer->sgpr != pair_lo + 1))
                continue;
              state.slots[written_slot] =
                  StashedPcHalf{.value = *value, .high = transfer->sgpr == pair_lo + 1};
              break;
            }
          } else {
            ...
            std::erase_if(state.slots, [&](const auto &item) {
              return (item.first.vgpr & 0xffu) == (*dst & 0xffu) &&
                     item.first.lane == transfer->lane.lane;
            });
          }
        } else if (const auto encoded_dst = operand_register(inst.dst_operand(0), RegClass::VGPR)) {
          // An unproven lane selector may overwrite any lane in the destination
          // register. Drop every carried slot for that physical selector.
          std::erase_if(state.slots, [&](const auto &item) {
            return (item.first.vgpr & 0xffu) == (encoded_dst->index & 0xffu);
          });
        }
      ...
      if (mnemonic == "v_readlane_b32") {
        const auto transfer = fixed_lane_transfer(inst, ctx.wavefront_size);
        invalidate_written_sgprs(ctx, index, builders);
        const auto src_phys = transfer && transfer->kind == FixedLaneTransfer::Kind::Read
                                  ? physical_vgpr(transfer->lane.vgpr, kSrc0BankShift)
                                  : std::nullopt;
        if (transfer && src_phys) {
          auto slot = state.slots.find(VectorLaneSlot{*src_phys, transfer->lane.lane});
          if (slot != state.slots.end()) {
            read_halves[transfer->sgpr] = slot->second;
            active_read_halves.insert(transfer->sgpr);
          }
        }
        continue;
      }
...
[[nodiscard]] std::vector<IndirectCallFixup> discover_indirect_branch_edges_unfiltered(
    ...
    std::span<const uint64_t> extra_leaders, ExternalEntryPolicy entry_policy,
    std::vector<PcAddressBuilder> *pc_builders, uint32_t wavefront_size,
    std::span<const uint64_t> external_entries) {
  std::vector<IndirectCallFixup> recovered;
      ...
      if (std::ranges::none_of(iteration_recovered, [&](const IndirectCallFixup &candidate) {
        ...
        if (existing.source_call_carrier.cls != RegClass::SGPR) {
          ...
          if (consumer_block != blocks.end()) {
            ...
            // edge.
            if (!consumer_block->external_entry && predecessor_count == 1)
              continue;
          }
        }
  ...
  if (!has_indirect_consumer) {
    ...
    const auto unfiltered =
        discover_indirect_branch_edges_unfiltered(insts, text, arch, extra_leaders, entry_policy,
                                                  nullptr, /*wavefront_size=*/0, extra_leaders);
