...
struct LivenessAnalysisOptions {
  ...
  /// snapshots are stored for live_before()/find_free_*() queries. Keeping this
  /// false preserves the historical behavior of materializing every instruction.
  ...
  ///
  /// @details Callers can pass an empty span together with
  /// restrict_live_before_to_instructions=true to request no instruction-level
  /// snapshots. Pointers outside the analyzed block scope are ignored.
...
public:
  ...
  /// liveness for a dead run that is also encodable for the target instruction.
