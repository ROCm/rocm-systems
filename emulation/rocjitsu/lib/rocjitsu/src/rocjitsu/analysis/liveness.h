...
struct LivenessAnalysisOptions {
  ...
  /// @brief Primary kernel entry block where architectural VGPR_MSB state is zero.
  ///
  /// @details Retained for single-entry callers. When entry_blocks is non-empty,
  /// it is the complete set of independently callable ABI entries instead.
  ...
  /// scratch VGPRs available. Overlapping ranges use the smallest matching
  /// ceiling.
