...
///   intersect it, and an absent event clears it. Retirement establishes new
///   readiness only with this proof. Different producers on different paths
///   conservatively cannot establish readiness after the join.
...
/// readiness only when a producing generation has completed on every path.
...
struct WaitcheckStateOps : WaitcheckTarget {
  ...
  // Retirement proves a committed generation only for producers present on all
  // paths and after all counter facets of that generation retire. A different,
  // still-pending producer does not erase this older value: record it in both
