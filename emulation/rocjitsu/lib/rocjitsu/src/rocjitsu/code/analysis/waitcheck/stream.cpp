...
RegisterEffects register_effects(const Instruction &inst, rj_code_arch_t arch, uint32_t wave_size) {
  ...
  for (int i = 0; i < inst.num_src_operands(); ++i) {
    ...
    if (i == 0 && name.starts_with("scratch_") && !scratch_vector_address_enabled(inst, arch))
      continue;
    if (i == 0 && name.starts_with("s_buffer_") && ref->cls == RegClass::SGPR)
      ref->width = 4;
...
bool is_control_flow(const Instruction &inst) {
  return (inst.flags() & (BRANCH | COND_BRANCH | INDIRECT_BRANCH | INDIRECT_CALL)) != 0;
}
...
std::optional<std::string_view> deferred_reason(const Instruction &inst,
  ...
  if (name.starts_with("v_movrel") || name.starts_with("s_movrel"))
  ...
  if (name.starts_with("s_") && events.empty() && !inst.is_waitcnt() &&
      !(inst.flags() & PROGRAM_TERMINATOR) && name != "s_nop" && name != "s_delay_alu" &&
  ...
  util::Result check_dependencies(const Instruction &inst, const RegisterEffects &effects,
    ...
    for (const auto &pending : state_.pending) {
      for (const auto &event : pending) {
        auto dependency = event.regs - event.old_value_regs;
        std::optional<RegisterRef> reg;
        auto access = WaitcheckAccess::Read;
        if (event.check_uses)
          reg = first_intersection(dependency, effects.uses);
        if (!reg && event.check_defs) {
          if (allow_committed_waw && event.produces_regs)
            dependency -= effects.defs;
          if (!ordered_waw(event, current))
            reg = first_intersection(dependency, effects.defs);
