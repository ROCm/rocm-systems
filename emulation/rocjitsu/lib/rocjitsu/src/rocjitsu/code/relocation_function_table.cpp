...
[[nodiscard]] bool range_in_image(std::span<const uint8_t> image, uint64_t offset,
                                  uint64_t size) {
  ...
  if (mnemonic == "s_swap_pc_i64" || mnemonic == "s_swappc_b64") {
    if (dispatches != nullptr && dst_pair && src0_pair) {
      ...
      if (value != state.end() && value->second.kind == PairValueKind::TableEntry) {
        dispatches->push_back({.table_index = static_cast<size_t>(value->second.value),
...
[[nodiscard]] PairState meet_predecessors(
    const BasicBlock &block, const std::unordered_map<const BasicBlock *, size_t> &positions,
  ...
  for (const Elf64_Shdr &symtab : sections) {
    ...
    for (size_t index = 0; index < count; ++index) {
      ...
      if (duplicate == candidates.end())
        candidates.push_back({.vaddr = symbol.st_value,
                              .size = symbol.st_size,
                              .got_slots = {},
                              .entries = {}});
    }
  ...
  for (const Elf64_Shdr &relocations : sections) {
    ...
    for (size_t index = 0; index < count; ++index) {
      ...
      if (symbol_index >= linked_symbols->sh_size / sizeof(Elf64_Sym))
        continue;
      Elf64_Sym symbol{};
      if (!read_object(image, linked_symbols->sh_offset +
      ...
      const auto candidate = std::ranges::find_if(candidates, [&](const ObjectCandidate &item) {
        return item.vaddr == target;
      });
...
std::vector<RelocationTableDispatch> discover_relocation_table_dispatches(
  ...
  dispatches.erase(std::ranges::unique(dispatches, {}, [](const RelocationTableDispatch &item) {
                     return std::pair{item.source_call_offset, item.table_index};
                   }).begin(),
                   dispatches.end());
  return dispatches;
}
