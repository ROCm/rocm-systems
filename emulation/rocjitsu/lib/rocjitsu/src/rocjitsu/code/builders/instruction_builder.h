...
/// @brief Maximum byte distances encodable by SOPP's signed dword displacement.
...
/// @param prefer_literal64 On CDNA5, use the literal64 add form even when the delta would fit a
///        32-bit literal. The wide form leaves both halves writable if a later translation changes
///        the distance; a literal32 instruction cannot be widened in place. It costs one extra
///        word, so callers with a fixed-size window ask for it only when the window can hold it.
...
[[nodiscard]] inline constexpr uint32_t build_s_mov_b64_encoding(uint16_t sdst, uint16_t ssrc0,
                                                                 rj_code_arch_t arch) {
