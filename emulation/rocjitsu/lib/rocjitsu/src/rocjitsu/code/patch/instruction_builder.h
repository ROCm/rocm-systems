...
///
/// Retained for matching legacy instruction sequences. New instruction
/// emission should use the generated architecture builders.
...
/// @brief Get the s_wait_alu opcode for a target ISA.
[[nodiscard]] inline constexpr uint32_t sopp_op_wait_alu(rj_code_arch_t arch) {
  switch (arch) {
  case ROCJITSU_CODE_ARCH_RDNA4:
    return rdna4::kSWaitAluSopp;
  case ROCJITSU_CODE_ARCH_GFX1250:
    return gfx1250::kSWaitAluSopp;
  default:
    return rdna4::kSWaitAluSopp;
  }
}
...
/// @brief Append a canonical scalar builder for a relocated PC-relative target.
///
/// @details The current PC is captured into @p pc_sreg / @p pc_sreg+1. This
/// appends the smallest positive or negative scalar add/sub sequence needed to
/// turn that pair into the final relocated target.
...
[[nodiscard]] inline constexpr uint32_t build_s_wait_alu(uint16_t simm16, rj_code_arch_t arch) {
  return build_sopp_encoding(arch, sopp_op_wait_alu(arch), simm16);
}
