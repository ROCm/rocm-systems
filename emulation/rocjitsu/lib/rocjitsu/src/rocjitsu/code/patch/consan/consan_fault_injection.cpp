...
struct FaultApplicationState {
  explicit FaultApplicationState(const ConSanTransformArtifacts &result)
      : program_inventory(result.program_inventory), fault_sites(result.fault_sites),
        ...
        staged_text_fragments(result.staged_text_fragments), outcome(result.outcome),
        transform_failure_cause(result.transform_failure_cause), warnings(result.warnings),
  ...
  void note_event_mutation(ConSanProgramSiteId source_site, ConSanSyncEventMutationKind kind,
    ...
    synchronization_mutation.events.push_back({.source_event = synchronization.event_id(*event),
                                               .kind = kind,
                                               .relocated_text_offset = relocated_text_offset});
  }
...
select_ordinary_acquire_mutation_target(const ConSanFaultSelectionView &inventory,
  ...
  return select_ordinary_acquire_mutation_target(
      inventory, ConSanFaultSelection{
                     ...
                     .kernel_name_filter = debug.test_kernel_name_filter,
                     .ordinal = mutation.fault_ordinary_index,
                 });
}
...
resolve_exact_barrier_drop_group(const ConSanFaultSelectionView &inventory,
  ...
  return resolve_exact_barrier_drop_group(
      ...
      ConSanFaultSelection{
          ...
          .companion_sequence_identity = mutation.fault_barrier_companion_sequence_identity,
          .kernel_name_filter = debug.test_kernel_name_filter,
          .ordinal = 0,
      });
}
...
void resolve_consan_fault_mutations(const AmdGpuCodeObject &code_object, rj_code_arch_t arch,
  ...
  if (mutation.fault_drop_barrier && !has_only_rocclr_runtime_kernels(program_inventory)) {
    ...
    if (!mutation.fault_barrier_sequence_identity.empty() ||
      ...
      const bool grouped = !mutation.fault_barrier_companion_site_identity.empty() ||
                           !mutation.fault_barrier_companion_sequence_identity.empty();
      std::optional<ExactBarrierDropGroup> group;
      std::optional<ExactBarrierDropPair> pair;
      std::string reason;
      if (grouped) {
        const auto resolution = resolve_exact_barrier_drop_group(selection, mutation, debug);
        group = resolution.group;
        reason = exact_barrier_drop_group_issue_message(resolution.issue, resolution.member_issue);
      } else {
        const auto resolution = resolve_exact_barrier_drop_pair(selection, mutation, debug);
      ...
      if ((grouped && !group) || (!grouped && !pair)) {
        ...
        if (group) {
          ...
          if (!retain_member_identities(plan, group->first.sequence->member_event_ids))
            return;
          auto second = sync.sequence_member_identities(group->second.sequence->member_event_ids);
          if (!second) {
            result.errors.emplace_back(
                "ConSan fault planning could not resolve typed synchronization members");
            return;
          }
          plan.ordered_member_identities.insert(plan.ordered_member_identities.end(),
          ...
          if (!retain_member_identities(plan, pair->sequence->member_event_ids))
            return;
        }
        result.fault_plans.push_back(std::move(plan));
      }
    } else if (primary != nullptr &&
               is_member_of_qualified_logical_barrier_pair(selection, *primary) &&
      ...
      const auto companion =
          std::ranges::find_if(selection.fault_sites, [&](const auto &candidate) {
            return candidate.kind == ConSanFaultSiteKind::Barrier &&
                   ...
                   sync.find_unique_sequence_containing(candidate.source_site) == sequence;
          });
      if (companion == selection.fault_sites.end()) {
        result.warnings.emplace_back(
        ...
        if (mutation.fault_barrier_move_direction == ConSanBarrierMoveDirection::LegacyMarker) {
          ...
          result.fault_plans.push_back(std::move(plan));
        } else if (mutation.fault_barrier_destination_identity.empty()) {
          result.warnings.emplace_back(
          ...
          if (destination == artifacts.barrier_move_destinations.end()) {
            result.warnings.emplace_back(
                ...
                mutation.fault_barrier_destination_identity + "'");
          } else if (!owned_by_requested_kernel(destination->execution_owners, selection, debug)) {
            result.warnings.emplace_back(
            ...
            targets.move_companion = &*companion;
            targets.move_sequence = sequence;
            targets.move_destination = &*destination;
  ...
  if (mutation.fault_mutate_barrier_id_scope) {
      ...
      if (selected == sync.sync_sequences.end()) {
        reject_application(
            "ConSan barrier ID/scope dry run found no logical sequence with exact identity '" +
                mutation.fault_barrier_sequence_identity + "'",
    ...
    if (sequence == nullptr) {
      ...
      if (members[0] == nullptr || members[1] == nullptr) {
        ...
        if (group != nullptr && !group_identity) {
          result.errors.emplace_back(
              "ConSan fault planning could not resolve barrier lifecycle initialization");
          return;
        }
        ...
        plan.target_barrier_id = mutation.fault_barrier_target_id;
  ...
  if (mutation.fault_mutate_barrier_participants) {
    ...
    if (mutation.fault_barrier_target_participant_mask) {
    ...
    } else if (!mutation.fault_barrier_target_participant_count ||
               *mutation.fault_barrier_target_participant_count == 0u ||
               *mutation.fault_barrier_target_participant_count > 63u ||
               *mutation.fault_barrier_target_participant_count == *init->participant_count) {
      mark_participant_mutation_unsupported(
          result, "the target count must be a distinct value in the encodable range [1, 63]");
    } else {
      targets.participant_group = group;
      targets.participant_init = init;
      auto identities = sync.sequence_member_identities(group->member_event_ids);
      ...
      plan.logical_sequence_identity = *group_identity;
      plan.ordered_member_identities = std::move(*identities);
      plan.original_barrier_id = init->barrier_id;
      plan.target_barrier_id = init->barrier_id;
      plan.original_barrier_scope = init->scope;
      plan.target_barrier_scope = init->scope;
      result.fault_plans.push_back(std::move(plan));
    }
  }
    ...
    if (atomic == nullptr) {
      if (!mutation.fault_site_identity.empty()) {
        result.warnings.emplace_back("ConSan atomic fault found no site with exact identity '" +
                                     mutation.fault_site_identity + "'");
      } else {
        result.warnings.emplace_back(
            "ConSan atomic fault found no patchable flat/global atomic at index " +
            std::to_string(mutation.fault_atomic_index));
      }
    }
  }
  if (mutation.fault_atomic_wrong_address) {
    ...
    } else if (mutation.fault_atomic_address_delta == 0u ||
               mutation.fault_atomic_address_delta % sizeof(uint32_t) != 0u ||
               mutation.fault_atomic_address_delta > 0x7fffffu) {
      result.errors.emplace_back(
  ...
  if (mutation.fault_atomic_weaken_order) {
    ...
    if (atomic == nullptr) {
    ...
    } else if (atomic_order_boundary.fence == nullptr &&
      ...
      plan.companion_identity =
          atomic_order_boundary.fence != nullptr
              ...
              : std::optional<std::string>(atomic_order_boundary.sequence->identity +
                                           "|release-wait-boundary");
      plan.logical_sequence_identity = atomic_order_boundary.sequence->identity;
      if (!retain_member_identities(plan, atomic_order_boundary.sequence->member_event_ids))
        return;
      result.fault_plans.push_back(std::move(plan));
    }
  }
  ...
  if (mutation.fault_ordinary_wrong_address || mutation.fault_ordinary_weaken_order ||
      mutation.fault_ordinary_weaken_scope) {
    const auto target = select_ordinary_acquire_mutation_target(selection, mutation, debug);
    if (!target) {
      result.warnings.emplace_back(
  ...
  if (apply && artifacts.errors.empty()) {
    ...
    artifacts.transform_failure_cause = transaction.transform_failure_cause;
    if (!accepted) {
      artifacts.mutation.fault.applied = mechanisms_succeeded
                                             ? transaction.mutation.fault.applied
                                             : applied_fault_mutation_count(artifacts.patches);
      return;
    }
    artifacts.warnings = std::move(transaction.warnings);
    artifacts.outcome = transaction.outcome;
    artifacts.replacement = std::move(transaction.replacement);
    artifacts.patches = std::move(transaction.patches);
    artifacts.mutation = std::move(transaction.mutation);
    artifacts.synchronization_mutation = std::move(transaction.synchronization_mutation);
    artifacts.staged_text_fragments = std::move(transaction.staged_text_fragments);
    if (!artifacts.staged_text_fragments.empty()) {
      const std::span<const uint8_t> descriptor_image =
          artifacts.replacement.empty()
      ...
      artifacts.mutation.fault.applied = applied_fault_mutation_count(artifacts.patches);
    }
  }
}
...
try_apply_barrier_id_scope_fault_patch(const AmdGpuCodeObject &code_object,
  ...
  const std::optional<std::string> lifecycle_identity =
      lifecycle_group != nullptr ? sync.barrier_lifecycle_identity(*lifecycle_group) : std::nullopt;
  if (lifecycle_group != nullptr && !lifecycle_identity) {
    result.errors.emplace_back(
        "ConSan barrier ID/scope mutation lost its lifecycle initialization");
    return;
  }
  ...
  for (const ConSanSyncEvent *member : members) {
    ...
    if (source.operand_source == ConSanBarrierSite::OperandSource::Immediate) {
      ...
      std::memcpy(replacement.data() + source.file_offset + sizeof(uint32_t), &literal,
                  sizeof(literal));
    }
  }
  ...
  result.mark_modified();
  result.mutation.applied_fault_logical_identity = lifecycle_identity.value_or(selected->identity);
  result.warnings.emplace_back(
...
static void try_apply_barrier_participant_fault_patch(const AmdGpuCodeObject &code_object,
  ...
  if (group == nullptr || init == nullptr)
    return;
  const std::optional<std::string> group_identity =
      result.program_inventory.sync().barrier_lifecycle_identity(*group);
  if (!group_identity) {
    result.errors.emplace_back(
        "ConSan barrier participant mutation lost its lifecycle initialization");
    return;
  }
  ...
  replacement_literal =
      (replacement_literal & ~kParticipantCountMask) | (target_participant_count << 16u);
  result.replacement.assign(original_bytes.begin(), original_bytes.end());
  ...
  patch.target_participant_count = target_participant_count;
  result.patches.push_back(std::move(patch));
  result.mark_modified();
  result.mutation.applied_fault_logical_identity = *group_identity;
  result.warnings.emplace_back("ConSan barrier participant mutation rewrote one proven static "
                               ...
                               std::to_string(target_participant_count));
}
...
static void try_apply_barrier_move_fault_patch(const AmdGpuCodeObject &code_object,
  ...
  const auto original_bytes = std::span<const uint8_t>(
      reinterpret_cast<const uint8_t *>(code_object.image_data()), code_object.image_size());
  if (targets.move_primary == nullptr || targets.move_companion == nullptr ||
  ...
  if (direction != ConSanBarrierMoveDirection::LegacyMarker) {
    ...
    for (const ConSanFaultSite *member : {targets.move_primary, targets.move_companion}) {
      ...
      if (event == nullptr) {
        result.errors.emplace_back(
            "ConSan markerless barrier move lost its synchronization-event identity");
        return;
      }
      target_fragment.sync_relocations.push_back(
          {.source_event = synchronization.event_id(*event),
           .fragment_relative_offset = static_cast<uint32_t>(
...
static void try_apply_atomic_fault_patch(const AmdGpuCodeObject &code_object, rj_code_arch_t arch,
  ...
  std::memcpy(words.data(), result.replacement.data() + source->decoded_file_offset(),
              source->size());
  std::span<uint8_t> atomic_instruction(reinterpret_cast<uint8_t *>(words.data()), source->size());
  if (mutate_address) {
  ...
  if (mutate_order) {
    ...
    if (fence != nullptr) {
      ...
      for (uint32_t offset = 0; offset < fence_source->size; offset += sizeof(uint32_t))
        std::memcpy(result.replacement.data() + fence_source->file_offset + offset, &nop,
                    sizeof(nop));
  ...
  if (mutate_scope) {
    ...
    result.warnings.emplace_back("ConSan atomic fault lowered " +
                                 std::string(source->mnemonic_view()) + " scope from " +
                                 std::to_string(rewrite.previous_value) + " to wave");
  }
  std::memcpy(result.replacement.data() + source->decoded_file_offset(), words.data(),
              source->size());
  result.mark_modified();
  if (mutate_address) {
    result.warnings.emplace_back("ConSan atomic fault rewrote " +
                                 std::string(source->mnemonic_view()) +
                                 " with validated address delta " + std::to_string(address_delta));
  }
}
...
static void try_apply_lds_fault_patch(const AmdGpuCodeObject &code_object, rj_code_arch_t arch,
  ...
  const uint16_t target_address_vgpr = *requested_target_address_vgpr;
  if (target_address_vgpr == *address_vgpr) {
    result.errors.emplace_back(
        "ConSan LDS address fault requires a distinct 8-bit replacement VGPR");
    return;
  }
  ...
  for (const ConSanExecutionOwner &owner : execution_owners) {
    ...
    const auto allocated =
        read_descriptor_vgpr_allocation_count(original_bytes, kernel->descriptor_file_offset, arch);
    if (!allocated) {
      result.errors.emplace_back(
  ...
  if (source->decoded_file_offset() > original_bytes.size() ||
      source->size() > original_bytes.size() - source->decoded_file_offset()) {
  ...
  info.original_size = source->size();
  info.fault_primary_identity = selected->identity;
  info.fault_original_address_vgpr = address_vgpr;
  info.fault_target_address_vgpr = target_address_vgpr;
  info.owner_descriptor_file_offsets =
      result.program_inventory.execution_owner_descriptors(execution_owners);
  result.patches.push_back(std::move(info));
  result.mutation.applied_fault_logical_identity = selected->identity;
  result.mark_modified();
  result.warnings.emplace_back("ConSan LDS fault rewrote " + std::string(source->mnemonic_view()) +
                               " address from v" + std::to_string(*address_vgpr) + " to v" +
                               std::to_string(target_address_vgpr));
}

static void try_apply_ordinary_fault_patch(const AmdGpuCodeObject &code_object,
                                           const OrdinaryAcquireMutationTarget *target,
                                           bool mutate_address, bool mutate_order,
                                           bool mutate_scope, uint32_t address_delta,
                                           FaultApplicationState &result) {
  if (!target) {
  ...
  if (site_source == nullptr || site_source->decoded_file_offset() > candidate.size() ||
      ...
      cache_source->file_offset > candidate.size() ||
      cache_source->size > candidate.size() - cache_source->file_offset) {
  ...
  const auto make_info = [&](ConSanPatchKind kind, uint64_t anchor, uint32_t size,
    ...
    info.owner_descriptor_file_offsets = result.program_inventory.execution_owner_descriptors(
        result.execution_owners(*target->site));
    return info;
  };
  ...
  if (mutate_address) {
    result.warnings.emplace_back("ConSan ordinary fault shifted global_load_b32 by validated " +
                                 std::to_string(address_delta) + " byte(s)");
  }
  if (mutate_order) {
    result.warnings.emplace_back(
...
static bool apply_resolved_consan_fault_mutations(const AmdGpuCodeObject &code_object,
  ...
  if (targets.move_primary != nullptr && transaction.errors.empty()) {
    try_apply_barrier_move_fault_patch(code_object, arch, targets,
                                       mutation.fault_barrier_move_direction, transaction);
  }
  if (targets.retarget_sequence != nullptr && transaction.errors.empty()) {
    try_apply_barrier_id_scope_fault_patch(
        code_object, targets.retarget_sequence, targets.retarget_group,
  ...
  if ((targets.atomic_address || targets.atomic_order || targets.atomic_scope) &&
      transaction.errors.empty()) {
    try_apply_atomic_fault_patch(
        code_object, arch, targets.atomic, targets.atomic_encoding, targets.atomic_order_boundary,
        targets.atomic_address, targets.atomic_order, targets.atomic_scope,
        mutation.fault_atomic_address_delta, mutation.fault_atomic_order_edge, transaction);
  }
  if (targets.lds != nullptr && transaction.errors.empty()) {
    try_apply_lds_fault_patch(code_object, arch, targets.lds, mutation.fault_lds_address_vgpr,
                              transaction);
  }

  if (targets.ordinary && transaction.errors.empty()) {
    try_apply_ordinary_fault_patch(
        code_object, targets.ordinary ? &*targets.ordinary : nullptr,
        mutation.fault_ordinary_wrong_address, mutation.fault_ordinary_weaken_order,
        mutation.fault_ordinary_weaken_scope, mutation.fault_ordinary_address_delta, transaction);
  }
  transaction.mutation.fault.applied =
...
[[nodiscard]] BarrierSite barrier_site_from_inventory(const ProgramInventory &inventory,
  ...
  if (source == nullptr || container == nullptr)
    return {};
  return {.text_offset = source->text_offset(),
          .file_offset = source->decoded_file_offset(),
          .size = source->size(),
          .range_name =
              std::string(container->is_kernel() ? "kernel:" : "function:") + container->name,
