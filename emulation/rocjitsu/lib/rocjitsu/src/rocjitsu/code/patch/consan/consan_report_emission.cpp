...
RecordEmitter::RecordEmitter(std::vector<uint32_t> &words, uint16_t address_vgpr,
...
bool RecordEmitter::materialize_address(uint64_t address) {
...
bool RecordEmitter::load(uint32_t offset, uint16_t destination_vgpr) {
  return append_load_u32_vgpr_at_offset(words_, address_vgpr_, offset, destination_vgpr, arch_);
}

bool RecordEmitter::store_vgpr(uint32_t offset, uint16_t value_vgpr) {
  return append_store_u32_vgpr_at_offset(words_, address_vgpr_, offset, value_vgpr, arch_);
}

bool RecordEmitter::store_literal(uint32_t offset, uint32_t value) {
...
bool RecordEmitter::store_sgpr(uint32_t offset, uint16_t value_sgpr) {
  words_.push_back(build_v_mov_b32_e32(temporary_vgpr_, value_sgpr, arch_));
  return store_vgpr(offset, temporary_vgpr_);
}

bool RecordEmitter::store_private(uint32_t offset, uint32_t private_offset) {
...
bool RecordEmitter::store_workgroup(uint32_t offset, const WorkgroupSource &source,
  ...
  return detail::append_workgroup_source_value(words_, source, temporary_vgpr_, arch_) &&
         store_vgpr(offset, temporary_vgpr_);
}

namespace {

ReportDispatchIdSource plan_dispatch_id_sources(const ReportDispatchIdPlanningContext &context,
                                                bool literal_permitted) {
  return {
      .sgpr = context.point.dispatch_sgpr.base(),
      .private_offset = context.private_offset,
      .literal = context.point.dispatch_sgpr.base() || context.private_offset || !literal_permitted
                     ? std::nullopt
                     : std::optional<uint64_t>{context.resources.report_dispatch_id},
  };
}

} // namespace

ReportDispatchIdSource target_dispatch_id_sources(const ReportDispatchIdPlanningContext &context,
                                                  rj_code_arch_t arch) {
  const TargetProfile *target = target_profile(arch);
  return plan_dispatch_id_sources(context, target && target->dispatch_identity ==
                                                         DispatchIdentitySource::CodeObjectLiteral);
}

ReportDispatchIdSource bound_dispatch_id_sources(const ReportDispatchIdPlanningContext &context) {
  return plan_dispatch_id_sources(context, true);
}

bool append_report_dispatch_id_word(std::vector<uint32_t> &words,
                                    const ReportDispatchIdSource &source, uint16_t destination_vgpr,
...
bool append_report_dispatch_id_pair(std::vector<uint32_t> &words,
                                    const ReportDispatchIdSource &source, uint16_t low_vgpr,
                                    uint16_t high_vgpr, rj_code_arch_t arch) {
  return append_report_dispatch_id_word(words, source, low_vgpr,
                                        /*high_word=*/false, arch) &&
         append_report_dispatch_id_word(words, source, high_vgpr,
                                        /*high_word=*/true, arch);
}

bool append_compare_report_dispatch_id_word(std::vector<uint32_t> &words,
                                            const ReportDispatchIdSource &source,
                                            uint16_t value_vgpr,
  ...
  if (source.sgpr) {
    ...
    if (clobberable_literal_temporary_vgpr == value_vgpr ||
        !append_report_dispatch_id_word(words, source, clobberable_literal_temporary_vgpr,
                                        high_word, arch)) {
      return false;
    }
...
bool append_store_report_dispatch_id_pair(RecordEmitter &record,
                                          const ReportDispatchIdSource &source,
                                          uint32_t low_offset) {
  if (!source.is_well_formed())
    return false;
