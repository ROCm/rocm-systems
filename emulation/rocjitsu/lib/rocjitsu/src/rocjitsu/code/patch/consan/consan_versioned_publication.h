...
/// @file consan_versioned_publication.h
/// @brief Shared lane-state algebra for bounded ConSan publication transactions.
...
class PublicationExec {
public:
  PublicationExec(InstructionSequence &sequence, uint16_t narrow_save, rj_code_arch_t arch)
...
[[nodiscard]] inline bool append_publication_loads(std::vector<uint32_t> &words,
                                                   uint16_t slot_address_vgpr,
                                                   std::initializer_list<PublicationField> fields,
                                                   rj_code_arch_t arch) {
  for (const PublicationField field : fields) {
    if (!detail::append_load_u32_vgpr_at_offset(words, slot_address_vgpr, field.offset, field.vgpr,
                                                arch)) {
      return false;
    }
  }
...
[[nodiscard]] inline bool append_publication_stores(std::vector<uint32_t> &words,
                                                    uint16_t slot_address_vgpr,
                                                    std::initializer_list<PublicationField> fields,
                                                    rj_code_arch_t arch) {
  for (const PublicationField field : fields) {
    if (!detail::append_store_u32_vgpr_at_offset(words, slot_address_vgpr, field.offset, field.vgpr,
                                                 arch)) {
      return false;
    }
  }
...
append_version_transition(std::vector<uint32_t> &words, InstructionSequence &sequence,
                          PublicationExec &exec, uint16_t slot_address_vgpr,
  ...
  if (!desired || !expected ||
      ...
      !append_global_atomic_wait(words, arch)) {
    return false;
  }
...
[[nodiscard]] bool
append_bounded_version_claim(std::vector<uint32_t> &words, InstructionSequence &sequence,
                             PublicationExec &exec, const VersionClaim &claim,
  ...
  if (!exec.restore(claim.eligible_exec_sgpr) || !append_candidate() ||
      ...
      !append_version_transition(words, sequence, exec, claim.slot_address_vgpr,
                                 claim.version_offset, claim.base_version_vgpr, claim.desired_vgpr,
