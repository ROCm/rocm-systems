...
append_cdna_full_workgroup_payload_restore(std::vector<uint32_t> &words,
  ...
  for (const auto &[destination, source] : *copies) {
    if (destination != source &&
      ...
      errors.emplace_back("ConSan MOI could not restore a guest AMDHSA workgroup payload SGPR");
      return false;
    }
  }
  return true;
...
build_moi_dense_barrier_entry_island(uint64_t island_text_offset, uint64_t dispatcher_text_offset,
  ...
  if (!append_moi_scc_preserving_indirect_jump(words, island_text_offset, dispatcher_text_offset,
                                               ...
                                               /*capture_scc=*/!derive_key_at_entry, arch) ||
      words.size() != moi_dense_entry_island_words(derive_key_at_entry, key_encodes_scc)) {
    return std::nullopt;
  }
  return words;
}
...
void try_apply_private_epoch_prologue_patch(const MoiOptions &options, rj_code_arch_t arch,
  ...
  for (const PlannedPrivateEpochPrologue &item : planned) {
    ...
    auto words = build_private_epoch_prologue_words(
        prologue_text_offset, item.active_entry_text_offset, item.emission, arch, result.errors);
    if (!words)
      return;
    ...
    if (item.has_kernarg_preload) {
      ...
      if (words->size() * sizeof(uint32_t) <= kAmdhsaKernelEntryAlignment &&
        ...
        words =
            build_private_epoch_prologue_words(*primary_body_offset, item.active_entry_text_offset,
                                               item.emission, arch, result.errors);
        if (!words)
          return;
...
[[nodiscard]] bool
moi_entry_scalar_backup_preserves_persistent_outputs(const ConSanMoiOperatingPoint &point,
...
[[nodiscard]] bool moi_owner_epoch_prologue_uses_vgpr(
  ...
  const uint16_t epoch_width =
      workgroup_shadow
          ? static_cast<uint16_t>(point.automatic_moi_persistent_vgprs && target != nullptr
                                      ? moi_workgroup_shadow_preferred_zero_vgpr_count(*target)
                                      : 2u)
          : 1u;
...
void try_apply_owner_epoch_prologue_patch(
  ...
  for (const PlannedOwnerEpochPrologue &item : target_kernels) {
    ...
    bool instrument_entry_in_place = item.instrument_entry_in_place;
