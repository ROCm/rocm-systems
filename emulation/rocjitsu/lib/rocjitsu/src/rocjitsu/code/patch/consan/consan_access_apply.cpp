...
make_access_attribution(const ObservationPlan &observation, const Candidate &candidate) {
  ...
  for (const ExecutionOwner &owner : candidate.site().execution_owners)
...
assemble_appended_body(const AppendedBodyPatchPlan &plan, std::span<const uint32_t> probe_words,
                       std::string_view probe_name, std::vector<std::string> &errors,
                       const AppendedBodyOptions &options) {
  std::vector<uint32_t> body;
  if (options.trailing_guest_word_count > probe_words.size()) {
    errors.emplace_back("ConSan " + std::string(probe_name) +
  ...
  if (!appended_body_vgpr_bank_mode_is_valid(options.arch, options.incoming_vgpr_bank_mode)) {
    errors.emplace_back("ConSan " + std::string(probe_name) +
  ...
  const bool select_low_vgpr_bank =
      appended_body_uses_selectable_vgpr_bank(options.arch, options.incoming_vgpr_bank_mode);
  ...
  const bool valid_embedded_guest_plan = embedded_guest_vgpr_bank_plan_is_valid(
  ...
  if (plan.body_size != 0u &&
    ...
    errors.emplace_back(
        "ConSan " + std::string(probe_name) + " body size changed after placement: emitted=" +
...
[[nodiscard]] bool scalar_spill_requires_dynamic_vgpr_frame(const ProgramInventory &inventory,
                                                            const ResolvedScratchPlan &resources,
                                                            const OperatingPoint &point) {
  if (!point.has_scalar_spill()) {
    return false;
  }
  return std::ranges::any_of(resources.owner_descriptor_file_offsets, [&](uint64_t offset) {
    const ProgramContainer *kernel = inventory.find_kernel_by_descriptor(offset);
...
/// Mode policy decides whether scalar spilling is enabled, but no mode
/// privately interprets the owner stack model or reservoir geometry.
[[nodiscard]] std::optional<SgprSpillSequence> build_sgpr_spill_sequence(
    const ProgramInventory &inventory, const ResolvedScratchPlan &resources, const Request &request,
    const BoundRuntimeResources &bound_resources, const OperatingPoint &point,
    SpillManagers &managers, rj_code_arch_t arch, std::vector<std::string> &warnings,
    std::optional<uint32_t> private_layout_base, const VgprSpillSequence *vgpr_spill) {
  if ((!point.has_scalar_spill()) || !point.exec_save_sgpr)
    return std::nullopt;
  if (!is_capability_arch(arch) || resources.owner_descriptor_file_offsets.empty()) {
    warnings.emplace_back(
        "ConSan spill-backed scalar state requires at least one supported kernel owner");
    return std::nullopt;
  }
  const auto private_limit = address_free_private_limit(arch);
  if (!private_limit) {
    warnings.emplace_back("ConSan spill-backed Inline scalar state has no private capacity");
    return std::nullopt;
  }
  const uint16_t count =
      exec_save_sgpr_count(resolve_exec_save_requirement(request, bound_resources, point), arch);
  if (scalar_spill_requires_dynamic_vgpr_frame(inventory, resources, point)) {
    if (vgpr_spill == nullptr || !vgpr_spill->uses_dynamic_stack_frame) {
      warnings.emplace_back(
          "ConSan dynamic-stack scalar spill requires an established VGPR spill frame");
      return std::nullopt;
    }
    auto sequence = build_dynamic_stack_sgpr_spill_sequence(
        *point.exec_save_sgpr, count, resources.base, *vgpr_spill,
        static_cast<uint32_t>(resources.count) * SpillManager::kSlotBytes, arch);
    if (!sequence)
      warnings.emplace_back("ConSan could not encode the dynamic-stack scalar spill sequence");
    return sequence;
  }
  (void)managers;
  ...
  if (vgpr_spill != nullptr) {
    if (vgpr_spill->uses_dynamic_stack_frame || vgpr_spill->vgpr_count == 0u) {
      warnings.emplace_back("ConSan fixed-stack scalar spill has no preserved transfer VGPR");
      return std::nullopt;
    }
    ...
    auto sequence = build_sgpr_spill_sequence(scalar_manager, *point.exec_save_sgpr, count,
                                              transfer_vgpr, arch);
    if (!sequence) {
      warnings.emplace_back("ConSan could not encode the private-memory scalar spill sequence");
      return std::nullopt;
    }
    return sequence;
  }
  ...
  if (reservoir_end > REGISTER_SET_MAX_VGPRS || resources.required_vgpr_count < reservoir_end) {
    warnings.emplace_back(
        "ConSan fixed-stack scalar spill has no planned lane-backed VGPR reservoir");
    return std::nullopt;
  }
  ...
  auto sequence = build_lane_sgpr_spill_sequence(*point.exec_save_sgpr, count, reservoir_vgpr,
                                                 total_private_bytes, arch);
  if (!sequence) {
    warnings.emplace_back("ConSan could not encode the lane-backed scalar spill sequence");
    return std::nullopt;
  }
  return sequence;
}
