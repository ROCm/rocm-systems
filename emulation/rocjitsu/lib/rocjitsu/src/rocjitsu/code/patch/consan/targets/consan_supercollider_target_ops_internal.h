...
build_cdna3_cdna4_ds_load_word0(const AccessLoweringForm &form, uint32_t original_word0);

[[nodiscard]] std::optional<SuperColliderDirectToLdsTransfer>
build_cdna3_cdna4_direct_to_lds_transfer(std::array<uint32_t, 2> original_words,
                                         uint32_t width_bits, uint16_t address_vgpr,
                                         uint16_t payload_vgpr, uint16_t readback_vgpr,
                                         rj_code_arch_t arch);

[[nodiscard]] std::optional<uint32_t>
build_rdna3_rdna4_cdna5_ds_load_word0(const AccessLoweringForm &form, uint32_t original_word0);
