...
[[nodiscard]] std::optional<uint32_t> build_sgpr_to_vgpr_move(uint16_t vdst, uint16_t ssrc,
  ...
  if (arch == ROCJITSU_CODE_ARCH_RDNA3)
    return build_rdna3_v_mov_b32(vdst, ssrc, arch);
...
std::optional<VgprSpillSequence> build_vgpr_spill_sequence(SpillManager &manager,
                                                           ...
                                                           rj_code_arch_t arch) {
  if ((arch != ROCJITSU_CODE_ARCH_RDNA3 && !is_rdna4_family_arch(arch) &&
  ...
  const auto wait_store = arch == ROCJITSU_CODE_ARCH_RDNA3   ? build_rdna3_s_wait_vmcnt0(arch)
  ...
  const auto wait_load = arch == ROCJITSU_CODE_ARCH_RDNA3   ? build_rdna3_s_wait_vmcnt0(arch)
                         : arch == ROCJITSU_CODE_ARCH_CDNA3 ? build_cdna3_s_wait_vmcnt0(arch)
                         : arch == ROCJITSU_CODE_ARCH_CDNA4 ? build_cdna4_s_wait_vmcnt0(arch)
  ...
  for (uint16_t i = 0; i < vgpr_count; ++i) {
    ...
    if (arch == ROCJITSU_CODE_ARCH_RDNA3) {
      const auto store = build_rdna3_address_free_scratch_store_b32(vgpr, *offset, arch);
      const auto load = build_rdna3_address_free_scratch_load_b32(vgpr, *offset, arch);
      if (!store || !load)
        return std::nullopt;
...
std::optional<SgprSpillSequence> build_sgpr_spill_sequence(SpillManager &manager,
                                                           uint16_t sgpr_base, uint16_t sgpr_count,
                                                           uint16_t transfer_vgpr,
                                                           rj_code_arch_t arch) {
  if ((arch != ROCJITSU_CODE_ARCH_RDNA3 && !is_rdna4_family_arch(arch) &&
       arch != ROCJITSU_CODE_ARCH_CDNA3 && arch != ROCJITSU_CODE_ARCH_CDNA4) ||
      sgpr_count == 0 || static_cast<uint32_t>(sgpr_base) + sgpr_count > REGISTER_SET_MAX_SGPRS ||
      transfer_vgpr >= REGISTER_SET_MAX_VGPRS) {
    return std::nullopt;
  }

  SpillManager planned_manager = manager;
  if (!planned_manager.allocate_slots(RegisterRef{RegClass::SGPR, sgpr_base, 1u}, sgpr_count))
    return std::nullopt;
  const auto required_private_bytes =
      normalize_address_free_scratch_private_size(arch, planned_manager.total_private_bytes());
  const auto wait_store = arch == ROCJITSU_CODE_ARCH_RDNA3   ? build_rdna3_s_wait_vmcnt0(arch)
  ...
  sequence.restore_words.reserve(static_cast<size_t>(sgpr_count) * 5u + 1u);
  ...
  for (uint16_t i = 0; i < sgpr_count; ++i) {
    ...
    if (arch == ROCJITSU_CODE_ARCH_RDNA3) {
      ...
      const auto store = build_cdna3_address_free_scratch_store_b32(transfer_vgpr, *offset, arch);
      const auto load = build_cdna3_address_free_scratch_load_b32(transfer_vgpr, *offset, arch);
      if (!store || !load)
        return std::nullopt;
      sequence.save_words.insert(sequence.save_words.end(), store->begin(), store->end());
      sequence.restore_words.insert(sequence.restore_words.end(), load->begin(), load->end());
    } else if (arch == ROCJITSU_CODE_ARCH_CDNA4) {
      const auto store = build_cdna4_address_free_scratch_store_b32(transfer_vgpr, *offset, arch);
      const auto load = build_cdna4_address_free_scratch_load_b32(transfer_vgpr, *offset, arch);
      if (!store || !load)
        return std::nullopt;
      sequence.save_words.insert(sequence.save_words.end(), store->begin(), store->end());
      sequence.restore_words.insert(sequence.restore_words.end(), load->begin(), load->end());
    } else {
      const auto store = build_address_free_scratch_store_b32(transfer_vgpr, *offset, arch);
      const auto load = build_address_free_scratch_load_b32(transfer_vgpr, *offset, arch);
      if (!store || !load)
        return std::nullopt;
      sequence.save_words.insert(sequence.save_words.end(), store->begin(), store->end());
      sequence.restore_words.insert(sequence.restore_words.end(), load->begin(), load->end());
  ...
  // v_readfirstlane_b32 is a VALU-to-SALU transfer.  The restored SGPRs may
  // be consumed immediately after this transaction (including as report
  // pointers), so make the target-specific dependency separation part of the
  // spill contract just as the lane-backed scalar spill does.
  sequence.restore_words.push_back(*restore_dependency_wait);
  sequence.save_words.push_back(*wait_store);
...
build_dynamic_stack_vgpr_spill_sequence(uint16_t vgpr_base, uint16_t vgpr_count,
  ...
  const auto capture_scc =
      arch == ROCJITSU_CODE_ARCH_RDNA3
          ? build_rdna3_s_cselect_b32(saved_scc_sgpr, scalar_positive_inline_u32(1),
  ...
  const auto advance_stack =
      arch == ROCJITSU_CODE_ARCH_RDNA3   ? build_rdna3_s_add_u32(stack_top_sgpr, stack_top_sgpr,
  ...
  const auto restore_scc =
      arch == ROCJITSU_CODE_ARCH_RDNA3
          ? build_rdna3_s_cmp_lg_u32(saved_scc_sgpr, scalar_positive_inline_u32(0), arch)
      : arch == ROCJITSU_CODE_ARCH_CDNA3
...
build_dynamic_stack_borrowed_sgpr_spill_sequence(uint16_t vgpr_base, uint16_t vgpr_count,
  ...
  if ((arch != ROCJITSU_CODE_ARCH_RDNA3 && !is_rdna4_family_arch(arch)) ||
      vgpr_count < kReservoirCount || vgpr_end > REGISTER_SET_MAX_VGPRS ||
      scalar_reservoir_vgpr_base < vgpr_base || reservoir_end > vgpr_end ||
      borrowed_end > REGISTER_SET_ALLOCATABLE_SGPRS ||
  ...
  const auto capture_scc =
      arch == ROCJITSU_CODE_ARCH_RDNA3
          ? build_rdna3_s_cselect_b32(borrowed_sgpr_base, scalar_positive_inline_u32(1),
                                      scalar_positive_inline_u32(0), arch)
          : build_rdna4_s_cselect_b32(borrowed_sgpr_base, scalar_positive_inline_u32(1),
                                      scalar_positive_inline_u32(0), arch);
  const auto advance_stack = arch == ROCJITSU_CODE_ARCH_RDNA3
                                 ? build_rdna3_s_add_u32(kDynamicStackTopSgpr, kDynamicStackTopSgpr,
                                                         /*literal source=*/255u, arch)
