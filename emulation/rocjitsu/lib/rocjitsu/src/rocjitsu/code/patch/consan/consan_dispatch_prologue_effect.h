...
/// @file consan_dispatch_prologue_effect.h
...
/// register overflow.
enum class DispatchIdPreloadSupport : uint8_t {
...
struct DispatchIdPreloadPlan {
  DispatchIdPreloadSupport support = DispatchIdPreloadSupport::InvalidDispatchPosition;
  ...
  [[nodiscard]] constexpr bool supported() const {
    return support == DispatchIdPreloadSupport::SupportedAlreadyEnabled ||
           support == DispatchIdPreloadSupport::SupportedInsert;
  }
  [[nodiscard]] constexpr bool descriptor_change_required() const {
    return support == DispatchIdPreloadSupport::SupportedInsert;
  }
  [[nodiscard]] constexpr bool requires_kernarg_reload() const { return kernarg_reload_count != 0; }

  bool operator==(const DispatchIdPreloadPlan &) const = default;
};
...
class DispatchIdCapture {
public:
  DispatchIdCapture() = default;

  [[nodiscard]] static DispatchIdCapture in_sgprs(uint16_t base) {
    return DispatchIdCapture{Kind::Sgpr, base};
  }
  [[nodiscard]] static DispatchIdCapture in_vgprs(uint16_t base) {
    return DispatchIdCapture{Kind::Vgpr, base};
  }
  ...
  bool operator==(const DispatchIdCapture &) const = default;

private:
  enum class Kind : uint8_t { None, Sgpr, Vgpr };

  DispatchIdCapture(Kind kind, uint16_t base) : kind_(kind), base_(base) {}
...
struct DispatchIdPrologueEffect {
  DispatchIdPreloadPlan preload;
  ...
  bool operator==(const DispatchIdPrologueEffect &) const = default;
};
