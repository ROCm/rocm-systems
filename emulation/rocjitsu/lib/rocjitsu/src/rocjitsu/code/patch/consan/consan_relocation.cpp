...
decode_relocatable_entry_instruction(std::span<const uint8_t> text, uint64_t offset,
  ...
  if (!decoder) {
    errors.emplace_back("ConSan could not create an entry-instruction decoder");
    return std::nullopt;
  }
  const auto decode_one = [&](uint64_t instruction_offset)
    ...
    if (instruction_offset > text.size() || sizeof(uint32_t) > text.size() - instruction_offset) {
      errors.emplace_back("ConSan kernel entry instruction exceeds .text");
      return std::nullopt;
    }
    ...
    std::unique_ptr<Instruction> instruction = decode_bounded_instruction(
        *decoder, std::span<const uint32_t>(decode_words).first(available / sizeof(uint32_t)),
        instruction_offset);
    if (!instruction) {
      errors.emplace_back("ConSan could not decode the kernel entry instruction");
      return std::nullopt;
    }
    if (instruction->size() <= 0 || instruction->size() % sizeof(uint32_t) != 0 ||
        static_cast<size_t>(instruction->size()) > available) {
      errors.emplace_back("ConSan kernel entry instruction has invalid decoded bounds");
      return std::nullopt;
    }
    ...
    if (mnemonic.starts_with("s_branch") || mnemonic.starts_with("s_cbranch") ||
      ...
      errors.emplace_back(
          "ConSan dynamic-stack entry trampoline cannot relocate PC-relative control flow");
      return std::nullopt;
    }
  ...
  for (uint32_t index = 0; index < following_instruction_count; ++index) {
    ...
    if (following->second == "s_clause") {
      errors.emplace_back("ConSan cannot relocate a nested s_clause entry run");
      return std::nullopt;
    }
