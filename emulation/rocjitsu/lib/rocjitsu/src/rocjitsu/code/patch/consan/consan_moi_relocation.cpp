...
[[nodiscard]] bool append_scc_preserving_indirect_target(std::vector<uint32_t> &words,
                                                         uint64_t words_text_offset,
                                                         uint64_t target_text_offset,
                                                         uint16_t pc_sgpr, uint16_t saved_scc_sgpr,
                                                         bool capture_scc, rj_code_arch_t arch) {
  if (capture_scc) {
...
bool append_moi_scc_preserving_indirect_jump(std::vector<uint32_t> &words,
                                             ...
                                             rj_code_arch_t arch) {
  if (!append_scc_preserving_indirect_target(words, words_text_offset, target_text_offset, pc_sgpr,
                                             saved_scc_sgpr, capture_scc, arch))
    return false;
