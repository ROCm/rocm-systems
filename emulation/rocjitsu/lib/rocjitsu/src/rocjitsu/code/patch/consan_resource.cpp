...
ConSanRegisterPlan plan_consan_registers(const ConSanRegisterRequest &request,
  ...
  const uint32_t fresh_start = align_up(
      ...
      request.alignment);
  if (range_fits(fresh_start, request.count, request.architecture_limit) &&
      !range_intersects(request.forbidden, request.reg_class, static_cast<uint16_t>(fresh_start),
                        request.count)) {
    plan.source = ConSanRegisterAllocationSource::DescriptorGrowth;
    plan.base = static_cast<uint16_t>(fresh_start);
    plan.required_descriptor_count = static_cast<uint16_t>(fresh_start + request.count);
    return plan;
  }
