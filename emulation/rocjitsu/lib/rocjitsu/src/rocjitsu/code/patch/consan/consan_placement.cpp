...
local_cave_owner_ranges(const ProgramInventory &inventory, uint64_t text_size) {
  ...
  owners.reserve(inventory.containers().size());
...
find_liveness_scratch_vgpr(const ConSanProgramSite &access, const Instruction *inst,
  ...
  uint16_t search_start =
      std::max(access_scratch_search_start(access), min_auto_scratch_vgpr.value_or(0));
  ...
  while (search_start < REGISTER_SET_MAX_VGPRS) {
    ...
    if (max_auto_scratch_vgpr && candidate_end == *max_auto_scratch_vgpr &&
      ...
      return std::nullopt;
    if (candidate_end <= 256 && access_scratch_tuple_base_is_valid(access, *candidate) &&
...
choose_scratch_vgpr(const ConSanProgramSite &access, std::optional<uint16_t> requested_scratch_vgpr,
                    ...
                    std::optional<uint16_t> min_auto_scratch_vgpr,
                    std::optional<uint16_t> max_auto_scratch_vgpr, uint16_t required_vgprs) {
  if (requested_scratch_vgpr) {
    if (static_cast<uint32_t>(*requested_scratch_vgpr) + required_vgprs <= 256 &&
        access_scratch_tuple_base_is_valid(access, *requested_scratch_vgpr) &&
        !is_forbidden_scratch_vgpr_run(access, *requested_scratch_vgpr, required_vgprs))
      return *requested_scratch_vgpr;
    return std::nullopt;
  }
...
[[nodiscard]] uint32_t count_nop_padding(std::span<const uint8_t> bytes, uint64_t offset,
                                         rj_code_arch_t arch, uint32_t max_word_count) {
  const uint32_t nop = build_s_nop(0, arch);
  uint32_t count = 0;
