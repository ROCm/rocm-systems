...
static std::optional<ConSanRuntimeStaticMapping> make_sampled_access_runtime_mapping(
    ConSanStaticAccessAttribution access, ConSanLdsAccessKind access_kind, uint32_t first_slot,
    uint32_t range_count, uint32_t bank_count, const ConSanPatchLoweringProduct &patch) {
...
MoiDispatchIdentityPlan plan_sampled_dispatch_identity(const ConSanRequest &request,
  ...
  return {
      .needs_dispatch_id =
          facts.access_reports_need_explicit_identity &&
          (request.moi_runtime_sample_stride > 1u || facts.has_access_or_atomic_consumer),
...
const MoiModeOperations kSampledModeOperations = {
    ...
    .operand_overlap_spill = sampled_operand_overlap_spill,
