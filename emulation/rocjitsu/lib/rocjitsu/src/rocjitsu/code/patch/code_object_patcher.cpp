...
[[nodiscard]] std::optional<uint64_t> loaded_vaddr_to_file_offset(const std::vector<uint8_t> &image,
                                                                  const Elf64_Ehdr &ehdr,
                                                                  uint64_t loaded_vaddr,
                                                                  uint64_t size) {
  const auto phdrs = read_program_headers(image, ehdr);
  for (const Elf64_Phdr &phdr : phdrs) {
    if (phdr.p_type != PT_LOAD || loaded_vaddr < phdr.p_vaddr)
      continue;

    const uint64_t segment_delta = loaded_vaddr - phdr.p_vaddr;
    if (segment_delta > phdr.p_filesz || size > phdr.p_filesz - segment_delta)
      continue;

    const uint64_t file_offset = phdr.p_offset + segment_delta;
    if (file_offset < phdr.p_offset || !image_contains_range(image.size(), file_offset, size))
      continue;
    return file_offset;
  }
  return std::nullopt;
...
void apply_kernel_descriptor_resource_translation(KD &desc, const KdTranslation &translation,
  ...
  if (translation.skipped) {
    // Skipped kernels are target-ISA `s_endpgm` stubs. Do not preserve the
    ...
    // can make an otherwise empty dispatch fail as an invalid allocation.
    //
...
std::optional<std::vector<AppendedKernelDescriptor>>
CodeObjectPatcher::append_kernel_descriptor_translations(
    std::span<const KdTranslation> translations, rj_code_arch_t target_arch, uint64_t alignment) {
  if (translations.empty())
    return std::vector<AppendedKernelDescriptor>{};
  ...
  std::vector<AppendedKernelDescriptor> appended;
...
bool CodeObjectPatcher::annotate_virtual_lds_descriptor_by_vaddr(
    uint64_t descriptor_vaddr, const VirtualLdsDescriptorDispatchMetadata &metadata) {
  if (image_.size() < sizeof(Elf64_Ehdr))
    return false;
  const auto header = *reinterpret_cast<const Elf64_Ehdr *>(image_.data());
  const auto descriptor_file_offset =
      loaded_vaddr_to_file_offset(image_, header, descriptor_vaddr, sizeof(KD));
  ...
  write_virtual_lds_descriptor_dispatch_metadata(*desc, metadata);
  return true;
}
