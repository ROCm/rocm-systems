...
[[nodiscard]] util::inline_vector<Elf64_Shdr>
read_section_headers(const util::inline_vector<uint8_t> &image, const Elf64_Ehdr &ehdr) {
  ...
  util::inline_vector<Elf64_Shdr> shdrs(ehdr.e_shnum);
  std::memcpy(shdrs.data(), image.data() + ehdr.e_shoff, shdrs.size() * sizeof(Elf64_Shdr));
  return shdrs;
}

[[nodiscard]] util::inline_vector<Elf64_Phdr>
read_program_headers(const util::inline_vector<uint8_t> &image, const Elf64_Ehdr &ehdr) {
  ...
  util::inline_vector<Elf64_Phdr> phdrs(ehdr.e_phnum);
  std::memcpy(phdrs.data(), image.data() + ehdr.e_phoff, phdrs.size() * sizeof(Elf64_Phdr));
  return phdrs;
}

void write_elf_tables(util::inline_vector<uint8_t> &image, const Elf64_Ehdr &ehdr,
...
void insert_file_bytes(util::inline_vector<uint8_t> &image, Elf64_Ehdr &ehdr,
                       util::inline_vector<Elf64_Shdr> &shdrs,
                       util::inline_vector<Elf64_Phdr> &phdrs, uint64_t file_offset,
...
void shift_symbols_in_moved_sections(util::inline_vector<uint8_t> &image, const Elf64_Ehdr &ehdr,
                                     std::span<const Elf64_Shdr> shdrs,
                                     const util::inline_vector<bool> &shift_section_vaddr,
                                     uint64_t delta) {
...
void grow_text_function_symbols(util::inline_vector<uint8_t> &image, const Elf64_Ehdr &ehdr,
...
void shift_relocation_offsets_in_moved_sections(
    util::inline_vector<uint8_t> &image, const Elf64_Ehdr &ehdr, std::span<const Elf64_Shdr> shdrs,
    const util::inline_vector<bool> &shift_section_vaddr, uint64_t delta) {
  assert(shdrs.size() == shift_section_vaddr.size() && "section shift map size mismatch");
...
void adjust_kernel_descriptor_entry_offsets_in_moved_sections(
    util::inline_vector<uint8_t> &image, std::span<const Elf64_Shdr> shdrs,
    const util::inline_vector<bool> &shift_section_vaddr, uint64_t delta) {
  assert(shdrs.size() == shift_section_vaddr.size() && "section shift map size mismatch");
  if (delta == 0)
    return;
...
bool CodeObjectPatcher::replace_text(std::span<const uint8_t> new_text) {
  ...
  if (growth != 0) {
    ...
    util::inline_vector<uint8_t> inserted(padded_file_delta, 0);
    util::inline_vector<bool> shift_section_vaddr(shdrs.size(), false);
    for (size_t i = 0; i < shdrs.size(); ++i) {
    ...
    util::inline_vector<bool> shift_segment_vaddr(phdrs.size(), false);
...
util::inline_vector<uint8_t> CodeObjectPatcher::emit() const { return image_; }
