...
std::vector<Elf64_Shdr> CodeObjectPatcher::section_headers() const {
  ...
  return read_section_headers(image_, header).value_or(std::vector<Elf64_Shdr>{});
}
...
bool CodeObjectPatcher::has_relocations_within_text() const {
  ...
  const auto maybe_shdrs = read_section_headers(image_, header);
  if (!maybe_shdrs)
    return false;
  const auto &shdrs = *maybe_shdrs;
  ...
  if (growth != 0) {
    ...
    try {
      ...
      if (!shift_symbols_in_moved_sections(image, header, shdrs, shift_section_vaddr,
        ...
        return malformed();
      shift_relative_addends_into_moved_sections(image, header, shdrs, old_text_end_vaddr,
                                                 *padded_file_delta);
...
bool CodeObjectPatcher::replace_text(std::span<const uint8_t> new_text,
  ...
  for (const PcRelativeTextRelocation &relocation : code_relocations) {
    ...
    std::memcpy(image.data() + text_offset_ + relocation.target_literal_offset, &delta,
                sizeof(delta));
  }
  shdrs[*text_index].sh_size = new_text.size();
  if (!relocate_text_symbols(image, header, shdrs, *text_index, text_size_, new_text.size(),
...
SidecarDescriptorAppendResult CodeObjectPatcher::append_sidecar_descriptor_translations_impl(
  ...
  if (!shift_relocation_offsets_in_moved_sections(image, header, shdrs, shift_section_vaddr,
      ...
      !adjust_kernel_descriptor_entry_offsets_in_moved_sections(image, shdrs, shift_section_vaddr,
                                                                delta))
    return SidecarDescriptorAppendResult::malformed_input();
  shift_relative_addends_into_moved_sections(image, header, shdrs, *insertion_vaddr, delta);
