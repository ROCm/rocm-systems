...
/// @file consan_dispatch_preload.h
...
[[nodiscard]] constexpr uint16_t amdhsa_dispatch_id_prefix_sgpr_count(bool private_segment_buffer,
                                                                      bool dispatch_ptr,
                                                                      bool queue_ptr,
                                                                      bool kernarg_segment_ptr) {
...
[[nodiscard]] constexpr DispatchIdPreloadPlan
plan_dispatch_id_preload(uint16_t original_user_sgpr_count, uint16_t system_sgpr_count,
                         uint16_t dispatch_id_prefix_sgpr_count, bool dispatch_id_already_enabled,
                         uint16_t sgpr_limit = 106, uint16_t user_sgpr_initialization_limit = 16) {
  DispatchIdPreloadPlan plan;
  ...
  if (dispatch_id_prefix_sgpr_count > original_user_sgpr_count ||
    ...
    plan.support = DispatchIdPreloadSupport::InvalidDispatchPosition;
    return plan;
  }
  if (original_user_sgpr_count > user_sgpr_initialization_limit) {
    plan.support = DispatchIdPreloadSupport::UserSgprInitializationLimit;
    return plan;
  }
  if (dispatch_id_already_enabled) {
    if (original_required_count > sgpr_limit) {
      plan.support = DispatchIdPreloadSupport::SgprAllocationLimit;
      return plan;
    }
    plan.required_sgpr_count = static_cast<uint16_t>(original_required_count);
    plan.support = DispatchIdPreloadSupport::SupportedAlreadyEnabled;
    return plan;
  }
  ...
  plan.support = DispatchIdPreloadSupport::SupportedInsert;
  return plan;
}

[[nodiscard]] constexpr std::optional<uint16_t>
dispatch_id_restore_source(const DispatchIdPreloadPlan &plan, uint16_t guest_destination_sgpr) {
