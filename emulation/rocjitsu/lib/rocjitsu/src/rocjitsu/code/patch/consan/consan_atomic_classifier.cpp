...
using Reason = AtomicClassifierReason;

[[nodiscard]] AtomicLoweringClassification reject(Reason reason) {
  return {
      .form = std::nullopt,
...
[[nodiscard]] AtomicLoweringClassification normalized(AtomicLoweringForm form,
                                                      Reason address_reason, Reason exact_reason,
...
[[nodiscard]] bool is_flat_family(const AtomicSite &site, bool is_rmw) {
  return site.mnemonic.starts_with("flat_atomic") ||
         (!is_rmw &&
          (site.mnemonic.starts_with("flat_load") || site.mnemonic.starts_with("flat_store")));
}

[[nodiscard]] bool is_global_family(const AtomicSite &site, bool is_rmw) {
...
[[nodiscard]] Reason causal_reason(const AtomicSite &site, const AtomicLoweringForm &form) {
  if (!site.scope)
    return Reason::MissingOrderingMetadata;
  if (!memory_scope_is_supported(*site.scope) || *site.scope == MemoryScope::Wavefront)
    return Reason::UnsupportedScope;
  if (form.kind == AtomicLoweringFormKind::LdsVectorOffset && *site.scope != MemoryScope::Workgroup)
    return Reason::UnsupportedScope;
...
[[nodiscard]] Reason exact_reason(const AtomicSite &site, const AtomicLoweringForm &form) {
...
AtomicLoweringClassification classify_atomic_lowering(const AtomicSite &site, rj_code_arch_t arch,
                                                      bool is_rmw) {
  ...
  const bool compare_exchange = is_rmw && atomic_is_compare_exchange(site);
  ...
  const auto finish = [&](AtomicLoweringForm form, Reason address_reason = Reason::None) {
  ...
  if (site.mnemonic.starts_with("ds_")) {
    ...
    return finish({
        .kind = AtomicLoweringFormKind::LdsVectorOffset,
  ...
  if (site.mnemonic.starts_with("buffer_")) {
    ...
    if (*site.raw_ioffset < kSigned24Min || *site.raw_ioffset > kSigned24Max)
      return reject(Reason::UnsupportedOffset);
    return finish({
        .kind = AtomicLoweringFormKind::BufferResourceVectorOffset,
        .instruction_size = site.size,
        .value_width_bits = site.width_bits,
        .value_register_count = value_register_count,
        .data_register_count = data_register_count,
  ...
  if (site.size != expected_size || !site.raw_saddr || !site.raw_vaddr || !site.raw_ioffset)
    return reject(Reason::UnsupportedEncoding);
  if ((memory.scale_offset == ScaleOffsetCapability::Supported && !site.raw_scale_offset) ||
      (memory.scale_offset != ScaleOffsetCapability::Supported &&
  ...
  AtomicLoweringForm form{
      ...
      .sign_extend_vector_offset = memory.vector_offset_extension == VectorOffsetExtension::Sign,
      .is_rmw = is_rmw,
      .compare_exchange = compare_exchange,
      .returns_old_value = site.returns_old_value.value_or(false),
  };
  const Reason address_reason = kind == AtomicLoweringFormKind::FlatScalarVectorAddress &&
