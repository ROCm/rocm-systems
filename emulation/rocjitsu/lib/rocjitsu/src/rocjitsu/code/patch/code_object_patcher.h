...
/// @brief Location of a kernel descriptor appended into a loaded ELF segment.
struct AppendedKernelDescriptor {
...
public:
  ...
  /// @brief Append non-symbolized kernel descriptors in loaded memory after .text.
  ///
  /// @details Virtual-LDS sidecar descriptors must be visible to the GPU loader,
  /// but ROCR does not need to discover them through the AMDHSA symbol table.
  /// The patcher therefore places descriptor bytes in the executable LOAD tail
  /// immediately after the translated .text payload, without growing the .text
  /// section itself. Runtime metadata records the returned virtual addresses.
  [[nodiscard]] std::optional<std::vector<AppendedKernelDescriptor>>
  append_kernel_descriptor_translations(std::span<const KdTranslation> translations,
                                        rj_code_arch_t target_arch, uint64_t alignment = 64);

  /// @brief Write rocjitsu virtual-LDS dispatch facts into descriptor reserved bytes.
  ///
  /// @details Virtual-LDS sidecar descriptors are appended after `.text`, which can move
  /// later allocated sections in file space. The normal kernel descriptor address is the
  /// stable value ROCR places in AQL packets, so late annotations resolve from loaded virtual
  /// address back to the current file offset instead of reusing pre-insertion offsets.
  [[nodiscard]] bool
  annotate_virtual_lds_descriptor_by_vaddr(uint64_t descriptor_vaddr,
