...
std::optional<uint16_t> flat_check_trap_compare_vgpr(const ProgramSite &access) {
...
std::optional<uint16_t> check_trap_compare_vgpr(const ProgramSite &access, uint16_t chunk_index,
                                                rj_code_arch_t arch,
  ...
  if (access.kind == LdsAccessKind::Read) {
    ...
    const uint32_t register_limit = arch_has_selectable_vgpr_bank(arch) ? 1024u : 256u;
  ...
  if (access.kind == LdsAccessKind::Write) {
    ...
    const bool two_address = access.lowering.form &&
