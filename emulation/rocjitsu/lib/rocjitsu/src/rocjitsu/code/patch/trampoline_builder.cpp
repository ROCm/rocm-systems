...
std::optional<TrampolineBytes> TrampolineBuilder::emit_probe_call(const TrampolinePlan &plan,
  ...
  for (const SpecialStateSlot &s : plan.special_state_saves)
    env.push_back(s.width == 2 ? build_s_mov_b64_encoding(s.temp_base, s.operand, plan.arch)
  ...
  if (full_mask_exec)
    env.push_back(build_s_mov_b64_encoding(scalar_operand_exec_lo(plan.arch),
  ...
  if (full_mask_exec)
    env.push_back(
        build_s_mov_b64_encoding(scalar_operand_exec_lo(plan.arch), exec_temp, plan.arch));
  ...
  for (const SpecialStateSlot &s : plan.special_state_saves)
    env.push_back(s.width == 2 ? build_s_mov_b64_encoding(s.operand, s.temp_base, plan.arch)
