...
std::optional<std::vector<uint32_t>> consan_detail::build_moi_relocated_guest_access_words(
  ...
  if (ranges.size() != 2u ||
      (candidate.site().kind != ConSanLdsAccessKind::Read &&
  ...
  const bool load = candidate.site().kind == ConSanLdsAccessKind::Read;
...
resolve_moi_exec_save_requirement(const ConSanRequest &request,
  ...
  return MoiExecSaveRequirement{
      ...
      .automatic_banked_record_capture = mode_semantics.automatic_banked_record_capture,
...
uint16_t moi_exec_save_sgpr_count(const MoiExecSaveRequirement &requirement, rj_code_arch_t arch) {
  ...
  return operations.exec_save_sgpr_count(requirement,
                                         consan_moi_impl::resolve_moi_target_facts(arch));
}
