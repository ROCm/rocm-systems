...
/// requirements before calling `assemble_appended_body`. This value then
...
/// mode evidence policy and owns no storage; every span and output pointer is
/// valid only for the duration of the assembly call.
struct AppendedBodyOptions {
  explicit AppendedBodyOptions(rj_code_arch_t arch) : arch(arch) {}
...
appended_body_uses_selectable_vgpr_bank(rj_code_arch_t arch,
                                        std::optional<uint16_t> incoming_vgpr_bank_mode) {
  return arch_has_selectable_vgpr_bank(arch) && incoming_vgpr_bank_mode.value_or(0u) != 0u;
}

[[nodiscard]] inline bool
appended_body_vgpr_bank_mode_is_valid(rj_code_arch_t arch,
                                      std::optional<uint16_t> incoming_vgpr_bank_mode) {
  return incoming_vgpr_bank_mode.value_or(0u) == 0u || arch_has_selectable_vgpr_bank(arch);
}

[[nodiscard]] inline size_t
appended_body_vgpr_bank_transition_word_count(rj_code_arch_t arch,
                                              std::optional<uint16_t> incoming_vgpr_bank_mode,
                                              bool has_vgpr_spill, bool wraps_embedded_guest) {
  if (!appended_body_uses_selectable_vgpr_bank(arch, incoming_vgpr_bank_mode))
    return 0u;
  return (has_vgpr_spill ? 4u : 2u) + (wraps_embedded_guest ? 2u : 0u);
}

[[nodiscard]] constexpr bool
embedded_guest_vgpr_bank_plan_is_valid(bool wraps_embedded_guest, bool selects_low_vgpr_bank,
