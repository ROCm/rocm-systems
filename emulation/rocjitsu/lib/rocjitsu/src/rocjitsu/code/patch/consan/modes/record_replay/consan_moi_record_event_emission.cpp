...
[[nodiscard]] std::optional<std::vector<uint32_t>> build_barrier_record_cave_words(
  ...
  const uint16_t value_vgpr = static_cast<uint16_t>(*options.scratch_vgpr + 5u);
  require_emission(append_reserve_bounded_dynamic_record_slot(
  ...
  require_emission(append_guarded_dynamic_record(words, record_words, *options.moi_exec_save_sgpr,
...
[[nodiscard]] std::optional<std::vector<uint32_t>> build_atomic_record_cave_words(
  ...
  const auto report_scope = site.scope ? consan_moi_record_replay_scope(*site.scope) : std::nullopt;
  if (!report_scope) {
  ...
  if (is_compare_exchange) {
    ...
    require_emission.append("ConSan MOI atomic record CAS could not capture its outcome mask",
  ...
  require_emission(append_reserve_bounded_dynamic_record_slot(
                       ...
                       atomic_record_capacity, slot_vgpr, lane_rank_vgpr, *options.scratch_vgpr,
                       arch),
...
[[nodiscard]] std::optional<std::vector<uint32_t>> build_fence_record_cave_words(
  ...
  if (static_cast<uint32_t>(*options.scratch_vgpr) + address_plan.scratch_vgpr_count > 256u ||
      reject_atomic_candidate_scratch_overlap(communication_lowering_form, *options.scratch_vgpr,
                                              address_plan.scratch_vgpr_count, errors) ||
      reject_optional_scratch_range_overlap(options.moi_owner_epoch_vgprs.owner(),
                                            *options.scratch_vgpr, address_plan.scratch_vgpr_count,
                                            "MOI owner", errors) ||
  ...
  }
  {
    ...
    require_emission(append_reserve_bounded_dynamic_record_slot(
                         ...
                         record_capacity_or_count, slot_vgpr, value_vgpr, *options.scratch_vgpr,
                         arch),
    ...
    record
        ...
        .literal(offsetof(ConSanMoiFenceRecord, semantics), communication_site.raw_th.value_or(0u))
