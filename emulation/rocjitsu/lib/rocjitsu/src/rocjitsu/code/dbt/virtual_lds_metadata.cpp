...
constexpr std::array<uint8_t, 4> kDescriptorDispatchMagic = {'R', 'J', 'L', 'D'};
constexpr uint32_t kDescriptorDispatchOffsetMask = 0x00FFFFFFu;
constexpr uint32_t kDescriptorDispatchFlagsShift = 24;
...
static_assert(sizeof(DescriptorDispatchRecord) == 16);
static_assert(sizeof(DescriptorDispatchRecord) <=
              sizeof(rocr::llvm::amdhsa::kernel_descriptor_t::reserved1));
...
void write_virtual_lds_descriptor_dispatch_metadata(
  ...
  DescriptorDispatchRecord record{};
  record.virtual_descriptor_delta = metadata.virtual_descriptor_delta;
  record.kernarg_size = metadata.kernarg_size;
  record.backing_pointer_kernarg_offset =
      (metadata.backing_pointer_kernarg_offset & kDescriptorDispatchOffsetMask) |
      (static_cast<uint32_t>(metadata.flags) << kDescriptorDispatchFlagsShift);
  std::memcpy(descriptor.reserved0, kDescriptorDispatchMagic.data(),
              kDescriptorDispatchMagic.size());
  std::memcpy(descriptor.reserved1, &record, sizeof(record));
}

std::optional<VirtualLdsDescriptorDispatchMetadata> read_virtual_lds_descriptor_dispatch_metadata(
    const rocr::llvm::amdhsa::kernel_descriptor_t &descriptor) {
  std::array<uint8_t, 4> magic{};
  std::memcpy(magic.data(), descriptor.reserved0, magic.size());
  if (magic != kDescriptorDispatchMagic)
    return std::nullopt;

  DescriptorDispatchRecord record{};
  std::memcpy(&record, descriptor.reserved1, sizeof(record));
  const uint16_t flags =
      static_cast<uint16_t>(record.backing_pointer_kernarg_offset >> kDescriptorDispatchFlagsShift);
  record.backing_pointer_kernarg_offset &= kDescriptorDispatchOffsetMask;
  if (record.backing_pointer_kernarg_offset < record.kernarg_size)
    return std::nullopt;

  return VirtualLdsDescriptorDispatchMetadata{
      .virtual_descriptor_delta = record.virtual_descriptor_delta,
      .kernarg_size = record.kernarg_size,
      .backing_pointer_kernarg_offset = record.backing_pointer_kernarg_offset,
      .flags = flags,
  };
}
