...
/// @brief Mutable register and spill choices for one virtual-LDS kernel.
...
struct KdTranslation {
  ...
  uint32_t virtual_lds_size = 0;
  bool needs_virtual_lds_buffer = false;
  uint32_t virtual_lds_kernarg_pointer_offset = 0;
  int16_t virtual_lds_workgroup_id_sgpr_x = -1;
  int16_t virtual_lds_workgroup_id_sgpr_y = -1;
  int16_t virtual_lds_workgroup_id_sgpr_z = -1;
  uint32_t kernarg_wrapper_original_pointer_offset = 0;
  /// Source kernarg bytes copied into the virtual-LDS wrapper prefix.
  uint32_t kernarg_size = 0;
  /// Kernarg bytes advertised by the translated descriptor.
  ...
  uint8_t user_sgpr_repair_start = 0;
  uint8_t user_sgpr_repair_count = 0;
