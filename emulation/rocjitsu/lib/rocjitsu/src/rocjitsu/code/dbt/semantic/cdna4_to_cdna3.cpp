...
// CDNA3 non-memory instruction builders used by the narrow semantic rules
// below. Memory encodings deliberately go through Cdna3MemoryInstructionBuilder
// so LDS virtualization has one target-specific place to redirect loads/stores
// without duplicating raw bitfield construction in every rule body.
...
constexpr uint16_t kInlineConstFloatOne = 242;
constexpr uint16_t kM0 = 124;
constexpr uint16_t kSrcDpp8Lo = 233;
constexpr uint16_t kSrcDpp8Hi = 234;
constexpr uint16_t kSrcSdwa = 249;
constexpr uint16_t kSrcDpp = 250;
constexpr uint16_t kSrcLiteral64 = 254;
constexpr uint16_t kSrcLiteral32 = 255;
...
constexpr uint8_t kCdna3Vop2OpXorB32 = 21;
...
constexpr uint16_t kCdna3OpAddF32 = 257;
constexpr uint16_t kCdna3OpMulF32 = 261;
constexpr uint16_t kCdna3OpAddU32 = 308;
constexpr uint16_t kCdna3OpPermB32 = 493;
constexpr uint16_t kCdna3OpMed3F32 = 470;
constexpr uint16_t kCdna3OpAndOrB32 = 513;
constexpr uint16_t kCdna3OpOr3B32 = 514;
...
constexpr uint8_t kCdna3OpMfmaF32_32x32x8Bf16 = 96;
constexpr uint8_t kCdna3OpMfmaF32_16x16x16Bf16 = 97;
...
void emit_cdna3_f32_to_bf16_rne(std::vector<uint32_t> &words, uint8_t dst_half, uint8_t bias_tmp,
  ...
  emit_cdna3_vop3(words, kCdna3OpMovB32, dst_half, src);
  emit_cdna3_vop3(words, kCdna3OpLshrrevB32, bias_tmp, scalar_positive_inline_u32(16),
                  vgpr_src(dst_half));
  emit_cdna3_vop3(words, kCdna3OpAndB32, bias_tmp, scalar_positive_inline_u32(1),
                  vgpr_src(bias_tmp));
  emit_cdna3_vop2_literal(words, kCdna3Vop2OpAddU32, bias_tmp, bias_tmp, 0x7fffu);
  emit_cdna3_vop3(words, kCdna3OpAddU32, dst_half, vgpr_src(dst_half), vgpr_src(bias_tmp));
  emit_cdna3_vop3(words, kCdna3OpLshrrevB32, dst_half, scalar_positive_inline_u32(16),
                  vgpr_src(dst_half));
}
...
void emit_cdna3_mubuf(std::vector<uint32_t> &words, const cdna4::MubufMachineInst &src, uint8_t op,
  ...
  operands.sc0 = src.sc0 != 0;
  operands.sc1 = src.sc1 != 0;
  operands.nt = src.nt != 0;
...
void emit_cdna3_b16_zero_extend(std::vector<uint32_t> &words, uint8_t reg) {
  ...
  emit_cdna3_vop3(words, kCdna3OpLshlrevB32, reg, shift16, vgpr_src(reg));
  emit_cdna3_vop3(words, kCdna3OpLshrrevB32, reg, shift16, vgpr_src(reg));
}
...
ExpandResult lower_cdna4_bitop3_to_cdna3(const Bitop3Inst &inst, const LivenessAnalysis &liveness,
  ...
  if (truth_table == 0xec || truth_table == 0xf8 || truth_table == 0xfe) {
    ...
    if (truth_table == 0xec) {
      // S1 | (S0 & S2)
      emit_cdna3_vop3(words, kCdna3OpAndOrB32, vdst, src[0], src[2], src[1]);
    } else if (truth_table == 0xf8) {
      // S0 | (S1 & S2)
      emit_cdna3_vop3(words, kCdna3OpAndOrB32, vdst, src[1], src[2], src[0]);
    } else {
      // S0 | S1 | S2
      emit_cdna3_vop3(words, kCdna3OpOr3B32, vdst, src[0], src[1], src[2]);
    }
    if (is_b16)
  ...
  if (!is_b16 && truth_table == 0xc8) {
    uint8_t term = vdst;
    std::optional<VgprWindow> spilled_term;
    if (vdst_aliases_any_vgpr_source(vdst, src)) {
      auto scratch = liveness.find_free_run(&inst, 1);
      if (scratch) {
        term = static_cast<uint8_t>(*scratch);
        context.require_vgprs(static_cast<uint32_t>(*scratch) + 1);
      } else {
        const auto forbidden = bitop3_spill_forbidden_ranges(vdst, src);
        spilled_term =
            choose_vgpr_window_or_spill(inst, liveness, context, 1, 1,
        ...
        term = spilled_term->base;
        if (!spilled_term->spilled)
          context.require_vgprs(static_cast<uint32_t>(spilled_term->base) + 1);
      }
    }

    std::vector<uint32_t> words;
    if (spilled_term && spilled_term->spilled) {
      emit_cdna3_scratch_store_b32(words, spilled_term->base, spilled_term->spill_offset);
    ...
    auto emit_or = [&](uint8_t dst, uint16_t src0, uint16_t src1) {
      emit_cdna3_vop3(words, kCdna3OpOrB32, dst, src0, src1);
    };
    ...
    if (spilled_term && spilled_term->spilled) {
      emit_cdna3_scratch_load_b32(words, spilled_term->base, spilled_term->spill_offset);
      emit_cdna3_wait_all(words);
    }
    return ExpandResult::success(std::move(words));
  }
  ...
  uint8_t acc = vdst;
  uint8_t term = 0;
  std::optional<VgprWindow> spill_scratch;
  if (scratch_count != 0) {
    ...
    if (scratch) {
      ...
      acc = spill_scratch->base;
      if (needs_term_temp)
        term = static_cast<uint8_t>(spill_scratch->base + 1);
      if (!spill_scratch->spilled)
        context.require_vgprs(static_cast<uint32_t>(spill_scratch->base) + scratch_count);
    }
  }

  std::vector<uint32_t> words;
  if (spill_scratch && spill_scratch->spilled) {
    for (uint16_t i = 0; i < spill_scratch->count; ++i) {
      emit_cdna3_scratch_store_b32(words, static_cast<uint8_t>(spill_scratch->base + i),
                                   spill_scratch->spill_offset + static_cast<uint32_t>(i) * 4u);
    }
    emit_cdna3_wait_all(words);
  }
  ...
  if (spill_scratch && spill_scratch->spilled) {
    for (uint16_t i = 0; i < spill_scratch->count; ++i) {
      emit_cdna3_scratch_load_b32(words, static_cast<uint8_t>(spill_scratch->base + i),
                                  spill_scratch->spill_offset + static_cast<uint32_t>(i) * 4u);
    }
    emit_cdna3_wait_all(words);
  }
...
ExpandResult lower_permlane_swap_b32_cdna4_to_cdna3(const Instruction &inst,
  ...
  // The ISA generator now models the swap operands as read/write values, so the
  // normal liveness query protects their pre-write contents. Explicitly forbid
  // only the architectural outputs: those registers are intentionally written
  // near the end of the lowering and cannot be restored as scratch victims.
  ...
  emit_cdna3_vop3(words, kCdna3OpXorB32, partner_addr, scalar_positive_inline_u32(half_wave_lanes),
                  vgpr_src(lane));
...
ExpandResult lower_cvt_pk_bf16_f32_cdna4_to_cdna3(const Instruction &inst,
  ...
  const uint8_t vdst = static_cast<uint8_t>(src.vdst);
  auto scratch = choose_vgpr_window_or_spill(inst, liveness, context, 3, 1, {{vdst, 1}});
  if (!scratch)
    return failed_existing_expand_rule(
...
ExpandResult lower_cvt_f32_bf16_vop1_cdna4_to_cdna3(const Instruction &inst, uint64_t offset,
  ...
  if (src.src0 == kSrcSdwa) {
    ...
    if (sdwa.src0_sel == kSdwaWord0) {
      ...
      emit_cdna3_vop3(words, kCdna3OpLshlrevB32, vdst, scalar_positive_inline_u32(16), vsrc);
    } else {
      ...
      emit_cdna3_vop3(words, kCdna3OpLshrrevB32, vdst, scalar_positive_inline_u32(16), vsrc);
      emit_cdna3_vop3(words, kCdna3OpLshlrevB32, vdst, scalar_positive_inline_u32(16),
                      vgpr_src(vdst));
    }
    return ExpandResult::success(std::move(words));
  ...
  emit_cdna3_vop3(words, kCdna3OpLshlrevB32, static_cast<uint8_t>(src.vdst),
...
void emit_cdna3_bf16_half_as_f32(std::vector<uint32_t> &words, uint8_t dst, uint16_t src,
  ...
  if (high_half) {
    emit_cdna3_vop3(words, kCdna3OpLshrrevB32, dst, scalar_positive_inline_u32(16), src);
    emit_cdna3_vop3(words, kCdna3OpLshlrevB32, dst, scalar_positive_inline_u32(16), vgpr_src(dst));
  } else {
    emit_cdna3_vop3(words, kCdna3OpLshlrevB32, dst, scalar_positive_inline_u32(16), src);
  }
  if (negate) {
    ...
    emit_cdna3_vop2_literal(words, kCdna3Vop2OpXorB32, dst, dst, 0x80000000u);
  }
}
...
ExpandResult lower_dot2_f32_bf16_cdna4_to_cdna3(const Instruction &inst,
  ...
  emit_cdna3_vop3(words, kCdna3OpMulF32, a_lo, vgpr_src(a_lo), vgpr_src(b_lo));
  emit_cdna3_vop3(words, kCdna3OpMulF32, a_hi, vgpr_src(a_hi), vgpr_src(b_hi));
  emit_cdna3_vop3(words, kCdna3OpAddF32, a_lo, vgpr_src(a_lo), vgpr_src(a_hi));
  emit_cdna3_vop3(words, kCdna3OpAddF32, static_cast<uint8_t>(dot.vdst), vgpr_src(a_lo),
                  static_cast<uint16_t>(dot.src2));
  if (dot.clamp) {
    emit_cdna3_vop3(words, kCdna3OpMed3F32, static_cast<uint8_t>(dot.vdst),
...
[[nodiscard]] constexpr WideKMfmaLowering lowering_for_shape(WideKMfmaShape shape) {
  ...
  case WideKMfmaShape::F32_16x16x32_BF16:
    return {shape, kCdna3OpMfmaF32_16x16x16Bf16, 4, 4, 2};
  case WideKMfmaShape::F32_32x32x16_BF16:
    return {shape, kCdna3OpMfmaF32_32x32x8Bf16, 16, 4, 2};
  }
...
ExpandResult lower_wide_k_mfma_cdna4_to_cdna3(const Instruction &inst,
  ...
  // split order is observable at BF16 last-bit precision.  The high-then-low
  // order matches the native MI300X Qwen q_proj reference exactly; low-then-high
  // produced a one-BF16-ULP drift in one q_proj output element.  Treat this as
  // a fixed layout decision, not a tuning knob: do not flip back to
  // low-half-first to chase later Qwen drift unless new ISA evidence or a more
  // direct hardware result contradicts the captured high-first behavior.
  //
  ...
  if (needs_scratch) {
    ...
    std::optional<uint16_t> scratch =
        liveness.find_free_run(&inst, lowering.dst_regs, 0, kMfmaTupleBaseAlignment);
...
[[nodiscard]] ExpandResult expand_mubuf_load_to_lds_cdna4_to_cdna3(const Instruction &inst,
  ...
  const uint16_t scratch_alignment = data_count >= 3 ? 2 : 1;
  auto scratch =
...
constexpr uint16_t kEncVop3p = 0x1A7;
constexpr uint16_t kEncVop3pMfma = kEncVop3p;
...
constexpr uint16_t kCdna4Op_v_mfma_f32_16x16x32_bf16 = 53;
constexpr uint16_t kCdna4Op_v_mfma_f32_32x32x16_bf16 = 55;
constexpr uint16_t kCdna4Op_v_mfma_f32_16x16x32_f16 = 84;
constexpr uint16_t kCdna4Op_v_mfma_f32_32x32x16_f16 = 85;
constexpr uint16_t kCdna4Op_v_permlane16_swap_b32 = 89;
...
constexpr uint16_t kCdna4Op_v_cvt_f32_bf16 = 91;
...
const TranslationRule kExpandRules_cdna4_to_cdna3[] = {
    {kEncVop1, kCdna4Op_v_permlane16_swap_b32, RuleAction::Expand, 0, 0, nullptr,
    ...
    {kEncVop1, kCdna4Op_v_cvt_f32_bf16, RuleAction::Expand, 0, 0, nullptr,
     expand_cvt_f32_bf16_vop1_cdna4_to_cdna3, nullptr, nullptr},
    {kEncVop1Hi1, kCdna4Op_v_permlane16_swap_b32, RuleAction::Expand, 0, 0, nullptr,
     expand_permlane16_swap_b32_cdna4_to_cdna3, nullptr, nullptr},
    {kEncVop1Hi1, kCdna4Op_v_permlane32_swap_b32, RuleAction::Expand, 0, 0, nullptr,
     expand_permlane32_swap_b32_cdna4_to_cdna3, nullptr, nullptr},
    {kEncVop1Hi1, kCdna4Op_v_cvt_f32_bf16, RuleAction::Expand, 0, 0, nullptr,
     expand_cvt_f32_bf16_vop1_cdna4_to_cdna3, nullptr, nullptr},
    {kEncVop1Hi2, kCdna4Op_v_permlane16_swap_b32, RuleAction::Expand, 0, 0, nullptr,
     expand_permlane16_swap_b32_cdna4_to_cdna3, nullptr, nullptr},
    {kEncVop1Hi2, kCdna4Op_v_permlane32_swap_b32, RuleAction::Expand, 0, 0, nullptr,
     expand_permlane32_swap_b32_cdna4_to_cdna3, nullptr, nullptr},
    {kEncVop1Hi2, kCdna4Op_v_cvt_f32_bf16, RuleAction::Expand, 0, 0, nullptr,
     expand_cvt_f32_bf16_vop1_cdna4_to_cdna3, nullptr, nullptr},
    {kEncVop1Hi3, kCdna4Op_v_permlane16_swap_b32, RuleAction::Expand, 0, 0, nullptr,
     expand_permlane16_swap_b32_cdna4_to_cdna3, nullptr, nullptr},
    {kEncVop1Hi3, kCdna4Op_v_permlane32_swap_b32, RuleAction::Expand, 0, 0, nullptr,
     expand_permlane32_swap_b32_cdna4_to_cdna3, nullptr, nullptr},
    {kEncVop1Hi3, kCdna4Op_v_cvt_f32_bf16, RuleAction::Expand, 0, 0, nullptr,
     expand_cvt_f32_bf16_vop1_cdna4_to_cdna3, nullptr, nullptr},
    ...
    {kEncVop3, kCdna4Op_v_cvt_pk_bf16_f32, RuleAction::Expand, 0, 0, nullptr,
     expand_cvt_pk_bf16_f32_cdna4_to_cdna3, nullptr, nullptr},
    {kEncVop3p, kCdna4Op_v_dot2_f32_bf16, RuleAction::Expand, 0, 0, nullptr,
     expand_dot2_f32_bf16_cdna4_to_cdna3, nullptr, nullptr},
    {kEncVop3pMfma, kCdna4Op_v_mfma_f32_16x16x32_bf16, RuleAction::Expand, 0, 0, nullptr,
     expand_mfma_f32_16x16x32_bf16_cdna4_to_cdna3, nullptr, nullptr},
    {kEncVop3pMfma, kCdna4Op_v_mfma_f32_32x32x16_bf16, RuleAction::Expand, 0, 0, nullptr,
