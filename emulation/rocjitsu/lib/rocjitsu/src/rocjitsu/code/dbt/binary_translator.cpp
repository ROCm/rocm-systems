...
generated_branch_island_pool_offsets(std::span<const uint8_t> text, rj_code_arch_t arch) {
  ...
  constexpr uint64_t kPoolBytes =
      (2u + kDirectBranchIslandPoolSlots) * static_cast<uint64_t>(sizeof(uint32_t));
  for (uint64_t offset = 0; offset + 2 * sizeof(uint32_t) <= text.size();
...
TranslatedCodeObject BinaryTranslator::translate(const AmdGpuCodeObject &obj) {
  ...
  const auto generated_island_pool_offsets =
      generated_branch_island_pool_offsets(text, guest_arch_);
  ...
  for (const KernelTranslationScope &scope : scopes) {
    ...
    for (auto &[source_call_offset, consumer] : recovered_indirect_by_call) {
      ...
      if (single_effective_target) {
        ...
        if (contiguous_builder) {
          std::memcpy(&preceding_word, text.data() + first.source_getpc_offset - sizeof(uint32_t),
                      sizeof(preceding_word));
        }
        if (contiguous_builder &&
            preceding_word == build_s_nop(kLongDirectBranchMarkerNopImmediate, guest_arch_)) {
          consumer.preserve_marked_long_transfer = true;
          consumer.marked_window_begin = first.source_getpc_offset - sizeof(uint32_t);
    ...
    for (BasicBlock *block : scope.blocks) {
      ...
      for (auto it = block->instructions().begin(); it != block->instructions().end(); ++it) {
        ...
        if (generated_island_pool_offsets.contains(offset)) {
          const uint64_t source_end = offset + kGeneratedIslandPoolBytes;
          if (source_end > text.size()) {
            auto failure = make_kernel_failure(
                DiagnosticKind::Legalization,
                "generated direct branch island pool extends beyond source .text", offset);
            if (fail_or_skip_kernel(scope, std::move(failure), output_begin, descriptor_snapshot,
                                    text_relocations_begin, data_relocations_begin)) {
              skip_scope = true;
              break;
          ...
          for (uint64_t source_word = offset; source_word < source_end;
               source_word += sizeof(uint32_t)) {
            target_offset_by_source_offset.emplace(source_word,
                                                   target_offset + (source_word - offset));
          }
          kernel_text.insert(kernel_text.end(), text.begin() + static_cast<std::ptrdiff_t>(offset),
    ...
    // grow all requested windows in one rebuild and retry. Each fixup can grow
    // at most once to its exact final form; insertions can move a later transfer
    // across the range boundary, so repeat until the monotonic layout is fixed.
