...
struct RelocationFunctionTable {
  ...
  /// Populated callee entries, sorted by slot address, target offset, and ABI
  /// symbol provenance. A malformed input may relocate one slot to multiple
  /// targets; those entries are retained as a conservative callee set rather
  /// than interpreted as a one-to-one slot map. Slots without a qualifying
  /// text relocation are not possible callees and do not appear here.
