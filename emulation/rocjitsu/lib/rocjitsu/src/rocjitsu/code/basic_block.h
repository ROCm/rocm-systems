...
public:
  ...
  static FailureOr<std::vector<std::unique_ptr<BasicBlock>>>
  build(const CodeObject &co, Decoder &decoder, rj_code_arch_t arch, DecodeErrorEmitter emit_error,
  ...
  /// @brief Compatibility entry point for callers that report malformed code by exception.
  ...
  /// @brief Build basic blocks only from the supplied instruction ranges.
  static std::vector<std::unique_ptr<BasicBlock>> build(const CodeObject &co, Decoder &decoder,
                                                        rj_code_arch_t arch,
                                                        std::span<const uint64_t> extra_leaders,
                                                        std::span<const CodeRange> code_ranges) {
    return build(co, decoder, arch, extra_leaders, ExternalEntryPolicy::InferPredecessorless,
                 code_ranges);
  }
...
private:
  ...
  build_impl(const CodeObject &co, Decoder &decoder, rj_code_arch_t arch,
             ...
             DecodedSection *prepared = nullptr, std::span<const CodeRange> permitted_ranges = {});

  void note_successor_issue(SuccessorIssue issue) {
    if (issue != SuccessorIssue::IndirectControlFlow)
      note_static_successor_issue(static_cast<StaticSuccessorIssue>(issue));
