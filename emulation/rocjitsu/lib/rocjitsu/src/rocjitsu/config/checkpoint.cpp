...
serialize_config(flatbuffers::FlatBufferBuilder &builder, const SoC &soc,
  ...
  const auto fb_choices = builder.CreateVector(choices);
  // Preserve automatic requests, including -1 helpers and zero engines/budget.
  ...
  auto result = fb::CreateSimulationConfig(
      ...
      cpu_dispatch_threads, cpu_thread_budget, async_helper_threads, fb_choices);
  builder.ForceDefaults(false);
  return result;
}
...
void save_checkpoint(const std::string &path, const SoC &soc, uint64_t tick,
  ...
  auto config_offset =
      serialize_config(builder, soc, engine_config, cpu_dispatch_threads, cpu_thread_budget,
                       async_helper_threads, thread_allocations);
