...
extern "C" int rj_run_vfio_server(const char *config_path, const char *socket_path, int ready_fd) {
  ...
  //
  // The handler does no work of its own, deliberately. Formatting a message
  // and handing it to the logger both allocate, and an allocation failure
  // while handling an allocation failure leaves this frame by the one route
  // it exists to close. `run_vfio_server` has already reported whatever it
  // knows -- it logs its own failures and is where the detail lives -- so
  // there is nothing here worth the risk of saying.
  try {
