...
use rj_backend_rocjitsu::dbt::{self, EnvLookup, RocjitsuDbt};
use rj_core::common::{MaybeRef, SimpleValue};
use rj_core::emulator::{EmulatorDef, ExecMode, get_emulator_backend};
...
fn def_for_guest(gfx_target_version: u32) -> EmulatorDef {
    let mut agent = rj_core::agent::AgentDef::default();
...
fn a_containerised_session_points_the_hook_at_its_in_container_path() {
    ...
    ctx.profile.containerize = Some(rj_core::profile::ContainerizedDef {
        provider: None,
    ...
    assert!(
        tools_lib.starts_with("/mnt/rocjitsu/lib/"),
...
fn find_dbt_translate() -> Option<PathBuf> {
    ...
    let candidate =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../build/tools/rj_dbt_translate");
...
fn find_gfx950_code_object() -> Option<PathBuf> {
    ...
    let in_tree = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../build/kernels/triton_cdna4_matmul_dynamic_32x32x64.hsaco");
...
///
/// This validates what is in rocjitsu's scope — that the integration
