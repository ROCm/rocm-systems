...
//!
//! * `HSA_TOOLS_LIB` — path to `librocjitsu_hooks.so` (rocjitsu injects
...
//!   e.g. `gfx950`. rocjitsu derives this from the profile's agent.
...
use rj_core::discovery::{self, LibSearch, RuntimeLocation};
...
fn options_schema() -> Vec<OptionDef> {
    vec![
        OptionDef {
            ...
            dtype: rj_core::common::SimpleType::String,
            description: format!(
...
/// Search policy rocjitsu uses to locate the rocjitsu HSA tools hook
...
fn hooks_lib_search() -> LibSearch<'static> {
    LibSearch {
        ...
        // rocjitsu's in-tree hooks build output, relative to the rocjitsu
        // binary, so a monorepo build finds a fresh build without extra
        // configuration. Three levels up from `cli/target/<profile>` is
        // the rocjitsu project directory, and `build/` is where a plain
        // `cmake -B build` puts the hooks.
        binary_relative_dirs: &["../../../build/lib/rocjitsu/src/rocjitsu/hooks"],
        system_fallbacks: true,
    }
}

/// Returns the path rocjitsu should pass as `HSA_TOOLS_LIB`, located via
...
/// [`hooks_preload`], so what `rocjitsu emulators -l` shows and what the
...
mod tests {
    ...
    fn validate_rejects_untranslatable_guest_without_override() {
        ...
        // An owned gfx1250 (MI450X) agent: a valid rocjitsu agent, but not
