//! Built-in agents, topologies and profiles that rocjitsu preloads into
//! `<ROCJITSU_CLI_CONFIG_DIR>/{agent,topology,profile}/`.
//!
//! Historically these shipped as `agents/*.json` files embedded into
//! `rj_core` at build time and parsed at runtime. They now live
//! here as strongly-typed [`rj_core::agent::AgentDef`] /
//! [`rj_core::topology::TopologyDef`] constructors so the data is
//! validated by the compiler instead of by a runtime parse.
//!
//! This crate owns both the builtin *data* and the policy for writing
//! it to disk. It relies on `rj_core` only for the low-level path
//! resolution ([`rj_core::paths`]) and JSON serialization
//! ([`rj_core::state::write_json`]), and registers what it ships with
//! [`rj_core::store`] so the store can tell a document rocjitsu seeded
...
use rj_core::store::{BuiltinDocuments, DocKind, is_pristine_builtin};

pub use agents::{agents, mi300x, mi350x, mi450x};
pub use profiles::profiles;
pub use topologies::{default_topology, topologies};

// Tell `rj_core` what rocjitsu ships. The core store has to be able to
...
fn builtin_documents() -> Vec<(DocKind, String, serde_json::Value)> {
    fn collect<T: Serialize>(
        ...
        for (name, document) in documents {
            // These are rocjitsu's own structs serialising into a JSON
...
/// then say everything it left alone at once — `rocjitsu state builtins`
...
pub struct Ensured {
    ...
    /// The documents that differ from the ones rocjitsu ships and were
...
///
/// With `force` — `rocjitsu state builtins`, which exists so a rocjitsu
...
/// is the outcome `rocjitsu state builtins --help` describes as ordinary,
...
fn ensure<T: Serialize>(
    ...
    for (name, document) in documents {
        ...
        rj_core::state::write_json(&path, &document)?;
...
mod tests {
    ...
    fn ensure_agents_writes_then_skips() {
        ...
        names.sort();
        assert_eq!(rj_core::agent::store::list().unwrap(), names);
        ...
        for name in &names {
            assert!(
                rj_core::agent::store::get(name).is_ok(),
    ...
    fn a_forced_rewrite_leaves_an_edited_builtin_alone() {
        // `rocjitsu state builtins` used to overwrite a builtin the user had
        ...
        let mut mine = rj_core::store::profile_get("mi350x").unwrap();
        mine.description = Some("my own mi350x".to_string());
        rj_core::state::write_json(&rj_core::paths::profile_path("mi350x"), &mine).unwrap();
        ...
        assert_eq!(
            rj_core::store::profile_get("mi350x").unwrap(),
            mine,
        ...
        std::fs::remove_file(rj_core::paths::profile_path("mi350x")).unwrap();
        let clean = ensure_profiles(true).unwrap();
        assert!(clean.edited.is_empty());
        assert!(clean.documents.iter().all(|(_, w)| *w));
        assert_eq!(
            rj_core::store::profile_get("mi350x").unwrap().description,
            None
        );
    ...
    fn every_edited_builtin_of_a_kind_is_named_by_one_pass() {
        ...
        for name in ["mi300x", "mi350x", "mi450x"] {
            let mut mine = rj_core::store::profile_get(name).unwrap();
            mine.description = Some(format!("my own {name}"));
            rj_core::state::write_json(&rj_core::paths::profile_path(name), &mine).unwrap();
        }
