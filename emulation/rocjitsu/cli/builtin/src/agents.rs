...
//! They are not rocjitsu's to choose: they say what the emulator will
...
//!
//! The *shape* of the machine is rocjitsu's own and stays here: the
...
//! they mirror are 320, 288 and 256 — an emulated grid rocjitsu picked, not
...
/// is rocjitsu's own; `preset` supplies the per-CU `num_wf_slots`,
...
/// module docs on why the CU count is rocjitsu's to pick.
...
mod tests {
    ...
    /// The number of CUs a builtin emulates is rocjitsu's, not the
    ...
    /// rocjitsu chose to emulate, and changing it changes every result
        ...
        for (name, agent) in agents() {
            assert_eq!(
                ...
                "`{name}` no longer emulates 256 CUs; the preset's own CU \
                 count is not rocjitsu's to adopt — see the module docs"
            );
        }
    }
