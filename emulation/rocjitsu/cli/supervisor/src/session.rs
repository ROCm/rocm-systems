...
use rj_core::container::{ContainerState, container_name};
use rj_core::emulator::EmulatorDaemon;
use rj_core::error::{Result, RocJITsuError};
use rj_core::exec::{ExecDef, ExecId, InjectionDef};
use rj_core::profile::{FileMount, ProfileDef};
use rj_core::session::{SessionContext, SessionDef, SessionHealth, SessionId, state};
use tokio::sync::watch;

use crate::exec::Exec;
use rj_container::NodeClient;
use rj_core::proto::{ContainerTargets, SessionDescription};

/// Path the CLI binary is bind-mounted at inside every node container.
const CONTAINER_CLI_BIN: &str = "/mnt/rocjitsu/bin/rocjitsu";

/// RocJITsu config root inside every node container, bind-mounted read-only
/// so an in-container process can resolve by-name profile/topology
/// references.
const CONTAINER_CONFIG_DIR: &str = "/mnt/rocjitsu/config";
...
const CONTAINER_LIB_DIR: &str = "/mnt/rocjitsu/lib";
...
/// ran `rocjitsu host --rank N` here, which meant every container had a
/// second rocjitsu process inside it polling a bind-mounted directory.
///
/// Taken from `rj_container`, which is what actually launches the
/// containers, so the plan and the launch cannot disagree.
use rj_container::CONTAINER_IDLE_COMMAND;
...
pub struct Session {
    ...
    /// How many `rocjitsu exec` clients are currently borrowing this
    ...
    /// Set when a client has asked over the control socket for this
    /// session to end.
    ///
    /// Separate from `closing`, which teardown publishes once it has
    /// already begun. This is the request that *starts* teardown, and it
    /// exists for the session that has no workload of its own: nothing
    /// else would ever end it, short of finding the run's pid and
    /// signalling it.
    ...
    cancel: rj_container::Cancel,
}

/// A `rocjitsu exec` client's claim on a session.
///
...
struct Inner {
    ...
    /// than keyed by pid: two `rocjitsu exec`s could in principle come
    ...
    /// this grid, and a `rocjitsu exec` that had to guess it handed its
    ...
    /// what `rocjitsu run` without `--nproc-per-node` asks for.
    job_nproc: u32,
    ...
    /// would hand `rocjitsu run`'s ranks one `MASTER_PORT` and a
    /// `rocjitsu exec` in another terminal a different one — and the two
...
impl Session {
    ...
    pub fn new(def: SessionDef, profile: ProfileDef) -> Result<Arc<Self>> {
        let runtime_dir = rj_core::paths::session_runtime_dir(&def.id);
        std::fs::create_dir_all(&runtime_dir)
            .map_err(|e| RocJITsuError::io(runtime_dir.clone(), e))?;
        ...
        Ok(Arc::new(Self {
            ...
            cancel: rj_container::Cancel::new(),
        }))
    }
    ...
    #[must_use]
    pub fn cancel_switch(&self) -> rj_container::Cancel {
        self.cancel.clone()
    }
    ...
    /// the count is a property of the command `rocjitsu run` was given,
    ...
    /// finished, so a concurrent `rocjitsu session wait` blocks for its
    ...
    /// mirror image: it would tell a `rocjitsu exec` in another terminal
    ...
    /// flips the session's [`Cancel`](rj_container::Cancel)
    ...
    /// `rocjitsu exec` starts its processes in *its own* terminal, in its
    ...
    /// `rocjitsu run`, which calls [`Session::wait_for_borrowers`] before
    ...
    pub fn attach(&self, borrower: Option<u32>, exec: Option<ExecId>) -> Option<SessionLease> {
        ...
        // borrower, so a borrower absent from the map was one `rocjitsu
    ...
    /// How many `rocjitsu exec` clients are borrowing this session.
    #[must_use]
    ...
    /// started and is watching in another terminal, and no timeout rocjitsu
    /// could pick would be right for it. `rocjitsu run` races this against
    ...
    /// Ask this session to end, as `rocjitsu session stop` does.
    ///
    /// Records the request and returns; the run owning the session is
    /// what acts on it, by returning from whatever it is waiting on and
    /// going into its ordinary teardown. Tearing down from here instead
    /// would run teardown on a control-socket task while the run process
    /// carried on waiting, which is the same session being destroyed and
    /// used at once.
    pub fn request_stop(&self) {
        self.stop_requested.send_replace(true);
    }

    /// Resolve once someone has asked this session to end.
    ///
    /// Already-requested resolves immediately, so a run that reaches its
    /// wait after the request arrived does not sit there having missed
    /// it.
    pub async fn wait_stop_requested(&self) {
        let mut rx = self.stop_requested.subscribe();
    ...
    pub async fn start_exec(
        ...
        // actually installed, and exits 0. RocJITsu reports success for a
        ...
        let id = {
            let mut inner = self.lock();
            if inner.tearing_down {
                return Err(RocJITsuError::SessionNotFound(format!(
    ...
    async fn spawn_exec(
        ...
        // the visible cost was the control socket: `rocjitsu run` polls its
        ...
        // nobody. A `rocjitsu exec` in another terminal sat there and was
        ...
        .await
        .map_err(|e| RocJITsuError::other(format!("starting an exec: {e}")))?
    }

    /// Describe this session for a client that wants to start processes
    /// in it.
    ///
    /// The same description `rocjitsu run` builds its own specs from, so an
    ...
    /// `rocjitsu exec` in another terminal joins the job rather than
    ...
    pub fn describe(&self) -> Result<SessionDescription> {
        ...
        // `rocjitsu exec` handed one would run its workload directly on the
        // host, unemulated, against whatever GPU is really installed, and
        // exit 0. The control socket is bound before bring-up starts (so
        // this run is visible to `rocjitsu state purge` from the first
        ...
        let (injection, containers, head_port, job_nproc) = {
            ...
            // exec and any `rocjitsu exec` in another terminal — rendezvous
            // on the same port.
    ...
    /// # Errors
    ///
    /// Returns [`RocJITsuError::ExecNotFound`] if there is no such exec.
    pub async fn remove_exec(&self, id: &ExecId) -> Result<()> {
        let exec = self
            .lock()
            .execs
            .remove(id)
            .ok_or_else(|| RocJITsuError::ExecNotFound(id.to_string()))?;
    ...
    pub async fn teardown(&self) {
        {
            ...
            if already {
                ...
                // first, so `wait_ready` unblocks and `rocjitsu run` reaches
        ...
        // Tell the borrowers. `rocjitsu run` normally waits for them to
        ...
        // that rollback actually happen: without it `rocjitsu run` returns,
        ...
        // above cannot reach: a `rocjitsu exec` starts its processes in its
        ...
        if let Some(backend) =
            rj_core::emulator::get_emulator_backend(&self.ctx.profile.emulator.emulator)
        {
        ...
        if let Some(state) = containers
            && let Err(e) =
                tokio::task::spawn_blocking(move || rj_core::container::teardown(&state)).await
        {
    ...
    /// that — a `rocjitsu exec` that was `SIGKILL`ed. Its workload is
    /// reparented to init, still tagged with this session, still holding
    /// the emulated device; `rocjitsu cleanup` will not touch it, correctly,
    ...
    /// [`rj_core::reclaim`] gives at length: the borrower's own record
    ...
    /// is *staying*. Without it, `rocjitsu exec --help`'s promise that the
    /// workload "dies with it" held only for a borrower that outlived the
    /// run — `SIGKILL` a `rocjitsu exec` while its run is still up and its
    /// workload carried on inside the session indefinitely, holding the
    /// emulated device and invisible to `rocjitsu cleanup`, which is right
    ...
    ///   [`rj_core::reclaim`] states for the same reason.
    /// * A borrower still holding a lease owns whatever descends from it.
    ///   Two `rocjitsu exec`s at once is ordinary — one finishing must not
    ///   take the other's workload with it — and the first's departure is
    ///   exactly when this runs.
    ///
    /// What is left after both is a process started by a `rocjitsu exec`
    ...
    pub async fn reap_departed_borrowers(&self) {
        ...
        // and the answer arriving. A `rocjitsu exec` that attached inside
        ...
        // `SIGKILL`. Two `rocjitsu exec`s at once is ordinary, and one
...
/// `ROCJITSU_EXEC`. That mark is inherited by everything the workload forks
...
///
/// [`rj_core::reclaim::stranded_workloads`] with no live session
...
fn processes_in_session(session: &SessionId) -> Vec<u32> {
    rj_core::reclaim::stranded_workloads(&[])
        .into_iter()
...
/// evidence [`rj_core::reclaim`] reads the session mark from, and for
...
/// rocjitsu older than the mark, or one that scrubbed its own environment.
...
/// # Errors
///
/// Returns [`RocJITsuError::ProfileNotFound`] if a by-name reference cannot
/// be resolved.
pub fn resolve_profile(reference: &MaybeRef<ProfileDef>) -> Result<ProfileDef> {
    match reference {
        ...
        // outside the config root. Today `rocjitsu run` happens to have
        // validated already, but `Run::start` is public API and this is
        // the one place the name becomes a path.
        MaybeRef::Ref(name) => rj_core::store::profile_get(name),
    }
}
...
///
/// Previously this was handled by a second rocjitsu process running inside
...
pub fn plan_container(ctx: &SessionContext, injection: &InjectionDef) -> ContainerPlan {
    ...
    // The rocjitsu binary itself, so anything in-container that needs it can
    // find it at a stable path.
    if let Ok(bin) = std::env::current_exe() {
        mounts.push(FileMount {
            host_path: bin.to_string_lossy().into_owned(),
            container_path: CONTAINER_CLI_BIN.to_string(),
            read_only: true,
        });
    }
    ...
    // per-node `rocjitsu host` process used to re-resolve the whole
    ...
    let config_dir = rj_core::paths::rocjitsu_config_dir();
    ...
    env.push((
        "ROCJITSU_CLI_RUNTIME_DIR".to_string(),
        CONTAINER_RUNTIME_DIR.to_string(),
    ));
    env.push((
        "ROCJITSU_CLI_CONFIG_DIR".to_string(),
...
mod tests {
    ...
    fn a_session(dir: &std::path::Path, profile: ProfileDef) -> Arc<Session> {
        let _guard = rj_core::paths::test_env_lock();
        rj_core::paths::set_test_root(dir);
        let def = make_def(
    ...
    fn a_multi_entry_preload_keeps_every_entry() {
        ...
        // while rocjitsu reported success.
        assert_eq!(
    ...
    fn a_live_sessions_shape_is_the_one_it_was_created_with() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let _guard = rj_core::paths::test_env_lock();
        rj_core::paths::set_test_root(dir.path());
        ...
        rj_core::topology::store::put("shared", &topology).unwrap();
    ...
    async fn teardown_waits_for_a_bring_up_that_is_still_creating_things() {
        ...
        // returned early let `rocjitsu run` exit, the runtime drop the
    ...
    async fn a_session_that_is_tearing_down_never_reports_itself_ready() {
        ...
        // `rocjitsu exec` in another terminal that a session whose
    ...
    fn container_plan_mounts_the_interposer_and_puts_it_on_the_library_path() {
        ...
        let paths: Vec<&str> = plan
            ...
            .collect();
        assert!(
            paths.contains(&"/mnt/rocjitsu/lib/librocjitsu.so"),
            "{paths:?}"
        );
        assert!(
            paths.contains(&"/mnt/rocjitsu/lib/libextra.so"),
            "{paths:?}"
        );
        ...
        assert_eq!(env["LD_PRELOAD"], "/mnt/rocjitsu/lib/librocjitsu.so");
        assert!(env["LD_LIBRARY_PATH"].starts_with("/mnt/rocjitsu/lib"));
    }
    ...
    fn container_plan_does_not_mount_the_same_path_twice() {
        ...
        let lib_mounts: Vec<_> = plan
            .mounts
            .iter()
            .filter(|m| m.container_path.starts_with("/mnt/rocjitsu/lib/"))
            .collect();
    ...
    fn container_plan_preserves_an_emulator_supplied_library_path() {
        ...
        let plan = plan_container(&ctx(dir.path().to_path_buf()), &injection);
        let env: BTreeMap<_, _> = plan.env.iter().cloned().collect();
        assert_eq!(env["LD_LIBRARY_PATH"], "/mnt/rocjitsu/lib:/existing");
    }
    ...
    fn host_paths_under_the_scratch_directory_are_remapped_into_the_container() {
        ...
        assert_eq!(
            env["ROCJITSU_RUNTIME_DIR"], "/mnt/rocjitsu/runtime/rocjitsu",
    ...
    fn container_entrypoint_just_idles() {
        ...
        // rocjitsu process inside it: workloads arrive via `provider exec`.
        let dir = tempfile::tempdir().unwrap();
