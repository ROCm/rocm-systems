//! The socket a `rocjitsu run` serves so other terminals can find it.
//!
//! One socket per run, named after its session, living for exactly as
//! long as the run does. It answers [`Request::Describe`] and
//! [`Request::Attach`] with a [`rj_core::proto::SessionDescription`] and nothing else —
//! see [`rj_core::proto`] for why that is the whole protocol.
//!
...
pub enum BindError {
    ...
    #[error("a rocjitsu run is already serving session `{0}`")]
    ...
    #[error(
        "the control socket path is {} bytes, over the {SUN_PATH_MAX}-byte limit \
         ...
         Point ROCJITSU_CLI_RUNTIME_DIR (or XDG_RUNTIME_DIR) at a shorter directory.",
        .path.as_os_str().len(),
        .path.display(),
    )]
    PathTooLong {
    ...
    #[error(
        "the directory rocjitsu serves its control sockets from is not private: \
         {} ({reason}). Anyone who can reach it can `rocjitsu exec` into this session, \
         which runs commands as you. Point ROCJITSU_CLI_RUNTIME_DIR at a directory only you \
    ...
    /// interpolated into this message: rocjitsu prints an error together
    ...
    #[error("could not create rocjitsu's control socket at {}", .path.display())]
    Io {
...
/// by an older rocjitsu, or widened since.
...
async fn handle(stream: UnixStream, run: Arc<Run>) {
    ...
    // [`Session::reap_departed_borrowers`](rj_supervisor::session::Session::reap_departed_borrowers).
    //
    ...
    let response = match serde_json::from_slice::<Request>(&frame) {
        ...
        // Recorded, not performed. Teardown belongs to the run process,
        // which is waiting on this signal; doing it here would tear the
        // session down from a task the session itself is serving.
        Ok(Request::Stop) => {
            run.request_stop();
            Response::Stopping
        }
    ...
    match ended {
        ...
        LeaseEnd::ClientGone => {
            ...
            // notice it until teardown — which is how `rocjitsu exec`'s
...
mod tests {
    ...
    async fn every_directory_created_for_a_socket_is_private() {
        ...
        let path = shared.join("rocjitsu/run/private.sock");
        let _socket = ControlSocket::bind(&path).await.unwrap();

        for level in [shared.join("rocjitsu"), shared.join("rocjitsu/run")] {
    ...
    async fn a_directory_that_was_already_there_is_narrowed_before_it_is_used() {
        ...
        // level rocjitsu *creates* is private by construction —
        ...
        // directory rocjitsu is about to serve its socket out of already
        ...
        assert_eq!(
            ...
            "a directory that already existed was served out of at mode {mode:04o}; \
             anyone who can reach it can `rocjitsu exec` into this session"
        );
    }
    ...
    async fn a_failure_to_create_the_socket_names_the_path_once() {
        ...
        // and named neither the path nor what rocjitsu was doing with it.
