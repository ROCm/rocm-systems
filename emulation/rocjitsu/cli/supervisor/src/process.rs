...
//!
//! This module is where rocjitsu's zombie problem was, and where it is
...
//!    reparented to init, while rocjitsu reported the exec finished and
...
//!
//! # What is deliberately not covered: `SIGKILL` of rocjitsu itself
//!
//! Every property above needs rocjitsu to still be running — a signal it
//! sends, a `Drop` it runs, a task tokio cancels. `kill -9`, and the OOM
//! killer picking rocjitsu during a large emulated job, leave no code of
...
//! to strand its workloads, and `rocjitsu cleanup` reclaims them: every
//! workload carries [`ROCJITSU_SESSION`](rj_core::container::ENV_SESSION)
//! in its environment, the same way every container carries a
//! `rocjitsu.owner` label, so the recovery path can find what is stranded
//! without any bookkeeping that a `SIGKILL` could interrupt.
//!
//! # Standard streams
//!
//! A workload gets the *caller's* streams, not ones rocjitsu manufactured.
//! Whoever spawns a process here — `rocjitsu run`, or a `rocjitsu exec` in
...
//! terminal.
//!
//! That is worth stating because the obvious alternative is what rocjitsu
...
//! stdin *is* the terminal, not because rocjitsu emulated one.
//!
...
//! Capturing puts rocjitsu back in the middle so it can prefix each chunk
...
/// sitting in front of. RocJITsu never allocates a pseudo-terminal: if the
...
pub enum StdioMode {
    ...
    /// the terminal directly with rocjitsu not in the middle.
    ///
...
impl StdioMode {
    ...
    /// `rocjitsu run -- bash` an ordinary interactive shell.
    ///
    /// More than one and every process is captured, so rocjitsu can label
    ...
    /// one there: `rocjitsu exec --node 2 -- bash`. That is a single-process
    ...
    /// Whether output is pumped through rocjitsu rather than written to
...
pub struct SpawnSpec {
    ...
    ///
    /// Inheriting is the default because rocjitsu runs in the terminal the
    ...
    ///
    /// `rocjitsu run --clear-env-vars` asks for the strict form, for a run
...
/// Variables whose value is a `:`-separated search list, where rocjitsu's
...
/// not happen.
///
/// RocJITsu's entries go first: the interposer and the emulator's libraries
...
pub fn spawn(spec: &SpawnSpec, output: mpsc::Sender<OutputChunk>) -> Result<Spawned, String> {
    ...
    cmd.args(&spec.args)
        ...
        //
        // It also means no group rocjitsu signals is ever *rocjitsu's own*,
        // which is what keeps `kill(-pgid)` from reaching the shell the
        // user started rocjitsu from.
        //
        ...
        // `rj_ctl::run`'s terminal handoff, which is exactly what a
        ...
        //
        // The only backstop. `SIGKILL` of rocjitsu runs none of this and is
        // deliberately not covered here; see the module documentation for
        // why, and `rocjitsu cleanup` for what recovers from it.
        .kill_on_drop(true);
    ...
    // to the terminal without rocjitsu ever seeing the bytes.
...
/// One `:`-separated search list built from rocjitsu's entries and the
...
impl Spawned {
    ...
    ///
    /// Used for `rocjitsu exec signal`, where the caller asked to deliver a
    ...
    /// delivers `rocjitsu exec signal` to a proxy and leaves the workload
    /// untouched.
    ...
    pub async fn terminate(&mut self) -> Exit {
        ...
        // with rocjitsu reporting the exec as finished.
        ...
        // survives teardown with rocjitsu reporting the exec as finished,
        ...
        let status = match tokio::time::timeout(TERM_GRACE, self.child.wait()).await {
            Ok(status) => {
                ...
                // signalled running inside the container with rocjitsu
    ...
    /// while rocjitsu removed the session it belonged to. Attaching the
...
mod tests {
    ...
    async fn env_is_not_inherited_unless_asked() {
        ...
        // did with `ROCJITSU_TEST_LEAK` — proves nothing: the child prints
    ...
    #[test]
    fn a_search_path_keeps_the_callers_entries_behind_rocjitsus() {
        ...
        // rocjitsu's own entries still have to win the search.
        assert_eq!(
            merged_search_path("/rocjitsu/lib", std::ffi::OsStr::new("/home/me/lib")),
            std::ffi::OsString::from("/rocjitsu/lib:/home/me/lib")
        );
    ...
    async fn a_search_path_the_caller_does_not_have_is_passed_through() {
        ...
        s.inherit_env = true;
        s.env.insert(key.to_string(), "/rocjitsu/py".to_string());
        let (_, chunks) = run(&s).await;
        assert_eq!(text(&chunks, StdStream::Stdout).trim(), "/rocjitsu/py");
    }
    ...
    async fn a_workload_that_exits_on_its_own_still_has_its_tree_reaped() {
        ...
        // the grandchild running, reparented to init, while rocjitsu
        // reported the exec finished and deleted the session it belonged
        // to — `rocjitsu cleanup` would then call it a stranded process of
    ...
    async fn a_captured_process_gets_no_stdin_at_all() {
        // `--capture-all` means rocjitsu is reading the output, and there
