...
//!
//! * `rocjitsu run`, which describes the session it just brought up, and
//! * `rocjitsu exec`, which was handed the same description over the run's
//!   socket
//!
...
//! rocjitsu resolved — and both callers necessarily agree on it.
//!
...
pub(crate) const CONTAINER_RUNTIME_DIR: &str = "/mnt/rocjitsu/runtime";

/// The exec a workload process was started by, recorded in its
/// environment.
///
/// The third mark, beside [`ENV_SESSION`] and [`ENV_RUNTIME`], and the
/// one that answers a question the other two cannot: *which* rocjitsu
/// process in this session started it. A run and every `rocjitsu exec`
...
/// [`rj_core::container`] because nothing outside this crate sets it
/// or reads it: it is a supervisor-internal attribution, not part of the
/// on-process contract `rocjitsu cleanup` recovers from a crash with.
pub const ENV_EXEC: &str = "ROCJITSU_EXEC";

/// Sanitizer runtime(s) to load ahead of the emulator's interposer.
///
/// A `:`-separated list of absolute paths, in `LD_PRELOAD` order, set on
/// the rocjitsu process by whoever built the emulator — in practice
/// rocjitsu's CTest suite, which knows its own `RJ_ASAN_SHARED_LIBRARY` /
/// `RJ_TSAN_SHARED_LIBRARY` and passes them through.
///
/// It exists because a shared-sanitizer build of `librocjitsu.so` cannot
/// be `LD_PRELOAD`ed on its own: `libasan` has to be the *first* object
/// in the link order or the interception it installs comes too late, and
/// `ld.so` refuses the process outright ("ASan runtime does not come
/// first in initial library list"). The interposer is the emulator's, so
/// the sanitizer beside it can only come from the build that produced it.
///
/// Set on the launcher and read once, rather than passed as a flag,
/// because every workload in the session needs it and nobody types it.
...
///
/// The one ambient read on this path, kept next to the pure function that
/// consumes it so a caller passing something else (a test, or a future
/// caller with the value in hand) is the normal case rather than a
/// workaround.
#[must_use]
pub fn sanitizer_preload_from_env() -> Option<String> {
    std::env::var(ENV_SANITIZER_PRELOAD).ok()
}

/// The emulator's `LD_PRELOAD` with any sanitizer runtime placed ahead of
/// it.
///
/// Order is the whole content of this function, and it is the order the
/// C++ launcher used: sanitizer, then interposer, then — added later, by
/// [`process_env`] — whatever the user's `--env LD_PRELOAD` asked for.
///
/// Entries that do not exist are dropped rather than passed on. A
/// sanitizer path is a build's claim about its own tree, and a stale or
/// wrong one is not a preload that gets ignored: `ld.so` fails the whole
/// workload with "cannot be preloaded", which reads as the emulator being
/// broken rather than as one variable being out of date.
#[must_use]
pub fn with_sanitizer_preload(interposer: Option<&str>, sanitizer: Option<&str>) -> Option<String> {
    let existing = |path: &&str| {
        if std::path::Path::new(path).exists() {
            return true;
        }
        tracing::warn!(
            "{ENV_SANITIZER_PRELOAD} names {path}, which does not exist; not preloading it"
        );
        false
    };
    let entries: Vec<&str> = sanitizer
        .unwrap_or_default()
        .split(':')
        .filter(|entry| !entry.is_empty())
        .filter(existing)
        .chain(interposer.into_iter().filter(|p| !p.is_empty()))
        .collect();
...
impl CallerStreams {
    ...
    ///
    /// The call both `rocjitsu run` and `rocjitsu exec` make, once, on their
...
/// the owning `rocjitsu run` was started with — and it is what every rank
...
///
/// For `rocjitsu run` the two are equal, which is why conflating them went
/// unnoticed: the run's own exec is the job. For `rocjitsu exec` they are
/// not. Computing the world from the exec's count told a
/// `rocjitsu exec` into a `--nproc-per-node 3` job that the world had two
...
///
/// * `rocjitsu exec -- cmd` starts one process per node, each of them its
///   node's local rank 0 — ranks `0`, `P`, `2P`, … of a `WORLD_SIZE` of
///   `N * P`.
/// * `rocjitsu exec --node n -- cmd` starts exactly that node's local rank
...
/// [`SessionDescription::workdir`] — the directory `rocjitsu run` was
/// typed in — which is what makes `rocjitsu run -- ./my-app` behave like
/// running `./my-app`.
///
...
/// `chdir` error about a path the user never asked for. So rocjitsu passes
...
/// of rocjitsu's, and `--workdir` overrides it; a path there is checked
...
pub fn build_specs(
        ...
        .ok_or_else(|| {
            RocJITsuError::other(format!(
                "{node_count} nodes x {job_nproc} processes per node is {} processes, \
                 more than the {MAX_WORLD_SIZE} rocjitsu will start for one exec",
    ...
    if let Some(targets) = &desc.containers {
        ...
        std::fs::create_dir_all(&dir).map_err(|e| RocJITsuError::io(dir, e))?;
    }
    ...
    // necessarily agree on it: a `rocjitsu exec` reached this session
    ...
    // could differ.
    let runtime = rj_core::container::owning_runtime();
    ...
    for node in nodes {
        for local in 0..nproc {
            ...
            // the point of `rocjitsu exec` — so a run reaping what a
            ...
            specs.push(match &desc.containers {
                ...
                Some(targets) => {
                    let container = targets
                        ...
                        .ok_or_else(|| {
                            RocJITsuError::other(format!(
                                "session has no container for node {node}"
                            ))
                        })?
                        .to_string();
                    // No [`Cancel`](rj_container::Cancel) on this
                    ...
                    let engine = rj_container::Engine::with_provider(&targets.provider);
                    // A `--workdir` in a containerised session names a
                    // path in the *image's* filesystem, so the host-side
                    // check `rocjitsu run` and `rocjitsu exec` do before they
                    ...
                    // [`Engine::check_workdir`](rj_container::Engine::check_workdir).
                    if let Some(workdir) = args.workdir.as_deref()
                        && asked.insert(workdir)
                    {
                        engine
                            .check_workdir(&container, workdir)
                            .map_err(|e| RocJITsuError::other(e.to_string()))?;
                    }
                    let mut env = env;
                    ...
                    // for that process, so a variable rocjitsu sets on both
                    ...
                    // container's own `rocjitsu.runtime` label, neither of
                    ...
                    SpawnSpec {
                        ...
                        // other rocjitsu can tell, to no runtime directory
                        // at all.
                        env: BTreeMap::from([
...
/// pty per rank would only give rocjitsu's output labeller a second stream
/// to untangle.
///
...
/// they are not, it is destructive: `rocjitsu run --image X -- job > out
...
/// The rocjitsu/`torch.distributed` environment for one workload process.
///
...
fn proc_env(
    ...
    vec![
        ...
        // `SIGKILL`ed — see [`rj_core::reclaim`].
        (ENV_SESSION.to_string(), session.as_str().to_string()),
        // And which rocjitsu that session belongs to. The session name is
        ...
        // healthy session of a rocjitsu running elsewhere on the machine.
        //
        // Resolved, not inherited: this overrides whatever the caller
        // exported, which is also what makes it a correct *input* for a
        // nested `rocjitsu` — a workload that runs one sees the state
        ...
        // GPU per node.
        (ENV_NCCL_HOSTID.to_string(), format!("rocjitsu-node-{node}")),
    ]
}
...
mod tests {
    ...
    use super::*;
    use rj_core::proto::ContainerTargets;
    ...
    fn job(node_count: u32, nproc: u32) -> SessionDescription {
        SessionDescription {
            ...
            head_addr: "rocjitsu-s-node-0".to_string(),
            head_port: 6000,
        }
    }
    ...
    fn a_single_process_job_gets_the_terminal_whole() {
        // The interactive case: `rocjitsu run -- bash` on a one-node
    ...
    fn a_multi_node_job_labels_every_rank_and_gives_none_of_them_stdin() {
        ...
        // and one terminal cannot be shared between readers — so rocjitsu
    ...
    fn naming_one_node_makes_the_exec_interactive() {
        ...
        // terminal. This is what `rocjitsu exec --node 2 -- bash` is for.
    ...
    fn a_named_node_keeps_the_rank_identity_of_that_node() {
        ...
        assert_eq!(at("ROCJITSU_RANK"), "2");
        assert_eq!(at("RANK"), "2");
        assert_eq!(at("WORLD_SIZE"), "4", "the session's size, not this exec's");
        assert_eq!(at("MASTER_ADDR"), "rocjitsu-s-node-0");
    }
    ...
    // ----- sanitizer preload ordering ------------------------------------
    //
    // Ported from the C++ launcher's `launch_preload_test.cpp`, whose
    // subject was `prepend_launch_preloads`. The ordering it asserted is
    // now split across two places — [`with_sanitizer_preload`] puts the
    // sanitizer ahead of the interposer, [`process_env`] puts the user's
    // own preload behind both — so the cases are stated against whichever
    // half decides them, and the last one against the pair together.

    /// A file that exists, since a sanitizer path that is not there is
    /// deliberately dropped.
    fn a_real_library(dir: &tempfile::TempDir, name: &str) -> String {
    ...
    #[test]
    fn without_a_sanitizer_the_interposer_is_the_whole_preload() {
        assert_eq!(
            with_sanitizer_preload(Some("/tmp/librocjitsu.so"), None),
            Some("/tmp/librocjitsu.so".to_string())
        );
        // An empty value is how a CMake variable that was never computed
        // arrives, and it means the same as unset.
        assert_eq!(
            with_sanitizer_preload(Some("/tmp/librocjitsu.so"), Some("")),
            Some("/tmp/librocjitsu.so".to_string())
        );
    }

    #[test]
    fn a_sanitizer_runtime_goes_ahead_of_the_interposer() {
        // The point of the variable. ASan has to be the first object in
        // the initial library list or `ld.so` refuses the process.
        let dir = tempfile::tempdir().unwrap();
        let asan = a_real_library(&dir, "libasan.so.8");
        assert_eq!(
            with_sanitizer_preload(Some("/tmp/librocjitsu.so"), Some(&asan)),
            Some(format!("{asan}:/tmp/librocjitsu.so"))
        );
    }

    #[test]
    fn several_sanitizer_runtimes_keep_the_order_they_were_given() {
        // The variable is a `:`-separated list like `LD_PRELOAD` itself,
        // so a build with both runtimes states their order rather than
        // having one picked for it.
        let dir = tempfile::tempdir().unwrap();
        let asan = a_real_library(&dir, "libasan.so.8");
        let tsan = a_real_library(&dir, "libtsan.so.2");
        assert_eq!(
            with_sanitizer_preload(Some("/tmp/librocjitsu.so"), Some(&format!("{asan}:{tsan}"))),
            Some(format!("{asan}:{tsan}:/tmp/librocjitsu.so"))
        );
    }

    #[test]
    fn a_sanitizer_path_that_is_not_there_is_dropped_rather_than_passed_on() {
        // `ld.so` fails the whole workload on a preload it cannot open,
        // so a stale path would present as the emulator being broken.
        // The C++ version reached the same place from the other side: it
        // took the runtime from `dladdr`, and refused anything that did
        // not canonicalise to an existing file.
        assert_eq!(
            with_sanitizer_preload(
                Some("/tmp/librocjitsu.so"),
                Some("/nonexistent/libasan.so.8")
            ),
            Some("/tmp/librocjitsu.so".to_string())
        );
        // Including when it is the only thing named: an unloadable
        // sanitizer must not become the preload on its own.
        assert_eq!(
            with_sanitizer_preload(None, Some("/nonexistent/libasan.so.8")),
            None
        );
    }

    #[test]
    fn an_emulator_that_preloads_nothing_still_gets_its_sanitizer() {
        // `rocjitsu-dbt` sets `HSA_TOOLS_LIB` and no `LD_PRELOAD` at all.
        // A sanitizer build of it still needs its runtime loaded first.
        let dir = tempfile::tempdir().unwrap();
        let asan = a_real_library(&dir, "libasan.so.8");
        assert_eq!(
            with_sanitizer_preload(None, Some(&asan)),
            Some(asan.clone())
        );
        assert_eq!(with_sanitizer_preload(None, None), None);
    }

    #[test]
    fn an_unset_or_empty_user_preload_leaves_the_emulators_alone() {
        // Two of the C++ cases: no `LD_PRELOAD` in the environment, and
        // one set to the empty string. Neither may leave a stray `:`.
        let specs = build_specs(&desc(1), &exec_def(1, None), &id(), CAPTURED).unwrap();
        assert_eq!(
            specs[0].env.get("LD_PRELOAD").map(String::as_str),
        ...
        let mut def = exec_def(1, None);
        def.exec.env.insert("LD_PRELOAD".to_string(), String::new());
        let specs = build_specs(&desc(1), &def, &id(), CAPTURED).unwrap();
        assert_eq!(
            specs[0].env.get("LD_PRELOAD").map(String::as_str),
            Some("/lib/interpose.so")
    ...
    #[test]
    fn the_users_own_preload_stays_behind_the_sanitizer_and_the_interposer() {
        // The full order, end to end, and the one the C++ test named:
        // sanitizer, interposer, then whatever the user asked for.
        let dir = tempfile::tempdir().unwrap();
        let asan = a_real_library(&dir, "libasan.so.8");
        let mut session = desc(1);
        session.ld_preload = with_sanitizer_preload(Some("/lib/interpose.so"), Some(&asan));

        let mut def = exec_def(1, None);
        def.exec
            .env
            .insert("LD_PRELOAD".to_string(), "/user/mine.so".to_string());
        let specs = build_specs(&session, &def, &id(), CAPTURED).unwrap();
        assert_eq!(
            specs[0].env.get("LD_PRELOAD").map(String::as_str),
            Some(format!("{asan}:/lib/interpose.so:/user/mine.so").as_str())
        );
    }
    ...
    fn naming_a_node_of_a_multi_process_job_gets_that_nodes_first_rank() {
        // `rocjitsu exec --node 1 -- bash` on a three-process-per-node job.
    ...
    fn an_exec_may_fill_as_many_of_a_nodes_slots_as_the_job_has() {
        ...
        // exactly, which is the equivalence `rocjitsu exec` rests on.
    ...
    fn every_rank_spells_the_rendezvous_address_the_same_way() {
        ...
        assert_eq!(
            addrs,
            vec!["rocjitsu-s-node-0"; specs.len()],
            "the ranks disagree about where the rendezvous is"
        );
        // `ROCJITSU_HEAD_ADDR` is the same value under rocjitsu's own name,
        // so it splits in exactly the same way if it is computed twice.
        let heads: Vec<&str> = specs
            .iter()
            .filter_map(|s| s.env.get("ROCJITSU_HEAD_ADDR").map(String::as_str))
            .collect();
    ...
    fn a_containerised_exec_goes_through_the_provider_and_records_its_pid() {
        ...
        d.containers = Some(ContainerTargets {
            ...
            names: vec![
                "rocjitsu-s-node-0".to_string(),
                "rocjitsu-s-node-1".to_string(),
            ],
    ...
    fn every_process_is_tagged_with_its_session() {
        ...
        // owning run being `SIGKILL`ed, so `rocjitsu cleanup` can find what
        // was stranded. See [`rj_core::reclaim`].
        let specs = build_specs(&job(2, 2), &exec_def(2, None), &id(), CAPTURED).unwrap();
        assert_eq!(specs.len(), 4);
        for spec in &specs {
            assert_eq!(
                spec.env.get("ROCJITSU_SESSION").map(String::as_str),
                Some("s"),
    ...
    fn every_process_records_the_runtime_directory_that_owns_it() {
        ...
        let _paths = rj_core::paths::test_env_lock();
        // The other half of the tag. A session name means something only
        // to the rocjitsu that issued it, so without this a `rocjitsu
        // cleanup` running under a different `ROCJITSU_CLI_RUNTIME_DIR` sees a
        // healthy workload as a crashed run's leftovers.
        let runtime = rj_core::container::owning_runtime();
        let specs = build_specs(&job(2, 2), &exec_def(2, None), &id(), CAPTURED).unwrap();
        for spec in &specs {
            assert_eq!(
                spec.env.get("ROCJITSU_CLI_RUNTIME_DIR").map(String::as_str),
                Some(runtime.as_str()),
                "rank {} does not say which rocjitsu owns it",
                spec.node
            );
        }
    }
    ...
    fn a_user_supplied_runtime_does_not_become_the_ownership_marker() {
        ...
        // this test stamps and the value it asserts the same one.
        let _paths = rj_core::paths::test_env_lock();
        // Same rule as the rank variables: rocjitsu's own bookkeeping wins
        ...
        assert_eq!(
            specs[0]
                .env
                .get("ROCJITSU_CLI_RUNTIME_DIR")
                .map(String::as_str),
            Some(rj_core::container::owning_runtime().as_str())
        );
    }
    ...
    fn every_process_says_which_exec_started_it() {
        ...
        // departed `rocjitsu exec` left behind would take its own live
        // workload with it.
        let specs = build_specs(&job(2, 2), &exec_def(2, None), &id(), CAPTURED).unwrap();
        assert_eq!(specs.len(), 4);
        for spec in &specs {
    ...
    fn a_containerised_provider_client_is_tagged_too() {
        ...
        d.containers = Some(ContainerTargets {
            provider: "podman".to_string(),
            names: vec!["rocjitsu-s-node-0".to_string()],
            scratch: scratch.path().to_path_buf(),
        });
        let specs = build_specs(&d, &exec_def(1, None), &id(), CAPTURED).unwrap();
        assert_eq!(
            specs[0].env.get("ROCJITSU_SESSION").map(String::as_str),
            Some("s")
        );
        ...
        assert!(
            specs[0]
                .args
                .windows(2)
                .any(|w| w[0] == "-e" && w[1] == "ROCJITSU_SESSION=s"),
            "{:?}",
            specs[0].args
        );
        ...
        // there, and a nested rocjitsu would resolve it to nothing.
        assert!(
            specs[0].args.windows(2).any(|w| w[0] == "-e"
                && w[1] == format!("ROCJITSU_CLI_RUNTIME_DIR={CONTAINER_RUNTIME_DIR}")),
            "{:?}",
            specs[0].args
        );
    }
    ...
    fn a_terminal_caller_reaches_provider_dash_t_and_a_redirected_one_does_not() {
        ...
        // they type `rocjitsu run --image X -- bash` was the one branch
    ...
    fn a_workload_inherits_the_callers_environment_by_default() {
        // RocJITsu's parent is the terminal the user typed in, so what they
    ...
    fn clearing_the_environment_does_not_change_a_containerised_exec() {
        ...
        // sees exactly what rocjitsu passes with `-e`. The flag governs the
    ...
    /// in for the container's. The same stand-in `rj_container`'s
    ...
    fn containerised_nodes(nodes: u32, scratch: &std::path::Path) -> SessionDescription {
        ...
        d.containers = d.containers.map(|mut c| {
            c.names = (0..nodes).map(|n| format!("rocjitsu-s-node-{n}")).collect();
            c
        });
        d.env.insert(
            ENV_NCCL_SOCKET_IFNAME.to_string(),
    ...
    #[test]
    fn an_interface_the_user_named_beats_rocjitsus_correction() {
    ...
    fn a_containerised_workdir_the_image_lacks_is_refused_by_name() {
        ...
        assert!(message.contains("rocjitsu-s-node-0"), "{message}");
