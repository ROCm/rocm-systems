//! `rj_supervisor`: the engine a `rocjitsu run` is built out of.
//!
//! # One owner, no exceptions
//!
//! Every process rocjitsu starts is owned by the process that asked for it,
//! and dies with it. `rocjitsu run` owns its session's containers, its
//! emulator daemon and its workload processes; a `rocjitsu exec` in another
...
//!
//! That is a deliberate reversal. RocJITsu previously had a long-lived
//! supervisor daemon owning every session, because sessions outlived the
//! commands that created them; before that, a detached `rocjitsu host` per
...
//! * [`rpc`] — the socket a run serves so `rocjitsu exec` can find it.
