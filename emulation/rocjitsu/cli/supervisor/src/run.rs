...
//!
//! A `Run` is created by `rocjitsu run` and lives exactly as long as that
...
//! grow without bound.
//!
//! Making `rocjitsu run` the owner removes the map, and with it every one
...
impl Run {
    ...
    /// call it before serving the control socket, so that a `rocjitsu exec`
    ...
    ///
    /// How `rocjitsu run` shows bring-up progress: every phase publishes
    ...
    /// did, which is the moment the run learns that a `rocjitsu exec` is
    /// not coming back.
    ...
    /// Record that a client has asked this session to end.
    ...
    pub async fn wait_stop_requested(&self) {
        self.session.wait_stop_requested().await;
    }
    ...
    /// rocjitsu. Progress-based timing alone would not cover them, because
    ...
    /// # Errors
    ///
    /// Returns [`RocJITsuError::Timeout`] if the session is still not
    ...
    pub async fn wait_ready(&self, timeout: Duration) -> Result<SessionHealth> {
        ...
        //
        // *Suspending* the clock covers a phase whose duration rocjitsu does
        ...
        // and `rocjitsu run` tears down a session whose image was
        ...
        loop {
            ...
            match changed {
                ...
                Ok(Err(_)) => return Err(RocJITsuError::SessionNotFound(id.to_string())),
                Err(_elapsed) => {
                    return Err(RocJITsuError::Timeout(format!(
    ...
    async fn bring_up(session: Arc<Session>) {
        match Self::bring_up_inner(&session).await {
            ...
            Err(e) => {
                ...
                // caller reads it from and how `rocjitsu run` renders the
    ...
    async fn bring_up_inner(session: &Arc<Session>) -> Result<()> {
        ...
        if session.ctx.daemon {
            ...
            let daemon = tokio::task::spawn_blocking(move || {
                let Some(backend) =
                    rj_core::emulator::get_emulator_backend(&ctx.profile.emulator.emulator)
                else {
                    return Ok(None);
                };
            ...
            .await
            .map_err(|e| RocJITsuError::other(format!("emulator daemon task failed: {e}")))?;
            match daemon {
                Ok(handle) => {
                    if handle.is_some() {
                    ...
                    // rocjitsu's own bookkeeping and nothing else. What
                    // they should see instead is printed by `rocjitsu run`.
    ...
    async fn check_daemon_capability(ctx: &SessionContext) -> Result<()> {
        ...
        tokio::task::spawn_blocking(move || {
            rj_core::emulator::get_emulator_backend(&kind)
                .map_or(Ok(()), |backend| backend.daemon_capability())
        })
        .await
        .map_err(|e| RocJITsuError::other(format!("emulator capability task failed: {e}")))?
    }
    ...
    ///
    /// A sanitizer runtime named by [`crate::spec::ENV_SANITIZER_PRELOAD`]
    /// is folded into the injection here rather than into the workload's
    /// environment further down, so that it is a preload like any other:
    /// a containerised session bind-mounts every entry of
    /// `InjectionDef::ld_preload` and rewrites each to its in-container
    /// path. Prepend it after that and the workload would be handed a
    /// host path for a file the container does not have.
    async fn resolve_injection(ctx: &SessionContext) -> Result<InjectionDef> {
        let ctx = ctx.clone();
        let sanitizer = crate::spec::sanitizer_preload_from_env();
        tokio::task::spawn_blocking(move || {
            let kind = &ctx.profile.emulator.emulator;
            let mut injection = match rj_core::emulator::get_emulator_backend(kind) {
                Some(backend) => backend.injection_def(&ctx)?,
                None => return Err(RocJITsuError::other(format!("unknown emulator `{kind}`"))),
            };
            injection.ld_preload = crate::spec::with_sanitizer_preload(
                injection.ld_preload.as_deref(),
                sanitizer.as_deref(),
            );
            Ok(injection)
        })
        .await
        .map_err(|e| RocJITsuError::other(format!("emulator injection task failed: {e}")))?
    }
    ...
    async fn bring_up_containers(session: &Arc<Session>, injection: &InjectionDef) -> Result<()> {
        ...
        // `covers_rocjitsu_dir`, which is true for the reserved directory
        // and its ancestors and false for everything *below* it — it has
        // to be, because rocjitsu's own mounts are all down there and a
        // check that caught them would refuse every containerised
        // session. So a user mount at `/mnt/rocjitsu/runtime/rj_config.json`
        ...
        // is the user's and everything in `plan.mounts` is rocjitsu's.
        reject_mounts_over_rocjitsus_own(&def.mounts, &plan.mounts)?;
        ...
        let result = tokio::task::spawn_blocking(move || -> Result<_> {
            let engine = rj_container::Engine::resolve(&def)
            ...
            if let (Some(tag), Some(dockerfile)) = (
                rj_core::profile::hacks_image_tag(&def.image, &def.hacks),
                rj_core::profile::hacks_dockerfile(&def.image, &def.hacks),
            ) {
                if engine.image_present(&tag) {
                    ...
                    engine.build_image(&tag, &dockerfile).map_err(|e| {
                        RocJITsuError::other(format!("building derived image {tag} failed: {e}"))
                    })?;
                }
                def.image = tag;
            }
            ...
            let mut last_phase: Option<rj_container::BringUpPhase> = None;
            ...
            outcome.map_err(|e| {
                ...
                RocJITsuError::other(context)
            })
        })
        .await
        .map_err(|e| RocJITsuError::other(format!("container bring-up task failed: {e}")))??;
        ...
        if let Some((containers, mut clients)) = session.set_containers(containers, clients) {
            ...
            // and a warning about it is rocjitsu narrating its own
            ...
            let _ = tokio::task::spawn_blocking(move || {
                ...
                rj_core::container::teardown(&containers);
            })
            .await;
        }
        Ok(())
...
/// Refuse a user mount that overlaps one of rocjitsu's own.
///
/// `rocjitsu` bind-mounts its binary, the session scratch directory, the
...
/// # Errors
///
/// Returns an error naming both the user's mount and the rocjitsu
/// destination it lands on.
fn reject_mounts_over_rocjitsus_own(
    user: &[rj_core::profile::FileMount],
    ours: &[rj_core::profile::FileMount],
) -> Result<()> {
    for mount in user {
        let Some(hit) = ours.iter().find(|ours| {
            rj_core::container::container_paths_overlap(&mount.container_path, &ours.container_path)
        }) else {
            continue;
        };
        return Err(RocJITsuError::other(format!(
            "--mount {}: `{}` inside the container is rocjitsu's own — it is where rocjitsu \
            ...
            hit.container_path,
            hit.host_path,
            rj_core::container::CONTAINER_ROCJITSU_DIR,
        )));
    }
    Ok(())
}
...
mod tests {
    ...
    async fn a_waiter_that_arrives_late_still_learns_why_bring_up_failed() {
        ...
        // `rocjitsu run` then said "session failed to start (stopped)",
    ...
    async fn bring_up_that_keeps_moving_is_never_timed_out() {
        ...
        // four nodes, and `rocjitsu run` tore it down.
        let dir = tempfile::tempdir().unwrap();
        let run = stalled_run(dir.path());
    ...
    async fn bring_up_that_stops_moving_is_timed_out() {
        ...
        assert!(
            matches!(err, RocJITsuError::Timeout(_)),
