...
use rj_core::exec::{ExecDef, ExecId, ExecStatus, NodeStatus};
...
pub struct Exec {
    ...
    /// rocjitsu a background process group and diverts Ctrl-C away from
...
impl Exec {
    ...
    pub fn start(
        ...
        for (node, reason) in failures {
            eprintln!("rocjitsu: node {node}: {reason}");
        }
    ...
    /// The pid map is keyed by *global* rank, so `rocjitsu exec --node 2`
    ...
    pub async fn signal(&self, sig: i32) -> Result<()> {
        let signal = Signal::try_from(sig)
            .map_err(|_| RocJITsuError::other(format!("invalid signal: {sig}")))?;
...
impl Drop for Exec {
    ...
    /// but `rocjitsu exec` never builds a `Run` — it holds an `Exec`
...
mod tests {
    ...
    use super::*;
    use rj_core::exec::{ExecArgs, StdStream};
    use rj_core::session::SessionId;
    ...
    async fn a_failed_spawn_still_produces_a_terminal_exec() {
        ...
        // The reason itself goes to rocjitsu's own stderr — it is a rocjitsu
    ...
    async fn a_single_node_exec_offers_its_terminal_whatever_rank_it_is() {
        // `rocjitsu exec --node 2 -- bash`: one process, on the caller's
    ...
    async fn a_grid_never_offers_its_terminal_even_when_only_one_rank_survives() {
        ...
        // handed the terminal away, which diverts Ctrl-C from rocjitsu and
