...
/// its arguments; see `rj_supervisor::CallerStreams`.
const CAPTURED: rj_supervisor::CallerStreams = rj_supervisor::CallerStreams::new(false);
...
const STUB_ENV: &str = "ROCJITSU_STUB_EMULATOR";
...
impl EmulatorBackend for Stub {
    ...
    fn injection_def(&self, _ctx: &SessionContext) -> rj_core::error::Result<InjectionDef> {
        Ok(InjectionDef {
...
impl EmulatorBackend for NoDaemonStub {
    ...
    fn start_daemon(
        &self,
        _ctx: &SessionContext,
    ) -> rj_core::error::Result<Option<Box<dyn rj_core::emulator::EmulatorDaemon>>> {
        Err(rj_core::error::RocJITsuError::other(
...
impl EmulatorBackend for NoCapabilityStub {
    ...
    fn daemon_capability(&self) -> rj_core::error::Result<()> {
        Err(rj_core::error::RocJITsuError::other(
...
/// Point rocjitsu's XDG roots at a scratch directory for this process.
///
...
fn profile(nodes: u32) -> ProfileDef {
    ProfileDef {
        ...
        emulator: EmulatorDef {
            ...
            exec_mode: rj_core::emulator::ExecMode::default(),
            options: rj_core::common::SimpleMap::default(),
            topology: MaybeRef::Owned(rj_core::topology::TopologyDef {
                num_nodes: nodes,
                gpus_per_node: 1,
                agent: MaybeRef::Owned(rj_core::agent::AgentDef::default()),
            }),
        },
        containerize: None,
    }
...
async fn every_node_gets_a_process_with_its_own_rank() {
    ...
    let (exec, mut output) = run
        .exec(&def(&run, "test \"$RANK\" = \"$ROCJITSU_RANK\"", None))
        .await
        .expect("exec starts");
    let drain = tokio::spawn(async move { while output.recv().await.is_some() {} });
    tokio::time::timeout(Duration::from_secs(30), exec.wait_finished())
...
/// shape `rj_core::reclaim`'s `Tagged` uses, for the same reason.
///
...
async fn a_workload_that_forks_has_its_whole_tree_reaped() {
    ...
    assert!(
        rj_supervisor::process::wait_gone(pid, Duration::from_secs(10)).await,
...
async fn a_workload_that_exits_normally_takes_its_forks_with_it() {
    ...
    // finishes, which is what almost every run does. `rocjitsu run -- sh -c
    // "nohup sleep 900 & echo spawned"` exited 0, removed the session's
    // scratch directory, and left the `sleep` running — and `rocjitsu
...
async fn a_description_lets_another_process_build_the_same_processes() {
    // This is what `rocjitsu exec` does: it never sees the `Session`, only
    ...
    let specs = rj_supervisor::build_specs(&desc, &def(&run, "exit 0", None), &id, CAPTURED)
        .expect("specs");
...
async fn an_exec_built_from_a_description_runs_like_one_built_by_the_run() {
    // The equivalence `rocjitsu exec` depends on, asserted directly.
    let run = start(1).await;
    let desc = run.describe().unwrap();
    let d = def(&run, &format!("test -n \"${STUB_ENV}\""), None);

    let id = ExecId::new("x-2").unwrap();
    let specs = rj_supervisor::build_specs(&desc, &d, &id, CAPTURED).unwrap();
    let (exec, mut output) = rj_supervisor::Exec::start(id, d, specs);
    let drain = tokio::spawn(async move { while output.recv().await.is_some() {} });
    tokio::time::timeout(Duration::from_secs(30), exec.wait_finished())
        .await
        .expect("exec finishes");
...
async fn a_concurrent_destroy_waits_for_the_one_already_running() {
    ...
    // fails, publishing the terminal health first, so `rocjitsu run` reaches
...
// ---------------------------------------------------------------------
//
// `rocjitsu exec` runs its workload in its own process, in its own
...
async fn teardown_does_not_begin_while_a_borrower_holds_a_lease() {
    ...
    // lets go. Before leases existed, a `rocjitsu run -- sleep 5` finishing
...
/// A process standing in for one a `rocjitsu exec` started and then failed
/// to reap, killed when the test ends however it ends.
///
/// Tagged exactly as a real one is — [`ENV_SESSION`] and [`ENV_RUNTIME`]
/// say which session it is in, and `ROCJITSU_EXEC` which invocation started
...
impl Borrowed {
    /// A stand-in whose exec id is a borrower's: the shape `rocjitsu exec`
    /// builds for itself, which is by construction not one the run has
    /// ever issued.
    fn spawn(session: &rj_core::session::SessionId) -> Self {
        Self::tagged(session, &format!("x-{}-stand-in", std::process::id()))
    }

    fn tagged(session: &rj_core::session::SessionId, exec: &str) -> Self {
        use rj_core::container::{ENV_RUNTIME, ENV_SESSION, owning_runtime};
        Self(
            std::process::Command::new("/bin/sh")
                .env(rj_supervisor::spec::ENV_EXEC, exec)
    ...
    ///
    /// `try_wait`, not [`rj_supervisor::process::wait_gone`]. In the
...
async fn a_borrower_that_died_does_not_leave_its_workload_in_the_session() {
    // `kill -9` on a `rocjitsu exec`. Its workload is its own child, in its
    ...
    //
    // `rocjitsu cleanup` will not touch it, and is right not to: the
...
/// Attach to `run`'s control socket the way `rocjitsu exec` does, and hand
...
async fn attach_over_the_socket(
    ...
    match serde_json::from_slice::<Response>(&frame).unwrap() {
        ...
        other => panic!("the run answered an attach with {other:?}"),
    }
}
...
async fn serving(run: &Arc<Run>) -> (std::path::PathBuf, tokio::task::JoinHandle<()>) {
    let path = rj_core::paths::run_socket_path(run.id());
    let socket = rj_supervisor::rpc::ControlSocket::bind(&path)
        .await
...
async fn a_borrower_that_disappears_has_its_workload_reaped_before_teardown() {
    // `kill -9` on a `rocjitsu exec` *while its run carries on*. Teardown
    ...
    // and untouchable by `rocjitsu cleanup`, which will not reclaim a live
    // session's processes and is right not to. `rocjitsu exec --help`
    ...
    for pid in &ours {
        assert!(
            ...
            "the run's own workload (pid {pid}) was killed by another \
             terminal's `rocjitsu exec` exiting"
        );
    }
    assert!(
        !exec.is_ended(),
...
async fn a_departing_borrower_does_not_take_a_second_borrowers_workload_with_it() {
    ...
    let mut staying = std::process::Command::new("/bin/sh")
        ...
        .env(rj_supervisor::spec::ENV_EXEC, "x-999999-staying")
        .env(rj_core::container::ENV_SESSION, run.id().as_str())
        .env(
            rj_core::container::ENV_RUNTIME,
            rj_core::container::owning_runtime(),
        )
...
async fn a_reparented_workload_of_a_live_borrower_is_spared() {
    ...
    let mut staying = std::process::Command::new("/bin/sh")
        ...
        .env(rj_supervisor::spec::ENV_EXEC, exec.as_str())
        .env(rj_core::container::ENV_SESSION, run.id().as_str())
        .env(
            rj_core::container::ENV_RUNTIME,
            rj_core::container::owning_runtime(),
...
async fn tearing_one_session_down_leaves_another_sessions_processes_alone() {
    ...
    // session rather than reusing `rocjitsu cleanup`'s "everything under
    // this runtime directory" scan. Two runs under one `ROCJITSU_CLI_RUNTIME_DIR`
...
/// the behaviour.
use rj_supervisor::process::INHERITED_ENV as KEPT_WHEN_CLEARED;
...
async fn the_callers_environment_reaches_the_workload() {
    // The default, and the point of it: rocjitsu's parent is the terminal
...
async fn every_description_of_a_session_names_the_same_rendezvous() {
    ...
    // rendezvous port each time, `rocjitsu run`'s ranks and a `rocjitsu exec`
    ...
    // actually sees.
    let a = rj_supervisor::build_specs(
        &first,
        &def(&run, "true", None),
    ...
    .unwrap();
    let b = rj_supervisor::build_specs(
        &second,
        &def(&run, "true", None),
...
async fn naming_a_node_runs_there_and_only_there() {
    ...
    let (exec, mut output) = run
        .exec(&def(&run, "echo \"on $ROCJITSU_RANK\"", Some(2)))
        .await
        .expect("exec starts");
...
async fn a_multi_node_exec_labels_every_ranks_output_with_its_rank() {
    ...
    let run = start(3).await;
    let (exec, mut output) = run
        .exec(&def(&run, "echo \"line from $ROCJITSU_RANK\"", None))
        .await
        .expect("exec starts");
...
async fn a_single_process_exec_writes_straight_to_the_terminal() {
    // A one-process job writes to the terminal directly, with rocjitsu out
    // of the way entirely. If anything arrived on this channel, output
    // would be passing through rocjitsu when it should not — which is how
    ...
    assert!(
        ...
        "an inheriting exec must pipe nothing through rocjitsu"
    );
    run.destroy().await;
}
...
async fn tearing_down_ends_a_container_bring_up_rather_than_waiting_it_out() {
    // The `Cancel` switch has thorough tests in `rj_container` and
    ...
    // rest of that pull with rocjitsu looking like it had ignored the
    // signal.
    //
    ...
    let mut p = profile(1);
    p.containerize = Some(rj_core::profile::ContainerizedDef {
