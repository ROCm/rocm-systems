...
**rocjitsu** is the user-facing UX — a single command-line tool — for the
...
$ rocjitsu run --profile cdna4 -- ./my-rocm-app --flag
```

That second command is the whole system. There is nothing to start
beforehand and nothing to clean up afterwards: `rocjitsu run` brings the
...
* **One command, no daemon.** `rocjitsu run` *is* the runtime. There is no
  ...
  command you typed, which is why what rocjitsu tells you about it is true
...
* **Nothing is left behind.** Every workload process is owned by a
  ...
  cannot be otherwise: a `SIGKILL` runs no code of rocjitsu's at all, so
  its leftovers are *recovered* rather than prevented — see
  `rocjitsu cleanup` below.
* **Your terminal, not a pipe.** A workload inherits your real stdin,
  stdout and stderr, so `rocjitsu run -- bash` is a real shell: prompt,
...
* **A second terminal when you want one.** While a run is up it serves a
  socket naming its session, so `rocjitsu exec -- <command>` from another
...
* **Easy to script.** Every list/show command accepts `--json` for
  machine-readable output, and `rocjitsu run` exits with the workload's own
...
* **A drop-in for `rocjitsu`.** `rocjitsu --config cfg.json -- ./app` works
  just like the upstream `rocjitsu` CLI, so existing scripts keep
  running — including `--attach`, which rocjitsu takes as a spelling of
  `--daemon`. A flag before the `--` that rocjitsu does not accept is a
...
| **Session**  | The emulated machine a workload runs on. Owned by the `rocjitsu run` that created it, and alive exactly as long as that command is. |
| **Exec**     | One command invocation in a session, running in the terminal that launched it. |

A typical flow is: pick or create a **profile** and `rocjitsu run` a command
...
same session while the first is still running, `rocjitsu exec` it from
another terminal.
...
`rocjitsu run` prints the session id it created on stderr
(`rocjitsu: session <id>`). From a **second terminal**, while that run is
still up:
...
rocjitsu exec --session <id> -- python -c 'import torch; print(torch.cuda.device_count())'
```

`--session` may be omitted whenever exactly one `rocjitsu run` is live —
one terminal running the job, another exec'ing into it, which is the
case that matters. When it would be ambiguous rocjitsu lists the
...
[`docs/building.md`](docs/building.md) — and `rocjitsu emulators` reports
...
```sh
rocjitsu run --profile cdna4 --num-nodes 2 --nproc-per-node 4 -- python train.py
```
...
**One process** — `rocjitsu run -- bash` on a one-node profile — gets your
...
byte-exact and stdout stays separate from stderr. RocJITsu is not in the
middle at all.

**More than one** and every rank's output is piped through rocjitsu and
...
somewhere you cannot see. If you piped something in, rocjitsu says once
...
```sh
rocjitsu paths        # config, runtime, profiles, topologies, agents,
                    # sessions, runs
```

The `runs` directory is the interesting one: a live run publishes a
socket there named after its session
(`$XDG_RUNTIME_DIR/rocjitsu/run/<session>.sock`), and that is the entire
...
`rocjitsu cleanup` reclaims what a run that died abruptly could not: its
...
run, an OOM kill, or a machine losing power runs no code of rocjitsu's at
all, so the leak is expected and this is the recovery. Sessions whose run
still answers are left completely alone, as is anything rocjitsu did not
...
`rocjitsu.owner`, `rocjitsu.session` and `rocjitsu.runtime` labels on containers
and networks, `ROCJITSU_SESSION` and `ROCJITSU_CLI_RUNTIME_DIR` in every workload's
...
The runtime mark is what keeps this safe when two rocjitsu processes share a
...
session belonging to a different `ROCJITSU_CLI_RUNTIME_DIR` and would otherwise
...
runtime directory that was — including work left by a rocjitsu older than
the mark, which has to be cleaned up by hand.

`rocjitsu state purge` is the blunter version: it does all of the above and
then removes the runtime directory (`--all` takes the config directory
with it). There is no separate state directory: rocjitsu writes nothing
...
rocjitsu is a single Cargo workspace with no Node.js, no SPA and no
optional server components. One build produces the `rocjitsu` binary:

```sh
cd emulation/rocjitsu/cli
cargo build --workspace          # debug build -> target/debug/rocjitsu
...
serves while it is alive, `rocjitsu exec` borrowing that session from
...
`ROCJITSU_E2E_ALLOW_SKIP` / `-DROCJITSU_ALLOW_TEST_SKIP=ON`, which turn that
...
* [`docs/building.md`](docs/building.md) — building rocjitsu and rocjitsu.
...
* [`docs/demo-debugging-at-scale.md`](docs/demo-debugging-at-scale.md) —
  presentation deck on debugging ROCm at scale with rocjitsu and rocjitsu.
