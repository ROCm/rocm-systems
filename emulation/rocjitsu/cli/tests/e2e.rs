//! End-to-end tests for the `rocjitsu` CLI.
//!
...
//! list any more: `rocjitsu run` *is* the runtime, and a session exists
...
//! `rocjitsu exec` borrowing that session from another terminal.
...
/// `rocjitsu exec` still holds the session.
///
...
fn version_identifies_the_source_revision() {
    ...
    assert_eq!(
        lines.next(),
        Some(concat!("rocjitsu cli ", env!("CARGO_PKG_VERSION")))
    );
    let identity = lines
        .next()
...
fn paths_reports_the_overridden_directories() {
    ...
    // which is precisely what rocjitsu no longer has.
    assert!(
        !out.contains("socket:"),
...
/// delete it, and a pristine builtin refuses to be deleted — rocjitsu would
...
fn a_topology_naming_an_agent_that_is_not_there_is_refused() {
    ...
    assert!(err.contains("rocjitsu agent list"), "{err}");
...
fn deleting_a_document_says_which_others_it_just_broke() {
    // The delete is allowed: these files are the user's and rocjitsu has no
    ...
    // document is exactly the shape rocjitsu writes.
...
fn run_streams_output_and_propagates_the_exit_code() {
    ...
    // command's, so rocjitsu never sees the bytes and cannot decorate them.
    assert!(
        !stdout.contains("[0]"),
...
fn captured_output_is_complete_for_a_process_that_writes_and_exits_immediately() {
    ...
    // rocjitsu's streams: the workload writes to this test's pipe itself
    // and there is no pump, no channel and no printer between them, so
    // the version of this test that ran `rocjitsu run -- echo` asserted
...
fn a_run_serves_a_socket_only_while_it_is_alive() {
    ...
    // run that is fully up rather than one still starting: rocjitsu
    ...
    // session has, so leaving one behind would mean `rocjitsu exec` offers
...
fn an_interrupted_run_exits_with_128_plus_the_signal() {
    ...
    // the one `rocjitsu run` has to speak because it reports its workload's
    ...
    for (signal, expected) in [
        ...
        assert_eq!(
            out.status.code(),
            Some(expected),
            "{signal:?} must leave `rocjitsu run` exiting 128 + {}: {}",
            signal as i32,
...
fn a_run_waits_for_a_borrower_before_tearing_its_session_down() {
    ...
    // observed from outside. `rocjitsu exec` starts its workload in its own
    ...
    // `rocjitsu run -- sleep 1` beside a longer exec did exactly that.
    let env = Env::new();
    if skip_without_emulator() {
        return;
    }
...
fn exec_builds_the_same_process_grid_as_the_run() {
    ...
    let out = env.ok(&[
        ...
        "/bin/sh",
        "-c",
        "echo rank-$ROCJITSU_RANK/$WORLD_SIZE",
    ]);
...
fn exec_without_a_live_run_says_so() {
    // A session only exists while its `rocjitsu run` does, which is
    ...
    assert!(
        err.contains("no `rocjitsu run` is running"),
...
fn a_run_on_a_missing_profile_fails_clearly() {
    ...
    // exists under a different `ROCJITSU_CLI_CONFIG_DIR` than the one in force.
    assert!(
        err.contains(&env.profile_dir().display().to_string()),
        "the error must say where rocjitsu looked: {err}"
    );
...
fn env_flags_reach_the_workload() {
    ...
    let out = env.ok(&[
        ...
        "p",
        "--env",
        "ROCJITSU_E2E_VALUE=surprise",
        "--",
        "/bin/sh",
        "-c",
        "echo $ROCJITSU_E2E_VALUE",
    ]);
...
fn the_rank_environment_is_injected() {
    ...
    let out = env.ok(&[
        ...
        "/bin/sh",
        "-c",
        "echo $ROCJITSU_RANK/$RANK/$WORLD_SIZE/$LOCAL_RANK",
    ]);
...
fn the_rocjitsu_dropin_shape_routes_to_run() {
    // `rocjitsu [flags] -- ./app` with no subcommand is the upstream
    // `rocjitsu` invocation, and has to keep working so rocjitsu can be
...
fn cleanup_leaves_a_run_in_another_runtime_directory_alone() {
    // Two rocjitsu processes on one machine, each with its own `$XDG_RUNTIME_DIR`:
    ...
    // A second, entirely separate rocjitsu installation.
    let b = Env::new();
    assert!(
    ...
    // The claim `rocjitsu cleanup --help` makes: a session whose run still
...
fn stdin_reaches_the_workload() {
    ...
    // and no pseudo-terminal in between, which is what makes `rocjitsu run
    ...
    let out = rx
        ...
        .expect("`cat` must see EOF when rocjitsu's stdin closes, not block forever")
        .unwrap();
    let stdout = String::from_utf8_lossy(&out.stdout);
...
//
// Nothing below needs a session — these are the answers rocjitsu gives
...
///
/// The drop-in shape (`rocjitsu [flags] -- ./app`, no subcommand) exists so
/// rocjitsu can replace `rocjitsu` on an existing command line. Deciding
/// what counts as "no subcommand" from a hardcoded list of names went
/// wrong the moment one was added or aliased, and `rocjitsu cleanup -- echo
...
fn a_subcommand_before_a_double_dash_is_not_treated_as_a_drop_in_run() {
    ...
    assert!(
        combined.contains("Usage: rocjitsu cleanup"),
...
/// `rocjitsu about` says what this build is and what it is built from.
///
...
fn about_describes_the_build_in_text_and_in_json() {
    ...
    assert!(text.contains("rocjitsu"), "{text}");
    assert!(
    ...
    assert!(
        ...
        "rocjitsu has dependencies, so an empty licence manifest is a bug, \
...
/// rocjitsu never destroys a configuration document it did not write.
///
/// All three refusals are one rule seen from three sides, so they are
/// asserted together: a name that already exists, a builtin rocjitsu will
...
/// edits are gone.
#[test]
fn rocjitsu_refuses_to_quietly_destroy_a_document_it_did_not_write() {
    let env = Env::new();
    if skip_without_emulator() {
        return;
    }
    ...
    assert!(
        ...
        "the refusal must name the spelling rocjitsu would have used: {err}"
    );
    let list = env.ok(&["profile", "list"]);
    assert!(
...
/// document is gone, the user was asked and said no, and rocjitsu could not
...
fn a_declined_delete_changes_nothing_and_reports_that_it_did_not() {
    ...
    // that is now the only place rocjitsu will ask. A pipe carrying the
    ...
    // failure rather than a "no": a `rocjitsu profile delete x` in a cron
...
/// `rocjitsu state builtins` writes what it says it wrote.
///
...
fn state_builtins_writes_every_shipped_document_and_names_where() {
    ...
    assert!(!entries.is_empty(), "rocjitsu ships builtins: {json}");
...
/// `rocjitsu emulators` is how a user finds out what this build can do.
///
...
/// An argument rocjitsu cannot honour is diagnosed before anything starts.
///
...
/// rocjitsu says what is wrong with the input, names it, and creates no
...
fn an_unusable_argument_is_diagnosed_and_no_session_is_created() {
    ...
    let cases: &[(&[&str], &[&str])] = &[
        ...
        // More processes than rocjitsu will start for one exec. Refused up
    ...
    for (args, expected) in cases {
        ...
        for fragment in *expected {
            assert!(
                ...
                "`rocjitsu {}` must explain itself and mention {fragment:?}; said: {err}",
                args.join(" ")
            );
        }
        assert!(
            env.live_runs().is_empty(),
            "`rocjitsu {}` was rejected but left a session behind: {:?}",
            args.join(" "),
            env.live_runs()
        );
    }
...
/// `RJ_CONFIG` is the default for `run --config`.
///
/// The variable and the flag name the same thing — the emulator config
/// file to use verbatim — and the C++ runtime already reads the variable,
/// so a shell that has it exported should not have to say it twice.
///
/// Asserted through the child process rather than in a unit test because
/// the workspace forbids `unsafe`, and `std::env::set_var` is `unsafe`
/// under edition 2024. A path that does not exist is the cheapest proof
/// that the value was read at all: the failure names it.
#[test]
fn rj_config_supplies_the_emulator_config_when_the_flag_is_omitted() {
    let env = Env::new();
    env.create_profile("p");

    let out = env
        .rocjitsu()
        .env("RJ_CONFIG", "/nonexistent/from-env.json")
        .args(["run", "--profile", "p", "--", "/bin/true"])
        .output()
        .expect("failed to run rocjitsu");
    let err = String::from_utf8_lossy(&out.stderr);
    assert!(
        !out.status.success() && err.contains("/nonexistent/from-env.json"),
        "the run must have tried the file RJ_CONFIG names; said: {err}"
    );

    // And yields to the flags `--config` refuses to sit beside, so an
    // exported variable cannot turn an unrelated command line into a
    // usage error. `--plugin` names a real plugin, so this gets as far as
    // bring-up and fails there or succeeds — either way it is not the
    // config file being complained about.
    let out = env
        .rocjitsu()
        .env("RJ_CONFIG", "/nonexistent/from-env.json")
        .args([
    ...
    let err = String::from_utf8_lossy(&out.stderr);
    assert!(
        !err.contains("/nonexistent/from-env.json"),
        "`--plugin` synthesises a config, so RJ_CONFIG must be ignored; said: {err}"
    );
...
/// rocjitsu's own code would silently change it.
#[test]
fn arguments_the_parser_rejects_exit_two() {
    ...
    for args in cases {
        ...
        assert_eq!(
            out.status.code(),
            Some(2),
            "`rocjitsu {}` must be a parse error (exit 2), not a runtime one; \
             got {:?}\nstderr: {}",
            args.join(" "),
            out.status.code(),
            String::from_utf8_lossy(&out.stderr),
...
/// A subcommand rocjitsu does not have is a parse error that names the one
/// it thinks you meant.
///
...
fn an_unknown_subcommand_is_a_parse_error_that_points_at_the_real_one() {
    ...
    assert_eq!(
        out.status.code(),
        Some(1),
        "a request rocjitsu understood and could not satisfy exits 1, not 2: {:?}",
        out.status.code()
    );
    assert!(
...
/// alone.
///
/// `--json` exists so a script can pipe rocjitsu into `jq`, and that only
...
fn every_json_command_prints_one_document_on_stdout_and_nothing_on_stderr() {
    ...
    for command in commands {
        ...
        assert!(
            out.status.success(),
            "`rocjitsu --json {}` failed: {:?}\n{stderr}",
            command.join(" "),
            out.status.code()
        );
        serde_json::from_str::<serde_json::Value>(&stdout).unwrap_or_else(|e| {
            panic!(
                "`rocjitsu --json {}` did not print one JSON document ({e}): {stdout:?}",
                command.join(" ")
            )
        });
        assert!(
            stderr.is_empty(),
            "`rocjitsu --json {}` wrote to stderr, which a script reading both \
...
fn a_command_that_fails_under_json_writes_nothing_to_stdout() {
    ...
    for command in commands {
        ...
        assert!(
            !out.status.success(),
            "`rocjitsu --json {}` was supposed to fail",
            command.join(" ")
        );
        assert!(
            out.stdout.is_empty(),
            "`rocjitsu --json {}` failed but left {:?} on stdout",
            command.join(" "),
            String::from_utf8_lossy(&out.stdout)
        );
        assert!(
            !out.stderr.is_empty(),
            "`rocjitsu --json {}` failed silently, so nothing says why",
            command.join(" ")
        );
    }
}

/// `rocjitsu help <cmd>` and `rocjitsu <cmd> --help` are the same page.
///
...
fn help_for_a_subcommand_is_the_same_page_as_its_own_help_flag() {
    ...
    for command in [
        ...
        assert_eq!(
            ...
            "`rocjitsu help {command}` and `rocjitsu {command} --help` differ"
        );
...
///
/// The two ways to get the drop-in shape wrong are a flag rocjitsu does not
/// have and a command with no `--` in front of it, and both used to end
/// somewhere expensive. `rocjitsu --nodes 2 -- ./app` (the plural is the
...
/// for rocjitsu's own argument; `rocjitsu ./app` dead-ended on "unrecognized
...
fn a_mistyped_drop_in_is_a_parse_error_rather_than_a_session() {
    ...
    assert_eq!(
        out.status.code(),
        Some(2),
        "a flag rocjitsu does not have is a parse error: {:?}",
        out.status.code()
    );
    let err = String::from_utf8_lossy(&out.stderr);
    assert!(
    ...
    assert!(
        ...
        "rocjitsu's own flag was handed to the workload: {err}"
    );
    assert!(
...
///
/// `rocjitsu -vvv -- ./app` has to route to `run` like `rocjitsu -- ./app`
...
/// With several runs live, `rocjitsu exec` asks which one.
///
...
/// `rocjitsu exec --node N` reaches one node of a session, not all of them.
///
...
fn an_application_defined_signal_is_forwarded_and_does_not_end_the_run() {
    ...
    // matches a line `rocjitsu run` prints before it has even bound its
...
/// on hardest: `rocjitsu run -- make` has to be substitutable for `make`.
...
fn the_run_exits_with_the_workloads_own_code_however_the_workload_ended() {
    ...
    let cases: &[(&str, i32)] = &[
        ...
        // back as -1 or 0.
        ("exit 255", 255),
        // Signals, via the shell convention rocjitsu documents.
        ("kill -TERM $$", 143),
        ("kill -SEGV $$", 139),
        ("kill -KILL $$", 137),
    ];
...
/// lossy `String` in the middle — so `rocjitsu run -- ./app > out.bin` has
...
///
/// `rocjitsu [opts] -- ./app ...` exists so rocjitsu can be dropped in front
...
/// including spellings rocjitsu has flags of its own for. A parser that
...
fn the_drop_in_shape_hands_every_argument_to_the_workload_untouched() {
    ...
    // And a run flag before the separator is still rocjitsu's, so the shape
...
/// "the directory `rocjitsu run` was started from" is a decision rather
/// than an inevitability, and a run that quietly landed in `/` or in its
/// own scratch directory would break `rocjitsu run -- ./configure` for
/// reasons nobody would look for in rocjitsu.
#[test]
fn a_workload_starts_in_the_directory_it_was_given() {
    ...
    let run_in = |cwd: &std::path::Path, args: &[&str]| -> String {
        ...
        let out = env
            ...
            .current_dir(cwd)
            .output()
            .unwrap_or_else(|e| panic!("running `rocjitsu {}`: {e}", argv.join(" ")));
        assert!(
            out.status.success(),
            "`rocjitsu {}` failed: {}",
            argv.join(" "),
...
/// rank identity and the `ROCJITSU_SESSION`/`ROCJITSU_CLI_RUNTIME_DIR` marker are not
/// ambient, they are the session, and dropping the marker would make the
/// run's workloads invisible to `rocjitsu cleanup` after a `SIGKILL`.
#[test]
fn clear_env_vars_drops_the_ambient_environment_but_keeps_what_the_session_needs() {
    ...
    let ambient = "ROCJITSU_E2E_AMBIENT";
    let dump = |clear: bool| -> String {
        let mut cmd = env.rocjitsu();
...
/// names rocjitsu owns are not for sale.
///
...
#[test]
fn an_env_flag_is_one_pair_and_the_names_rocjitsu_owns_are_refused() {
    let env = Env::new();
    if skip_without_emulator() {
        return;
    }
    ...
    // A name rocjitsu sets itself is ignored rather than obeyed — and said
    ...
    assert_eq!(
        ...
        "[0]",
        "`--env RANK=…` overwrote the rank rocjitsu assigned"
    );
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
...
fn the_rank_grid_is_the_product_of_nodes_and_processes_per_node() {
    ...
    let out = env.ok(&[
        ...
        "/bin/sh",
        "-c",
        "echo grid $RANK $LOCAL_RANK $WORLD_SIZE $ROCJITSU_RANK",
    ]);
    ...
    // is silent: an underscore rocjitsu does not know becomes an
...
fn every_process_on_a_node_shares_one_environment_apart_from_its_own_rank() {
    ...
    // `--clear-env-vars`, so what is compared is the environment rocjitsu
    ...
    for key in ["WORLD_SIZE", "MASTER_PORT", "ROCJITSU_SESSION"] {
...
///
/// The claim `rocjitsu exec --help` makes is that the process "still
...
fn an_exec_joins_the_running_job_rather_than_standing_up_a_new_one() {
    ...
    // the session is up.
    let report = "echo id=$ROCJITSU_SESSION rank=$RANK world=$WORLD_SIZE port=$MASTER_PORT";
...
///
/// `rocjitsu exec` is a second front door to the same machinery, and the
...
fn an_exec_that_cannot_start_its_command_fails_the_way_a_run_does() {
    ...
    // A working directory that is not there: a rocjitsu-level failure, so
...
fn the_harness_removes_its_runtime_root_even_when_a_test_panics() {
    ...
    std::fs::create_dir_all(runtime.join("rocjitsu/run")).unwrap();
    std::fs::write(runtime.join("rocjitsu/run/pretend.sock"), b"x").unwrap();
