//! Performance + accuracy benchmark across the rocjitsu emulator
//! backends (`rocjitsu`, `hotswap`).
//!
//! This is an *integration benchmark*: it drives the real `rocjitsu`
...
//!   `rocjitsu run`, sampled over N iterations (after one warm-up), with
...
//!
//! Set `ROCJITSU_BENCH=1` to run it. Left to itself it does nothing at
...
//! skip past this point defers to `ROCJITSU_E2E_ALLOW_SKIP` exactly as the
//! session suites do.
//!
...
//! `ROCJITSU_BENCH_REPORT`). Iteration count is configurable via
//! `ROCJITSU_BENCH_ITERS` (default 5).
...
///
/// Named like the other `ROCJITSU_BENCH_*` knobs in this file, and read
/// the same way [`harness::ENV_ALLOW_SKIP`] is: presence is enough, the
/// value is not inspected.
const ENV_RUN_BENCH: &str = "ROCJITSU_BENCH";
...
/// One row of `rocjitsu emulators --json`.
...
// There is nothing to tear down beyond that — each `rocjitsu run` here is a
...
/// The emulator registry as rocjitsu sees it on this host.
fn registry(env: &Env) -> Vec<EmulatorRow> {
    let out = env
        .rocjitsu()
        .args(["emulators", "--json"])
        .output()
        .expect("failed to run `rocjitsu emulators --json`");
...
fn iters() -> usize {
    std::env::var("ROCJITSU_BENCH_ITERS")
        .ok()
...
fn report_path() -> PathBuf {
    std::env::var("ROCJITSU_BENCH_REPORT")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
...
/// Outcome of a single timed `rocjitsu run`.
struct Sample {
    wall_ms: f64,
    exit_ok: bool,
    correct: bool,
...
/// Run `rocjitsu run --profile <emu> -- <args...>` once, timing the full
/// invocation and checking the accuracy sentinel.
///
/// The workload inherits this command's stdout — rocjitsu does not sit in
...
fn timed_run(env: &Env, emulator: &str, args: &[&Path], sentinel: Option<&str>) -> Sample {
    ...
    // `rocjitsu run` is the session: the wall clock below covers bring-up,
    // the workload, and teardown, because they all happen inside this one
    // process. Nothing is left running when it returns.
    let out = cmd.output().expect("failed to spawn `rocjitsu run`");
...
fn render_report(results: &[EmulatorResult], n: usize) -> String {
    ...
    writeln!(s, "# RocJITsu Emulator Benchmark Report").unwrap();
    writeln!(s).unwrap();
    writeln!(s, "_Generated {now}_").unwrap();
    writeln!(s).unwrap();
    writeln!(
        s,
        "Comparison of the rocjitsu emulator backends running an identical real \
         HIP GPU workload (a vector-add kernel that self-validates its result \
         on-device) end-to-end through the `rocjitsu` CLI."
    )
    .unwrap();
    writeln!(s).unwrap();
    ...
    writeln!(
        s,
        "- **Driver:** each run is a full `rocjitsu run --profile <emulator> -- \
    ...
    writeln!(
        s,
        "- **Overhead baseline:** a `rocjitsu run ... -- /bin/true` is timed \
    ...
    writeln!(
        s,
        "## Performance (end-to-end `rocjitsu run`, milliseconds)"
    )
    .unwrap();
    writeln!(s).unwrap();
    writeln!(
    ...
    writeln!(
        s,
        "> Note: latencies are dominated by fixed `rocjitsu` session \
