...
//!
//! It is not any more. `rocjitsu run` owns its session, so a container is
//! launched with `run --rm` and *no* `-d`: the provider client is a child
//! process rocjitsu holds for as long as the container should live, and
...
const STALL_WORKDIR: &str = "/rocjitsu-test-stalls-then-missing";
...
impl Env {
    ...
    /// "this list is empty" is the mock's answer to "did rocjitsu leak a
    /// container?".
    ...
    /// The runtime directory a rocjitsu run in this environment resolves,
    /// and therefore stamps on everything it creates.
    fn own_runtime(&self) -> String {
        self.base.runtime().join("rocjitsu").display().to_string()
    }

    /// Give a resource the `rocjitsu.runtime` label the mock reads back on
...
/// * `network inspect` and `image inspect` fail, so rocjitsu takes the
///   create/pull paths;
/// * `run …` records the container and then *stays in the foreground*,
///   like a non-detached client, until the rocjitsu that spawned it dies;
...
# real engine recovers it from the `rocjitsu.session` label. Containers are
# `rocjitsu-<session>-node-<rank>`, networks are `rocjitsu-<session>`.
session_of() {
  s=${1#rocjitsu-}
  printf '%s' "${s%-node-*}"
}
...
# from the name, and it is the difference between a resource this rocjitsu
# may reclaim and one belonging to a rocjitsu elsewhere on the machine — so
...
# Record `--label rocjitsu.runtime=<dir>` from an argv, if it carries one.
record_runtime() {
  name=$1; shift
  prev=""
  for a in "$@"; do
    if [ "$prev" = "--label" ]; then
      case "$a" in
        rocjitsu.runtime=*) printf '%s' "${a#rocjitsu.runtime=}" > "$STATE/labels/$name" ;;
      esac
    fi
    prev=$a
  done
...
case "$1" in
  pull)
    ...
    # and the digest of what was pulled on stdout. RocJITsu shows a user
  ...
  build)
    ...
    # exited without reading would fail rocjitsu's write with EPIPE and
    # test a code path no engine produces.
    cat > /dev/null
    exit 0 ;;
  # Listing verbs, used by orphan reclamation after a run died without
  # tearing down. A real engine filters on the rocjitsu.owner label; the
  # mock creates nothing that is not rocjitsu's, so everything it holds is
  # a candidate.
  ps)
  ...
  network)
    case "$2" in
      ...
      inspect)
        if [ "$3" = "--format" ]; then
          case "$4" in
            *rocjitsu.session*) session_of "$5" ;;
            *rocjitsu.runtime*) runtime_of "$5" ;;
            *) printf rocjitsu ;;
          esac
          exit 0
        fi
  ...
  run)
    ...
    # lets `exec` below emulate the mount; without it the wrapper rocjitsu
    ...
    while [ $# -gt 0 ]; do
      case "$1" in
        ...
        --name) name="$2"; shift ;;
        --label)
          case "$2" in rocjitsu.runtime=*) runtime_label=${2#rocjitsu.runtime=} ;; esac
          shift ;;
        *:/mnt/rocjitsu/runtime|*:/mnt/rocjitsu/runtime:*)
          echo "${1%%:/mnt/rocjitsu/runtime*}" > "$STATE/runtime-mount" ;;
      esac
      shift
    done
    ...
    # as long as the rocjitsu that asked for it.
    #
    # What this proves, and what it does not. It proves rocjitsu asked for
    # `--rm` and no `-d`, that it holds the client for the session's
    # whole life, and that nothing in rocjitsu's own teardown depends on
    ...
    # init when rocjitsu is SIGKILLed and goes on holding its container
    # (which is the leak `rocjitsu cleanup` exists to reclaim, and why the
  ...
  exec)
    ...
    # Consume the flags rocjitsu builds, collecting `-e K=V` into the
    # environment, then run the command. This is the mock's real job:
    # it proves the argv rocjitsu produces is one an engine can execute.
    shift
    envs=""
    workdir=""
    while [ $# -gt 0 ]; do
    ...
    # neither can this. A rocjitsu that execs into a session whose
    ...
    # it here would let rocjitsu pass a *host* path as the container
    ...
    # wrapper rocjitsu generates actually lands where rocjitsu reads it.
    host_runtime=""
      ...
      for a in "$@"; do
        case "$a" in
          *"/mnt/rocjitsu/runtime"*)
            a=$(printf '%s' "$a" | sed "s#/mnt/rocjitsu/runtime#$host_runtime#g") ;;
        esac
  ...
  inspect)
    ...
    # template naming rocjitsu.owner), or the session an orphan belonged to.
    # `inspect -f` is the running-state probe bring-up polls before it
    # lets the first exec near the container.
    if [ "$2" = "--format" ]; then
      case "$3" in
        *rocjitsu.session*) session_of "$4" ;;
        *rocjitsu.runtime*) runtime_of "$4" ;;
        *rocjitsu.owner*) printf rocjitsu ;;
        *) echo true ;;
      esac
      exit 0
    fi
...
fn a_containerised_run_brings_up_executes_and_cleans_up() {
    ...
    assert!(
        log.contains("network create --label rocjitsu.owner=rocjitsu"),
        "missing network create:\n{log}"
    );
    // Ownership is a label, not a name. Teardown and `state purge` both
    // check it before removing anything, so a container rocjitsu did not
    // create is never destroyed by a name collision.
    assert!(
        log.contains("--label rocjitsu.session="),
        "resources must record the session they belong to:\n{log}"
    );
    assert!(
        log.contains("run --rm --name rocjitsu-"),
        "missing container run:\n{log}"
    );
    assert!(
        log.contains("--label rocjitsu.owner=rocjitsu"),
        "node containers must be labelled as rocjitsu's:\n{log}"
    );
    // The workload reaches the container through the provider's `exec`,
    // not through a second rocjitsu process inside it.
    assert!(
        log.contains("exec -i"),
    ...
    assert!(
        log.contains("rm -f rocjitsu-"),
        "container not removed:\n{log}"
    );
    assert!(
        log.contains("network rm rocjitsu-"),
        "network not removed:\n{log}"
    );
    assert!(
...
fn node_containers_are_launched_with_rm_and_are_not_detached() {
    ...
    // would survive the `rocjitsu run` that made it — the leak the daemon
    // era lived with, back when a session had to outlive its command.
    // `--rm` is what deletes the container once the client is gone,
    // including when rocjitsu was `SIGKILL`ed and never ran a line of
    // teardown code.
    let log = env.provider_log();
    let run_line = log
        .lines()
    ...
    assert!(
        run_line.starts_with(&format!("run --rm --name rocjitsu-{id}-node-0")),
    ...
    assert_eq!(env.live_containers(), vec![format!("rocjitsu-{id}-node-0")]);
...
fn a_container_does_not_outlive_the_run_that_created_it() {
    ...
    let id = run.await_ready(READY);
    let container = format!("rocjitsu-{id}-node-0");
    ...
    // been decided at launch time — the client is rocjitsu's child so it
...
fn the_node_container_entrypoint_just_idles() {
    ...
    // arrive from outside through `provider exec`. Running rocjitsu in
    ...
    assert!(
        ...
        "no rocjitsu host may run inside the container any more:\n{log}"
    );
...
#[test]
fn the_container_carries_the_in_container_rocjitsu_directories() {
    let env = Env::new();
    if skip_without_emulator() {
        return;
    }
    ...
    let log = env.provider_log();
    // The in-container rocjitsu directories point at their mounts, so an
    // emulator runtime inside the container resolves the same assets the
    // run wrote outside it.
    assert!(
        log.contains("-e ROCJITSU_CLI_RUNTIME_DIR=/mnt/rocjitsu/runtime"),
        "missing ROCJITSU_CLI_RUNTIME_DIR:\n{log}"
    );
    assert!(
        log.contains("-e ROCJITSU_CLI_CONFIG_DIR=/mnt/rocjitsu/config"),
        "missing ROCJITSU_CLI_CONFIG_DIR:\n{log}"
    );
    assert!(
        log.contains(":/mnt/rocjitsu/runtime"),
...
fn the_rank_environment_is_injected_at_exec_time() {
    ...
    let out = env.base.ok(&[
        ...
        "/bin/sh",
        "-c",
        "echo rank=$ROCJITSU_RANK world=$WORLD_SIZE local=$LOCAL_RANK",
    ]);
    ...
    assert!(
        exec_line.contains("-e ROCJITSU_RANK=0"),
...
fn container_state_is_never_written_to_disk_and_dies_with_the_session() {
    ...
    let log = env.provider_log();
    assert!(
        log.contains(&format!("rm -f rocjitsu-{id}-node-0")),
        "container not removed:\n{log}"
    );
    assert!(
        log.contains(&format!("network rm rocjitsu-{id}")),
        "network not removed:\n{log}"
    );
...
fn a_multi_node_containerised_session_launches_one_container_per_node() {
    ...
    for rank in 0..3 {
        assert!(
            log.lines()
                .any(|l| l.starts_with("run --rm --name rocjitsu-")
...
fn an_exec_runs_inside_the_containers_of_a_live_run() {
    ...
    let out = env.base.ok(&[
        ...
        "/bin/sh",
        "-c",
        "echo exec-in-container rank=$ROCJITSU_RANK world=$WORLD_SIZE",
    ]);
    ...
    // No `-t`, ever. RocJITsu allocates no pseudo-terminal: the provider
...
fn ending_a_containerised_run_kills_the_workload_inside_the_container() {
    ...
    // process running — invisible to rocjitsu and still holding the
...
fn cleanup_reclaims_containers_no_live_run_accounts_for() {
    ...
    // here.
    let orphan = "rocjitsu-deadsession-node-0";
    let network = "rocjitsu-deadsession";
    ...
    let out = env
        ...
        .env("ROCJITSU_CONTAINER_PROVIDER", &env.provider)
        .output()
        .unwrap();
    let text =
...
fn cleanup_spares_a_container_created_by_another_runtime_directory() {
    ...
    // Two rocjitsu processes sharing one container engine. The other one's session
    ...
    // The `rocjitsu.runtime` label is the only thing that says otherwise.
    let theirs = "rocjitsu-theirsession-node-0";
    ...
    assert_eq!(
        ...
        "cleanup removed a container belonging to another rocjitsu"
    );
}
...
/// bound, so `rocjitsu exec` from another terminal connected happily, sat
...
fn provider_chatter_stays_off_a_redirected_stdout() {
    ...
    // `rocjitsu run --profile cp -- ./app > out.txt`, typed at a prompt:
    // stdout is a file, stderr is still the terminal. Both halves matter.
    // Without a terminal on stderr rocjitsu captures the provider instead
    ...
    // and rocjitsu inherited that descriptor — so a redirected run's file
    ...
    // on the stream rocjitsu talks about itself on.
    assert!(
...
fn a_mount_whose_host_path_is_missing_is_refused_before_anything_is_created() {
    ...
    // must not mean two things, so rocjitsu decides it before either is
    // asked.
    ...
    assert!(
        !missing.exists(),
        "rocjitsu created the host path it refused to mount"
    );
    let log = env.provider_log();
    assert!(
        !log.contains("run --rm"),
...
#[test]
fn a_mount_laid_over_rocjitsus_own_directory_is_refused() {
    let env = Env::new();
    if skip_without_emulator() {
        return;
    }
    ...
    // RocJITsu bind-mounts its own binary, config and session scratch under
    // `/mnt/rocjitsu` in every node container. A user mount at or above
    // that path overlaps them, and the engine resolves the overlap by
    // creating rocjitsu's destinations inside the user's host directory —
    ...
    let err = env.base.fails(&[
        ...
        "cp",
        "--mount",
        &format!("{}:/mnt/rocjitsu", mine.display()),
        "--",
        "/bin/true",
    ]);
    assert!(
        err.contains(&mine.display().to_string()) && err.contains("/mnt/rocjitsu"),
    ...
    // And a mount *inside* one of rocjitsu's own, which is the same
    // collision in the direction that reads as harmless. `/mnt/rocjitsu` is
    ...
    // every path below the reserved directory, because rocjitsu's own
    // mounts are all down there — so this was refused by nothing until
    // the user's mounts were checked against rocjitsu's concrete
    ...
    let err = env.base.fails(&[
        ...
        "cp",
        "--mount",
        &format!("{}:/mnt/rocjitsu/runtime/rj_config.json", config.display()),
        "--",
        "/bin/true",
    ]);
    assert!(
        err.contains("/mnt/rocjitsu/runtime"),
        "the refusal must name the rocjitsu mount the user's lands inside: {err}"
    );
    let log = env.provider_log();
    assert!(
        !log.contains("run --rm"),
    ...
    let out = env.base.ok(&[
        ...
        "cp",
        "--mount",
        &format!("{}:/mnt/rocjitsu-of-my-own", mine.display()),
        "--",
        "/bin/echo",
        "mounted-elsewhere",
    ]);
...
fn a_provider_that_is_not_a_container_engine_is_refused_by_name() {
    ...
    assert!(
        ...
        "the refusal must name the provider and what rocjitsu asked it: {err}"
    );
}
...
fn the_provider_environment_variable_loses_to_the_profile_and_says_so() {
    ...
    // `--container-provider`) beats `ROCJITSU_CONTAINER_PROVIDER`, which
    // beats autodetection. The bug was not the order — rocjitsu cannot tell
    ...
    let out = env
        ...
        .env("ROCJITSU_CONTAINER_PROVIDER", "/nonexistent/other-engine")
        .output()
        .unwrap();
    ...
    assert!(
        stderr.contains("ROCJITSU_CONTAINER_PROVIDER=/nonexistent/other-engine")
...
#[test]
fn a_derived_hack_image_is_labelled_like_everything_else_rocjitsu_creates() {
    let env = Env::new();
    if skip_without_emulator() {
        return;
    }
    ...
    // A `rocjitsu-hack-…` image is host state that outlives the session
    ...
    assert!(
        build.contains("--label rocjitsu.owner=rocjitsu"),
        "a derived image must be marked as rocjitsu's: {build}"
    );
    ...
    assert!(
        build.contains("--label rocjitsu.runtime="),
...
/// images at all, which leaves the image half of `rocjitsu cleanup` — the
/// only host state rocjitsu deliberately lets outlive a session — resting
/// on two decisions that no test could reach: whether an image is in use,
/// and whether it is rocjitsu's. Both are answered by the engine, so both
/// need an engine that can be told what to say.
///
/// `ps` is the interesting one: cleanup asks it twice with different
/// arguments. `ps --all --filter … --format {{.ID}}` lists rocjitsu's
...
/// The images `rocjitsu cleanup --dry-run --json` says it would remove.
fn images_cleanup_would_remove(env: &Env, provider: &Path) -> Vec<String> {
    let out = env
        .base
        .rocjitsu()
        .args(["cleanup", "--dry-run", "--json"])
        .env("ROCJITSU_CONTAINER_PROVIDER", provider)
        .output()
        .unwrap();
    assert!(
        out.status.success(),
...
fn an_image_is_left_alone_when_the_engine_cannot_say_what_is_using_it() {
    ...
    let env = Env::new();
    let id = "abcdef012345";
    let tag = "rocjitsu-hack-demo:latest";

    let mute = env.base.root().join("mute-engine.sh");
    write_image_provider(&mute, id, tag, "rocjitsu", &env.own_runtime(), "exit 1");
    assert!(
    ...
    write_image_provider(&answers, id, tag, "rocjitsu", &env.own_runtime(), "exit 0");
    assert_eq!(
...
#[test]
fn an_image_is_reclaimed_only_when_it_says_rocjitsu_built_it() {
    // The engine is asked for images carrying `rocjitsu.owner=rocjitsu` and
    ...
    // text rather than failing, and `rocjitsu-hack-…` images left by a
    // rocjitsu older than the labels look exactly like ours by name. Only
    // a positive `rocjitsu.owner` makes an image rocjitsu's to delete.
    let env = Env::new();
    let id = "abcdef012345";
    let tag = "rocjitsu-hack-demo:latest";
    ...
    assert!(
        ...
        "an image the engine says is not rocjitsu's was reclaimed anyway"
    );
    ...
    write_image_provider(&ours, id, tag, "rocjitsu", &env.own_runtime(), "exit 0");
    assert_eq!(
        images_cleanup_would_remove(&env, &ours),
        vec![tag.to_string()],
        "an image rocjitsu built under this runtime directory must be \
         reclaimable, or the two assertions above proved nothing"
    );
}

/// A real container engine on this machine that already has the image
/// these tests need, preferring podman exactly as rocjitsu does.
///
...
/// reparented when the test kills its `rocjitsu run` and goes on holding
/// the container.
...
fn a_real_engine_creates_and_removes_the_container_with_the_run() {
    ...
    // rocjitsu asks for the container, a real `podman`/`docker` creates it,
    ...
    let container = RealContainer {
        engine,
        name: format!("rocjitsu-{id}-node-0"),
    };
    assert!(
        container.exists(),
