...
# rocjitsu + rocjitsu (the software GPU emulator).
#
# Unlike the corpus's stock kmd.so env, this file does NOT set LD_PRELOAD or
# ROCJITSU_RUNTIME_DIR by hand: rocjitsu injects the rocjitsu kmd.so interposer and
# generates the sim config itself when it runs each IREE tool via
# `rocjitsu run --profile <profile> -- <tool> ...`. This file's only job is to make
...
# Let rocjitsu own the emulator wiring; clear anything a previous shell may have
