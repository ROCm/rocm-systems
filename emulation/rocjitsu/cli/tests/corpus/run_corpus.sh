#!/usr/bin/env bash
#
# run_corpus.sh — run the rocjitsu-corpus gfx1250 regression through rocjitsu.
#
...
# through `rocjitsu run --profile <profile> -- <tool> ...`, so rocjitsu injects the
...
#     under a non-containerised rocjitsu profile.
#   * container mode (when ROCJITSU_CORPUS_IMAGE is set): the IREE tools come
#     from inside the image (see tests/corpus/Dockerfile) and the corpus runs
#     under a *containerised* rocjitsu profile built around that image. The
...
#   ROCJITSU_BIN               rocjitsu binary (default: rocjitsu on PATH)
#   CORPUS_ROOT              rocjitsu-corpus checkout (required)
#   ROCJITSU_CORPUS_EMULATOR   emulator to run under (default: hotswap)
#   ROCJITSU_CORPUS_PROFILE    rocjitsu profile name (default: corpus-<emulator>[-img])
#   ROCJITSU_CORPUS_ENV_FILE   corpus env file (default: <script>/env/<emulator>.sh)
#   ROCJITSU_CORPUS_ONLY       all | e2e | matmul (default: all)
#   ROCJITSU_CORPUS_OUT_DIR    results/logs output dir (default: corpus runner default)
#   ROCJITSU_CORPUS_IMAGE      container image with the IREE tools -> enables
#                            container mode (default: unset -> host mode)
#   ROCJITSU_CORPUS_PROVIDER   container provider for container mode (auto-detect)
#   ROCJITSU_CORPUS_AGENT      builtin GPU agent the profile pins
...
# ---- inputs -----------------------------------------------------------------
ROCJITSU_BIN=${ROCJITSU_BIN:-rocjitsu}
CORPUS_ROOT=${CORPUS_ROOT:-}
EMULATOR=${ROCJITSU_CORPUS_EMULATOR:-hotswap}
ENV_FILE=${ROCJITSU_CORPUS_ENV_FILE:-$script_dir/env/$EMULATOR.sh}
ONLY=${ROCJITSU_CORPUS_ONLY:-all}
OUT_DIR=${ROCJITSU_CORPUS_OUT_DIR:-}
IMAGE=${ROCJITSU_CORPUS_IMAGE:-}

if [[ -n "$IMAGE" ]]; then
  MODE=container
  PROFILE=${ROCJITSU_CORPUS_PROFILE:-corpus-$EMULATOR-img}
else
  MODE=host
  PROFILE=${ROCJITSU_CORPUS_PROFILE:-corpus-$EMULATOR}
fi

case "$ONLY" in
  all | e2e | matmul) ;;
  *) die "ROCJITSU_CORPUS_ONLY must be all, e2e, or matmul (got: $ONLY)" ;;
esac

# Builtin GPU agent the profile pins. HotSwap only supports MI450X.
if [[ -n "${ROCJITSU_CORPUS_AGENT:-}" ]]; then
  AGENT=$ROCJITSU_CORPUS_AGENT
...
command -v "$ROCJITSU_BIN" >/dev/null 2>&1 \
  || skip "rocjitsu binary not found (ROCJITSU_BIN=$ROCJITSU_BIN)"
...
# `rocjitsu run`, so the corpus runner stays unmodified and unaware it is driving
# an emulator. `$1` is the wrapper name, `$2` the command the wrapper invokes
# inside `rocjitsu run` (a host path in host mode, a bare tool name in container
...
#!/usr/bin/env bash
exec "$ROCJITSU_BIN" run --profile "$PROFILE" -- "$target" "\$@"
EOF
...
ensure_profile() {
  ...
  if "$ROCJITSU_BIN" profile show "$PROFILE" >/dev/null 2>&1; then
    return 0
  fi
  note "creating rocjitsu profile '$PROFILE' (mode=$MODE emulator=$EMULATOR)"
  printf '%s\n' "$1" | "$ROCJITSU_BIN" profile import - >/dev/null
}
...
JSON
  ensure_profile "$profile_json" \
    || skip "could not create rocjitsu profile '$PROFILE' (is emulator '$EMULATOR' installed?)"
  ...
  provider=${ROCJITSU_CORPUS_PROVIDER:-}
  if [[ -z "$provider" ]]; then
  ...
  # rocjitsu merges the emulator's own GPU device/mount injection on top.
  provider_field=""
  [[ -n "${ROCJITSU_CORPUS_PROVIDER:-}" ]] && provider_field="\"provider\": \"$provider\","
  read -r -d '' profile_json <<JSON
{
  "name": "$PROFILE",
  "emulator": {
...
JSON
  ensure_profile "$profile_json" \
    || skip "could not create containerised rocjitsu profile '$PROFILE' (is emulator '$EMULATOR' installed?)"
...
# ---- run the corpus regression through rocjitsu -------------------------------
args=(--only "$ONLY")
