# rocjitsu end-to-end testing matrix

This document defines the end-to-end (E2E) test matrix for `rocjitsu`. It
...
combination below and exercises it against the real `rocjitsu` binary
...
2. **Run** — execute the payload via `rocjitsu run`, which creates a
...
* **`rocjitsu` without its KMD library** — the software emulator is
  skipped when `rocjitsu` reports it as not installed (its KMD library
  could not be located by rocjitsu's own discovery).
...
The `rocjitsu` rows run whenever `rocjitsu` discovers its KMD library; see
