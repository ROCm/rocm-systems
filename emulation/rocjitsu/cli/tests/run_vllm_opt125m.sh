#!/usr/bin/env bash
#
# Smoke-test serving vLLM through rocjitsu on an *emulated* MI350X.
#
# It launches the upstream `vllm/vllm-openai-rocm:latest` image via
# `rocjitsu run --daemon`, serves `facebook/opt-125m`, and then exercises
# the OpenAI-compatible API with a single text completion ("a
# generation").
#
# Notes on how this maps onto rocjitsu's CLI:
...
#     mode (as requested).
#
# This uses the manylinux-built rocjitsu from
# `scripts/rocjitsu-docker-build.sh` rather than `cargo run`. rocjitsu
...
#   ROCJITSU_BIN  manylinux rocjitsu binary (default: <rocjitsu>/build/manylinux/bin/rocjitsu)
#   SKIP_BUILD  set to 1 to reuse an existing ROCJITSU_BIN without rebuilding
...
ROCJITSU_BIN="${ROCJITSU_BIN:-$ROCJITSU_DIR/build/manylinux/bin/rocjitsu}"
...
# Build the glibc-compatible rocjitsu binary in the manylinux container
# unless it already exists (or SKIP_BUILD=1 forces reuse).
if [[ "${SKIP_BUILD:-0}" != "1" && ! -x "$ROCJITSU_BIN" ]]; then
  log "building manylinux rocjitsu via scripts/rocjitsu-docker-build.sh"
  "$ROCJITSU_DIR/scripts/rocjitsu-docker-build.sh" "$ROCJITSU_DIR/build/manylinux"
fi
[[ -x "$ROCJITSU_BIN" ]] || fail "rocjitsu binary not found: $ROCJITSU_BIN (run scripts/rocjitsu-docker-build.sh)"
log "using rocjitsu binary: $ROCJITSU_BIN"
...
# ---------------------------------------------------------------------------
# Launch the server through rocjitsu in the background. rocjitsu execs the
...
# ---------------------------------------------------------------------------
RUN_LOG="$(mktemp -t rocjitsu-vllm.XXXXXX.log)"
log "starting vLLM (model=$MODEL) via rocjitsu run --daemon"
if [[ "$LOG_KERNELS" == "1" ]]; then
...
cleanup() {
  log "cleaning up"
  # Stop the backgrounded rocjitsu run, which tears down the session and
  # its container/network.
  if kill -0 "$ROCJITSU_PID" 2>/dev/null; then
    kill "$ROCJITSU_PID" 2>/dev/null || true
    wait "$ROCJITSU_PID" 2>/dev/null || true
  fi
...
# ---------------------------------------------------------------------------
# Wait for rocjitsu to launch the node container (named
# `rocjitsu-<session>-node-0`) and learn its IP on the session network.
# ---------------------------------------------------------------------------
log "waiting for the node container to appear"
DEADLINE=$(( $(date +%s) + READY_TIMEOUT ))
while :; do
  CONTAINER="$("$PROVIDER" ps --filter 'name=rocjitsu-' --filter 'status=running' \
    --format '{{.Names}}' 2>/dev/null | grep -E 'rocjitsu-.*-node-0$' | head -n1 || true)"
  [[ -n "$CONTAINER" ]] && break
  if ! kill -0 "$ROCJITSU_PID" 2>/dev/null; then
    cat "$RUN_LOG" >&2 || true
    fail "rocjitsu run exited before the container started"
  fi
...
while :; do
  ...
  if ! kill -0 "$ROCJITSU_PID" 2>/dev/null; then
    cat "$RUN_LOG" >&2 || true
    fail "rocjitsu run exited before the server became ready"
  fi
...
log "PASS: vLLM served $MODEL through rocjitsu and returned a generation"
