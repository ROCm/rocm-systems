...
//!
//! A `rocjitsu run` is a foreground process, so a test owns its lifetime
...
/// A private rocjitsu installation: its own XDG root and config store.
...
impl Env {
    ...
    /// deep tempdir path plus `rocjitsu/run/<session>.sock` can exceed it,
    ...
    pub(crate) fn new() -> Self {
        ...
        Self {
            ...
            bin: PathBuf::from(env!("CARGO_BIN_EXE_rocjitsu")),
            dir,
        }
    }

    /// The rocjitsu binary under test.
    ...
    pub(crate) fn session_scratch(&self, id: &str) -> PathBuf {
        self.runtime.join("rocjitsu/session").join(id)
    }
    ...
    pub(crate) fn rocjitsu(&self) -> Command {
        ...
        c.env("XDG_CONFIG_HOME", &self.config)
            ...
            .env_remove("RJ_CONFIG");
        c
    }

    /// The environment every rocjitsu invocation in this test needs.
    ///
    ...
    pub(crate) fn run(&self, args: &[&str]) -> Output {
        self.rocjitsu()
            .args(args)
            .output()
            .unwrap_or_else(|e| panic!("failed to run `rocjitsu {}`: {e}", args.join(" ")))
    }

    /// Run a rocjitsu command that must succeed, returning its stdout.
    pub(crate) fn ok(&self, args: &[&str]) -> String {
        let out = self.run(args);
        assert!(
            out.status.success(),
            "`rocjitsu {}` failed with {:?}\nstdout: {}\nstderr: {}",
            args.join(" "),
            out.status.code(),
    ...
    /// Run a rocjitsu command that must fail, returning its stderr.
    pub(crate) fn fails(&self, args: &[&str]) -> String {
        let out = self.run(args);
        assert!(
            !out.status.success(),
            "`rocjitsu {}` unexpectedly succeeded\nstdout: {}",
            args.join(" "),
            String::from_utf8_lossy(&out.stdout)
        );
    ...
    /// Spawn a background `rocjitsu run` and wait until it is serving.
    ///
    ...
    pub(crate) fn spawn_run(&self, args: &[&str], argv: &[&str]) -> Run {
        ...
        let child = cmd.spawn().expect("spawning `rocjitsu run`");
        Run {
            child: Some(child),
...
/// A background `rocjitsu run`, killed when this value is dropped.
...
///
/// `rocjitsu run` narrates its own state transitions on stderr — bring-up
...
impl Run {
    ...
    /// file appearing. `rocjitsu run` binds the socket *before* bring-up —
    /// so that a run in the middle of a long image pull is visible to
    /// `rocjitsu state purge` and is not mistaken for an orphan — but only
    ...
    pub(crate) fn await_ready(&mut self, timeout: Duration) -> String {
        ...
        while Instant::now() < deadline {
            ...
            {
                panic!("`rocjitsu run` exited before serving its socket: {status:?}");
            }
        ...
        panic!("`rocjitsu run` did not start serving within {timeout:?}");
    }
    ...
    pub(crate) fn wait(&mut self, timeout: Duration) -> Output {
        ...
        loop {
            match self.child.as_mut().map(std::process::Child::try_wait) {
                ...
                Some(Ok(None)) => {}
                Some(Err(e)) => panic!("waiting on `rocjitsu run`: {e}"),
            }
            assert!(
                Instant::now() < deadline,
                "`rocjitsu run` did not exit within {timeout:?}"
            );
            std::thread::sleep(Duration::from_millis(20));
        }
        ...
        let mut out = match rx.recv_timeout(timeout) {
            ...
            Err(_) => panic!(
                "`rocjitsu run` exited but its output pipes are still open after {timeout:?}: \
        ...
        if let Some((watch, done)) = self.stderr.take() {
            assert!(
                ...
                "`rocjitsu run` exited but its stderr pipe is still open after {timeout:?}: \
...
/// injection — and rocjitsu ships exactly one, the emulator it exists to
...
/// rocjitsu-only build does not, and these suites cannot run there.
///
...
pub(crate) const ENV_ALLOW_SKIP: &str = "ROCJITSU_E2E_ALLOW_SKIP";
...
/// `rocjitsu run` binds its socket before serving it and only begins to
...
/// `marker`.
///
/// rocjitsu's own CLI processes are excluded, and that exclusion is what
...
/// `rocjitsu run` / `rocjitsu exec` that was told to launch it — so `pgrep
...
///
/// Filtering on `comm` rather than on a pid list: `rocjitsu exec` clients
...
pub(crate) fn find_processes(marker: &str) -> Vec<u32> {
    ...
    String::from_utf8_lossy(&output.stdout)
        ...
        .filter(|pid| *pid != me && !is_rocjitsu_process(*pid))
        .collect()
}

/// Whether `pid` is a rocjitsu CLI process rather than a workload.
fn is_rocjitsu_process(pid: u32) -> bool {
    std::fs::read_to_string(format!("/proc/{pid}/comm")).is_ok_and(|comm| comm.trim() == "rocjitsu")
}
