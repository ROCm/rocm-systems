...
//! the `rocjitsu run` that created it is alive, so every claim in this file
//! is anchored to one pid: kill that process, or ask it to stop, and
//! nothing it started may still be there afterwards. `rocjitsu exec`
...
/// Assert a live `rocjitsu run` is not accumulating unreaped children.
///
...
fn scratch_root(env: &Env) -> std::path::PathBuf {
    env.runtime().join("rocjitsu/session")
}
...
fn rapid_run_churn_leaves_nothing_behind() {
    ...
    // session is live. `rocjitsu exec` picks its target from the sockets in
...
fn interrupting_a_run_tears_down_its_workload() {
    ...
    // The interrupt reaches `rocjitsu run` alone: every child leads its own
...
/// A `rocjitsu run` killed outright strands its workload — and
/// `rocjitsu cleanup` reclaims it.
///
/// `SIGKILL` is the one case the run cannot handle: no signal handler
/// runs, no teardown, no `Drop`, no cancelled task. RocJITsu used to close
...
/// `unsafe` was to stop paying for a guarantee rocjitsu was not making.
#[test]
fn a_killed_run_strands_its_workload_and_cleanup_reclaims_it() {
    ...
    assert!(
        ...
        "the workload did not outlive its `SIGKILL`ed run. That is a better \
         ...
         a guarantee rocjitsu no longer documents, and `rocjitsu cleanup` is no \
         longer the only thing standing between a `kill -9` and a leak."
    );

    // The recovered half. Nothing on disk records these processes; the
    // only evidence is `ROCJITSU_SESSION` in each one's environment, which
...
/// `rocjitsu cleanup` must be safe to run while other runs are healthy.
///
/// The command exists for the machine-wide mess a crash leaves, so it
/// scans every process and every rocjitsu-labelled container. That is only
...
///
/// Every `rocjitsu exec` opens a connection to the run's socket and asks
...
fn output_is_not_lost_under_churn() {
    ...
    // piped through rocjitsu, buffered until they form whole lines, and
...
fn a_socket_left_by_a_killed_run_does_not_wedge_the_next_run() {
    ...
    assert!(
        err.contains(&dead_id) || err.contains("rocjitsu run"),
