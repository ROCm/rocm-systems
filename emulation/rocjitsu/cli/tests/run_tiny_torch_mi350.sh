...
# rocjitsu, using a Python venv populated from the gfx950-dcgpu (cdna4 /
...
#   4. Run tiny_torch.py via `rocjitsu run --profile mi350x` with
...
#   ROCJITSU_SKIP_BUILD  set to 1 to reuse an already-built rocjitsu prefix
#                 (otherwise scripts/rocjitsu.sh rebuilds via Docker)
#
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROCJITSU_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
FIXTURE="$ROCJITSU_DIR/tests/fixtures/ml/tiny_torch.py"
PROFILE="mi350x"
...
# 3. Run the fixture through rocjitsu on the emulated MI350X. Use the venv's
#    python3 explicitly so the workload imports the nightly torch. Run rocjitsu
#    via scripts/rocjitsu.sh, which builds the glibc-portable rocjitsu + rocjitsu
...
#    Set ROCJITSU_SKIP_BUILD=1 to reuse an already-built prefix.
cd "$ROCJITSU_DIR"
log "running tiny_torch.py via rocjitsu --profile $PROFILE"
set +e
OUTPUT="$("$ROCJITSU_DIR/scripts/rocjitsu.sh" run --profile "$PROFILE" \
  -- "$VENV_PY" "$FIXTURE" 2>&1)"
STATUS=$?
set -e
printf '%s\n' "$OUTPUT"
