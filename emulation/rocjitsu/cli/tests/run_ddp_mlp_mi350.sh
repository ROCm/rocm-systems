...
# MI350X GPUs through rocjitsu, launched with `torchrun`. No physical GPU is
# required: rocjitsu emulates the devices.
#
# How it maps onto rocjitsu:
#   * `rocjitsu run --gpus-per-node N` makes the emulated node expose N GPUs
#     (rocjitsu synthesizes N KFD device nodes), so `torch.cuda.device_count()`
#     is N inside the workload.
#   * rocjitsu exports `MASTER_ADDR`/`MASTER_PORT` on every node, so
...
#   * rocjitsu runs the emulator as a separate daemon process by default, so the
#     rank processes share GPU memory through it. Pass `rocjitsu run --in-process`
...
#   2. `rocjitsu run --gpus-per-node $NPROC -- torchrun --standalone
#      --nproc_per_node=$NPROC ddp_mlp.py`, asserting rank 0 prints
#      `ddp_mlp_ok`.
#
# Env knobs:
#   VENV          venv location (default: <rocjitsu>/.venv-mi350)
...
FIXTURE="$ROCJITSU_DIR/tests/fixtures/ml/ddp_mlp.py"

PROFILE="${PROFILE:-mi350x}"
NPROC="${NPROC:-2}"
STEPS="${STEPS:-50}"
VENV="${VENV:-$ROCJITSU_DIR/.venv-mi350}"
...
# 3. Run DDP training through rocjitsu on the emulated node. Build & run rocjitsu
#    from the workspace (cargo run) so its rocjitsu KMD discovery/preload work;
#    the first `--` ends cargo's args, the second ends rocjitsu's.
cd "$ROCJITSU_DIR"
...
# 4. Assert success.
[[ $STATUS -eq 0 ]] || fail "rocjitsu run exited with status $STATUS"
