//! Matrix-driven end-to-end tests for the `rocjitsu` CLI.
//!
...
//! ```
//!
//! Every combination is exercised end-to-end against the real `rocjitsu`
...
//!
//! There is no daemon to start and no session to delete. `rocjitsu run`
...
//! consequently prints the emulator configuration rocjitsu synthesised for
...
//! honestly just "rocjitsu accepted the flag and the run completed".
...
/// How long one combination's `rocjitsu run` gets to finish.
///
/// Generous, because it covers session bring-up (which has its own
/// 60-second budget inside rocjitsu) as well as the payload. It exists to
...
impl Payload {
    ...
    /// print a sentinel: the goal of this suite is to prove the *rocjitsu
    ...
    fn argv(self) -> Vec<String> {
        let body = match self {
            ...
            // SIGSEGV-style code. rocjitsu must still tear the session down.
    ...
    ///
    /// Asserting on it is what separates "the workload ran" from "rocjitsu
...
///
/// It prints the emulator configuration rocjitsu synthesised for this
...
/// `rocjitsu run` — a bring-up, a workload and a teardown — and seventy-two
...
struct Caps {
    /// Per-emulator `(installed, supported)` from `rocjitsu emulators`.
...
/// Query `rocjitsu emulators --json` for the install/support state of
...
fn probe_caps() -> Caps {
    ...
    // write the developer's real rocjitsu directories regardless.
...
/// An isolated rocjitsu installation plus a mock container provider for one
/// combination.
struct Env {
    base: BaseEnv,
    provider: PathBuf,
...
impl Env {
    ...
    ///
    /// The name matters: rocjitsu decides how to pass GPU groups through to
...
case "$1" in
  ...
  network)
    ...
    if [ "$2" = inspect ]; then
      [ "$3" = "--format" ] && { printf rocjitsu; exit 0; }
      exit 1
    fi
    exit 0 ;;
  run)
    # Deliberately does not return. A node container is no longer started
    # detached: the provider client is a child rocjitsu owns, and killing it
  ...
  rm) exit 0 ;;
  inspect)
    # Either the ownership check (a Go template naming rocjitsu.owner), or
...
fn run_combo(c: &Combo, caps: &Caps) -> Outcome {
    ...
    //    the `rocjitsu run` that created it does, so once the process has
...
///
/// What is checked is the emulator configuration rocjitsu synthesised —
...
///
/// This is how a user finds the session to `rocjitsu exec` into from
/// another terminal, so it is worth insisting the line is there.
fn session_id(stderr: &str) -> Option<String> {
    stderr
        .lines()
        .find_map(|line| line.strip_prefix("rocjitsu: session "))
