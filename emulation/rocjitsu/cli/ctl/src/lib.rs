...
//!   [`rj_core::store`]. No session, no processes, nothing running.
//! * **Execution** — `run` and `exec`, in [`run`]. These own processes.
//!
//! There is no client/server split any more, and therefore no trait to
//! abstract one. `rocjitsu run` *is* the runtime: it holds its session in
//! its own address space. `rocjitsu exec` is the only command that talks
...
use rj_core::emulator::ExecMode;
use rj_core::profile::{ContainerizedDef, FileMount, Hack, PortMapping, ProfileDef};
use rj_core::registry::EmulatorInfo;
use rj_core::session::SessionId;
use rj_core::store::{DocKind, Stored};

/// The level rocjitsu logs at when nothing asks for more.
const DEFAULT_LOG: &str = "warn";

/// Initialize the global tracing subscriber.
///
/// `-v` / `-vv` wins over `ROCJITSU_CLI_LOG`: one is a decision about this
...
///
/// An unusable `ROCJITSU_CLI_LOG` is reported and then ignored, rather than
...
/// is exactly what a rocjitsu with nothing to say looks like.
pub fn init_logging(verbose: u8) {
    ...
    let filter = match (explicit, std::env::var("ROCJITSU_CLI_LOG")) {
        ...
        (None, Ok(value)) if !value.is_empty() => parse_log_filter(&value).unwrap_or_else(|why| {
            eprintln!(
                "rocjitsu: ROCJITSU_CLI_LOG={value:?}: {why}. Logging at the default level \
                 ({DEFAULT_LOG}) instead. It takes a level (`info`, `debug`, `off`) or \
                 a comma-separated list of `<target>=<level>` directives \
                 (`warn,rj_supervisor=debug`); `-v`/`-vv` say the same thing \
                 without it."
            );
...
/// Read a `ROCJITSU_CLI_LOG` value, rejecting the ones that only look valid.
///
/// [`EnvFilter`] accepts a bare word as a *target* directive at trace
/// level, so `ROCJITSU_CLI_LOG=not-a-level` parses cleanly and then matches
/// nothing rocjitsu ever logs to — every message, at every level,
...
/// the filter syntax spells it (`rj_supervisor=debug`), which is also
...
/// completes "ROCJITSU_CLI_LOG=…: {reason}".
...
/// [`rj_core::registry::registry`] probes each one for its identity
...
pub fn default_emulator() -> Option<EmulatorInfo> {
    let specs = registry();
    rj_core::registry::default_emulator(&specs).cloned()
}
...
/// Render the emulator registry for the `rocjitsu emulators` command:
...
/// support reason).
///
/// The long form answers "where did rocjitsu look?" because that is the
...
/// ([`rj_core::registry::default_emulator`]) from the `installed`
...
/// `rocjitsu emulators -l` on such a host should be told, rather than
...
fn daemon_caveat(name: &str) -> Option<String> {
    let backend = rj_core::emulator::get_emulator_backend(name)?;
...
fn emulators_cmd(long: bool, json: bool) {
    ...
    if long {
        for spec in &specs {
            ...
            // run this at all — where rocjitsu looked for it and what they
            ...
            // fills `support` — is on the path of every `rocjitsu run` that
...
/// the user can always force a full rewrite with `rocjitsu state
/// builtins`.
///
/// Shared by the CLI ([`dispatch`]) and the daemon so both surfaces
/// auto-unpack the builtins the first time they run, instead of
/// requiring the user to invoke `rocjitsu state builtins` by hand.
///
/// A failure here is said out loud rather than only `tracing::warn!`ed,
/// because it is never local to the builtins. Everything rocjitsu knows
...
pub fn ensure_builtins_present() {
    let outcome = [
        rj_builtin::ensure_agents(false).map(|_| ()),
        rj_builtin::ensure_topologies(false).map(|_| ()),
        rj_builtin::ensure_profiles(false).map(|_| ()),
    ]
    .into_iter()
    .find_map(Result::err);
    if let Some(e) = outcome {
        ...
        eprintln!(
            "rocjitsu: could not write rocjitsu's builtin configuration: {:#}",
            anyhow::Error::new(e)
        );
        eprintln!("rocjitsu: {}", config_dir_hint());
    }
}
...
/// moves — `ROCJITSU_CLI_CONFIG_DIR` and `XDG_CONFIG_HOME` — are both inherited
...
fn config_dir_hint() -> String {
    let dir = rj_core::paths::rocjitsu_config_dir();
    format!(
        "profiles, agents and topologies live in {}; \
         until it is readable and writable rocjitsu will behave as though \
         there are none. Fix its permissions, or point ROCJITSU_CLI_CONFIG_DIR at a \
...
pub fn validate_profile(def: &ProfileDef) -> Result<(), String> {
    ...
    match rj_core::emulator::get_emulator_backend(kind) {
...
/// All user-facing `rocjitsu` control subcommands. These are flattened
/// into the top-level `rocjitsu` subcommand list by the root binary.
...
pub enum CtlCmd {
    ...
    /// Run a command inside a session an existing `rocjitsu run` owns.
    ///
    /// The process runs in *this* terminal, as a child of this command,
    /// and dies with it.
    Exec(ExecArgsCli),

    /// Manage rocjitsu's on-disk state (builtin documents, purge).
    #[command(subcommand)]
    State(StateCmd),

    /// Reclaim what a `rocjitsu run` that died abruptly left behind.
    ///
    ...
    /// machine losing power leave no code of rocjitsu's to run at all —
    ...
    ///
    /// Safe to run at any time: sessions whose `rocjitsu run` still answers
    /// are left completely alone, as is anything rocjitsu did not create.
    ///
    /// The scope is this runtime directory. Every workload and container
    /// records the `ROCJITSU_CLI_RUNTIME_DIR` it was started under, and anything
    /// recording a different one — a CI job's, a test suite's, another
    /// terminal's — belongs to a rocjitsu whose live sessions this command
    /// cannot see, so it is left for that rocjitsu to reclaim.
    Cleanup {
    ...
    /// it with `rocjitsu exec` for as long as it is up.
    Run(RunArgs),

    /// Start, list and stop sessions that outlive a single command.
    #[command(subcommand)]
    Session(SessionCmd),

    /// Print where rocjitsu stores its state on this machine.
    Paths,
}

// ----- session ---------------------------------------------------------------

/// Sessions that exist for longer than one command.
///
/// A session is owned by exactly one process for its whole life, and
/// that has not changed: `rocjitsu session start` *is* that process. It
/// brings the session up, prints its id, and then does nothing but hold
/// it open, so other terminals can `rocjitsu exec --session <id>` into it
/// for as long as it runs. Background it, or leave it in a terminal of
/// its own.
///
/// There is deliberately no forking here. A session with no owner would
/// have nothing to remove its containers, stop its emulator daemon or
/// delete its scratch directory when it ends — which is the state
/// `rocjitsu cleanup` exists to repair, and not one to create on purpose.
#[derive(Subcommand, Debug)]
// `Start(RunArgs)` is most of the 368 bytes. Boxing to even the variants
// out is the same pessimisation `TopCmd` declines for the same reason:
// exactly one of these is ever built, by clap, and it is moved once into
// `session_cmd`.
#[allow(clippy::large_enum_variant)]
pub enum SessionCmd {
    /// Bring a session up with no workload and hold it open.
    ///
    /// Takes every override `rocjitsu run` does. Ends on Ctrl-C, or when
    /// `rocjitsu session stop` asks it to.
    Start(RunArgs),

    /// List the sessions that are up on this machine.
    ///
    /// Only sessions whose owning process still answers. A socket file
    /// left behind by a run that was killed is not a session; it is
    /// removed on the way past, which is the same probe `rocjitsu cleanup`
    /// uses.
    List,

    /// Ask a session to tear itself down.
    ///
    /// Returns as soon as the session has accepted, not when teardown has
    /// finished: removing a container per node takes as long as it takes.
    Stop {
        /// Session to stop. Optional when exactly one is up.
...
/// Arguments for `rocjitsu profile create`.
///
...
pub struct ProfileCreateArgs {
    ...
    /// first installed backend; see `rocjitsu emulators`.
    #[arg(long)]
    pub emulator: Option<String>,
    /// Agent name from `<ROCJITSU_CLI_CONFIG_DIR>/agent/` (e.g. `MI300X`,
...
/// Arguments for `rocjitsu exec`.
#[derive(Args, Debug)]
pub struct ExecArgsCli {
    /// Session to run in.
    ///
    /// Optional, and usually omitted: with exactly one `rocjitsu run`
    /// live — one terminal running the job, another one exec'ing into it
    /// — rocjitsu picks it. Naming one is only needed when several runs
    /// are up at once.
    // A flag rather than a positional because everything after `--`
    // belongs to the command: with both positional, `rocjitsu exec --
    ...
    /// to `1`, which is what makes a plain `rocjitsu exec` interactive.
    ///
    /// It is not the job's shape and cannot change it. Ranks are
    /// numbered in the *session's* grid — the one the `rocjitsu run` that
    /// owns it was started with — so a process started here sees the
    /// same `WORLD_SIZE` and the same rendezvous as the ranks already
    /// running, and `rocjitsu exec --node 2` is that node's rank 0 of the
    ...
    /// the run is already using: `rocjitsu exec -- cmd` on a live job puts
    ...
    /// variable — because rocjitsu's parent is your shell and what is in
    ...
    /// exported. The exceptions are the job's identity — `ROCJITSU_*`,
    /// `RANK`, `LOCAL_RANK`, `WORLD_SIZE`, `MASTER_ADDR`, `MASTER_PORT`,
    /// `NCCL_HOSTID` — which rocjitsu sets last, because a rank that
    ...
    /// The command and its arguments. Use `--` to separate from rocjitsu
    /// flags.
...
pub enum StateCmd {
    ...
    /// ones already on disk after upgrading rocjitsu.
    ///
    ...
    Builtins,
    /// Remove rocjitsu's runtime directory and reclaim orphaned containers.
    ///
    /// Refuses while any `rocjitsu run` is live: a run owns its session and
    ...
    Purge {
        ...
        #[arg(short = 'f', long)]
        force: bool,
        /// Also remove the rocjitsu config directory: every agent,
        /// topology and profile, including the ones you wrote. RocJITsu
...
pub struct RunArgs {
    ...
    /// `rocjitsu-dbt`, `hotswap`). See `rocjitsu emulators` for
    ...
    // `run_cmd`. Use `rocjitsu exec` to join a run that is already up.
    /// Working directory.
    ///
    /// On a containerised session this names a directory *inside* the
    /// container; otherwise one on this machine, which must exist.
    /// Defaults to the directory `rocjitsu run` was started from.
    #[arg(long)]
    ...
    /// when omitted. The `ROCJITSU_CONTAINER_PROVIDER` environment variable
    /// has the same effect.
    #[arg(long = "container-provider")]
    ...
    /// from the profile (the upstream `rocjitsu --config`). Defaults to
    /// the file `RJ_CONFIG` names.
    ///
    ...
    // already applies. Use `rocjitsu exec --node N` for a terminal on one
    ...
    ///
    /// Optional. Omitted, the session comes up with no workload and
    /// stays up until it is stopped — see `rocjitsu session start`, which
    /// is the spelling for that.
    #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
    argv: Vec<String>,
}

/// Hand-written rather than derived, so that `RunArgs::default()` is the
/// same set of arguments clap produces for a bare `rocjitsu run -- …`.
///
...
// =============================================================================
// A mistyped rocjitsu flag is not a workload
// =============================================================================
...
/// and said no" (this) from "rocjitsu tried and could not" (1), and a
...
async fn dispatch_inner(cmd: CtlCmd, json: bool) -> anyhow::Result<ExitCode> {
    ...
    // startup so they are always available under <ROCJITSU_CLI_CONFIG_DIR>/. Errors
    // here are non-fatal; the user can recover via `rocjitsu state
    // builtins`.
    ...
    match cmd {
        ...
        CtlCmd::Cleanup { dry_run } => {
            ...
            // Zero would say the machine is clean. Something rocjitsu set
            // out to remove is still there.
            Ok(ExitCode::from(u8::from(unfinished > 0)))
        }
        CtlCmd::Run(a) => run::run_cmd(a).await,
        CtlCmd::Session(c) => run::session_cmd(c, json).await,
        CtlCmd::Paths => {
            print_paths(json);
            Ok(ExitCode::from(0))
        }
...
async fn profile_cmd(cmd: ProfileCmd, json: bool) -> anyhow::Result<ExitCode> {
    match cmd {
        ProfileCmd::List { long } => {
            let names = rj_core::store::profile_list()?;
            if json && long {
                ...
                for n in names {
                    match rj_core::store::profile_get(&n) {
                        Ok(p) => println!(
                            "{:<24} {:<16} {}",
                            p.name,
                            p.emulator.emulator,
        ...
        ProfileCmd::Show { name } => {
            let p = rj_core::store::profile_get(&name)?;
        ...
        ProfileCmd::Create(a) => {
            ...
            let stored = rj_core::store::profile_put(&p)?;
        ...
        ProfileCmd::Delete { name, force } => {
            return delete_document(json, DocKind::Profile, &name, force, |name| {
                rj_core::store::profile_delete(name)
            });
        }
    }
    Ok(ExitCode::from(0))
...
fn long_profiles(names: &[String]) -> Vec<serde_json::Value> {
    names
        .iter()
        .map(|n| match rj_core::store::profile_get(n) {
...
///
/// Replacing an untouched builtin is deliberately allowed — it is rocjitsu's
...
/// another matter: `rocjitsu topology create MI350X-2x8` exited 0 with
...
fn report_replaced_builtin(kind: DocKind, name: &str, stored: Stored) {
    ...
    eprintln!(
        "rocjitsu: {name} was a builtin {kind} rocjitsu ships, and this replaced it. \
         Nothing you wrote was lost; `rocjitsu {kind} delete {name}` restores the \
         shipped version."
    );
}
...
async fn topology_cmd(cmd: TopologyCmd, json: bool) -> anyhow::Result<ExitCode> {
    match cmd {
        TopologyCmd::List => {
            let names = rj_core::store::topology_list()?;
            if json {
                println!("{}", serde_json::to_string_pretty(&names)?);
            } else {
                for n in names {
        ...
        TopologyCmd::Show { name } => {
            let t = rj_core::store::topology_get(&name)?;
        ...
        TopologyCmd::Create {
            ...
            let t = rj_core::topology::TopologyDef {
                num_nodes,
                gpus_per_node,
                agent: MaybeRef::Ref(DocKind::Agent.canonical(&agent)),
            };
            let stored = rj_core::store::topology_put(&name, &t)?;
        ...
        TopologyCmd::Delete { name, force } => {
            return delete_document(json, DocKind::Topology, &name, force, |name| {
                rj_core::store::topology_delete(name)
            });
        }
    }
    Ok(ExitCode::from(0))
...
async fn agent_cmd(cmd: AgentCmd, json: bool) -> anyhow::Result<ExitCode> {
    match cmd {
        AgentCmd::List => {
            let names = rj_core::store::agent_list()?;
            if json {
                println!("{}", serde_json::to_string_pretty(&names)?);
            } else {
                for n in names {
        ...
        AgentCmd::Show { name } => {
            let a = rj_core::store::agent_get(&name)?;
            println!("{}", serde_json::to_string_pretty(&a)?);
        }
        AgentCmd::Import { name, file } => {
            let (bytes, from) = read_input(&file)?;
            let a: rj_core::agent::AgentDef =
                serde_json::from_slice(&bytes).map_err(|e| json_error(from, e))?;
            let stored = rj_core::store::agent_put(&name, &a)?;
        ...
        AgentCmd::Delete { name, force } => {
            return delete_document(json, DocKind::Agent, &name, force, |name| {
                rj_core::store::agent_delete(name)
            });
        }
    }
    Ok(ExitCode::from(0))
...
/// `rocjitsu profile import --json f.json` emitted `imported profile x`
...
///   text mode both printed nothing at all, so `rocjitsu profile delete x`
...
fn delete_document(
    ...
    name: &str,
    force: bool,
    delete: impl FnOnce(&str) -> rj_core::error::Result<()>,
    ...
    rj_core::store::validate_name(kind, name)?;
    if !kind.path(name).exists() {
        return Err(rj_core::error::RocJITsuError::not_found(kind, name).into());
    }
    ...
    if json {
        ...
        println!(
            "deleted your {word} {name}; rocjitsu ships a builtin of that name, \
...
/// `--json`, on the strength of rocjitsu *shipping* a document of that
/// name. Nothing had written it: a delete removes a file and stops, and
/// the shipped copy reappears only when the next command's
/// [`ensure_builtins_present`] notices it missing. So `rocjitsu agent
...
/// for a file that would not exist until it ran rocjitsu again.
///
...
/// config directory rocjitsu cannot write is exactly the case where the
/// old claim was worst, and it is the case where this one says no.
fn restore_shipped(kind: DocKind, name: &str) -> bool {
    if rj_core::store::shipped(kind, name).is_none() {
        return false;
    }
    ensure_builtins_present();
...
fn read_input(file: &str) -> anyhow::Result<(Vec<u8>, std::path::PathBuf)> {
    ...
    let bytes = if file == "-" {
        ...
        std::io::Read::read_to_end(&mut std::io::stdin().lock(), &mut buf)
            .map_err(|e| rj_core::error::RocJITsuError::io(&from, e))?;
        buf
    } else {
        std::fs::read(file).map_err(|e| rj_core::error::RocJITsuError::io(&from, e))?
    };
    Ok((bytes, from))
}
...
/// user has. [`RocJITsuError::Json`] is the shape the rest of rocjitsu
/// reports a bad document in.
///
/// [`RocJITsuError::Json`]: rj_core::error::RocJITsuError::Json
fn json_error(path: std::path::PathBuf, source: serde_json::Error) -> anyhow::Error {
    anyhow::Error::new(rj_core::error::RocJITsuError::Json { path, source })
}
...
fn build_profile_create(a: ProfileCreateArgs, interactive: bool) -> anyhow::Result<ProfileDef> {
    ...
    let spec = match a.emulator.as_deref() {
        ...
        None => default_emulator().ok_or_else(|| {
            anyhow::anyhow!(
                "this build of rocjitsu has no emulator backends compiled in; \
    ...
    let agent = match a.agent {
        Some(a) => a,
        None if interactive => {
            let known = rj_core::agent::store::list().unwrap_or_default();
            if known.is_empty() {
                "MI350X".to_string()
            } else {
    ...
    let topo = rj_core::topology::TopologyDef {
        num_nodes,
        gpus_per_node,
        agent: MaybeRef::Ref(DocKind::Agent.canonical(&agent)),
    };
    Ok(ProfileDef {
        name,
        description,
        emulator: rj_core::registry::make_def(&spec, topo),
        containerize,
    })
}
...
/// The one way rocjitsu says it has no such emulator.
///
...
fn unknown_emulator(name: &str) -> String {
    format!(
        "unknown emulator `{name}`; this build has: {}. See `rocjitsu emulators`.",
...
/// `rocjitsu emulators --json`), so the mistake is knowable here — and it
        ...
        if schema.is_empty() {
            anyhow::bail!(
                "unknown option `{key}` for emulator `{emulator}`, which accepts no options. \
                 What it emulates comes from its agent and topology (`rocjitsu agent list`, \
                 `rocjitsu topology list`), or from a config file passed with `--config`."
            );
        }
        anyhow::bail!(
            "unknown option `{key}` for emulator `{emulator}`; \
             it accepts: {}. See `rocjitsu emulators -l`.",
            schema.join(", ")
        );
    }
    Ok(())
...
/// host (see `rocjitsu emulators -l`), so an unloadable name is knowable
        ...
        if available.is_empty() {
            anyhow::bail!(
                "no plugin `{name}` for emulator `{emulator}`: this host has none of its \
                 plugins installed. `rocjitsu emulators -l` lists what was found."
            );
        }
        anyhow::bail!(
            "no plugin `{name}` for emulator `{emulator}`; \
             this host has: {}. See `rocjitsu emulators -l`.",
            available.join(", ")
        );
    }
    Ok(())
...
/// Reject a process grid rocjitsu will not start, before it has built the
    ...
    if world > u64::from(max) {
        anyhow::bail!(
            "{num_nodes} nodes x {nproc_per_node} processes per node is {world} processes, \
             more than the {max} rocjitsu will start for one exec"
        );
    }
    Ok(())
}

/// Descriptors rocjitsu needs for itself, beside the workload's.
///
...
/// and rocjitsu holds the read end of each, which suggests two — but
/// counting `/proc/<rocjitsu>/fd` against a live grid gives a slope of
...
///
/// A one-process job inherits rocjitsu's own streams instead and costs
...
/// rocjitsu bounds. The symptom is bad in a specific way: the ranks that
...
fn ensure_descriptors_for(world: u64) -> anyhow::Result<()> {
    ...
    anyhow::bail!(
        "{world} processes need about {need} open files ({DESCRIPTORS_PER_RANK} per rank for \
         its captured output, plus {DESCRIPTOR_RESERVE} for rocjitsu itself), and this process \
         may not \
...
fn profile_node_count(profile: &ProfileDef) -> Option<u32> {
    match &profile.emulator.topology {
        ...
        MaybeRef::Ref(name) => rj_core::store::topology_get(name).ok().map(|t| t.num_nodes),
    }
}
...
/// The section that makes a document a DBT-guest config instead.
///
/// A guest config describes a synthetic GPU to lay over a host and names
/// the machine to emulate in its own `simulator_config` (or does not
...
/// rocjitsu had already been told was unusable. Following rocjitsu's own
...
/// still refused is refused by the emulator saying so. What rocjitsu
...
/// Whether this profile's drop-in `--config` names a DBT-guest config.
///
/// Read from the file rather than remembered from [`check_config_file`]:
/// the answer is wanted in two places that do not pass values to each
/// other, and getting it is one parse of a small file the previous check
/// has just left in the page cache. An unreadable or unparsable file is
/// not a guest config — it is not a config at all, and `check_config_file`
/// is what says so.
pub(crate) fn names_a_guest_config(profile: &ProfileDef) -> bool {
...
/// same file the C++ runtime reads it for.
const ENV_RJ_CONFIG: &str = "RJ_CONFIG";

/// The emulator config file this run should use: an explicit `--config`,
/// or failing that the file [`ENV_RJ_CONFIG`] names.
///
/// Honouring the variable means `RJ_CONFIG=… rocjitsu run …` needs no
/// second spelling of a path the environment already carries — the
/// variable and the flag mean precisely the same thing.
///
/// The variable is ignored when any of the flags `--config` refuses to
/// sit beside is present. Those synthesise a config, so obeying both
/// would be a contradiction; declaring the default to clap instead would
/// make it one, since clap counts an environment value as present for
/// conflict purposes and an ambient `RJ_CONFIG` would then turn every
/// `--plugin` command line in the shell into a usage error.
fn effective_config(a: &RunArgs) -> Option<String> {
    config_or(a, std::env::var(ENV_RJ_CONFIG).ok())
}

/// [`effective_config`] with the variable's value passed in, so the
/// precedence rules can be tested. Setting an environment variable is
/// `unsafe` under this workspace's `unsafe_code = "forbid"`, so the
/// alternative would be to test this only through a child process.
fn config_or(a: &RunArgs, from_env: Option<String>) -> Option<String> {
    if a.config.is_some() {
        return a.config.clone();
    }
    if a.gpus_per_node.is_some()
        || a.exec_mode.is_some()
        || !a.options.is_empty()
        || !a.plugins.is_empty()
    {
    ...
    from_env.filter(|path| !path.is_empty())
}
...
fn apply_profile_overrides(
    ...
    if a.num_nodes.is_some() || a.gpus_per_node.is_some() {
        let mut topo = match &profile.emulator.topology {
            ...
            // `TopologyNotFound`.
            MaybeRef::Ref(name) => rj_core::store::topology_get(name)?,
        };
...
/// arguments.
///
/// An empty `argv` yields an empty command rather than panicking. `exec`
/// marks the field `required`; `run` does not, but a `run` with no argv
/// is a workload-less session and is held open before this is reached.
...
/// The variables rocjitsu sets on every workload process itself, after
...
/// with the grid rocjitsu actually built, would deadlock its own
/// collectives rather than merely misreport. Naming the constants rather
/// than the strings keeps this list in step with
/// [`rj_supervisor`]'s, which is what actually applies them.
const ROCJITSU_OWNED_ENV: [&str; 11] = [
    rj_core::container::ENV_SESSION,
    rj_core::container::ENV_RUNTIME,
    rj_core::container::ENV_RANK,
    rj_core::container::ENV_TORCH_RANK,
    rj_core::container::ENV_HEAD_ADDR,
    rj_core::container::ENV_HEAD_PORT,
    rj_core::container::ENV_MASTER_ADDR,
    rj_core::container::ENV_MASTER_PORT,
    rj_core::container::ENV_WORLD_SIZE,
    rj_core::container::ENV_LOCAL_RANK,
    rj_core::container::ENV_NCCL_HOSTID,
];

/// Which of `env`'s keys rocjitsu will overwrite, in the order given.
///
/// The precedence itself is deliberate (see [`ROCJITSU_OWNED_ENV`]); what
/// was not deliberate is that the losing side was invisible, so
/// `--env RANK=3` looked accepted and did nothing.
fn rocjitsu_owned_env<'a>(env: impl IntoIterator<Item = &'a String>) -> Vec<&'static str> {
    env.into_iter()
        .filter_map(|key| {
            ROCJITSU_OWNED_ENV
                .iter()
                .find(|owned| *owned == key)
                .copied()
        })
        .collect()
}
...
/// (`rocjitsu run --gdb`). `["./add_one", "5"]` becomes
...
struct Resource {
    ...
    /// [`rj_core::container::image_labels`]. Naming the session that
...
struct Reclaimed {
    ...
    processes: Vec<rj_core::reclaim::Stranded>,
...
///
/// `ROCJITSU_CONTAINER_PROVIDER` names one deliberately, and is then the
...
///
/// The candidate list mirrors [`rj_core::container::detect_provider`],
/// which returns only the first match and so cannot be reused here.
fn cleanup_providers() -> Vec<String> {
    if let Ok(explicit) = std::env::var("ROCJITSU_CONTAINER_PROVIDER")
        && !explicit.is_empty()
    {
        return vec![explicit];
    }
...
/// A derived `--hack` image one runtime directory's rocjitsu built.
...
struct DerivedImage {
    ...
    /// user recognises `rocjitsu-hack-…:latest`, not twelve hex digits.
    name: String,
}
...
/// The `rocjitsu.owner` and `rocjitsu.runtime` labels an engine reports for
...
fn image_marks(provider: &str, id: &str) -> Option<(Option<String>, Option<String>)> {
    use rj_core::container::{LABEL_OWNER, LABEL_RUNTIME};
    let template = format!(
...
/// everything else rocjitsu creates. Both marks are required *positively*:
/// `rocjitsu.owner` says a rocjitsu built this, and `rocjitsu.runtime` says
/// which one. An image the engine listed that carries neither is
/// somebody else's — including a `rocjitsu-hack-…` left by a rocjitsu older
...
fn reclaimable_images(provider: &str, reclaimed: &[String]) -> Vec<DerivedImage> {
    use rj_core::container::{LABEL_OWNER, LABEL_OWNER_VALUE};
    let ours = rj_core::container::owning_runtime();
    ...
    for id in order {
        ...
        if owner != LABEL_OWNER_VALUE || !rj_core::container::same_runtime(&runtime, &ours) {
            continue;
        }
...
/// Reclaim everything belonging to a session no live `rocjitsu run` owns.
///
...
async fn cleanup(dry_run: bool) -> Reclaimed {
    ...
    // 1. Workload processes, found by the `ROCJITSU_SESSION` each carries.
    //
    ...
    let processes = tokio::task::spawn_blocking({
        let live = live.clone();
        move || {
            let stranded = rj_core::reclaim::stranded_workloads(&live);
            if !dry_run {
                rj_core::reclaim::reap(&stranded);
            }
            stranded
        }
    })
    ...
    // 2. Containers and networks, found by the `rocjitsu.owner` label, on
    ...
    for provider in cleanup_providers() {
        let found = tokio::task::spawn_blocking({
            ...
            move || {
                let orphans = rj_core::container::orphans(&provider, &live);
                if !dry_run {
                    ...
                    rj_core::container::remove_orphans(&provider, &orphans);
                }
                orphans
            }
        })
    ...
    // and a `rocjitsu run` started in that window binds its socket and
    ...
    if let Ok(entries) = std::fs::read_dir(rj_core::paths::session_runtime_root()) {
...
async fn state_cmd(cmd: StateCmd, json: bool) -> anyhow::Result<ExitCode> {
    match cmd {
        ...
        StateCmd::Purge { force, all } => {
            let prompt = if all {
                "purge ALL rocjitsu state, including agents, topologies and profiles?"
            } else {
                "purge all rocjitsu runtime/state and stop all sessions?"
            };
...
/// `rocjitsu state builtins`: refresh every builtin document on disk.
///
...
fn builtins_cmd(json: bool) -> anyhow::Result<ExitCode> {
    let kinds = [
        (DocKind::Agent, rj_builtin::ensure_agents(true)?),
        (DocKind::Topology, rj_builtin::ensure_topologies(true)?),
        (DocKind::Profile, rj_builtin::ensure_profiles(true)?),
    ];
    ...
    // what did land. On stderr: this is a note about files rocjitsu did not
    ...
    if !edited.is_empty() {
        ...
        eprintln!(
            "rocjitsu: {} builtin document{} differ{} from the one{} rocjitsu ships and \
             {} left alone:",
            edited.len(),
            n.pick("", "s"),
            n.pick("s", ""),
        ...
        eprintln!(
            "rocjitsu: rewriting one would discard your edits, and everything else was \
             refreshed. To take the shipped version of one after all, delete it \
             (`rocjitsu <kind> delete <name>`) and run `rocjitsu state builtins` again."
        );
    }
    Ok(ExitCode::from(0))
}

/// Stop every live run and remove rocjitsu's on-disk state.
async fn purge(all: bool, json: bool) -> anyhow::Result<ExitCode> {
    ...
    if !live.is_empty() {
        ...
        anyhow::bail!(
            "{} `rocjitsu run` process{} {} still running ({}). \
    ...
    // not remove itself. This is `rocjitsu cleanup`, run for its effect
    ...
    // `rocjitsu run` started in that window is a healthy session whose
    ...
    if !racing.is_empty() {
        ...
        anyhow::bail!(
            "{} `rocjitsu run` started while this purge was working ({}); \
    ...
    let mut targets = vec![rj_core::paths::rocjitsu_runtime_dir()];
    if all {
        targets.push(rj_core::paths::rocjitsu_config_dir());
    }
    ...
    if json {
        ...
        if !ok {
            eprintln!(
                "rocjitsu: the purge is incomplete; {unfinished} thing{} could not be removed",
...
/// followed by a bare `purged` line, so `rocjitsu state purge --json | jq`
...
/// who is watching, and rocjitsu cannot tell whether anybody is on the far
...
/// in the background — leaves `rocjitsu topology delete x` blocked forever,
...
/// Ctrl-D is not a "no". Taking it for one is how `rocjitsu profile delete
...
/// Every directory `rocjitsu paths` reports, as one document.
fn paths_json() -> serde_json::Value {
    serde_json::json!({
        "config": rj_core::paths::rocjitsu_config_dir(),
        "runtime": rj_core::paths::rocjitsu_runtime_dir(),
        "profiles": rj_core::paths::profile_root(),
        "topologies": rj_core::paths::topology_root(),
        "agents": rj_core::paths::agent_root(),
        "sessions": rj_core::paths::session_runtime_root(),
        "runs": rj_core::paths::run_socket_root(),
    })
}

/// Print where rocjitsu keeps things on this machine.
///
...
/// this command exists precisely so that nobody has to infer where rocjitsu
...
mod tests {
    ...
    use super::*;
    use rj_core::common::SimpleMap;
    use rj_core::emulator::{EmulatorDef, EmulatorKind};
    ...
    fn run_args_parse_repeated_plugin_flags() {
        ...
        let w = Wrap::try_parse_from([
            "rocjitsu", "--plugin", "race", "--plugin", "logging", "--", "./app",
        ])
        .expect("`rocjitsu run --plugin race --plugin logging -- ./app` should parse");
        assert_eq!(
            w.run.plugins,
    ...
    /// Parse `rocjitsu run`'s arguments exactly as the real command does.
    fn parse_run(args: &[&str]) -> Result<RunArgs, clap::Error> {
        ...
        let mut argv = vec!["rocjitsu"];
        argv.extend_from_slice(args);
        Wrap::try_parse_from(argv).map(|w| w.run)
    }

    /// `rocjitsu run --help`, as clap renders it.
    ...
    fn parse_ctl(args: &[&str]) -> Result<CtlCmd, clap::Error> {
        use clap::Parser;
        #[derive(Parser)]
        struct Wrap {
            #[command(subcommand)]
        ...
        let mut argv = vec!["rocjitsu"];
        argv.extend_from_slice(args);
        Wrap::try_parse_from(argv).map(|w| w.cmd)
    }

    #[test]
    fn run_no_longer_requires_a_workload() {
        // `argv` was `required = true`, so the only way to get a session
        // was to give it something to run — and `--session <id>` on
        // `exec`, which names a session, had no way to bring one into
        // existence in the first place.
        let a =
            parse_run(&["--profile", "mi300x"]).expect("`rocjitsu run --profile mi300x` parses");
        assert!(a.argv.is_empty());
        assert_eq!(a.profile, "mi300x");

        // A workload still parses the same way, `--` and all.
        let a = parse_run(&["--", "./app", "--flag"]).unwrap();
        assert_eq!(a.argv, vec!["./app".to_string(), "--flag".to_string()]);
    }

    #[test]
    fn session_start_takes_the_same_overrides_as_run() {
        // The point of the subcommand: it is `run` without the workload,
        // not a second, poorer way of describing a session.
        let cmd = parse_ctl(&[
            "session",
            "start",
            "--profile",
        ...
        .expect("`rocjitsu session start` parses");
        match cmd {
            CtlCmd::Session(SessionCmd::Start(a)) => {
                assert_eq!(a.profile, "mi300x");
                assert_eq!(a.num_nodes, Some(2));
                assert!(a.argv.is_empty());
            }
    ...
    #[test]
    fn session_stop_names_a_session_or_leaves_it_to_be_inferred() {
        // Optional, because the common case is one session; named, for
        // when it is not. `run_rocgdb_official.py` calls the bare form.
        match parse_ctl(&["session", "stop"]).expect("a bare `session stop` parses") {
            CtlCmd::Session(SessionCmd::Stop { session }) => assert_eq!(session, None),
            other => panic!("expected `session stop`, got {other:?}"),
        }
        match parse_ctl(&["session", "stop", "abc123"]).expect("a named `session stop` parses") {
            CtlCmd::Session(SessionCmd::Stop { session }) => {
                assert_eq!(session.as_deref(), Some("abc123"));
            }
            other => panic!("expected `session stop`, got {other:?}"),
        }
        assert!(matches!(
            parse_ctl(&["session", "list"]).expect("`session list` parses"),
            CtlCmd::Session(SessionCmd::List)
        ));
    }
    ...
    fn exec_help_says_how_nproc_per_node_relates_to_the_shape_of_the_job() {
        ...
        // in `rocjitsu exec --help` with nothing beside it — and the rule
        ...
        let (_, after) = help
            ...
            .expect("`rocjitsu exec --help` must list --nproc-per-node");
    ...
    fn a_log_filter_that_would_silence_everything_is_refused() {
        ...
        // this parses and then matches nothing rocjitsu logs — the whole
        ...
        for good in ["info", "off", "warn,rj_supervisor=debug", "rj_ctl=trace"] {
    ...
    #[test]
    fn the_variables_rocjitsu_owns_are_the_ones_reported_as_ignored() {
        let keys = [
            "RANK".to_string(),
            "PYTHONPATH".to_string(),
            "WORLD_SIZE".to_string(),
        ];
        assert_eq!(rocjitsu_owned_env(keys.iter()), vec!["RANK", "WORLD_SIZE"]);
        assert!(rocjitsu_owned_env(["HSA_XNACK".to_string()].iter()).is_empty());
    }
    ...
    /// call. Taking rocjitsu's advice therefore turned a config rocjitsu had
    ...
    fn a_config_that_describes_no_machine_is_refused_whichever_mode_runs_it() {
        ...
        // A DBT-guest config is the exception, and the check refused
        ...
        .unwrap();
        assert!(
            check_config_file(&guest.to_string_lossy())
                .unwrap()
                .is_absolute()
        );
    ...
    fn a_guest_config_is_recognised_by_its_section_alone() {
        ...
        fn set(profile: &mut ProfileDef, path: String) {
            profile
                .emulator
                .options
                .insert("config".to_string(), SimpleValue::String(path));
        }

        set(
            &mut profile,
            write("machine.json", r#"{"vm": {}, "topology": {}}"#),
        );
        assert!(!names_a_guest_config(&profile));
        ...
        // Unreadable and unparsable files are `check_config_file`'s to
        // refuse; this answers the narrower question, and answers it no.
        set(
            &mut profile,
            dir.path().join("absent.json").to_string_lossy().to_string(),
        );
        assert!(!names_a_guest_config(&profile));
        set(&mut profile, write("junk.json", "not json"));
        assert!(!names_a_guest_config(&profile));
    }
    ...
    fn config_refuses_the_flags_it_would_have_to_ignore() {
        ...
        // machine has is rocjitsu's business, not the emulator config's.
    ...
    fn a_cleanup_report_says_what_each_thing_was() {
        ...
        let reclaimed = Reclaimed {
            resources: vec![
                ...
                Resource {
                    id: "rocjitsu-s-1".to_string(),
                    kind: "network",
                    provider: "docker".to_string(),
                    session: Some("s-1".to_string()),
                },
                Resource {
                    id: "rocjitsu-hack-90d1e055da77723c:latest".to_string(),
                    kind: "image",
                    provider: "docker".to_string(),
                    session: None,
                },
            ],
            failures: vec!["could not remove /run/rocjitsu/s-2: denied".to_string()],
            ..Reclaimed::default()
        };
        ...
        // text has to say so — a user reading "removed rocjitsu-hack-…"
        ...
        assert_eq!(
            ...
            "would remove image rocjitsu-hack-90d1e055da77723c:latest (docker)"
        );
...
case "$1" in
  images)
    printf 'aaa111\trocjitsu-hack-ours:latest\naaa111\trocjitsu-hack-ours:also\nbbb222\trocjitsu-hack-theirs:latest\nccc333\tsomebody-elses:latest\n'
    ;;
  image)
    # $5 is the id; the template asks for owner and runtime, tab-joined.
    case "$5" in
      aaa111) printf 'rocjitsu\t{ours}\n' ;;
      bbb222) printf 'rocjitsu\t/some/other/runtime\n' ;;
...
"#
    ...
    fn cleanup_reclaims_only_the_derived_images_this_runtime_built() {
        ...
        std::fs::create_dir_all(rj_core::paths::rocjitsu_runtime_dir()).unwrap();
        let ours = rj_core::container::owning_runtime();
        ...
        assert_eq!(
            ...
            vec![DerivedImage {
                id: "aaa111".to_string(),
                name: "rocjitsu-hack-ours:latest".to_string(),
            }],
    ...
    fn an_image_a_live_session_is_still_running_is_left_alone() {
        ...
        let engine = fake_engine(
            root.path(),
            &ours,
            "live-container\\trocjitsu-hack-ours:also\\n",
        );
    ...
    fn a_purge_that_left_something_behind_does_not_claim_to_have_purged() {
        ...
        let ok = purge_json(
            false,
            &[std::path::PathBuf::from("/run/rocjitsu")],
            &[],
            &reclaimed,
        );
        ...
        let failed = purge_json(
            false,
            &[],
            &["could not remove /run/rocjitsu: Permission denied".to_string()],
            &reclaimed,
        );
    ...
    fn purge_fails_when_the_runtime_directory_survives() {
        ...
        let root = tempfile::tempdir().unwrap();
        rj_core::paths::set_test_root(root.path());
        let runtime = rj_core::paths::rocjitsu_runtime_dir();
    ...
    fn paths_names_every_directory_a_document_lives_in() {
        ...
        // answer to "where does rocjitsu read the agent I edited?" was left
        ...
        for (key, dir) in [
            ...
            assert_eq!(
                ...
                "`rocjitsu paths` must report {key}"
            );
        }
    ...
    fn the_long_profile_list_carries_its_long_fields_into_json() {
        ...
        rj_core::state::write_json(&rj_core::paths::profile_path(&p.name), &p).unwrap();
    ...
    fn state_builtins_refreshes_every_kind_and_survives_an_edited_one() {
        ...
        let mut agent = rj_core::store::agent_get("mi300x").unwrap();
        agent.vm.gpu.num_xcds = 1;
        rj_core::state::write_json(&rj_core::paths::agent_path("mi300x"), &agent).unwrap();
        let mut topology = rj_core::store::topology_get("MI350X-2x8").unwrap();
        topology.num_nodes = 3;
        rj_core::state::write_json(&rj_core::paths::topology_path("MI350X-2x8"), &topology)
            .unwrap();
        let mut profile = rj_core::store::profile_get("mi450x").unwrap();
        profile.description = Some("mine".to_string());
        rj_core::state::write_json(&rj_core::paths::profile_path("mi450x"), &profile).unwrap();
        ...
        assert_eq!(
            rj_core::store::agent_get("mi300x").unwrap().vm.gpu.num_xcds,
            1
        );
        assert_eq!(
            rj_core::store::topology_get("MI350X-2x8")
                .unwrap()
                .num_nodes,
            3
        );
        assert_eq!(
            rj_core::store::profile_get("mi450x").unwrap().description,
            Some("mine".to_string())
        );
        rj_core::paths::clear_test_root();
        assert_eq!(
    ...
    ///
    /// It inherits rocjitsu's own streams rather than being captured, so it
    ...
    ///
    /// End-of-file was already refused, so `rocjitsu topology delete x <
    ...
    /// not there.
    ///
    /// The prompt came first, so `rocjitsu profile delete proflie` asked
    ...
    /// The shipped version is on disk before rocjitsu says it is back.
    ///
    /// Deleting an edited builtin removes the user's copy and the
    /// shipped one takes its place — but nothing wrote it: the claim was
    /// made from the fact that rocjitsu *ships* one, and the file only
    ...
    fn a_restored_builtin_is_on_disk_before_it_is_claimed() {
        ...
        assert!(
            rj_core::paths::agent_path("mi300x").is_file(),
    ...
    /// One sentence for "rocjitsu has no such emulator", wherever it is
    /// said.
    #[test]
    fn every_unknown_emulator_is_reported_the_same_way() {
        ...
        assert!(
            from_validate.contains("rocjitsu emulators"),
    ...
    fn run_args_default_matches_clap() {
        ...
        let w = Wrap::try_parse_from(["rocjitsu", "--", "./app"])
            .expect("`rocjitsu run -- ./app` should parse");
        assert_eq!(w.run.profile, RunArgs::default().profile);
    }

    /// Parse `argv` as `run` arguments, for the `RJ_CONFIG` cases below.
    fn run_args(argv: &[&str]) -> RunArgs {
        ...
        let mut full = vec!["rocjitsu"];
        full.extend_from_slice(argv);
        Wrap::try_parse_from(full)
            .expect("arguments should parse")
            .run
    }

    #[test]
    fn rj_config_is_the_default_for_the_config_flag() {
        let a = run_args(&["--", "./app"]);
        assert_eq!(
            config_or(&a, Some("/from/env.json".to_string())).as_deref(),
            Some("/from/env.json")
        );
        // The flag is the more specific request, so it wins.
        let explicit = run_args(&["--config", "/from/flag.json", "--", "./app"]);
        assert_eq!(
            config_or(&explicit, Some("/from/env.json".to_string())).as_deref(),
            Some("/from/flag.json")
        );
        // Unset and set-but-empty are the same request: none.
        assert_eq!(config_or(&a, None), None);
        assert_eq!(config_or(&a, Some(String::new())), None);
    }

    #[test]
    fn rj_config_yields_to_the_flags_config_refuses_to_sit_beside() {
        // `--config` conflicts with each of these, so a config coming
        // from the environment must not turn an otherwise valid command
        // line into a contradiction. Each is checked on its own: a
        // single `||` chain that had lost a term would still pass a test
        // that only ever set one of them.
        for argv in [
            &["--gpus-per-node", "2", "--", "./app"][..],
            &["--exec-mode", "clocked", "--", "./app"][..],
            &["--option", "key=value", "--", "./app"][..],
            &["--plugin", "race", "--", "./app"][..],
        ] {
            let a = run_args(argv);
            assert_eq!(
                config_or(&a, Some("/from/env.json".to_string())),
                None,
                "{argv:?} should ignore RJ_CONFIG"
            );
        }
    }
}
