//! `rocjitsu run` and `rocjitsu exec`: the two commands that start processes.
//!
//! They look symmetrical and are not, and the asymmetry is the whole
//! design:
//!
//! * `rocjitsu run` **owns** a session. It brings one up in its own
...
//!   it.
//! * `rocjitsu exec` **borrows** one. It asks a live run to describe its
...
//! terminal — not to the terminal the user typed `rocjitsu exec` into.
//! Having the client spawn is what makes `rocjitsu exec -- bash` an
...
//! [`rj_supervisor::build_specs`], from the same description, so a
...
use rj_core::exec::{ExecArgs, ExecDef, ExecId};
use rj_core::proto::SessionDescription;
use rj_core::session::{CreateSessionRequest, SessionId};
use rj_supervisor::{Exec, Run, rpc::ControlSocket};
use tokio::sync::mpsc;

use crate::{ExecArgsCli, RunArgs, SessionCmd, apply_profile_overrides, parse_envs, split_argv};
...
/// `rocjitsu run`: own a session for the lifetime of one command.
pub async fn run_cmd(a: RunArgs) -> anyhow::Result<ExitCode> {
    ...
    let mut profile = rj_core::store::profile_get(&a.profile).map_err(profile_error)?;
    ...
    // `rocjitsu exec` finds it, and `rocjitsu state purge` uses it to decide
    ...
    let socket = ControlSocket::bind(&rj_core::paths::run_socket_path(&session)).await?;

    // A DBT guest is synthesised by the interposer inside the workload's
    // own process, on top of a host resolved for that process; a daemon
    // hosts one emulated machine for several processes to share, and has
    // no guest overlay in it. So a guest config runs in-process, and does
    // so without being asked to: the daemon is rocjitsu's default, and
    ...
    let run = Arc::new(Run::start(CreateSessionRequest {
        ...
        daemon: !a.in_process && !crate::names_a_guest_config(&profile),
    })?);
    ...
    // grid — so a `rocjitsu exec` from another terminal has to be told it
    ...
    eprintln!("rocjitsu: session {session}");
    ...
    // listener was still bound, so a `rocjitsu exec` in another terminal
...
/// that lists what *is* there. Where rocjitsu looked used to be added here
/// too, and is not any more — [`RocJITsuError::ProfileNotFound`] carries
/// the directory itself now, so saying it again printed the same path
/// twice in one sentence.
///
/// [`RocJITsuError::ProfileNotFound`]: rj_core::error::RocJITsuError::ProfileNotFound
fn profile_error(e: rj_core::error::RocJITsuError) -> anyhow::Error {
    if e.is_not_found() {
        anyhow::anyhow!("{e}. `rocjitsu profile list` shows what is there.")
    } else {
        anyhow::Error::new(e)
    }
}

/// Refuse a `rocjitsu run` that cannot work, before it costs a session.
///
...
/// [`rj_supervisor::build_specs`], and a missing `--workdir` by the
...
fn check_run_args(a: &RunArgs, profile: &rj_core::profile::ProfileDef) -> anyhow::Result<()> {
    ...
    // share and no daemon to share it through. Refused here rather than
    // by the backend, which would have brought a whole session up first
    // to say the same thing.
    if a.daemon && crate::names_a_guest_config(profile) {
        anyhow::bail!(
            "--daemon and a `dbt_guest` config contradict each other. A guest GPU is \
             synthesised inside the workload's own process, so it cannot be shared \
             through a daemon; run it without --daemon."
        );
    }
    Ok(())
}

/// The body of `rocjitsu run`, with teardown guaranteed by the caller.
async fn run_owned(
    run: &Arc<Run>,
    a: &RunArgs,
    ...
    // by the caller's teardown, not left for `rocjitsu state purge`.
    //
    ...
    let health = tokio::select! {
        ...
        sig = interrupts.next() => {
            ...
            // rocjitsu's own bookkeeping and reads like a fault. What the
            // user needs to know is that the interrupt arrived and that
            // the half-built session is being removed, which is what the
            // caller does next.
            eprintln!(
                "rocjitsu: {}: session {} was still starting; removing what it had created",
                signal_name(sig),
                run.id()
            );
    ...
    // No workload: the session is the point. Hold it open so other
    // terminals can `rocjitsu exec` into it, and let the caller's teardown
    // remove it when this returns. Before the borrower wait below, which
    // has nothing left to say once this has already waited for a stop.
    if a.argv.is_empty() {
        eprintln!(
            "rocjitsu: session {session} is up with no workload; \
             `rocjitsu exec --session {session} -- <command>` to use it"
        );
        eprintln!("rocjitsu: Ctrl-C or `rocjitsu session stop {session}` to tear it down");
        return hold_session(run, interrupts, serving).await;
    }
    ...
    let outcome: anyhow::Result<ExitCode> = async {
        ...
        // `run` flag: `rocjitsu exec` reaches an existing job's node and
        ...
        // accepting into the backlog, and silent. A `rocjitsu exec` in
    ...
    //
    // A `rocjitsu exec` in another terminal borrows this session, and its
    ...
    // silently — a `rocjitsu run -- sleep 5` that does not return after
...
/// caller's.
///
/// stderr, not stdout: this is rocjitsu talking about itself, and it must
/// not land in the middle of a workload's piped output.
async fn report_progress(
    mut health: tokio::sync::watch::Receiver<rj_core::session::SessionHealth>,
) {
...
/// How long a phase may go without saying anything before rocjitsu does.
...
/// [`Run::wait_ready`](rj_supervisor::Run::wait_ready) deliberately
...
/// having to guess whether rocjitsu will notice a Ctrl-C.
fn stall_notice(health: &rj_core::session::SessionHealth, elapsed: Duration) -> Option<String> {
    use rj_core::session::state;
...
///   a race.
fn progress_line(health: &rj_core::session::SessionHealth, last: Option<&str>) -> Option<String> {
    let terminal_failure =
        health.terminal && health.state.as_deref() == Some(rj_core::session::state::FAILED);
    health
        .message
        .as_deref()
...
async fn wait_for_borrowers(
    ...
    eprintln!(
        "rocjitsu: this command has finished, but {borrowers} `rocjitsu exec` \
         borrower(s) are still using session {}",
        run.id()
    );
    eprintln!("rocjitsu: waiting for them; Ctrl-C to tear the session down anyway");
    ...
    tokio::select! {
        ...
        _ = interrupts.next() => {
            ...
            eprintln!(
                "rocjitsu: tearing down session {} with {} borrower(s) still attached",
                run.id(),
                run.borrowers()
            );
        }
...
/// Hold a workload-less session open until someone ends it.
///
/// Three ways out, and they mean different things to the shell. An
/// interrupt is the user; a stop request is another `rocjitsu`; a session
/// that stops being healthy underneath us is a failure, and reporting it
/// is what stops `rocjitsu session start` from sitting on a session whose
/// emulator daemon has died.
async fn hold_session(
    ...
    tokio::select! {
        () = run.wait_stop_requested() => {
            eprintln!("rocjitsu: stopping session {}", run.id());
            Ok(ExitCode::SUCCESS)
        }
        sig = interrupts.next() => {
            eprintln!("rocjitsu: {}: tearing down session {}", signal_name(sig), run.id());
            Ok(ExitCode::from(u8::try_from(128 + sig).unwrap_or(130)))
        }
        message = unhealthy(run.watch_health()) => {
            anyhow::bail!("session {} stopped being healthy: {message}", run.id())
        }
        () = &mut *serving => unreachable!("the control socket serves until dropped"),
    }
}

/// Resolve with its reason the first time the session reports itself
/// unhealthy.
async fn unhealthy(
    mut health: tokio::sync::watch::Receiver<rj_core::session::SessionHealth>,
) -> String {
    loop {
        ...
        if health.changed().await.is_err() {
            // The session is gone, which the caller's teardown covers.
            return std::future::pending().await;
        }
    }
}

/// `rocjitsu session`: start, list and stop sessions that outlive a
/// command.
///
/// # Errors
///
/// Returns an error when the session cannot be brought up, named, or
/// reached.
pub async fn session_cmd(cmd: SessionCmd, json: bool) -> anyhow::Result<ExitCode> {
    match cmd {
        SessionCmd::Start(a) => {
            if !a.argv.is_empty() {
                anyhow::bail!(
                    "`rocjitsu session start` brings a session up with no workload. \
                     To run one, use `rocjitsu run -- {}`.",
                    a.argv.join(" ")
                );
            }
            run_cmd(a).await
        ...
        SessionCmd::List => {
            list_sessions(json).await;
            Ok(ExitCode::SUCCESS)
        }
        SessionCmd::Stop { session } => stop_session(session).await,
    }
}

/// Print the sessions whose owning run still answers.
async fn list_sessions(json: bool) {
    let live = answering_runs().await;
    if json {
        let ids: Vec<&str> = live.iter().map(SessionId::as_str).collect();
        match serde_json::to_string_pretty(&serde_json::json!({ "sessions": ids })) {
            Ok(text) => println!("{text}"),
            Err(e) => eprintln!("error: could not encode the session list: {e}"),
        }
        return;
    }
    if live.is_empty() {
        eprintln!("no sessions are up");
        return;
    }
    for id in &live {
        println!("{id}");
...
/// Ask the run owning `session` to tear it down.
async fn stop_session(session: Option<String>) -> anyhow::Result<ExitCode> {
    let session = match session {
        Some(id) => SessionId::new(id).map_err(|e| anyhow::anyhow!("not a session id: {e}"))?,
        None => sole_live_run().await?,
    };
    request_stop(&session).await?;
    eprintln!("rocjitsu: asked session {session} to stop");
    Ok(ExitCode::SUCCESS)
}

/// Send one [`Request::Stop`] and wait for the acknowledgement.
async fn request_stop(session: &SessionId) -> anyhow::Result<()> {
    use futures::{SinkExt as _, StreamExt as _};
    use rj_core::proto::{Request, Response, codec};

    let path = rj_core::paths::run_socket_path(session);
    let stream = tokio::net::UnixStream::connect(&path).await.map_err(|e| {
        anyhow::anyhow!(
            "no `rocjitsu run` is serving session {session} ({e}). \
             A session exists only while the `rocjitsu run` that created it is alive."
        )
    })?;
    let mut framed = tokio_util::codec::Framed::new(stream, codec());
    framed
        .send(serde_json::to_vec(&Request::Stop)?.into())
        .await?;

    let answered = tokio::time::timeout(DESCRIBE_TIMEOUT, framed.next()).await;
    match answered {
        Ok(Some(frame)) => match serde_json::from_slice::<Response>(&frame?)? {
            Response::Stopping => Ok(()),
            Response::Error(message) => anyhow::bail!("{message}"),
            Response::Description(_) => {
                anyhow::bail!("session {session} answered a stop with a description")
            }
        },
        // A run old enough not to know `Stop` closes on the unknown
        // request rather than answering it, and so does one that took the
        // request and exited before its reply got out. Neither is worth
        // failing over: the caller asked for the session to end.
        Ok(None) => Ok(()),
...
/// `rocjitsu exec`: run a command inside a session someone else owns.
pub async fn exec_cmd(a: ExecArgsCli) -> anyhow::Result<ExitCode> {
    ...
    // a long-lived run outlives many `rocjitsu exec` invocations. A start
    ...
    // `rocjitsu exec` spawns its ranks as its own children in its own
    // terminal, so the streams to ask about are this process's; see
    // [`rj_supervisor::CallerStreams`].
    let specs =
        rj_supervisor::build_specs(&desc, &def, &id, rj_supervisor::CallerStreams::probe())?;
    ...
    let closing = async {
        lease.closed().await;
        eprintln!("rocjitsu: the run owning session {session} is shutting down");
    };
...
fn exec_def(
    ...
    //
    // A `rocjitsu:` line on stderr, not a `tracing` record: this is rocjitsu
    // talking to the person who typed the flag, and every other thing
    // rocjitsu says to them looks the same. As a log line it arrived with a
    ...
    for key in crate::rocjitsu_owned_env(env.keys()) {
        eprintln!(
            "rocjitsu: --env {key}=… is ignored: rocjitsu sets {key} itself on every process, \
...
async fn supervise_locally(
    exec: Arc<Exec>,
    output: mpsc::Receiver<rj_supervisor::OutputChunk>,
    interrupts: &mut Interrupts,
    relays: &mut Relays,
) -> anyhow::Result<ExitCode> {
    ...
    let printer = tokio::spawn(rj_supervisor::output::print_labelled(output));
    ...
    // catching it to checkpoint, a shell with `trap ''` — and rocjitsu
    ...
    let interrupted = async {
        ...
        eprintln!(
            "rocjitsu: {}: asked the workload to stop; \
        ...
        eprintln!("rocjitsu: not waiting any longer; stopping the workload");
    };
    ...
    let relaying = async {
        loop {
            ...
            eprintln!("rocjitsu: {}: forwarded to the workload", signal_name(sig));
            exec.signal(sig).await.ok();
        }
    };
    tokio::pin!(relaying);
    ...
    tokio::select! {
        ...
        () = interrupted => {
            ...
            tokio::select! {
                () = exec.terminate() => {}
                _ = interrupts.next() => {
                    eprintln!("rocjitsu: killing the workload outright");
                    exec.kill_now();
...
/// because that is how the line reads: `rocjitsu: interrupted: …`. The two
...
/// `rocjitsu run --nproc-per-node 2 -- ./job < input` reads an immediate
/// EOF in every rank, which from the outside is indistinguishable from a
/// broken pipe, a bad path, or rocjitsu dropping the data.
///
...
/// what they piped in is being discarded is rocjitsu describing something
/// that did not happen.
fn warn_if_stdin_is_dropped(exec: &Arc<Exec>) {
    ...
    eprintln!(
        "rocjitsu: this job runs several processes, so none of them is given stdin — \
         ...
         (`rocjitsu exec --node N`) if it has to be read."
    );
}
...
fn stdin_source(fd: std::os::fd::RawFd) -> StdinSource {
    ...
    let Ok(stat) = nix::sys::stat::fstat(fd) else {
        // No such descriptor: stdin was closed before rocjitsu started.
...
/// rocjitsu unreachable, because the tty delivers `SIGINT` to the
...
/// interactive or not, and a `rocjitsu run -- ./job` whose job ignores
/// `SIGINT` became a run that could not be interrupted from its own
/// terminal: Ctrl-C went to the job, the job ignored it, rocjitsu never
/// woke, and nothing was printed. The second interrupt the help promises
/// "stops waiting for it" never arrived, because rocjitsu was not listening
...
/// rocjitsu stays the foreground group and the documented two-interrupt
...
/// being stopped is the ask. RocJITsu hands the terminal over, continues
/// it, and from then on Ctrl-C belongs to the workload, which for an
/// interactive program is exactly right and is what `rocjitsu run -- bash`
/// needs.
    ...
    loop {
        match process_state(pid) {
            ...
            Some(b'T' | b't') => {
                ...
                // which is a hang with no way out; rocjitsu may have lost
...
/// made again: a signal whose default disposition kills rocjitsu outright
...
///   handling it gives up: a core dump of rocjitsu tells nobody anything,
...
/// they end rocjitsu where it stands — and fail the second: nobody sends
...
/// precisely why `rocjitsu cleanup` exists.
///
...
/// multi-minute image pull killed rocjitsu outright, with no teardown,
...
/// rocjitsu handles rather than something that happens to it.
struct Interrupts {
...
/// rocjitsu draws no conclusion from them, and no number of them ends a
/// run. See [`Relays`].
...
/// Arm every signal rocjitsu handles, in both groups.
///
/// # Errors
///
...
impl Interrupts {
    ...
    /// rocjitsu reports and the signal it forwards to the workload are
...
/// node and then a network. A `rocjitsu exec` arriving in either sat in
...
/// while this process is still using it. `rocjitsu exec` starts its
...
async fn attach_inner(
    ...
    let stream = tokio::net::UnixStream::connect(&path).await.map_err(|e| {
        anyhow::anyhow!(
            "no `rocjitsu run` is serving session {session} ({e}). \
             A session exists only while the `rocjitsu run` that created \
             it is alive."
        )
    })?;
    let mut framed = tokio_util::codec::Framed::new(stream, codec());
    ...
    match serde_json::from_slice::<Response>(&frame?)? {
        ...
        Response::Error(message) => anyhow::bail!("{message}"),
        Response::Stopping => {
            anyhow::bail!("session {session} answered an attach with a stop acknowledgement")
        }
    }
}
...
/// leaves its socket behind — the documented, expected leak `rocjitsu
...
async fn sole_live_run() -> anyhow::Result<SessionId> {
    ...
    match live.len() {
        1 => Ok(live[0].clone()),
        0 => anyhow::bail!(
            "no `rocjitsu run` is running. Start one in another terminal, \
        ...
        _ => {
            ...
            anyhow::bail!(
                "several runs are live ({}); name the one you mean, \
                 e.g. `rocjitsu exec --session {} -- <command>`",
                names.join(", "),
                names[0]
            )
        }
...
/// `SIGKILL`ed. That is the right answer for `rocjitsu exec`, which wants to
...
/// [`ControlSocket::bind`](rj_supervisor::rpc::ControlSocket::bind)
...
pub async fn answering_runs() -> Vec<SessionId> {
    ...
    for id in live_runs() {
        let path = rj_core::paths::run_socket_path(&id);
...
/// at all, because `rocjitsu exec --session <id>` should say that *that*
/// session is gone rather than that it does not exist.
fn live_runs() -> Vec<SessionId> {
    let Ok(entries) = std::fs::read_dir(rj_core::paths::run_socket_root()) else {
        return Vec::new();
    };
...
///
/// Every workload process leads its own process group, so that rocjitsu can
...
/// interactive `bash` started by `rocjitsu run -- bash` would hang on the
...
/// to rocjitsu, and a shell's own job control works inside it.
///
/// # Why it is lent rather than given
///
/// "Delivered to the workload rather than to rocjitsu" is the cost as well
...
/// rocjitsu stays interruptible from the window it was started in.
///
/// # SIGTTOU
///
/// Giving the terminal *back* is itself a background write to it, which
/// earns `SIGTTOU` and would stop rocjitsu exactly when it is trying to
...
/// leave rocjitsu to be stopped by `SIGTTOU` in the middle of teardown.
struct TerminalHandoff {
    /// The process group to restore, i.e. rocjitsu's own.
...
impl TerminalHandoff {
    /// Whether rocjitsu has a terminal that is its to lend.
    ///
    ...
    /// a pipe, a CI runner, `< /dev/null` — or with rocjitsu already in the
    ...
    /// terminal (a pipe, a CI runner, `< /dev/null`), or rocjitsu is not
    ...
    fn give_to(pid: u32) -> Option<Self> {
        ...
        // Only the foreground group may hand the terminal on. If rocjitsu
...
mod tests {
    ...
    fn exec_carries_the_node_it_was_given() {
        // The regression that made `rocjitsu exec --node 2` a no-op: the
    ...
    #[test]
    fn a_missing_profile_says_where_rocjitsu_looked_exactly_once() {
        ...
        // found: ghost (rocjitsu looked in /…/profile). rocjitsu looked in
        // /…/profile; `rocjitsu profile list` shows what is there."
        use rj_core::store::DocKind;
        ...
        let e = format!(
            "{:#}",
            profile_error(rj_core::error::RocJITsuError::not_found(
                DocKind::Profile,
                "ghost"
            ))
        );
        rj_core::paths::clear_test_root();
        assert_eq!(
            e.matches(&dir).count(),
            1,
        ...
        assert!(e.contains("rocjitsu profile list"), "{e}");

        // Anything that is not a missing profile is passed through
        // untouched; there is no list to point at.
        let other = format!(
            "{:#}",
            profile_error(rj_core::error::RocJITsuError::other("disk on fire"))
        );
    ...
    /// hold [`rj_core::paths::test_env_lock`] — a `MutexGuard` across
    ...
    fn phase(state: &str, message: &str) -> rj_core::session::SessionHealth {
        rj_core::session::SessionHealth::phase(false, state, Some(message.to_string()))
    }
    ...
    fn a_fatal_reason_is_reported_once_and_by_the_error_path() {
        ...
        let failed = rj_core::session::SessionHealth::failed(reason);
        assert!(
            failed.terminal && failed.state.as_deref() == Some(rj_core::session::state::FAILED),
    ...
    fn a_pull_that_has_gone_quiet_says_how_long_it_has_been_quiet_for() {
        ...
        // and rocjitsu suspends the readiness clock for it on purpose — so
        ...
        assert_eq!(
            stall_notice(
                &rj_core::session::SessionHealth::phase(true, "ready", None),
                STALL_NOTICE
            ),
            None
        );
    ...
    fn a_piped_stdin_that_no_rank_will_read_is_reported() {
        ...
        // pipe: `rocjitsu run --nproc-per-node 2 -- ./job < input` gives
    ...
    fn nothing_is_being_discarded_when_nothing_was_piped_in() {
        ...
        // A descriptor that is not open at all — `rocjitsu run … 0<&-`.
    ...
    async fn every_signal_that_means_stop_is_handled_rather_than_fatal() {
        ...
        // intent. Unhandled, the default disposition ends rocjitsu where it
    ...
    async fn the_application_defined_signals_are_handled_but_not_interrupts() {
        ...
        //
        // Handled: unhandled, `SIGUSR1`'s default disposition ends rocjitsu
    ...
    async fn closing_the_terminal_is_an_interrupt_and_not_a_death() {
        ...
        // Unhandled, its default disposition ends rocjitsu outright: exit
    ...
    fn a_run_that_was_killed_is_not_a_candidate_for_exec() {
        ...
        // documented, expected, and what `rocjitsu cleanup` exists for. So
        // counting files rather than answers meant a single `kill -9`
        // broke `rocjitsu exec` with no `--session`: with a corpse beside a
        // live run it reported "several runs are live" and offered the
        // dead one as a candidate.
        let _lock = rj_core::paths::test_env_lock();
        let root = tempfile::tempdir().unwrap();
        rj_core::paths::set_test_root(root.path());
        std::fs::create_dir_all(rj_core::paths::run_socket_root()).unwrap();

        let dead = SessionId::new("s-dead").unwrap();
        let dead_path = rj_core::paths::run_socket_path(&dead);
        ...
        let _listener =
            std::os::unix::net::UnixListener::bind(rj_core::paths::run_socket_path(&alive))
                .unwrap();
    ...
    fn a_corpse_alone_is_not_a_run_to_exec_into() {
        ...
        let dead = SessionId::new("s-dead").unwrap();
        drop(
            std::os::unix::net::UnixListener::bind(rj_core::paths::run_socket_path(&dead)).unwrap(),
        );
        ...
        assert!(e.contains("no `rocjitsu run` is running"), "{e}");
    }
}
