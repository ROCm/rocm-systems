...
//!
//! `rocjitsu run -- ./app` and `rocjitsu -- ./app` are the same invocation in
//! two spellings, and both have the same failure mode: a rocjitsu flag the
//! user misremembered is not a rocjitsu flag, so it becomes the workload.
//! `rocjitsu run --nodes 2 -- ./app` used to bring a whole emulated machine
...
//! unknown rocjitsu flag start the positional. By the time clap hands back
//! an `argv`, these two are identical:
//!
//! ```text
//! rocjitsu run --nodes 2 ./app      # a typo: `--nodes` became the command
//! rocjitsu run -- --nodes 2 ./app   # deliberate: a program called `--nodes`
//! ```
//!
...
//! `rocjitsu run --nodes 2 -- ./app` an error and left `rocjitsu run --nodes
...
//! `rocjitsu run --nodes 2 -- ./app` *has* one and was still wrong, because
//! the mistake is in front of it. And `rocjitsu run ./app` is a spelling
...
enum Cluster {
    /// Every letter is one rocjitsu takes and none of them takes a value:
    /// `-v`, `-vvv`.
    AllFlags,
    ...
    ValueNext,
    /// A letter rocjitsu does not take, before any value-taking one.
    Unknown,
}
...
impl AcceptedFlags {
    ...
    /// list would reject `rocjitsu run --json -- ./app`.
    #[must_use]
...
/// Where a scan of rocjitsu's own flags should stop.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Span {
    /// The invocation has a `--`, so everything given to this scan is
    /// rocjitsu's and a flag token anywhere in it that rocjitsu does not take
    /// is a mistake.
    BeforeSeparator,
    /// The invocation has no `--`, so rocjitsu's flags run until the first
    /// token that is not one — that token is the command, and everything
    /// after it belongs to the workload.
    ///
    /// `rocjitsu run ./app --verbose` passes `--verbose` to `./app`, which
    /// is what the user meant and what rocjitsu has always done. The scan
    ...
    while let Some(token) = span.get(i) {
        if !is_flag_token(token) {
            match stop {
                ...
                // A value, or a positional rocjitsu takes. Either way not
                // a mistyped flag.
...
pub fn unknown_flag_error(flag: &str, known: &AcceptedFlags, usage: &str, help: &str) -> String {
    let mut msg = format!(
        "error: unexpected argument '{flag}' found\n\n\
         Everything before `--` is read as rocjitsu's own flags and everything \
...
mod tests {
    ...
    #[test]
    fn a_flag_rocjitsu_does_not_take_is_found_before_a_separator() {
        let flags = run_flags();
        assert_eq!(
    ...
    #[test]
    fn a_flag_rocjitsu_does_not_take_is_found_without_a_separator() {
        // The case the post-parse check could not see at all: no `--`
        // anywhere, so `--nodes` became the command and rocjitsu brought a
    ...
    #[test]
    fn a_workloads_own_flags_are_not_rocjitsus_business() {
        // `rocjitsu run ./app --verbose` is the shape that works today.
    ...
    #[test]
    fn the_spellings_rocjitsu_really_does_accept_are_accepted() {
        let flags = run_flags();
        for good in [
            "--profile",
            "--profile=mi350x",
    ...
    fn bundled_short_flags_and_root_globals_are_accepted() {
        ...
        // subcommand's own list alone would reject `rocjitsu run --json --
        // ./app`.
        let root = clap::Command::new("rocjitsu")
            .arg(
                clap::Arg::new("json")
                    .long("json")
                    .global(true)
        ...
        assert!(!flags.accepts("-vqv"), "`q` is not a flag rocjitsu has");
    ...
    fn a_bundle_ending_in_a_valued_short_is_accepted_in_both_forms() {
        ...
        assert!(
            ...
            "`q` is not a flag rocjitsu has"
        );

        // Clap's own verdict on the same tokens, so this cannot drift
        // from the parser it is guarding.
        for argv in [
            vec!["rocjitsu", "run", "-vokey=value", "--", "./app"],
            vec!["rocjitsu", "run", "-vo", "key=value", "--", "./app"],
        ] {
            assert!(
