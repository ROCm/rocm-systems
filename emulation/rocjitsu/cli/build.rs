...
//! every crate rocjitsu links, with its version and SPDX license. It is
//! written twice: `OUT_DIR/about.txt` for a human and
//! `OUT_DIR/about.json` for `--json`. Both are embedded into the binary
//! via `include_str!`, so `rocjitsu about` needs no files at runtime and
...
fn main() {
    ...
    let (text, json) = generate_manifest().unwrap_or_else(|e| {
        println!("cargo:warning=rocjitsu about: could not build dependency manifest: {e}");
        (
...
fn generate_manifest() -> Result<(String, String), String> {
    ...
    // this workspace's own crates (they're first-party).
    let mut entries: Vec<(String, String, String)> = Vec::new();
    for pkg in packages {
        let name = pkg.get("name").and_then(|v| v.as_str()).unwrap_or("");
        if name.is_empty()
            || name == "rocjitsu"
            || name.starts_with("rj_")
            || name == "rocjitsu_sys"
        {
            continue;
        }
    ...
    out.push_str(&format!(
        "rocjitsu links {} third-party crate(s):\n\n",
        entries.len()
    ));
