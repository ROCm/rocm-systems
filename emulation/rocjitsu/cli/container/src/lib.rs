...
//! containerised parts of a rocjitsu session.
//!
//! The *static* configuration ([`rj_core::profile::ContainerizedDef`],
//! [`rj_core::profile::FileMount`]) and the *runtime* record
//! ([`rj_core::container::ContainerState`], naming + provider
//! resolution, and dependency-free [`teardown`](rj_core::container::teardown))
//! live in `rj_core`. This crate adds the imperative orchestration:
...
use rj_core::container::{ContainerState, NodeContainer};
use rj_core::profile::{ContainerizedDef, FileMount, PortMapping};
...
/// `rocjitsu host --rank N` here, which meant every container had a second
/// rocjitsu process inside it polling a bind-mounted directory.
...
/// without the wait rocjitsu reports that in the worst possible way. The
...
/// `rocjitsu run` that dies — cleanly, by `SIGKILL`, or with its terminal —
...
fn drain_lines(reader: impl std::io::Read + Send + 'static, sink: LineRing, draining: &Draining) {
    ...
    std::thread::spawn(move || {
        for line in BufReader::new(reader)
            ...
            // reads as though rocjitsu had lost something.
...
/// different fixes behind what rocjitsu used to report identically as
/// "stopped immediately".
...
/// number is the whole point of the message rocjitsu writes instead, and a
/// remedy that cannot name the port is not much better than the engine's.
///
/// A message that matches but names no port is not a match: rocjitsu would
...
/// nothing else to offer, dropped from one where rocjitsu supplies the
...
pub enum ContainerError {
    ...
    #[error(
        "no container provider found; install podman or docker, or set ROCJITSU_CONTAINER_PROVIDER"
    )]
    NoProvider,
    ...
    NotRunning {
        ...
        name: String,
        /// How long rocjitsu waited for it.
    ...
    /// and 127 from an image rocjitsu only ever asked to run
    ...
    #[error("container `{name}` stopped immediately (after {waited:?}, {}){}",
        ...
        if .output.is_empty() {
            format!(
                "; it said nothing, and the only thing rocjitsu asked it to run was `{}`, \
    ...
    /// flag rocjitsu passed on their behalf, about a conflict with a
    /// process that has nothing to do with this session.
    #[error(
        "container `{name}` could not start: host port {port} is already in use on this \
         machine, so the container could not publish onto it. Nothing of rocjitsu's is holding \
    ...
    ///
    /// Checked by rocjitsu rather than left to the provider, because the
    ...
    Mount {
        ...
        /// (`HOST:CONTAINER[:ro]`).
        spec: String,
        /// The host path rocjitsu resolved it to.
        path: String,
        /// What is wrong with that path, and what to do about it.
        problem: String,
    },

    /// A bind mount names a container path rocjitsu has already spoken for.
    ///
    /// Every containerised session mounts rocjitsu's own binary, config
    /// directory, session scratch and emulator libraries under
    /// [`rj_core::container::CONTAINER_ROCJITSU_DIR`]. A user mount at
    /// or above that path is laid over them, and the engine resolves the
    /// overlap by creating rocjitsu's destinations *inside the user's host
    ...
    #[error(
        "--mount {spec}: the container path `{container_path}` is at or above `{reserved}`, \
         which is where rocjitsu bind-mounts its own binary, configuration and session scratch \
         inside every node container. The two overlap, and what the container creates under \
         rocjitsu's paths lands in `{host_path}` on the host instead — root-owned `bin`, `config`, \
    ...
    ReservedMount {
        ...
        /// The tree rocjitsu keeps for itself.
        reserved: &'static str,
    },
    ...
    /// The configured provider is not a container engine rocjitsu can
    /// drive.
    ///
    /// Checked before the session is built, because everything rocjitsu
...
/// [`Cancel`] is how the owner of a bring-up — the `rocjitsu run` whose
...
/// A `Stdio` that writes to rocjitsu's own stderr.
///
...
/// rocjitsu's stdout, which belongs to the workload alone — the promise
/// that `rocjitsu run … > out.txt` is byte-exact is only worth as much as
...
/// given.
///
/// The *container* side is refused for one reason, which is that rocjitsu
/// mounts things there too: see
/// [`ContainerError::ReservedMount`]. Two shapes of *host* path are
/// trouble, and each is trouble differently on the two engines rocjitsu
/// drives:
///
...
///   the volume outlives the session, the run, and `rocjitsu cleanup`,
...
///   `statfs …: no such file or directory`. RocJITsu rejects it on both,
...
fn resolve_mount(mount: &FileMount) -> Result<FileMount> {
    ...
    //
    // RocJITsu's own mounts are all strictly below the reserved directory
    ...
    // the same list.
    if rj_core::container::covers_rocjitsu_dir(&mount.container_path) {
        return Err(ContainerError::ReservedMount {
            spec: mount.to_volume_arg(),
            host_path: mount.host_path.clone(),
            container_path: mount.container_path.clone(),
            reserved: rj_core::container::CONTAINER_ROCJITSU_DIR,
        });
    }
    ...
    match host.try_exists() {
        ...
        Ok(false) => Err(refuse(
            &host,
            "does not exist. Create it before the run, or correct the path — rocjitsu will \
...
/// the only one rocjitsu can be sure about: everything else is the
    ...
    if image.trim().is_empty() {
        return Err(ContainerError::Image {
            ...
            problem: "a containerised session runs every node in an image, and this names none. \
                      ...
                      the profile one with `rocjitsu profile create … --image <reference>`."
                .to_string(),
        });
    }
    Ok(())
...
/// engine.
///
/// Everything rocjitsu does to a container is a `Command` built around this
...
/// it on. RocJITsu deliberately does not check the *name* — the provider is
...
fn check_provider(provider: &str) -> Result<()> {
    ...
    if rj_core::container::provider_binary(provider).is_none() {
        return Err(refuse(format!(
            "{}. rocjitsu drives `podman` or `docker`; name one of those, or the path to a \
             container engine — or name none at all and rocjitsu will use whichever is installed",
    ...
    let probed = rj_core::container::retrying_etxtbsy(|| {
    ...
    match probed {
        ...
        Ok(status) => Err(refuse(format!(
            "`{provider} --version` failed ({}), so this does not look like a container engine. \
             rocjitsu speaks the podman/docker command line and nothing else",
            exit_phrase(&status.code(), &status.signal())
        ))),
        Err(e) => Err(refuse(format!(
            "cannot be run: {e}. It has to be an executable rocjitsu can spawn"
        ))),
    }
}
...
pub enum BringUpPhase {
    ...
    /// `node 1/1 (rocjitsu-s-node-0) started failed: …` about a container
...
pub struct NodeSpec<'a> {
    ...
    /// it — but part of its *provenance*: the provider client rocjitsu
    /// spawns is marked with it, so a client stranded by a `SIGKILL`ed
    /// run can be found and reaped by [`rj_core::reclaim`] later.
    pub session: &'a rj_core::session::SessionId,
    /// Image to run.
    pub image: &'a str,
    ...
    /// reclamation can prove it is rocjitsu's before removing it.
    pub labels: &'a [(String, String)],
}

/// A resolved container provider plus the operations rocjitsu performs on
...
impl Engine {
    ...
    /// "explicit > `ROCJITSU_CONTAINER_PROVIDER` > autodetect (podman then
    /// docker)" policy — see
    /// [`resolve_provider`](rj_core::container::resolve_provider) for
    ...
    pub fn resolve(def: &ContainerizedDef) -> Result<Self> {
        ...
        let provider = rj_core::container::resolve_provider(def.provider.as_deref())
    ...
    ///
    /// `command` is the container's foreground process (PID 1). RocJITsu
    /// runs each node's own `rocjitsu host --session <id> --rank <n>` here
    ...
    /// with `rocjitsu host …` tacked on as arguments instead of running
    /// rocjitsu.
    ///
    ...
    pub fn run_argv(
        ...
        let &NodeSpec {
            // The session marks the *client process* rocjitsu spawns, not
        ...
        let mut argv = vec![
            ...
            // and rocjitsu owns it as a child process, so the container's
            // lifetime is bounded by the `rocjitsu run` that asked for it
        ...
        // anything, so a user's own `rocjitsu-s1-node-0` is safe.
        for (k, v) in labels {
        ...
        // The container's foreground process. RocJITsu hosts the node from
        // inside the container, so this is normally `rocjitsu host ...`.
        // The first element overrides the image ENTRYPOINT (so it runs
        // rocjitsu rather than the image's own entrypoint); the rest become
    ...
    /// inside the container. RocJITsu still allocates none of its own, and
    ...
    /// most needs to see. What they see depends on where rocjitsu's stderr
    /// goes:
    ///
    ...
    ///
    /// # Provider chatter never lands on rocjitsu's stdout
    ///
    /// Not even the provider's *own* stdout, which is why the pass-
    /// through branch duplicates rocjitsu's stderr onto the child's stdout
    /// rather than inheriting. rocjitsu's stdout belongs to the workload
    /// and to nothing else: `rocjitsu run … > out.txt` from a terminal has
    ...
    ///
    /// Used to realise profile [hacks](rj_core::profile::Hack): a
    ...
    ///
    /// When rocjitsu's stderr is a terminal the lines are echoed there as
    ...
    /// The image is labelled as rocjitsu's, like every container and
    /// network rocjitsu creates. A derived image is host state that
    ...
    /// itself. See
    /// [`image_labels`](rj_core::container::image_labels) for why
    ...
    pub fn build_image(&self, tag: &str, dockerfile: &str) -> Result<()> {
        ...
        for (k, v) in rj_core::container::image_labels() {
    ...
    /// output the user asked for; discarding is what rocjitsu used to do,
    /// and it meant a `podman run` that refused instantly — a bound
    /// port, a device that does not exist, an entrypoint the image
    /// cannot run — said so into nothing, and rocjitsu reported only that
    ...
    /// [`rj_core::container::ENV_SESSION`] makes on its behalf.
    ...
    /// it was — which is what `rocjitsu cleanup` reads.
    ///
    ...
    pub fn launch_node(
        ...
        let child = spawn_retrying_etxtbsy(|| {
            Command::new(&self.provider)
                .args(&argv)
                .env(rj_core::container::ENV_SESSION, spec.session.as_str())
                .env(
    ...
    /// Refusing on an unanswered question would mean rocjitsu inventing a
    ...
    /// (rocjitsu injects `ROCJITSU_RANK`/`ROCJITSU_HEAD_ADDR`/`ROCJITSU_HEAD_PORT`
    ...
    pub fn bring_up<F, P>(
        &self,
        session: &rj_core::session::SessionId,
    ...
    {
        let network = rj_core::container::network_name(session);
        // Every resource this call creates carries rocjitsu's ownership
        ...
        let labels = rj_core::container::owner_labels(session);
        ...
        // [`rj_core::container::teardown`]: a container's name is
        // derived from the session id and is not proof that rocjitsu
        ...
        let rollback = |engine: &Engine, nodes: &[NodeContainer], clients: &mut Vec<NodeClient>| {
            ...
            rj_core::container::teardown(&state);
        };
        ...
        for rank in 0..node_count {
            let name = rj_core::container::container_name(session, rank);
    ...
    /// Blocking on that one hung a `rocjitsu run` before the pull, with no
...
///
/// Delegates to [`rj_core::container::retrying_etxtbsy`] rather than
...
/// bring-up, while the identical call shape in `rj_core` retried and
...
fn spawn_retrying_etxtbsy<T>(run: impl FnMut() -> std::io::Result<T>) -> std::io::Result<T> {
    rj_core::container::retrying_etxtbsy(run)
}
...
mod tests {
    ...
    fn session() -> &'static rj_core::session::SessionId {
        static SESSION: std::sync::OnceLock<rj_core::session::SessionId> =
            std::sync::OnceLock::new();
        SESSION.get_or_init(|| rj_core::session::SessionId::new("s").unwrap())
    }

    /// The ownership labels bring-up stamps on every resource.
    fn labels() -> Vec<(String, String)> {
        rj_core::container::owner_labels(session())
    }
    ...
    fn mock_provider(dir: &Path, log: &Path) -> std::path::PathBuf {
        ...
        let script = format!(
            "#!/bin/sh\necho \"$@\" >> {log}\n\
             if [ \"$1\" = run ]; then \
             echo \"client-env ROCJITSU_SESSION=${{ROCJITSU_SESSION-unset}} \
             ROCJITSU_CLI_RUNTIME_DIR=${{ROCJITSU_CLI_RUNTIME_DIR-unset}}\" >> {log}; \
    ...
    fn run_argv_includes_network_env_and_mounts() {
        let env = vec![
            ("ROCJITSU_RANK".to_string(), "0".to_string()),
            ("ROCJITSU_HEAD_PORT".to_string(), "5000".to_string()),
        ];
        ...
        let command = vec![
            "/mnt/rocjitsu/bin/rocjitsu".to_string(),
            "host".to_string(),
            "--session".to_string(),
            "s".to_string(),
            "--rank".to_string(),
        ...
        assert!(
            joined.starts_with("run --rm --name rocjitsu-s-node-0 --hostname rocjitsu-s-node-0")
        );
        assert!(joined.contains("--security-opt seccomp=unconfined"));
        assert!(joined.contains("--group-add keep-groups"));
        assert!(joined.contains("--network rocjitsu-s"));
        assert!(joined.contains("-e ROCJITSU_RANK=0"));
        assert!(joined.contains("-e ROCJITSU_HEAD_PORT=5000"));
        ...
        assert!(joined.contains("--entrypoint /mnt/rocjitsu/bin/rocjitsu"));
    ...
    fn run_argv_is_pinned_element_by_element() {
        ...
        let env = vec![("ROCJITSU_RANK".to_string(), "0".to_string())];
        let labels = vec![("rocjitsu.owner".to_string(), "rocjitsu".to_string())];
        let command = vec!["/bin/rocjitsu".to_string(), "host".to_string()];
    ...
    fn ensure_network_creates_when_absent() {
        ...
        engine.ensure_network("rocjitsu-s", &labels()).unwrap();
        let recorded = std::fs::read_to_string(&log).unwrap();
        assert!(
            recorded.contains("network inspect rocjitsu-s"),
            "{recorded:?}"
        );
        // Labelled, so teardown can prove the network is rocjitsu's before
        // removing it and orphan reclamation can find it later.
        assert!(
            recorded.contains("network create --label rocjitsu.owner=rocjitsu")
                && recorded.contains("--label rocjitsu.session=s")
                && recorded.contains(" rocjitsu-s"),
            "{recorded:?}"
        );
    }
    ...
    fn launch_node_owns_the_client_and_never_detaches() {
        ...
        assert_eq!(client.name, "rocjitsu-s-node-0");
        ...
        assert!(
            recorded.contains("run --rm --name rocjitsu-s-node-0"),
    ...
    fn the_provider_client_carries_the_session_that_owns_it() {
        ...
        // which is exactly what `rocjitsu cleanup` reads — and the marker
        // is documented as being set on "every container provider client
        // rocjitsu spawns", so an unmarked client makes that a lie and the
        ...
        let _client = engine
            .launch_node("rocjitsu-s-node-0", &bare_spec(), &[], 0)
            .unwrap();

        let recorded = log_containing(&log, "client-env");
        assert!(
            recorded.contains(&format!(
                "client-env ROCJITSU_SESSION={} ROCJITSU_CLI_RUNTIME_DIR={}",
                session().as_str(),
                rj_core::container::owning_runtime()
            )),
    ...
    fn a_node_that_dies_just_after_reporting_up_fails_the_bring_up() {
        ...
        //
        // The container's last words here go to stdout, which rocjitsu
        ...
        let provider = scripted_provider(
            ...
            "dying-provider.sh",
            "case \"$1\" in\n\
             run) sleep 0.1; echo 'Error relocating /mnt/rocjitsu/lib/librocjitsu.so: \
        ...
        assert!(
            message.contains("rocjitsu-s-node-0")
    ...
    fn a_container_that_dies_mid_session_can_say_why() {
        ...
        let provider = scripted_provider(
            ...
            "oom-provider.sh",
            "case \"$1\" in\n\
             run) echo 'container rocjitsu-s-node-0 was killed by the OOM killer' >&2; \
             exit 137 ;;\n\
             *) exit 0 ;;\n\
             esac\n",
        );
        ...
        assert!(
            report.contains("rocjitsu-s-node-0")
        ...
        let mut client = healthy
            .launch_node("rocjitsu-s-node-1", &bare_spec(), &[], 1)
            .unwrap();
    ...
    fn bring_up_refuses_a_mount_whose_host_path_does_not_exist() {
        ...
        // the host and starts the container; podman refuses. RocJITsu
        ...
        assert!(
            ...
            "a missing host path must be rocjitsu's refusal, not the provider's: {message}"
        );
        assert!(
    ...
    fn bring_up_makes_a_relative_mount_absolute() {
        ...
        // and leave it behind afterwards, where `rocjitsu cleanup` (which
    ...
    fn a_wedged_network_inspect_does_not_become_a_network() {
        ...
        assert!(
            err.to_string().contains("looking for network rocjitsu-s"),
            "the error must name what was interrupted: {err}"
        );
        assert!(
            waited < std::time::Duration::from_secs(5),
    ...
    fn bring_up_pulls_creates_network_and_launches_each_node() {
        ...
        let session = rj_core::session::SessionId::new("s").unwrap();
        ...
        let (state, clients) = engine
            .bring_up(
                ...
                2,
                6000,
                |rank| vec![("ROCJITSU_RANK".to_string(), rank.to_string())],
                |phase| phases.push(phase),
            )
            .unwrap();

        assert_eq!(state.image, "img:latest");
        assert_eq!(state.network.as_deref(), Some("rocjitsu-s"));
        assert_eq!(state.head_port, 6000);
        assert_eq!(state.nodes.len(), 2);
        assert_eq!(state.nodes[0].name, "rocjitsu-s-node-0");
        assert_eq!(state.nodes[1].name, "rocjitsu-s-node-1");
        ...
        assert_eq!(clients[0].name, "rocjitsu-s-node-0");
        assert_eq!(clients[1].rank, 1);
        assert_eq!(clients[1].name, "rocjitsu-s-node-1");
        ...
        assert!(
            recorded.contains("run --rm --name rocjitsu-s-node-1"),
            "{recorded:?}"
        );
        assert!(recorded.contains("-e ROCJITSU_RANK=0"), "{recorded:?}");
        assert!(recorded.contains("-e ROCJITSU_RANK=1"), "{recorded:?}");
        ...
        assert_eq!(
            ...
            vec![
                ...
                BringUpPhase::CreatingNetwork {
                    network: "rocjitsu-s".to_string()
                },
                BringUpPhase::LaunchingNode {
                    rank: 0,
                    total: 2,
                    name: "rocjitsu-s-node-0".to_string()
                },
                BringUpPhase::NodeStarted {
                    rank: 0,
                    total: 2,
                ...
                BringUpPhase::LaunchingNode {
                    rank: 1,
                    total: 2,
                    name: "rocjitsu-s-node-1".to_string()
                },
                BringUpPhase::NodeStarted {
                    rank: 1,
                    total: 2,
    ...
    #[test]
    fn bring_up_refuses_a_mount_laid_over_rocjitsus_own_directory() {
        // The user's mount and rocjitsu's own overlap, and the engine
        // resolves the overlap by creating rocjitsu's destinations inside
        ...
        let err = bring_up_one(
            ...
            &def_with(
                &provider,
                vec![mount(&format!("{}:/mnt/rocjitsu", host.display()))],
            ),
        )
        .unwrap_err();
        ...
        assert!(
            ...
            "a mount over rocjitsu's own tree must be refused: {message}"
        );
        // Both paths, because the collision is between them and the user
        // knows only one of them exists.
        assert!(
            message.contains(&host.display().to_string()) && message.contains("/mnt/rocjitsu"),
    ...
    #[test]
    fn an_ancestor_of_rocjitsus_directory_is_refused_too() {
    ...
    #[test]
    fn the_mounts_rocjitsu_adds_itself_are_not_refused() {
        // Everything rocjitsu bind-mounts lives *below* the reserved
        ...
        bring_up_one(
            ...
            &def_with(
                &provider,
                vec![
                    mount(&format!("{}:/mnt/rocjitsu/runtime", scratch.display())),
                    mount(&format!("{}:/mnt/rocjitsu/config:ro", scratch.display())),
                ],
            ),
        )
        .unwrap();
    ...
    fn a_provider_that_is_not_a_container_engine_is_refused() {
        ...
        assert!(
            ...
            "the error must name what was configured and what rocjitsu can drive: {message}"
        );
        ...
        assert!(
            ...
            "the error must say what rocjitsu asked it: {message}"
        );
    ...
    #[test]
    fn a_derived_image_is_labelled_as_rocjitsus() {
        // Nothing else records that a `rocjitsu-hack-…` image exists: it is
        ...
        engine
            .build_image("rocjitsu-hack-abc:latest", "FROM img:latest\n")
            .unwrap();

        let recorded = std::fs::read_to_string(&log).unwrap();
        assert!(
            recorded.contains("--label rocjitsu.owner=rocjitsu"),
            "a derived image must be marked as rocjitsu's:\n{recorded}"
        );
        assert!(
            recorded.contains(&format!(
                "--label rocjitsu.runtime={}",
                rj_core::container::owning_runtime()
            )),
    ...
    fn a_build_is_not_started_for_a_bring_up_that_is_already_cancelled() {
        ...
        assert!(
            err.to_string().contains("building image rocjitsu-hack-abc"),
    ...
    fn a_host_port_already_taken_names_the_port_and_the_remedy() {
        ...
        // another program entirely, and about a `-p` rocjitsu passed on
        // the user's behalf.
        let dir = tempfile::tempdir().unwrap();
        let provider = scripted_provider(
            dir.path(),
    ...
    fn podmans_spelling_of_a_bound_port_is_recognised_too() {
        ...
        // As does one that matches but names no port, where rocjitsu would
    ...
    fn a_relative_workdir_is_refused_without_asking_the_container() {
        ...
        let err = engine
            .check_workdir("rocjitsu-s-node-0", "build")
            .unwrap_err();
        let message = err.to_string();
        assert!(
        ...
        engine
            .check_workdir("rocjitsu-s-node-0", &dir.path().to_string_lossy())
            .unwrap();
    }
    ...
    fn a_workdir_that_is_a_file_is_not_reported_as_a_missing_directory() {
        ...
        let err = engine
            .check_workdir("rocjitsu-s-node-0", &file.to_string_lossy())
            .unwrap_err();
        let message = err.to_string();
        assert!(
        ...
        assert!(
            message.contains("not a directory") && message.contains("rocjitsu-s-node-0"),
    ...
    fn a_refusal_spread_over_several_lines_reads_as_one() {
        ...
        // as `; ; ` and read as though rocjitsu had lost something.
        let dir = tempfile::tempdir().unwrap();
        let provider = scripted_provider(
            dir.path(),
            "chatty-provider.sh",
    ...
    fn a_workdir_the_container_does_not_have_is_named_before_the_exec() {
        ...
        let err = engine
            .check_workdir("rocjitsu-s-node-0", "/nope/nope")
            .unwrap_err();
        let message = err.to_string();
        assert!(matches!(err, ContainerError::Workdir { .. }), "{message}");
        assert!(
            message.contains("--workdir /nope/nope") && message.contains("rocjitsu-s-node-0"),
    ...
    fn a_container_that_cannot_answer_about_its_workdir_is_believed() {
        ...
        gone.check_workdir("rocjitsu-s-node-0", "/nope/nope")
            .unwrap();
    }
}
