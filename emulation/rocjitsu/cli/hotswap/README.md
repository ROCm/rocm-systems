...
rocjitsu wires these into a workload through the **HotSwap env contract**: the
...
By default rocjitsu does **not** build HotSwap — it *finds* and *uses* an existing
install. An opt-in CMake flag (`ROCJITSU_BUILD_HOTSWAP`, see below) can build it
from source as part of the rocjitsu build.

If rocjitsu cannot find hotswap, it won't run things using hotswap.

## Usage with rocjitsu

Once a HotSwap tree is installed somewhere rocjitsu can find it (see
[Where rocjitsu looks](#where-rocjitsu-looks)):

```sh
# Create a profile that uses HotSwap, then run under it:
rocjitsu profile create rdna --emulator hotswap
rocjitsu run --profile rdna -- ./my-rocm-app --flag
```

rocjitsu discovers the install and builds the HotSwap env contract (mirroring the
...
If rocjitsu can't find a HotSwap install, `rocjitsu profile create --emulator
hotswap` fails with guidance describing exactly which locations were searched
and how to make it discoverable. rocjitsu also fails loudly at run time rather
than silently running the workload unemulated.

### Where rocjitsu looks

Discovery is
1. `$HOTSWAP_HOME` — the HotSwap install root (`<root>/lib`).
2. `../../build/hotswap/` relative to the `rocjitsu` binary — where the
   `ROCJITSU_BUILD_HOTSWAP` source build stages, so a monorepo build is found
   automatically.
...
location from [Where rocjitsu looks](#where-rocjitsu-looks), e.g.:
...
rocjitsu discovers this tree automatically (it probes `../../build/hotswap/lib`
...
| --- | --- | --- |
| `ROCJITSU_HOTSWAP_STAGE` | Where artifacts are staged | `build/hotswap` |
| `ROCJITSU_HOTSWAP_LLVM_SRC` | Existing llvm-project (HotSwap fork) checkout | `llvm-project-hotswap` |
| `ROCJITSU_HOTSWAP_ROCR_REPO` / `_REF` | ROCR fork URL / ref | `martin-luecke/rocm-systems` @ `users/mluecke/hotswap-compatibility` |
| `ROCJITSU_HOTSWAP_TESTING_REPO` / `_REF` | intercept repo URL / ref | `harsh-amd/rocm-hotswap-testing` @ `mluecke/hotswap-env-contract` |
| `ROCJITSU_HOTSWAP_ROCM_PATH` | ROCm prefix for the ROCR build | `$ROCM_PATH` or `/opt/rocm` |
| `ROCJITSU_HOTSWAP_JOBS` | Parallel build jobs | host CPU count |
...
```mermaid
flowchart TD
    ENV["HOTSWAP_* env contract<br/>LD_PRELOAD=libhotswap_intercept.so<br/>LD_LIBRARY_PATH=…/hotswap/lib"] -->|set by rocjitsu| APP
...
rocjitsu's job is discovery + wiring: it finds the HotSwap tree and sets the env
...
- [`../README.md`](../README.md) — the rocjitsu CLI/dashboard overview.
