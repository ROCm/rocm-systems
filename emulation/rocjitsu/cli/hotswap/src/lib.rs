//! `rj_backend_hotswap` — HotSwap integration for the rocjitsu binary.
//!
...
//!
//! rocjitsu does not build HotSwap by default (the `ROCJITSU_BUILD_HOTSWAP`
...
//! the shared [`rj_core::discovery`] search policy — and wires it
...
use rj_core::discovery::{self, LibSearch};
...
/// The HIP intercept library HotSwap ships. It is the artifact rocjitsu
...
/// `llvm-tools/`, `runtime/hotswap_py/`). All rocjitsu system mounts live
/// under `/mnt/rocjitsu`; the HotSwap install root is bind-mounted here so
/// the injected `LD_PRELOAD`/`HOTSWAP_HOME`/`PYTHONPATH` paths resolve
/// inside each node's container.
pub const CONTAINER_HOTSWAP_DIR: &str = "/mnt/rocjitsu/emulator/hotswap";
...
impl EmulatorBackend for Hotswap {
    ...
    fn validate_profile(&self, _def: &ProfileDef) -> std::result::Result<(), String> {
        // HotSwap is not bundled or built by rocjitsu; it must be
    ...
    fn runtime(&self) -> RuntimeStatus {
        ...
        // search, so `rocjitsu emulators` still stats the candidates once.
        //
...
impl Hotswap {
    ...
    ///
    /// Both are threaded in for the reason `rj_backend_rocjitsu::dbt` threads
    ...
    pub fn injection_def_for(
        ...
        // `installed()` reports, so what rocjitsu says about the backend
        // and what it will actually do cannot disagree.
        let dir = install.map_err(RocJITsuError::Other)?;
...
/// no GPU HotSwap can target, which `rocjitsu emulators` already reports —
...
fn physical_target_gfx() -> Option<String> {
    target_gfx_for(
        &rj_core::hardware::gpu_gfx_versions(),
...
fn build_hotswap_env(
    ...
    // `/mnt/rocjitsu` and the workload must reference the in-container path;
...
/// this crate (rather than `rj_core`) so that all hotswap-specific
...
pub fn support_status() -> SupportStatus {
    let present = rj_core::hardware::gpu_gfx_versions();
    ...
    let detected = if present.is_empty() {
        ...
        present
            .iter()
            .map(|v| rj_core::hardware::gfx_name(*v))
            .collect::<Vec<_>>()
            .join(", ")
    };
...
/// Search policy rocjitsu uses to locate the HotSwap install. Discovery
/// is anchored on `libhotswap_intercept.so`; its directory is the
/// HotSwap lib dir (where the patched ROCR and COMGR also live).
///
/// Kept deliberately small (see the crate README's "Where rocjitsu
/// looks"): an explicit `$HOTSWAP_HOME` install root, then the in-tree
/// `ROCJITSU_BUILD_HOTSWAP` build output. No generic system fallbacks —
...
pub fn lib_search() -> LibSearch<'static> {
    LibSearch {
        ...
        // This is what the `ROCJITSU_BUILD_HOTSWAP` source build stages to
        // (under `build/hotswap`), and the var rocjitsu injects.
        home_env: &["HOTSWAP_HOME"],
        lib_name: LIB_NAME,
        // The in-tree `ROCJITSU_BUILD_HOTSWAP` source build stages to
        // `build/hotswap/lib`. Relative to the rocjitsu binary at
        // `target/<profile>/rocjitsu`, that is `../../build/hotswap/lib`,
...
/// is installed anywhere rocjitsu knows to look.
...
/// rocjitsu said HotSwap was not installed, built the injection anyway,
...
/// Multi-line, user-facing guidance describing where rocjitsu looked for
...
mod tests {
    ...
    fn session_ctx() -> SessionContext {
        SessionContext {
            id: rj_core::session::SessionId::new("hotswap-test").unwrap(),
            profile: ProfileDef {
                name: "hotswap-test".to_string(),
                description: None,
                emulator: rj_core::emulator::EmulatorDef {
                    emulator: "hotswap".to_string(),
                    plugins: Default::default(),
                    exec_mode: rj_core::emulator::ExecMode::Functional,
                    options: Default::default(),
                    topology: rj_core::common::MaybeRef::Ref("unused".to_string()),
                },
                containerize: None,
            },
