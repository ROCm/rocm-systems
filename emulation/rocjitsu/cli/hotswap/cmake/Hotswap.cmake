...
# produces, staged into a single tree that rocjitsu discovers and wires into the
# workload via the HotSwap env contract:
#
#   ${ROCJITSU_HOTSWAP_STAGE}/
...
# The top-level project is declared `project(rocjitsu NONE)`; the intercept stage
...
# rocjitsu discovers this automatically (it probes `../../build/hotswap/lib`
# relative to the binary, honours `$HOTSWAP_HOME`, and injects `HOTSWAP_HOME`).
set(ROCJITSU_HOTSWAP_STAGE
    "${CMAKE_BINARY_DIR}/hotswap"
    CACHE PATH
    "Directory the built HotSwap artifacts are staged into"
)
...
set(ROCJITSU_HOTSWAP_LLVM_SRC
    "${CMAKE_CURRENT_SOURCE_DIR}/llvm-project-hotswap"
    CACHE PATH
    "Existing llvm-project (HotSwap fork) checkout to build COMGR from"
)
set(ROCJITSU_HOTSWAP_LLVM_REPO
    "https://github.com/martin-luecke/llvm-project"
    CACHE STRING
    "llvm-project (HotSwap fork) git URL, used when the checkout is absent"
)
set(ROCJITSU_HOTSWAP_LLVM_REF
    "users/mluecke/hotswap-landing-parity"
    CACHE STRING
    "llvm-project (HotSwap fork) git ref"
)

# Stage 2: HotSwap-patched ROCR (rocm-systems fork).
set(ROCJITSU_HOTSWAP_ROCR_REPO
    "https://github.com/martin-luecke/rocm-systems"
    CACHE STRING
    "rocm-systems (HotSwap fork) git URL"
)
set(ROCJITSU_HOTSWAP_ROCR_REF
    "users/mluecke/hotswap-compatibility"
    CACHE STRING
    "rocm-systems (HotSwap fork) git ref"
)

# Stage 3: HIP intercept + python runtime (testing repo).
set(ROCJITSU_HOTSWAP_TESTING_REPO
    "https://github.com/harsh-amd/rocm-hotswap-testing"
    CACHE STRING
    "HotSwap testing repo git URL (HIP intercept + python runtime)"
)
set(ROCJITSU_HOTSWAP_TESTING_REF
    "mluecke/hotswap-env-contract"
    CACHE STRING
    "HotSwap testing repo git ref"
)

# ROCm install used to satisfy ROCR's find_package() calls (hsakmt, libdrm...).
set(ROCJITSU_HOTSWAP_ROCM_PATH
    "$ENV{ROCM_PATH}"
    CACHE PATH
    "ROCm install prefix for the ROCR build"
)
if(NOT ROCJITSU_HOTSWAP_ROCM_PATH)
    set(ROCJITSU_HOTSWAP_ROCM_PATH "/opt/rocm")
endif()
...
set(ROCJITSU_HOTSWAP_JOBS
    "${_hotswap_nproc}"
    CACHE STRING
    "Parallel jobs for the HotSwap source build"
)

# Inherit the build type from the parent rocjitsu configure so the HotSwap
...
set(_hotswap_stage "${ROCJITSU_HOTSWAP_STAGE}")
...
message(STATUS "rocjitsu: HotSwap source build ENABLED")
message(STATUS "rocjitsu:   stage dir   ${_hotswap_stage}")
message(STATUS "rocjitsu:   build type  ${_hotswap_build_type}")
message(STATUS "rocjitsu:   build jobs  ${ROCJITSU_HOTSWAP_JOBS}")
message(STATUS "rocjitsu:   ROCm path   ${ROCJITSU_HOTSWAP_ROCM_PATH}")
...
if(EXISTS "${ROCJITSU_HOTSWAP_LLVM_SRC}/llvm/CMakeLists.txt")
    message(
        STATUS
        "rocjitsu:   COMGR source (in-tree) ${ROCJITSU_HOTSWAP_LLVM_SRC}"
    )
    set(_hotswap_llvm_src "${ROCJITSU_HOTSWAP_LLVM_SRC}")
    set(_hotswap_comgr_src_args DOWNLOAD_COMMAND "")
else()
    message(
        STATUS
        "rocjitsu:   COMGR source (clone)   ${ROCJITSU_HOTSWAP_LLVM_REPO}@${ROCJITSU_HOTSWAP_LLVM_REF}"
    )
    set(_hotswap_llvm_src "${CMAKE_CURRENT_BINARY_DIR}/hotswap-llvm-src")
    set(_hotswap_comgr_src_args
        GIT_REPOSITORY
        "${ROCJITSU_HOTSWAP_LLVM_REPO}"
        GIT_TAG
        "${ROCJITSU_HOTSWAP_LLVM_REF}"
        GIT_SHALLOW
        TRUE
    )
endif()
...
ExternalProject_Add(
    ...
    CMAKE_ARGS
        ...
        -DLLVM_USE_LINKER=lld -DBUILD_TESTING=OFF -DLLVM_APPEND_VC_REV=OFF
    ...
    BUILD_COMMAND
        ...
        ${ROCJITSU_HOTSWAP_JOBS} --target libraries_sha_header
        opencl-c-base.inc_target
    COMMAND
        ${CMAKE_COMMAND} --build <BINARY_DIR> --parallel
        ${ROCJITSU_HOTSWAP_JOBS} --target amd_comgr
    ...
    COMMAND
        ${CMAKE_COMMAND} --build <BINARY_DIR> --parallel
        ${ROCJITSU_HOTSWAP_JOBS} --target clang llvm-objcopy lld llc llvm-mc
    # Stage the COMGR runtime library and the LLVM tools into the HotSwap tree.
    INSTALL_COMMAND
        ${CMAKE_COMMAND} -DHOTSWAP_ROOT=<BINARY_DIR>/lib
        -DHOTSWAP_GLOB=libamd_comgr.so* -DHOTSWAP_DST=${_hotswap_libdir} -P
        ${_hotswap_copy}
    COMMAND
        ${CMAKE_COMMAND} -DHOTSWAP_ROOT=<BINARY_DIR>/bin
        "-DHOTSWAP_GLOB=lld$<SEMICOLON>ld.lld$<SEMICOLON>llc$<SEMICOLON>llvm-mc"
...
ExternalProject_Add(
    ...
    GIT_REPOSITORY "${ROCJITSU_HOTSWAP_ROCR_REPO}"
    GIT_TAG "${ROCJITSU_HOTSWAP_ROCR_REF}"
    GIT_SHALLOW TRUE
    ...
    CMAKE_ARGS
        -DCMAKE_BUILD_TYPE=${_hotswap_build_type}
        -DCMAKE_INSTALL_PREFIX=${ROCJITSU_HOTSWAP_ROCM_PATH}
        "-DCMAKE_PREFIX_PATH=${ROCJITSU_HOTSWAP_ROCM_PATH}$<SEMICOLON>${_hotswap_comgr_build}"
        -Damd_comgr_DIR=${_hotswap_comgr_build}/tools/comgr/lib/cmake/amd_comgr
        -DBUILD_SHARED_LIBS=ON -DIMAGE_SUPPORT=ON
        -DROCR_ENABLE_HOTSWAP_COMGR_ADAPTER=ON
    BUILD_COMMAND
        ${CMAKE_COMMAND} --build <BINARY_DIR> --parallel
        ${ROCJITSU_HOTSWAP_JOBS}
    INSTALL_COMMAND
        ${CMAKE_COMMAND} -DHOTSWAP_ROOT=<BINARY_DIR>
        -DHOTSWAP_RECURSE_ANCHOR=libhsa-runtime64.so
        -DHOTSWAP_GLOB=libhsa-runtime64.so* -DHOTSWAP_DST=${_hotswap_libdir} -P
        ${_hotswap_copy}
    USES_TERMINAL_BUILD TRUE
    USES_TERMINAL_INSTALL TRUE
)
...
ExternalProject_Add(
    hotswap_intercept
    GIT_REPOSITORY "${ROCJITSU_HOTSWAP_TESTING_REPO}"
    GIT_TAG "${ROCJITSU_HOTSWAP_TESTING_REF}"
    GIT_SHALLOW TRUE
    SOURCE_DIR "${_hotswap_testing_src}"
    CONFIGURE_COMMAND ${CMAKE_COMMAND} -E make_directory ${_hotswap_libdir}
    BUILD_COMMAND
        ${CMAKE_CXX_COMPILER} -shared -fPIC -std=c++17 -O2 -o
        ${_hotswap_libdir}/libhotswap_intercept.so
        <SOURCE_DIR>/runtime/tools/hotswap_intercept.cpp -ldl
    INSTALL_COMMAND
        ${CMAKE_COMMAND} -E copy_directory <SOURCE_DIR>/runtime/hotswap_py
        ${_hotswap_pydir}
    USES_TERMINAL_BUILD TRUE
)
