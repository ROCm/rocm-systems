...
///
/// RocJITsu uses ids as on-disk directory names; they must be safe across
...
/// `def.json` — and a backend reaching back into rocjitsu's storage to
/// answer a question rocjitsu already knows the answer to was the wrong
...
pub struct SessionDef {
    ...
    /// `rocjitsu run --daemon` / `rocjitsu session start --daemon`.
    #[serde(default)]
    pub daemon: bool,
...
pub struct CreateSessionRequest {
    /// Pre-validated id; if `None` rocjitsu generates one.
