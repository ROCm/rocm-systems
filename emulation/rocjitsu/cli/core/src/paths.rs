//! XDG-compliant filesystem paths used by rocjitsu.
//!
//! # What is (and is not) on disk
//!
//! RocJITsu keeps its **configuration** on disk — profiles, topologies and
...
//!
//! RocJITsu does **not** keep session or exec state on disk. Sessions,
//! execs, their process table, their output and their health live in
//! memory inside the `rocjitsu run` that owns them (see
//! `rj_supervisor`), and a second terminal reaches them over that
...
//! channel between rocjitsu processes: it exists because emulator runtimes
...
//! |--------------------------|--------------------|------------------------------|
//! | Profiles                 | `$XDG_CONFIG_HOME` | `rocjitsu/profile/<name>.json` |
//! | Topologies               | `$XDG_CONFIG_HOME` | `rocjitsu/topology/<name>.json`|
//! | Agents                   | `$XDG_CONFIG_HOME` | `rocjitsu/agent/<name>.json`   |
//! | Per-run control socket   | `$XDG_RUNTIME_DIR` | `rocjitsu/run/<session>.sock`  |
//! | Per-session scratch      | `$XDG_RUNTIME_DIR` | `rocjitsu/session/<id>/`       |
//!
//! Two environment variables provide direct overrides for the per-app
//! directories, bypassing the XDG base lookup:
//!
//! * `$ROCJITSU_CLI_CONFIG_DIR` — overrides the rocjitsu config dir (would otherwise
//!   be `$XDG_CONFIG_HOME/rocjitsu`).
...
pub const APP_NAMESPACE: &str = "rocjitsu";

/// Directory holding one socket per live `rocjitsu run`.
...
///
/// Falls back to `$TMPDIR/rocjitsu-<uid>` if unset (per XDG spec note).
#[must_use]
pub fn xdg_runtime_dir() -> PathBuf {
    ...
    PathBuf::from(tmp).join(format!("rocjitsu-{uid}"))
}
...
/// `<rocjitsu_runtime_dir>/run`.
#[must_use]
pub fn run_socket_root() -> PathBuf {
    rocjitsu_runtime_dir().join(RUN_SOCKET_DIR)
}

/// Path of the socket a `rocjitsu run` serves for its session.
///
...
/// Root directory for rocjitsu profiles: `<rocjitsu_config_dir>/profile`.
#[must_use]
pub fn profile_root() -> PathBuf {
    rocjitsu_config_dir().join("profile")
}
...
/// Root directory for rocjitsu topologies: `<rocjitsu_config_dir>/topology`.
#[must_use]
pub fn topology_root() -> PathBuf {
    rocjitsu_config_dir().join("topology")
}
...
/// Root directory for rocjitsu agents: `<rocjitsu_config_dir>/agent`.
#[must_use]
pub fn agent_root() -> PathBuf {
    rocjitsu_config_dir().join("agent")
}
...
/// `<rocjitsu_runtime_dir>/session`.
#[must_use]
pub fn session_runtime_root() -> PathBuf {
    rocjitsu_runtime_dir().join("session")
}
...
/// discovery file, and its daemon socket). It carries no rocjitsu session
...
/// (and the `ROCJITSU_*` env overrides are ignored to keep tests
...
/// duration of any operation that touches rocjitsu state on disk to avoid
...
/// Process-wide lock to use whenever tests redirect rocjitsu directories.
///
/// Tests should hold this for the duration of any operation that
/// touches rocjitsu state on disk to avoid clobbering by parallel tests.
...
mod tests {
    ...
    fn a_runs_socket_is_named_after_its_session() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        set_test_root(tmp.path());
        let runs = tmp.path().join("runtime/rocjitsu/run");
