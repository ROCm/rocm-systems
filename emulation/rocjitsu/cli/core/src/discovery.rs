...
//!
//! rocjitsu does not bundle or build its emulator backends. Instead it
...
//!    holding the `rocjitsu` binary — the library installed beside it. It
...
//!    relative to the `rocjitsu` binary), searched **even when
...
//! redirect a `rocjitsu` that is running out of a prefix which ships its
//! own copy. [`LibSearch::file_env`] (`$ROCJITSU_LIB` and friends) is the
//! override that still wins from anywhere.
//!
//! Every one of those locations is a fact about the machine rocjitsu is
//! running on, and the only question a user has when a backend reports
//! itself uninstalled is which of them rocjitsu looked at.
...
pub struct LibSearch<'a> {
    ...
    pub lib_name: &'a str,
    /// Backend-specific directories to probe relative to the `rocjitsu`
...
impl LibSearch<'_> {
    ...
    /// probed paths says where rocjitsu looked, and this says what to set
    ...
    fn search_dirs(&self) -> Vec<PathBuf> {
        ...
        // Everything relative to the rocjitsu binary, in one place because
        ...
        if let Some(dir) = &exe_dir {
            // Opt-in: `<prefix>/lib` beside `<prefix>/bin/rocjitsu`.
...
/// One environment variable a user can set to make rocjitsu find a
...
/// is not here, where rocjitsu looked for it.
///
/// The second case is the one that matters. "Not installed" on its own
/// leaves the user with no way to act: they cannot tell whether rocjitsu
...
pub enum RuntimeLocation {
    ...
    Missing {
        ...
        /// Environment variables that would change where rocjitsu looks.
        env: Vec<EnvHint>,
    },
...
/// ancestor of the rocjitsu binary for a build tree can probe a hundred
...
impl RuntimeLocation {
    ...
    pub fn explain_missing(&self) -> Option<String> {
        ...
        out.push_str(". `rocjitsu emulators -l` lists every location.");
        Some(out)
    }
}
...
pub fn locate_emulator_lib(search: &LibSearch<'_>) -> RuntimeLocation {
    ...
    // answer without rocjitsu assembling (and shelling out for) the rest
...
/// install `display_name` so rocjitsu can find `search.lib_name`. Lists
...
pub fn install_guidance(display_name: &str, search: &LibSearch<'_>) -> String {
    let mut msg = format!(
        "{display_name} is not installed, or rocjitsu could not find `{lib}`.\n\
         rocjitsu does not build {display_name}; install it yourself and place \
...
mod tests {
    ...
    fn missing_lib_yields_none_and_guidance() {
        ...
        let s = LibSearch {
            file_env: &["ROCJITSU_NONEXISTENT_TEST_EMULATOR_LIB"],
            dir_env: &["ROCJITSU_NONEXISTENT_TEST_EMULATOR_LIB_DIR"],
            home_env: &[],
    ...
    /// The two locations resolved relative to the `rocjitsu` binary, as
    /// exact paths.
    ///
    ...
    fn a_missing_lib_reports_where_it_looked_and_what_to_set() {
        ...
        assert_eq!(
            names.first(),
            Some(&"ROCJITSU_NONEXISTENT_TEST_EMULATOR_LIB")
        );
