//! Container runtime types shared across rocjitsu crates.
//!
...
//! `rj_container` crate, which builds on these shared types.
...
/// rocjitsu spawns, and inherited by anything they fork. It is what makes a
...
/// later rocjitsu could read — except on the processes themselves. This is
/// the process-side counterpart of [`LABEL_SESSION`], and
/// [`crate::reclaim`] is what reads it.
pub const ENV_SESSION: &str = "ROCJITSU_SESSION";

/// Environment variable naming the rocjitsu runtime directory that owns a
/// process.
///
/// [`crate::paths::rocjitsu_runtime_dir`] already reads this as an *input*
/// override, and setting it on a workload is coherent with that reading:
/// a nested `rocjitsu` inside a session sees the same state directory as
/// the run that owns it. What makes it a *marker* is that reclamation
/// reads it back. [`ENV_SESSION`] alone says "some rocjitsu started this",
/// which is not enough on a machine where two rocjitsu processes are running under
...
/// physical GPU). In rocjitsu's non-containerised multi-node mode every
...
/// `rj_supervisor::spec` for where that override is applied.
...
/// The directory inside every node container that belongs to rocjitsu.
///
/// A node container is not only the user's image. RocJITsu bind-mounts its
...
/// this function's business: everything rocjitsu itself mounts is below it,
/// so a test that answered "yes" there could not tell rocjitsu's own mounts
...
/// text. `/mnt/rocjitsu/../rocjitsu` names the reserved directory and used
...
/// that looks harmless.
///
/// Component-wise via `normalise_container_path`, so `/mnt/rocjitsu` and
/// `/mnt/rocjitsu/../rocjitsu/runtime` overlap and `/mnt/rocjitsu` and
/// `/mnt/rocjitsu-of-my-own` do not. A textual `starts_with` would get the
...
/// refuse a relative one outright), so `mnt/rocjitsu` describes the
/// reserved directory just as `/mnt/rocjitsu` does, and reading it any
...
pub fn container_name(session: &SessionId, rank: u32) -> String {
    format!("rocjitsu-{}-node-{rank}", session.as_str())
}

/// Deterministic virtual-network name connecting a session's nodes.
pub fn network_name(session: &SessionId) -> String {
    format!("rocjitsu-{}", session.as_str())
}

/// Label stamped on every container and network rocjitsu creates.
///
/// Names alone are not ownership. `rocjitsu-s1-node-0` is a name any user
/// can give a container of their own, and rocjitsu's cleanup paths do
/// `rm -f <name>` — so without a mark that says "rocjitsu made this", a
/// collision means destroying someone else's container. The label is
/// checked before every removal.
pub const LABEL_OWNER: &str = "rocjitsu.owner";

/// Value [`LABEL_OWNER`] carries. Anything else is not ours.
pub const LABEL_OWNER_VALUE: &str = "rocjitsu";
...
/// came from.
pub const LABEL_SESSION: &str = "rocjitsu.session";

/// Label recording which rocjitsu runtime directory created a resource.
///
/// [`LABEL_OWNER`] says "a rocjitsu made this" and [`LABEL_SESSION`] says
/// which session, but neither says *which* rocjitsu — and two rocjitsu processes
/// running under different `ROCJITSU_CLI_RUNTIME_DIR` directories keep separate
...
/// [`ENV_RUNTIME`].
pub const LABEL_RUNTIME: &str = "rocjitsu.runtime";
...
/// The labels stamped on a derived image rocjitsu builds.
///
...
///
/// What it does carry is the same two marks everything else rocjitsu
/// creates does — that a rocjitsu made it, and which runtime directory's
/// rocjitsu that was — because an image is host state like any other and
/// the alternative is an untagged `rocjitsu-hack-…` nobody can attribute.
#[must_use]
...
///
/// Resolved rather than copied out of the environment: `ROCJITSU_CLI_RUNTIME_DIR`
/// is one of three ways the directory can be named (see
/// [`crate::paths::rocjitsu_runtime_dir`]), and a user who set none of them
...
pub fn owning_runtime() -> String {
    let dir = crate::paths::rocjitsu_runtime_dir();
    dir.canonicalize()
...
/// Whether `name` is a container rocjitsu created.
#[must_use]
...
/// Whether `name` is a network rocjitsu created.
#[must_use]
...
/// `rocjitsu-s1-node-0` themselves carries no rocjitsu owner label, so it
...
pub const ENV_PROVIDER: &str = "ROCJITSU_CONTAINER_PROVIDER";

/// The container engines rocjitsu knows how to drive, in the order it
...
/// second looks backwards. The reason it is not is that rocjitsu cannot
...
/// exported `ROCJITSU_CONTAINER_PROVIDER` silently beat a flag the user
...
pub struct ContainerState {
    ...
    /// Port the head node may listen on (carried in `ROCJITSU_HEAD_PORT`).
    pub head_port: u16,
...
/// derived from the session id, so `rocjitsu-s1-node-0` is a name a user
...
/// One rocjitsu-owned container or network whose session is not live.
...
///
/// Split out from [`reclaim_orphans`] so `rocjitsu cleanup --dry-run` can
...
/// shared with other work: a container rocjitsu did not create is never a
/// candidate, whatever it is called. Filtering by [`LABEL_RUNTIME`] is
/// what keeps it safe on an engine shared with *another rocjitsu*: `live`
...
pub fn orphans(provider: &str, live: &[SessionId]) -> Vec<Orphan> {
    ...
    for (verb, is_network) in [
        ...
        for resource in labelled(provider, verb) {
            ...
            // that a container created by a rocjitsu older than this label
...
/// Find and remove every rocjitsu-owned container and network whose session
...
/// rocjitsu could read — except the resources themselves, which carry
...
/// One rocjitsu-owned resource, as a listing plus its labels describe it.
struct Labelled {
    /// The resource's id.
    name: String,
...
/// Every rocjitsu-owned resource the given listing verb reports, with the
...
/// policy covers both `output()` and `spawn()`; `rj_container` calls
...
impl PinnedRuntime {
    pub(crate) fn new() -> Self {
        ...
        std::fs::create_dir_all(crate::paths::rocjitsu_runtime_dir()).unwrap();
        Self {
            _lock: lock,
            _root: root,
        }
...
mod tests {
    ...
    /// [`LABEL_OWNER`], so a test can stand up resources rocjitsu does
    /// *not* own.
    ...
    fn names_are_deterministic() {
        ...
        assert_eq!(container_name(&s, 0), "rocjitsu-abc-node-0");
        assert_eq!(container_name(&s, 3), "rocjitsu-abc-node-3");
        assert_eq!(network_name(&s), "rocjitsu-abc");
    }
    ...
    fn every_installed_engine_is_a_candidate_and_not_just_the_first() {
        ...
        //
        // `ROCJITSU_CONTAINER_PROVIDER` overrides the whole list, and a
    ...
    fn teardown_removes_every_container_and_network() {
        ...
        let state = ContainerState {
            ...
            network: Some("rocjitsu-s1".to_string()),
            head_port: 12345,
            nodes: vec![
                NodeContainer {
                    rank: 0,
                    name: "rocjitsu-s1-node-0".to_string(),
                },
                NodeContainer {
                    rank: 1,
                    name: "rocjitsu-s1-node-1".to_string(),
                },
            ],
        };
        teardown(&state);
        ...
        assert!(
            recorded.contains("rm -f rocjitsu-s1-node-0"),
            "{recorded:?}"
        );
        assert!(
            recorded.contains("rm -f rocjitsu-s1-node-1"),
            "{recorded:?}"
        );
        assert!(recorded.contains("network rm rocjitsu-s1"), "{recorded:?}");
    }

    #[test]
    fn teardown_spares_a_container_rocjitsu_did_not_create() {
        // Container names are derived from the session id, so
        // `rocjitsu-s1-node-0` is a name any user can give a container of
        // their own — and a session with a colliding id would then have
        // rocjitsu `rm -f` it. Ownership is the label, not the name.
        let dir = tempfile::tempdir().unwrap();
        let log = dir.path().join("provider.log");
        ...
        assert!(
            ...
            "rocjitsu removed a container it does not own:\n{recorded}"
        );
        assert!(
            !recorded.contains("network rm"),
            "rocjitsu removed a network it does not own:\n{recorded}"
        );
    }
    ...
    fn the_recorded_runtime_is_absolute() {
        ...
        // that reads it back, and the readers are `rocjitsu cleanup`
        ...
        // `ROCJITSU_CLI_RUNTIME_DIR`. A *relative* one, because that is the only
        // shape where the fallback changes the answer: it is what a
        // `ROCJITSU_CLI_RUNTIME_DIR=rt` in somebody's shell profile resolves to,
        // and recording it verbatim would name a different directory to
        // every `rocjitsu cleanup` started from somewhere else. Nothing is
        // created for it; the test lock this fixture holds is what keeps
        // the process-wide root ours to move.
        crate::paths::set_test_root(Path::new("rocjitsu-runtime-that-is-not-there"));
        assert!(
    ...
    #[test]
    fn a_mount_over_rocjitsus_own_directory_is_recognised() {
        ...
        // config and scratch mounts rocjitsu adds to every node container.
        assert!(covers_rocjitsu_dir(CONTAINER_ROCJITSU_DIR));
        assert!(covers_rocjitsu_dir("/mnt/rocjitsu/"));
        assert!(covers_rocjitsu_dir("/mnt//rocjitsu"));
        assert!(covers_rocjitsu_dir("/mnt/./rocjitsu"));
        assert!(covers_rocjitsu_dir("/mnt"));
        assert!(covers_rocjitsu_dir("/"));

        // Everything else is the user's to mount, including the paths
        // *below* the reserved tree — rocjitsu's own mounts are all down
        // there, so a check that caught them would refuse every
        // containerised session there is.
        assert!(!covers_rocjitsu_dir("/mnt/rocjitsu/lib"));
        assert!(!covers_rocjitsu_dir("/mnt/mine"));
        assert!(!covers_rocjitsu_dir("/data"));
        assert!(!covers_rocjitsu_dir(""));
    }

    #[test]
    fn an_overlapping_mount_is_recognised_in_both_directions() {
        // Above: the case `covers_rocjitsu_dir` already had. A mount at
        // `/mnt` buries everything rocjitsu put under `/mnt/rocjitsu`.
        assert!(container_paths_overlap("/mnt", "/mnt/rocjitsu/runtime"));
        // Below: the case it could not have, and the one that reads as
        // harmless. `covers_rocjitsu_dir` answers "no" for every path under
        // the reserved tree, because rocjitsu's own mounts are all down
        // there — so a user mount at
        // `/mnt/rocjitsu/runtime/rj_config.json` passed it, overlaid the
        ...
        assert!(container_paths_overlap(
            "/mnt/rocjitsu/runtime/rj_config.json",
            "/mnt/rocjitsu/runtime"
        ));
        // The same location, however it is spelled.
        assert!(container_paths_overlap("/mnt/rocjitsu", "/mnt/rocjitsu/"));
        assert!(container_paths_overlap(
            "/mnt/rocjitsu",
            "/mnt/rocjitsu/../rocjitsu"
        ));
        ...
        assert!(!container_paths_overlap(
            "/mnt/rocjitsu",
            "/mnt/rocjitsu-of-mine"
        ));
        assert!(!container_paths_overlap("/data", "/mnt/rocjitsu"));
        // An empty path is not a mount over anything.
        assert!(!container_paths_overlap("", "/mnt/rocjitsu"));
    }
    ...
    fn a_mount_that_reaches_the_reserved_directory_through_dot_dot_is_recognised() {
        ...
        // rocjitsu's own tree, and a reviewer walked the original bug back
        // through it with a `..`: the comparison was by literal
        // component, so `/mnt/rocjitsu/../rocjitsu` was two components
        // longer than the reserved path and read as somewhere else
        // entirely — while naming exactly the directory the check
        // exists to protect.
        assert!(covers_rocjitsu_dir("/mnt/rocjitsu/../rocjitsu"));
        assert!(covers_rocjitsu_dir("/mnt/rocjitsu/lib/.."));
        assert!(covers_rocjitsu_dir("/mnt/rocjitsu/lib/../.."));
        assert!(covers_rocjitsu_dir("/data/../mnt/rocjitsu"));
        ...
        // rocjitsu's own mounts live.
        assert!(!covers_rocjitsu_dir("/mnt/rocjitsu/../mine"));
        assert!(!covers_rocjitsu_dir("/mnt/rocjitsu/lib/../lib"));
    }

    #[test]
    fn a_derived_image_is_marked_as_rocjitsus_but_not_as_a_sessions() {
        let _runtime = PinnedRuntime::new();
        assert_eq!(
            image_labels(),
            vec![
    ...
    fn a_runtime_that_no_longer_exists_matches_only_itself() {
        ...
        // exists to prevent.
        let gone = "/tmp/rocjitsu-no-such-runtime-a";
        let also_gone = "/tmp/rocjitsu-no-such-runtime-b";
    ...
    /// already rocjitsu's; what distinguishes these is the session and
        ...
        std::fs::write(
            ...
            format!(
                "#!/bin/sh\n\
                 ...
                 case \"$1 $2\" in\n\
                 \"ps --all\") printf 'rocjitsu-live-node-0\\nrocjitsu-dead-node-0\\n\
                 rocjitsu-elsewhere-node-0\\nrocjitsu-unmarked-node-0\\n' ;;\n\
                 \"network ls\") printf 'rocjitsu-dead\\nrocjitsu-elsewhere\\n' ;;\n\
                 \"inspect --format\"|\"network inspect\")\n\
                 case \"$*\" in\n\
                 *{runtime_label}*)\n\
                 case \"$last\" in\n\
                 rocjitsu-unmarked-node-0) printf '<no value>' ;;\n\
                 rocjitsu-elsewhere-node-0|rocjitsu-elsewhere) echo /somewhere/else ;;\n\
                 *) echo '{ours}' ;;\n\
                 esac ;;\n\
                 *)\n\
                 case \"$last\" in\n\
                 rocjitsu-live-node-0) echo live ;;\n\
                 rocjitsu-dead-node-0|rocjitsu-dead) echo dead ;;\n\
                 rocjitsu-elsewhere-node-0|rocjitsu-elsewhere) echo elsewhere ;;\n\
                 rocjitsu-unmarked-node-0) echo unmarked ;;\n\
                 esac ;;\n\
                 esac ;;\n\
                 esac\n\
                 exit 0\n",
    ...
    fn reclaim_removes_only_orphans_and_only_ours() {
        ...
        assert_eq!(removed, vec!["rocjitsu-dead-node-0", "rocjitsu-dead"]);
        let recorded = std::fs::read_to_string(&log).unwrap();
        assert!(
            recorded.contains(&format!("--filter label={LABEL_OWNER}={LABEL_OWNER_VALUE}")),
            "the listing must be filtered to rocjitsu's own resources:\n{recorded}"
        );
        assert!(
            recorded.contains("rm -f rocjitsu-dead-node-0"),
            "{recorded}"
        );
        assert!(recorded.contains("network rm rocjitsu-dead"), "{recorded}");
        assert!(
            !recorded.contains("rocjitsu-live-node-0\n")
                || !recorded.contains("rm -f rocjitsu-live"),
    ...
    fn a_resource_of_another_runtime_directory_is_never_an_orphan() {
        // The session belongs to a rocjitsu running under a different
        // `ROCJITSU_CLI_RUNTIME_DIR`, so it is absent from *this* runtime's list of
        ...
        let recorded = std::fs::read_to_string(&log).unwrap();
        assert!(
            !recorded.contains("rm -f rocjitsu-elsewhere-node-0"),
            "another runtime directory's container was removed:\n{recorded}"
        );
        assert!(
            !recorded.contains("network rm rocjitsu-elsewhere"),
    ...
    fn a_resource_with_no_runtime_label_is_left_alone() {
        // Made by a rocjitsu older than `rocjitsu.runtime`, so nothing says
        ...
        // destroying a container that some other rocjitsu is using.
        let _runtime = PinnedRuntime::new();
        let dir = tempfile::tempdir().unwrap();
        let log = dir.path().join("provider.log");
        let provider = mock_reclaim_provider(dir.path(), &log);
        ...
        let recorded = std::fs::read_to_string(&log).unwrap();
        assert!(
            !recorded.contains("rm -f rocjitsu-unmarked-node-0"),
    ...
    fn teardown_is_idempotent() {
        ...
        let state = ContainerState {
            provider: provider.to_string_lossy().to_string(),
            image: "img:latest".to_string(),
            network: Some("rocjitsu-s2".to_string()),
            head_port: 1,
            nodes: vec![NodeContainer {
                rank: 0,
                name: "rocjitsu-s2-node-0".to_string(),
            }],
        };
        ...
        let recorded = std::fs::read_to_string(&log).unwrap();
        assert_eq!(
            recorded.matches("rm -f rocjitsu-s2-node-0").count(),
            2,
            "{recorded:?}"
        );
    }
