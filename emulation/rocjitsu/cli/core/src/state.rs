//! On-disk readers/writers for rocjitsu state.
//!
...
pub fn read_json<T: DeserializeOwned>(path: &Path) -> Result<T> {
    let bytes = fs::read(path).map_err(|e| RocJITsuError::Io {
        path: path.to_path_buf(),
        source: e,
    })?;
    serde_json::from_slice(&bytes).map_err(|e| RocJITsuError::Json {
        path: path.to_path_buf(),
        source: e,
    })
}
...
pub fn write_bytes(path: &Path, bytes: &[u8]) -> Result<()> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|e| RocJITsuError::Io {
...
/// sees half a document, but that name is rocjitsu's business. Naming it in
