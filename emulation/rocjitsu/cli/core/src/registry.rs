...
//! `rj_builtin` crate; the on-disk store policy for them lives in
...
pub struct EmulatorInfo {
    ...
    ///
    /// `installed` says whether rocjitsu can emulate; this says why, which
...
pub fn registry() -> Vec<EmulatorInfo> {
    let mut out: Vec<EmulatorInfo> = inventory::iter::<EmulatorBackendDef>
        ...
        .map(|def| {
            ...
            // for a build tree beside the rocjitsu binary stats a hundred
...
mod tests {
    ...
    const MISSING_LIB: &str = "librocjitsu-no-such-runtime.so";

    /// The environment variable that would point at it.
    const MISSING_LIB_ENV: &str = "ROCJITSU_NO_SUCH_RUNTIME_LIB";
    ...
    /// A backend that reports itself uninstalled must say where rocjitsu
    /// looked. Without it, `rocjitsu emulators` answers the user's
