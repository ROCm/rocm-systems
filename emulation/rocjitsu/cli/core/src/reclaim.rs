...
//! `rocjitsu run` that owns it, so a run that was `SIGKILL`ed — by an
...
//! to every later rocjitsu.
//!
//! RocJITsu used to prevent that with `PR_SET_PDEATHSIG`, which asked the
...
//! that knew about the session, which rules out anything rocjitsu would
...
//! pid file recording only the ranks rocjitsu spawned would have missed.
//!
//! The cost of inheritance is the same property in reverse: anything
//! started from an interactive `rocjitsu exec -- bash` inherits the tag and
...
//! runtime directory. Two rocjitsu processes can be running under two different
//! `ROCJITSU_CLI_RUNTIME_DIR` directories — a test suite beside an interactive
...
//! rocjitsu older than [`ENV_RUNTIME`] is never reclaimed by this one — a
//! leak, recoverable with `kill` by the user who can see it, and a much
//! smaller wrong than destroying somebody's running job.
//!
//! Skipped, though, is not the same as unmentioned. "`rocjitsu cleanup`
//! found nothing" and "`rocjitsu cleanup` found a running process it dared
...
/// rocjitsu* in the [module documentation](self) — and a caller that only
...
/// started it is skipped rather than claimed. See *Which rocjitsu* in the
...
/// sharing the caller's session. Running `rocjitsu cleanup` from inside a
/// shell started by `rocjitsu exec` is not exotic — it is exactly what a
...
/// exactly what it reported. `rocjitsu cleanup --dry-run` prints the scan
...
/// The escalation in `rj_supervisor::process::terminate` is for
...
/// rocjitsu-led group, which is a wider claim than the marker supports.
...
///
/// The runtime is `None` for a process tagged by a rocjitsu that predates
...
mod tests {
    ...
    impl Tagged {
        ...
        /// started by a rocjitsu older than the marker's second half would
        /// be.
        ...
        fn marked(session: Option<&str>, runtime: Option<&str>, script: &str) -> Self {
            ...
            match session {
                ...
                // suite may be run from inside a `rocjitsu exec` shell,
                // which exports it.
    ...
    fn a_process_owned_by_another_runtime_is_not_a_candidate() {
        // The bug this pair of markers exists for. `rocjitsu cleanup` under
        // one `ROCJITSU_CLI_RUNTIME_DIR` has never heard of a session belonging to
        // another, so by session name alone a perfectly healthy workload
        // of that other rocjitsu is an orphan — and reclaiming it means
    ...
    fn a_process_with_no_recorded_runtime_is_not_a_candidate() {
        // Tagged with a session but by a rocjitsu that predates the runtime
    ...
    fn a_live_session_is_left_alone() {
        // The whole safety property: `rocjitsu cleanup` runs while other
    ...
    fn a_grandchild_that_inherited_the_tag_is_reclaimed_too() {
        ...
        // file: rocjitsu only knows the pids of the ranks it spawned, and a
    ...
    fn a_process_of_another_runtime_is_not_even_mentioned() {
        ...
        // different `ROCJITSU_CLI_RUNTIME_DIR` is not. Listing it would invite
    ...
    fn a_tag_is_found_beside_an_environment_variable_that_is_not_utf8() {
        ...
        // invisible to `rocjitsu cleanup`. It is read as bytes and split on
    ...
    fn an_untagged_process_is_never_a_candidate() {
        ...
        // `ROCJITSU_CLI_RUNTIME_DIR` to point rocjitsu at a directory: it passes the
    ...
    fn the_scan_never_reports_the_caller() {
        // `rocjitsu cleanup` run from inside a `rocjitsu exec -- bash` of the
