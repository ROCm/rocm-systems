...
//! from `<ROCJITSU_CLI_CONFIG_DIR>/agent/`.
//!
//! Topologies themselves live at `<ROCJITSU_CLI_CONFIG_DIR>/topology/<name>.json`.
...
pub struct TopologyDef {
    ...
    /// [`AgentDef`] or a name resolvable under `<ROCJITSU_CLI_CONFIG_DIR>/agent/`.
...
/// On-disk topology store backed by `<ROCJITSU_CLI_CONFIG_DIR>/topology/`.
///
