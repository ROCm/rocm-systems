...
//!
//! These are files people write and edit by hand, and rocjitsu acts on
...
//! rocjitsu — and dropping it silently means the machine that comes up is
...
//! by a newer rocjitsu that added a field is rejected by an older one
//! rather than degraded. That is the trade we want. An older rocjitsu
...
impl PortMapping {
    ...
    pub fn parse(spec: &str) -> Result<Self, String> {
        ...
        // port of 0 as "pick any free one", but rocjitsu never tells the
...
/// are deliberately scoped to containerised sessions, where rocjitsu owns
...
pub enum Hack {
    ...
    /// (e.g. the bind-mounted rocjitsu host and emulator interposers)
...
pub fn hacks_image_tag(base: &str, hacks: &[Hack]) -> Option<String> {
    ...
    Some(format!("rocjitsu-hack-{:016x}:latest", hasher.finish()))
}
...
/// runtime types and `rj_container` for the engine that drives the
/// provider CLI.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
/// by sessions. Profiles live in `$XDG_CONFIG_HOME/rocjitsu/profile/`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ProfileDef {
    ...
    /// Free-form description shown in `rocjitsu profile list -l`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl ProfileDef {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        match crate::emulator::get_emulator_backend(kind) {
            ...
            None => Err(format!(
                "unknown emulator {kind:?}; run `rocjitsu emulators` for the \
