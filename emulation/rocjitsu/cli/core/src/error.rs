...
/// Result alias used throughout rocjitsu.
pub type Result<T> = std::result::Result<T, RocJITsuError>;

/// Everything that can go wrong in rocjitsu's control plane.
///
...
/// formatter that walks it, use [`RocJITsuError::full_message`].
#[derive(Debug, Error)]
pub enum RocJITsuError {
    ...
    #[error("profile not found: {name} (rocjitsu looked in {dir})")]
    ProfileNotFound {
    ...
    #[error("topology not found: {name} (rocjitsu looked in {dir})")]
    TopologyNotFound {
        /// The name that was asked for.
        name: String,
    ...
    #[error("agent not found: {name} (rocjitsu looked in {dir})")]
    AgentNotFound {
        /// The name that was asked for.
        name: String,
    ...
    // reach: a `rocjitsu run` holds its session in its own address space,
    // and the one thing that does speak over a socket — `rocjitsu exec` —
...
impl RocJITsuError {
    /// Build an [`RocJITsuError::Other`] from anything string-like.
    pub fn other(msg: impl Into<String>) -> Self {
        Self::Other(msg.into())
    }

    /// Report a document rocjitsu could not find, naming the directory it
    /// looked in.
    ///
    /// "Where did rocjitsu look?" is the next question every one of these
    /// raises, and it is not a question the reader can answer for
    /// themselves: both `ROCJITSU_CLI_CONFIG_DIR` and `XDG_CONFIG_HOME` move the
    ...
    /// Build an [`RocJITsuError::Io`] for `path`.
...
mod tests {
    ...
    fn not_found_classification() {
        assert!(RocJITsuError::SessionNotFound("s".into()).is_not_found());
        assert!(RocJITsuError::ExecNotFound("e".into()).is_not_found());
        assert!(RocJITsuError::not_found(DocKind::Profile, "p").is_not_found());
        assert!(!RocJITsuError::Other("boom".into()).is_not_found());
        assert!(!RocJITsuError::SessionExists("s".into()).is_not_found());
    }

    #[test]
    fn a_missing_document_says_where_rocjitsu_looked() {
        // The next question a "not found" raises is which directory was
        // read, and it is not one the reader can answer: ROCJITSU_CLI_CONFIG_DIR and
        ...
        for (kind, root) in [
            ...
            let e = RocJITsuError::not_found(kind, "ghost").to_string();
