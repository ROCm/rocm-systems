...
/// usable, and where it is — or, when it is not here, where rocjitsu
/// looked for it.
///
...
pub struct RuntimeStatus {
    ...
    pub installed: bool,
    /// Where the runtime library is, or where rocjitsu looked.
...
/// A backend integration rocjitsu can drive to emulate a workload.
///
...
/// definition (see `rj_supervisor::session::resolve_profile`).
...
mod tests {
    ...
    /// `rj_core` — so anything that resolves an emulator, such as
    ...
    const NO_SUCH_LIB: &str = "librocjitsu-emulator-trait-test.so";
    ...
    /// bare "no" and allowed to skip the answer `rocjitsu emulators -l`
