...
pub struct ExecDef {
    ...
    /// terminal: `rocjitsu exec --node 2 -- bash` is a shell on node 2 of a
    ...
    /// the caller's.
    ///
    /// By default a workload inherits everything the terminal rocjitsu was
    /// started from had exported: rocjitsu's parent *is* the user's shell,
    ...
    /// what rocjitsu passes explicitly reaches the workload.
    #[serde(default)]
    pub clear_env: bool,
}
...
pub struct InjectionDef {
    ...
    /// convention these target locations live under `/mnt/rocjitsu`.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub mounts: Vec<FileMount>,

    /// Host paths of shared libraries the workload needs available. For
    /// a containerised session each is bind-mounted into `/mnt/rocjitsu/lib`
    ...
    /// bind-mounted rocjitsu binary and emulator interposers were built
