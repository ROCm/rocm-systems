//! The wire protocol between `rocjitsu exec` and the `rocjitsu run` that owns
//! a session.
//!
//! # Shape
//!
//! A `rocjitsu run` owns exactly one session, for exactly as long as that
...
//!
//! `rocjitsu exec` borrows a session it does not own, and the run that does
...
//!
//! Because the exec'd process is not the run's child. `rocjitsu exec` runs
//! in a different terminal from the `rocjitsu run` that owns the session,
...
pub enum Request {
    ...
    /// Ask the run to tear its session down.
    ///
    /// Answered immediately with [`Response::Stopping`]; the teardown
    /// happens after, and can take as long as removing a container per
...
pub enum Response {
    ...
    /// The run accepted a [`Request::Stop`] and is tearing down.
    Stopping,
    ...
    /// [`RocJITsuError`](crate::error::RocJITsuError).
    Error(String),
}

/// Everything `rocjitsu exec` needs to start a process inside a session it
/// does not own.
///
...
pub struct SessionDescription {
    ...
    /// as the owning `rocjitsu run` was asked for with `--nproc-per-node`.
    ///
    /// Part of the description because it is part of the session's
    /// *shape*, not of one command: `RANK`, `LOCAL_RANK` and
    /// `WORLD_SIZE` are numbered in this grid, and a `rocjitsu exec` that
