...
//! whoever called [`Daemon::start`] — the `rocjitsu run` process. Its C API
//! is three calls —
...
//!
//! The wrapper lives in this crate rather than in `rj_backend_rocjitsu`
...
// through `&mut self` or the owning `stop`. RocJITsu moves the owner
...
impl Daemon {
    ...
    /// For rocjitsu that was at the *end* of bring-up, after the image pull,
    ...
    pub fn probe(lib_path: &Path) -> Result<(), DaemonError> {
        ...
        // rocjitsu library located by rocjitsu's own discovery, and loading it
    ...
    /// gives callers an explicit, ordered shutdown point — rocjitsu stops
...
mod tests {
    ...
    fn probing_a_library_without_the_daemon_api_reports_the_missing_symbol() {
        ...
        // predates the daemon API looks exactly like this, and rocjitsu
