...
//!
//! rocjitsu drives the rocjitsu functional emulator directly through its
...
//! a ROCm wheel / system install and is never present when rocjitsu is
...
/// Publish the per-invocation runtime config handoff through the
/// library's own writer (`rj_dbt_write_handoff`).
///
/// In DBT guest mode the KMD interposer and the HSA tools hook both read
/// this file, and they have to agree with each other and with the
/// launcher on which physical GPU was selected — a second copy of the
/// format living in Rust would be a contract that can drift silently.
/// So the handoff is written by the same code that reads it, and the only
/// thing this crate contributes is the destination directory.
///
/// `host_gpu_id` is the resolved DBT host GPU; zero writes no GPU line,
/// which is the shape a non-DBT invocation publishes.
///
/// # Errors
///
/// Returns a diagnostic when the library cannot be loaded, predates DBT
/// guest support, or fails to publish the handoff.
pub fn write_dbt_handoff(
    ...
    unsafe {
        ...
        let write_handoff = *lib
            .get::<FnDbtWriteHandoff>(b"rj_dbt_write_handoff\0")
            .map_err(|_| {
                "librocjitsu.so does not export rj_dbt_write_handoff; this build predates \
                 DBT guest support"
                    .to_string()
            })?;
...
/// Render a path as a C string, rejecting the interior NUL a C API
/// cannot represent.
...
// rocjitsu checkout is the common case.
//
