...
//!
//! It is deliberately best-effort. rocjitsu does not build rocjitsu, so on
...
/// filesystem root looking for a rocjitsu source tree beside rocjitsu —
...
fn locate_include_dir() -> Option<PathBuf> {
    ...
    // `.../rocjitsu/rocjitsu_sys` -> `.../rocjitsu` -> `...` -> ...
