# rocjitsu on-disk state layout

RocJITsu's **configuration** lives on disk in standard
...
with rocjitsu may read and write these files directly.

RocJITsu's **session state does not live on disk at all**. A session, its
process table, its output and its health are held in memory by the
`rocjitsu run` process that created them, and they cease to exist when it
...
| ------------------- | ---------------- | ------------------- | ---------------------------------------- |
| Config              | `ROCJITSU_CLI_CONFIG_DIR`  | `$XDG_CONFIG_HOME`  | `rocjitsu/` (profiles, agents, topologies) |
| Run sockets         | `ROCJITSU_CLI_RUNTIME_DIR` | `$XDG_RUNTIME_DIR`  | `rocjitsu/run/`                            |
| Emulator scratch    | `ROCJITSU_CLI_RUNTIME_DIR` | `$XDG_RUNTIME_DIR`  | `rocjitsu/session/<id>/`                   |

Each `ROCJITSU_*` variable, when set, fully overrides the corresponding
directory; otherwise the XDG variable (or its standard default) is used.
`ROCJITSU_CLI_RUNTIME_DIR` is also an *output*: rocjitsu stamps the resolved runtime
...
rocjitsu using a different one. See [Cleanup](#cleanup).
`rocjitsu paths` prints the resolved directories — config, runtime,
...
`ROCJITSU_STATE` are not consulted at all: nothing rocjitsu writes has to
...
```text
<runtime>/rocjitsu/
├── run/<session>.sock   # one socket per live `rocjitsu run`
...
recognised. `rocjitsu run` applies that test at bind time — if something
answers on the path, another run already owns the id and it refuses; if
nothing does, the file is a corpse and is unlinked. `rocjitsu exec` applies
the same test from the other side, and reports "no `rocjitsu run` is
...
```text
<config>/rocjitsu/profile/<name>.json
```
...
session exists exactly as long as the `rocjitsu run` that created it; every
...
It names a process rocjitsu cannot otherwise reach; it is not a channel and
...
`<runtime>/rocjitsu/session/<id>/`. It is **not** a channel between rocjitsu
...
RocJITsu never reads these back to answer a control-plane query. The
...
container at `/mnt/rocjitsu/runtime`, so an in-container emulator runtime
...
| Question | Answer |
| --- | --- |
| Where do run sockets live? | `rocjitsu paths` (the `runs:` line) |
| Which sessions are live? | the `.sock` files in that directory that answer when connected to |
| How do I start something in one? | `rocjitsu exec -- <command>`, or `rocjitsu exec -s <id> -- <command>` when several runs are up |
| What did it print? | the terminal its `rocjitsu run` is in; a multi-node job labels every rank's lines automatically |

`rocjitsu exec` may omit `--session` because the common case is one run in
...
leaves its socket behind — the expected leak `rocjitsu cleanup` exists for —
...
`rocjitsu exec --session <id>` never consults that list, so a session that
...
* Emulator scratch files are written the same way, because an emulator
  runtime may read them while rocjitsu is rewriting them.

## Cleanup

* A `rocjitsu run` tears its session down on every exit path, including the
...
* Nothing covers a `SIGKILL`ed run, an OOM kill, or a machine that lost
  power: no signal handler, no `Drop` and no cancelled task of rocjitsu's
  runs, so the containers keep running, the workloads are reparented to
  init and the scratch directory stays. `rocjitsu cleanup` is the recovery.
  It finds them by the marks left on the resources themselves — the
  `rocjitsu.owner`, `rocjitsu.session` and `rocjitsu.runtime` labels on
  containers and networks, `ROCJITSU_SESSION` and `ROCJITSU_CLI_RUNTIME_DIR` in every
...
* Cleanup is scoped to one runtime directory, which is why the runtime
  mark exists. The sockets under `<runtime>/rocjitsu/run/` are the whole
  registry of what is live *there*, so a cleanup cannot see the live
  sessions of a rocjitsu running under a different `ROCJITSU_CLI_RUNTIME_DIR`, and
  ...
  work left behind by a rocjitsu older than the mark, which has to be
  removed by hand.
* `rocjitsu state purge` does the same and then removes the whole runtime
