...
This guide covers building the `rocjitsu` CLI and (optionally) the
`rocjitsu` GPU emulator that rocjitsu drives.

rocjitsu is a single Cargo workspace ([`emulation/rocjitsu/cli/`](../)). One
`cargo build` produces the unified `rocjitsu` binary from a set of crates —
...
signalled as a unit, and each `rocjitsu run` serves one socket under
`$XDG_RUNTIME_DIR/rocjitsu/run/` so `rocjitsu exec` in another terminal can
find it.
...
This builds the `rocjitsu` binary at `target/debug/rocjitsu` (or
`target/release/rocjitsu`). The build embeds:
...
- the **third-party dependency manifest** that `rocjitsu about` prints,
...
rocjitsu configures, installs and tests with the same recipe as the rest
...
Options worth knowing: `ROCJITSU_CARGO_FEATURES` (comma-separated extra
cargo features), `ROCJITSU_CARGO_PROFILE` (default `release`),
`ROCJITSU_BUILD_HOTSWAP` (build HotSwap's LLVM + COMGR + ROCR stack from
source — a long build, hence opt-in), `ROCJITSU_LINT_TESTS` (on; turn it
off to register only `cargo_test`), and `ROCJITSU_ALLOW_TEST_SKIP`, which
is the `ctest` spelling of `ROCJITSU_E2E_ALLOW_SKIP=1` described below.

## Building rocjitsu

rocjitsu is the emulator rocjitsu drives, so rocjitsu needs its libraries to
bring a session up. Without them:

* `rocjitsu emulators` reports the backend as not installed;
* `rocjitsu run` fails at bring-up, naming the missing library;
...
If this build deliberately excludes rocjitsu, set ROCJITSU_E2E_ALLOW_SKIP=1
to accept the skips.
```

Set `ROCJITSU_E2E_ALLOW_SKIP=1` for a build that intentionally excludes
rocjitsu — a docs-only CI job, say. Prefer building rocjitsu where you
can: those suites are where rocjitsu's session and process lifecycle is
actually covered.

### Option A — let rocjitsu find them
...
3. an install layout — `<prefix>/lib` next to a `<prefix>/bin/rocjitsu`.
...
4. a sibling monorepo build, found by walking up from the `rocjitsu`
...
cannot override a `<prefix>/lib` beside the `rocjitsu` you are running —
only `ROCJITSU_LIB` or `$LD_LIBRARY_PATH` can.

You do not have to reason about any of this in the dark. `rocjitsu
...
If nothing is found rocjitsu still builds and runs; the backend is simply
...
```sh
./target/debug/rocjitsu state builtins        # extract agents/topologies
./target/debug/rocjitsu profile create gpu --emulator rocjitsu --no-input
./target/debug/rocjitsu run --profile gpu -- \
...
serves while it lives, and `rocjitsu exec` borrowing that session from
...
Because a session only exists while the `rocjitsu run` that owns it is
...
`cargo_fmt`) unless `ROCJITSU_LINT_TESTS` is turned off, so a CMake-driven
build catches a lint regression the same way it catches a failing test.
`cmake --build build --target rocjitsu_lint` runs just those two without the
test suite.

## Troubleshooting

- **`command not found: <cmd>` from `rocjitsu run`** — the program you asked
  rocjitsu to run doesn't exist on `PATH` inside the session. The rank that
  couldn't start reports `rocjitsu: node <n>: <reason>` on rocjitsu's own
  stderr and records exit code 127, rather than the exec quietly running
  the ranks that did start.
- **`no rocjitsu run is serving session <id>`** — the run that owned the
  session has exited. A session exists exactly as long as its `rocjitsu
  run` does; start one in another terminal and `rocjitsu exec` into that.
- **A backend reported as not installed** — run `rocjitsu emulators -l`
