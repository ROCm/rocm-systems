...
rocjitsu is a single Cargo workspace that builds one unified `rocjitsu`
...
you invoked, and `rocjitsu [--config c.json] -- ./app` is a `rocjitsu`-shaped
alias for `run`.
...
* **Execution commands** — `run` and `exec`. These own processes.
  `rocjitsu run` owns a whole session; `rocjitsu exec` owns the processes it
  starts inside somebody else's.

The word "daemon" survives in rocjitsu only as the *emulator's* daemon
(`--daemon` / `--in-process`, the out-of-process rocjitsu runtime). RocJITsu
itself has none.
...
| Crate               | Role                                                                 |
| ------------------- | -------------------------------------------------------------------- |
| `rocjitsu` (root)     | The unified binary: parses argv, owns a Tokio runtime, dispatches.   |
| `rj_ctl`        | The command line: the `CtlCmd` subcommand tree, `dispatch`, and `run`/`exec` themselves. |
| `rj_core`       | Shared types (`ProfileDef`, `SessionDef`, `ExecDef`, …), XDG path resolution for configuration, atomic file I/O and the config store built on it, the `run`↔`exec` wire protocol, and the emulator/registry traits. |
| `rj_supervisor` | The engine a `rocjitsu run` is built out of: `Run`, `Session`, `Exec`, the shared spec builder, process spawn/supervise/reap, and the socket a run serves. |
| `rj_container`  | Container provider abstraction (podman/docker) for containerised sessions. |
| `rj_builtin`    | Embedded builtin agents, topologies, and profiles, plus their unpackers. |
| `rj_backend_rocjitsu`   | The `rocjitsu` (and `rocjitsu-dbt`) backend.                         |
| `rj_backend_hotswap`    | The `hotswap` load-time ISA-rewriting backend.                       |
...
feature simply adds or removes it from `rocjitsu emulators`. Those features
are the only optional things left in the build: there is no `daemon`,
`webui` or `http` feature, no `ROCJITSU_ENABLE_DAEMON` or
`ROCJITSU_ENABLE_WEBUI` cmake option, and no Node.js toolchain.
...
┌────────────────────────────────────┐    ┌────────────────────────────────┐
│ rocjitsu run -- ./app                │    │ rocjitsu exec -- bash            │
...
The socket lives at `$XDG_RUNTIME_DIR/rocjitsu/run/<session>.sock` and is
...
`rocjitsu paths` for where it is on your machine.

## Why the run owns its session

A session exists exactly while the `rocjitsu run` that created it is alive.
It cannot be started on its own, listed, attached to, or left behind —
there is no `rocjitsu session` subcommand tree, because there is nothing for
it to name that outlives its creator.

That is the third answer to the same question, and the first two are the
argument for it.

**First there were files.** A detached `rocjitsu host` per session, with the
...
**Now the command is the owner.** `rocjitsu run` brings the session up in
...
everything, not just for rocjitsu.
...
`rocjitsu cleanup` reclaims the rest — containers, per-session network,
stranded workloads and scratch directory — matching on marks rocjitsu put on
the resources themselves, so a shared engine is safe. It runs at any time
and simply skips a session whose run still answers. `rocjitsu state purge`
...
`rocjitsu exec` runs a command inside a session another process owns, and it
...
terminal the user typed `rocjitsu exec` into. Having the client spawn is the
whole reason `rocjitsu exec -- bash` is an interactive shell *in the window
...
`rj_supervisor::build_specs`, from the same description. That is not
...
`--` belongs to the command: with both positional, `rocjitsu exec -- bash`
could equally mean "session `bash`". It can usually be omitted entirely —
with exactly one run live, rocjitsu picks it; when the guess would be
...
Whoever spawns it — `rocjitsu run`, or a `rocjitsu exec` in another terminal —
...
The obvious alternative is what rocjitsu used to do, and it was worse in
...
stdin *is* the terminal, not because rocjitsu emulated one. `rocjitsu attach`,
`rocjitsu logs` and the `--tty` flag are gone with the machinery that needed
them: there is no stream to attach to and no buffer to tail, because the
bytes never pass through rocjitsu at all.
...
labels. RocJITsu goes back in the middle so it can prefix every line with
...
multi-node job. `rocjitsu exec --node N` fills it. Naming one node leaves
...
group alone, so doing it on a grid would kill one rank and leave rocjitsu
...
it. In the foreground, the container's lifetime is bounded by the `rocjitsu
...
* The client's stdin goes to `/dev/null` and **both** its output streams
  ...
  rocjitsu's terminal would interleave provider chatter with the output the
...
Every rocjitsu process runs a single Tokio runtime created in `main`.
...
* **Ctrl-C reaches rocjitsu, not the workload.** Children lead their own
  process groups, so the terminal's foreground group is the rocjitsu process
...
* **The run socket is served with `select!`** against the workload rather
  ...
  a run is visible to `rocjitsu state purge` from its first instant rather
...
* **Stale sockets are tested, not locked.** A socket file outlives a
  ...
  cannot be fooled by a stale lock. `rocjitsu exec` applies the same test
...
* **The socket's directory chain is private by construction.** Anyone who
  ...
  every directory rocjitsu creates on the way to it is created `0700` by
...
RocJITsu previously hand-rolled `unsafe` in four places outside the FFI
...
| `pre_exec` calling `prctl(PR_SET_PDEATHSIG)`, in a `rocjitsu_sys` crate that existed to hold it | deleted with the guarantee it enforced: a `SIGKILL`ed run may strand its workloads, and `rocjitsu cleanup` reclaims them |
...
`PR_SET_PDEATHSIG` asked the kernel to kill each workload when rocjitsu
died, covering the one case rocjitsu cannot handle itself — `kill -9`, or
...
and per-session network carries the `rocjitsu.owner`, `rocjitsu.session` and
`rocjitsu.runtime` labels, and every workload carries `ROCJITSU_SESSION` and
`ROCJITSU_CLI_RUNTIME_DIR` in its environment, so `rocjitsu cleanup` can find and
...
*rocjitsu* that run was. Without the second, a cleanup reclaimed across
runtime directories: the run sockets under one `ROCJITSU_CLI_RUNTIME_DIR` are the
...
cost is that work left by a rocjitsu older than the mark is never reclaimed
...
1. Create a crate implementing `rj_core::emulator::EmulatorBackend`.
...
2. Register it into the registry with an `inventory::submit!` in its
   `lib.rs`.
3. Add it as an optional, feature-gated dependency of the root `rocjitsu`
...
in `rocjitsu emulators` and becomes selectable via `--emulator`.
...
* **End-to-end tests** under `tests/` drive the actual binary through the
  ...
  streams and exit code, the socket it serves while it lives, `rocjitsu
