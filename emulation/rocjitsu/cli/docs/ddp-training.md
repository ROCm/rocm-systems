# Distributed training with PyTorch DDP on rocjitsu
...
```sh
rocjitsu run --profile mi350x --gpus-per-node 2 \
...
```mermaid
flowchart LR
  subgraph host["rocjitsu run (1 emulated node)"]
...
1. **Multiple emulated GPUs.** `rocjitsu run --gpus-per-node N` tells
...
2. **`torchrun` rendezvous works out of the box.** rocjitsu exports the
   standard `torch.distributed` variables `MASTER_ADDR` and
   `MASTER_PORT` on every node (aliasing rocjitsu's own
   `ROCJITSU_HEAD_ADDR` / `ROCJITSU_HEAD_PORT`). For a single node,
   `torchrun --standalone` picks its own loopback rendezvous; for
   multi-node you can point `torchrun --rdzv-endpoint` at
   `$MASTER_ADDR:$MASTER_PORT`.

3. **One emulator shared by every rank (default).** rocjitsu hosts a single
   ...
   not a service of its own: the daemon is `dlopen`ed into the `rocjitsu
   ...
   `rocjitsu` drop-in accepts). Pass `rocjitsu run --in-process` to instead
...
`site-packages/_rocm_sdk_devel` after this step. rocjitsu auto-detects
...
The first `--` ends `cargo run`'s arguments; the second ends rocjitsu's,
so everything after it is the workload `torchrun` launches once per
rank. rocjitsu already runs the shared out-of-process emulator by default
(pass `--in-process` to opt out).

The command runs in the foreground and *is* the session: rocjitsu prints
`rocjitsu: session <id>` on stderr as it comes up, and everything it
brought up is gone when the command returns.

### 3. Read the output

By default the workload inherits this terminal. Its processes write to
your stdout and stderr directly — rocjitsu never sees the bytes — so
redirection behaves exactly as it would without rocjitsu (`> train.log`
captures stdout and leaves the emulator's warnings on stderr), and rank 0
also inherits stdin, which is what makes `rocjitsu run -- bash` an
interactive shell.

That is the right default here: `torchrun` is what forks the ranks, so
rocjitsu is supervising a single process, and the fixture tags its own
lines with `[rank N]`. As soon as *rocjitsu* is the thing launching several
ranks — the multi-node case below — rocjitsu labels every rank's lines
itself.
...
1. Reads `RANK`, `WORLD_SIZE`, `LOCAL_RANK` (set by `torchrun`) and
   `MASTER_ADDR` / `MASTER_PORT` (set by rocjitsu).
...
|----------|---------|---------|
| `NPROC` | `2` | GPUs / ranks per node (`torchrun --nproc_per_node`, `rocjitsu --gpus-per-node`). |
| `STEPS` | `50` | Optimizer steps. |
| `PROFILE` | `mi350x` | rocjitsu profile / emulated GPU. |
...
rocjitsu emulates each *node* as its own rank process and exports the full
...
```sh
rocjitsu run --profile mi350x --num-nodes 2 --gpus-per-node 2 \
...
- Every rank's output is piped through rocjitsu, which prefixes
...
The outer `[0]`/`[1]` is rocjitsu's label — the global rank of the process
...
- **No rank gets stdin.** Capturing costs you the terminal, so
  ...
  `rocjitsu exec --node N -- bash` for a terminal on one of its nodes.
...
If it stalls instead, the rocjitsu label tells you which rank went quiet,
and the fixture's own timestamps tell you where.

### Poking at a live run from another terminal

A session exists exactly as long as the `rocjitsu run` that created it. For
...
rocjitsu exec -- .venv-mi350/bin/python3 -c \
  'import torch; print(torch.cuda.device_count())'
```

`rocjitsu exec` needs no session id while exactly one run is live; name one
with `-s <id>` (the id `rocjitsu run` printed) when several are. It takes
the same `--node`, `--env`, `--nproc-per-node` and `--workdir`
flags as `run`, and the process it starts is a child of *terminal 2*, on
terminal 2's streams — so `rocjitsu exec -- bash` really is an interactive
...
- **Several ranks' lines are interleaved and you cannot tell them
  apart.** When rocjitsu is the thing launching the ranks it already
  labels every line with the rank that wrote it. A job `torchrun` forks
  is a single process as far as rocjitsu can see, so any labels there are
  the fixture's own — launch the ranks with `--num-nodes` /
  `--nproc-per-node` instead if you want rocjitsu's. There is no flag
...
- **Multi-GPU run hangs / times out after a few steps.** Known
  ...
  stopped first.
- **`no rocjitsu run is running` from `rocjitsu exec`.** Nothing owns a
...
- **`KMD preload library not found`.** You are running an installed
  `rocjitsu` from `PATH` instead of the workspace build. Run via
  `cargo run --` from `emulation/rocjitsu/cli` (see
