...
`rocjitsu` is organized into a small set of verbs, each managing one kind of
resource. Every command supports `--help`, and most support `--json` for
machine-readable output.

```text
rocjitsu <command> [options]
rocjitsu <command> --help        # per-command help
```
...
*borrows* one from a live `rocjitsu run`. No command starts anything in the
...
| `-V`, `--version` | Print the rocjitsu version and installed RocJITsu build identity. |
...
| Command            | Purpose                                                        |
| ------------------ | -------------------------------------------------------------- |
| `rocjitsu run`       | Bring up a session, run a command in it, tear it down.         |
| `rocjitsu exec`      | Run a command in the session a live `rocjitsu run` owns.         |
| `rocjitsu emulators` | List emulator backends and their install / support status.    |
| `rocjitsu profile`   | Manage profiles (reusable emulator presets).                   |
| `rocjitsu topology`  | Manage topologies (rack/node/GPU layouts).                     |
| `rocjitsu agent`     | Manage agents (hardware GPU definitions).                      |
| `rocjitsu state`     | Manage rocjitsu's on-disk state (builtins, purge).              |
| `rocjitsu cleanup`   | Reclaim what a run that died abruptly left behind.            |
| `rocjitsu paths`     | Print where rocjitsu stores its state.                          |
| `rocjitsu about`     | Show version, copyright, and third-party licenses.            |

## `rocjitsu run`

```text
rocjitsu run [--profile NAME] [--emulator NAME]
...
```sh
rocjitsu run --profile mi350x -- pytest -x my_tests/
```
...
```sh
$ rocjitsu run --profile mi350x -- ./app
rocjitsu: session s-20260730-191636-1f4a-0
```

Bring-up says what it is doing. Each phase it enters is printed on stderr,
prefixed like the session line — `rocjitsu: pulling image <ref> (this can
take a while)…`, `rocjitsu: creating session network <name>`, `rocjitsu:
...
exactly when a user reaches for one, killed rocjitsu with the network and
...
```text
rocjitsu: interrupted: asked the workload to stop; interrupt again to stop waiting for it
rocjitsu: not waiting any longer; stopping the workload
rocjitsu: killing the workload outright
```
...
process group, so Ctrl-C goes to *it* and not to rocjitsu. That is
ordinary job control, and it is what makes `rocjitsu run -- bash` behave
...
RocJITsu catches them so that their default disposition cannot kill the run
out from under a job, forwards them to the workload, and draws no
conclusion of its own:

```text
rocjitsu: SIGUSR1: forwarded to the workload
```
...
* `--emulator NAME` overrides the profile's backend (see
  `rocjitsu emulators` for what this build has).
...
* `-o`/`--option KEY=VALUE` sets an emulator option directly. Repeatable.
  ...
  from one the emulator ignored. `rocjitsu emulators -l` lists the options
  each backend takes.
...
* `--config PATH` hands the backend an explicit emulator config file
  ...
  **refused** alongside `--config` rather than silently ignored. Put them
  ...
  the process grid rocjitsu builds, not the emulator's configuration, so
  the two compose.
...
* `--mount HOST[:CONTAINER[:ro|rw]]` adds a bind mount to every node
  ...
  RocJITsu will not create it for you: the two engines disagree about what
  ...
  The **container** path has one reservation: `/mnt/rocjitsu`, which is
  where every containerised session mounts rocjitsu's own binary, config
  ...
  directory *underneath* rocjitsu's, so the container creates rocjitsu's
  ...
  a path *below* it cannot be told from one of rocjitsu's own, so testing
  for those would refuse every containerised session. Everything outside
  `/mnt/rocjitsu` is yours.
...
* `--container-provider podman|docker|PATH` chooses the engine;
  ...
  `ROCJITSU_CONTAINER_PROVIDER`, which is the default for when nothing
...
client is a child process rocjitsu owns, so killing it stops the container
and `--rm` removes it. There is no container that can outlive the `rocjitsu
...
* `--workdir DIR` is the working directory for the command; it defaults
  to the directory you ran `rocjitsu run` in.
* `--env KEY=VALUE` injects extra environment variables. Repeatable.
  ...
  job's identity (`ROCJITSU_*`, `RANK`, `LOCAL_RANK`, `WORLD_SIZE`,
  `MASTER_ADDR`, `MASTER_PORT`, `NCCL_HOSTID`) is set by rocjitsu last and
...
The workload **inherits rocjitsu's own stdin, stdout and stderr**. There is
no pseudo-terminal anywhere in rocjitsu: if your terminal is a terminal, an
...
```sh
rocjitsu run --profile mi350x -- bash        # a real shell: prompt, echo,
                                           # line editing, Ctrl-C, job control
rocjitsu run --profile mi350x -- python3     # a working REPL
```

and what makes this work too:

```sh
rocjitsu run --profile mi350x -- ./app > out.txt 2> err.txt   # streams stay split
```
...
every rank as well, aliasing rocjitsu's own `ROCJITSU_HEAD_ADDR`/
`ROCJITSU_HEAD_PORT`. Give each node at least N GPUs (`--gpus-per-node`) so
...
RocJITsu says so once, on stderr, when you piped something in that nothing
will read — an immediate EOF in every rank is otherwise indistinguishable
from a broken pipe or a bad path:

```text
rocjitsu: this job runs several processes, so none of them is given stdin — one
...
it. For a terminal on one node of a multi-process job, use `rocjitsu exec
--node N` — see below.
...
stderr *are* yours. That is what makes `rocjitsu run -- bash` an ordinary
interactive shell — prompt, echo, line editing, job control — and what
keeps redirection byte-exact with stdout and stderr distinct. RocJITsu is
not in the middle.

**A multi-process job** — more than one node, or `--nproc-per-node`
above one — has every rank's stdout and stderr piped through rocjitsu,
...
```sh
$ rocjitsu run --profile mi350x --num-nodes 2 -- sh -c 'echo hello from $ROCJITSU_RANK'
rocjitsu: session s-20260730-191636-1f4a-0
[0] hello from 0
[1] hello from 1
```
...
* **No rank gets stdin.** One terminal cannot be shared between readers,
  ...
  `rocjitsu exec --node N` — see below.

### The workload's environment

A workload inherits the environment of the terminal you ran rocjitsu from.
RocJITsu's parent *is* your shell, so anything you exported there — an API
...
3. rocjitsu's rank variables (`RANK`, `WORLD_SIZE`, `LOCAL_RANK`,
   `MASTER_ADDR`, `MASTER_PORT`, `ROCJITSU_RANK`, …).

RocJITsu's own go last, so a workload cannot break its own rendezvous by
...
and the rest of the identity set (`ROCJITSU_*`, `LOCAL_RANK`, `WORLD_SIZE`,
...
```sh
rocjitsu run --profile mi350x --clear-env-vars -- ./benchmark
```
...
what rocjitsu passes it with `-e`, and nothing else.

## `rocjitsu exec`

```text
rocjitsu exec [-s|--session ID] [-n|--node N] [--nproc-per-node N]
            [--clear-env-vars] [--env KEY=VALUE]... [--workdir DIR]
            -- <cmd> [args...]
```

Run a command inside the session a live `rocjitsu run` owns — the usual
shape is one terminal running the job and another one exec'ing into it:

```sh
# terminal 1
$ rocjitsu run --profile mi350x -- ./train.py
rocjitsu: session s-20260730-191636-1f4a-0

# terminal 2
$ rocjitsu exec -- rocm-smi
$ rocjitsu exec -- bash            # an interactive shell, in *this* window
```
...
`rj_supervisor::build_specs` the run itself uses — so a command
...
Spawning here is what makes `rocjitsu exec -- bash` an interactive shell in
...
* `-s`/`--session ID` names the session. It is optional and usually
  omitted: with exactly one run live, rocjitsu picks it. With several, the
  ...
  with both positional, `rocjitsu exec -- bash` could equally mean "session
  `bash`".

  The candidates are runs that *answer*, not socket files. A `SIGKILL`ed
  run leaves its socket behind — the expected leak `rocjitsu cleanup` exists
...
* `--nproc-per-node N`, `--env KEY=VALUE`, `--clear-env-vars` and
  ...
  ```sh
  $ rocjitsu exec --node 1 -- bash                    # 1 process → interactive
  $ rocjitsu exec --node 1 --nproc-per-node 3 -- ./a  # 3 processes → captured
  [3] ...
  [4] ...
  [5] ...
  ```
...
```sh
$ rocjitsu exec -- sh -c 'echo RANK=$RANK LOCAL_RANK=$LOCAL_RANK WORLD_SIZE=$WORLD_SIZE'
[0] RANK=0 LOCAL_RANK=0 WORLD_SIZE=6
[3] RANK=3 LOCAL_RANK=0 WORLD_SIZE=6

$ rocjitsu exec --node 1 -- sh -c 'echo RANK=$RANK LOCAL_RANK=$LOCAL_RANK WORLD_SIZE=$WORLD_SIZE'
...
`rocjitsu exec -- rocm-smi` on a live job puts a process on rank 0 while
...
reading: **`rocjitsu exec` is for tooling that joins a job, not for adding
...
```text
rocjitsu: this command has finished, but 1 `rocjitsu exec` borrower(s) are
rocjitsu: still using session s-20260730-191636-1f4a-0
rocjitsu: waiting for them; Ctrl-C to tear the session down anyway
```
...
`rocjitsu run -- sleep 5` beside a longer `rocjitsu exec` pulled all of that
out from under a live job.

The wait is unbounded, because no timeout rocjitsu could pick would be
...
```text
error: no `rocjitsu run` is serving session s-20260730-191636-1f4a-0 (…).
...
`$XDG_CONFIG_HOME/rocjitsu/profile/<name>.json`.

```text
rocjitsu profile list [-l|--long]
rocjitsu profile show <name>
rocjitsu profile create [<name>] [--emulator NAME] [--agent NAME]
                      [--num-nodes N] [--gpus-per-node N]
                      [--description TEXT]
                      [--image IMAGE] [--mount SPEC]... [--port SPEC]...
                      [--container-provider PROV]
                      [--no-input]
rocjitsu profile import <file>          # use '-' for stdin
rocjitsu profile delete <name> [-f|--force]
```
...
* Neither `create` nor `import` will overwrite a profile that is already
  ...
  is an error naming the path, and `rocjitsu profile delete <name>` is how
  you say you meant it. An untouched builtin is the one exception: rocjitsu
...
```text
rocjitsu topology list
rocjitsu topology show <name>
rocjitsu topology create <name> [--agent NAME] [--num-nodes N] [--gpus-per-node N]
rocjitsu topology import <name> <file>   # use '-' for stdin
rocjitsu topology delete <name> [-f|--force]
```
...
```text
rocjitsu agent list
rocjitsu agent show <name>
rocjitsu agent import <name> <file>      # use '-' for stdin
rocjitsu agent delete <name> [-f|--force]
```
...
* **A prompt nobody can answer is an error**, not a "no". With stdin at
  end-of-file — `< /dev/null`, or a pipe that closed — rocjitsu reports
  that there is nobody to answer the question, names `--force`, and
  exits 1 without touching anything.
* **A piped answer still counts.** `echo y | rocjitsu profile delete x`
...
same thing again. It applies to every document rocjitsu reads — `import` on
all three kinds, and the profiles read by `run` and `exec`.

## `rocjitsu state`

```text
rocjitsu state builtins                 # (re)write builtin agents/topologies/profiles
rocjitsu state purge [-f|--force] [--all]
```

rocjitsu writes any *missing* builtin agent, topology and profile on every
...
wrote agent     mi300x -> /home/me/.config/rocjitsu/agent/mi300x.json
kept  agent     mi350x -> /home/me/.config/rocjitsu/agent/mi350x.json
wrote topology  MI350X-1x1 -> /home/me/.config/rocjitsu/topology/MI350X-1x1.json
…
rocjitsu: 1 builtin document differs from the one rocjitsu ships and was left alone:
  agent     mi350x -> /home/me/.config/rocjitsu/agent/mi350x.json
rocjitsu: rewriting one would discard your edits, and everything else was refreshed. To take the shipped version of one after all, delete it (`rocjitsu <kind> delete <name>`) and run `rocjitsu state builtins` again.
```
...
* Deleting an **untouched** builtin is refused. RocJITsu writes any missing
...
* It does everything `rocjitsu cleanup` does, below, and then removes the
  runtime directory. The config directory (profiles, topologies, agents)
  is left alone unless `--all` is given.
* It refuses while any `rocjitsu run` is live, and tells you which.
...
rocjitsu's at all, and then three things survive it:
...
`rocjitsu.owner`, `rocjitsu.session` and `rocjitsu.runtime` labels, and every
workload carries `ROCJITSU_SESSION` and `ROCJITSU_CLI_RUNTIME_DIR` in its
environment. Anything without those marks is never a candidate, so this
is safe on an engine shared with other work.

Safe to run at any time. A session whose `rocjitsu run` still answers on
...
would remove container rocjitsu-s-20260730-191636-1f4a-0-node-0 of session s-20260730-191636-1f4a-0 (podman)
would remove network rocjitsu-s-20260730-191636-1f4a-0 of session s-20260730-191636-1f4a-0 (podman)
would remove scratch directory /run/user/1000/rocjitsu/session/s-20260730-191636-1f4a-0
```
...
Anything rocjitsu meant to remove and could not is reported on **stderr**,
...
`ROCJITSU_CONTAINER_PROVIDER` names one engine deliberately and is then the
only one consulted.
...
`ROCJITSU_CLI_RUNTIME_DIR` are the entire registry of what is live there, so a
cleanup has no way to see the live sessions of a rocjitsu running under a
different one. Without the `rocjitsu.runtime` mark it would read those
healthy jobs as a crashed run's leftovers and `SIGKILL` them — which is
what makes two concurrent rocjitsu processes (a CI job beside an interactive
...
leaves behind is work started by a rocjitsu older than the mark: it is
...
inherited, so anything started from an interactive `rocjitsu exec -- bash`
...
Print the resolved rocjitsu directories. Useful in scripts and for verifying
that environment overrides took effect.

```sh
$ rocjitsu paths
config:     /home/me/.config/rocjitsu
runtime:    /run/user/1000/rocjitsu
profiles:   /home/me/.config/rocjitsu/profile
topologies: /home/me/.config/rocjitsu/topology
agents:     /home/me/.config/rocjitsu/agent
sessions:   /run/user/1000/rocjitsu/session
runs:       /run/user/1000/rocjitsu/run
```

`runs` is where each live `rocjitsu run` serves its socket, as
...
Print the rocjitsu version, copyright, and the third-party crates (with
...
`--help`/`--version` keep working), are left untouched. `rocjitsu --help`
...
Everything before the separator is read as rocjitsu's own flags and
everything after it as the workload, so a flag-shaped token before `--`
that `run` does not accept is a mistake rocjitsu reports rather than a
program to execute:

```sh
$ rocjitsu --nodes 2 -- ./app
error: unexpected argument '--nodes' found

Everything before `--` is read as rocjitsu's own flags and everything after it as the
command to run, so '--nodes' is a mistyped flag rather than part of the workload.

  tip: a similar flag exists: '--num-nodes'

Usage: rocjitsu [OPTIONS] -- <COMMAND> [ARGS]...
...
global flags plus everything `rocjitsu run` takes — aliases included — plus
`--attach`.

Two neighbouring shapes are diagnosed the same way, both exit 2:

* `rocjitsu --` with nothing after it, which asks for a workload and then
  names none.
* `rocjitsu ./app` — a `--` that was forgotten. Only when what you typed
...
| Variable                     | Purpose                                                          |
| ---------------------------- | ---------------------------------------------------------------- |
| `ROCJITSU_CLI_LOG`                 | Tracing-subscriber filter, e.g. `debug` or `rj_supervisor=debug`. Takes precedence over `-v`/`-vv`. |
| `ROCJITSU_CONTAINER_PROVIDER`  | Default container provider when nothing else names one (`podman`/`docker`/path). See below. |
| `ROCJITSU_CLI_CONFIG_DIR`              | Override the config dir (else `$XDG_CONFIG_HOME/rocjitsu`).         |
| `ROCJITSU_CLI_RUNTIME_DIR`             | Override the runtime dir — run sockets and emulator scratch (else `$XDG_RUNTIME_DIR/rocjitsu`). Also stamped on every workload; see below. |
| `XDG_CONFIG_HOME` / `XDG_RUNTIME_DIR` | Standard XDG base directories used when the `ROCJITSU_*` overrides are unset. |

rocjitsu sets these *in* every workload process, on every rank:
`ROCJITSU_RANK` (the node's rank, 0 = head), `ROCJITSU_HEAD_ADDR` and
`ROCJITSU_HEAD_PORT`, plus their `torch.distributed` aliases `RANK`,
...
It also sets `ROCJITSU_SESSION` and `ROCJITSU_CLI_RUNTIME_DIR`, which together say
which run a process belongs to. Both are resolved by rocjitsu and replace
whatever the caller exported, so a nested `rocjitsu` inside a session sees
the state directory of the run it is inside — and `rocjitsu cleanup` can
tell that run's processes from those of a rocjitsu using a different
...
`ROCJITSU_CLI_RUNTIME_DIR` is the session scratch at `/mnt/rocjitsu/runtime` and
`ROCJITSU_CLI_CONFIG_DIR` the config directory at `/mnt/rocjitsu/config`. Host-side,
such a session is still attributed correctly — the container carries the
`rocjitsu.runtime` label and the engine client process carries the host
path.

`ROCJITSU_CONTAINER_PROVIDER` is read as a *default*, and the order is
...
One place it does outrank everything: `rocjitsu cleanup` consults *every*
engine installed on the machine, but a set `ROCJITSU_CONTAINER_PROVIDER`
...
`rocjitsu` you are running is searched before them, so from an install
prefix that ships its own library only `ROCJITSU_LIB` or
`LD_LIBRARY_PATH` will redirect the search. `rocjitsu emulators -l` prints
...
| ---- | ------------------------------------------------------------------- |
| 0    | Success.                                                            |
| 1    | A rocjitsu-level error (bad arguments, resource not found, a session that failed to come up, no run serving the named session, a prompt nobody could answer, …). For `cleanup` and `state purge`, also: something they set out to remove is still there. |
...
asked and said no" (`2`) from "rocjitsu tried and could not" (`1`). See
[Confirmation prompts](#confirmation-prompts).

A **prompt that reaches end-of-file is an error**, exit 1, not a silent
"no". `rocjitsu profile delete x < /dev/null` used to take the immediate
