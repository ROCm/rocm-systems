...
zero physical GPUs, using rocjitsu as the UX over the rocjitsu emulator."
-->
...
flowchart LR
    ...
    A --> M["rocjitsu<br/>(UX · orchestration · process ownership)"]
...
| **rocjitsu** | The user-facing **UX**: one CLI that drives rocjitsu (and other backends) at scale. |

<!--
Key framing: rocjitsu is the engine, rocjitsu is the cockpit. The app never changes.
-->
...
| **Session** | The emulated cluster **one `rocjitsu run` holds in its own process**. It exists exactly as long as that command does. |
| **Exec** | A command started in a live run's session from **another terminal**, running in *that* terminal. |

> Flow: pick a **profile** → `rocjitsu run` it → optionally `rocjitsu exec` into it
...
$ rocjitsu profile create cdna4 --emulator rocjitsu --agent MI350X
created profile cdna4

$ rocjitsu run --profile cdna4 -- python3 train.py
rocjitsu: session s-20260616-191636-3b41-0
tiny_torch_ok
```
...
The session id on stderr is the only thing rocjitsu prints of its own.
-->
...
There is no server. `rocjitsu run` **is** the runtime: it brings a session up in
its own address space, runs your command, and takes the session with it when it
exits.

```mermaid
flowchart TB
    CFG[("$XDG_CONFIG_HOME/rocjitsu<br/>profiles · topologies · agents")]
    RUN["rocjitsu run<br/>owns the emulator, the containers,<br/>the workload — and its terminal"]
    SOCK[["$XDG_RUNTIME_DIR/rocjitsu/run/&lt;session&gt;.sock"]]
    EX["rocjitsu exec<br/>(another terminal)"]
...
RocJITsu used to have a supervisor daemon owning every session; before that, a
...
```mermaid
flowchart TB
    root["rocjitsu (root binary)"]
    root --> ctl["rj_ctl<br/>CLI verbs · run · exec"]
    root --> core["rj_core<br/>types · XDG paths · proto · traits"]
    ctl --> sup["rj_supervisor<br/>session/exec/process engine"]
    ctl --> builtin["rj_builtin<br/>embedded agents/topologies/profiles"]
    sup --> cont["rj_container<br/>podman/docker provider"]
    root -.link-only.-> rj["rj_backend_rocjitsu (+ dbt)"]
    root -.link-only.-> hot["rj_backend_hotswap"]
    rj --> sys["rocjitsu_sys (FFI)"]
```

Backends are **link-only** — they self-register via `inventory`. Turn a
Cargo feature on/off and the backend appears/disappears from `rocjitsu emulators`.
...
All three share the **same** rocjitsu UX. Switching is one flag:

```console
$ rocjitsu run --profile cdna4 --emulator rocjitsu-dbt -- ./app
```
...
flowchart TB
    subgraph daemonmode["DAEMON mode (the default)"]
      A2["workload + interposer"] -->|"Unix socket + SCM_RIGHTS"| DD["rocjitsu daemon<br/>dlopen'd into the rocjitsu run process<br/>(rocjitsu_sys FFI)"]
...
- **Daemon (default)** — the VM lives in the `rocjitsu run` process; every workload's
...
```console
$ rocjitsu topology create cdna4-2node --agent MI450X --num-nodes 2 --gpus-per-node 2
created topology cdna4-2node

$ rocjitsu topology show cdna4-2node | jq '{nodes:.num_nodes, gpus:.gpus_per_node, agent:.agent}'
{ "nodes": 2, "gpus": 2, "agent": "MI450X" }
```

A **profile** binds this topology to an emulator + options. `rocjitsu run` can
...
per node). One `rocjitsu run` owns all of it, and one rocjitsu daemon — hosted
inside that same process — serves every node's emulated GPUs.

```mermaid
flowchart TB
    subgraph proc["rocjitsu run — one process, one terminal"]
    ...
    SESS -->|"ROCJITSU_RANK=0 · HEAD"| N0["node 0 workload"]
    SESS -->|"ROCJITSU_RANK=1"| N1["node 1 workload"]
    SESS -->|"…"| NE["…"]
    SESS -->|"ROCJITSU_RANK=3"| N3["node 3 workload"]

    N0 -.->|"$ROCJITSU_RUNTIME_DIR/daemon.sock"| DAEM
    N1 -.-> DAEM
    N3 -.-> DAEM
    N1 -. "rendezvous: ROCJITSU_HEAD_ADDR/PORT" .-> N0
    N3 -. rendezvous .-> N0
```
...
```console
$ rocjitsu profile create super --emulator rocjitsu --agent MI350X \
      --num-nodes 32 --gpus-per-node 8
$ rocjitsu run --profile super -- ./all_reduce_perf -b 8 -e 1G
```
...
```mermaid
flowchart TB
    O["rocjitsu run<br/>(real host)"]
    O -->|"pull image"| IMG[("registry")]
    O -->|"create per-session network"| NET["rocjitsu-&lt;session&gt;"]
    O -->|"podman/docker run --rm (foreground child)"| C0["rocjitsu-&lt;session&gt;-node-0"]
    O -->|"podman/docker run --rm (foreground child)"| C1["rocjitsu-&lt;session&gt;-node-1"]
...
- Provider auto-detected: **podman** or **docker** (`--container-provider`, or
  `ROCJITSU_CONTAINER_PROVIDER`, to force).
- Containers are **not detached**. Each provider client is a child process rocjitsu
...
Every workload process is handed its place in the job. RocJITsu's variables are
...
flowchart TB
    ...
    RUN -->|"ROCJITSU_RANK=0<br/>MASTER_ADDR=localhost"| H0
    RUN -->|"ROCJITSU_RANK=1<br/>MASTER_ADDR=&lt;head&gt;"| H1
    H1 -.->|collectives| H0
```

| Variable | Meaning |
| -------- | ------- |
| `ROCJITSU_RANK` | This node's rank (0 = head). |
| `RANK` / `LOCAL_RANK` / `WORLD_SIZE` | Per-process torch/RCCL identity across the whole job. |
| `ROCJITSU_HEAD_ADDR` / `ROCJITSU_HEAD_PORT` | Where rank 0 lives. Mirrored as `MASTER_ADDR` / `MASTER_PORT` so `torch.distributed` initialises with no launcher. |
| `NCCL_HOSTID` | `rocjitsu-node-<rank>` — distinct per node, or RCCL rejects the emulated GPUs as duplicates. |
...
`rocjitsu exec` builds its process grid from the **same** `build_specs`, off the
...
$ rocjitsu run --profile cdna4 -- python3 tests/fixtures/ml/tiny_torch.py
rocjitsu: session s-20260616-191636-3b41-0
tiny_torch_ok
...
```console
$ rocjitsu run --profile cdna4 -- ./app > out.log 2> err.log
$ rocjitsu run --profile cdna4 -- bash          # and this is an interactive shell
```
...
# DEMO 2 — a second terminal: `rocjitsu exec`

```console
# Terminal 1 — hold a cluster up with a shell.
$ rocjitsu run --profile cdna4 -- bash
rocjitsu: session s-20260616-191636-3b41-0
bash-5.2$

# Terminal 2 — the run is the only one live, so no id is needed.
$ ls $(rocjitsu paths --json | jq -r .runs)
s-20260616-191636-3b41-0.sock

$ rocjitsu exec -- env | grep -E 'ROCJITSU_|ROCJITSU|LD_PRELOAD'
ROCJITSU_RANK=0
ROCJITSU_HEAD_ADDR=localhost
LD_PRELOAD=/.../lib/librocjitsu.so
ROCJITSU_RUNTIME_DIR=/run/user/1000/rocjitsu/session/s-20260616-191636-3b41-0

$ rocjitsu exec -- python3 probe_gpu.py       # runs in *this* terminal
```

`rocjitsu exec` asks the run one question (`Attach`), then spawns the process
...
```console
$ rocjitsu profile create cluster --emulator rocjitsu --agent MI450X \
      --num-nodes 2 --gpus-per-node 2
created profile cluster

$ rocjitsu run --profile cluster -- ./all_reduce_perf -b 8 -e 128M
rocjitsu: session s-20260616-193312-3b41-0
[0] #                                          out-of-place
...
```console
$ rocjitsu profile create boxed --emulator rocjitsu --agent MI350X --num-nodes 2 \
      --image rocm/dev-ubuntu:6.4 --container-provider podman
created profile boxed

$ rocjitsu run --profile boxed -- ./app &
rocjitsu: session s-20260616-200311-3b41-0

$ podman ps --format '{{.Names}}\t{{.Image}}'
rocjitsu-s-20260616-200311-3b41-0-node-0   rocm/dev-ubuntu:6.4
rocjitsu-s-20260616-200311-3b41-0-node-1   rocm/dev-ubuntu:6.4

# ...and when the run is gone:
$ podman ps -a --filter name=rocjitsu- --format '{{.Names}}'
$ podman network ls --filter name=rocjitsu- --format '{{.Name}}'
```

Each node is a container, connected by a per-session network, with the emulated
GPU served from the rocjitsu process outside them. Nothing is detached and nothing
...
A workload dies. rocjitsu gets out of the way and gives you the truth.

```console
$ rocjitsu run --profile cdna4 -- python3 crashy_kernel.py
rocjitsu: session s-20260616-201455-3b41-0
...
$ rocjitsu run --profile cdna4 -- python3 crashy_kernel.py 2> rj.log
$ grep 'arch mismatch' rj.log
[rj] code object arch mismatch: gfx942 != gfx950
```

The workload's exit code **is** rocjitsu's exit code (masked to a byte, so the
...
```console
$ rocjitsu run --profile cluster -- python3 long_train.py
rocjitsu: session s-20260616-203901-3b41-0
...
- Ctrl-C reaches **rocjitsu**, not the workload: children lead their own process
  groups, so the terminal's foreground group is the run alone.
- RocJITsu forwards the signal, then waits — so the workload gets its chance to
...
```console
$ rocjitsu run --profile cdna4 -- sh -c \
    'test -S $ROCJITSU_RUNTIME_DIR/daemon.sock && echo DAEMON_SOCKET_PRESENT'
DAEMON_SOCKET_PRESENT

$ rocjitsu run --profile cdna4 -- env | grep -E 'LD_PRELOAD|ROCJITSU'
LD_PRELOAD=/.../_rocm_sdk_devel/lib/librocjitsu.so
ROCJITSU_RUNTIME_DIR=/run/user/1000/rocjitsu/session/s-20260616-204410-3b41-0
```

- The rocjitsu VM is stood up **inside the rocjitsu run process** (in-process FFI
  via `rocjitsu_sys`), and every workload — every node, every `rocjitsu exec` from
...
Existing `rocjitsu` scripts keep working — rocjitsu is a drop-in.
...
$ rocjitsu --config cfg.json -- ./app --flag
# == rocjitsu run --config cfg.json -- ./app --flag
```
...
rocjitsu ships an E2E matrix that is the full cross-product of how teams debug:
...
rocjitsu testing matrix — 72 combinations
...
```console
$ rocjitsu profile create checked --emulator rocjitsu --agent MI350X
created profile checked

$ rocjitsu run --profile checked --plugin race -- ./my_kernel
...
- **rocjitsu** = the UX that runs it **at scale**: profiles, topologies, and one
  command that owns everything it starts.
- **`rocjitsu run` is the runtime** — the session lives in that process and dies
  with it. `rocjitsu exec` borrows it from another terminal.
- **Topology** models the rack; **containers** isolate each node; **rank + head**
  wiring (`ROCJITSU_RANK`, `MASTER_ADDR/PORT`, `WORLD_SIZE`) comes for free.
- **`[rank]` labels** appear automatically once a job has more than one
  process, so you always know which rank said what.

```console
$ rocjitsu profile create cdna4 --emulator rocjitsu --agent MI450X --num-nodes 2
$ rocjitsu run --profile cdna4 -- ./your-rocm-app
```
