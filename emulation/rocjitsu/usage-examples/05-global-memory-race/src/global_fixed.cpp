...
__global__ void sum_atomic(int *result, int *data, int N) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < N) {
    atomicAdd(result, data[idx]);  // SAFE: atomic operation
  }
}
...
int main() {
  ...
  sum_atomic<<<10, 100>>>(d_result, d_data, N);
  ...
  std::cout << (result == N ? "SUCCESS" : "FAILED") << std::endl;
