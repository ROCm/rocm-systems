...
struct RaceWavefrontState : WavefrontState {
  ...
  raceemulator::WaveRaceState *race_state = nullptr;
};

class RaceDetectionPlugin : public ExecutionPlugin {
public:
  RaceDetectionPlugin();
  ~RaceDetectionPlugin() override;
...
private:
  ...
  RaceWavefrontState *get_state(Wavefront &wf) {
    return static_cast<RaceWavefrontState *>(wf.plugin_state(slot_index()));
  }
  RaceWavefrontState *get_state(Wavefront *wf) { return wf ? get_state(*wf) : nullptr; }

  std::map<WorkgroupKey, std::unique_ptr<raceemulator::RaceDetector>> detectors_;
