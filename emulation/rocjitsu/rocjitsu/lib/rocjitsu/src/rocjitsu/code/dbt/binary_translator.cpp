...
TranslatedCodeObject BinaryTranslator::translate_once(const AmdGpuCodeObject &obj,
  ...
  // translation keeps the historical 128-VGPR headroom unless a retry proves
  // that a larger allocation is required.
  descriptor_options.minimum_vgprs =
      std::max(minimum_vgprs, kConservativeLoweringMinimumVgprs);
