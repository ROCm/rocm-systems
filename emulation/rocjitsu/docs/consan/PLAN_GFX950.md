...
- 2026-07-14: `X1H` and `X1I0` are `ACTIVE` in parallel with trap propagation.
  A separate Clang-21 hip-moi gfx1201 build is green and its baseline emulator
  run passes 42/46 registered tests. Three tutorial failures are reduced to an
  expect-failure helper that bypasses the cross-compiling emulator; one wide
...
- 2026-07-14: `X1BSC` is `DONE` and `X1BRR` is `ACTIVE`. The corrected
  SuperCollider broad gfx1201 sweep passes 209/209 in 175.42 seconds (183.30
  seconds including the Rocjitsu wrapper). Record/replay follows with the
  documented four-patch limit.
...
- 2026-07-14: `X1BSA` is `DONE` and `X1BIS` is `ACTIVE`. Sampled passes
  ...
  ownership.
- 2026-07-14: `X1BIS` is `DONE` and `X1BG` is reached. Inline-shadow passes
  209/209 with `RJ_CONSAN_MAX_PATCHES=1` and `hw_id` ownership in 178.32
  seconds (186.32 seconds wrapped). Thus the no-hook baseline and all four
  broad sanitizer profiles each pass 209/209 serially, with no waiver,
...
- `X1BSC`, `X1BRR`, `X1BSA`, and `X1BIS` (DONE): all four broad profiles pass
  209/209, and `X1BG` collects their accepted results.
