...
Start with [Core controls](#core-controls), [Presets](#presets), and
[Kernel allowlists](#generate-and-use-a-kernel-allowlist). Fine-grained settings
are grouped under [Expert controls](#expert-controls).
...
| Selection | Behavior with standard settings | Primary tradeoff |
| --- | --- | --- |
| default or `RJ_CONSAN_MODE=default` | Patch all admitted supported sites; use workgroup and LDS-cell strides of 256 with offset zero; retain bounded sampled causal windows and synchronization metadata. | Default mode: bounded retained state, lower overhead, and probabilistic detection. |
...
Start with the standard settings, choose a [preset](#presets) for a different
coverage/overhead trade-off in the default mode, and use an
[allowlist](#generate-and-use-a-kernel-allowlist) to focus on relevant kernels.
Ordinary runs do not need low-level register, buffer, or event overrides.
The hook logs
...
SuperCollider (clear any preset exported for the default mode):
...
| `RJ_CONSAN_PRESET=low\|default\|high\|max` | `default` | Tune coverage versus overhead for the **default mode only**; see [Presets](#presets). |
...
<a id="consan-presets"></a>

## Presets

`RJ_CONSAN_PRESET=low|default|high|max` currently tunes **only the default
mode** (formerly Sampled), not SuperCollider. It sets the default workgroup and
LDS-cell sampling strides. It does not change the analysis mode or
SuperCollider's behavior.

With the default mode, unset, empty, and `default` preserve standard behavior.
Names are case-insensitive; invalid names are rejected. With
`RJ_CONSAN_MODE=supercollider`, leave `RJ_CONSAN_PRESET` unset or empty:
any explicit nonempty preset, including `default`, is rejected.
...
Presets change only the two sampling strides. Unless explicitly overridden,
all eligible static sites remain instrumented, offsets stay zero, and report
capacity, synchronization tracking, epoch analysis, and diagnostic limits keep
their standard settings. These are
...
For finer tuning, explicit sampling controls override preset defaults; see
[ConSan event and sampling controls](#consan-event-and-sampling-controls).
`RJ_CONSAN_LOG=1` shows the preset and resolved settings.
...
Reuse the same generated file when comparing modes. To use SuperCollider,
set `RJ_CONSAN_MODE=supercollider` and leave `RJ_CONSAN_PRESET` unset or empty. The command clears the inline allowlist variable
...
Most runs need only the controls above. The following controls set resource
limits, change evidence collection, or support validation and debugging.
