...
| `moi_persistent_sgprs` | Placement; thirteen planning/emission files | Persistent scalar owner/epoch/workgroup ABI; scalar and per-owner vector allocations share the inseparable `ConSanMoiOwnerEpochRegisters` pair while workgroup refinements remain independent | Move into authoritative register/persistent-state allocation; loose persistent owner/epoch fields deleted in the fifth refactoring |
