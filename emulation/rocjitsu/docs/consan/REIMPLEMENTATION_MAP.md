...
| `moi_branch_only_spill` | Placement; Record/Replay, Sampled, Inline/barrier/prologue | Selected branch-only preservation alternative and its optional dynamic-stack borrowed pair | `ConSanMoiBranchOnlyScalarSpill`; independently representable flag and borrowed pair deleted during the fifth refactoring |
| `moi_dispatch_identity` | Placement, pipeline, prologue and emitters | Exclusive scalar/vector persistent dispatch representation, scalar-placement provenance, and component-private fallback policy | `ConSanMoiDispatchIdentityAllocation`; four loose fields and their ambiguous-state validation deleted during the fifth refactoring |
| `moi_persistent_sgprs` | Placement; thirteen planning/emission files | Persistent scalar owner/epoch/workgroup ABI; scalar, code-object-wide vector, per-owner vector, exact-coordinate emission, and relay views share the inseparable `ConSanMoiOwnerEpochRegisters` pair while explicitly named site-materialized temporaries may be partial | Move into authoritative register/persistent-state allocation; loose accepted persistent owner/epoch fields deleted in the fifth refactoring |
| `moi_exact_workgroup_vgprs` | Placement; candidate, prologue and access emission | Persistent exact-entry workgroup vector tuple shared by Record/Replay and Sampled, represented by the all-or-none `ConSanMoiPersistentWorkgroupTuple<T>` mechanism | Persistent-state allocation; loose per-coordinate optional storage and the old falsely Record/Replay-local name were deleted in the fifth refactoring |
| `moi_exact_workgroup_private_offsets` | Deleted from the code-object-wide operating point in the fifth refactoring; `ConSanMoiPrivateStateLayout::exact_workgroup_offsets` and planned-patch products carry it only at the owning site | Site-local exact-entry workgroup tuple shared by Record/Replay and Sampled, represented by the all-or-none `ConSanMoiPersistentWorkgroupTuple<T>` mechanism | Explicit private-layout binding passed to mode-local emission; the architecture gate prevents restoration of the broad field or Record/Replay-local terminology |
