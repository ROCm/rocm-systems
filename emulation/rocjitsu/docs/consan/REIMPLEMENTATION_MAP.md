...
| `moi_persistent_sgprs` | Placement; thirteen planning/emission files | Persistent scalar owner/epoch/workgroup ABI; scalar, code-object-wide vector, per-owner vector, Record/Replay emission, and relay views share the inseparable `ConSanMoiOwnerEpochRegisters` pair while explicitly named site-materialized temporaries may be partial | Move into authoritative register/persistent-state allocation; loose accepted persistent owner/epoch fields deleted in the fifth refactoring |
| `moi_record_replay_workgroup_vgprs` | Placement; candidate, prologue and access emission | Persistent replay workgroup vector tuple, represented by the shared all-or-none `ConSanMoiPersistentWorkgroupTuple<T>` mechanism | Persistent-state allocation; loose per-coordinate optional storage deleted in the fifth refactoring |
| `moi_record_replay_workgroup_private_offsets` | Deleted from the code-object-wide operating point in the fifth refactoring; private layout and planned-patch products carry it only at the owning site | Site-local replay workgroup private tuple, represented by the shared all-or-none `ConSanMoiPersistentWorkgroupTuple<T>` mechanism | Explicit private-layout binding passed to record and Sampled emission; the architecture gate prevents restoration of the broad field |
