...
| `moi_record_replay_workgroup_private_offsets` | Placement; barrier, record, Sampled and candidate paths | Persistent replay workgroup private tuple, represented by the shared all-or-none `ConSanMoiPersistentWorkgroupTuple<T>` mechanism | Persistent-state allocation; loose per-coordinate optional storage deleted in the fifth refactoring |
