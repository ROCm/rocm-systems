...
defaults to 128 MiB and can be explicitly raised up to 1 GiB. Aggregate live
...
| Variable | Default | Meaning |
| --- | --- | --- |
| `RJ_CONSAN_AUTO_REPORT_BUFFER_SIZE=N` | 128 MiB | Expert cap up to 1 GiB for HSA-tool-owned allocation; ordinary inventory still requests exact bytes below the cap. `0` disables automatic allocation. |
