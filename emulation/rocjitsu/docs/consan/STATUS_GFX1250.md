...
| P2 | `torch.linalg.vector_norm` and large-row `torch.softmax` | 🟩 Exact norm/softmax; 4,756/4,756 accesses; current paired 315.57x | 🟧 Current Rocjitsu clean run exceeds 600 seconds; prior full bundle retained | 🟨 Exact norm/softmax; 4,626/4,756 accesses and complete 4,482/4,482 barriers | 🟨 Exact norm/softmax; 4,254/4,756 accesses and 2,145/2,352 barriers; 1,022 contended publications | Current one-repetition artifacts use the corrected software-GPU environment.  Clean-tree paired SuperCollider artifact `consan-green-expansion-20260721-norm-softmax-supercollider-overhead-independent-011` accepts 108.98/35,541.53/116.27 ms and repeats the exact oracle with complete access coverage.  Reviewed fault artifact `...-supercollider-fault-independent-010` applies exactly one whole barrier mutation, observes the precommitted no-diagnosis/pass-oracle result, and passes discovery plus exact target-dispatch health before and after.  Sampled artifact `...-sampled-independent-006` is dynamically complete with only 130 static access resource failures.  Original-object inspection attributes all 130 to five kernels that genuinely use `s104`, overlapping standard Sampled's code-object-wide dispatch-ID pair; closing them needs a per-owner identity fallback rather than a VGPR spill relaxation.  Commit `8d0366b5a8` closes the separate undersized-descriptor Inline object, which now patches all 1,172 accesses and 624 barriers; artifact `...-inline-lds-aperture-independent-006` shows its remaining dynamic loss comes from 1,022 bounded compare/exchange retries exhausted by cross-workgroup contention in a different large reduction object.  The Record/Replay Rocjitsu attempt `consan-revalidation-gfx1250-20260720-pytorch-norm-softmax-record-replay-clean-rocjitsu-038` reaches its 600-second bound.  Prior one-repetition Record/Replay clean artifact `352`, paired artifact `354`, and reviewed fault artifact `357` retain its historical full bundle. |
...
| Validation target | 🟦 Green expansion active | The registry resolves every gfx1250 workload artifact and the full doctor passes.  Current-tip revalidation retains 82 accepted runnable cells with one-repetition paired overheads, while 11 attempted runnable cells remain explicitly non-green and three green aggregate/survey indicators have no runtime denominator.  New bounded work is recorded in the matrix and `/tmp/CONSAN_GFX1250_REVALIDATION_PROGRESS.md`; no blanket four-profile acceptance is claimed. |
