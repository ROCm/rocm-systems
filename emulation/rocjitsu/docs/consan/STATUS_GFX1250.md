...
The progress colors deliberately use a short evidence ladder.  Blue means the
clean workload works and green means the entire acceptance bundle works.
Inventory, fault, resource, and freeze work is recorded in the cell text and
progress log without introducing more promotion colors.  There are no
intermediate promotion levels between blue and green.

- ⬜ **unseen:** not yet run or not yet supported;
- 🟨 **assessed / clean partial:** target-specific static inventory and a
  concrete executable-oracle plan exist, but the standard clean run is still
  pending; or the clean oracle passes with an identified coverage or semantic
  gap;
- 🟦 **clean complete:** standard-profile clean execution, independent oracle,
  and required static/dynamic coverage are accepted; all further work remains
  blue until final acceptance;
- 🟩 **accepted:** every required gate is retained at one frozen
...
- 🟧 **blocked:** instrumentation runs, but a current clean-oracle failure,
  timeout, false diagnostic, or other end-to-end blocker prevents the first
  progress rung;
- 🟥 **contradicted:** current evidence disproves a previously claimed rung;
- `N/A` only when a fresh gfx1250 inventory proves semantic absence and records
  a typed reason.
...
corpus revision `aa54cc8`.  A yellow row records target-specific assessment
and an executable proof plan, not acceptance.  Candidates enter the current matrix only after they have an
independent numeric oracle and a standard-profile clean run; they then follow
the same blue-to-green promotion contract as the existing workloads.
...
| P2 | Reduced `sk_sgemm_runtime_smoke` | 🟩 Exact numeric oracle; 640/640 accesses; paired and reviewed-fault bundles accepted | 🟩 Exact numeric oracle; 640/640 accesses; 22/22 barriers; 8/8 fences; paired and reviewed-fault bundles accepted | 🟩 Exact numeric oracle; 640/640 accesses; 40/40 barriers; paired and reviewed-fault bundles accepted | 🟧 Backend-dependent after legal 92 KiB group growth; no verdict | The shared Tensile wrapper now emits the machine-readable oracle result required by fault acceptance.  Current-tip paired artifacts `313`, `315`, and `317` accept.  Reviewed exact-one barrier-drop artifacts `314`, `316`, and `326` preserve the numeric oracle, complete coverage, and device health for SuperCollider, Record/Replay, and Sampled.  The Sampled campaign uses a later executed barrier pair after artifact `319` showed that the first reviewed pair destabilized the software device after preserving its oracle.  Inline artifact `085` segfaults immediately after legal resource growth, while an independent-software-backend cross-check accepts the same growth and remains compute-active to 120 seconds. |
...
| P2 | `005_sk_f8gemm_quick` | 🟦 Exact oracle; 1772/1772 accesses; paired bundle accepted; reviewed fault loses postflight health | 🟩 Exact oracle; 1772/1772 accesses; 44/44 barriers; 16/16 fences; full bundle | 🟩 Exact oracle; 1772/1772 accesses; 80/80 barriers; full bundle | 🟧 Patch analysis complete; launch fails after legal dispatch-resource growth | The focused regression proves that `ds_bpermute_b32` is not raceable LDS traffic.  SuperCollider clean artifact `224` supersedes stale pre-fix artifact `081`; current-tip one-repetition paired artifact `352` accepts both 44.0--44.5-second baselines and the 64.8-second complete instrumented run.  Reviewed exact-one attempt `353` reaches `device_lost` and fails postflight health, so the cell honestly remains blue and rotates.  Record/Replay paired artifact `332` and reviewed fault artifact `331` accept.  Sampled paired artifact `335` accepts in 213.7 seconds between 45.2- and 45.6-second baselines; reviewed exact-one barrier-drop artifact `334` accepts with the precommitted pass-oracle/no-diagnosis outcome and healthy containment.  Artifact `333` is retained as a rejected preflight-health attempt with no mutation.  Inline artifact `220` reaches the same software-backend resource-growth boundary as reduced SGEMM. |
| P2 | `006_sk_hgemm_quick` | 🟨 Exact oracle for one of two applicable objects; compute-active through 150 seconds | 🟩 Exact oracle; 8162/8162 accesses; 292/292 barriers; 80/80 fences; full bundle | 🟩 Exact oracle; 8162/8162 accesses; 544/544 barriers; full bundle | 🟧 Patch analysis complete; launch fails after legal dispatch-resource growth | SuperCollider artifact `225` reaches one of two applicable objects before its bound.  Record/Replay paired artifact `340` and reviewed fault `339` accept; early-barrier artifact `338` preserves the oracle but loses postflight health, while `337` is a zero-mutation preflight rejection.  Sampled paired artifact `342` accepts in 207.6 seconds between 128.7- and 123.3-second baselines; late-barrier reviewed fault artifact `341` accepts exact-one mutation, its pass-oracle/no-diagnosis policy, and healthy containment.  Inline artifact `223` reaches the established software-backend resource-growth boundary. |
...
| P2 | `019_spmm_f16_sb` closure | 🟨 Selected `ds_store_b16` representative; clean run pending | 🟩 Four exact orientations; 31,265/31,265 accesses; clean, paired, and reviewed-fault bundle accepted | 🟨 Selected `ds_store_b16` representative; clean run pending | 🟨 Selected `ds_store_b16` representative; clean run pending | Current-tip one-repetition artifact `consan-validation-gfx1250-tensile-spmm-f16-sb-rr-inferred-all-365` accepts all four exact numeric orientations in 166.3 seconds with complete static and dynamic analysis, 31,265/31,265 accesses, 1,141/1,141 barriers, all 35 `ds_store_b16` sites, and zero resource failures.  Paired artifact `consan-validation-gfx1250-tensile-spmm-f16-sb-rr-overhead-366` accepts the complete run between two exact baselines.  Frozen reviewed artifact `consan-validation-gfx1250-tensile-spmm-f16-sb-rr-fault-377` applies exactly one logical signal/wait mutation as two physical rewrites, preserves the exact oracle, observes the precommitted no-diagnosis outcome, retains 31,265/31,265 accesses and all 1,140 surviving barriers, and passes both health gates. |
...
| P1 | Collision-heavy `torch.scatter_reduce` (`sum`, BF16 and FP32) | 🟩 Exact collision sums; 23/23 accesses; full bundle | 🟩 Exact collision sums; 23/23 accesses; full bundle | 🟩 Exact collision sums; 23/23 accesses; full bundle | 🟩 Exact collision sums; 23/23 accesses; full bundle | Current-tip clean artifact `322` accepts all four profiles after making record visibility conditional for a workload with no applicable executed LDS events.  Paired artifacts `305`, `306`, `323`, and `324` accept both baselines and complete instrumentation.  Reviewed artifacts `329` and `330` record atomic-order and atomic-scope weakening as typed N/A in all four profiles: inventory proves the relaxed singleton reduction atomics have no synchronization edge to weaken. |
| P1 | `torch.histc` with a shared-memory-sized bin count | 🟩 Exact counts; 133/133 supported accesses; refreshed paired and reviewed-fault bundles accepted | 🟩 Exact counts; 175/175 accesses and 84/84 barriers; refreshed paired and reviewed-fault bundles accepted | 🟩 Exact counts; 175/175 accesses and 168/168 applicable barriers; refreshed paired and reviewed-fault bundles accepted | 🟩 Exact counts; 175/175 accesses and 84/84 barriers; refreshed paired and reviewed-fault bundles accepted | Current-tip clean artifacts `289`-`291` admit 42 relaxed LDS-atomic accesses while correctly typing their synchronization role as not applicable; every MOI engine is statically and dynamically complete.  Frozen paired artifacts `296`, `300`, `302`, and `308` accept both baselines and complete instrumentation.  Reviewed barrier-drop artifacts `299`, `301`, `303`, and `309` each apply exactly one mutation, fail the exact oracle, retain complete supported-access plus all surviving-barrier coverage, and pass containment health; Inline emits 60 diagnostics while the other profiles preserve their precommitted no-diagnosis outcomes. |
| P2 | `torch.linalg.vector_norm` and large-row `torch.softmax` | 🟧 Literal64 decode fixed; next object has 1,172/1,172 supported but 0 placed sites | 🟩 Exact norm/softmax; 4,756/4,756 accesses; 2,352/2,352 barriers; full bundle | 🟦 Exact norm/softmax; 4,263/4,756 accesses and 4,216/4,478 barriers | 🟨 Exact norm/softmax; 3,945/4,756 accesses and 4,095/4,704 barriers; dynamic gap 1,022 | Current-tip one-repetition Record/Replay clean artifact `352` accepts in 44.3 seconds with exact norm and CPU softmax oracles plus complete static/dynamic analysis.  Paired artifact `354` accepts both baselines and the complete instrumented run.  Reviewed fault artifact `357` applies exactly one barrier-pair mutation, preserves the exact oracles, matches its precommitted no-diagnosis result, covers 4,756/4,756 accesses and all 2,351 surviving barriers, and passes both health gates.  The old artifact `116` residual was entirely placement-missing; subsequent shared/dense dispatch and owner/scalar planning fixes close it without a row-specific implementation change.  Current accounting coalesces qualified signal/wait pairs into ordering events.  Inline evidence remains in `122`; Sampled in `118`. |
| P1 | PyTorch cluster synchronization | 🟨 Exact candidate executes; clean run pending | 🟩 Exact copy/sentinel oracle; 23/23 accesses; 2/2 cluster barriers; full bundle | 🟨 Exact candidate executes; clean run pending | 🟨 Exact candidate executes; clean run pending | Current-tip one-repetition artifact `consan-validation-gfx1250-pytorch-cluster-rr-barrieronly-363` accepts in 6.2 seconds with the exact tensor-copy and four-sentinel oracle, 23/23 accesses, both cluster barriers, and 72 visible barrier records with none dropped.  Paired artifact `consan-validation-gfx1250-pytorch-cluster-rr-overhead-364` accepts both exact baselines and the complete Record/Replay run, measuring 466.4 ms versus 483.8 and 467.0 ms.  Reviewed barrier-drop artifact `consan-validation-gfx1250-pytorch-cluster-rr-fault-370` applies exactly one logical pair mutation as two physical rewrites, preserves the exact oracle, matches its precommitted no-diagnosis outcome, and passes both health gates.  Conservative CFG pairing admits the signal/wait as one sequence; commit `fa3085347e` retains barrier tracking for barrier-only objects, and commit `f2798173d3` retains logical mutation plans through successful standalone composition. |
...
- 2026-07-20: The cluster and F16 sparse-closure Record/Replay rows each
  ...
  1,141/1,141 barriers between two passing baselines.  Both cells remain blue
...
- 2026-07-20: The first standard full-SGEMM Record/Replay run establishes a
  ...
  there through the 900-second bound.  The cell advances from yellow to orange
...
- 2026-07-20: Current-tip one-repetition norm+softmax Record/Replay artifact
  ...
  and resource-planning fixes.  The cell remains blue only for its paired and
...
- 2026-07-20: F8 quick-GEMM SuperCollider paired artifact `352` accepts both
  ...
  health.  The cell stays blue and rotates; the active campaign refocuses on
  Record/Replay.

- 2026-07-20: Promoted `torch.mode` Record/Replay from blue to green at commit
...
- 2026-07-20: Promoted histogram Record/Replay from blue to green.  Frozen
...
- 2026-07-20: Promoted histogram Inline from blue to green at the current
...
- 2026-07-20: Promoted P0 `torch.topk` Record/Replay from yellow to blue.
...
- 2026-07-20: Advanced P0 `torch.topk` SuperCollider to 160,752/160,848.
  ...
  now place; the blue cell's last 96 accesses are led by 84 sites lacking the
...
- 2026-07-20: Advanced P0 `torch.topk` SuperCollider from 159,514/160,848 to
  ...
  than retained as apparent coverage.  The cell remains blue with 628
  supported accesses still unplaced.

- 2026-07-20: Advanced P0 `torch.topk` SuperCollider within blue.  Unfiltered
...
- 2026-07-20: Promoted P0 `torch.mode` SuperCollider from yellow to green.
...
- 2026-07-20: Promoted `torch.sort` SuperCollider from yellow to green on
...
- 2026-07-20: Reused byte-LDS SuperCollider support on `torch.histc`.
  ...
  blue rather than green because two LDS atomic compare-store sites remain
...
- 2026-07-20: Promoted the P0 PyTorch/Triton tensor-descriptor SuperCollider
  cell from yellow to green.  The 23 missing support-object sites were
  ...
  green in all four columns while `XP3` remains blue/ACTIVE for the other
  PyTorch P0/P1 cells.
...
- 2026-07-20: Promoted the P0 PyTorch/Triton tensor-descriptor workload to
  ...
  corrects SuperCollider from blue to yellow: its six admitted descriptor
...
- 2026-07-20: Histogram validation now retains an accepted four-profile
  ...
  Record/Replay, Sampled, and Inline Shadow.  The cells remain blue until the
...
- 2026-07-20: Committed-tip histogram artifacts `186`-`188` confirm that the
  ...
  dynamic completeness but remain blue because 126 unqualified atomic
  ...
  and complete analysis.  Its cell stays blue only until fault, resource, and
  same-tip freeze evidence are refreshed.

- 2026-07-20: Promoted histogram Inline Shadow from blue to green.  Same-tip
  ...
  blue on their separately typed atomic-qualification blocker.
...
- 2026-07-20: Isolated the remaining `torch.sort` SuperCollider denominator.
  ...
  blocker, so the cell stays yellow while `XP3` advances another P0/P1 cell.
...
- 2026-07-19: The committed-tip `004_sk_mxf8gemm_tdm` Inline Shadow attempt
  ...
  correctness result.  The cell remains blue at 3/4 accepted profiles; work
...
- 2026-07-19: Completed the first large Stream-K kernel under all four clean
  ...
  across spill-backed instrumentation.  The grouped P1 cell remains blue only
...
- 2026-07-19: Advanced the first large Stream-K kernel to 3/4 clean profiles.
  ...
  sole remaining clean blocker for `001`; the aggregate row is now blue.
...
- 2026-07-19: Promoted `torch.mode` from unseen to clean-partial.  The new
  ...
  later instrumentation failure.  The row remains yellow; no filter or
  denominator reduction is accepted as a shortcut.

- 2026-07-19: Promoted the PyTorch/Triton tensor-descriptor row from white to
  blue.  Artifact `consan-validation-gfx1250-pytorch-tdm-all-008` runs the
...
- 2026-07-19: Promoted all four TP2-family cells from blue to green with a
...
- 2026-07-19: Promoted all four TP1-prefill cells from blue to green with a
...
- 2026-07-19: Promoted all four Jakub cells from blue to green with a frozen
...
- 2026-07-19: Promoted Qwen Inline from blue to green with a frozen bundle at
...
- 2026-07-19: Promoted Qwen Inline from blocked orange to clean-complete blue.
...
- 2026-07-19: Promoted Qwen SuperCollider and Sampled from blue to green with
...
- 2026-07-19: Promoted Qwen Record/Replay from blue to green with a frozen
...
- 2026-07-18: Promoted all four WMMA-attention cells directly from blue to
...
- 2026-07-18: Promoted all four WMMA-attention cells from yellow to blue at
...
- 2026-07-18: D128-pressure contained barrier-drop execution reached all four
  ...
  the acceptance rerun.  The row remains blue until that fresh fault run and
  the resource/frozen-revision gates pass.

- 2026-07-18: Promoted all four D128-pressure cells from yellow to blue at
...
- 2026-07-18: Promoted all four D128-block cells from yellow to blue at
...
- 2026-07-18: Promoted all four tree atomic-OR cells from blue to green at
...
- 2026-07-18: Completed tree's resource qualification while its cells remained
  blue.  Three-sample paired bundle
...
- 2026-07-18: Promoted all four tree atomic-OR cells from clean-partial yellow
  to clean-complete blue.  Fresh retained bundle
  ...
  freeze remain before any tree cell can advance beyond blue.
...
- 2026-07-18: Stream-K completes inventory and reviewed policy in all four
  blue cells.  Fresh same-state clean artifacts accept SuperCollider at `4/4`
...
- 2026-07-18: Both reviewed Stream-K fault families pass contained execution
  ...
  committed-revision rerun remain open, so the row remains blue rather than
  green.
...
- 2026-07-18: All four CLIP clean cells now have retained target-code
  ...
  cells only to blue; long-bound clean acceptance remains active.
- 2026-07-18: SuperCollider's CLIP inventory exposed 16 omitted
  ...
  before the independent workload oracle, so the cell remains blue while a
...
- 2026-07-18: Started the matrix-first CLIP campaign.  The complete doctor and
  ...
  static result.  This promotes only that active evidence cell to blue;
  unexecuted profiles remain white.
...
- 2026-07-18: Ran the first retained validation row for D128 block attention.
  ...
  evidence is absent.  The four row cells therefore advance to active blue,
  not accepted green.
- 2026-07-18: Completed clean D128-pressure validation for baseline and all
  ...
  pressure cells are active blue rather than green.
- 2026-07-18: Clean WMMA-attention validation passes baseline and all four
  ...
  remain absent, so the four WMMA cells advance to active blue rather than
  green.
- 2026-07-18: Clean Stream-K arrival validation passes baseline and all four
  ...
  inventory.  All four cells are active blue pending that review and the rest
...
- 2026-07-18: Clean tree atomic-OR validation passes baseline and all four
  ...
  absence.  All target-native hip-moi clean rows are now active blue; fault,
...
- 2026-07-18: Sharktank TP1 prefill now passes its clean baseline and all four
  ...
  at 280/352 accesses, so all four TP1 cells are active blue rather than green.
- 2026-07-18: Retained TP1 SuperCollider validation at
  ...
  clean-complete in all four profiles, but every cell remains blue until its
...
- 2026-07-18: TP1 decode/combined now has retained clean evidence for baseline
  ...
  generic 30-second default.  These cells remain blue pending their complete
...
- 2026-07-18: TP2-family baseline and all four clean profiles pass at
  ...
  software-environment bound.  The four cells remain blue until the full
...
- 2026-07-18: The retained Qwen SuperCollider clean run at
  ...
  workspace ROCr v2 layout.  Both Qwen cells are active blue pending fault,
...
- 2026-07-18: Canonical Qwen Record/Replay artifact
  ...
  tests pass.  The Qwen Record/Replay cell is active blue pending fault,
  containment, performance, health, and frozen committed-tip promotion.
- 2026-07-18: Qwen Inline Shadow remains the last open clean P0 profile.
  ...
  1,200-second bound before producing an analysis verdict, so the cell is blue
...
- 2026-07-18: Commit `183904667d` replaces that serial clear with an exact
  ...
  executes the same aggregate lane/store work.  Qwen Inline remains blue, and
...
- 2026-07-18: Commit `036f987835` emits one 64-bit LDS store per local shadow
  ...
  blue until the next canonical run completes the same gates over all 508
...
- 2026-07-18: Replacement artifact
  ...
  Inline remains blue while a compact exact initialization representation is
...
- 2026-07-18: The initial Jakub profile artifact exposed 0/0 admitted accesses
  ...
  and no coverage rejection.  The cell is blue rather than green because
...
- 2026-07-18: Canonical committed-tip artifact
  ...
  admitted barrier denominator.  The four cells are blue pending reviewed
...
- 2026-07-18: Required-workgroup-size metadata now reaches the Inline local
  ...
  31 seconds versus the prior 32.20 seconds, so the Qwen cell stays blue: its
...
- 2026-07-18: The near-capacity local-mirror experiment at `11d5c0d009`
  ...
  dispatch.  The Inline cell remains blue: neither wider clearing nor external
...
- 2026-07-18: The packed-validity implementation now host-qualifies.  For the
  ...
  cell remains blue until focused and canonical execution prove correctness,
...
- 2026-07-18: A target-native two-workgroup runtime probe with the same
  ...
  verdict.  The Inline cell therefore remains blue.  Correctness of the lazy
...
- 2026-07-18: The four-byte exact local state now executes.  Its low 23 bits
  ...
  exceeds a diagnostic 300-second bound.  The Inline cell remains blue while
...
- 2026-07-18: The retained eager-compact acceptance artifact
  ...
  remains blue pending the isolated and canonical runs.
- 2026-07-18: Canonical compact-lazy artifacts `046` and `047` both exceeded
  ...
  intentionally pending, so this matrix cell remains active blue.
- 2026-07-20: Record/Replay no longer rejects `torch.sort` when a gfx1250
  ...
  advances from orange to blue; its remaining blocker is static/access
...
- 2026-07-20: The remaining `torch.sort` engine probes establish two more
  ...
  complete, promoting that cell to blue.  Sampled artifact `112` reaches a
...
- 2026-07-20: The P2 reduction row is now assessed in every engine.
  ...
  is dynamically complete, promoting that cell to blue.  Sampled `114` and
...
- 2026-07-20: Sampled reduction artifact `118` promotes its cell from orange
  to blue.  ConSan now reserves a legal high dispatch-ID pair and excludes
...
- 2026-07-20: A bounded high EXEC-save window now excludes only gfx1250 owner
  ...
  reduction artifact `122` therefore advances from orange to yellow: both
  ...
  below blue.  The matching orphan dense-host bug is fixed by retaining
...
- 2026-07-20: The same high-window fallback removes P0 `torch.topk`'s prior
  ...
  object and does not yet reach execution, so the cell remains yellow rather
  than claiming a clean verdict.
- 2026-07-20: Histogram Record/Replay artifact `124` promotes its cell from
  orange to blue with exact 64-bin counts, dynamic completeness, 112/133
...
- 2026-07-20: Histogram Inline artifact `127` promotes the fourth clean cell
  from orange to blue with exact counts, dynamic completeness, 112/133
...
- 2026-07-20: The scatter-reduce row did not need reshaping: fresh artifacts
  ...
  to blue; Record/Replay and Inline remain yellow because they emit no visible
...
- 2026-07-20: Unrestricted SuperCollider top-k artifact `133` promotes the P0
  cell from orange to blue.  Both the FP64 spill case and BF16 coverage case
...
- 2026-07-20: Bounded SuperCollider SPMM-F8 artifact `135` completes the first
  ...
  partial evidence while keeping the cell yellow until the complete runner
  reaches a verdict.
- 2026-07-20: Record/Replay SPMM-F8 artifact `136` advances the corresponding
  cell from pending yellow to a concrete orange correctness blocker.  Six
...
- 2026-07-20: SPMM-F8 Record/Replay advances from orange to blue after
  ...
  analysis before the 180-second bound.  The cell remains blue, not green,
  because the unrestricted workload has not yet completed.
- 2026-07-20: `torch.sort` Record/Replay advances from blue to green.  The
...
- 2026-07-20: The same disjoint-tuple fix promotes `torch.sort` Inline Shadow
  from blue to green without another source change.  Clean artifact `209`
...
- 2026-07-20: All four scatter-reduce cells advance from blue to green.
  ...
  `322`, `323`, and `324`.
- 2026-07-20: F8 quick-GEMM Record/Replay advances from blue to green.
...
- 2026-07-20: F8 quick-GEMM Sampled advances from blue to green.  Paired
  ...
  successful retry.
- 2026-07-20: HGEMM quick Record/Replay advances from blue to green.  Fresh
...
- 2026-07-20: HGEMM quick Sampled advances from blue to green.  Reviewed
