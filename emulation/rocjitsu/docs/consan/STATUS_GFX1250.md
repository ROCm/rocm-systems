...
| P2 | `torch.linalg.vector_norm` and large-row `torch.softmax` | 🟩 Exact norm/softmax; 4,756/4,756 accesses; current paired 315.57x | 🟩 Exact norm/softmax; 4,756/4,756 accesses and 2,352/2,352 barriers; current paired 435.17x | 🟨 Exact norm/softmax; 4,626/4,756 accesses and complete 4,482/4,482 barriers | 🟨 Exact norm/softmax; 4,254/4,756 accesses and 2,145/2,352 barriers; 1,022 contended publications | Current one-repetition artifacts use the corrected software-GPU environment.  Clean-tree paired SuperCollider artifact `consan-green-expansion-20260721-norm-softmax-supercollider-overhead-independent-011` accepts 108.98/35,541.53/116.27 ms and repeats the exact oracle with complete access coverage.  Reviewed fault artifact `...-supercollider-fault-independent-010` applies exactly one whole barrier mutation, observes the precommitted no-diagnosis/pass-oracle result, and passes discovery plus exact target-dispatch health before and after.  Record/Replay clean artifact `consan-green-expansion-20260721-norm-softmax-rr-independent-012` accepts the exact oracle with complete 4,756/4,756 access and 2,352/2,352 barrier coverage in 43.30 seconds.  Clean-tree paired artifact `...-rr-overhead-independent-013` accepts 96.46/42,534.82/99.02 ms, or 435.17x against the mean baseline, and repeats complete coverage.  Reviewed artifact `...-rr-fault-independent-014` applies exactly one whole barrier mutation, observes the precommitted no-diagnosis/pass-oracle result, retains complete surviving-site coverage, and passes both exact target-dispatch health gates.  Sampled artifact `...-sampled-independent-006` is dynamically complete with only 130 static access resource failures.  Original-object inspection attributes all 130 to five kernels that genuinely use `s104`, overlapping standard Sampled's code-object-wide dispatch-ID pair; closing them needs a per-owner identity fallback rather than a VGPR spill relaxation.  Commit `8d0366b5a8` closes the separate undersized-descriptor Inline object, which now patches all 1,172 accesses and 624 barriers; artifact `...-inline-lds-aperture-independent-006` shows its remaining dynamic loss comes from 1,022 bounded compare/exchange retries exhausted by cross-workgroup contention in a different large reduction object.  The slower-emulator Record/Replay attempt `consan-revalidation-gfx1250-20260720-pytorch-norm-softmax-record-replay-clean-rocjitsu-038` reaches its 600-second bound. |
| P1 | PyTorch cluster synchronization | 🟩 Exact oracle; 23/23 applicable accesses; current paired 1.01x | 🟩 Exact oracle; 23/23 accesses and 2/2 barriers; current paired 1.01x | N/A Fresh run types both barrier events unsupported: no preceding supported causal access window | 🟩 Exact oracle; 23/23 accesses and 1/1 barrier; current paired 0.94x | Current one-repetition SuperCollider artifact `consan-green-expansion-20260721-cluster-sc-independent-015` accepts the exact cluster copy/barrier sentinel oracle with complete applicable access coverage in 5.83 seconds.  Clean-tree paired artifact `...-cluster-sc-overhead-independent-018` accepts 465.30/475.21/474.44 ms, or 1.01x against the mean baseline, and repeats complete coverage.  Current inventory `...-cluster-inventory-independent-019` retains the reviewed barrier pair; artifact `...-cluster-sc-fault-independent-020` applies exactly one whole mutation, observes the precommitted no-diagnosis/pass-oracle outcome, and passes the exact cluster target-dispatch health smoke before and after.  Record/Replay clean artifact `...-cluster-rr-independent-021` accepts the same exact oracle with complete 23/23 access and 2/2 barrier coverage.  Paired artifact `...-cluster-rr-overhead-independent-022` accepts 470.89/477.61/473.79 ms, or 1.01x, and reviewed artifact `...-cluster-rr-fault-independent-023` applies exactly one whole mutation with the precommitted no-diagnosis/pass-oracle result and healthy target dispatch before and after.  Sampled artifact `...-cluster-sampled-independent-016` explicitly rejects both cluster barrier events because this barrier-only object has no supported causal access window; its unrelated applicable accesses remain complete at 23/23.  Commit `61f98b14f4` adds aggregate execution evidence for barrier-only Inline objects without changing access-bearing probes.  Clean artifact `...-cluster-inline-clean-independent-025` accepts the exact oracle with complete 23/23 access and 1/1 barrier coverage in 6.74 seconds.  Paired artifact `...-cluster-inline-paired-independent-027` accepts 499.49/470.13/500.53 ms, or 0.94x, and reviewed artifact `...-cluster-inline-fault-independent-028` applies exactly one whole mutation with the precommitted no-diagnosis/pass-oracle result and healthy target dispatch before and after.  The slower-emulator Record/Replay attempt `consan-revalidation-gfx1250-20260720-pytorch-cluster-load-sync-record-replay-clean-rocjitsu-038` reaches its 600-second bound.  Conservative CFG pairing admits the signal/wait as one sequence; commit `fa3085347e` retains barrier tracking for barrier-only objects, and commit `f2798173d3` retains logical mutation plans through successful standalone composition. |
| Survey | Cluster-memory and inter-workgroup synchronization from PyTorch | 🟩 Executable cluster-scope synchronization full bundle accepted | 🟩 Executable cluster-scope synchronization full bundle accepted | 🟨 Survey complete; executable candidate selected | 🟩 Executable cluster-scope synchronization full bundle accepted | Clustered placement was already covered.  The concrete row above supplies callable cluster-scope synchronization with accepted clean, paired, reviewed-fault, containment, and provenance evidence in SuperCollider, Record/Replay, and Inline Shadow.  Sampled freshly types this barrier-only signal as not applicable because no supported causal access window precedes it.  The current clustered-copy lowering uses ordinary global loads, so it is not claimed as evidence for a distinct cluster-memory opcode. |
...
| Validation target | 🟦 Green expansion active | The registry resolves every gfx1250 workload artifact and the full doctor passes.  Current-tip revalidation retains 86 accepted runnable cells with one-repetition paired overheads, while six attempted runnable cells remain explicitly non-green, one fresh cell is typed N/A, and five green aggregate/survey indicators have no runtime denominator.  New bounded work is recorded in the matrix and `/tmp/CONSAN_GFX1250_REVALIDATION_PROGRESS.md`; no blanket four-profile acceptance is claimed. |
