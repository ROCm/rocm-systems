...
| P1 | `016_spmm_tdm_all` | 🟩 1610/1610 accesses; current paired 1.15x | 🟩 1610/1610 accesses; 512/512 barriers; current paired 1.24x | 🟩 1610/1610 accesses; 494/494 barriers; current paired 1.19x | 🟧 Historical focused execution depended on simulator backing beyond the architectural 160 KiB LDS limit; current strict-capacity verdict and overhead are pending | Multi-type exact numeric matrix covers both supported transpose widths.  Uninstrumented artifact `consan-revalidation-gfx1250-20260720-tensile-spmm-tdm-all-baseline-ldsfix-061` passes every numeric oracle in 25.10 seconds.  Historical Inline artifacts `consan-revalidation-gfx1250-20260721-tensile-spmm-tdm-all-inline-shadow-densefix-062` and `-063` grew one instrumented workgroup to 175,104 bytes.  Their numeric progress is retained as debugging evidence but no longer classifies current support after removal of oversized simulator backing.  A new strict-160-KiB run must either select an external-shadow lowering or reject the transformation cleanly.  Artifact `consan-revalidation-gfx1250-20260720-tensile-spmm-all-all-nullfix-014` continues to accept the first three profiles. |
