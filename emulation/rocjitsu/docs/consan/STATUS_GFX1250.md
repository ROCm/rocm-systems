...
| **P1 Sharktank TP1 prefill** | 🟩 Current exact oracle with complete 352/352 access coverage | 🟩 Current exact oracle with complete 352/352 access plus 74/74 barrier coverage, zero diagnostics, and a complete dynamic verdict | 🟩 Current exact oracle with complete 352/352 access plus 64/64 applicable-barrier coverage | 🟩 Current exact oracle with complete 352/352 access plus 37/37 barrier coverage |
| **P1 Sharktank TP1 decode/combined** | 🟩 Current exact decode/combined oracles with complete 704/704 access coverage | 🟧 Both exact oracles pass and static coverage is complete at 704/704 accesses plus 148/148 barriers, but the production sampling stride selects no visible records and the dynamic verdict is incomplete | 🟩 Current exact oracles with complete 704/704 access plus 128/128 applicable-barrier coverage | 🟩 Current exact oracles with complete 704/704 access plus 74/74 barrier coverage |
| **P2 Sharktank TP2 family** | 🟩 Current split prefill, decode, and combined rows pass their exact oracles with complete 920/920 access coverage apiece; prior paired/fault evidence retained | 🟩 Current split rows pass all three exact oracles with complete 920/920 access plus 96/96 barrier coverage apiece and zero diagnostics; prior paired/fault evidence retained | 🟩 Current split rows pass all three exact oracles with complete 920/920 access plus 84/84 applicable-barrier coverage apiece; prior paired/fault evidence retained | 🟩 Current split rows pass all three exact oracles with complete 920/920 access plus 48/48 barrier coverage apiece; prior paired/fault evidence retained |
| **P4 hip-moi D128 block attention** | 🟩 Current exact host-reference oracles with complete 18/18 access coverage | 🟧 Current exact oracles and complete 18/18 access plus 8/8 barrier patches, but zero visible records at the production stride leave the dynamic verdict incomplete | 🟩 Current exact oracles with complete 18/18 access plus 8/8 applicable-barrier coverage | 🟩 Current exact oracles with complete 18/18 access plus 4/4 barrier coverage |
| **P4 hip-moi D128 pressure attention** | 🟩 Current four-oracle row with complete 24/24 access coverage | 🟧 Current four-oracle execution reaches a complete 24/24 access plus 8/8 barrier verdict, but produces 3,072 forbidden replay access-conflict diagnostics and exits 89 | 🟩 Current four-oracle row with complete 24/24 access plus 8/8 applicable-barrier coverage | 🟥 Current strict load rejects the object with `inline-shadow probe dense dispatcher exceeds its reserved relay space` before workload execution |
| **P4 hip-moi WMMA attention** | 🟩 Current exact host-reference oracles with complete 18/18 access coverage | 🟧 Current exact oracles and complete 18/18 access plus 8/8 barrier patches, but zero visible records at the production stride leave the dynamic verdict incomplete | 🟩 Current exact oracles with complete 18/18 access plus 8/8 applicable-barrier coverage | 🟩 Current exact oracles with complete 18/18 access plus 4/4 barrier coverage |
| **P4 hip-moi Stream-K arrival** | 🟩 Current exact clean row with complete 4/4 access coverage | 🟧 The exact workload and complete 4/4 access, 8/8 barrier, 10/10 atomic, and 16/16 fence patches run, but zero visible records at the production stride leave the dynamic verdict incomplete | 🟧 The exact workload runs with complete 4/4 access and 8/8 barrier coverage, but only 2/10 atomics are patched and the strict verdict is incomplete | 🟩 Current exact clean row with complete 4/4 access, 4/4 barrier, and 10/10 atomic coverage |
| **P4 hip-moi tree atomic-OR** | 🟩 Current exact clean row with complete 4/4 access coverage | 🟧 The exact workload and complete 4/4 access, 8/8 barrier, 10/10 atomic, and 16/16 fence patches run, but zero visible records at the production stride leave the dynamic verdict incomplete | 🟧 The exact workload runs with complete 4/4 access and 8/8 barrier coverage, but only 2/10 atomics are patched and the strict verdict is incomplete | 🟩 Current exact clean row with complete 4/4 access, 4/4 barrier, and 10/10 atomic coverage |
| **P4 Jakub cooperative matmul** | 🟩 Current four-oracle row with complete 70/70 access coverage | 🟧 All four exact oracles pass with complete 70/70 access plus 8/8 barrier patches, but zero visible records at the production stride leave the dynamic verdict incomplete | 🟩 Current four-oracle row with complete 70/70 access plus 8/8 applicable-barrier coverage | 🟩 Current four-oracle row with complete 70/70 access plus 4/4 barrier coverage |

### 2026-08-23 final current-tip correctness refresh

The publication-window refresh reran clean correctness and strict coverage,
without repeating retained overhead or fault campaigns, at
`/home/ossci/xx/consan-validation/final-refresh-20260823-gfx1250-v5`.
TP1 prefill and all three split TP2 modes accept all four engines. TP1
decode/combined accepts three engines but exposes a Record/Replay dynamic-
evidence regression. The six P4 rows expose a common small-workload
Record/Replay sampling gap, two Sampled ordered-atomic coverage gaps, a D128
pressure clean Record/Replay false positive, and a D128 pressure Inline Shadow
transform failure. Every other refreshed P4 profile accepts its exact oracle
and strict coverage contract. The matrix records those present failures rather
than carrying earlier green cells forward. These are correctness/acceptance
results, not new timing claims.

The separate Tensile Stream-K SGEMM Record/Replay cell remains yellow for the
current second-client clustered-dispatch segfault described below; the refresh
stopped after that required shard failed rather than rerunning the remaining
long shards or a fault campaign.
...
compute-active through 310 dispatches at 180 seconds, including a final
2,374-cluster output dispatch, but does not emit its exact oracle or teardown
verdict before the bound. This supersedes the old static counts but not the
cell's orange maturity: Sampled shares the same full-row emulator-throughput
return point and must not be converted into repeated unbounded waits.
...
SuperCollider does not share that throughput limit. The three split clean rows
pass exact prefill, decode, and combined oracles in 19.00, 67.75, and 50.74
seconds with complete 920/920 access coverage each. Independently bracketed
measurements record 0.99x, 1.00x, and 1.12x slowdown. A prospectively frozen
`detected/pass` trial at the later publication edge completed with the exact
oracle but no instability diagnosis or independent reach witness; it remains
rejected without relabeling.

A distinct prospective SuperCollider trial selected the earlier non-LDS
DPP-phase signal/wait at `.text+0x87b0/.text+0x87f8` and froze
`not_detected/pass`. Final-ISA review proves that pair dominates the later
`.text+0x90ac` store and `.text+0x90dc` load runtime witnesses in the executed
attention specialization. The accepted row applies exactly one mutation,
passes the exact prefill oracle with no diagnosis, retains complete 920/920
access coverage and all analysis/dynamic verdicts, allocates no report memory,
leaves zero live bytes, and passes both pre/post discovery and dispatch health
probes. The rejected stronger hypothesis and accepted qualified miss therefore
remain separate reviewed evidence.

The split Sampled rows complete instead of sharing that Record/Replay
throughput frontier. Prefill, decode, and combined accept their unchanged
exact oracles in 21.82, 70.17, and 55.08 seconds. Each row is statically and
dynamically complete at 920/920 accesses and 84/84 applicable barriers across
the two rank objects. Paired baseline-before/instrumented/baseline-after
measurements record 1.38x, 1.05x, and 1.16x slowdown respectively.

Fresh inventory also showed that the old reviewed TP2 mutation named a loaded
matmul specialization that prefill did not execute. The replacement is the
reached attention publication edge at `.text+0x90cc`: LDS stores immediately
precede its native `s_barrier_signal` / `s_barrier_wait` pair and peer LDS
loads follow it. Dropping that edge exactly once produces seven required
Sampled conflicts while the exact prefill oracle remains correct. The accepted
fault bundle retains complete 920/920 access and 82/82 surviving-barrier
coverage, bounded report memory with zero live bytes after cleanup, healthy
pre/post discovery and dispatch probes, and exact mutation accounting. A host
regression pins the reached prefill specialization, exact signal/wait identity,
`detected/pass` policy, and mandatory diagnostic so this cannot silently
regress to the old static-presence-only contract.

The three split Inline Shadow rows likewise complete: prefill, decode, and
combined pass exact oracles in 51.35, 91.57, and 94.77 seconds with complete
920/920 access and 48/48 barrier coverage each. Their independently bracketed
slowdowns are 3.57x, 1.41x, and 2.09x. The precommitted publication-edge
mutation applies exactly once, preserves the exact prefill result, and emits
one required Inline state-mismatch diagnosis. It retains complete 920/920
access and 47/47 surviving-barrier coverage, bounded 268,497,816-byte report
allocation with zero live bytes after cleanup, exact mutation accounting, and
healthy pre/post discovery and dispatch probes.

The mode-isolation readiness change exposes no new device-observable
synchronization semantic. `Gfx1250Fp16WmmaPipeline` owns the target-native
matrix/LDS publication shape, while the architecture-general
`DoubleBufferedPipeline` correct/incorrect pair owns this now-reached
publication contract on all five simulator targets and physical gfx950. The
cross-target heterogeneous-object and module-lifecycle pairs own independent
multi-object attribution and teardown. The host manifest/fault regressions are
...
| P2 | `000_sk_sgemm_quick` | 🟨 First problem: 12/12 exact numeric rows; 640/640 accesses; static/dynamic complete | 🟨 Exact-size sharding preserves both benchmark blocks. A current own-build-runtime refresh reconfirms the size-128 first client with 640/640 accesses, 44/44 barriers, 8/8 fences, zero diagnostics, and a complete verdict. Its second client is statically complete at 33,372/33,372 accesses, 1,782/1,782 barriers, and 322/322 fences but segfaults on its first clustered dispatch, before an oracle or teardown verdict. The campaign was stopped once this required shard failed; the other five shards and reviewed fault remain unqualified | 🟨 First problem: 12/12 exact numeric rows; 640/640 accesses; 40/40 barrier members | 🟧 First problem: 12/12 exact rows and complete static coverage; interrupted second problem leaves dynamic analysis incomplete | Six fail-closed exact-size shards replace the obsolete monolithic process while retaining both solution spaces. Record/Replay stays yellow until every shard plus paired/fault evidence is accepted. |
...
A final current-tip correctness refresh at
`/home/ossci/xx/consan-validation/prep-20260823-gfx1250-sgemm-rr-all-shards-own-runtime-v3`
resolved both HIP and HSA from `TheRock/build/dist/rocm`, eliminating an
initial mixed-runtime attempt from consideration.  The three small baselines
again pass both clients.  Under Record/Replay, the size-128 first benchmark
block again passes its exact oracle with complete 640-access, 44-barrier, and 8-fence
coverage.  The second benchmark block is statically complete at 33,372
accesses, 1,782 barriers, and 322 fences, then segfaults on its first clustered
dispatch immediately after growing private storage from zero to 120 bytes.
It therefore has no oracle or teardown verdict.  This is a current
correctness failure, not an overhead result.  Once that required shard failed,
the remaining bounded executions were stopped rather than consuming the final
publication window; no reviewed fault can qualify a failing clean row.  The
cell remains yellow, and the earlier accepted two-client artifact is retained
as historical rather than current-tip evidence.
