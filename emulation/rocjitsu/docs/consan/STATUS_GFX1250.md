...
| P0 | `torch.topk`, FP64 spill and BF16 coverage cases | 🟨 Exact FP64/BF16 values and indices; static and dynamic analysis complete at 160,956/160,956 accesses; paired and reviewed-fault refresh pending | 🟨 Exact FP64/BF16 values and indices; dynamically complete; 160,345/161,136 accesses and 11,423/11,423 barriers | 🟨 Exact FP64/BF16 values and indices; dynamically complete; 102,598/161,136 accesses and 15,182/15,182 barriers | 🟧 Both large objects finish patching; client signals during execution before an oracle | One-repetition SuperCollider artifact `consan-green-expansion-20260721-pytorch-topk-sc-all-supported-177` is accepted in 158.4 seconds, passes both exact FP64/BF16 value-and-index oracles, and is statically and dynamically complete at 160,956/160,956 accesses. Preplanned branch-only fallback closes four final placement gaps, signed 16-bit loads close eight admission gaps, and 96-bit store readback brings all 108 formerly unsupported accesses into the supported and patched denominator. The cell remains yellow only until paired-overhead and reviewed-fault evidence are refreshed at the frozen revision. Record/Replay artifact `consan-validation-gfx1250-pytorch-topk-rr-collapsed-364` passes both exact oracles in 308.3 seconds and leaves 791 `forbidden_overlap` resource failures across eight `gatherTopK` containers; exact-object artifact `consan-topk-rr-offline-dumps-047` localizes them. Sampled artifact `consan-green-expansion-20260721-pytorch-topk-sampled-scaled-clean-095` passes both exact oracles in 80.30 seconds, covers 102,598/161,136 accesses and all 15,182 barriers, and has no forbidden diagnostics or overflow. Inline artifact `consan-green-expansion-20260721-pytorch-topk-inline-bankfix-114` reproduces the execution signal at 116.7 seconds after both large transformations, independently of the fixed barrier-bank issue. |
