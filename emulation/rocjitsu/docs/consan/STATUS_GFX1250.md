...
- 2026-07-22: P0 `torch.topk` Sampled remains yellow after a current-tip
  one-repetition E2E at `b2e3405884`.  Artifact
  `consan-validation-gfx1250-pytorch-topk-sampled-spill-current-20260722-256`
  returns normally in 93.52 seconds and passes both exact FP64/BF16 value and
  index oracles.  Dynamic analysis is complete, but static coverage remains
  102,598/161,136 accesses and 15,182/15,182 barriers.  The newly available
  spill-backed Sampled scalar path emits no spill for these objects; 5,128
  owner-level `forbidden_overlap` failures and their dependent placement gaps
  remain.  This disproves the narrower scalar-only diagnosis, so the row
  rotates without another software-device run until the real owner resource
  shape has a focused reproducer.
...
- 2026-07-22: P0 TopK Sampled gains bounded owner-local scalar recovery.
  ...
  need true spill-backed Sampled scalar state; the yellow cell rotates rather
  than monopolizing the campaign.

- 2026-07-22: P3 SPMM F8 Record/Replay clean E2E is accepted at clean
  `ca92a75079` with rebuilt hook `983813d1...e878`.  One-repetition artifact
  `consan-green-expansion-20260722-spmm-f8-ml-rr-barrierfix-clean-210`
  completes all seven clients in 1203.95 seconds with exact numeric results,
  172,468/172,468 accesses, 3,060/3,060 barriers, complete static, analysis,
...
- 2026-07-22: P3 SPMM F8 Record/Replay advances within yellow.  Bounded
  ...
  clients and every exact numeric row in 1202.66 seconds, with complete
  172,468/172,468 access coverage.  It localizes the remaining gap to 190 of
  3,060 barrier sites in two late MT128x128x256 objects, all with typed
  `instrumentation_patch_missing` outcomes.  The run is localization-only
  because the repository and hook changed while it was alive.  The common
  dense barrier router incorrectly required one host to reach an entire large
  owner; barriers are now partitioned into sorted signed-branch windows.  A
  focused two-window regression and the complete 388-test `ConSanMoi.*`,
  304-test `ConSan.*`, and 47-test validator gates pass.  The cell remains
  yellow pending a clean rebuilt-hook E2E rerun and the remaining green gates.

- 2026-07-22: P3 SPMM F8 Sampled is green at committed tip.  Artifact
  `consan-green-expansion-20260722-spmm-f8-ml-sampled-overhead-final-clean-203`
  passes all seven generated clients with exact numeric results, complete
  ...
  230,928.55-ms baseline, a 4.88x workload-time slowdown.  Fresh inventory
  artifact `consan-green-expansion-20260722-spmm-f8-ml-sampled-inventory-final-204`
  finds 403 barrier sequences and 806 constituent sites.  Reviewed fault
  artifact `consan-green-expansion-20260722-spmm-f8-ml-sampled-fault-rotate3-final-208`
  precommits and applies exactly one barrier-to-NOP mutation at PC `0xb5904`.
  It accepts the qualified detector miss, exact passing numeric oracle,
  complete surviving 172,468/172,468 access and 6,118/6,118 barrier-member
  coverage, bounded cleanup, and healthy independent probes before and after.
