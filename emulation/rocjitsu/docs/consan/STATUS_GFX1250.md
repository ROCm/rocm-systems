...
| P1 | PyTorch cluster synchronization | 🟩 Exact oracle; 25/25 applicable accesses; current paired 1.02x | 🟩 Exact oracle; 25/25 accesses and 2/2 barriers; current paired 1.03x | 🟦 Exact oracle and clean-complete 25/25 accesses plus 4/4 barrier members; current paired 1.03x; reviewed cluster-barrier drop yields an invalid patched object and launch failure | 🟩 Exact oracle; 25/25 accesses and 2/2 barriers; current paired 1.24x | Commit `b9af8082f7` gives each of 512 cluster threads a distinct LDS element, creating a causal LDS store/barrier/load window without intentional duplicate writers.  Clean all-profile artifact `consan-green-expansion-20260721-cluster-causal-all-clean-independent-034` accepts the exact oracle with complete static and dynamic evidence in all four modes.  One-repetition paired artifact `...-cluster-causal-all-overhead-independent-035` accepts 469.59/466.49 ms bracketing baselines and measures 1.02x SuperCollider, 1.03x Record/Replay, 1.03x Sampled, and 1.24x Inline Shadow against their 468.04 ms mean.  Inventory `...-cluster-causal-inventory-independent-036` distinguishes the selected `barrier_id=-3`, cluster-scoped pair from a compiler-added workgroup pair.  Reviewed all-profile artifact `...-cluster-causal-fault-all-independent-037` requests, plans, and applies exactly that whole cluster pair once per profile and passes containment health before and after.  SuperCollider, Record/Replay, and Inline Shadow preserve the oracle and retain their green bundles.  Sampled rejects the mutated object during final validation and the workload reports HIP code 209 rather than executing, so its clean-complete evidence is blue and records a fault-composition gap rather than the superseded barrier-only `N/A`. |
| Survey | Cluster-memory and inter-workgroup synchronization from PyTorch | 🟩 Executable cluster-scope synchronization full bundle accepted | 🟩 Executable cluster-scope synchronization full bundle accepted | 🟦 Executable causal cluster synchronization is clean-complete; fault composition gap remains | 🟩 Executable cluster-scope synchronization full bundle accepted | Clustered placement was already covered.  The concrete row above now supplies a causal LDS access window around callable cluster-scope synchronization in all four modes.  Sampled clean and paired execution are complete, but composing the exact cluster-pair mutation with Sampled produces an invalid patched object, so its survey indicator remains blue.  The lowering still uses ordinary LDS and global instructions, so it is not claimed as evidence for a distinct cluster-memory opcode. |
