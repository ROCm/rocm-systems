...
| P2 | `torch.linalg.vector_norm` and large-row `torch.softmax` | 🟧 Literal64 decode fixed; 1,172/1,172 sites supported but historical placement gap remains | 🟧 Current Rocjitsu clean run exceeds 600 seconds; alternate software GPU exits before verdict; prior full bundle retained | 🟦 Exact norm/softmax; 4,263/4,756 accesses and 4,216/4,478 barriers | 🟨 Undersized-LDS report fix is unit-complete; exact E2E rerun pending | Current SuperCollider and Sampled attempts `consan-green-expansion-20260721-norm-softmax-supercollider-002` and `...-sampled-002` reach their 180-second bounds before an applicable PyTorch object; fixed-Inline artifact `...-inline-lds-aperture-002` does the same through 300 seconds.  Commit `8d0366b5a8` fixes the Inline root cause exposed by artifact `122`: a one-byte descriptor placeholder now receives the full external LDS aperture when decoded accesses prove it undersized.  Both focused report-sizing tests pass; current E2E execution has not yet reached that object, so the cell does not promote.  The Record/Replay attempt `consan-revalidation-gfx1250-20260720-pytorch-norm-softmax-record-replay-clean-rocjitsu-038` reaches its 600-second bound, while the alternate software-GPU cross-check exits before a verdict.  Prior one-repetition Record/Replay clean artifact `352`, paired artifact `354`, and reviewed fault artifact `357` retain the historical full bundle.  Sampled evidence remains in `118`; Inline evidence in `122`. |
