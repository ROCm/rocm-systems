...
| P2 | `torch.linalg.vector_norm` and large-row `torch.softmax` | 🟦 Exact norm/softmax; complete 4,756/4,756 accesses; provisional paired 290.43x | 🟧 Current Rocjitsu clean run exceeds 600 seconds; prior full bundle retained | 🟨 Exact norm/softmax; 4,626/4,756 accesses and complete 4,482/4,482 barriers | 🟨 Exact norm/softmax; 4,254/4,756 accesses and 2,145/2,352 barriers; 1,022 contended publications | Current one-repetition clean artifacts `consan-green-expansion-20260721-norm-softmax-supercollider-independent-006`, `...-sampled-independent-006`, and `...-inline-lds-aperture-independent-006` use the corrected software-GPU environment.  SuperCollider accepts in 39.65 seconds and closes its entire historical placement gap.  Provisional paired artifact `...-supercollider-overhead-independent-007` accepts 114.33/35,027.38/126.93 ms but records the documentation-only dirty tree, so a clean-tree freeze remains.  Sampled is dynamically complete with only 130 static access resource failures.  Commit `8d0366b5a8` closes the separate undersized-descriptor Inline object, which now patches all 1,172 accesses and 624 barriers; its remaining dynamic loss comes from 1,022 bounded compare/exchange retries exhausted by cross-workgroup contention in a different large reduction object.  The Record/Replay Rocjitsu attempt `consan-revalidation-gfx1250-20260720-pytorch-norm-softmax-record-replay-clean-rocjitsu-038` reaches its 600-second bound.  Prior one-repetition Record/Replay clean artifact `352`, paired artifact `354`, and reviewed fault artifact `357` retain the historical full bundle. |
...
| Validation target | 🟦 Green expansion active | The registry resolves every gfx1250 workload artifact and the full doctor passes.  Current-tip revalidation retains 81 accepted runnable cells with one-repetition paired overheads, while 12 attempted runnable cells remain explicitly non-green and three green aggregate/survey indicators have no runtime denominator.  New bounded work is recorded in the matrix and `/tmp/CONSAN_GFX1250_REVALIDATION_PROGRESS.md`; no blanket four-profile acceptance is claimed. |
