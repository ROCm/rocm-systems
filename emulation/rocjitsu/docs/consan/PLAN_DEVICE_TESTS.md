...
| Physical-gfx950 HipKittens BF16, FP8, and MXFP8, Record/Replay and BF16 Sampled | These kernels' DS readers consume fragments written by `buffer_load_dwordx4 ... lds`, whose architectural LDS destination is implicit in M0 and physical lane identity. A prospectively frozen reached BF16 barrier removal corrupted the exact result but initially emitted no diagnostic because those direct writers were absent from the inventory. The adjacent checked-in members retain the direct producer and peer DS reader and differ only by that publication barrier. | The pair contributes 20 green baseline/all-engine simulator rows on gfx942/gfx950 and 10 green physical-gfx950 rows. A host regression pins the documented CDNA3/CDNA4 operand and destination contract while rejecting unsupported encodings. Record/Replay E2E qualification preserves exact BF16, optimized/reference FP8, and 65,536-element MXFP8 results. BF16 covers 128/128 accesses and 32/32 barriers at 4.52x; FP8 and MXFP8 each cover 96/96 accesses and 5/5 barriers at 1.65x and 3.21x. Three separately prospectively reviewed reached barrier faults produce the required detector-owned diagnostics with complete surviving coverage, containment, cleanup, and health evidence. Current BF16 Sampled qualification independently covers all 128 accesses and 32 barriers at 1.47x, then emits 24 conflicts for the same exact-one phase-reuse deletion with 1,024/1,024 selected windows claimed and complete containment, cleanup, and health. The focused `CdnaDirectToLdsPublication` plus `CdnaMfmaPipeline` Sampled gate passes 12/12 rows; no duplicate fixture is warranted. |
