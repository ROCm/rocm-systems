...
The portable tier has 46 adjacent correct/incorrect all-engine pairs. Each
common pair runs as baseline plus all four engines on five RocJitsu targets
and physical `gfx950`, for 60 rows per scenario. Nine common engine-scoped pairs
are deliberately baseline plus one applicable engine: eight pressure/scale shapes
and the cross-queue dispatch-identity contract. Family and
target pairs run only where their semantic and ISA forms genuinely apply. The
current registered `consan-device` matrix is 3,465 tests: 2,878 simulator rows and 587
...
| Cross-queue kernel identity | Sharktank-derived batches launch distinct producer and consumer kernels on independent streams whose queue-local AQL packet IDs may coincide. The correct member proves that disjoint static kernel owners cannot create a false cross-dispatch conflict; the incorrect member removes only the consumer kernel's own publication edge and still requires the same-owner conflict. | Baseline and Record/Replay pass all 24 rows across five RocJitsu targets and physical gfx950. A VM regression pins each dispatch's own full-width AQL packet ID, and a host report regression keeps same-owner and missing-provenance conflicts fail-closed while accepting only complete disjoint owner sets. |
...
| FLAT atomic publication | Target-native FLAT release/add and return/acquire forms publish an LDS payload; the incorrect member retains both atomics but acquires an already-ready unrelated object | Paired and green in all 60 baseline/all-engine/backend rows. Disassembly retains CDNA3/4 `flat_atomic_add`, RDNA3 `flat_atomic_add_u32`, and RDNA4/CDNA5 instruction-scoped `flat_atomic_add_u32` forms; exact counters and controls remain independent oracles. |
...
| Sparse Record/Replay dense pipeline | Qwen and TP2-style kernels execute 642 static LDS sites, including 640 ordered warmup store/load sites, before a two-wave publication edge. The correct/incorrect members differ only by that edge, retain every exact warmup and control result for two workgroups, and run with a stride-two cached dispatch/workgroup predicate. | Baseline and Record/Replay contribute 24 green rows across all five RocJitsu targets plus physical gfx950. CDNA3/4, RDNA3, and RDNA4/CDNA5 use their target-native LDS and wait spellings. A five-target host regression proves that dispatch plus full x/y/z/cluster identity is mixed once at entry, while an RDNA4 host regression pins independent compact entries for both split-barrier members after 130 access sites. |
| Sparse Record/Replay spill pressure | A Qwen/Tensile-shaped two-workgroup object composes stride-two runtime selection, 130 ordered LDS access sites, broad scalar liveness, and a full odd-VGPR bank that forces 128 access probes onto spill-backed scratch. The gfx1250 form additionally keeps every ordinary scalar window live except one compact routing tuple, forcing entry selection to preserve borrowed guest scalar state. The correct/incorrect members differ only by the cross-wave publication edge and retain exact selected and rejected workgroup results. | Baseline and Record/Replay contribute 24 green rows across all five RocJitsu targets plus physical gfx950. The current 20-row simulator and four-row physical rechecks pass; gfx1250 is complete at 260/260 accesses and 4/4 barrier members while its exact kernel-argument-dependent outputs protect the compact entry window. CDNA3/4 retain all 128 scratch spill plans. A gfx1250 kernarg-preload host reduction plus a five-target entry-encoding regression protect the seven-register entry union without freezing the future sanitizer spill representation. |
| Dense branch-only Record/Replay pressure | The TopK-shaped companion keeps the complete portable ordinary SGPR bank live across 64 repeated LDS store/load sites while retaining full alternating-VGPR pressure, two waves, two workgroups, and the same stride-two selection. The correct member publishes the tile and checks every warmup, tile, and control value exactly; the incorrect member removes only that publication edge and requires the conflict. | Baseline and Record/Replay contribute 24 green rows across all five RocJitsu targets plus physical gfx950. The first cross-target run exposed insufficient long-corridor greedy budgeting, missing appended relay capacity through dense body regions, and an AMDHSA entry-backup bug that restored the raw dispatch-preload occupant over the guest-semantic workgroup SGPR. Focused host regressions cover the bounded route, semantic backup-slot override in both lane and memory representations, all five lane encodings, and full-site preservation across AMDHSA families without freezing the future spill implementation. |
...
| Physical-gfx950 MFMA attention, all four engines | The exact-context kernel combines MFMA/live accumulator state with repeated score publication through physical barriers. A reviewed exact-one removal of the first score-publication barrier retains the following semantic edge and therefore checks containment and qualified-miss bookkeeping without changing the race contract. | The current Sampled E2E bundle passes its exact oracle with 58/58 access and 12/12 applicable-barrier coverage at 2.57x, then accepts the prospectively frozen `not_detected/pass` mutation with 58/58 access and 11/11 surviving-barrier coverage. Inline Shadow passes 58/58 accesses, 14/14 barriers, and 8,704 visible exact-shadow cells at 3.32x, then accepts its prospectively frozen qualified miss with 13/13 surviving barriers. Both retain bounded memory, cleanup, provenance, and healthy pre/post probes; the Inline selector and policy are checked in and host-tested. `OrderedTileHandoff` owns the portable publication edge and `CdnaMfmaPipeline` transports it with native MFMA/AccVGPR pressure across CDNA3/CDNA4 and physical gfx950. The latter's 30 baseline/all-engine rows pass, and the publication idea is separately transported to every target. Adding a prototype-adjacent-barrier fixture would duplicate behavior rather than expand coverage. |
| Physical-gfx950 tree atomic-OR, Sampled and Inline Shadow | Three producer subgroups publish MFMA partials through release atomic-OR operations; the final subgroup polls the bitmask, performs a distinct acquire-release RMW, and consumes every producer's LDS partials. Weakening the producer-only release must be distinguished from the final acquire-release edge. | The source-matched clean Sampled row passes the exact MFMA-partials oracle with complete 48/48 access, 4/4 barrier, and 3/3 atomic coverage at 15.89x. A prospectively selected exact-one producer-release weakening at audited stride one preserves the numerical oracle and emits 12 Sampled conflicts. Inline Shadow passes 48/48 accesses, 6/6 barriers, and 3/3 atomics at 14.75x; the freshly inventoried producer-only weakening preserves the oracle and emits 768 required diagnostics with complete surviving coverage and bounded teardown. The checked-in selector/policy is host-tested. The adjacent `TreeAtomicOr` pair changes only the same producer-release edge, while `FlatAtomicPublication` executes native target-specific release/acquire forms. Both pairs require exact/no-diagnostic correct behavior versus the intended conflict, and all 120 baseline/all-engine rows pass across all five RocJitsu targets and physical gfx950. |
...
| Physical-gfx950 PyTorch `torch.mode`, Record/Replay | The former coarse-grained auto-report preference reduced host snapshot time but regressed multi-GPU gfx950 execution: device writes could fault even though the same path passed on a single-GPU host. The correctness fix restores the known-good fine-grained preference for every MOI engine, accepting the prior `torch.mode` latency until a topology-safe optimization is designed. | A focused host regression offers coarse and fine regions in both orders and requires fine selection for Record/Replay, Sampled, and Inline Shadow. Every real Record/Replay device row rejects nonzero coarse-grained snapshot bytes, so the allocation contract is exercised across all simulator targets and physical gfx950 rather than only modeled by the unit test. |
...
| Physical-gfx950 D128-pressure attention, all four engines | The full-KV FastContext loop contains consecutive workgroup barriers. Dropping exactly the first is a semantically redundant qualified miss because the adjacent second barrier preserves publication; all four engines must retain the pair in clean execution without inventing a conflict. A separate SuperCollider trial prospectively expected an ExactContext K/V-publication drop to fail, but physical scheduling masked it; that trial remains rejected without relabeling. | The architecture-general double-buffered-pipeline correct member retains two adjacent publication barriers, while its adjacent incorrect member removes the complete publication edge and requires the semantic conflict diagnostic. All 60 baseline/all-engine rows pass on RocJitsu gfx942, gfx950, gfx1100, gfx1201, and gfx1250 plus physical gfx950. The source-matched E2E bundles pass exact oracles with complete engine-applicable coverage, record 17.06x SuperCollider, 19.38x Record/Replay, 17.32x Sampled, and 23.17x Inline Shadow paired overhead, and independently accept prospectively reviewed exact-one redundant-barrier misses with complete mutation, bounded report memory, cleanup, health, and hook-hash evidence. The Inline fault selector and policy are checked in and host-tested. No duplicate prototype-shaped device pair is warranted because the existing pair owns both the correct adjacent-barrier contract and the incorrect missing-publication contract. |
| Physical-gfx950 Jakub FP16 attention matmul, all four engines | Pipelined and double-buffered MFMA kernels publish wide LDS fragments between eight producer/consumer waves, retire peer readers before overwriting a reused stage, and retain exact host-reference results. | All four source-matched E2E bundles pass their four exact oracles with complete engine-applicable coverage and 2.96x SuperCollider, 4.37x Record/Replay, 2.95x Sampled, and 3.18x Inline Shadow paired overhead. Three prospectively frozen Inline hypotheses remain rejected without relabeling: two positive initial-publication trials were schedule-masked and one later publication edge diagnosed under a negative policy. A fourth distinct precommitted reader-retirement drop is accepted with all exact oracles passing, one required Inline diagnosis, 338/338 access plus 34/34 surviving-barrier coverage, bounded cleanup, health, and exact provenance. `ReusedLdsGemmPipeline` owns the same reader-retirement-before-overwrite contract on all five RocJitsu targets and physical gfx950, while `CdnaMfmaPipeline` adds native CDNA3/CDNA4 matrix and accumulator pressure. Their combined 90 baseline/all-engine rows provide the adjacent correct/no-diagnostic and incorrect/required-diagnostic gate; the accepted target selector and policy also have focused host coverage. |
...
| Sharktank TP2 family, physical gfx950 and emulated gfx1250 | Six executed generated code objects combine prefill/decode attention with several matmul shapes. The prefill attention kernel has early DPP-phase synchronization that must be distinguished from its later semantic LDS publication edges; deleting the former exactly once is schedule-masked, while deleting an actual publication edge is the behavioral race the sanitizer must diagnose. | The physical-gfx950 Sampled bundle passes all three exact oracles with complete 1524/1524 access and 150/150 applicable-barrier coverage at a 1.69x maximum paired slowdown. Its prospectively frozen DPP-phase mutation is reached once and accepted as `not_detected/pass`, with 149/149 surviving barriers, 240 sampled windows, bounded teardown, and healthy pre/post probes. On gfx1250, the same denominator now uses three bounded source-faithful processes. All three SuperCollider clean/paired rows pass exact oracles with complete 920/920 access coverage and at most 1.12x slowdown; a stronger precommitted publication-detection trial remains rejected, while a distinct prospective DPP-phase qualified miss accepts with reviewed reach, exact output, containment, and health. All three Sampled clean/paired rows pass with complete 920/920 access and 84/84 barrier coverage and at most 1.38x slowdown. All three Inline Shadow clean/paired rows pass with complete 920/920 access and 48/48 barrier coverage and at most 3.57x slowdown; the precommitted reached publication drop produces the required Inline diagnosis while preserving the exact oracle, containment, health, and cleanup. Fresh runtime evidence rejected the old loaded-but-undispatched matmul site; the reviewed replacement at `.text+0x90cc` is now a shared Sampled/Inline behavioral fault, and a host regression pins the mode partition plus both selected-site policies. Prefill Record/Replay completely patches both rank objects but remains execution-bound, so that cell is not promoted. `DoubleBufferedPipeline` carries the actual correct/incorrect publication contract across all five simulator targets and physical gfx950, while `CdnaMfmaPipeline`, `Gfx1250Fp16WmmaPipeline`, and the portable object-lifecycle pairs retain the native matrix/resource and multi-object ideas. A device test whose incorrect member merely encodes the schedule-masked no-op or Python process lifetime would violate the behavioral-pair rule, so no duplicate fixture is warranted. |
...
The Sampled incorrect member now uses nine native waves against eight
identity banks, making a retained cross-wave collision deterministic rather
than probabilistic. Both adjacent members enable the in-kernel checker and
retain exact scalar checksums. This exposed and now guards a shared emitter
bug in which the checker overwrote the immutable guest-EXEC SGPR snapshot with
temporary VCC: a focused host test pins that register-state invariant, and all
twelve paired rows pass on the five simulated targets plus physical gfx950.
...
| Atomic diversity | Arrival add, tree OR, reusable generation-counter release/acquire, release CAS plus language-level acquire load, language-level release store plus acquire CAS, target-native FLAT release/add plus returning/acquire publication, integer/FP32/FP64 LDS histogram collisions, exact integer and relaxed scalar-BF16 global collision sums, native packed-BF16 global collision sums, MoE bins, cache-qualified VGLOBAL publication, and gfx12 instruction-scoped atomics are paired. | The FLAT pair preserves the CDNA3/4, RDNA3, and RDNA4/CDNA5 instruction families found in physical Stream-K/tree objects and gfx1250 fault manifests while changing only the acquired synchronization object. The FP64 histogram executes native `ds_add_f64` on CDNA3/4/5 and the compiler-selected LDS compare/exchange loop on RDNA3/4. The scalar BF16 payload uses the compiler-selected 32-bit CAS loop on every current target. The native packed-BF16 form runs on CDNA3/4/5 and RDNA4 and is typed N/A on RDNA3; a direct host regression independently covers packed FP16 and BF16 additions through both global and LDS collision paths. The arithmetic atomics remain exact oracles beside their pair's truthful LDS publication defect. |
...
| Scale | The registered 3,465-row matrix crosses 46 portable all-engine pairs, nine common engine-scoped pairs, focused family/target contracts, five simulator targets, physical CDNA4, and every engine. It includes cross-queue dispatch identity, FLAT-atomic publication, RCCL-style partial barriers, adjacent/strided dual-address B64 traffic, sparse access-dense Record/Replay with spill-backed VGPR pressure, complete ordinary-SGPR branch-only pressure, bounded many-owner Record/Replay pressure, bidirectional CDNA5 async data movement, native tensor-descriptor staging, runtime-indexed LDS aliasing, deterministic in-place state replay, heterogeneous objects, module and graph lifecycle, a ragged mixed-precision backward reduction, large LDS/deep pipelines, matrix variants, long-range live state, compiler-generated kernarg preloads, and clustered dispatch while staying checked in and bounded. | Maximum-capacity and production-sized heterogeneous objects remain E2E responsibilities; add another medium object pair only if a distinct concrete failure mode justifies it. |
...
ConSan flavors.
The 46 common all-engine pairs contribute 2,760 rows across the five RocJitsu
targets plus physical `gfx950`; nine common baseline/one-engine pairs add 216.
Non-rectangular family, target, and focused implementation regressions add 488
more rows, and the physical health check adds one, for 3,465 total.
...
| --- | ---: | ---: |
| RocJitsu `gfx942` | 63 | 566; dense SCC omits unsupported Record/Replay, while long-range/full-pressure, sparse-dense, sparse-spill, dense branch-only and many-owner pressure, cross-queue identity, dense-threshold, independent-scalar, and CDNA kernarg-preload contracts retain their declared engine scopes |
| RocJitsu `gfx950` | 65 | 586; the same engine restrictions apply, plus native transpose and 160-KiB LDS pairs |
| Physical `gfx950` | 65 | 587, including the health check and focused scalar-spill rows |
| RocJitsu `gfx1100` | 58 | 532 |
| RocJitsu `gfx1201` | 59 | 542 |
| RocJitsu `gfx1250` | 70 | 652 |
...
3,465 rows in **236.08 seconds** on this host. CTest reports **5,461.24 seconds
of summed test-process duration**; this is a parallelism-insensitive capacity
metric, not CPU time, because individual tests can wait on the simulator,
runtime, or physical GPU. The budget applies to this label-only device command.
Its 236.08-second result is
...
This measurement supersedes both the 3,369-row/234.11-second checkpoint and
the earlier 67.91-second result, which allowed
all physical gfx950 processes to compete for one GPU and eventually reproduced
a GPU memory fault. All 587 currently registered physical rows share the
...
- the simulator preloading every batched kernel with the queue producer
  frontier instead of that dispatch's own full-width AQL packet ID. A VM
  regression submits two packets before execution and requires preloads zero
  and one;
- Record/Replay report interpretation trusting a queue-local packet ID across
  independent queues without consulting static kernel ownership. The host
...
- sparse Record/Replay borrowing a guest scalar window whose four access-body
  ...
  under the standard stride. The planner and entry prologue now protect the
  full union on every supported family; a gfx1250 kernarg-preload host
  reduction, a five-target encoding regression, and the adjacent portable
  sparse-pressure device pair with target-native gfx1250 compact scalar
  pressure retain the semantic contract;
- compact RDNA4 Record/Replay reserving one entry per logical split barrier
  while lowering both signal and wait members. The access-heavy portable pair
  pushed the later bodies beyond direct SOPP reach and exposed the missing
  member slot; a focused 130-access host regression and the gfx1201 device row
  now require complete two-member lowering;
- CDNA3/CDNA4 Record/Replay choosing per-site returns for a compact dense
  stream whose access bodies all require spill save/restore. Body growth
  stranded later returns even though every original site could still reach a
  local entry. Dense spill-backed groups now use the shared return-PC route;
  a two-target host regression and the all-target Qwen-shaped sparse/spill
  pair require all 130 sites to survive independently of the spill ABI;
...
now has 46 all-engine common pairs and nine common engine-scoped pairs. In
addition to the earlier lifecycle, graph, replay, D16-high,
...
and Sampled owner-pressure contracts, added Qwen-shaped sparse spill-backed
and TopK-shaped full-SGPR branch-only Record/Replay pressure, transported the 160-KiB LDS pipeline to
...
four-CTA cluster topology, native K=32 FP16 WMMA fragment staging, and
cross-queue dispatch identity and bounded many-owner Record/Replay pressure. All 3,465 registered rows pass together after
the audit.

| Contract family | Applies and runs | Capability-based disposition elsewhere |
| --- | --- | --- |
| Forty-six common all-engine pairs | CDNA3/4/5 and RDNA3/4, plus physical CDNA4 | None; these are the rectangular semantic baseline. The newest members transport FLAT-atomic release/acquire publication, RCCL partial-barrier generations, Top-K prefix, full-low-bank Stream-K, large-text relay, live-SCC subword stores, tied-address/destination LDS reduction, and adjacent plus 4-KiB-separated dual-address B64 publication across target-native wave sizes, waits, barriers, and LDS spellings. |
...
**Implemented evidence:** 63 pair names produce 566 green RocJitsu rows. In addition
to the 55 common pair names (46 all-engine plus nine baseline/one-engine pairs),
...
**Implemented evidence:** 65 pair names contribute 586 green RocJitsu rows. Physical
gfx950 repeats the applicable contracts and contributes 587 rows including
...
**Implemented evidence:** 58 pairs produce 532 green RocJitsu rows. Beyond the
55 common pair names, `RdnaWmmaPipeline` cross-pollinates the gfx12 attention
...
**Implemented evidence:** 59 pairs produce 542 green RocJitsu rows. The target
...
**Implemented evidence:** 70 pairs produce 652 green RocJitsu rows. In addition
to the 55 common pair names, gfx1250 runs the wide group-FLAT contract, shares
...
The portable and target-specific tier is qualified in one combined 3,465-row
invocation. It includes the cross-target pressure transports and cross-queue
identity, FLAT-atomic release/acquire publication, RCCL-style partial barrier,
adjacent/stride-64 dual-address B64 publication, gfx1250
...
async-transfer additions from the transport audit. Forty-six common
all-engine pairs plus nine common engine-scoped pairs and family/target pairs
...
complete checkpoint passes all 2,878 simulator and 587 physical rows without
expected-failure exemptions in 236.08 seconds. That warm-run wall time remains
below the 5--20-minute review heuristic, so the evidence-driven residual-gap
audit remains open even though the matrix is green. Exact timing and
process-duration evidence is recorded above.
