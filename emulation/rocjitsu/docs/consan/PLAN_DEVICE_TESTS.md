...
| Physical-gfx950 PyTorch `torch.sort`, Record/Replay bounded replay | Dropping one reached float radix barrier corrupts lane-derived LDS offsets as well as the exact sorted values. The resulting trace contains ordinary conflict ranges and offsets near the 32-bit boundary; its dense scratch estimate is 1,073,741,824 entries. The old host reader skipped replay above one million entries, losing the very diagnostics needed for the incorrect workload. | Auto replay now keeps its one-million-entry memory bound but runs the sparse model fail-closed. A focused hook regression requires an overlimit record to produce a bounded replay, a metadata-full diagnostic, and an incomplete verdict rather than a skip. The same prospectively frozen E2E mutation changes from zero detections to four access-conflict plus six metadata-full diagnostics while retaining the original rejected `oracle=pass` disposition. The architecture-general `SegmentedTopK` adjacent pair already owns the device-observable radix-phase publication and missing-edge contract across all five RocJitsu targets plus physical gfx950; the new regression remains host-side because its distinct invariant is bounded post-device replay, not target ISA behavior. All 168 HSA-hook tests and all 296 simulator Record/Replay device rows pass. |
