...
| **P3 PyTorch native histogram** | 🟩 Exact oracle; clean-complete 135/135 ordinary LDS accesses; exact post-accumulation barrier drop is a precommitted qualified miss; overhead 0.966x | 🟧 Exact oracle and complete 135/135 ordinary LDS accesses; 42/84 supported barriers patch, with the rest lacking reachable entry islands | 🟧 Exact oracle and clean execution; complete 135/135 accesses + 86/168 barrier members patch, while unreachable barrier relays remain explicit coverage gaps | 🟧 Exact oracle and clean execution; complete 135/135 accesses + 43/84 barriers patch, while unreachable barrier relays remain explicit coverage gaps | Current shared-relay hook `2f91fcf1…`: Sampled `rdna4-histc-sampled-after-dense-relay` and Inline `rdna4-histc-inline-dense-relay-sized20` recover every ordinary LDS access without a clean diagnostic. Inline reserves a conservative 20-word dense-dispatch envelope; the initial undersized attempts failed closed before execution and are retained as `...after-dense-relay` and `...sized`. The remaining gaps are now isolated to barrier entry placement. SuperCollider hook `2dec0c65…`: clean `rdna4-histc-sc-final-clean`, paired overhead `...-final-overhead`, fresh inventory `rdna4-histc-final-inventory` (84 barrier sequences, 104 atomic sites), and accepted fault `...-final-fault-accepted`. The exact float32 post-accumulation barrier-pair removal applies once but is schedule-masked. Relaxed accumulations are not qualified MOI ordering events and correctly remain `atomic=0/0` |
...
| **D1** `test/test_reductions.py` histogram | 🟩 Promoted: exact oracle, clean 135/135, exact fault qualified miss, 0.966x | 🟧 Promoted: exact oracle, 135/135 accesses, 42/84 barriers | 🟧 Promoted: exact oracle, 135/135 accesses, 86/168 barrier members | 🟧 Promoted: exact oracle, 135/135 accesses, 43/84 barriers | Ordinary `torch.histc` is now in the main matrix. Its native object adds a real precompiled-library placement and relaxed-atomic difficulty class. All engines now instrument every supported ordinary LDS site; the remaining partial cells isolate barrier-relay placement. The relaxed atomics remain accesses rather than qualified memory-ordering events. The reviewed exact post-accumulation barrier drop is schedule-masked by this launch and retained as a qualified miss. |
