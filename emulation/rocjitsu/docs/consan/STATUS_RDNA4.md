...
certificate has advanced through validation checkpoint `3c599609cf`, using
rebuilt hook SHA-256
`78bb43d9638cff504db749b9ea225e251e52e3f1280a587d06147aca3e472ef7`.
...
|---|---|---|---|---|---|
| **D0** `test/test_sort_and_select.py` and `test/test_reductions.py` | 🟥 Promoted; typed 2,039/63,474 verdict in 54.12 seconds | 🟥 Promoted; semantic inventory and automatic allocation complete, all-site lowering exceeds 75 seconds | 🟥 Promoted; shared fat-object construction/lowering gate | 🟥 Promoted; shared fat-object construction/lowering gate | A decode-style `topk` over one 151,936-element Qwen vocabulary row with `k=50` is now in the main matrix. Its exact baseline passes. The native rocPRIM path loads two large operator objects and exposes both whole-code-object scalability and a dominant far-relay placement gap that the compact Inductor client does not. Record/Replay no longer has an untyped semantic-analysis or report-capacity blocker; checkpoint `3c599609cf` isolates the remaining host-side lowering cost. |
