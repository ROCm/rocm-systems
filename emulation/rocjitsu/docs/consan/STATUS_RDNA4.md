...
| **P4 hip-moi D128 pressure attention** | 🟥 RDNA4 `flat_store_b16` readback now encodes, but code-object patch planning exceeds the standard 30-second gate before an analysis verdict | 🟩 Exact oracle; clean 12/12 + 4/4; exact barrier drop emits an attributed diagnostic and breaks the oracle; overhead 14.30x | 🟥 Correct workload crashes before an analysis verdict | 🟩 Exact oracle; clean 12/12 + 4/4; exact barrier drop emits an attributed diagnostic; overhead 13.72x | SuperCollider discriminator `rdna4-ca1eb5456e-clean-d128-pressure-sc-127` isolates patch-planning time after fixing the prior 16-bit encoding error. Record/Replay checkpoint `baed32a85e`: `...-114`, `...-115`, `...-116`, `...-119`, and `...-124`. Inline checkpoint `79aea7420c`: `...-071`, `...-081`, `...-086`, and `...-090` |
| **P4 hip-moi WMMA attention** | 🟩 Exact oracle; clean 12/12; exact barrier drop is a precommitted qualified miss and breaks the oracle; overhead 158.07x | 🟩 Exact oracle; clean 12/12 + 4/4; exact barrier drop emits an attributed diagnostic and breaks the oracle; overhead 11.99x | 🟥 Correct workload crashes before an analysis verdict | 🟧 Complete 12/12 + 4/4 and one clean run passes, but two of three repeated correct-workload runs emit false diagnostics | SuperCollider checkpoint `ca1eb5456e`: clean `...-128`, paired overhead `...-129`, inventory `...-130`, and accepted fault `...-131`; RDNA4 `flat_store_b16` readback support removed the former patch-time invalid result. Record/Replay checkpoint `baed32a85e`: `...-114`, `...-115`, `...-116`, `...-120`, and `...-125`. Inline repeated attempt `...-078` retains its intermittent false positive |
