...
certificate has advanced through validation checkpoint `5950e01f4d`, using
rebuilt hook SHA-256
`ec6091b4894443830d6b8f27cbd8026569e4c81a21c7df23abfa3889654d89d3`.
...
| **P3 PyTorch native histogram** | 🟩 Exact oracle; clean-complete 135/135 ordinary LDS accesses; exact post-accumulation barrier drop is a precommitted qualified miss; overhead 0.966x | 🟧 Exact oracle and complete 135/135 ordinary LDS accesses; 42/84 supported barriers patch, with the rest lacking reachable entry islands | 🟨 Exact oracle; clean-complete 135/135 accesses + 168/168 barrier members; overhead 1.056x; reviewed effective fault pending | 🟧 Exact oracle and clean execution; complete 135/135 accesses + 43/84 barriers patch, while unreachable barrier relays remain explicit coverage gaps | Sampled checkpoint `5950e01f4d`, hook `ec6091b4…`: the gfx1250 dense barrier router now uses the equivalent gfx1201 `s_call_b64` encoding and admits large RDNA4 barrier sets. Committed clean artifact `rdna4-histc-sampled-dense-barrier-committed-20260722` closes all 82 former placement gaps at 135/135 accesses + 168/168 barrier members with no dynamic-incomplete event. Paired artifact `rdna4-histc-sampled-dense-barrier-overhead-20260722` measures 0.03921 ms Sampled versus a 0.03713 ms paired baseline (1.056057x). The synthetic ten-pair far-relay regression passes with all 724 ConSan and 20 ConSan hook tests. The gfx1201 reference spec has no histogram fault contract, so a freshly reviewed effective mutation remains the yellow→green gate. Current shared-relay hook `2f91fcf1…`: Inline `rdna4-histc-inline-dense-relay-sized20` recovers every ordinary LDS access without a clean diagnostic. Inline reserves a conservative 20-word dense-dispatch envelope; the initial undersized attempt failed closed before execution. Artifact `rdna4-histc-inline-qualified135-regression` reconfirms the exact 135-site edge of the automatic RDNA4 dense-routing qualification envelope with 135/135 accesses and 43/84 barriers. Record/Replay and Inline gaps remain isolated to barrier entry placement. SuperCollider hook `2dec0c65…`: clean `rdna4-histc-sc-final-clean`, paired overhead `...-final-overhead`, fresh inventory `rdna4-histc-final-inventory` (84 barrier sequences, 104 atomic sites), and accepted fault `...-final-fault-accepted`. The exact float32 post-accumulation barrier-pair removal applies once but is schedule-masked. Relaxed accumulations are not qualified MOI ordering events and correctly remain `atomic=0/0` |
