...
`🟦 N/A — RESOLVED EXCLUSION` is deliberately outside the ordered maturity
scale. Blue records a completed selection decision; it is not part of the
red-to-green instrumentation maturity sequence.
...
| **P2 PyTorch Qwen-vocabulary top-k** | 🟥 Exact values and indices pass and clean analysis reaches a typed verdict in 54.00 seconds, but only 2,039/63,474 supported accesses patch | 🟨 Exact values-and-indices oracle; clean-complete 63,474/63,474 accesses + 7,100/7,100 barriers in 90.04 seconds without diagnostics or tuning; overhead 1.537x; reviewed fault pending | 🟧 Exact values-and-indices oracle and clean execution pass without diagnostics or tuning; useful partial coverage reaches 57,153/63,474 accesses + 12,978/14,200 barrier members in 102.05 seconds; overhead and fault pending | 🟧 Exact oracle passes and useful partial execution reaches 38,365/63,474 accesses + 3,450/7,100 barriers in 99.84 seconds without tuning, but one clean diagnostic rejects the strict run | Record/Replay artifact `rdna4-topk-rr-compact-gate-clean-20260722`, hook `b9adbfb2…`, finishes in 87.77 seconds with the exact oracle, no diagnostic, dynamic completeness, and typed partial static coverage of 57,153/63,474 accesses plus 5,620/7,100 barriers. It uses the standard profile with no tuning. Current-hook paired artifact `rdna4-topk-rr-overhead-20260722`, hook `03155d04…`, measures 0.354279 ms instrumented against a mean-of-bracketing-medians baseline of 0.283589 ms (1.249270x); the runner's only rejection is the already-typed partial static coverage. Artifact `rdna4-topk-rr-function-access-relay-20260722`, hook `fdba6f32…`, first extends the liveness-checked relocatable dense host to owned device functions and closes all 6,321 function-access omissions. Current hook `3b14b804…` factors that multi-owner SGPR proof into shared placement and applies it to function-local barrier routers too. Clean artifact `rdna4-topk-rr-function-barrier-relay-20260722` is fully accepted at 63,474/63,474 accesses + 7,100/7,100 barriers with the exact oracle and no diagnostic. Paired artifact `rdna4-topk-rr-full-coverage-paired-overhead-20260722` measures 0.436617 ms instrumented against a mean-of-bracketing-medians baseline of 0.284115 ms (1.536764x), while retaining complete static and dynamic coverage. All 734 ConSan tests and 20 ConSan hook tests pass. Reviewed-fault evidence remains the only yellow-to-green gate. Current exact inventory `rdna4-topk-current-inventory-20260722` completes in 37.17 seconds and retains 5,000 stable barrier-pair identities for the 40 MiB rocPRIM object. A precommitted exact BF16 descending segmented-sort helper mutation is retained in rejected artifacts `rdna4-topk-rr-fault-discovery{-180s,}-20260722`: both the 120- and 180-second contained trials time out while transforming that object, before fault planning or GPU execution. This is a fault-composition scalability blocker, not evidence of an applied miss. Sampled checkpoint `551c284c40`, hook `f28c56eb…`, artifact `rdna4-topk-sampled-relay-scaled-final-20260722`: the committed trace-free hook exits zero with the exact oracle, no clean diagnostic, dynamic completeness, 90.04% supported-access coverage, and 91.39% supported-barrier-member coverage. The 23 MiB object is complete at 20,822/20,822 accesses + 4,200/4,200 barrier members; the 40 MiB object reaches 36,331/42,652 + 8,778/10,000, leaving only typed placement/lowering gaps. Lowering now reuses the pristine resource-planning CFG and indexes local relay-island searches by exact SOPP reach, with no user knob. Inline artifact `rdna4-topk-inline-indexed-final-20260722` uses the same committed hook and reaches a typed verdict rather than timing out: 60.44% of supported accesses and 48.59% of supported barriers patch, and the exact oracle passes, but an owner-1 store versus owner-4 load conflict over LDS `[4096,4104)` produces one visible clean diagnostic and the required strict exit 89. That false positive and incomplete placement keep Inline orange. The automatic report-sizing and instrumentation passes still repeat about 19 seconds of whole-object semantic analysis for the larger object; fault and paired-overhead evidence remain the Sampled orange-to-yellow gate. All 727 ConSan unit tests pass, and current compact Qwen regressions remain complete at 20/20 + 26/26 for Sampled and 20/20 + 14/14 for Record/Replay in `rdna4-topk-optimizer-qwen{-rr}-regression-20260722`. SuperCollider checkpoint `512c09ea0a`, artifact `rdna4-topk-sc-threshold-regression-65`, hook `2dec0c65…`, reconfirms exactly 941/42,652 and 1,098/20,822 patched accesses after the moderate-object relay fix. The two 23 MiB and 40 MiB objects retain preflight-candidate-only CFG construction, but 61,435 sites still lack reachable relay placement, far below usable coverage. Earlier Record/Replay checkpoints `3c599609cf` and `ff1afe848c` removed quadratic semantic and resource-planning searches and established automatic 128 MiB-bounded report sizing before the final dense-placement fixes. |
| **P2 PyTorch causal SDPA** | 🟩 Independent CPU oracle; clean 158/158; exact barrier drop emits an attributed diagnostic and breaks the oracle; overhead 1.946x | 🟨 Independent CPU oracle; clean-complete 158/158 accesses + 22/22 barriers + 2/2 atomics + 2/2 fences; overhead 7.795x; two reviewed exact drops break the oracle through unattributed hardware traps but emit no replay diagnostic | 🟥 Literal dispatch-ID fallback now removes the persistent-pair blocker, but the full-pressure attention kernel still has no safe transient scalar probe/router state; only the separate 27/27-access fill object patches | 🟥 Literal dispatch identity and gfx1201 transient-window preservation are now available, but this kernel has no common dead scalar pair for its indirect router; the attention object's 131 accesses + 22 barriers + 2 atomics remain unpatched and strict execution fails closed | Checkpoint `a4dfe2e49f`, hook `70d13b68…`: committed artifacts `rdna4-sdpa-{sampled-literal,inline-scalar-fallback}-committed-20260722` both reach a typed verdict in under 9.9 seconds without a knob. They prove that the hardware dispatch-ID pair is no longer load-bearing on gfx1201. Inline additionally proves that all 155 attention-object resource plans have spillable VGPR windows, but no scalar entry/return router state is dead across every site; fully spill-backed indirect routing is the remaining distinct capability, not ordinary VGPR spilling. All 728 ConSan and 20 ConSan hook tests pass. Current Record/Replay artifact `rdna4-sdpa-rr-safe-entry-return-20260722`, checkpoint `e49f6b0c33`, reconfirms exact static and dynamic completeness in 9.59 seconds. It also caught a real regression: cumulative cave growth pushed the owner/epoch entry prologue beyond SOPP reach, and the old long return reused a probe-dead SGPR pair that still held kernel-entry ABI state. RDNA-family entry returns now use entry-undefined VCC instead. Earlier checkpoint `8a5669d5e5` retains paired SC/RR overhead, inventory, and accepted SC fault artifacts. Rejected RR fault rows retain two distinct unattributed-trap outcomes. Current exact inventory `rdna4-sdpa-current-inventory-20260722`, hook `03155d04…`, completes in 7.25 seconds and retains all 22 pair identities. A bounded late-pair discriminator in `rdna4-sdpa-rr-late18-discovery-20260722b` applies no acceptable evidence because the mutated process times out at 30 seconds; the sweep stops there rather than treating a hang as detection. No user tuning knobs are used |
...
The fixed-bound top-k fault-composition follow-up
`rdna4-topk-rr-fault-reused-inventory-20260722`, hook `d81d0ce9…`,
eliminates the later redundant pristine apply pass.  It nevertheless spends
the full 180-second bound in the initial semantic-planning pass and publishes
no mutation counters.  This localizes the blocker to initial exact fault
planning rather than mutation application or replay execution.  It is not
evidence of an applied miss, and the timeout will not be raised again.

The current top-k Record/Replay artifact
`rdna4-topk-rr-compact-gate-clean-20260722` replaces the old 75-second
discriminator. Profiling identified and removed two more concrete quadratic
dense-relay searches: overlap queries now use merged occupied intervals and
barrier-host discovery visits only the owning kernel's CFG slice. The 40 MiB
object's barrier placement falls to about 1.8 seconds. Compact qualification
logs retain signed aggregate coverage and verdict records while verbose fault
inventory still retains exact identities, reducing this clean log from about
92 MiB to 12 KiB. A conservative RDNA4 dense-dispatch reservation also closes
the two-site spill-backed router failure exposed by the second 23 MiB object.
The current standard untuned run finishes in 90.04 seconds, passes the exact
values-and-indices oracle, emits no clean diagnostic, and patches all
63,474/63,474 accesses plus 7,100/7,100 barriers.  Extending the relocatable
dense host from kernels to owned device functions closes all 6,321 former
function-access omissions.  Reusing the same multi-owner liveness proof for
function-local barrier routers closes the remaining 1,480 barriers.  The
paired dispatch slowdown is 1.536764x.  No user knob is involved.  This is now
yellow rather than green only because a reviewed contained fault still cannot
finish its initial semantic-planning pass within the fixed 180-second bound.
...
|---|---|---|---|---|---|
| **D0** `llama-rdna4-mul-mat-vec-q` | **GREEN** 🟩 Independent CPU oracle; clean-complete 462/462 accesses; exact barrier drop is a reviewed qualified miss; 19.225x process overhead | **GREEN** 🟩 Independent CPU oracle; clean-complete 462/462 accesses + 44/44 barriers + 63/63 atomics + 72/72 fences; exact drop is a reviewed qualified miss and breaks the oracle; 14.934x overhead | **GREEN** 🟩 Independent CPU oracle; clean-complete 462/462 accesses + 88/88 barrier members; exact drop is a reviewed qualified miss and breaks the oracle; 17.548x overhead | **ORANGE** 🟧 Exact oracle passes; useful partial execution reaches 132/462 accesses + 18/44 barriers, but one clean diagnostic and 221,184 unsupported dynamic events reject the strict run | The canonical client now uses a 1,024-element embedding and `2e-2` clean GPU/CPU tolerance. The corpus's 128-element setup shape schedule-masked the reviewed barrier deletion and made Record/Replay's fault outcome nondeterministic; the larger shape remains compact, passes every clean profile oracle, and makes the exact `0xb8cc/0xb8d0` deletion reproducibly exceed the oracle bound by about 40 orders of magnitude under Record/Replay and Sampled. Record/Replay artifacts `rdna4-llama-matvec-1024-{rr-clean-final,rr-overhead-final,record-replay-fault-final2}-20260722`, hook `b9adbfb2…`, retain exact full coverage, five complete paired repetitions (2,455.813 ms versus a 164.440 ms bracketed baseline), one exact mutation, healthy containment, no claimed diagnostic, and the precommitted oracle failure under reviewed spec SHA `e0c24144…`. The same current shape is clean-complete for SuperCollider and Sampled in `rdna4-llama-matvec-1024-all-clean-20260722`; their paired artifacts measure 19.225x and 17.548x. SuperCollider's independently confirmed deletion remains schedule-masked, while Sampled uses the same effective reviewed spec as Record/Replay. Inline reconfirms its existing placement and false-positive gap at the current shape. No user control selects the shape or narrows instrumentation coverage. |
| **D0** `llama-rdna4-rms-norm` | **YELLOW** 🟨 Exact CPU oracle; clean-complete 22/22 LDS accesses; 1.194x process overhead; reviewed effective fault pending | **YELLOW** 🟨 Exact oracle; clean-complete 22/22 accesses + 11/11 barriers; 1.350x overhead; reviewed effective fault pending | **YELLOW** 🟨 Exact oracle; clean-complete 22/22 accesses + 22/22 barrier members; 1.404x overhead; reviewed effective fault pending | **YELLOW** 🟨 Exact oracle; clean-complete 22/22 accesses + 11/11 barriers; 1.375x overhead; reviewed effective fault pending | Current scripted clean and paired evidence is `rdna4-llama-rms-{all,overhead}-scripted-20260722`, hook `f28c56eb…`. All four engines pass the independent 128-element GPU-versus-CPU binary-output oracle without a diagnostic or user knob; every one of five overhead repetitions remains complete. The strong precommitted drop discriminator at checkpoint `e990e90858` removes the executed `0x1824/0x1830` pair exactly once for every engine, but rejected artifact `rdna4-llama-rms-all-fault-strong-20260722` stays bitwise correct and produces no diagnostic in all four trials. Follow-up inventory `rdna4-llama-rms-move-inventory-20260722` finds 262 legal move destinations only in the unexecuted `group_norm` kernel and none in the dispatched RMS specialization, so barrier move is not admitted as a misleading fault option. The earlier Inline retry-counter and dispatch-ID-zero fixes remain load-bearing. A reviewed fault with an observable semantic effect is still required for green. One bounded `1024x4` discriminator ruled out the shallow default-shape explanation: Record/Replay remained complete at 22/22 accesses + 10/10 barriers and the exact specialization-local pair deletion was still schedule-masked. Two additional exact-one Record/Replay discriminators, retained in `rdna4-llama-rms-rr-barrier-id-{cluster,trap}-proof-fixed-20260722`, rewrite the same executed pair from the anonymous workgroup barrier to the legal cluster (`-3`) and trap/workgroup (`-2`) barrier IDs. Both mutations apply, pass the strengthened machine-code proof, preserve the CPU oracle bitwise, and emit no diagnostic. This also fixed a general composition defect: nonadjacent signal/wait pairs are now admitted for every barrier mutation family, while final validation proves intervening pair bytes unchanged and still requires lifecycle members to be contiguous. Host-side evidence is 301/301 ConSan tests plus 65/65 validation-runner tests; the live discriminator used hook `361b0b70…`. |
...
|---|---|---|---|---|---|
| **D0** `test/test_sort_and_select.py` and `test/test_reductions.py` | 🟥 Promoted; typed 2,039/63,474 verdict in 54.12 seconds | 🟨 Promoted; exact clean execution, 63,474/63,474 accesses + 7,100/7,100 barriers in 90.04 seconds, no diagnostic or tuning; 1.537x overhead; reviewed fault pending | 🟧 Promoted; exact clean execution, 57,153/63,474 accesses + 12,978/14,200 barrier members, no diagnostic or tuning | 🟧 Promoted; exact oracle and useful 38,365/63,474 access coverage, but partial barrier coverage and one clean diagnostic | A decode-style `topk` over one 151,936-element Qwen vocabulary row with `k=50` is now in the main matrix. Its exact baseline passes. The native rocPRIM path loads two large operator objects and exposes both whole-code-object scalability and far-relay placement pressure that the compact Inductor client does not. Current Record/Replay patches all 63,474/63,474 accesses and 7,100/7,100 barriers and exits normally with the exact oracle; Sampled patches 57,153/63,474 accesses, and their barrier models use different denominators. Record/Replay's only remaining green gate is reviewed-fault evidence: paired overhead is 1.536764x, while exact fault composition does not complete within a fixed 180-second bound. The committed Inline artifact completes in 99.84 seconds with 60% access and 49% barrier coverage but exposes a clean-input causal conflict. |
