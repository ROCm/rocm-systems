...
All previously listed workloads have engine assessments. Production FP16
matmul remains gray because it was not part of the final five-workload
empirical corpus. The FP8 row now records the frozen physical-gfx1201 result
from [the empirical study](GFX1201_EMPIRICAL_STUDY.md). Coverage counts use each
engine's own admitted-site model; in particular, Sampled reports split-barrier
members where other engines report logical barriers.
...
|---|---|---|---|---|
| **P0 production FP16 RDNA4 matmul** | 🩶 Not assessed by the current empirical study | 🩶 Not assessed by the current empirical study | 🩶 Not assessed by the current empirical study | 🩶 Not assessed by the current empirical study |
| **P0 production FP8 RDNA4 matmul** | 🟥 Exact small-shape plus sampled 4096³ host oracle; rejected at 75/227 access sites | 🟥 Exact oracle; rejected because bounded dynamic history was incomplete | 🟩 Exact oracle; clean-complete at 227/227 accesses plus 48/48 barrier members; 4.72x GPU slowdown; reviewed barrier drop diagnosed 0/32 with 0/32 oracle manifestations | 🟥 Exact oracle; rejected at 215/227 access sites |
