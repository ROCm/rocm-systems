...
certificate has advanced through validation checkpoint `baed32a85e`, using
rebuilt hook SHA-256
`a271e9fd7341747e01afa1764675bcba4edba9ad33cae7413f4a61e6431eaba3`.
...
| **P4 hip-moi D128 block attention** | 🟥 Correct workload crashes before an analysis verdict | 🟩 Exact oracle; clean 12/12 + 4/4; exact barrier drop emits an attributed diagnostic and breaks the oracle; overhead 11.21x | 🟥 Correct workload crashes before an analysis verdict | 🟧 Complete 12/12 accesses + 4/4 barriers, but a diagnostic rejects the correct workload | Checkpoint `baed32a85e`: clean `...-114`, paired overhead `...-115`, inventory `...-116`, and accepted fault `...-118`; discovery mismatch `...-117` established the stronger replay behavior |
| **P4 hip-moi D128 pressure attention** | 🟥 Correct workload crashes before an analysis verdict | 🟩 Exact oracle; clean 12/12 + 4/4; exact barrier drop emits an attributed diagnostic and breaks the oracle; overhead 14.30x | 🟥 Correct workload crashes before an analysis verdict | 🟩 Exact oracle; clean 12/12 + 4/4; exact barrier drop emits an attributed diagnostic; overhead 13.72x | Record/Replay checkpoint `baed32a85e`: clean `...-114`, paired overhead `...-115`, inventory `...-116`, and accepted fault `...-124`; discovery mismatch `...-119` established the stronger behavior. Inline checkpoint `79aea7420c`: `...-071`, `...-081`, `...-086`, and `...-090` |
| **P4 hip-moi WMMA attention** | 🟥 Correct workload crashes before an analysis verdict | 🟩 Exact oracle; clean 12/12 + 4/4; exact barrier drop emits an attributed diagnostic and breaks the oracle; overhead 11.99x | 🟥 Correct workload crashes before an analysis verdict | 🟧 Complete 12/12 + 4/4 and one clean run passes, but two of three repeated correct-workload runs emit false diagnostics | Record/Replay checkpoint `baed32a85e`: clean `...-114`, paired overhead `...-115`, inventory `...-116`, and accepted fault `...-125`; discovery mismatch `...-120` established the stronger behavior. Inline repeated attempt `...-078` retains its intermittent false positive |
| **P4 hip-moi Stream-K arrival** | 🟩 Exact oracle; clean 4/4 accesses; exact order and scope weakenings are precommitted qualified misses; overhead 558.83x | 🟩 Exact oracle; clean 4/4 accesses + 15/15 atomics + 4/4 barriers + 16/16 fences; exact order and scope weakenings are precommitted qualified misses; overhead 53.00x | 🟥 Correct workload crashes before an analysis verdict | 🟧 Complete 4/4 + 15/15 + 4/4, but a diagnostic rejects the correct workload | Record/Replay checkpoint `baed32a85e`: clean `...-114`, paired overhead `...-115`, inventory `...-116`, and accepted exact faults `...-122` and `...-123`. SuperCollider checkpoint `79aea7420c`: `...-073`, `...-082`, `...-087`, `...-091`, and `...-092` |
| **P4 hip-moi tree atomic-OR** | 🟩 Exact oracle; clean 4/4 accesses; exact order and scope weakenings are precommitted qualified misses; overhead 591.81x | 🟧 Exact oracle and complete 4/4 accesses + 15/15 atomics + 4/4 barriers, but four supported fences fail placement at 12/16 | 🟥 Correct workload crashes before an analysis verdict | 🟧 Complete 4/4 + 15/15 + 4/4, but a diagnostic rejects the correct workload | Current Record/Replay discriminator `rdna4-baed32a85e-clean-p4-rr-114` isolates the remaining four `instrumentation_patch_missing` fences after ineligible fences stopped poisoning coverage. Other retained evidence is at checkpoint `79aea7420c` |
| **P4 Jakub attention variants** | 🟩 Exact oracle; clean 31/31 accesses; exact barrier drop is a precommitted qualified miss; overhead 103.75x | 🟩 Exact oracle; clean 31/31 + 4/4; exact barrier drop is a precommitted qualified miss; overhead 11.69x | 🟥 Correct workload crashes before an analysis verdict | 🟩 Exact oracle; clean 31/31 + 4/4; exact barrier drop emits an attributed diagnostic; overhead 11.03x | Record/Replay checkpoint `baed32a85e`: clean `...-113`, paired overhead `...-115`, inventory `...-116`, and accepted fault `...-121`. SuperCollider and Inline checkpoint `79aea7420c`: `...-076`, `...-084`, `...-085`, `...-089`, `...-096`, and `...-097` |
...
| **D1** `torch.compile` softmax selected from the reduction/softmax survey | 🟩 Promoted: clean 4/4; exact drop qualified miss; 0.960x | 🟩 Promoted: clean 4/4 + 3/3; exact drop detected; 1.052x | 🟧 Promoted but clean false conflict | 🟧 Promoted but 0/4 access patches | The `128x256` exact client is in the main matrix.  Record/Replay detects the third exact barrier-pair drop even though the numeric oracle remains schedule-masked; a distinct PyTorch shape is still desirable for Sampled and Inline Shadow. |
