...
The newer standard untuned run finishes in 89.94 seconds, passes the exact
values-and-indices oracle, emits no clean diagnostic, and patches all
63,474/63,474 accesses plus 5,620/7,100 barriers.  Extending the relocatable
dense host from kernels to owned device functions closes all 6,321 former
function-access omissions without a user knob.  That remains useful orange
evidence rather than full acceptance; function-barrier placement and reviewed
...
|---|---|---|---|---|---|
| **D0** `test/test_sort_and_select.py` and `test/test_reductions.py` | 🟥 Promoted; typed 2,039/63,474 verdict in 54.12 seconds | 🟧 Promoted; exact clean execution, 63,474/63,474 accesses + 5,620/7,100 barriers in 89.94 seconds, no diagnostic or tuning; 1.249x overhead | 🟧 Promoted; exact clean execution, 57,153/63,474 accesses + 12,978/14,200 barrier members, no diagnostic or tuning | 🟧 Promoted; exact oracle and useful 38,365/63,474 access coverage, but partial barrier coverage and one clean diagnostic | A decode-style `topk` over one 151,936-element Qwen vocabulary row with `k=50` is now in the main matrix. Its exact baseline passes. The native rocPRIM path loads two large operator objects and exposes both whole-code-object scalability and far-relay placement pressure that the compact Inductor client does not. Current Record/Replay patches 63,474/63,474 accesses and exits normally with the exact oracle; Sampled patches 57,153/63,474, and their barrier models use different denominators. Record/Replay's remaining work is function-barrier placement closure and reviewed-fault evidence; paired overhead is now quantified, while exact fault composition does not complete within a fixed 180-second bound. The committed Inline artifact completes in 99.84 seconds with 60% access and 49% barrier coverage but exposes a clean-input causal conflict. |
