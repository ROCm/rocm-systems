...
certificate has advanced through validation checkpoint `8a8ebaf3c5`, using
rebuilt hook SHA-256
`2dec0c65953b2d3c859ce99c84e1644b61ef0e4f179a9014388633245d0602b7`.
...
| **P1 PyTorch collision-heavy scatter-reduce** | 🟩 Exact BF16/FP32 collision oracle; clean-complete 27/27 LDS accesses; exact BF16 global-atomic scope weakening is a precommitted qualified miss; overhead 1.010x | 🟩 Exact oracle; clean-complete 27/27 accesses; exact BF16 global-atomic scope weakening is a precommitted qualified miss; overhead 0.998x | 🟧 Exact oracle; 14/27 supported accesses patch, with 13 helper-fill sites reporting `instrumentation_patch_missing` | 🟧 Exact oracle; the same 14/27 helper-fill accesses patch | SuperCollider checkpoint `8a8ebaf3c5`, hook `2dec0c65…`: clean `rdna4-scatter-sc-final-clean`, paired overhead `...-overhead-fixed`, inventory `rdna4-scatter-final-inventory`, and accepted fault `...-final-fault`. Keeping full CFG relay donors for moderate code objects restores all 27 sites while preserving bounded analysis for the 23 MiB and 40 MiB top-k objects. Record/Replay checkpoint `99061989a8`, hook `71cc2454…`: clean `rdna4-scatter-rr-current-clean`, paired overhead `...-final-overhead`, and accepted fault `...-final-fault`. The selected `global_atomic_pk_add_bf16` scope mutation applies exactly once, leaves the exact collision oracle unchanged, and is retained as a qualified miss. The executed reductions use relaxed singleton global atomics, so MOI correctly reports `atomic=0/0`; the LDS sites belong to a bundled boolean-fill helper |
...
| **P3 PyTorch native histogram** | 🟩 Exact oracle; clean-complete 135/135 ordinary LDS accesses; exact post-accumulation barrier drop is a precommitted qualified miss; overhead 0.966x | 🟧 Exact oracle and complete 135/135 ordinary LDS accesses; 42/84 supported barriers patch, with the rest lacking reachable entry islands | 🟧 Exact oracle and clean execution; 122/135 accesses and 86/168 barrier members patch, while unreachable relays remain explicit coverage gaps | 🟧 Exact oracle; 100/135 accesses and 43/84 barriers patch with no clean diagnostic | Current hook `2dec0c65…`: clean `rdna4-histc-sc-final-clean`, paired overhead `...-final-overhead`, fresh inventory `rdna4-histc-final-inventory` (84 barrier sequences, 104 atomic sites), and accepted fault `...-final-fault-accepted`. The exact float32 post-accumulation barrier-pair removal applies once but is schedule-masked: neither SuperCollider nor the independent histogram oracle reports a failure. The rejected `...-final-fault` and `...-final-fault-second` artifacts preserve the two precommitted effective-fault expectations. Relaxed accumulations are not qualified MOI ordering events and correctly remain `atomic=0/0`. Sampled checkpoint `ab8f0ea4c5`, hook `95e7ddd3…`, artifact `rdna4-histc-sampled-partial-002`; earlier checkpoint `0a7b607134` retains the other profiles |
| **P4 hip-moi D128 block attention** | 🟩 Exact oracle; clean 12/12; exact barrier drop is a precommitted qualified miss and breaks the oracle; overhead 164.77x | 🟩 Exact oracle; clean 12/12 + 4/4; exact barrier drop emits an attributed diagnostic and breaks the oracle; overhead 11.21x | 🟨 Exact oracle and clean-complete 12/12 accesses + 8/8 barrier members; overhead and reviewed fault pending | 🟩 Exact oracle; clean 12/12 accesses + 4/4 barriers; exact barrier drop emits a diagnostic and breaks the oracle; overhead 12.83x | Inline checkpoint `a902593b14`, hook `2dec0c65…`: clean `rdna4-generation-p4-d128-block`, paired overhead `rdna4-d128-block-inline-final-overhead`, inventory `...-final-inventory`, and accepted fault `...-final-fault-accepted`. The rejected historical-miss expectation in `...-final-fault` records the improved detector outcome after generation-qualified local shadows removed the former clean diagnostic. Sampled checkpoint `e96efba818`: clean `...-140`; SuperCollider checkpoint `ca1eb5456e`: `...-132` through `...-135`; Record/Replay checkpoint `baed32a85e`: `...-114` through `...-118` |
...
| **P4 hip-moi WMMA attention** | 🟩 Exact oracle; clean 12/12; exact barrier drop is a precommitted qualified miss and breaks the oracle; overhead 158.07x | 🟩 Exact oracle; clean 12/12 + 4/4; exact barrier drop emits an attributed diagnostic and breaks the oracle; overhead 11.99x | 🟨 Exact oracle and clean-complete 12/12 accesses + 8/8 barrier members; overhead and reviewed fault pending | 🟩 Exact oracle; clean 12/12 accesses + 4/4 barriers in five consecutive processes; exact barrier drop emits a diagnostic and breaks the oracle; overhead 13.24x | Inline checkpoint `be3ce27f3b`, hook `2dec0c65…`: clean `rdna4-generation-wmma-repeat-{1..5}`, paired overhead `rdna4-wmma-inline-final-overhead`, inventory `...-final-inventory`, and accepted fault `...-final-fault-accepted`. The rejected historical-miss expectation in `...-final-fault` records the improved detector outcome after generation-qualified local shadows removed the former intermittent clean diagnostic. Sampled checkpoint `e96efba818`: clean `...-142`; SuperCollider checkpoint `ca1eb5456e`: `...-128` through `...-131`; Record/Replay checkpoint `baed32a85e`: `...-114` through `...-125` |
...
|---|---|---|---|---|---|
| **D0** `test/test_sort_and_select.py` and `test/test_reductions.py` | 🟥 Promoted; typed 2,039/63,474 verdict in 54.12 seconds | 🟥 Promoted; 60-second fat-object analysis gate | 🟥 Promoted; shared fat-object construction gate | 🟥 Promoted; shared fat-object construction gate | A decode-style `topk` over one 151,936-element Qwen vocabulary row with `k=50` is now in the main matrix. Its exact baseline passes. The native rocPRIM path loads two large operator objects and exposes both whole-code-object scalability and a dominant far-relay placement gap that the compact Inductor client does not. |
...
| **D1** `test/test_reductions.py` histogram | 🟩 Promoted: exact oracle, clean 135/135, exact fault qualified miss, 0.966x | 🟧 Promoted: exact oracle, 135/135 accesses, 42/84 barriers | 🟧 Promoted: exact oracle, 122/135 accesses, 86/168 barrier members | 🟧 Promoted: exact oracle, 100/135 accesses, 43/84 barriers | Ordinary `torch.histc` is now in the main matrix. Its native object adds a real precompiled-library placement and relaxed-atomic difficulty class. SuperCollider leaves those atomics unchanged while instrumenting every supported ordinary LDS site; the relaxed atomics remain accesses rather than qualified memory-ordering events. The reviewed exact post-accumulation barrier drop is schedule-masked by this launch and retained as a qualified miss. Sampled preserves explicit partial coverage instead of rejecting the entire object when one far barrier has no relay island. |
