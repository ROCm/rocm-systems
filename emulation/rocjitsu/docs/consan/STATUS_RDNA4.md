...
certificate has advanced through validation checkpoint `45f8b118f3`, using
rebuilt hook SHA-256
`8c64e5ee47f34b4d6571e9b0b9820c5cf417fa5a5f90113d7210722fc0a2f4ab`.
...
| **D0** `kernels.gfx1201.llama.cpp.llama_rms_norm.default` | 🟨 Exact CPU oracle; clean-complete 22/22 LDS accesses; no mismatch; 1.306x process overhead; reviewed effective fault pending | 🟨 Exact oracle; clean-complete 22/22 accesses + 11/11 barriers; visible replay evidence and no conflict; 1.308x overhead; reviewed effective fault pending | 🟨 Exact oracle; clean-complete 22/22 accesses + 22/22 barrier members; sampled evidence and no clean diagnostic; 1.307x overhead; reviewed effective fault pending | 🟥 Static- and dynamic-complete 22/22 + 11/11 with 80 visible events, but the clean oracle still fails: only 40/128 outputs are nonzero and 74 differ beyond 1e-5 | Inline checkpoint `45f8b118f3`, hook `8c64e5ee…`, artifact `rdna4-rms-inline-dispatch-id-45f8b118`; exact baseline artifact `.pytest-artifacts-rdna4-llama-baseline`. The checkpoint fixes the valid hardware dispatch-ID-zero case that previously produced eight malformed snapshots and 72 dynamic-incomplete events. `RJ_CONSAN_MAX_PATCHES=6` remains exact; admitting the seventh site, the executed kernel's first LDS access at `0x1810`, introduces the oracle corruption. The failure is therefore localized below setup and whole-object analysis. Other profiles retain five-process medians of 0.163896 s baseline, 0.213989 s SuperCollider, 0.214359 s Record/Replay, and 0.214191 s Sampled. |
