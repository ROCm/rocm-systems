...
certificate has advanced through validation checkpoint `913fc2e7a6`, using
rebuilt hook SHA-256
`71cc2454b882a8e581543adb581f1a2a07ac71720afca1b6098ef003bd186d47`.
...
| **P1 PyTorch collision-heavy scatter-reduce** | 🟧 Exact BF16/FP32 collision oracle, but current-tip SuperCollider patches 0/27 LDS accesses after its long-range relay bank cannot close the bundled helper object | 🟩 Exact oracle; clean-complete 27/27 accesses; exact BF16 global-atomic scope weakening is a precommitted qualified miss; overhead 0.998x | 🟧 Exact oracle; 14/27 supported accesses patch, with 13 helper-fill sites reporting `instrumentation_patch_missing` | 🟧 Exact oracle; the same 14/27 helper-fill accesses patch | Record/Replay checkpoint `99061989a8`, hook `71cc2454…`: clean `rdna4-scatter-rr-current-clean`, paired overhead `...-final-overhead`, inventory `rdna4-scatter-final-inventory`, and accepted fault `...-final-fault`. The selected `global_atomic_pk_add_bf16` scope mutation applies exactly once, leaves the exact collision oracle unchanged, and is retained as a qualified miss. SuperCollider artifact `rdna4-scatter-sc-final-overhead` records 27 placement failures and supersedes the older 27/27 result. The executed reductions use relaxed singleton global atomics, so MOI correctly reports `atomic=0/0`; the LDS sites belong to a bundled boolean-fill helper |
