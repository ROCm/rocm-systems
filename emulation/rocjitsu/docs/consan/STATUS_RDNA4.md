...
| **P2 PyTorch Qwen-vocabulary top-k** | 🟥 Exact values and indices pass and clean analysis now reaches a typed verdict in 54.12 seconds, but only 2,039/63,474 supported accesses patch | 🟥 Correct-workload analysis of that fat code object exceeds even a 60-second discriminator before a patch verdict | 🟥 Shared fat-code-object construction gate | 🟥 Shared fat-code-object construction gate | Checkpoint `47742808e8`, artifact `rdna4-topk-sc-fast-inventory`, hook `403c35a3…`: clean SuperCollider no longer builds synchronization semantics it does not consume, skips absent flat-probe CFG construction, and restricts kernel-only liveness to preflight candidates. The two applicable objects patch 941/42,652 and 1,098/20,822 accesses; 61,435 sites lack reachable relay placement, so this remains far below the usable-coverage threshold. Checkpoint `48fb0737bb` removed the former atomic rejection. Record/Replay artifact `rdna4-pytorch-topk-rr-discriminator` retains the independent MOI construction timeout |
