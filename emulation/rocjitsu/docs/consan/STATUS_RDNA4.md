...
certificate has advanced through validation checkpoint `47742808e8`, using
rebuilt hook SHA-256
`403c35a300fb2eeff8c0e6914f55415f786f5cc547e8d93f996a44c1860a6945`.
...
|---|---|---|---|---|---|
| **P0 Qwen3-0.6B prefill** | 🟩 Exact oracle; clean 20/20; exact barrier drop emits an attributed diagnostic and breaks the oracle; overhead 1.044x | 🟩 Exact oracle; clean 20/20 + 14/14; exact drop is a precommitted qualified miss and breaks the oracle; overhead 0.988x | 🟩 Exact oracle; clean 20/20 + 26/26; deterministic 32-offset fault sweep detects 17/32 and every exact mutation breaks the oracle; overhead 1.003x | 🟧 Complete 20/20 accesses + 14/14 barriers, but one diagnostic rejects the correct workload | Current-tip SuperCollider regression artifact `rdna4-qwen-sc-after-fat-fastpath`, hook `403c35a3…`, reconfirms exact 20/20 clean coverage in 0.532 seconds after the fat-object optimization. Current Inline artifact `rdna4-qwen-inline-vcc-fix-runtime`, hook `ae96b66a…`, retains complete clean coverage but not the remaining diagnostic. Fresh discriminators localize it to the workgroup-local 64-bit LDS-shadow path: the retained prior instruction is unaligned and outside the complete emitted-site set, and repeated runs can report out-of-range access kinds. Other cells retain checkpoint `9c5ff65778`: clean `...-105`, paired overhead `...-098` through `...-100`, inventory `...-101`, and accepted faults `...-102` through `...-104`; the Sampled offset sweep is fault-sensitivity evidence, not clean-workload tuning |
