...
certificate has advanced through validation checkpoint `ca1eb5456e`, using
rebuilt hook SHA-256
`975a738e025973843cb5a66e728e55d9dbfdd4d61c27cd67168b65c92aa3dc16`.
...
| **P4 hip-moi D128 block attention** | 🟩 Exact oracle; clean 12/12; exact barrier drop is a precommitted qualified miss and breaks the oracle; overhead 164.77x | 🟩 Exact oracle; clean 12/12 + 4/4; exact barrier drop emits an attributed diagnostic and breaks the oracle; overhead 11.21x | 🟥 Patch fails closed because the barrier cannot reach its end-of-text reserved entry island | 🟧 Complete 12/12 accesses + 4/4 barriers, but a diagnostic rejects the correct workload | SuperCollider checkpoint `ca1eb5456e`: clean `...-132`, paired overhead `...-133`, inventory `...-134`, and accepted fault `...-135`; RDNA4 `flat_store_b16` readback support removed the former patch-time invalid result. Record/Replay checkpoint `baed32a85e`: `...-114` through `...-118` |
| **P4 hip-moi D128 pressure attention** | 🟥 RDNA4 `flat_store_b16` readback now encodes, but code-object patch planning exceeds the standard 30-second gate before an analysis verdict | 🟩 Exact oracle; clean 12/12 + 4/4; exact barrier drop emits an attributed diagnostic and breaks the oracle; overhead 14.30x | 🟥 Patch fails closed because the barrier cannot reach its end-of-text reserved entry island | 🟩 Exact oracle; clean 12/12 + 4/4; exact barrier drop emits an attributed diagnostic; overhead 13.72x | SuperCollider discriminator `rdna4-ca1eb5456e-clean-d128-pressure-sc-127` isolates patch-planning time after fixing the prior 16-bit encoding error. Record/Replay checkpoint `baed32a85e`: `...-114`, `...-115`, `...-116`, `...-119`, and `...-124`. Inline checkpoint `79aea7420c`: `...-071`, `...-081`, `...-086`, and `...-090` |
| **P4 hip-moi WMMA attention** | 🟩 Exact oracle; clean 12/12; exact barrier drop is a precommitted qualified miss and breaks the oracle; overhead 158.07x | 🟩 Exact oracle; clean 12/12 + 4/4; exact barrier drop emits an attributed diagnostic and breaks the oracle; overhead 11.99x | 🟥 Patch fails closed because the barrier cannot reach its end-of-text reserved entry island | 🟧 Complete 12/12 + 4/4 and one clean run passes, but two of three repeated correct-workload runs emit false diagnostics | SuperCollider checkpoint `ca1eb5456e`: clean `...-128`, paired overhead `...-129`, inventory `...-130`, and accepted fault `...-131`; RDNA4 `flat_store_b16` readback support removed the former patch-time invalid result. Record/Replay checkpoint `baed32a85e`: `...-114`, `...-115`, `...-116`, `...-120`, and `...-125`. Inline repeated attempt `...-078` retains its intermittent false positive |
| **P4 hip-moi Stream-K arrival** | 🟩 Exact oracle; clean 4/4 accesses; exact order and scope weakenings are precommitted qualified misses; overhead 558.83x | 🟩 Exact oracle; clean 4/4 accesses + 15/15 atomics + 4/4 barriers + 16/16 fences; exact order and scope weakenings are precommitted qualified misses; overhead 53.00x | 🟥 Patch fails closed because the barrier cannot reach its end-of-text reserved entry island | 🟧 Complete 4/4 + 15/15 + 4/4, but a diagnostic rejects the correct workload | Record/Replay checkpoint `baed32a85e`: clean `...-114`, paired overhead `...-115`, inventory `...-116`, and accepted exact faults `...-122` and `...-123`. SuperCollider checkpoint `79aea7420c`: `...-073`, `...-082`, `...-087`, `...-091`, and `...-092` |
| **P4 hip-moi tree atomic-OR** | 🟩 Exact oracle; clean 4/4 accesses; exact order and scope weakenings are precommitted qualified misses; overhead 591.81x | 🟧 Exact oracle and complete 4/4 accesses + 15/15 atomics + 4/4 barriers, but four supported fences fail placement at 12/16 | 🟥 Patch fails closed because the barrier cannot reach its end-of-text reserved entry island | 🟧 Complete 4/4 + 15/15 + 4/4, but a diagnostic rejects the correct workload | Current Record/Replay discriminator `rdna4-baed32a85e-clean-p4-rr-114` isolates the remaining four `instrumentation_patch_missing` fences after ineligible fences stopped poisoning coverage. Other retained evidence is at checkpoint `79aea7420c` |
| **P4 hip-moi Jakub attention variants** | 🟩 Exact oracle; clean 31/31 accesses; exact barrier drop is a precommitted qualified miss; overhead 103.75x | 🟩 Exact oracle; clean 31/31 + 4/4; exact barrier drop is a precommitted qualified miss; overhead 11.69x | 🟥 Patch fails closed because the barrier cannot reach its end-of-text reserved entry island | 🟩 Exact oracle; clean 31/31 + 4/4; exact barrier drop emits an attributed diagnostic; overhead 11.03x | Current Sampled discriminator `rdna4-3995da681a-clean-jakub-sampled-136` confirms the shared RDNA4 relay limit. Record/Replay checkpoint `baed32a85e`: clean `...-113`, paired overhead `...-115`, inventory `...-116`, and accepted fault `...-121`. SuperCollider and Inline checkpoint `79aea7420c`: `...-076`, `...-084`, `...-085`, `...-089`, `...-096`, and `...-097` |
