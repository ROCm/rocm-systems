...
| **P2 PyTorch causal SDPA** | 🟨 Independent CPU oracle and clean-complete 158/158 LDS accesses; overhead 1.946x; reviewed fault pending | 🟨 Independent CPU oracle; clean-complete 158/158 accesses + 22/22 barriers + 2/2 atomics + 2/2 fences; overhead 7.795x; reviewed fault pending | 🟥 Full-pressure attention kernel has no persistent dispatch-ID SGPR pair or fresh automatic EXEC-save window; only 14/27 accesses in a separate bundled fill kernel patch | 🟥 Same full-pressure SGPR blockers; strict fail-closed exit after only the separate fill kernel patches 14/27 accesses | Checkpoint `8a5669d5e5`, hook `fec3ef03…`: paired artifacts `rdna4-sdpa-sc-overhead-002` and `rdna4-sdpa-rr-overhead-003` retain ten-repetition measurements at one clean revision. Record/Replay remains dynamically complete after automatic report headroom was raised for repeated dispatches; no user tuning knobs are used. Clean artifact `rdna4-sdpa-rr-scalar-regression-001` retains the 158/158 + 22/22 + 2/2 + 2/2 scalar-spill regression |
