...
certificate is being assembled from validation checkpoint `b6be4081e1`, using
...
| **P4 hip-moi D128 block attention** | 🟥 Correct workload crashes before an analysis verdict | 🟧 Oracle passes and 12/12 accesses + 4/4 barriers patch dynamically; static analysis is incomplete | 🟥 Correct workload crashes before an analysis verdict | 🟧 Complete 12/12 accesses + 4/4 barriers, but a diagnostic rejects the correct workload | Current-tip clean discriminator `rdna4-79aea7420c-clean-d128-block-075` reproduces four distinct caveats |
| **P4 hip-moi D128 pressure attention** | 🟥 Correct workload crashes before an analysis verdict | 🟧 Oracle passes and 12/12 accesses + 4/4 barriers patch dynamically; static analysis is incomplete | 🟥 Correct workload crashes before an analysis verdict | 🟩 Exact oracle; clean 12/12 + 4/4; exact barrier drop emits an attributed diagnostic; overhead 13.72x | Checkpoint `79aea7420c`: clean `...-071`, paired overhead `...-081`, inventory `...-086`, and accepted fault `...-090` |
| **P4 hip-moi WMMA attention** | 🟥 Correct workload crashes before an analysis verdict | 🟧 Oracle passes and 12/12 accesses + 4/4 barriers patch dynamically; static analysis is incomplete | 🟥 Correct workload crashes before an analysis verdict | 🟧 Complete 12/12 + 4/4 and one clean run passes, but two of three repeated correct-workload runs emit false diagnostics | Clean `rdna4-79aea7420c-clean-wmma-attention-072`; repeated overhead attempt `...-078` exposes the intermittent false positive |
| **P4 hip-moi Stream-K arrival** | 🟩 Exact oracle; clean 4/4 accesses; exact order and scope weakenings are precommitted qualified misses; overhead 558.83x | 🟧 Oracle passes and all 4/4 accesses + 15/15 atomics + 4/4 barriers + 16/16 fences patch dynamically; static analysis is incomplete | 🟥 Correct workload crashes before an analysis verdict | 🟧 Complete 4/4 + 15/15 + 4/4, but a diagnostic rejects the correct workload | Checkpoint `79aea7420c`: clean `...-073`, paired overhead `...-082`, inventory `...-087`, and accepted exact faults `...-091` and `...-092` |
| **P4 hip-moi tree atomic-OR** | 🟩 Exact oracle; clean 4/4 accesses; exact order and scope weakenings are precommitted qualified misses; overhead 591.81x | 🟧 Oracle passes and 4/4 accesses + 15/15 atomics + 4/4 barriers patch dynamically, but static analysis is incomplete and fences are 12/16 | 🟥 Correct workload crashes before an analysis verdict | 🟧 Complete 4/4 + 15/15 + 4/4, but a diagnostic rejects the correct workload | Checkpoint `79aea7420c`: clean `...-074`, paired overhead `...-083`, inventory `...-088`, and accepted serial fault rows `...-094` and `...-095`; concurrent timeout `...-093` never requested a mutation |
| **P4 Jakub attention variants** | 🟩 Exact oracle; clean 31/31 accesses; exact barrier drop is a precommitted qualified miss; overhead 103.75x | 🟧 Oracle passes and 31/31 accesses + 4/4 barriers patch dynamically; static analysis is incomplete | 🟥 Correct workload crashes before an analysis verdict | 🟩 Exact oracle; clean 31/31 + 4/4; exact barrier drop emits an attributed diagnostic; overhead 11.03x | Checkpoint `79aea7420c`: clean `...-076`, paired overhead `...-084` and `...-085`, inventory `...-089`, and accepted faults `...-096` and `...-097` |
