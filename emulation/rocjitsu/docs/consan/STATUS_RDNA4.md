...
certificate has advanced through executable validation checkpoint `e49f6b0c33`, using
rebuilt hook SHA-256
`f9efe9880890bc99a1805782685b2707650de21f63a33bf8bfe2f4562364fc1f`.
...
| **P2 PyTorch/Inductor compiled softmax** | 🟩 Exact oracle; clean 4/4 LDS accesses; exact third barrier drop is a precommitted qualified miss; overhead 0.960x | 🟩 Exact oracle; clean 4/4 + 3/3; exact third barrier drop emits an attributed replay diagnostic; overhead 1.052x | 🟨 Exact oracle; clean-complete 4/4 accesses + 6/6 barrier members; 24 visible synchronization records and no conflict; overhead and reviewed fault pending | 🟧 Exact oracle and static 4/4 accesses + 3/3 barriers patch, but the 128-workgroup execution records 576 typed packed-workgroup-identity exclusions | Current Inline artifact `rdna4-compiled-softmax-inline-current-20260722`, hook `36f9cdb5…`, replaces the stale 32-bank diagnosis: it has 64 visible exact-shadow cells, zero malformed snapshots, and 576 `inline_unsupported_workgroups`. The remaining blocker is the injective bounds of the descriptor-shaped packed workgroup key, not report capacity or malformed state. Sampled checkpoint `c5d9c59672`, hook `87db680e…`, artifact `rdna4-compiled-softmax-sampled-address-aligned-clean-20260722`: runtime sampling selects LDS addresses, so qualified barriers no longer apply an unrelated workgroup-residue gate. Every reaching workgroup now advances its epoch and inspects its banked causal window; the former six clean conflicts and zero synchronization records become zero conflicts and 24 records without a user knob. Other profiles retain checkpoint `b6be4081e1`: clean `...-110`, paired overhead `...-111` and `...-112`, inventory `...-107`, and accepted faults `...-108` and `...-109` |
...
| **D0** Attention and model paths, including `test/inductor/test_fused_attention.py` and `test/nn/test_multihead_attention.py` | 🟩 Promoted: independent oracle; clean 158/158; exact fault detected; 1.946x | 🟨 Promoted: current clean 158/158 + 22/22 + 2/2 + 2/2; reviewed traps remain unattributed; 7.795x | 🟥 Promoted: full-pressure persistent/transient SGPR placement | 🟥 Promoted: same full-pressure SGPR placement | Ordinary causal `torch.nn.functional.scaled_dot_product_attention` is now in the main matrix. Current artifact `rdna4-sdpa-rr-safe-entry-return-20260722` reconfirms clean completeness after fixing the out-of-range entry return's ABI-state clobber. |
