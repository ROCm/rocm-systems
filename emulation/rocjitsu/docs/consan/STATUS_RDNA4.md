...
|---|---|---|---|---|---|
| **D0** `test/test_sort_and_select.py` and `test/test_reductions.py` | 🟥 Promoted; typed 2,039/63,474 verdict in 54.12 seconds | 🟧 Promoted; exact clean execution, 57,153/63,474 accesses + 5,620/7,100 barriers in 87.77 seconds, no diagnostic or tuning | 🟧 Promoted; exact clean execution, 57,153/63,474 accesses + 12,978/14,200 barrier members, no diagnostic or tuning | 🟧 Promoted; exact oracle and useful 38,365/63,474 access coverage, but partial barrier coverage and one clean diagnostic | A decode-style `topk` over one 151,936-element Qwen vocabulary row with `k=50` is now in the main matrix. Its exact baseline passes. The native rocPRIM path loads two large operator objects and exposes both whole-code-object scalability and far-relay placement pressure that the compact Inductor client does not. Current Record/Replay and Sampled both patch 57,153/63,474 accesses and exit normally with the exact oracle; their barrier models use different denominators. Record/Replay's remaining work is placement closure, overhead, and reviewed-fault evidence—not a hang or setup blocker. The committed Inline artifact completes in 99.84 seconds with 60% access and 49% barrier coverage but exposes a clean-input causal conflict. |
