...
| P1 | `torch.sort` over segmented rows | 🩶 Baseline exact; profile unassessed | 🟧 Exact oracle passes in 34.99 s, but coverage is incomplete at 19,311/37,636 accesses and 271/6,032 barriers, and clean replay emits one forbidden conflict | 🩶 Baseline exact; profile unassessed | 🩶 Baseline exact; profile unassessed | Exact values/indices with one-repetition evidence in `consan-gfx950-rr-pytorch-sort-fresh-20260722`; execution works, but coverage and the false clean diagnostic remain. |
