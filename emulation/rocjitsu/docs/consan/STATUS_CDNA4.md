...
| P1 | `torch.histc` with a shared-memory-sized bin count | 🩶 Baseline exact; profile unassessed | 🟨 Exact oracle passes in 7.76 s with zero diagnostics and complete 98/98 supported accesses plus 84/84 barriers after CDNA4 dense routing; static acceptance remains open for 12 unsupported LDS-atomic sites in the histogram object and 27 unsupported LDS mnemonics in a bundled fill object | 🩶 Baseline exact; profile unassessed | 🩶 Baseline exact; profile unassessed | Frozen one-repetition artifact `consan-gfx950-pytorch-histc-rr-dense-router-frozen-20260722`; commit `a529bd8877` closes all 28 access and 38 barrier placement omissions without changing RDNA4/gfx1250 routing. |
