...
healthy pre/post probes. The row remains yellow only because the unrelated
dirty worktree prevents clean-provenance acceptance.
...
### 2026-08-21 PyTorch `sort` Sampled diagnostic floor lift

Artifact
`/home/ossci/xx/consan-validation/rebase-20260821-gfx950-pytorch-sort-sampled-diagnostic60`
records a current-hook physical-gfx950 Sampled run under a bounded 60-second
diagnostic contract.  The process exits normally in 43.67 seconds, and the
exact sorted-values-and-indices oracle passes after 24.56 seconds of device
execution.  ConSan patches 52,904/56,884 supported accesses and all
5,422/5,422 barriers, emits zero forbidden diagnostics, and reports dynamic
completeness.  The remaining 3,980 access placement/lowering omissions keep
static analysis incomplete, and the row still exceeds its ordinary 30-second
contract.  This is useful current execution rather than a pre-verdict failure,
so the Sampled cell advances from red to orange without claiming acceptance.
