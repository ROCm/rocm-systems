...
| `hip_streamk_simple::m256_n256_k256` and `hip_streamk_two_tile::m256_n256_k256` | **Metadata-enabled; environment-blocked before compilation** | At enabling corpus revision `61b5af0b5ee9`, `corpus/kernels/configs/gfx950.json` enables HIP Stream-K and `corpus/kernels/cases/hip-stream-k/{simple_streamk,two_tile_streamk}/case.json` declare gfx950.  Each exact oracle uses one run, `--grid 8`, and `--validate`; separate `nan_results_rejected` cases are defined to reject non-finite results.  The current workspace TheRock distribution still has neither `rocthrustConfig.cmake` nor rocThrust headers, so no gfx950 build or ConSan profile has run. |
