...
| P1 | `torch.histc` with a shared-memory-sized bin count | 🩶 Baseline exact; profile unassessed | 🟨 Exact oracle passes in 9.12 s with zero diagnostics; the histogram object has complete 152/152 access and 84/84 barrier coverage, while decoupling dense CDNA4 access routing from barrier routing improves its separately loaded boolean-fill helper from 12/27 to 26/27 accesses.  Only one operand-overlap resource failure remains | 🩶 Baseline exact; profile unassessed | 🩶 Baseline exact; profile unassessed | One-repetition artifact `consan-gfx950-pytorch-histc-rr-access-router-20260722`; aggregate coverage is 178/179 accesses and 84/84 barriers with dynamic completeness and no placement/lowering failures.  The final `ds_write_b8` needs spill restoration before its displaced guest store. |
