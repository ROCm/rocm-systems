...
| **P4 hip-moi MFMA attention** | 🟩 Current candidate-tree physical run passes both exact oracles in 0.23 seconds with complete 58/58 access coverage; CDNA4 B16 store readback is now covered by a focused host regression; prior paired and reviewed-fault evidence retained | 🟧 Current physical run passes both exact oracles with zero diagnostics. The FastContext component is complete at 33 accesses plus 5 barriers; every missing site is in the full-pressure ExactContext component (25 accesses and 9 barriers), so the strict runner exits on incomplete coverage | 🟧 Current physical run passes both exact oracles. The FastContext component supplies all 33 patched accesses plus all 4/4 applicable barriers; the full-pressure ExactContext component accounts for all 25 access resource failures | 🟥 Current physical planner rejects before either oracle because the scalar owner/epoch prologue has no entry-local VGPR scratch in the exact-context kernel |
