...
| **P4 hip-moi MFMA attention** | 🟩 Current candidate-tree physical run passes both exact oracles in 0.23 seconds with complete 58/58 access coverage; CDNA4 B16 store readback is now covered by a focused host regression; prior paired and reviewed-fault evidence retained | 🟧 Current physical run passes both exact oracles with zero diagnostics, but only 33/58 accesses and 5/14 barriers patch; 25 access and 9 barrier sites fail resource planning, and the strict runner exits on incomplete coverage | 🟧 Current physical run passes both exact oracles with 33/58 accesses plus all 4/4 applicable barriers patched; the former pre-execution rejection is gone, but 25 access sites still fail resource planning | 🟥 Current physical planner rejects before either oracle because the scalar owner/epoch prologue has no entry-local VGPR scratch in the exact-context kernel |
