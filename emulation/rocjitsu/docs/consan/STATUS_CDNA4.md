...
| P2 | `rocblas_sgemm` compact exact cases | 🩶 Baseline exact; profile unassessed | 🟥 The exact `Square_64x64` baseline passes, but strict Record/Replay rejects the 7,240,872-byte rocBLAS object before the oracle with a non-installable transform outcome | 🩶 Baseline exact; profile unassessed | 🩶 Baseline exact; profile unassessed | One-repetition physical-device artifact `consan-gfx950-rocblas-sgemm-rr-assessment-20260722/run.log`; preflight finds 49,435 access ranges and 2,558,464 barrier records before the transform returns `unsupported` and strict policy terminates with exit code 92.  This is a bounded red assessment, not a claim that library dispatch alone supplies useful ConSan signal. |
...
| P1 | `torch.histc` with a shared-memory-sized bin count | 🩶 Baseline exact; profile unassessed | 🟨 Exact oracle passes in 9.63 s with zero diagnostics; the histogram object now has complete 110/110 access and 84/84 barrier coverage after dense routing plus CDNA4 subword-alias admission.  A separately loaded boolean-fill helper patches 12/27 accesses, leaving one resource failure and 14 placement/lowering failures; 42 LDS atomic accesses in co-resident histogram specializations are not yet represented in the coverage denominator | 🩶 Baseline exact; profile unassessed | 🩶 Baseline exact; profile unassessed | One-repetition artifact `consan-gfx950-pytorch-histc-rr-subword-frozen-20260722`; aggregate coverage is 122/137 accesses and 84/84 barriers with dynamic completeness.  The remaining fill-helper spill/placement gap and missing CDNA4 LDS-atomic access admission keep this cell yellow. |
