...
| P2 | `rocblas_sgemm` compact exact cases | 🟨 The repaired physical strict run passes the exact `Square_64x64` oracle in 28.963 seconds, patches all 49,435/49,435 supported LDS accesses, reports no check mismatches, and has complete static/analysis/dynamic verdicts; paired overhead and reviewed-fault evidence remain | 🟧 The former persistent-state rejection is fixed: a current standard-profile run produces a valid 57,097-patch transform, and the quiet bounded diagnostic passes the exact oracle in 16.669 seconds. The process still reaches its 45-second bound during post-oracle replay/teardown without a final coverage or dynamic verdict | 🟨 Current physical stride-1 diagnostic passes the exact oracle in 14.698 seconds with complete 49,435/49,435 access and 4,995/4,995 barrier coverage, 96 visible samples, zero diagnostics, and complete static/analysis/dynamic verdicts; the standard stride-256 cadence still selects no workgroup, and paired-overhead/reviewed-fault evidence remain | 🟨 Current physical strict Inline Shadow passes the exact oracle in 23.742 seconds with complete 49,435/49,435 access and 4,997/4,997 barrier coverage, zero forbidden diagnostics, and complete static/analysis/dynamic verdicts; paired-overhead and reviewed-fault evidence remain | The repaired SuperCollider evidence is retained under `consan-validation/rebase-20260821-gfx950-rocblas-supercollider-dense-final`; the other current evidence remains under `consan-validation/rebase-20260821-gfx950-rocblas-sgemm-current`, with older Record/Replay artifacts under `consan-gfx950-rocblas-sgemm-rr-assessment-20260722`.  Record/Replay is now a post-oracle teardown-latency frontier, not a state-planning rejection. |
