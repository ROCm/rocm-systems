...
| **P4 hip-moi MFMA attention** | 🟧 Current physical run passes both exact oracles and patches all 56 supported accesses, but two additional discovered access forms are unsupported, leaving analysis incomplete | 🟧 Current physical run passes both exact oracles with zero diagnostics, but only 33/58 accesses and 5/14 barriers patch; 25 access and 9 barrier sites fail resource planning, and the strict runner exits on incomplete coverage | 🟧 Current physical run passes both exact oracles with 33/58 accesses plus all 4/4 applicable barriers patched; the former pre-execution rejection is gone, but 25 access sites still fail resource planning | 🟥 Current physical planner rejects before either oracle because the scalar owner/epoch prologue has no entry-local VGPR scratch in the exact-context kernel |
