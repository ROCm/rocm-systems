...
cell remains orange.  The gfx950 RocJITsu simulator reaches the oracle without
the former invalid-address fault but currently produces different numerical
output for this VCC-return case; that simulator-only semantic gap is tracked
separately from the physical-device fix.
