...
| CDNA4 group-FLAT access inventory/emission | 🩶 Native vertical | `6e997a6a39` and `31552da206`: CDNA4 raw FLAT decoding retains op, segment, address/data/result VGPRs, offset, and cache fields; explicit `SRC_SHARED_BASE` provenance admits native two-word D16/D32/D64/D128 group accesses without weakening strict provenance. The complete D16 load family is normalized by memory width and destination half: unsigned/signed byte and B16 forms, including their high-half variants, share one target-neutral semantic contract across all four supported targets. Target-native high-half B16 and signed-byte runtime fixtures cover physical gfx1201 plus simulated gfx1250, gfx942, and gfx950, including sign extension and the CDNA SRAM-ECC destination behavior. The symmetric D16 store-family gap is tracked by `bd-1w9.34`. The retained `d128-block` Record/Replay run at `consan-validation/gfx950-take2-d128-block-record-007` executes successfully with all 12/12 admitted accesses and all 4/4 shared-function barriers patched and dynamically complete. The newly exercised many-candidate path uses target-aware CDNA4 `s_getpc_b64`/`s_setpc_b64` indirect islands. Broader FLAT forms and campaign-level fault/performance evidence remain open, so `A2` is active and no workload cell is promoted. |
...
| CDNA4 SuperCollider group-FLAT checks | 🩶 Native clean vertical | `c7ca36d375`: SuperCollider handles gfx950's two-word group-FLAT D16/D32/D64/D128 loads and stores. All low-half D16 loads use U16 comparison; high-half D16 loads first select the written upper half of both results into independently allocated scratch before the same U16 comparison. The lowering emits the target's complete group-FLAT wait, preserves the original instruction's destination semantics, and even-aligns the CDNA report address tuple. The high-half B16 and signed-byte runtime fixtures confirm real `inline-flat-load-check-trap` patches and exact output on physical gfx1201 plus simulated gfx1250, gfx942, and gfx950. A deterministic production-path high-half mismatch oracle remains tracked by `bd-1w9.35`; this clean runtime evidence is not presented as a mismatch-detection claim. Retained artifact `consan-validation/gfx950-take2-d128-block-supercollider-006` is analysis-, static-, and dynamic-complete with 12/12 supported accesses patched and zero report mismatches. The coverage denominator excludes global, private, and provenance-unknown FLAT operations outside this shared-memory detector's semantics. Racy/fault detection, trap breadth, and campaign evidence remain open in `SC1`. |
