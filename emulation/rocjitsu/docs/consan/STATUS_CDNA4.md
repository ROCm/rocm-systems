...
| **P4 hip-moi D128 pressure attention** | 🟧 Current rebuilt workload passes all four exact clean oracles, but instruments only 112/248 supported accesses; 136 sites fail placement or lowering | 🟧 Current rebuilt workload passes its first three exact clean oracles with 73/252 accesses plus 9/36 barriers instrumented. The indexed-VGPR scratch corruption is fixed and a 28-access prefix passes the fourth exact oracle; the next `ds_read_b32` access still GPU-faults | 🟥 Current rebuilt workload is rejected before its first oracle because the sampled probe cannot encode its entry island | 🟧 Current rebuilt workload passes its first three exact clean oracles with 73/252 accesses plus 9/36 barriers instrumented, then the fourth kernel GPU-faults; the previously suspected private-segment increase is not by itself sufficient to cause the Record/Replay failure and Inline Shadow still needs equivalent isolation |
...
671 `ConSanMoi.*` tests pass; on physical gfx950 the formerly corrupting
28-access prefix now passes the fourth exact numeric oracle.  Adding the 29th
access, `ds_read_b32 v50, v50 offset:132` at text offset `0x4d470`, exposes a
separate GPU fault.  Because the passing prefix already exercises the enlarged
private allocation, private-segment growth alone is no longer the supported
root-cause claim.  The unrestricted Record/Replay and Inline Shadow rows remain
non-green while that next boundary and Inline Shadow's corresponding behavior
are investigated.  These are current regressions, not accepted limitations;
every fix must retain a focused regression test and new physical-gfx950
evidence.
