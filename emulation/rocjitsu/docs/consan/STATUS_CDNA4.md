...
640-byte runtime private segment and faults when dispatch interception raises
it to 736 and 784 bytes respectively.  Sampled fails closed before execution
with `ConSan MOI sampled probe could not encode its entry island`.  These are
current regressions under investigation, not accepted limitations; every fix
must retain a focused regression test and a new physical-gfx950 artifact.
