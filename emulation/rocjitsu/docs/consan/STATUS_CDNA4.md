...
| P1 | `torch.sort` over segmented rows | 🩶 Baseline exact; profile unassessed | 🟥 Current strict run rejects the main code object before the oracle after 43.98 s with transform-error status 4112; the earlier executable artifact predates dense CDNA4 routing and had incomplete 19,311/37,636 access plus 271/6,032 barrier coverage, with zero visible barrier records causing its false replay conflict | 🩶 Baseline exact; profile unassessed | 🩶 Baseline exact; profile unassessed | Current one-repetition artifact `consan-gfx950-pytorch-sort-rr-current-20260722`; the bounded rerun establishes a red current frontier, so this resistant bundled object is deferred rather than receiving another long verbose run. |
