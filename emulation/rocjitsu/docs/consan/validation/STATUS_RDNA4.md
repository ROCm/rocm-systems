...
| --- | ---: | --- | --- | --- |
| Production HIP | P0 | FP16 matmul (`rdna4-matmul-fp16-production`) | 🟩 higher (lowest verified): clean pass; fault 8/8 (bar 6/8) | 🟨 sleep=15: clean pass; fault 0/8 (bar 6/8) |
| Production HIP | P0 | FP8 matmul (`rdna4-matmul-fp8-production`) | 🟩 higher (lowest verified): clean pass; fault 8/8 (bar 6/8) | 🟨 sleep=15: clean pass; fault 3/8 (bar 6/8) |
...
| Main E2E | P1 | Sharktank TP1 prefill (`tp1-prefill`) | 🟩 higher (lowest verified): clean pass; fault 8/8 (bar 6/8) | 🟨 delay-matrix: clean pass; fault 0/8 (bar 6/8); sleep calibration queued |
| Main E2E | P1 | Sharktank TP1 decode/combined (`tp1-decode-combined`) | 🟩 higher (lowest passing): clean pass; fault 8/8 (bar 6/8) | 🟨 delay-matrix: clean pass; fault 0/8 (bar 6/8); sleep calibration queued |
...
| Main E2E | P4 | hip-moi Stream-K arrival (`streamk-arrival`) | 🟨 default: clean pass; fault 0/8 (bar 6/8) | 🟨 Clean pass; atomic-scope fault applies offline; GPU qualification queued |
| Main E2E | P4 | hip-moi tree atomic-OR (`tree-atomic-or`) | 🟨 max: clean failed | 🟨 Clean pass; atomic-scope fault applies offline; GPU qualification queued |
