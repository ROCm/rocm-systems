...
| PyTorch | P2 | Inductor compiled softmax (`pytorch-rdna4-compiled-softmax`) | 🟩 higher (lowest passing): clean pass; fault 8/8 (bar 6/8); same-value writes allowed | 🟨 delay-matrix: clean pass; fault 2/8 (bar 6/8); matching-delay audit pending; same-value writes allowed |
...
| Main E2E | P3 | Sharktank CLIP BF16 (`clip-bf16`) | 🟩 high (lowest passing): clean pass; fault 8/8 (bar 6/8) | 🟨 delay-zero: clean pass; fault trials running |
| PyTorch | P3 | native histogram (`pytorch-torch-histc`) | 🟨 max: clean pass; fault 0/8 (bar 6/8); bank-capacity trial queued | 🟨 Clean pass; reviewed fault ready; delay=0 qualification queued |
