...
| Main E2E | P4 | hip-moi tree atomic-OR (`tree-atomic-or`) | 🟩 high: clean pass; fault 8/8 with complete publication evidence | 🟨 delay-zero: filtered clean pass; fault trials pending |
| Test corpus | P0 | HipKittens CDNA5 naive BF16 (`hipkittens-bf16fp32-cdna5-naive`) | 🟨 default: filtered clean pass; fault trials pending | 🟨 delay-zero: filtered clean pass; fault trials pending |
...
| Tensile | P1 | `007_sk_mxf4gemm_tdm` (`tensile-sk-mxf4gemm-tdm`) | 🟧 default: transform rejected as unsupported before execution (all 6 shards) | 🟥 delay-zero: numerical checks pass, but clean reports a SuperCollider mismatch |
...
| Tensile | P2 | `005_sk_f8gemm_quick` (`tensile-sk-f8gemm-quick`) | 🟧 default: 3/9 shards exceeded the 420 s client deadline; longer run pending | 🟧 campaign interrupted by host OOM; bounded clean rerun pending |
