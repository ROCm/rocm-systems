...
| Tensile | P1 | `001_sk_mxf8f4gemm_tdm` (`tensile-sk-mxf8f4gemm-tdm`) | 🟧 all 3 clean shards pass numerically; coverage incomplete at memory-backed scalar spills | 🟧 delay-zero: 2/3 clean shards pass; third hits the 300 s client deadline |
| Tensile | P1 | `004_sk_mxf8gemm_tdm` (`tensile-sk-mxf8gemm-tdm`) | 🟧 all 6 clean shards pass numerically; coverage incomplete at memory-backed scalar spills | 🟨 delay-zero: filtered clean pass; fault trials pending |
| Tensile | P1 | `007_sk_mxf4gemm_tdm` (`tensile-sk-mxf4gemm-tdm`) | 🟧 192 MiB report cap: all 6 clean shards pass numerically; memory-backed scalar spill coverage incomplete | 🟨 delay-zero: all 6 clean shards pass after address-bank fixes; fault trials pending |
