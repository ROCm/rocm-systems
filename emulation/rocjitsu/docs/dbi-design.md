...
| `ResolvedInstrumentationSite` | post-validation | `anchor_offset`, `original_size`, `original_bytes`, `mnemonic`, `kind`, `probe_index` (set for probe calls), `probe_args`, `force_full_exec` |
...
**The policy is a property of the probe, not of the site**, so it joins the argument shape in `ProbeKey`: two points naming one probe with different mask policies resolve to different `ProbeCallable`s rather than sharing one. Nothing verifies the claim that a probe is uniform: the caller declares it, the same way it declares the argument count. What keeps that from being dangerous is gate 5: a probe with implicit live-ins is rejected outright, so a full-exec probe can only read the arguments the trampoline handed it, and those were written under `EXEC = -1` and are therefore defined in every lane.
