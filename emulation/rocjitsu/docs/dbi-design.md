...
#### `live_at_anchor` is kernel-scoped, and three ways of computing it fail closed

`LivenessAnalysis` models exactly one kernel CFG and silently ignores edges leaving its block span, so one analysis over every decoded block answers from a graph that splices unrelated kernels together. `compute_anchor_liveness()` therefore builds one scope per kernel descriptor entry using the shared walk in `code/kernel_scope.h`, with the same context-sensitive call/return edges DBT uses (`code/scoped_cfg_edges.h`). A block owned by several scopes — a device function called by several kernels — is analyzed once per owning scope and the site takes the **union**, since a register live in any context that reaches the site must be preserved there.

Three conditions reject a probe-call site rather than plan against a live set that cannot be trusted. All three under-report what is live, which is the direction that lets a probe clobber guest state:

- **Anchor not reachable from any kernel descriptor entry.** No scope to analyze it in.
- **Anchor inside a device function whose address is only produced by a relocated function-pointer slot** (`discover_relocation_function_tables`). Its callers are not statically known, so what they still need at its return cannot be derived from the body. Supporting this needs a conservative caller-saved ABI floor, which does not exist yet.
- **Anchor can reach an indirect transfer with no known destination** — a block ending in `INDIRECT_BRANCH`/`INDIRECT_CALL` with no in-scope successor and no `ScopedCfgEdge`. `BasicBlock::build` records a `CallEdge` only when the callee returns through the exact SGPR pair the call site saved, so a helper that copies the link pair first is demoted to an ordinary successor and its return becomes such a sink. Note this is *not* the same as "cannot reach a program exit": an unrelated `s_endpgm` or the `s_trap 2` a HIP `assert()` lowers to satisfies that weaker question while leaving the unmodeled return exactly as wrong.

Liveness is built with no `ExecMaskAnalysis`, matching `BinaryTranslator`: every EXEC-masked vector def stays `Unknown` and is never promoted to a kill, so live sets are over-approximated and DBI spills more than strictly necessary. That is the safe direction (see issue #9733).
