...
Owns ELF-level mutations. The DBI orchestrator reads the original payload via `text_bytes()`, assembles the new `.text` locally (each anchor spliced in place, then the probe bodies, the entry prologue if there is one, and every trampoline appended after the original bytes with `append_words()`), and applies it with a single `replace_text()`. A patch carrying an entry prologue then adds its `.rocjitsu.kernarg` record through `append_nonalloc_section()`, and `emit()` returns the patched ELF buffer. When a site spills, the orchestrator also calls `set_private_segment_fixed_size(descriptor_file_offset, SpillManager::total_private_bytes())` to grow the kernel's scratch reservation to cover the DBI spill zone before `replace_text`. The patcher accepts any well-formed mutation request; layout decisions stay in the orchestrator. Trampolines live inside `.text` as a local code cave (the same layout DBT uses) rather than a separate section, so cave offsets are plain `.text`-relative bytes in `[text_size, text_size + cave_bytes)`.
...
## Kernel-entry prologue and framework storage

A probe at an arbitrary site cannot be handed a runtime value, because no register holding a launch value survives that far: the kernarg segment pointer is dead within a handful of instructions in every compiled kernel measured. A value a probe needs must therefore be captured at kernel entry and parked where per-site register selection cannot take it. That is what `code/patch/entry_prologue.h` provides, and `ProbeArgSource::LogBufferPtrLo` / `LogBufferPtrHi` are what read it back.

### Reserved storage

`plan_dbi_entry_storage` picks one aligned run of `kDbiEntryStorageRegisters` (4) SGPRs: a **persistent pair** written at entry and read at every later site, plus an **entry-only temp pair** live just inside the prologue. Four rather than two because the prologue restores the guest's kernarg pointer, which needs somewhere to stage the reload; dropping the restore would free the temp pair without re-opening storage selection.

The floor is `dbi_entry_storage_floor`: `explicit_ordinary_sgpr_bound` over every block reachable from the entry, folded with the descriptor's user-SGPR block and its enabled system SGPRs. It is **deliberately not `COMPUTE_PGM_RSRC1`'s SGPR count**, which is an allocation total whose tail can hold VCC, flat scratch, XNACK and granularity padding, so starting there can land in special-SGPR territory. The run must fit the kernel's existing allocation; the allocation is not grown, and a kernel with no room fails closed.

`explicit_ordinary_sgpr_bound` walks explicit operands, so it cannot see an M0-displaced scalar write. A kernel using `s_movreld_b32` (or RDNA `s_set_gpr_idx_on` over SGPRs) could write the reserved pair. The hole is documented at `analysis/liveness.cpp:92-97` and shared with DBT's `reserve_cdna3_virtual_lds_base_sgpr_pair`, which derives its floor from the same scan. Closing it means a mnemonic scan in the planner.

**Storage avoids what the probes clobber.** `compute_probe_reserved_registers` unions every probe body's ordinary clobbers with the link pair each returns through, and that set constrains selection. Unioned over *every* probe on the object, not only the ones reading the pointer, because the pair has to survive from entry to each later site either way. Without it, a probe whose body happened to use the chosen pair would fail at its every site with nothing the caller could do, since a body's clobber set is a property of the body rather than of the anchor.

### Prologue words

`build_dbi_entry_prologue` emits, for storage `s[res:res+3]` and kernarg pair `s[karg:karg+1]`:

```
s_load_dwordx2 s[res  :res+1], s[karg:karg+1], payload_offset
s_load_dwordx2 s[res+2:res+3], s[karg:karg+1], original_ptr_offset
s_waitcnt lgkmcnt(0)                  ; s_wait_kmcnt 0 on RDNA4
s_mov_b32 s[karg  ], s[res+2]
s_mov_b32 s[karg+1], s[res+3]
```

Both loads precede the restore because the CP delivers a pointer to the rocjitsu wrapper and the restore overwrites it, after which nothing in the wrapper is reachable. The temp pair exists because the second load cannot name the kernarg pair as both destination and address. The wait sits ahead of the **restore**, not merely ahead of the reads below it, because the restore writes the SBASE both loads are still in flight against.

The kernarg pair is the loads' address operand while the storage pairs are their destinations, so the two ride different SMEM fields of different widths (`kMaxSmemSbase` 126, `kMaxSmemSdata` 127) and are range-checked against the field each actually occupies.

### Placement

`Instrumentor::plan_entry_prologue` synthesizes a site at the kernel's entry offset and runs it through the ordinary `make_base_plan` + `TrampolineBuilder::build` path, so the entry becomes an anchor like any other and its original instruction runs from the cave after the prologue words. There is no second entry-patch mechanism. DBT's `append_relocated_kernel_text` is not reusable here: it reserves a NOP-padded stub in a freshly built `.text` ahead of a relocated body, and DBI splices in place, so there is no delta to rebase and no room ahead of the entry. The cave becomes `[probe bodies][entry prologue][trampolines]`.

The prologue is **not idempotent**. It is reached by any transfer to the entry offset and destroys the wrapper pointer on the way out, so a kernel whose loop header is its first instruction would re-run it with the pointer already restored. `is_relocatable_anchor` rejects the entry *being* a branch; nothing checks it being a branch *target*.

### The kernarg wrapper contract

The offsets the prologue bakes in come from `make_kernarg_extension_layout(desc.kernarg_size, {kDbiEntryPayloadLayout})`: a byte-for-byte copy of the guest's kernarg image, then the guest's own kernarg pointer, then the 8-byte DBI payload. Copying the prefix is what keeps CP kernarg preloads and source kernarg offsets valid.

On the success path the instrumentor writes a `.rocjitsu.kernarg` record under variant `kDbiKernargVariantName` (`"dbi"`) with one payload named `kDbiEntryPayloadName` (`"dbi-log-buffer"`), so a runtime can build a conforming wrapper without reproducing the prologue's arithmetic. Records are keyed by `(kernel_name, variant_name)`, which discriminates within one serialized payload rather than across sections: the loader-side reader stops at the first section of this name, so the instrumentor refuses an object that already carries one.

**The patched descriptor's `kernarg_size` is deliberately left at the guest's own size**, unlike DBT, which grows it to the wrapper size when it declares an extension. Nothing here allocates a wrapper either: the record states the contract, and the runtime that honours it is not in this change. Until it lands, both prologue loads address past the end of a real kernarg allocation, so a patched object with an entry prologue is simulation-only. The simulator tests work because the harness hands the CP a wrapper directly.

`kernarg_segment_ptr_slot` and `CommandProcessor::init_wavefront_regs` walk the user-SGPR enable-bit order independently, and the CP interleaves register writes with its walk so it cannot call the helper. `tests/dbi/kernarg_sgpr_slot_test.cpp` dispatches every combination to pin the agreement; a drift would have the prologue load from an unrelated pair with no fault and every unit test still green.

### Fail-closed list

Each aborts the whole patch with its own diagnostic. The four the planner raises also name the probe that required a prologue, since nothing in the kernel asked for one and the message would otherwise read as unprompted:

- the kernel preloads kernargs (`kernarg_preload != 0`). A CDNA3/CDNA4 kernel compiled with preloading has two hardware entries 256 bytes apart, and splicing only the descriptor entry would leave the reserved pair uninitialized on the firmware path. Preloading is opt-in, so no realistic workload pays today
- a multi-kernel code object, or no discovered SGPR allocation
- a descriptor with no `ENABLE_SGPR_KERNARG_SEGMENT_PTR`
- a kernel entry that does not decode, is not a relocatable anchor, or sits inside an `s_clause` run
- a code object that already declares a kernarg extension
- a queued point whose bytes overlap the kernel entry
- no aligned run left after excluding the probes' clobbers
- storage that is misaligned, out of SMEM field range, not one contiguous run, or overlapping the kernarg pair
- a wrapper offset past the target's SMEM immediate range

---

## Testing

- **Unit (`tests/patch/instrumentor_test.cpp`):** Validator coverage (each rejection path on synthetic anchors, including the bounds-overflow regression for `is_relocatable_anchor`), inline-nop plan guardrail, `make_trampoline_plan`, end-to-end `patch()` on a synthetic ELF (expected anchor splice + trampoline layout + reparse), a decoded round-trip of every word in the emitted trampoline, the probe-call path (probe body copied once and the trampoline call targets it), the spill formula, per-class spill planning (`plan_vgpr/sgpr/acc_spills` — ascending slots, class/arch/offset-cap rejections, the kernel-VGPR-count bridge bound), the drain ordering (`expect_drain_before_store` / `expect_drain_after_return`), EXEC/VCC/M0 preservation (incl. unconditional EXEC/VCC), the probe declaration contract (two points disagreeing on argument count, argument sources, or mask policy are each rejected; two points agreeing on all three share one copied body; two points differing only in their immediate *values* also share one body while each trampoline materializes its own constant), the Wave32/Wave64 `AnchorExecHi` pair (rejected on a Wave32 kernel, accepted on a Wave64 one), and the fail-closed cases (FLAT_SCRATCH clobber, zero-scratch, SGPR temp past the kernel allocation, arguments past the kernel's ordinary-VGPR bound). For the entry prologue: the orchestrator-level gates, the cave layout (the region between the probe bodies and the site trampoline holds both loads at the declared offsets, the restores, and the wait ordered ahead of them, with the storage pair recovered from the argument `v_mov` so the assertion is that the prologue wrote the pair the site reads), the `.rocjitsu.kernarg` round trip, and three storage-selection cases: the run moves off a probe's clobbers, exhausting every aligned run fails closed, and the exclusion covers a probe that never reads the pair.
...
- **Entry-prologue sim e2e (`tests/dbi/dbi_entry_prologue_sim_test.cpp`):** The only suite that *runs* the prologue. Two kernels, because its two halves are observed differently. The delivery kernel destroys the kernarg SGPR pair before the anchor, which is what makes the reason for the prologue observable rather than assumed: a probe that still receives the payload pointer received it from the framework's storage and not from a register that happened to survive. Emitting the argument from the kernarg pair instead of the reserved pair fails it on all three targets. The restore kernel reads the pair back into VGPRs after the anchor, which both observes the restore and keeps the pair live there, since dead registers are what the call envelope takes for its own temps. Two negative controls: nopping the payload `s_load_dwordx2` must stop the pointer arriving, and nopping the two restore moves must leave the guest seeing the CP's wrapper pointer. The wrapper image is built through `write_kernarg_extension_wrapper`, so the offsets loaded from are the ones a runtime would write. Parameterized across **CDNA3, CDNA4, and RDNA4**, though `DbiSim` dispatches Wave64 on all three; the prologue is pure scalar, so nothing here turns on the executing wave size. **No nop-the-wait control:** the simulator retires scalar loads synchronously, so it would fail to fail. Separately, `build_s_load_dwordx2` always sets RDNA4's SMEM `soffset` to NULL, so although the model reads that field nothing exercises a non-NULL value. Both need hardware.
- **CP kernarg placement (`tests/dbi/kernarg_sgpr_slot_test.cpp`):** Pins `CommandProcessor::init_wavefront_regs`'s user-SGPR walk against `kernarg_segment_ptr_slot` by dispatching all eight combinations of the properties preceding the kernarg pointer. Each case writes the expected slot as a literal and asserts both sides independently, so neither computes the other; a ninth case checks the adjacent pairs are clear. A consistency check between two in-tree copies of the AMDHSA contract, not a conformance check against hardware.
- **Unit (`tests/patch/entry_prologue_test.cpp`):** The storage planner and the prologue words on their own: the floor rule (operand scan vs ABI block, disabled workgroup dimensions), run placement above the floor and below the reserved link pair, the emitted word vector, and the rejections the orchestrator never reaches -- kernarg preload, a non-contiguous run, a run overlapping the kernarg pair, an odd or out-of-field pair base, and a wrapper offset past the target's SMEM immediate. Also the one case where the kernarg pair is not at s[0:1], which is what keeps the SBASE plumbing honest.
