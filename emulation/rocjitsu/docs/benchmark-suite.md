...
The suite runs sequentially on `gfx950` and `gfx1250`, with eight simulator threads.
...
failure. Diagnostics are uploaded for failed runs. Hosted results retain the
demo label because these CI runners are not the reserved benchmark host.
