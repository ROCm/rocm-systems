...
The translation paths currently wired into `BinaryTranslator` are:

- CDNA4 to CDNA3;
- CDNA4 to RDNA3; and
- CDNA4 to RDNA4.

Virtual LDS sidecars are currently specific to CDNA4 to CDNA3. The generated
ISA definitions and legalization tables cover more architectures than these
runtime translation paths.
...
+---------------------+
    ...
    v
+---------------------+
| Place and relocate   |  entry stubs, branches, calls, PC builders
+---------------------+
    |
    v
+---------------------+
...
DBT preserves at least the larger of those ranges and rounds the preload end to
an 8-byte boundary so it does not copy only half of a 64-bit value.
...
The default policy is fail closed. Examples include:

- a descriptor ABI or resource request the host cannot represent;
- an unrecovered reachable indirect branch;
- a branch or entry stub that cannot reach its relocated target;
- an `EXPAND` legalization without a semantic rule;
- unavailable scratch resources; or
- an unsupported virtual-LDS storage instruction.

There are two distinct debugging/runtime alternatives:

- `debug_continue_after_failure` copies failed source instructions only to
  collect more diagnostics. If any error was seen, the final code object is left
  unchanged; the partial target text is never returned for execution.
- `skip_failed_kernels` rolls back the failed scope and emits a minimal target
  `s_trap`/`s_endpgm` kernel stub. Other independent kernels can still be
  translated. A skipped preload kernel gets a trap stub at both hardware entry
  slots.

The load-time HSA path enables skipped-kernel handling so one unused unsupported
kernel does not prevent the rest of a large code object from loading.

## Implementation Map

The main files are:

- `lib/rocjitsu/src/rocjitsu/code/dbt/binary_translator.cpp` — phase
  orchestration, kernel transactions, and instruction dispatch.
- `lib/rocjitsu/src/rocjitsu/code/basic_block.cpp` — block construction,
  direct edges, and call modeling.
- `lib/rocjitsu/src/rocjitsu/analysis/indirect_branch_discovery.cpp` — static
  `s_getpc_b64` PC-builder recovery.
- `lib/rocjitsu/src/rocjitsu/analysis/liveness.cpp` — kernel-scoped register
  liveness used by semantic scratch allocation.
- `lib/rocjitsu/src/rocjitsu/code/dbt/semantic/` — handwritten semantic
  expansions.
- `lib/rocjitsu/src/rocjitsu/code/dbt/semantic_scratch.cpp` — reusable VGPR and
  private-spill allocation policy.
- `lib/rocjitsu/src/rocjitsu/code/dbt/lds_virtualization.cpp` — virtual-LDS
  entry state and ordinary DS storage lowering.
- `lib/rocjitsu/src/rocjitsu/code/dbt/kernel_descriptor_translator.cpp` —
  descriptor ABI and resource planning.
- `lib/rocjitsu/src/rocjitsu/code/patch/kernel_text_layout.cpp` — final kernel
  placement and control-flow relocation.
- `lib/rocjitsu/src/rocjitsu/code/patch/code_object_patcher.cpp` — ELF mutation.
- `lib/rocjitsu/src/rocjitsu/hooks/rj_hsa_dbt_hooks.cpp` — load-time translation
  policy and virtual-LDS dispatch rewriting.
- `lib/python/amdisa/legalization.py` and generated files under
  `lib/rocjitsu/src/rocjitsu/code/dbt/generated/` — legalization and encoding
  translation generation.

For hands-on inspection of a translation, see
[`rj_dbt_translate`](rj_dbt_translate.md).
