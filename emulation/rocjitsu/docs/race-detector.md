...
- **LDS races**: an LDS byte is read or written by one wave while another wave
  has an outstanding conflicting access to the same byte, without an
  intervening `s_barrier`. Same-wave native DS accesses are ordered; a wave
  that reads bytes targeted by its own outstanding direct-to-LDS operation
  must first issue the required `s_waitcnt vmcnt`.
