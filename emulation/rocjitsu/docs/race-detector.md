...
- **VGPR races**: a vector register is read or overwritten by an instruction
  ...
  `s_waitcnt lgkmcnt` insufficient), or asynchronous loads from different
  completion domains overwrite the same VGPR before the older load completes.
...
- **LDS races**: an LDS byte is read by one wave while another wave has an
  ...
  `s_barrier`. Same-wave native DS accesses are ordered; a wave that reads
  bytes targeted by its own outstanding direct-to-LDS operation must first
  issue the required `s_waitcnt vmcnt`.
