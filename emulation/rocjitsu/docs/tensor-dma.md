...
## Reusing the LDS mapping

`lds_element_offset` and `for_each_lds_run` in
`isa/arch/amdgpu/shared/tensor_dma.h` state, apart from the copy itself, where a
descriptor places its elements in LDS: the offset one element lands on, padding
included, and the stretches of the element stream a transfer covers — one per
iteration for a dense tile, one for a gather. The copy loops address LDS through
the same two, so an observer deriving the LDS a transfer touches, as the data
hazard plugin does to track the tile a `tensor_load_to_lds` writes, stays in step
