...
6. Override the per-hook `observes_*()` methods for the high-frequency hooks the
   ...
   thread-safe, and stable after dispatch. If hot hooks depend on per-wave
   state, install it with `set_wavefront_state()` and return false when
   `wavefront_state()` is null so live group replacement cannot expose an
   uninitialized wave. A consumer of
