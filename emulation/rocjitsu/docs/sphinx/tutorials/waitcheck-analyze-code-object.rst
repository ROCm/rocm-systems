.. Generated by ROCm docgen prompt_version=2026-06-14-v24-ship-gitattributes-editorconfig at 2026-07-20T14:28:35Z
...
When a wait is missing or too weak, the hardware might read a register before
the memory operation that produces its value has completed. These *wait
hazards* cause silent data corruption that is difficult to reproduce and
diagnose at runtime.

rocJITsu's **waitcheck** tool performs a static, forward dataflow analysis
over the final encoded instruction stream in an AMDGPU code object. It
tracks outstanding wait-counter events across the control-flow graph and
reports every instruction pair where a consumer uses a result before the
relevant counter has been waited on strongly enough.
...
(CDNA4), ``gfx1100`` (RDNA3), ``gfx1200`` (RDNA4), ``gfx1201`` (RDNA4),
and ``gfx1250``.
...
``loadcnt``, ``storecnt``, ``dscnt``, ``kmcnt``, ``samplecnt``, ``bvhcnt``,
and ``expcnt``. A hazard is reported when an instruction consumes a register
that was written by an in-flight memory operation whose specific counter has
not been waited on.
...
``lgkmcnt``, and ``expcnt`` fields. RDNA3 adds a separate ``vscnt``. A
hazard is reported when the encoded wait value is not strong enough (the
...
``depctr_sa_sdst``, where a scalar or vector ALU result is consumed
before the pipeline stage that produces it has drained. Embedded wait
fields in DSDIR and VINTERP instructions are also tracked.
...
When a hazard is found, waitcheck prints a diagnostic that identifies the
*producer* instruction (the memory or ALU operation that writes a result),
the *consumer* instruction (the instruction that reads that result too
early), and the wait that is missing or insufficient between them.

A typical diagnostic identifies:

- The section-relative offset of the producer and consumer (for example
  ``.text+0x1a0`` and ``.text+0x1b8``).
...
is the corresponding split wait (``s_wait_loadcnt``, ``s_wait_dscnt``,
and so on).
...
To check kernels at dispatch time without modifying the application, load
the waitcheck HSA tools library:
...
- See :doc:`/reference/waitcheck` for the full CLI option reference,
  environment variables, exit codes, and runtime tool configuration.
- See :doc:`/tutorials/inspect-code-object` for loading and disassembling
  code objects through the rocJITsu C API.
