...
| `async_helper_threads` | int | VM-wide async helpers; omitted/-1 selects from preferred allocations, 0 disables helpers. |
| `cpu_thread_budget` | int | Selection ceiling; omitted/0 uses affinity capped at 32, positive values override it. |
| `thread_allocations` | array | Preferred E/D/H triples for this target. Largest fitting effective allocation wins. |
...
dispatch workers and VM-wide async helpers: E + sum(D - 1) + H. Explicit knobs
override the selected triple; configs without a table use serial defaults.
Clocked mode always uses serial dispatch and no async helpers. See the source
