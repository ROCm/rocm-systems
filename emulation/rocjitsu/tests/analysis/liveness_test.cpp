...
TEST(CfgAnalysis, Gfx1250ClassifiesImplicitUnreachableStubTerminator) {
  ...
  const std::array cases = {
      ...
      Case{"ordinary fallthrough",
           {build_s_nop(0, ROCJITSU_CODE_ARCH_CDNA5), 0},
           false,
           false,
           true},
  ...
  for (const auto &test_case : cases) {
    ...
    auto blocks = BasicBlock::build(co, *decoder, ROCJITSU_CODE_ARCH_CDNA5);
...
TEST(CfgAnalysis, Gfx1250BankedLaneStashDoesNotSurviveDirectCall) {
  ...
  std::vector<uint32_t> words = {
      ...
      build_s_nop(0, ROCJITSU_CODE_ARCH_CDNA5), // 0x44: conforming callee body.
      rocjitsu::build_s_setpc_b64(kReturnSreg, ROCJITSU_CODE_ARCH_CDNA5),
      // 0x48: callee return.
  };
