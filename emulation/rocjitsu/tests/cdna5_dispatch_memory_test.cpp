...
TEST(Gfx1250SimulationTest, BatchedDispatchesPreloadTheirOwnAqlPacketIds) {
  ...
  std::array<uint64_t, 2> preloaded_dispatch_ids{sim.snapshot->snapshots()[0].sgpr64(0u),
                                                 sim.snapshot->snapshots()[1].sgpr64(0u)};
  std::ranges::sort(preloaded_dispatch_ids);
