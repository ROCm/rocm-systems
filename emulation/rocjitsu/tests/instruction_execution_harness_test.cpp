...
protected:
  ...
  uint64_t execute_ds(uint16_t opcode, uint64_t old_bits, uint64_t source_bits,
    ...
    const bool comparison = (opcode % 32) == 16 || (opcode % 32) == 17;
  ...
  std::array<uint32_t, 3> memory_words(bool buffer, bool compare = false) const {
    ...
    case ROCJITSU_CODE_ARCH_CDNA1:
      if (buffer)
        ...
        words =
            cdna1::build_flat(0, {.seg = 2, .glc = 1, .addr = 4, .data = 0, .saddr = 4, .vdst = 6});
      break;
    case ROCJITSU_CODE_ARCH_CDNA2:
      if (buffer)
        ...
        words =
            cdna2::build_flat(0, {.seg = 2, .glc = 1, .addr = 4, .data = 0, .saddr = 4, .vdst = 6});
      break;
    case ROCJITSU_CODE_ARCH_CDNA3:
      if (buffer)
...
TEST_P(AtomicPolicyExecutionTest, DsScalarDenormalModesAndRounding) {
  ...
  for (uint16_t opcode : {18, 19}) {
    ...
    EXPECT_EQ(execute_ds(opcode, 0x7fc00002, 0x7fc00004, 0, 0), 0x7fc00002u);
    EXPECT_EQ(execute_ds(opcode, 0x7fc00002, 0x7f800004, 0, 0),
              modern_policy() ? 0x7fc00002u : 0x7fc00004u);
  }
}
...
TEST_P(AtomicPolicyExecutionTest, MemoryAddDenormalsAndHostRounding) {
  ...
  for (bool buffer : {false, true}) {
    // CDNA1/2 lack a decoded global F32-add form; qualify their buffer form.
    if (!buffer &&
        (GetParam() == ROCJITSU_CODE_ARCH_CDNA1 || GetParam() == ROCJITSU_CODE_ARCH_CDNA2))
      continue;
    for (uint32_t mode : {0u, 0xffu}) {
      EXPECT_EQ(execute_memory(buffer, 1, 1, mode), modern_policy() ? 2u : 0u);
      EXPECT_EQ(execute_memory(buffer, 0x00800001, 0x80800000, mode), modern_policy() ? 1u : 0u);
      for (int host_round : {FE_TONEAREST, FE_UPWARD, FE_DOWNWARD, FE_TOWARDZERO}) {
        const int saved_round = std::fegetround();
        ASSERT_EQ(std::fesetround(host_round), 0);
