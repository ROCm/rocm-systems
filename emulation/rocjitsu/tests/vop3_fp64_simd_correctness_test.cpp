...
/// f64 VOP3 binary (add/mul/max/min) and unary (ceil/floor/trunc/rndne/sqrt)
/// ops on CDNA4. Reads src0/src1 as native<double> via split lo/hi VGPR pairs,
/// applies per-source abs/neg and result omod/clamp in the f64 domain (the
/// new apply_vop3_*_mod_f64 helpers), then op. Bit-exact for finite/Inf on the
/// add/mul/cvt/sqrt ops; the min/max ops accept the standard ±0 / NaN payload
/// divergences (test skips those lanes, matching the v_max_f32 / v_min_f32
/// convention).
...
void check_case(const Case &c, uint32_t abs, uint32_t neg, uint32_t omod, uint32_t clamp,
  ...
  for (uint32_t rot = 0; rot < kRotMax; ++rot) {
    ...
    for (uint32_t lane = 0; lane < WF_SIZE; ++lane) {
      ...
      if (!active) {
        EXPECT_EQ(simd[lane], dst_sentinel(lane)) << c.name << " abs=" << abs << " neg=" << neg
      ...
      if (c.kind == Kind::BinMinMax) {
        ...
        if (is_f64_nan(a) || is_f64_nan(b))
          continue; // accepted NaN-payload divergence
        if (is_f64_zero(a) && is_f64_zero(b))
          continue; // accepted ±0-tie divergence
      }
      // For NaN results (any op), payload may differ — accepted.
