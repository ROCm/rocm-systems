  ...
  void initialize(uint32_t num_wf_slots, uint32_t num_xcds, std::string_view arch,
    ...
    for (uint32_t xcd = 0; xcd < num_xcds; ++xcd) {
      for (uint32_t cu = 0; cu < num_cus; ++cu) {
        ...
        links += R"({"src":")" + prefix + ".cp.req_" + std::to_string(cu) +
                 R"(","dst":")" + prefix + ".se0.cu" + std::to_string(cu) +
                 R"(.cpl","latency":1,"weight":2})";
        links += R"(,{"src":")" + prefix + ".se0.cu" + std::to_string(cu) +
                 R"(.req","dst":")" + prefix + ".l2.cpl_" + std::to_string(cu) +
          ...
          {{"name":"se0","type":"shader_engine","children":[
            {{"name":"{}","type":"compute_unit","config":[
              ...
              {{"key":"lds_size_kb","value":"64"}}
                                   ...
                                   arch, wavefront_size, num_xcds, cu_range, num_wf_slots,
