...
struct PluginFixture {
  ...
  explicit PluginFixture(uint32_t num_wf_slots = 10, uint32_t num_xcds = 1)
      : PluginFixture(num_wf_slots, num_xcds, "cdna4", 64, 104, 256) {}

  PluginFixture(uint32_t num_wf_slots, std::string_view arch, uint32_t wavefront_size = 64,
                uint32_t sgprs_per_wf = 104, uint32_t vgprs_per_wf = 256)
      : PluginFixture(num_wf_slots, 1, arch, wavefront_size, sgprs_per_wf, vgprs_per_wf) {}

  PluginFixture(uint32_t num_wf_slots, uint32_t num_xcds, std::string_view arch,
                uint32_t wavefront_size, uint32_t sgprs_per_wf = 104, uint32_t vgprs_per_wf = 256) {
      ...
      "topology":{{"root":{{"name":"soc","type":"soc","children":[
        ...
        {{"pattern":"xcd[i].se0.cu0.req -> xcd[i].l2.cpl_0",
          ...
          "latency":1,"weight":10}}
                                   ...
                                   arch, wavefront_size, num_xcds, num_wf_slots, sgprs_per_wf,
                                   vgprs_per_wf, num_xcds, num_xcds);
