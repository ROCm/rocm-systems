...
/// Minimal SoC fixture with one SE per XCD and configurable XCD/CU counts.
struct PluginFixture {
  ...
  explicit PluginFixture(uint32_t num_wf_slots = 10, std::string_view arch = "cdna4",
    ...
    initialize(num_wf_slots, /*num_xcds=*/1, arch, wavefront_size, sgprs_per_wf, vgprs_per_wf,
               num_cus, async_helpers);
  }

  PluginFixture(uint32_t num_wf_slots, uint32_t num_xcds) {
    initialize(num_wf_slots, num_xcds, "cdna4", /*wavefront_size=*/64,
               /*sgprs_per_wf=*/104, /*vgprs_per_wf=*/256, /*num_cus=*/1);
  }

  void initialize(uint32_t num_wf_slots, uint32_t num_xcds, std::string_view arch,
                  uint32_t wavefront_size, uint32_t sgprs_per_wf, uint32_t vgprs_per_wf,
                  uint32_t num_cus, uint32_t async_helpers = 0) {
    std::string cu_range = "cu[0:" + std::to_string(num_cus) + "]";
    std::string links;
    for (uint32_t xcd = 0; xcd < num_xcds; ++xcd) {
      for (uint32_t cu = 0; cu < num_cus; ++cu) {
        if (!links.empty())
          links += ',';
        const std::string prefix = "xcd" + std::to_string(xcd);
        links += R"({"src":")" + prefix + ".cp.req_" + std::to_string(cu) + R"(","dst":")" +
                 prefix + ".se0.cu" + std::to_string(cu) + R"(.cpl","latency":1,"weight":2})";
        links += R"(,{"src":")" + prefix + ".se0.cu" + std::to_string(cu) + R"(.req","dst":")" +
                 prefix + ".l2.cpl_" + std::to_string(cu) + R"(","latency":1,"weight":10})";
      }
    }
          ...
          {{"name":"se0","type":"shader_engine","children":[
            {{"name":"{}","type":"compute_unit","config":[
              ...
              {{"key":"lds_size_kb","value":"64"}}
                                   ...
                                   async_helpers, arch, wavefront_size, num_xcds, cu_range,
                                   num_wf_slots, sgprs_per_wf, vgprs_per_wf, links);
  ...
  amdgpu::ComputeUnitCore *cu(uint32_t idx = 0, uint32_t xcd = 0) {
    return soc->xcd(xcd)->shader_engine(0)->compute_unit(idx);
  }
  ...
  uint64_t write_kernel(uint64_t addr, const uint32_t *code, size_t num_words,
    ...
    const uint32_t granule =
        descriptor_vgpr_count_granule_for_wavefront(cu()->arch(), cu()->wf_size()).value();
