...
TEST_F(SimulatedKfdTest, ScratchBackingGrowthDoesNotMistakePassthroughForMappedPages) {
  ...
  ASSERT_TRUE(memory->has_page_mapping(scratch_va + kInitialSize, process_id))
      << "identity passthrough deliberately remains visible to coarse mapping queries";
  ASSERT_EQ(memory->resolve_host_ptr(scratch_va + kInitialSize, process_id), nullptr)
      << "the inaccessible reservation must not become a dereferenceable host pointer";
