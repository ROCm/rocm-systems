...
void expect_virtual_lds_smoke_metadata(const std::vector<uint8_t> &elf_bytes) {
  ...
  EXPECT_FALSE(read_virtual_lds_descriptor_dispatch_metadata(virtual_kd).has_value())
      << "sidecar descriptors must not be eligible fallback inputs";
  ...
  EXPECT_EQ(dispatch_metadata->backing_pointer_kernarg_offset,
            record->backing_pointer_kernarg_offset);
  EXPECT_NE(dispatch_metadata->flags & kVirtualLdsFlagRuntimeStateBlock, 0u);
  EXPECT_NE(dispatch_metadata->flags & kVirtualLdsFlagWorkgroupIdX, 0u);
}
...
void run_virtual_lds_smoke(const std::vector<uint8_t> &elf_bytes, const Cdna3Target &target) {
  ...
  const bool packet_pointer = (record->flags & kVirtualLdsFlagBackingPointerInDispatchPacket) != 0;
  const size_t kernarg_bytes =
      packet_pointer
          ? std::max<size_t>(record->kernarg_size, sizeof(KernArgs))
          : static_cast<size_t>(record->backing_pointer_kernarg_offset) + sizeof(RuntimeState);
  void *kernarg = nullptr;
  ...
  RuntimeState runtime_state{.backing_base = reinterpret_cast<uintptr_t>(backing_dev),
                             ...
                             .stride_z = 0,
                             .reserved = 0};
  void *packet_state = nullptr;
  if (packet_pointer) {
    ASSERT_EQ(record->backing_pointer_kernarg_offset,
              static_cast<uint32_t>(offsetof(hsa_kernel_dispatch_packet_t, reserved2)));
    ASSERT_EQ(hsa_amd_memory_pool_allocate(kernarg_pool, sizeof(RuntimeState), 0, &packet_state),
              HSA_STATUS_SUCCESS);
    ASSERT_EQ(hsa_amd_agents_allow_access(2, both, nullptr, packet_state), HSA_STATUS_SUCCESS);
    std::memcpy(packet_state, &runtime_state, sizeof(runtime_state));
  } else {
    std::memcpy(static_cast<uint8_t *>(kernarg) + record->backing_pointer_kernarg_offset,
                &runtime_state, sizeof(runtime_state));
  }
  ...
  aql->kernarg_address = kernarg;
  if (packet_pointer)
    aql->reserved2 = reinterpret_cast<uintptr_t>(packet_state);
  aql->completion_signal = signal;
  ...
  hsa_signal_destroy(signal);
  hsa_queue_destroy(queue);
  if (packet_state != nullptr)
    hsa_amd_memory_pool_free(packet_state);
  hsa_amd_memory_pool_free(kernarg);
