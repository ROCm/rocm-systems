...
/// @file dbt_binary_e2e_test.cpp
/// @brief CPU-only DBT integration tests backed by compiled device kernels.
...
TEST(BinaryTranslatorE2E, TraceRecordsInstructionTranslationsWhenEnabled) {
  Executable exec(kernel_path("vector_add"));
  ASSERT_TRUE(exec.is_valid()) << "Failed to load vector_add.o";
  ASSERT_GT(exec.num_code_objects(ROCJITSU_CODE_TARGET_GFX950), 0u);
  ...
  struct TraceSnapshot {
    ...
    rocjitsu::Action action = rocjitsu::Action::Identity;
    bool semantic_lowering = false;
    bool emitted_in_cave = false;
    size_t target_word_count = 0;
  };

  // The trace callback is intentionally opt-in. Normal translation paths
  // should not pay for collecting per-instruction target words unless a
  // debugging tool or test asks for them.
  std::vector<TraceSnapshot> trace;
  BinaryTranslator translator(ROCJITSU_CODE_ARCH_CDNA4, ROCJITSU_CODE_ARCH_RDNA4);
  translator.set_trace_callback([&](const rocjitsu::TranslationTraceEvent &event) {
    trace.push_back({.has_legalization = event.legalization != nullptr,
                     .action = event.legalization ? event.legalization->action
                                                  : rocjitsu::Action::Identity,
                     .semantic_lowering = event.semantic_lowering,
                     .emitted_in_cave = event.emitted_in_cave,
  ...
  const auto has_semantic_expansion = std::ranges::any_of(trace, [](const auto &entry) {
    return entry.semantic_lowering && entry.emitted_in_cave && entry.target_word_count != 0;
  });
  EXPECT_TRUE(has_semantic_expansion);

  const auto has_cave_lowering = std::ranges::any_of(trace, [](const auto &entry) {
    return entry.has_legalization && entry.action == rocjitsu::Action::Lower &&
           entry.emitted_in_cave && entry.target_word_count != 0;
  });
  EXPECT_TRUE(has_cave_lowering);
}

TEST(BinaryTranslatorE2E, TranslatesMultiKernelCodeObject) {
  using KD = rocr::llvm::amdhsa::kernel_descriptor_t;

  auto image = rocjitsu::dbt_test::make_minimal_amdgpu_elf_with_two_kernel_descriptors();
  rocjitsu::AmdGpuCodeObject co(image.data(), image.size());
  ASSERT_TRUE(co.is_valid());
  ASSERT_FALSE(co.text_sections().empty());
  const auto *text = co.text_sections()[0];
  const auto *rodata = rocjitsu::dbt_test::find_section(co, ".rodata");
  ...
  EXPECT_EQ(rocjitsu::dbt_test::find_section(translated, ".rj_translations"), nullptr)
...
TEST(KernelDescriptorTranslator, IgnoresNonAllocExecutableSectionsForEntryRange) {
  ...
  auto image = rocjitsu::dbt_test::make_minimal_amdgpu_elf_with_descriptor_after_text();
...
TEST(BinaryTranslatorE2E, DISABLED_WriteTranslatedElfToFile) {
  Executable exec(kernel_path("vector_add"));
  ASSERT_TRUE(exec.is_valid());
  ASSERT_GT(exec.num_code_objects(ROCJITSU_CODE_TARGET_GFX950), 0u);
...
TEST(BinaryTranslatorE2E, DISABLED_DumpTranslation) {
  Executable exec(kernel_path("vector_add"));
  ASSERT_TRUE(exec.is_valid());
  ASSERT_GT(exec.num_code_objects(ROCJITSU_CODE_TARGET_GFX950), 0u);
