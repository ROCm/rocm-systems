...
TEST(CodeObjectPatcher, PreservedAdjacentFunctionIsNotRelocatedByPrecedingBodyEnd) {
  auto image = make_minimal_amdgpu_elf_with_load_segments();
  auto ehdr = read_elf_struct_for_test<Elf64_Ehdr>(image, 0);
  const auto shdrs = read_elf_array_for_test<Elf64_Shdr>(image, ehdr.e_shoff, ehdr.e_shnum);
  ...
  ASSERT_GE(symbols.size(), 3u);
  symbols[2].st_value = 0x1104u;
  symbols[2].st_size = 4u;
  write_bytes_for_test(image, symtab->sh_offset, symbols.data(),
                       symbols.size() * sizeof(Elf64_Sym));
  ...
  const auto bytes = std::span<const uint8_t>(reinterpret_cast<const uint8_t *>(text_words.data()),
                                              sizeof(text_words));
  // The first translated body expands from source [0, 4) to target [8, 16).
  // Its end marker shares source offset 4 with the unselected next function.
  constexpr std::array<TextOffsetRelocation, 2> mappings = {
      TextOffsetRelocation{
          .source_offset = 0, .target_offset = 8, .owner_descriptor_file_offset = 64},
      TextOffsetRelocation{
          .source_offset = 4, .target_offset = 16, .owner_descriptor_file_offset = 64},
  };
  ASSERT_TRUE(patcher.replace_text(bytes, mappings, {}, {}, false, nullptr,
                                   /*source_text_prefix_is_preserved=*/true));

  const auto patched = patcher.emit();
  ehdr = read_elf_struct_for_test<Elf64_Ehdr>(patched, 0);
  const auto patched_shdrs =
      read_elf_array_for_test<Elf64_Shdr>(patched, ehdr.e_shoff, ehdr.e_shnum);
  const auto patched_symtab = std::ranges::find_if(
      patched_shdrs, [](const Elf64_Shdr &section) { return section.sh_type == SHT_SYMTAB; });
  ASSERT_NE(patched_symtab, patched_shdrs.end());
  const auto patched_symbols = read_elf_array_for_test<Elf64_Sym>(
      patched, patched_symtab->sh_offset, patched_symtab->sh_size / sizeof(Elf64_Sym));
  ASSERT_GE(patched_symbols.size(), 3u);
  EXPECT_EQ(patched_symbols[2].st_value, 0x1104u);
  EXPECT_EQ(patched_symbols[2].st_size, 4u);
}
...
TEST(BinaryTranslator, ClientRewriteOwnsBoundedMultiInstructionSourceSpan) {
  ...
  const std::vector<uint32_t> words = {
      build_s_nop(0, ROCJITSU_CODE_ARCH_CDNA4),
      build_s_nop(1, ROCJITSU_CODE_ARCH_CDNA4),
      build_s_endpgm(ROCJITSU_CODE_ARCH_CDNA4),
  };
  auto image = make_minimal_amdgpu_elf_with_descriptor_after_text(words);
  AmdGpuCodeObject source(image.data(), image.size());
  ...
  translator.set_instruction_rewrite_callback(
      [&](const InstructionRewriteContext &context) -> std::optional<InstructionRewrite> {
        if (context.source_offset != 0)
          return std::nullopt;
        return InstructionRewrite{.prefix_words = {},
                                  .replacement_words = std::vector<uint32_t>{replacement_nop},
                                  .markers = {},
                                  .source_size = 2 * sizeof(uint32_t),
                                  .preserved_source_span_byte_offset = std::nullopt};
      });
  const auto result = translator.translate(source);
  ...
  const auto translated_words = std::span<const uint32_t>(
      reinterpret_cast<const uint32_t *>(text.data()), text.size() / sizeof(uint32_t));
  ASSERT_GE(translated_words.size(), 2u);
  EXPECT_EQ(translated_words[0], replacement_nop);
  EXPECT_EQ(translated_words[1], build_s_endpgm(ROCJITSU_CODE_ARCH_CDNA4));

  const auto rewrite = std::ranges::find_if(
      result.text_placements, [](const auto &placement) { return placement.source_offset == 0; });
  ASSERT_NE(rewrite, result.text_placements.end());
  EXPECT_EQ(rewrite->client_rewrite_source_size, 2 * sizeof(uint32_t));
  EXPECT_EQ(std::ranges::count(result.text_placements, sizeof(uint32_t),
                               &TranslatedTextPlacement::source_offset),
            0);
  const auto continuation = std::ranges::find(result.text_placements, 2 * sizeof(uint32_t),
                                              &TranslatedTextPlacement::source_offset);
  ASSERT_NE(continuation, result.text_placements.end());
  EXPECT_EQ(continuation->target_offset, sizeof(uint32_t));
}

TEST(BinaryTranslator, ClientRewriteRejectsSourceSpanContainingControlTransfer) {
  const std::vector<uint32_t> words = {
      build_s_nop(0, ROCJITSU_CODE_ARCH_CDNA4),
      build_s_branch(0, ROCJITSU_CODE_ARCH_CDNA4),
      build_s_endpgm(ROCJITSU_CODE_ARCH_CDNA4),
  };
  auto image = make_minimal_amdgpu_elf_with_descriptor_after_text(words);
  AmdGpuCodeObject source(image.data(), image.size());
  ...
  translator.set_instruction_rewrite_callback(
      [&](const InstructionRewriteContext &context) -> std::optional<InstructionRewrite> {
        if (context.source_offset != 0)
          return std::nullopt;
        return InstructionRewrite{.prefix_words = {},
                                  .replacement_words = std::vector<uint32_t>{words[0]},
                                  .markers = {},
                                  .source_size = 2 * sizeof(uint32_t),
                                  .preserved_source_span_byte_offset = std::nullopt};
      });
  ...
  EXPECT_FALSE(result.ok());
  EXPECT_TRUE(has_error_containing(result, DiagnosticKind::Legalization, "bounded source span"));
}

TEST(BinaryTranslator, ClientRewriteWrapsByteIdenticalSingleEntryPollingLoop) {
  const uint32_t before = build_s_nop(7, ROCJITSU_CODE_ARCH_CDNA4);
  ...
  translator.set_instruction_rewrite_callback(
      [&](const InstructionRewriteContext &context) -> std::optional<InstructionRewrite> {
        ...
        return InstructionRewrite{
            .prefix_words = {},
            .replacement_words = std::vector<uint32_t>{before, poll, retry, finish, after},
            .markers = {},
            .source_size = 3 * sizeof(uint32_t),
            .required_ordinary_sgpr_count = 0u,
            .preserved_source_span_byte_offset = sizeof(uint32_t),
        };
      });
  const auto result = translator.translate(source);
  ...
  ASSERT_NE(placement_for(0u), result.text_placements.end());
  ASSERT_NE(placement_for(sizeof(uint32_t)), result.text_placements.end());
...
TEST(BinaryTranslator, ClientRewriteCanPreserveAnUnreachableSourceTextPrefix) {
  ...
  ASSERT_TRUE(source.is_valid());
  ASSERT_EQ(source.text_sections().size(), 1u);
  const std::vector<uint8_t> source_text(source.text_sections().front()->data(),
                                         source.text_sections().front()->data() +
                                             source.text_sections().front()->size());
  ...
  const auto *translated_words = reinterpret_cast<const uint32_t *>(text.data());
  EXPECT_EQ(translated_words[rewritten->target_offset / sizeof(uint32_t)], inserted_nop);
  EXPECT_EQ(translated_words[rewritten->target_offset / sizeof(uint32_t) + 1u], source_nop);

  const Section *rodata = find_section(translated, ".rodata");
  ASSERT_NE(rodata, nullptr);
  const auto descriptor = read_kernel_descriptor_for_test(rodata->data());
  const uint64_t entry = static_cast<uint64_t>(static_cast<int64_t>(rodata->vaddr()) +
                                               descriptor.kernel_code_entry_byte_offset -
                                               static_cast<int64_t>(text.vaddr()));
  EXPECT_EQ(entry, rewritten->target_offset)
