...
TEST(InstructionBuilder, Cdna3MemoryBuilderBuildsDs) {
  ...
  EXPECT_EQ(actual.op, 63u);
  EXPECT_EQ(actual.vdst, 5u);
  EXPECT_EQ(actual.addr, 6u);
  EXPECT_EQ(actual.data0, 7u);
  EXPECT_EQ(actual.data1, 8u);
  EXPECT_EQ(actual.offset0, 9u);
  EXPECT_EQ(actual.offset1, 10u);
}
...
TEST(InstructionBuilder, Cdna3MemoryBuilderBuildsNonLdsMubuf) {
  ...
  EXPECT_EQ(actual.op, 23u);
  EXPECT_EQ(actual.offset, 0x345u);
  EXPECT_EQ(actual.offen, 1u);
  EXPECT_EQ(actual.idxen, 0u);
  EXPECT_EQ(actual.sc0, 1u);
  EXPECT_EQ(actual.sc1, 0u);
  EXPECT_EQ(actual.lds, 0u);
  EXPECT_EQ(actual.nt, 1u);
  EXPECT_EQ(actual.vaddr, 11u);
  EXPECT_EQ(actual.vdata, 14u);
  EXPECT_EQ(actual.srsrc, 12u);
  EXPECT_EQ(actual.acc, 0u);
  EXPECT_EQ(actual.soffset, 13u);
}
...
TEST(InstructionBuilder, Cdna3MemoryBuilderBuildsFlatScratchDwordLoad) {
  ...
  EXPECT_EQ(actual.encoding, 0x37u);
  EXPECT_EQ(actual.op, 20u);
  EXPECT_EQ(actual.offset, 0x1234u & 0x1FFFu);
  EXPECT_EQ(actual.seg, 1u);
  EXPECT_EQ(actual.sve, 0u);
  EXPECT_EQ(actual.saddr, 0x7Fu);
  EXPECT_EQ(actual.vdst, 17u);
}
...
TEST(InstructionBuilder, Cdna3BuildsVop2Literal) {
  ...
  EXPECT_EQ(actual.op, 52u);
  EXPECT_EQ(actual.src0, 0xFFu);
  EXPECT_EQ(actual.vsrc1, 6u);
  EXPECT_EQ(actual.vdst, 5u);
  EXPECT_EQ(actual.simm32, 5000u);
}
...
TEST(InstructionBuilder, Cdna3MemoryBuilderBuildsFlatGlobalLoad) {
  ...
  EXPECT_EQ(actual.encoding, 0x37u);
  EXPECT_EQ(actual.op, 20u);
  EXPECT_EQ(actual.offset, 0x3u);
  EXPECT_EQ(actual.pad_12, 1u);
  EXPECT_EQ(actual.lds, 0u);
  EXPECT_EQ(actual.seg, 2u);
  EXPECT_EQ(actual.sc0, 1u);
  EXPECT_EQ(actual.sc1, 1u);
  EXPECT_EQ(actual.nt, 0u);
  EXPECT_EQ(actual.addr, 21u);
  EXPECT_EQ(actual.saddr, 22u);
  EXPECT_EQ(actual.vdst, 23u);
}
...
TEST(InstructionBuilder, Cdna3MemoryBuilderBuildsFlatGlobalStore) {
  ...
  EXPECT_EQ(actual.encoding, 0x37u);
  EXPECT_EQ(actual.op, 31u);
  EXPECT_EQ(actual.offset, 0xFFCu);
  EXPECT_EQ(actual.pad_12, 0u);
  EXPECT_EQ(actual.lds, 0u);
  EXPECT_EQ(actual.seg, 2u);
  EXPECT_EQ(actual.sc0, 0u);
  EXPECT_EQ(actual.sc1, 1u);
  EXPECT_EQ(actual.nt, 1u);
  EXPECT_EQ(actual.addr, 24u);
  EXPECT_EQ(actual.data, 25u);
  EXPECT_EQ(actual.saddr, 26u);
  EXPECT_EQ(actual.acc, 1u);
}
...
std::vector<ExpectedCdna3Inst> expected_cdna3_cvt_pk_bf16_f32_sequence() {
  return {
      expect_vop3(321), // v_mov_b32 low source into scratch.
      expect_vop3(272), // v_lshrrev_b32 extracts low BF16 lsb.
      expect_vop3(275), // v_and_b32 isolates low BF16 lsb.
      expect_vop2(52),  // v_add_u32_e32 adds 0x7fff rounding bias.
      expect_vop3(308), // v_add_u32 applies low RNE bias.
      expect_vop3(272), // v_lshrrev_b32 keeps low rounded BF16 half.
      expect_vop3(321), // v_mov_b32 high source into scratch.
      expect_vop3(272), // v_lshrrev_b32 extracts high BF16 lsb.
      expect_vop3(275), // v_and_b32 isolates high BF16 lsb.
      expect_vop2(52),  // v_add_u32_e32 adds 0x7fff rounding bias.
      expect_vop3(308), // v_add_u32 applies high RNE bias.
      expect_vop3(272), // v_lshrrev_b32 keeps high rounded BF16 half.
      expect_vop3(274), // v_lshlrev_b32 moves high half into position.
      expect_vop3(276), // v_or_b32 packs low/high halves into vdst.
  };
}

std::vector<ExpectedCdna3Inst> expected_cdna3_cvt_f32_bf16_sequence() {
  return {
      expect_vop3(274), // v_lshlrev_b32 16, src; BF16 bits become FP32 high half.
  };
}

std::vector<ExpectedCdna3Inst> expected_cdna3_cvt_f32_bf16_sdwa_word0_sequence() {
  return {
      expect_vop3(274, 4, rocjitsu::scalar_positive_inline_u32(16), 256 + 4),
  };
}

std::vector<ExpectedCdna3Inst> expected_cdna3_cvt_f32_bf16_sdwa_word1_sequence() {
  return {
      expect_vop3(272, 4, rocjitsu::scalar_positive_inline_u32(16), 256 + 76),
      expect_vop3(274, 4, rocjitsu::scalar_positive_inline_u32(16), 256 + 4),
  };
}
...
expected_cdna3_dot2_f32_bf16_sequence(uint8_t vdst, uint16_t src0, uint16_t src1, uint16_t src2,
  ...
  auto expect_widen = [&](uint8_t dst, uint16_t src, bool high_half, bool negate) {
    if (high_half) {
      expected.push_back(expect_vop3(272, dst, rocjitsu::scalar_positive_inline_u32(16), src));
      expected.push_back(
          expect_vop3(274, dst, rocjitsu::scalar_positive_inline_u32(16), 256 + dst));
    } else {
      expected.push_back(expect_vop3(274, dst, rocjitsu::scalar_positive_inline_u32(16), src));
    }
    if (negate)
      expected.push_back(expect_vop2(21)); // v_xor_b32 literal flips FP32 sign.
  };
  ...
  expected.push_back(expect_vop3(261, a_lo, 256 + a_lo, 256 + b_lo));
  expected.push_back(expect_vop3(261, a_hi, 256 + a_hi, 256 + b_hi));
  expected.push_back(expect_vop3(257, a_lo, 256 + a_lo, 256 + a_hi));
  expected.push_back(expect_vop3(257, vdst, 256 + a_lo, src2));
  if (clamp)
    expected.push_back(expect_vop3(470, vdst, 256 + vdst, 128, 242)); // v_med3_f32 clamps [0,1].
  return expected;
}
...
std::vector<ExpectedCdna3Inst> expected_cdna3_ds_read_b64_tr_b16_sequence(bool acc_dst = false) {
  ...
  if (acc_dst) {
    expected.push_back(expect_vop3p(89)); // v_accvgpr_write_b32 low dword.
    expected.push_back(expect_vop3p(89)); // v_accvgpr_write_b32 high dword.
  }
  return expected;
}
...
std::array<uint32_t, 2> make_cdna4_buffer_load_lds_words(uint8_t op, uint8_t vaddr = 2) {
...
std::vector<Cdna4ToCdna3SemanticRuleCase> cdna4_to_cdna3_semantic_rule_cases() {
  return {
      ...
      {"VBitop3B16FastEc", 0x1A4, 563, make_cdna4_bitop3_words(563, 8, 0xec),
       expected_cdna3_bitop3_fast_sequence(513, true)},
      {"VBitop3B16FastF8", 0x1A4, 563, make_cdna4_bitop3_words(563, 8, 0xf8),
       expected_cdna3_bitop3_fast_sequence(513, true)},
      {"VBitop3B16FastFe", 0x1A4, 563, make_cdna4_bitop3_words(563, 8, 0xfe),
       expected_cdna3_bitop3_fast_sequence(514, true)},
      ...
      {"VCvtF32Bf16E32", 0xFC, 91, make_cdna4_cvt_f32_bf16_words(0xFC),
       expected_cdna3_cvt_f32_bf16_sequence(), 1},
      {"VCvtF32Bf16E32Hi1", 0xFD, 91, make_cdna4_cvt_f32_bf16_words(0xFD),
       expected_cdna3_cvt_f32_bf16_sequence(), 1},
      {"VCvtF32Bf16E32Hi2", 0xFE, 91, make_cdna4_cvt_f32_bf16_words(0xFE),
       expected_cdna3_cvt_f32_bf16_sequence(), 1},
      {"VCvtF32Bf16E32Hi3", 0xFF, 91, make_cdna4_cvt_f32_bf16_words(0xFF),
       expected_cdna3_cvt_f32_bf16_sequence(), 1},
      {"VCvtF32Bf16SdwaWord0", 0xFC, 91, make_cdna4_cvt_f32_bf16_sdwa_word0_words(),
       expected_cdna3_cvt_f32_bf16_sdwa_word0_sequence(), 2},
      {"VCvtF32Bf16SdwaWord1", 0xFC, 91, make_cdna4_cvt_f32_bf16_sdwa_word1_words(),
       expected_cdna3_cvt_f32_bf16_sdwa_word1_sequence(), 2},
      {"VDot2F32Bf16AccumAlias", 0x1A7, 26,
       make_cdna4_dot2_f32_bf16_words(/*vdst=*/0, /*src0=*/256 + 1, /*src1=*/256 + 2,
                                      /*src2=*/256),
       expected_cdna3_dot2_f32_bf16_sequence(/*vdst=*/0, /*src0=*/256 + 1,
                                             /*src1=*/256 + 2, /*src2=*/256,
                                             /*scratch_base=*/3)},
      {"VDot2F32Bf16OpSelNeg", 0x1A7, 26,
       make_cdna4_dot2_f32_bf16_words(/*vdst=*/0, /*src0=*/256 + 1, /*src1=*/256 + 2,
      ...
      {"VDot2F32Bf16OpSelHi2Clamp", 0x1A7, 26,
       make_cdna4_dot2_f32_bf16_words(/*vdst=*/0, /*src0=*/256 + 1, /*src1=*/256 + 2,
      ...
      {"VPermlane32SwapB32E32Hi3", 0xFF, 90, make_cdna4_permlane32_swap_b32_words(0xFF),
       expected_cdna3_permlane32_swap_sequence(), 1},
      {"MfmaF32_16x16x32Bf16", 0x1A7, 53, make_cdna4_mfma_words(53, 0, 256, 260),
       expected_cdna3_mfma_sequence(97)},
      {"MfmaF32_32x32x16Bf16", 0x1A7, 55, make_cdna4_mfma_words(55, 0, 256, 260),
       expected_cdna3_mfma_sequence(96)},
      {"MfmaF32_32x32x16Bf16Source1Acc", 0x1A7, 55, make_cdna4_mfma_words(55, 0, 256, 260, 272, 2),
       expected_cdna3_mfma_sequence(96, 272, 2)},
      ...
      {"DsReadB64TrB16Acc", 0x1B7, 227,
       make_cdna4_ds_read_b64_tr_b16_words(/*byte_offset=*/0, /*addr=*/2, /*acc=*/true),
       expected_cdna3_ds_read_b64_tr_b16_sequence(/*acc_dst=*/true)},
      {"BufferLoadDwordLds", 0x1C0, 20, make_cdna4_buffer_load_lds_words(20),
       expected_cdna3_buffer_load_lds_sequence(20, 13)},
      {"BufferLoadDwordx3Lds", 0x1C0, 22, make_cdna4_buffer_load_lds_words(22),
       expected_cdna3_buffer_load_lds_sequence(22, 222)},
      {"BufferLoadDwordx3LdsEvenScratch", 0x1C0, 22,
       make_cdna4_buffer_load_lds_words(22, /*vaddr=*/0),
       expected_cdna3_buffer_load_lds_sequence(22, 222, /*scratch_data=*/2)},
      {"BufferLoadDwordx4Lds", 0x1C0, 23, make_cdna4_buffer_load_lds_words(23),
       expected_cdna3_buffer_load_lds_sequence(23, 223)},
      {"BufferLoadDwordx4LdsEvenScratch", 0x1C0, 23,
       make_cdna4_buffer_load_lds_words(23, /*vaddr=*/0),
       expected_cdna3_buffer_load_lds_sequence(23, 223, /*scratch_data=*/2)},
  };
}
...
bool contains_smem_load_dwordx2_with_wait(const uint32_t *words, size_t word_count, uint8_t sdata,
  ...
  for (size_t i = 0; i + 2 < word_count; ++i) {
    ...
    if (actual.encoding == 0x30u && actual.op == 1u && actual.sdata == sdata &&
        ...
        words[i + 2] == rocjitsu::pack_sopp(12, 0))
      return true;
  }
  return false;
}
...
TEST(BinaryTranslatorE2E, SkipFailedKernelKeepsIndependentKernelTranslating) {
  ...
  ASSERT_TRUE(result.ok()) << (result.diagnostics.empty() ? ""
                                                          : result.diagnostics.front().message);
  ASSERT_FALSE(result.elf_bytes.empty());
  EXPECT_EQ(result.skipped_kernel_symbols, (std::vector<std::string>{"kernel0"}));
  ...
  EXPECT_EQ(translated_words[0], rocjitsu::build_s_endpgm(ROCJITSU_CODE_ARCH_CDNA3))
      << "the failed kernel body is replaced with a target-ISA no-op stub";
  ...
  EXPECT_EQ(entries, (std::vector<uint64_t>{0, sizeof(uint32_t)}));
}
...
TEST(BinaryTranslatorE2E, OversizedTargetLdsDescriptorEmitsVirtualVariantInsteadOfSkipping) {
  ...
  ASSERT_TRUE(result.ok()) << (result.diagnostics.empty() ? ""
                                                          : result.diagnostics.front().message);
  ASSERT_FALSE(result.elf_bytes.empty());
  EXPECT_TRUE(result.skipped_kernel_symbols.empty());
  ...
  EXPECT_EQ(record.normal_descriptor_vaddr, translated.kernel_descriptor_offset("kernel0"));
  EXPECT_NE(record.virtual_descriptor_vaddr, record.normal_descriptor_vaddr);
  EXPECT_EQ(record.static_lds_bytes, 105600u);
  EXPECT_EQ(record.kernarg_size, 16u);
  EXPECT_EQ(record.backing_pointer_kernarg_offset, 16u);
  ...
  const auto virtual_descriptor_offset =
      rocjitsu::loaded_vaddr_to_file_offset(result.elf_bytes, record.virtual_descriptor_vaddr);
  ...
  EXPECT_EQ(virtual_kd.kernarg_size, 40u);
  EXPECT_EQ(AMDHSA_BITS_GET(virtual_kd.compute_pgm_rsrc2,
                            rocr::llvm::amdhsa::COMPUTE_PGM_RSRC2_GRANULATED_LDS_SIZE),
            0u);
  EXPECT_FALSE(rocjitsu::read_virtual_lds_descriptor_dispatch_metadata(virtual_kd).has_value())
      << "the sidecar descriptor must not self-resolve into the virtual-LDS fallback path";

  const auto dispatch_metadata = rocjitsu::read_virtual_lds_descriptor_dispatch_metadata(normal_kd);
  ...
  EXPECT_NE(dispatch_metadata->flags & rocjitsu::kVirtualLdsFlagRuntimeStateBlock, 0u);
  EXPECT_NE(dispatch_metadata->flags & rocjitsu::kVirtualLdsFlagWorkgroupIdX, 0u);
  EXPECT_NE(dispatch_metadata->flags & rocjitsu::kVirtualLdsFlagWorkgroupIdY, 0u);
}

TEST(BinaryTranslatorE2E, VirtualLdsPreservesInlineKernargLoadsBeyondDescriptorSize) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
  ...
  constexpr uint32_t kExpectedPreserveBytes = 0x90;
  constexpr uint32_t kVirtualLdsRuntimeStateBytes = 24;
  ...
  ASSERT_EQ(parsed->size(), 1u);
  const auto &record = parsed->front();
  EXPECT_EQ(record.kernarg_size, kExpectedPreserveBytes);
  EXPECT_EQ(record.backing_pointer_kernarg_offset, kExpectedPreserveBytes);
  ...
  const auto virtual_kd = rocjitsu::read_elf_struct_for_test<rocjitsu::TestKernelDescriptor>(
      result.elf_bytes, *virtual_descriptor_offset);
  EXPECT_EQ(virtual_kd.kernarg_size, kExpectedPreserveBytes + kVirtualLdsRuntimeStateBytes);

  const auto dispatch_metadata = rocjitsu::read_virtual_lds_descriptor_dispatch_metadata(normal_kd);
  ASSERT_TRUE(dispatch_metadata.has_value());
  EXPECT_EQ(dispatch_metadata->kernarg_size, kExpectedPreserveBytes);
  EXPECT_EQ(dispatch_metadata->backing_pointer_kernarg_offset, kExpectedPreserveBytes);
}

TEST(BinaryTranslatorE2E, VirtualLdsPreservesDirectKernargLoadsInSharedHelperBlocks) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
  constexpr uint16_t kKernargSgpr = 0;
  constexpr uint32_t kDescriptorKernargSize = 0x90;
  constexpr uint32_t kExpectedPreserveBytes = 0xA4;
  constexpr uint32_t kExpectedStateOffset = 0xA8;
  constexpr uint32_t kVirtualLdsRuntimeStateBytes = 24;
  ...
  // not append its state inside a decoded direct kernarg load range from the
  // same ABI kernarg SGPR pair.
  std::vector<uint32_t> words = {
  ...
  const auto &record = parsed->front();
  EXPECT_EQ(record.kernarg_size, kExpectedPreserveBytes);
  EXPECT_EQ(record.backing_pointer_kernarg_offset, kExpectedStateOffset);
  ...
  const auto virtual_kd = rocjitsu::read_elf_struct_for_test<rocjitsu::TestKernelDescriptor>(
      result.elf_bytes, *virtual_descriptor_offset);
  EXPECT_EQ(virtual_kd.kernarg_size, kExpectedStateOffset + kVirtualLdsRuntimeStateBytes);

  const auto dispatch_metadata = rocjitsu::read_virtual_lds_descriptor_dispatch_metadata(normal_kd);
  ASSERT_TRUE(dispatch_metadata.has_value());
  EXPECT_EQ(dispatch_metadata->kernarg_size, kExpectedPreserveBytes);
  EXPECT_EQ(dispatch_metadata->backing_pointer_kernarg_offset, kExpectedStateOffset);
}
...
TEST(BinaryTranslatorE2E, DirectBranchRelocationUsesLongBranchWindowWhenExpandedOutOfRange) {
  ...
  words.push_back(rocjitsu::pack_sopp(5, static_cast<uint16_t>(kTargetWord - 1)));
  for (size_t i = 1; i < kTargetWord; ++i)
    words.push_back(rocjitsu::pack_sopp(4, 0));
  ...
  EXPECT_EQ(target_words[0], rocjitsu::pack_sopp(4, rocjitsu::kMaxDirectBranchTransferWords - 1));
...
TEST(BinaryTranslatorE2E, DirectBranchRelocationUsesIslandsWhenSgprsAreFull) {
  ...
  EXPECT_EQ(target_words[0], rocjitsu::pack_sopp(4, 1));
...
TEST_P(Cdna4ToCdna3SemanticRuleTranslationTest, TranslatesSingleInstruction) {
  ...
  ASSERT_FALSE(translated.text_sections().empty());
  EXPECT_EQ(rocjitsu::find_section(translated, ".rj_translations"), nullptr);
  expect_cdna3_text_matches(*translated.text_sections()[0], test_case.expected);
}
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersDsReadB32ToFlatGlobalLoad) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
  ...
  rocjitsu::BinaryTranslatorOptions options;
  options.force_virtual_lds = true;
  rocjitsu::BinaryTranslator translator(ROCJITSU_CODE_ARCH_CDNA4, ROCJITSU_CODE_ARCH_CDNA3, 0,
                                        options);
  auto result = translator.translate(source);
  ASSERT_FALSE(result.elf_bytes.empty());
  ...
  EXPECT_TRUE(contains_smem_load_dwordx2_with_wait(target_words, target_word_count, *saddr,
                                                   /*sbase_sgpr=*/0, /*offset=*/16));
}

TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersMubufDwordLdsToFlatGlobalStore) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsReservesFreshSgprsBelowCdnaSpecialTail) {
  using namespace rocr::llvm::amdhsa;
  ...
  expect_cdna3_translated_descriptor_sgprs_eq(result.elf_bytes, 64);
}

TEST(BinaryTranslatorE2E, ForceVirtualLdsKeepsDescriptorSpecialTailFree) {
  using namespace rocr::llvm::amdhsa;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsSpillsTouchedHighSgprPairWhenDescriptorSgprsAreFull) {
  using namespace rocr::llvm::amdhsa;
  ...
  const uint16_t saved_lo_src = static_cast<uint16_t>(256u + *saved_lo);
  const uint16_t saved_hi_src = static_cast<uint16_t>(256u + *saved_hi);
  EXPECT_TRUE(contains_smem_load_dwordx2_with_wait(target_words, target_word_count, /*sdata=*/100,
                                                   /*sbase_sgpr=*/0, /*offset=*/16));
  EXPECT_TRUE(contains_cdna3_vop1(target_words, target_word_count, /*op=*/2, /*vdst=*/100,
                                  /*src0=*/saved_lo_src));
  EXPECT_TRUE(contains_cdna3_vop1(target_words, target_word_count, /*op=*/2, /*vdst=*/101,
                                  /*src0=*/saved_hi_src));
}

TEST(BinaryTranslatorE2E, ForceVirtualLdsSpillPerUseGrowsDescriptorForBorrowedHighSgprs) {
  using namespace rocr::llvm::amdhsa;
  ...
  ASSERT_FALSE(result.elf_bytes.empty());
  ASSERT_TRUE(result.ok()) << result.diagnostics.front().message;
  expect_cdna3_translated_descriptor_sgprs_eq(result.elf_bytes, 104);
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsSpillPerUseCanGrowTinyNoAgprKernelsPastAccumOffset) {
  using namespace rocr::llvm::amdhsa;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsRaisesDescriptorForExplicitHighDsVgprs) {
  using namespace rocr::llvm::amdhsa;
  ...
  expect_cdna3_translated_descriptor_vgprs_at_least(result.elf_bytes, 117);
}

TEST(BinaryTranslatorE2E, ForceVirtualLdsSpillPerUseCapturesDispatchPointerBeforeGuestClobber) {
  using namespace rocr::llvm::amdhsa;
  ...
  const auto translated_kd = rocjitsu::read_elf_struct_for_test<rocjitsu::TestKernelDescriptor>(
      result.elf_bytes, translated_rodata->sectionOffset());
  const uint64_t descriptor_vaddr = translated.kernel_descriptor_offset("kernel");
  ASSERT_NE(descriptor_vaddr, 0u);
  const int64_t entry_delta = rocjitsu::read_kernel_descriptor_entry_offset(&translated_kd);
  const uint64_t entry_vaddr =
      static_cast<uint64_t>(static_cast<int64_t>(descriptor_vaddr) + entry_delta);
  const auto entry_file_offset =
      rocjitsu::loaded_vaddr_to_file_offset(result.elf_bytes, entry_vaddr);
  ASSERT_TRUE(entry_file_offset.has_value());

  const auto *entry_words =
      reinterpret_cast<const uint32_t *>(result.elf_bytes.data() + *entry_file_offset);
  constexpr size_t kVirtualLdsEntryScanWords = 48;
  // The descriptor entry must consume the dispatch-packet pointer before the
  // guest body executes `s_mov_b32 s0, 0`. Otherwise spill-per-use virtual LDS
  // reloads would read the backing pointer through a clobbered dispatch pointer.
  EXPECT_TRUE(contains_smem_load_dwordx2_with_wait(
      entry_words, kVirtualLdsEntryScanWords, /*sdata=*/100, /*sbase_sgpr=*/0,
      /*offset=*/offsetof(hsa_kernel_dispatch_packet_t, reserved2)));

  EXPECT_TRUE(contains_flat_scratch_dword_offset(entry_words, kVirtualLdsEntryScanWords, /*op=*/28,
                                                 /*offset=*/0, /*is_load=*/false));
  EXPECT_TRUE(contains_flat_scratch_dword_offset(entry_words, kVirtualLdsEntryScanWords, /*op=*/28,
                                                 /*offset=*/4, /*is_load=*/false));
}

TEST(BinaryTranslatorE2E, ForceVirtualLdsSpillPerUseKeepsEntryPrologueStableAfterVgprGrowth) {
  using namespace rocr::llvm::amdhsa;

  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
  const auto ds = make_cdna4_ds_read_b32_words();
  const auto grow_vgprs = make_cdna4_permlane32_swap_b32_words(/*encoding_id=*/0xFF);
  const uint32_t touch_s100 =
      rocjitsu::build_s_mov_b32(/*sdst=*/100, /*ssrc0=*/128, ROCJITSU_CODE_ARCH_CDNA4);
  const uint32_t touch_s101 =
      rocjitsu::build_s_mov_b32(/*sdst=*/101, /*ssrc0=*/128, ROCJITSU_CODE_ARCH_CDNA4);
  ...
  ASSERT_FALSE(result.elf_bytes.empty());
  ASSERT_TRUE(result.ok()) << result.diagnostics.front().message;
  expect_cdna3_translated_descriptor_vgprs_at_least(result.elf_bytes, 196);
  ...
  const auto saved_lo =
      find_cdna3_vop1_vdst(entry_words, kVirtualLdsEntryScanWords, /*op=*/1, /*src0=*/100);
  const auto saved_hi =
      find_cdna3_vop1_vdst(entry_words, kVirtualLdsEntryScanWords, /*op=*/1, /*src0=*/101);
  ASSERT_TRUE(saved_lo.has_value());
  ASSERT_TRUE(saved_hi.has_value());
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsEmitsRuntimeMetadataSection) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
  const auto ds = make_cdna4_ds_read_b32_words();
  auto image =
      rocjitsu::make_minimal_amdgpu_elf_with_descriptor_after_text({ds[0], ds[1], kCdna4SEndpgm});
  ...
  EXPECT_EQ(record.normal_descriptor_vaddr, translated.kernel_descriptor_offset("kernel"));
  EXPECT_EQ(record.virtual_descriptor_vaddr, translated.kernel_descriptor_offset("kernel"));
  ...
  const auto translated_kd = rocjitsu::read_elf_struct_for_test<rocjitsu::TestKernelDescriptor>(
      result.elf_bytes, translated_rodata->sectionOffset());
  EXPECT_EQ(translated_kd.kernarg_size, 40u);
}

TEST(BinaryTranslatorE2E, ForceVirtualLdsZeroKernargUsesDispatchPacketBackingPointer) {
  using namespace rocr::llvm::amdhsa;
  ...
  ASSERT_TRUE(saddr.has_value());
  EXPECT_TRUE(contains_smem_load_dwordx2_with_wait(
      target_words, target_word_count, *saddr, /*sbase_sgpr=*/0,
      /*offset=*/offsetof(hsa_kernel_dispatch_packet_t, reserved2)));
  ...
  ASSERT_EQ(parsed->size(), 1u);
  const auto &record = parsed->front();
  EXPECT_EQ(record.kernarg_size, 0u);
  EXPECT_EQ(record.backing_pointer_kernarg_offset,
            offsetof(hsa_kernel_dispatch_packet_t, reserved2));
  EXPECT_NE(record.flags & rocjitsu::kVirtualLdsFlagBackingPointerInDispatchPacket, 0u);
  EXPECT_NE(record.flags & rocjitsu::kVirtualLdsFlagRuntimeStateBlock, 0u);
  ...
  const auto translated_kd = rocjitsu::read_elf_struct_for_test<rocjitsu::TestKernelDescriptor>(
      result.elf_bytes, translated_rodata->sectionOffset());
  EXPECT_EQ(translated_kd.kernarg_size, 0u);
}
...
TEST(BinaryTranslatorE2E, LdsKernelEmitsNormalAndVirtualDescriptorVariants) {
  ...
  EXPECT_EQ(virtual_kd.kernarg_size, 40u);
  EXPECT_FALSE(rocjitsu::read_virtual_lds_descriptor_dispatch_metadata(virtual_kd).has_value())
      << "the sidecar descriptor must remain reachable only through fallback selection";
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsMaterializesLargeDsReadB32Offset) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersDsReadU16ToFlatGlobalLoadUshort) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersDsReadU8ToFlatGlobalLoadUbyte) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersDsReadI8ToFlatGlobalLoadSbyte) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersDsWriteB8ToFlatGlobalStoreByte) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersDsWriteB8D16HiToFlatGlobalStoreByteD16Hi) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsStagesDsWriteB96DataOverlappingAddressPair) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsStagesOddDsAddressIntoEvenGlobalAddressPair) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersAccDsWriteB128ToAccFlatGlobalStore) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsStagesOddWideStoreDataIntoEvenTuple) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsStagesOddWideLoadResultIntoEvenTuple) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersDsReadB128ToFlatGlobalLoad) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersAccDsReadB128ToAccFlatGlobalLoad) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
  const auto ds = make_cdna4_ds_read_b128_words(/*addr=*/28, /*vdst=*/30, /*byte_offset=*/0x40,
                                                /*acc=*/true);
  auto image =
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsStagesOddDsReadB128AddressIntoEvenGlobalPair) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsStagesOddDsReadB128ResultIntoEvenTuple) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersDsWrite2B32ToFlatGlobalStores) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersDsWrite2B64ToFlatGlobalStores) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsStagesDsWrite2B64DataOverlappingAddressPair) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersDsWrite2st64B64ToFlatGlobalStores) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersDsRead2B32ToFlatGlobalLoads) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsStagesDsRead2B32ResultsOverlappingAddressPair) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersDsRead2st64B32ToFlatGlobalLoads) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersDsRead2B64ToFlatGlobalLoads) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsStagesOddWideRead2ResultsIntoEvenTuple) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersDsRead2st64B64ToFlatGlobalLoads) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsUsesDistinctSpillSlotsForTwoAddressTemps) {
  using namespace rocr::llvm::amdhsa;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsLowersDsReadB64TrB16LoadToFlatGlobal) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsStagesOddDsReadB64TrB16AddressIntoEvenGlobalPair) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsMaterializesLargeDsReadB64TrB16Offset) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
  constexpr uint16_t kLargeDsOffset = 0x1234;
...
TEST(BinaryTranslatorE2E, ForceVirtualLdsRejectsUnsupportedDsMemoryOpcode) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
...
TEST(BinaryTranslatorE2E, Cdna4ToCdna3PermlaneRejectsDescriptorFullExecSave) {
  ...
  const auto permlane = make_cdna4_permlane32_swap_b32_words(/*encoding_id=*/0xFC);
...
TEST(BinaryTranslatorE2E, Cdna4ToCdna3Permlane16SwapWritesBothRowPairs) {
  constexpr uint32_t kCdna4SEndpgm = 0xBF810000u;
  const auto permlane = make_cdna4_permlane16_swap_b32_words(/*encoding_id=*/0xFC);
  auto image = rocjitsu::make_minimal_amdgpu_elf_with_descriptor_after_text(
      {permlane[0], permlane[1], kCdna4SEndpgm});
...
TEST(BinaryTranslatorE2E, PatchesOneRecoveredBuilderUsedByTwoSetpcConsumers) {
  ...
  EXPECT_EQ(target_words[2], kOriginalGetpcDelta);
  EXPECT_EQ(target_words[3], build_s_addc_u32(kPcSreg + 1, kPcSreg + 1, kInlineInt0));
  EXPECT_EQ(target_words[4], rocjitsu::pack_sopp(5, 6));
  EXPECT_EQ(target_words[5], rocjitsu::build_s_branch(11, ROCJITSU_CODE_ARCH_CDNA3));
  expect_nop_words(target_words, 6, 11, ROCJITSU_CODE_ARCH_CDNA3);
  EXPECT_EQ(target_words[11], rocjitsu::build_s_branch(5, ROCJITSU_CODE_ARCH_CDNA3));
  expect_nop_words(target_words, 12, 17, ROCJITSU_CODE_ARCH_CDNA3);
  EXPECT_EQ(target_words[17], rocjitsu::build_s_endpgm(ROCJITSU_CODE_ARCH_CDNA3));
}
...
TEST(BinaryTranslatorE2E, RewritesDistinctBuildersForOneMultiTargetSetpcConsumer) {
  ...
  const std::vector<uint32_t> words = {
      rocjitsu::pack_sopp(5, 5),                               // 0x00 -> builder B.
  ...
  const auto *target_words =
      reinterpret_cast<const uint32_t *>(translated.text_sections()[0]->data());
  ASSERT_GE(translated.text_sections()[0]->size(), 14 * sizeof(uint32_t));
...
TEST(KernelDescriptorTranslator, CdnaToCdnaVirtualizesOversizedStaticLdsDescriptor) {
  ...
  EXPECT_EQ(translations[0].lds_overflow_kernarg_pointer_offset, 16u);
  EXPECT_EQ(translations[0].target_kernarg_size, 40u);
  ...
  EXPECT_EQ(patched_kd.kernarg_size, 40u);
...
TEST(KernelDescriptorTranslator, VirtualLdsPreservesKernargPreloadRangeWhenSizeIsZero) {
  ...
  EXPECT_EQ(translations[0].lds_overflow_kernarg_pointer_offset, 32u);
  EXPECT_EQ(translations[0].target_kernarg_size, 56u);
}
...
TEST(KernelDescriptorTranslator, VirtualLdsRoundsOddKernargPreloadRangeToPointerBoundary) {
  ...
  EXPECT_EQ(translations[0].lds_overflow_kernarg_pointer_offset, 48u);
  EXPECT_EQ(translations[0].target_kernarg_size, 72u);
}

TEST(KernelDescriptorTranslator, VirtualLdsRejectsZeroKernargSizeWithoutPreloadRange) {
  auto image = rocjitsu::make_minimal_amdgpu_elf_with_descriptor_after_text();
  rocjitsu::enable_kernarg_segment_ptr_sgpr(image, /*kernarg_size=*/0);
  rocjitsu::AmdGpuCodeObject layout(image.data(), image.size());
  ASSERT_TRUE(layout.is_valid());
  ...
  EXPECT_EQ(translations[0].target_kernarg_size, 24u);

  const bool reported_unknown_kernarg_size =
      std::ranges::any_of(translations[0].diagnostics, [](const auto &diagnostic) {
        return diagnostic.message.find("descriptor kernarg size is zero") != std::string::npos;
      });
  EXPECT_TRUE(reported_unknown_kernarg_size);
}

TEST(KernelDescriptorTranslator, VirtualLdsUsesDispatchPacketPointerWhenKernargSizeIsZero) {
  using namespace rocr::llvm::amdhsa;
  ...
  EXPECT_TRUE(translations[0].needs_lds_overflow_buf);
  EXPECT_EQ(translations[0].kernarg_size, 0u);
  EXPECT_EQ(translations[0].target_kernarg_size, 0u);
  EXPECT_TRUE(translations[0].lds_overflow_pointer_in_dispatch_packet);
  EXPECT_EQ(translations[0].lds_overflow_kernarg_pointer_offset,
            offsetof(hsa_kernel_dispatch_packet_t, reserved2));
