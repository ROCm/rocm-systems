...
TEST(CodeObjectPatcher, ReplaceTextRelocatesTextSymbolsWithExactOffsetMap) {
  ...
  const auto *text_bytes = reinterpret_cast<const uint8_t *>(text_words.data());
  constexpr std::array<TextOffsetRelocation, 3> relocations = {
  ...
  EXPECT_EQ(symbols[3].st_value, 0x1108u);
...
TEST(KernelTextLayout, LongRecoveredSpecialCarrierUsesBranchIslandsWithoutClobberingCarrier) {
  ...
  layout.recovered_indirect_fixups.push_back(
      {.source_call_offset = 0,
       .source_target_offset = kSourceTargetOffset,
       .target_window_offset = kWindowOffset,
       .target_window_bytes = rocjitsu::kMaxRecoveredIndirectTransferWords * sizeof(uint32_t),
