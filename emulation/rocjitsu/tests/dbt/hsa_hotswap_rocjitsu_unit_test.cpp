...
// key entries on the translator's build id and locate the tree relative to that
// same shared object, so they agree by construction -- but nothing reports a
// disagreement. A tool writing keys the hook never computes would produce no
