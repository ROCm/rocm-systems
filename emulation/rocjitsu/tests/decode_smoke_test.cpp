...
TEST(CdnaVop3DecodeTest, WritelaneSrc0Encoding255IsRejectedWithoutConsumingTrailingWord) {
  struct Case {
    rj_code_arch_t arch;
    const char *arch_name;
  };
  ...
  // Invalid v_writelane_b32 v141, src0=255, lane 2 followed by a standalone
  // s_nop. CDNA VOP3 has a fixed two-dword encoding: unlike VOP1/VOP2, operand
  // encoding 255 is rejected rather than consuming a literal dword.
