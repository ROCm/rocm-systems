...
/// VOP3 form of the f16 VOPC relational compares on CDNA4. The scalar body
/// widens each f16 src to f32 (util::f16_to_f32) then applies abs/neg in the
/// f32 domain before comparing; the SIMD glue mirrors that order. The compare
/// result merges into an SGPR-pair dst (pointed at VCC=106 here so the same
/// harness checks via wf.vcc()). All four abs/neg combinations are swept; all
/// 16 cdna4-decodable relations are covered (the 17th, 't', is RDNA-only).
