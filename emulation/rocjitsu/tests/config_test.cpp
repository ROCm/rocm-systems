...
TEST(ConfigLoaderTest, ComputeUnitFunctionalQuantumConfig) {
  const char *json = R"({"max_ticks":1000,"num_threads":1,
    "vm":{"arch":"cdna3"},
    "topology":{"root":{"name":"soc","type":"soc","children":[
      {"name":"vram","type":"gpu_memory"},
      {"name":"xcd0","type":"xcd","children":[
        ...
        {"name":"se0","type":"shader_engine","children":[
          {"name":"cu0","type":"compute_unit","config":[
            {"key":"functional_quantum","value":"4096"}
          ]}
        ]}
      ]}
    ]},"links":[
  ...
  auto *cu = loaded.soc()->xcd(0)->shader_engine(0)->compute_unit(0);
  ASSERT_NE(cu, nullptr);
  EXPECT_EQ(cu->config().functional_quantum, 4096u);
  EXPECT_EQ(cu->functional_quantum(), 4096u);
}
