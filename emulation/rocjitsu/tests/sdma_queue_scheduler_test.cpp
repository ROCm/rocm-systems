...
public:
  ...
  AtomicLoadResult atomic_load(VmMemoryDomain, uint64_t address, uint32_t width) override {
    ...
    if (!atomic_load_outcomes_.empty()) {
      const VmAccessOutcome outcome = atomic_load_outcomes_.front();
      atomic_load_outcomes_.pop_front();
      if (outcome != VmAccessOutcome::Complete)
  ...
  void return_next_atomic_load(VmAccessOutcome outcome) {
    std::lock_guard lock(mutex_);
    atomic_load_outcomes_.push_back(outcome);
  }

  uint32_t atomic_load_attempts_at(uint64_t address) const {
    std::lock_guard lock(mutex_);
    const auto attempts = atomic_load_attempts_.find(address);
    return attempts == atomic_load_attempts_.end() ? 0 : attempts->second;
  }
...
private:
  ...
  std::deque<VmAccessOutcome> atomic_load_outcomes_;
  std::unordered_map<uint64_t, uint32_t> atomic_load_attempts_;
...
protected:
  ...
  static constexpr uint64_t kWritePointer = 0x88;
  ...
  SdmaQueueSchedulerTest() : memory_(std::make_shared<EngineMemory>()), scheduler_(gpu_vm_) {
    ...
    memory_->store<uint64_t>(kWritePointer, 0);
  }
...
TEST_F(SdmaQueueSchedulerTest, BindingFactoryRejectsVmPolledQueueWithoutVmPolling) {
  GpuQueueRegistry queues(gpu_vm_);
  QueueRegistrationRequest info = registration_info();
  info.doorbell = {.mode = QueueDoorbellMode::VmPolled, .address = 0x90};
  info.binding_factory = make_sdma_queue_binding_factory(scheduler_);
...
TEST_F(SdmaQueueSchedulerTest, BindingFactoryRejectsMalformedHostPublicationWords) {
  alignas(8) std::array<std::byte, 16> doorbell{};
  GpuQueueRegistry queues(gpu_vm_);
  QueueRegistrationRequest info = registration_info();
  info.doorbell = {.mode = QueueDoorbellMode::HostPolled, .host_base = doorbell.data()};
  info.binding_factory = make_sdma_queue_binding_factory(scheduler_);

  info.ring.producer_pointer_address = kWritePointer + 1;
  EXPECT_FALSE(queues.register_queue(info));

  info.ring.producer_pointer_address = kWritePointer;
  info.doorbell.host_base = doorbell.data() + 1;
  EXPECT_FALSE(queues.register_queue(info));

  info.doorbell.host_base = doorbell.data();
  info.doorbell.offset = 1;
  EXPECT_FALSE(queues.register_queue(info));
  EXPECT_EQ(scheduler_.active_queues(), 0u);
}
...
TEST_F(SdmaQueueSchedulerTest, PollsHostDoorbellWithoutFrontendNotification) {
  ...
  memory_->store<uint64_t>(kWritePointer, sizeof(uint32_t));
...
TEST_F(SdmaQueueSchedulerTest, HostDoorbellSynchronizesWithoutExtendingPublishedRange) {
  memory_->store<uint32_t>(kRing, 0);
  memory_->store<uint32_t>(kRing + sizeof(uint32_t), 0);
  memory_->store<uint64_t>(kWritePointer, 2 * sizeof(uint32_t));
  ...
  std::atomic_ref<uint64_t>(doorbell).store(sizeof(uint32_t), std::memory_order_release);
  ASSERT_TRUE(scheduler_.synchronize_host_queue_for_test(kProcessId, kQueueId));
  EXPECT_EQ(memory_->load<uint64_t>(kReadPointer), sizeof(uint32_t));
  EXPECT_EQ(memory_->atomic_load_attempts_at(kWritePointer), 1u);

  std::atomic_ref<uint64_t>(doorbell).store(2 * sizeof(uint32_t), std::memory_order_release);
  ASSERT_TRUE(scheduler_.synchronize_host_queue_for_test(kProcessId, kQueueId));
  EXPECT_EQ(memory_->load<uint64_t>(kReadPointer), 2 * sizeof(uint32_t));
  EXPECT_EQ(memory_->atomic_load_attempts_at(kWritePointer), 2u);
  EXPECT_TRUE(scheduler_.detach(queue));
}

TEST_F(SdmaQueueSchedulerTest, HostDoorbellPublicationDoesNotTearAcross32BitBoundary) {
  constexpr uint64_t kInitialCursor = (uint64_t{1} << 32) - sizeof(uint32_t);
  constexpr uint32_t kPackets = 128;
  memory_->store<uint64_t>(kReadPointer, kInitialCursor);
  memory_->store<uint64_t>(kWritePointer, kInitialCursor);
  alignas(8) uint64_t doorbell = kInitialCursor;
  SdmaQueueConfig info = queue_info();
  info.ring.initial_cursor = kInitialCursor;
  info.doorbell_base = &doorbell;
  info.last_doorbell = kInitialCursor;
  info.host_accessible = true;
  const SdmaQueueScheduler::Handle queue = scheduler_.attach(info, {});
  ASSERT_TRUE(queue);

  for (uint32_t packet = 0; packet < kPackets; ++packet) {
    const uint64_t begin = kInitialCursor + static_cast<uint64_t>(packet) * sizeof(uint32_t);
    const uint64_t end = begin + sizeof(uint32_t);
    memory_->store<uint32_t>(kRing + begin % kRingBytes, 0);
    memory_->store<uint64_t>(kWritePointer, end);
    std::atomic_ref<uint64_t>(doorbell).store(end, std::memory_order_release);
    ASSERT_TRUE(wait_until([this, end]() { return memory_->load<uint64_t>(kReadPointer) == end; }))
        << "publication stopped at packet " << packet;
  }

  EXPECT_EQ(memory_->load<uint64_t>(kReadPointer),
            kInitialCursor + static_cast<uint64_t>(kPackets) * sizeof(uint32_t));
  EXPECT_TRUE(scheduler_.detach(queue));
}

TEST_F(SdmaQueueSchedulerTest, RetriesUnavailableProducerPointerWithoutAnotherDoorbell) {
  memory_->store<uint32_t>(kRing, 0);
  memory_->store<uint64_t>(kWritePointer, sizeof(uint32_t));
  memory_->return_next_atomic_load(VmAccessOutcome::Unavailable);
  alignas(8) uint64_t doorbell = 0;
  SdmaQueueConfig info = queue_info();
  info.doorbell_base = &doorbell;
  info.host_accessible = true;
  ...
  EXPECT_TRUE(
      wait_until([this]() { return memory_->load<uint64_t>(kReadPointer) == sizeof(uint32_t); }));
  EXPECT_GE(memory_->atomic_load_attempts_at(kWritePointer), 2u);
  EXPECT_TRUE(scheduler_.detach(queue));
}

TEST_F(SdmaQueueSchedulerTest, ExplicitNotificationDoesNotReadProducerPointer) {
  memory_->store<uint32_t>(kRing, 0);
  SdmaQueueConfig info = queue_info();
  info.producer_pointer_address = kWritePointer + 1;
  const SdmaQueueScheduler::Handle queue = scheduler_.attach(info, {});
  ASSERT_TRUE(queue);
  ...
  EXPECT_TRUE(
      wait_until([this]() { return memory_->load<uint64_t>(kReadPointer) == sizeof(uint32_t); }));
  EXPECT_EQ(memory_->atomic_load_attempts_at(kWritePointer + 1), 0u);
  EXPECT_TRUE(scheduler_.detach(queue));
}

TEST_F(SdmaQueueSchedulerTest, RejectsMisalignedHostPublicationWords) {
  alignas(8) std::array<std::byte, 16> doorbell{};
  SdmaQueueConfig info = queue_info();
  info.host_accessible = true;

  info.producer_pointer_address = kWritePointer + 1;
  info.doorbell_base = doorbell.data();
  EXPECT_FALSE(scheduler_.attach(info, {}));

  info.producer_pointer_address = kWritePointer;
  info.doorbell_base = doorbell.data() + 1;
  EXPECT_FALSE(scheduler_.attach(info, {}));

  info.doorbell_base = doorbell.data();
  info.doorbell_offset = 1;
  EXPECT_FALSE(scheduler_.attach(info, {}));
}

TEST_F(SdmaQueueSchedulerTest, DoorbellOnlyWorkSurvivesDisableAndResume) {
  memory_->store<uint32_t>(kRing, 0);
  alignas(8) uint64_t doorbell = 0;
  SdmaQueueConfig info = queue_info();
  info.doorbell_base = &doorbell;
  ...
  const SdmaQueueScheduler::Handle queue = scheduler_.attach(info, {});
  ASSERT_TRUE(queue);
  ASSERT_EQ(scheduler_.update(queue, 0, 0, 0), QueueReconfigureStatus::Disabled);
  ...
  ASSERT_EQ(scheduler_.update(queue, kRing, kRingBytes, 100), QueueReconfigureStatus::Applied);
  EXPECT_TRUE(
      wait_until([this]() { return memory_->load<uint64_t>(kReadPointer) == sizeof(uint32_t); }));
  EXPECT_TRUE(scheduler_.detach(queue));
}
...
TEST_F(SdmaQueueSchedulerTest, ConcurrentDetachRejectsTheSecondCloser) {
  memory_->store<uint32_t>(kRing, 0);
  memory_->block_reads();
  const SdmaQueueScheduler::Handle queue = scheduler_.attach(queue_info(), {});
  ASSERT_TRUE(queue);
  ...
  std::future<bool> first =
      std::async(std::launch::async, [this, queue]() { return scheduler_.detach(queue); });
  const bool closing_observed = wait_until([this, queue]() {
    return scheduler_.notify(queue, sizeof(uint32_t)) == QueueSubmissionStatus::Faulted;
  });
  std::future<bool> second =
      std::async(std::launch::async, [this, queue]() { return scheduler_.detach(queue); });

  const std::future_status first_before_release = first.wait_for(20ms);
  const std::future_status second_before_release = second.wait_for(1s);
  memory_->unblock_reads();
  const std::future_status first_after_release = first.wait_for(1s);
  const std::future_status second_after_release = second.wait_for(1s);

  EXPECT_TRUE(closing_observed);
  EXPECT_EQ(first_before_release, std::future_status::timeout);
  EXPECT_EQ(second_before_release, std::future_status::ready);
  ASSERT_EQ(first_after_release, std::future_status::ready);
  ASSERT_EQ(second_after_release, std::future_status::ready);
  EXPECT_TRUE(first.get());
  EXPECT_FALSE(second.get());

  const SdmaQueueScheduler::Handle first_replacement =
      scheduler_.attach(queue_info(kQueueId + 1), {});
  const SdmaQueueScheduler::Handle second_replacement =
      scheduler_.attach(queue_info(kQueueId + 2), {});
  ASSERT_TRUE(first_replacement);
  ASSERT_TRUE(second_replacement);
  EXPECT_NE(first_replacement, second_replacement);
  EXPECT_EQ(scheduler_.active_queues(), 2u);
  EXPECT_TRUE(scheduler_.detach(first_replacement));
  EXPECT_TRUE(scheduler_.detach(second_replacement));
}

TEST_F(SdmaQueueSchedulerTest, RebindsAfterBlockedPacketDrains) {
...
TEST_F(SdmaQueueSchedulerTest, TerminalStateIsVisibleBeforeObserverNotification) {
...
TEST_F(SdmaQueueSchedulerTest, RejectsInvalidAdmissionWithoutThrowing) {
  ...
  for (void (*mutate)(SdmaRingConfig &) : {
           +[](SdmaRingConfig &ring) { ring.ring_bytes = 0; },
           +[](SdmaRingConfig &ring) { ++ring.ring_base; },
           +[](SdmaRingConfig &ring) { ring.read_pointer_address = 0; },
           +[](SdmaRingConfig &ring) { ++ring.read_pointer_address; },
           +[](SdmaRingConfig &ring) { ring.initial_cursor = 1; },
       }) {
    invalid = queue_info();
    mutate(invalid.ring);
...
TEST_F(SdmaQueueSchedulerTest, RejectsInvalidReconfigurationBeforeApplyingIt) {
  memory_->store<uint32_t>(kRing, 0);
  const SdmaQueueScheduler::Handle queue = scheduler_.attach(queue_info(), {});
  ASSERT_TRUE(queue);

  EXPECT_EQ(scheduler_.update(queue, kRing + 1, kRingBytes, 100), QueueReconfigureStatus::Invalid);
  EXPECT_EQ(scheduler_.notify(queue, sizeof(uint32_t)), QueueSubmissionStatus::Accepted);
  EXPECT_TRUE(
      wait_until([this]() { return memory_->load<uint64_t>(kReadPointer) == sizeof(uint32_t); }));
  EXPECT_TRUE(scheduler_.detach(queue));
...
TEST_F(SdmaQueueSchedulerTest, GracefulDetachClosesAfterPacketCallbackThrows) {
  const std::array<uint32_t, 2> trap = {6, 37};
  memory_->store(kRing, trap);
  SdmaPacketCallbacks callbacks;
  callbacks.deliver_interrupt = [](uint32_t) -> VmAccessOutcome {
    throw std::runtime_error("injected interrupt failure");
  };
  const SdmaQueueScheduler::Handle queue = scheduler_.attach(queue_info(), std::move(callbacks));
  ASSERT_TRUE(queue);

  EXPECT_EQ(scheduler_.notify(queue, sizeof(trap)), QueueSubmissionStatus::Accepted);
  ASSERT_TRUE(wait_until(
      [&]() { return scheduler_.notify(queue, sizeof(trap)) == QueueSubmissionStatus::Faulted; }));
  EXPECT_EQ(scheduler_.prepare_detach(queue), QueuePrepareCloseStatus::Ready);
  EXPECT_FALSE(scheduler_.contains(queue));
}
