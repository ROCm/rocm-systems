...
TEST(L1VectorCacheTest, CoalescedLoadCrossesCacheLineBoundary) {
  GpuMemory memory("memory");
  L2Cache l2("l2");
  l2.set_backing_memory(&memory);
  L1VectorCache l1(&l2);
  ...
  constexpr uint32_t kLanes = 8;
  constexpr uint32_t kElemSize = sizeof(uint32_t);
  constexpr uint64_t kBase = 0x4a0000 + L1VectorCache::LINE_SIZE - 16;
  std::array<uint64_t, 64> addrs{};
  std::array<uint32_t, kLanes> input{};
  for (uint32_t lane = 0; lane < kLanes; ++lane) {
    addrs[lane] = kBase + lane * kElemSize;
    input[lane] = 0xd8000000u | lane;
  }
  ...
  l1.load(addrs.data(), (1ULL << kLanes) - 1, kElemSize, 1,
          reinterpret_cast<uint8_t *>(output.data()), Mtype::RW, false, false,
          /*wf_size=*/64);
