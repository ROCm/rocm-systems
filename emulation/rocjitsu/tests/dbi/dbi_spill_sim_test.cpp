...
protected:
  ...
  void SetUp() override {
    ...
    const uint32_t probe_clobber_exec = build_s_mov_b64_encoding(
        scalar_operand_exec_lo(a_.base.arch), /*inline 0=*/128, a_.base.arch);
...
protected:
  ...
  void SetUp() override {
    ...
    const uint32_t probe_widen = build_s_mov_b64_encoding(
        scalar_operand_exec_lo(a_.base.arch), scalar_inline_neg_one(a_.base.arch), a_.base.arch);
  ...
  void expect_missing_store_full_mask_loses_high_lanes() {
    ...
    const uint32_t toggle = build_s_mov_b64_encoding(
        scalar_operand_exec_lo(a_.base.arch), scalar_inline_neg_one(a_.base.arch), a_.base.arch);
