...
"""Repeat a workload with deterministic Sampled offsets; never join run evidence."""
...
def parse_log(path: Path) -> dict:
    result = {"configurations": [], "reports": [], "verdicts": [], "examples": [],
              "geometry": [], "coverage": []}
    markers = (
        ("ConSan Sampled configuration ", "configurations"),
        ("ConSan MOI auto sampled conflict ", "examples"),
        ("ConSan MOI auto report reader=", "reports"),
        ("ConSan MOI auto report plan ", "geometry"),
        ("ConSan MOI auto report buffer ", "geometry"),
        ("ConSan MOI sampled diagnostic map ", "geometry"),
    ...
    result["reported_conflict_pairs"] = sum(
        int(row.get("sampled_conflicts", "0")) for row in result["reports"])
...
def main(argv=None) -> int:
    ...
    env.update(HSA_TOOLS_LIB=str(args.hook.resolve()), RJ_CONSAN_MODE="sampled",
