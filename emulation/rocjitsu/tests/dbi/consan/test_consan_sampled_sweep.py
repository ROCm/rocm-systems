...
import consan_sampled_sweep as sweep
...
assert "RJ_CONSAN_MOI_RUNTIME_SAMPLE_STRIDE" not in os.environ
assert "RJ_CONSAN_MOI_RUNTIME_SAMPLE_OFFSET" not in os.environ
wg = os.environ["RJ_CONSAN_MOI_WORKGROUP_SAMPLE_OFFSET"]
cell = os.environ["RJ_CONSAN_MOI_CELL_SAMPLE_OFFSET"]
print("ConSan Sampled configuration workgroup_offset=" + wg + " cell_offset=" + cell)
print("ConSan MOI auto report plan outcome=complete sampled_banks=8")
print("ConSan MOI sampled diagnostic map effective_banks_min=4 effective_banks_max=4")
print("ConSan MOI auto report reader=1 visible_sampled=0 sampled_conflicts=0 sampled_saturated_windows=3 sampled_static_mapping_malformed=0")
print("ConSan analysis verdict applicable=true analysis_complete=true static_complete=true dynamic_complete=true")
''')
            output = root / "output"
            with mock.patch.dict(os.environ, {"RJ_CONSAN_MOI_RUNTIME_SAMPLE_STRIDE": "1",
            ...
            for row, (wg, cell) in zip(summary["runs"], [(2, 7), (2, 1), (2, 0), (0, 7)]):
                ...
                self.assertEqual(row["evidence"]["reports"][0]["sampled_saturated_windows"], "3")
