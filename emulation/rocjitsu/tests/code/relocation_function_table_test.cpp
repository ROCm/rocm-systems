...
std::vector<uint8_t>
make_relocation_function_table_elf(std::span<const uint32_t> text_words = {}) {
  ...
  const std::array<uint32_t, 4> default_text = {0xbf800000u, 0xbf800000u, 0xbf800000u,
                                                0xbf800000u};
  if (text_words.empty())
...
TEST(RelocationFunctionTable, PatcherRetargetsRelativeTextAddends) {
  ...
  std::memcpy(shdrs.data(), patched.data() + ehdr.e_shoff,
