...
TEST(Pm4RingConsumerTest, ProcessesTheBoundedSupportedSubsetAcrossRingWrap) {
  ...
  EXPECT_EQ(fixture.memory->load<uint64_t>(0x80), 3u);
...
TEST(Pm4RingConsumerTest, StopsAtThePacketBudgetAndResumesAtTheNextPacket) {
  ...
  EXPECT_EQ(fixture.memory->load<uint64_t>(kReadPointer), 1u);
  ...
  EXPECT_EQ(fixture.memory->load<uint64_t>(kReadPointer), 2u);
}
...
TEST(Pm4RingConsumerTest, ReleasesAnUnreadyGartSnapshotBeforeRetry) {
  auto memory = std::make_shared<FlatMemory>();
  GpuVm vm;
  const AddressSpaceHandle handle = vm.initialize_gart_address_space();
  ASSERT_TRUE(handle);
  constexpr uint64_t kTable = 0x1000;
  constexpr uint64_t kPhysicalPage = 0x2000;
  constexpr uint64_t kAperture = 0x10000;
  constexpr uint64_t kRing = kAperture + 0x100;
  constexpr uint64_t kReadPointer = kAperture + 0x80;
  constexpr uint64_t kValidSystemReadWrite = (uint64_t{1} << 0) | (uint64_t{1} << 1) |
                                             (uint64_t{1} << 5) | (uint64_t{1} << 6) |
                                             (uint64_t{1} << 63);

  Pm4PacketProcessor packet_processor;
  Pm4RingConsumer consumer(packet_processor, vm, handle, kRing, 4 * sizeof(uint32_t), kReadPointer,
                           0);

  const Pm4RingResult blocked = consumer.consume(1);
  EXPECT_EQ(blocked.status, Pm4RingStatus::Blocked);
  EXPECT_FALSE(consumer.in_flight());

  memory->store<uint64_t>(kTable, kPhysicalPage | kValidSystemReadWrite);
  memory->store<uint32_t>(kPhysicalPage + 0x100, 0xffff1000);
  ASSERT_TRUE(vm.publish_gart(
      {.page_table_base = kTable, .aperture_start = kAperture, .aperture_end = kAperture + 0xfff},
      memory));

  const Pm4RingResult complete = consumer.consume(1);
  EXPECT_EQ(complete.status, Pm4RingStatus::Complete);
  EXPECT_EQ(complete.read_pointer, 1u);
  EXPECT_EQ(memory->load<uint64_t>(kPhysicalPage + 0x80), 1u);
}

TEST(Pm4RingConsumerTest, ClearsPublicationFaultAfterASuccessfulRetry) {
  RingConsumerFixture fixture;
  ASSERT_TRUE(fixture.access);
  constexpr uint64_t kRing = 0x100;
  constexpr uint64_t kReadPointer = 0x80;
  ...
  Pm4RingConsumer consumer(packet_processor, fixture.vm, fixture.handle, kRing,
                           4 * sizeof(uint32_t), kReadPointer, 0);
  fixture.memory->make_store_faulted_once(kReadPointer);

  EXPECT_EQ(consumer.consume(1).status, Pm4RingStatus::Faulted);
  EXPECT_EQ(consumer.consume(1).status, Pm4RingStatus::Complete);

  fixture.memory->make_store_unavailable_once(kReadPointer);
  EXPECT_EQ(consumer.consume(2).status, Pm4RingStatus::Blocked);
  EXPECT_EQ(consumer.prepare_close(), QueuePrepareCloseStatus::Busy);
}
