...
TEST(Gfx1250InstructionBuilder, BuildSSetVgprMsb) {
  EXPECT_EQ(build_gfx1250_s_set_vgpr_msb(/*mode=*/0x00, ROCJITSU_CODE_ARCH_CDNA5), 0xBF860000u);
  EXPECT_EQ(build_gfx1250_s_set_vgpr_msb(/*mode=*/0x40, ROCJITSU_CODE_ARCH_CDNA5), 0xBF860040u);
  EXPECT_EQ(build_gfx1250_s_set_vgpr_msb(/*mode=*/0x100, ROCJITSU_CODE_ARCH_CDNA5), 0xBF860100u);
  ...
  EXPECT_EQ(build_gfx1250_s_set_vgpr_msb_transition(
                /*previous_mode=*/0x44, /*new_mode=*/0x08, ROCJITSU_CODE_ARCH_CDNA5),
            0xBF864408u);
...
TEST(Gfx1250InstructionBuilder, BuildSCallI64) {
  const auto word = build_gfx1250_s_call_i64(/*sdst=*/24, /*simm16=*/-3, ROCJITSU_CODE_ARCH_CDNA5);
  ASSERT_TRUE(word);
  ...
  EXPECT_FALSE(build_gfx1250_s_call_i64(/*sdst=*/23, 0, ROCJITSU_CODE_ARCH_CDNA5));
  EXPECT_FALSE(build_gfx1250_s_call_i64(/*sdst=*/24, 0, ROCJITSU_CODE_ARCH_RDNA4));
}

TEST(Gfx1250InstructionBuilder, BuildVCmpNeU16Vcc) {
  const auto word =
      build_gfx1250_v_cmp_ne_u16_vcc(vector_source_vgpr(1), /*vsrc1=*/2, ROCJITSU_CODE_ARCH_CDNA5);
  ASSERT_TRUE(word);
  ...
  EXPECT_FALSE(build_gfx1250_v_cmp_ne_u16_vcc(/*src0=*/512, /*vsrc1=*/2, ROCJITSU_CODE_ARCH_CDNA5));
...
TEST(Gfx1250InstructionBuilder, BuildFixedLaneScalarTransfers) {
  const auto write =
      build_gfx1250_v_writelane_b32(/*vdst=*/40, /*ssrc=*/18, /*lane=*/0, ROCJITSU_CODE_ARCH_CDNA5);
  const auto read =
      build_gfx1250_v_readlane_b32(/*sdst=*/3, /*vsrc=*/40, /*lane=*/1, ROCJITSU_CODE_ARCH_CDNA5);
  ASSERT_TRUE(write);
  ASSERT_TRUE(read);
  ...
  EXPECT_FALSE(build_gfx1250_v_readlane_b32(/*sdst=*/106, /*vsrc=*/40, /*lane=*/0,
                                            ROCJITSU_CODE_ARCH_CDNA5));
  EXPECT_FALSE(
      build_gfx1250_v_readlane_b32(/*sdst=*/3, /*vsrc=*/256, /*lane=*/0, ROCJITSU_CODE_ARCH_CDNA5));
  EXPECT_FALSE(
      build_gfx1250_v_readlane_b32(/*sdst=*/3, /*vsrc=*/40, /*lane=*/64, ROCJITSU_CODE_ARCH_CDNA5));
