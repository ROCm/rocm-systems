...
TEST(ConSan, TargetProfileMatrixDefinesEveryAdmittedArchitecture) {
  struct ExpectedProfile {
    ...
    uint8_t wave32_vgpr_granularity;
    uint8_t wave64_vgpr_granularity;
    uint16_t ordinary_sgpr_limit;
    uint16_t reserved_sgpr_base;
    uint16_t reserved_sgpr_count;
    uint16_t user_sgpr_initialization_limit;
    uint32_t private_limit;
    uint32_t private_granularity;
    uint32_t group_segment_limit;
    ConSanDirectCallForm direct_call_form;
    bool kernarg_preload_overflow_recovery;
    bool cluster;
    bool selectable_vgpr_bank;
    bool even_vgpr_tuples;
    uint16_t semantic_form_mask;
  };
  constexpr std::array expected = {
      ExpectedProfile{ROCJITSU_CODE_TARGET_GFX942,
                      ROCJITSU_CODE_ARCH_CDNA3,
                      ConSanArchitectureFamily::Cdna,
                      ConSanEncodingFamily::Gfx9Cdna3,
                      ConSanAccumulatorModel::DescriptorPartitioned,
                      ConSanDispatchIdentitySource::PreloadedSgprPair,
                      ConSanWorkgroupIdentitySource::DescriptorSystemSgprs,
                      ConSanWaitCounterFamily::Gfx9,
                      0u,
                      8u,
                      102u,
                      0u,
                      ...
                      16u,
                      64u * 1024u,
                      ConSanDirectCallForm::SCallB64,
                      true,
                      false,
                      false,
                      true,
                      kConSanCdnaSemanticFormMask},
      ExpectedProfile{ROCJITSU_CODE_TARGET_GFX950,
                      ROCJITSU_CODE_ARCH_CDNA4,
                      ConSanArchitectureFamily::Cdna,
                      ConSanEncodingFamily::Gfx9Cdna4,
                      ConSanAccumulatorModel::DescriptorPartitioned,
                      ConSanDispatchIdentitySource::PreloadedSgprPair,
                      ConSanWorkgroupIdentitySource::DescriptorSystemSgprs,
                      ConSanWaitCounterFamily::Gfx9,
                      ...
                      true,
                      kConSanCdnaSemanticFormMask},
      ExpectedProfile{ROCJITSU_CODE_TARGET_GFX1100,
                      ROCJITSU_CODE_ARCH_RDNA3,
                      ConSanArchitectureFamily::Rdna,
                      ConSanEncodingFamily::Gfx11,
                      ConSanAccumulatorModel::None,
                      ConSanDispatchIdentitySource::CodeObjectLiteral,
                      ConSanWorkgroupIdentitySource::DescriptorSystemSgprs,
                      ConSanWaitCounterFamily::Gfx11,
                      8u,
                      4u,
                      106u,
                      0u,
      ...
      ExpectedProfile{ROCJITSU_CODE_TARGET_GFX1201,
                      ROCJITSU_CODE_ARCH_RDNA4,
                      ConSanArchitectureFamily::Rdna,
                      ConSanEncodingFamily::Gfx12,
                      ConSanAccumulatorModel::None,
                      ConSanDispatchIdentitySource::CodeObjectLiteral,
                      ConSanWorkgroupIdentitySource::CommandProcessorTtmps,
                      ConSanWaitCounterFamily::Gfx12,
                      8u,
                      4u,
                      106u,
                      0u,
                      ...
                      false,
                      kConSanCommonSemanticFormMask},
      ExpectedProfile{ROCJITSU_CODE_TARGET_GFX1250,
                      ROCJITSU_CODE_ARCH_CDNA5,
                      ConSanArchitectureFamily::Cdna,
                      ConSanEncodingFamily::Gfx12,
                      ConSanAccumulatorModel::SelectableVgprBank,
                      ConSanDispatchIdentitySource::CodeObjectLiteral,
                      ConSanWorkgroupIdentitySource::CommandProcessorTtmps,
                      ConSanWaitCounterFamily::Gfx12,
                      16u,
  ...
  ASSERT_EQ(kConSanTargetProfiles.size(), expected.size());
  for (size_t index = 0; index < expected.size(); ++index) {
    const ExpectedProfile &want = expected[index];
    const ConSanTargetProfile &profile = kConSanTargetProfiles[index];
    SCOPED_TRACE(rj_code_target_name(want.target));
    EXPECT_EQ(profile.target, want.target);
    EXPECT_EQ(profile.arch, want.arch);
    EXPECT_EQ(consan_target_profile(want.target), &profile);
    EXPECT_EQ(consan_target_profile(want.arch), &profile);
    EXPECT_EQ(profile.architecture_family, want.architecture_family);
    EXPECT_EQ(profile.encoding_family, want.encoding_family);
    EXPECT_EQ(profile.accumulator_model, want.accumulator_model);
    EXPECT_EQ(profile.dispatch_identity, want.dispatch_identity);
    EXPECT_EQ(profile.workgroup_identity, want.workgroup_identity);
    EXPECT_EQ(profile.wait_counter_family, want.wait_counter_family);
    EXPECT_EQ(consan_profile_vgpr_allocation_granularity(profile, 32u),
              want.wave32_vgpr_granularity);
    EXPECT_EQ(consan_profile_vgpr_allocation_granularity(profile, 64u),
              want.wave64_vgpr_granularity);
    EXPECT_EQ(profile.exec_register_width_bits, 64u);
    EXPECT_EQ(profile.global_address_width_bits, 64u);
    EXPECT_EQ(profile.lds_address_width_bits, 32u);
    EXPECT_EQ(profile.sgpr_allocation_granularity, 8u);
    EXPECT_EQ(profile.ordinary_sgpr_limit, want.ordinary_sgpr_limit);
    EXPECT_EQ(profile.reserved_ordinary_sgpr_base, want.reserved_sgpr_base);
    EXPECT_EQ(profile.reserved_ordinary_sgpr_count, want.reserved_sgpr_count);
    EXPECT_EQ(profile.user_sgpr_initialization_limit, want.user_sgpr_initialization_limit);
    EXPECT_EQ(profile.address_free_private_limit_bytes, want.private_limit);
    EXPECT_EQ(profile.private_allocation_granularity_bytes, want.private_granularity);
    EXPECT_EQ(profile.max_group_segment_bytes, want.group_segment_limit);
    EXPECT_EQ(profile.direct_branch_min_displacement_bytes, -131068);
    EXPECT_EQ(profile.direct_branch_max_displacement_bytes, 131072);
    EXPECT_EQ(profile.direct_call_form, want.direct_call_form);
    EXPECT_EQ(profile.supports_kernarg_preload_overflow_recovery,
              want.kernarg_preload_overflow_recovery);
    EXPECT_EQ(profile.has_cluster_facilities, want.cluster);
    EXPECT_EQ(profile.has_selectable_vgpr_bank, want.selectable_vgpr_bank);
    EXPECT_EQ(profile.requires_even_vgpr_tuples, want.even_vgpr_tuples);
    EXPECT_EQ(profile.semantic_form_mask, want.semantic_form_mask);

    const auto wave64 = consan_kernel_target_profile(profile, 64u);
    ASSERT_TRUE(wave64);
    EXPECT_EQ(wave64->target, &profile);
    EXPECT_EQ(wave64->wave_size, 64u);
    EXPECT_EQ(wave64->active_exec_mask_width_bits, 64u);
    EXPECT_EQ(wave64->vgpr_allocation_granularity, want.wave64_vgpr_granularity);
    const auto wave32 = consan_kernel_target_profile(profile, 32u);
    EXPECT_EQ(wave32.has_value(), want.wave32_vgpr_granularity != 0u);
    if (wave32) {
      EXPECT_EQ(wave32->target, &profile);
      EXPECT_EQ(wave32->active_exec_mask_width_bits, 32u);
      EXPECT_EQ(wave32->vgpr_allocation_granularity, want.wave32_vgpr_granularity);
    }
    EXPECT_FALSE(consan_kernel_target_profile(profile, 16u));

    ASSERT_EQ(consan_address_free_private_limit(profile.arch), want.private_limit);
    EXPECT_EQ(address_free_scratch_private_limit(profile.arch), want.private_limit);
    EXPECT_EQ(consan_normalize_address_free_private_size(profile.arch, 1u),
              want.private_granularity);
    EXPECT_EQ(normalize_address_free_scratch_private_size(profile.arch, 1u),
              want.private_granularity);
    EXPECT_FALSE(consan_normalize_address_free_private_size(profile.arch, want.private_limit + 1u));
    EXPECT_EQ(instrumentation::build_s_call_i64(0u, 0, profile.arch).has_value(),
              want.direct_call_form == ConSanDirectCallForm::SCallI64);
    for (ConSanCapabilityForm form : kConSanCapabilityForms) {
      EXPECT_EQ(consan_arch_supports_capability_form(profile.arch, form),
                (want.semantic_form_mask & consan_capability_form_bit(form)) != 0u);
    }
  }

  EXPECT_EQ(consan_target_profile(ROCJITSU_CODE_TARGET_GFX90A), nullptr);
  EXPECT_EQ(consan_target_profile(ROCJITSU_CODE_ARCH_CDNA2), nullptr);

  constexpr uint64_t branch_pc = 200000u;
  EXPECT_TRUE(compute_sopp_branch_simm16(branch_pc, branch_pc - 131068u));
  EXPECT_FALSE(compute_sopp_branch_simm16(branch_pc, branch_pc - 131072u));
  EXPECT_TRUE(compute_sopp_branch_simm16(branch_pc, branch_pc + 131072u));
  EXPECT_FALSE(compute_sopp_branch_simm16(branch_pc, branch_pc + 131076u));
}
