...
TEST(ConSan, InventoriesGfx1250TensorDmaAsRuntimeDescriptorRanges) {
  ...
  for (const Mode mode : {Mode::Default, Mode::SuperCollider}) {
    ...
    for (const auto &site : result.program_inventory.access_sites()) {
      ...
      EXPECT_EQ(site.lowering.replay_guest_access.available(), site.kind == LdsAccessKind::Write);
      EXPECT_FALSE(site.lowering.compare_observed_value.available());
    }
    EXPECT_EQ(test_admitted_accesses(result).size(), mode == Mode::Default ? 1u : 0u);
    EXPECT_EQ(access_decision_count(result, SiteDecisionKind::Unsupported),
              mode == Mode::Default ? 1u : 2u);
