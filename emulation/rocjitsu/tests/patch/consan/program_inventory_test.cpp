...
TEST(ConSanProgramInventory, ImmutableViewsRetainFactsAcrossCopyMoveAndBuilderLifetime) {
  ...
  EXPECT_EQ(
      moved.program_inventory.container(moved.program_inventory.access_sites().front().container),
      &moved.program_inventory.kernels().front());
  EXPECT_EQ(moved.program_inventory.access_sites().front().container,
...
TEST(ConSanProgramInventory, ContainerQueriesUseImmutableInventoryIdentity) {
  ...
  builder.publish_decoded_accesses(bytes);
  EXPECT_FALSE(builder.view().access_sites().back().container.valid());
}
...
TEST(ConSanProgramInventory, SequenceOwnersAreDerivedFromEveryMemberSource) {
  ...
  cross_container.program_sites()[1].container = cross_container.kernels()[1].id;
...
TEST(ConSanProgramInventory, MutableRevisionIsDeepCopiedFromPublishedInventory) {
  ...
  EXPECT_EQ(published.container(published.access_sites().front().container)->name, "original");
  ...
  EXPECT_EQ(published.access_sites().size(), 1u);
...
TEST(ConSanProgramInventory, SemanticRangeDeduplicationPreservesEveryAccessRecord) {
  ProgramInventoryBuilder builder;
...
TEST(ConSanProgramInventory, FlatHintsBecomeTypedAddressSpaceProvenanceAndConfidence) {
  ...
  builder.access_sites().front().operands.raw_op = 1;
  builder.access_sites().front().operands.raw_saddr = 2;
  builder.access_sites().front().operands.raw_scale_offset = true;
  builder.access_sites().front().operands.raw_vaddr = 3;
  builder.access_sites().front().operands.raw_vsrc = 4;
  builder.access_sites().front().operands.raw_vdst = 5;
