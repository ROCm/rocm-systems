...
TEST(ConSan, UniformAddressBroadcastCopiesAndClobbers) {
  for (auto arch : {ROCJITSU_CODE_ARCH_CDNA3, ROCJITSU_CODE_ARCH_CDNA4, ROCJITSU_CODE_ARCH_RDNA3,
    ...
    EXPECT_TRUE(tracker.contains(3));
    UniformAddressTracker join;
    EXPECT_FALSE(join.contains(3)); // facts are never imported into a CFG join
  }
}

TEST(ConSan, UniformAddressRejectsLaneModifiers) {
...
TEST(ConSan, UniformAddressForgetsRelativeRegisterWrites) {
...
TEST(ConSan, UniformAddressDeclinesUnmodeledRegisterModeWrites) {
...
TEST(ConSan, UniformAddressFactsDoNotCrossCfgJoins) {
  constexpr auto arch = ROCJITSU_CODE_ARCH_RDNA4;
...
TEST(ConSan, UniformStoreForgetsClobbersAndExecChanges) {
  constexpr auto arch = ROCJITSU_CODE_ARCH_RDNA4;
...
TEST(ConSan, UniformStoreGfx1250AllowsUnrelatedModeButRejectsBankWrites) {
  ...
  for (const auto &[mode, expected] :
    ...
    const auto result =
        test_lower_consan(make_gfx1250_code_object(words, "uniform_value"), test_options());
    ASSERT_TRUE(patch_succeeded(result)) << testing::PrintToString(result.errors);
    ASSERT_EQ(result.program_inventory.access_sites().size(), 1u);
...
TEST(ConSan, OwnsOneNormalizedScopeToReportAbiMapping) {
  EXPECT_EQ(sync_scope(MemoryScope::Wavefront), SyncScope::Wavefront);
  EXPECT_EQ(sync_scope(MemoryScope::Workgroup), SyncScope::Workgroup);
  EXPECT_EQ(sync_scope(MemoryScope::Agent), SyncScope::Agent);
  EXPECT_EQ(sync_scope(MemoryScope::System), SyncScope::System);
  EXPECT_FALSE(sync_scope(static_cast<MemoryScope>(4u)));
}

TEST(ConSan, AtomicSemanticReasonsAreTypedAndRenderStableTokens) {
  using Reason = detail::AtomicSemanticsReason;
  const std::array expected = {
      ...
      std::pair{Reason::SyncAbiRejectedQualifiedSequence,
                std::string_view{"sampled-sync-abi-rejected-qualified-sequence"}},
      std::pair{Reason::SyncAbiRejectedCasFailure,
  ...
  for (size_t index = 0; index < expected.size(); ++index) {
    ...
    EXPECT_EQ(detail::atomic_semantics_reason_name(expected[index].first), expected[index].second);
  }
  EXPECT_EQ(detail::atomic_semantics_reason_name(Reason::Count),
            "invalid-sampled-atomic-semantics-reason");
  EXPECT_EQ(detail::atomic_semantics_reason_name(static_cast<Reason>(255)),
            "invalid-sampled-atomic-semantics-reason");
}
...
constexpr std::array<Target, 2> kCdnaTargets = {{
...
std::vector<uint8_t> make_cdna4_full_pressure_access(std::span<const uint32_t> guest,
...
void expect_barrier_exact_existing_metadata_path(const TransformArtifacts &result,
                                                 const PatchInfo &patch, rj_code_arch_t arch,
                                                 SyncScope scope = SyncScope::Workgroup) {
  ...
  ASSERT_EQ(encoded.classification, SyncClassification::Valid);
  ...
  for (uint32_t offset :
       {offsetof(SyncMetadataPacked, address),
        offsetof(SyncMetadataPacked, address) + sizeof(uint32_t),
        offsetof(SyncMetadataPacked, byte_count), offsetof(SyncMetadataPacked, epoch_before),
...
TEST(ConSan, ModeInventoriesCodeObjectWithoutModification) {
  ...
  EXPECT_EQ(result.program_inventory.kernels().front().preflight_action, PreflightAction::NotRun);
  bool saw_no_instrumentation_warning = false;
  for (const std::string &warning : result.warnings)
    saw_no_instrumentation_warning |=
        warning.find("emitted no instrumentation") != std::string::npos;
  EXPECT_TRUE(saw_no_instrumentation_warning);
}

TEST(ConSan, DirectProbeWritesPackedWatchpointEntry) {
  ...
  options.set_owner_epoch_vgprs(16, 17);
  options.report_buffer_address = 0x123456780000ull;
  options.report_generation = 7;
  options.report_dispatch_id = 0x1122334455667788ull;
  ...
  EXPECT_EQ(runtime_mapping.access_kind, LdsAccessKind::Write);
  ...
  for (const std::string &warning : result.warnings) {
    saw_warning |= warning.find("direct sampled watchpoint") != std::string::npos;
  ...
  const auto save_guest_exec =
      build_s_mov_b64(*test_exec_save_sgpr(result), kAmdGpuExecLo, ROCJITSU_CODE_ARCH_RDNA4);
  const auto restore_guest_exec =
      build_s_mov_b64(kAmdGpuExecLo, *test_exec_save_sgpr(result), ROCJITSU_CODE_ARCH_RDNA4);
  ...
  const uint64_t causal_window = *options.report_buffer_address + sizeof(ReportHeader);
  ...
  EXPECT_EQ(
      count_subsequence(rewritten_words,
                        make_expected_literal_offset_store_words(
                            offsetof(CausalWindow, publication_state),
                            static_cast<uint32_t>(CausalPublicationState::Publishing), 8, 12)),
      0u);
  EXPECT_EQ(count_subsequence(rewritten_words,
                              make_expected_literal_offset_store_words(
                                  offsetof(CausalWindow, publication_state),
                                  static_cast<uint32_t>(CausalPublicationState::Ready), 8, 12)),
            1u);
  EXPECT_EQ(count_subsequence(rewritten_words, make_expected_literal_offset_store_words(
                                                   offsetof(CausalWindow, generation), 7, 8, 12)),
            1u);
  ...
  EXPECT_EQ(count_subsequence(rewritten_words, make_expected_literal_offset_store_words(
                                                   offsetof(CausalWindow, first_entry), 0u, 8, 12)),
            1u);
  ASSERT_TRUE(test_dispatch_id_sgpr(result));
  const uint16_t dispatch_sgpr = *test_dispatch_id_sgpr(result);
  ...
  expect_dispatch_store(offsetof(CausalWindow, dispatch_id), dispatch_sgpr);
  expect_dispatch_store(offsetof(CausalWindow, dispatch_id) + sizeof(uint32_t),
                        static_cast<uint16_t>(dispatch_sgpr + 1u));
  EXPECT_EQ(count_subsequence(rewritten_words,
                              make_expected_literal_offset_store_words(
                                  offsetof(CausalWindow, dispatch_id), 0x55667788u, 8, 12)),
            0u);
  ...
  ASSERT_TRUE(test_exec_save_sgpr(result));
  const uint16_t state_base = *test_exec_save_sgpr(result);
...
TEST(ConSan, Gfx1250DirectProbePassesFinalValidation) {
  ...
  options.set_owner_epoch_vgprs(90, 91);
  options.report_buffer_address = 0x123456780000ull;
  options.report_generation = 7;
  options.report_dispatch_id = 0x1122334455667788ull;
...
TEST(ConSan, Gfx1250DenseAccessesRelocateAcrossLargeKernel) {
  ...
  const std::vector<uint8_t> bytes =
      make_gfx1250_code_object(text_words, "gfx1250_partitioned_dense");
  ...
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(2u * kAccessesPerWindow);
  options.runtime_sample_stride = 1u;
  ...
  for (const PatchInfo &patch : result.patches) {
    if (patch.kind != PatchKind::TrampolineWatchpointStore)
      continue;
...
TEST(ConSan, Gfx1250DensePrivateStateUsesSpillSafeBodyGate) {
  ...
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(kAccessCount);
  ...
  const TransformArtifacts result = test_lower_consan(
      make_gfx1250_code_object(text_words, "gfx1250_dense_fast_gate_fallback"), options);
...
TEST(ConSan, Gfx1250DenseFastGateIncludesClusterWorkgroupId) {
  constexpr uint32_t kAccessCount = 9u;
  constexpr uint16_t kClusterWorkgroupTtmp = 6u;
  std::vector<uint32_t> text_words(
      8u, build_s_mov_b32(/*sdst=*/0, /*ssrc0=*/0, ROCJITSU_CODE_ARCH_CDNA5));
  ...
  const TransformArtifacts result = test_lower_consan(
      make_gfx1250_code_object(text_words, "gfx1250_dense_cluster_gate"), options);
  ...
  const auto prologue =
      std::ranges::find(result.patches, PatchKind::KernelEntryOwnerEpochPrologue, &PatchInfo::kind);
  ASSERT_NE(prologue, result.patches.end());
  ASSERT_TRUE(prologue->vgpr_state);
  ASSERT_TRUE(prologue->vgpr_state->exact_workgroup.cluster_workgroup_id());
  const uint16_t persistent_cluster = *prologue->vgpr_state->exact_workgroup.cluster_workgroup_id();
...
TEST(ConSan, Gfx1250DenseGateReservesCodeObjectWideClusterTuple) {
  constexpr uint32_t kAccessCount = 9u;
  std::vector<uint32_t> text_words(
      8u, build_s_mov_b32(/*sdst=*/0, /*ssrc0=*/0, ROCJITSU_CODE_ARCH_CDNA5));
  for (uint32_t index = 0; index < kAccessCount; ++index) {
  ...
  options.exact_workgroup_vgprs = PersistentWorkgroupRegisters{42u, 43u, 44u, 45u};
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(kAccessCount);
  ...
  const TransformArtifacts result = test_lower_consan(
      make_gfx1250_code_object(text_words, "gfx1250_dense_global_cluster_tuple"), options);
...
TEST(ConSan, Gfx1250DenseAccessesPreserveGuestVgprMsbMode) {
  constexpr uint32_t kAccessCount = 9u;
  ...
  const TransformArtifacts result =
      test_lower_consan(make_gfx1250_code_object(text_words, "gfx1250_dense_vgpr_msb"), options);
  ...
  for (const PatchInfo &patch : result.patches) {
    ...
    // embedded-guest pairs. The ConSan emitter must not duplicate either
...
TEST(ConSan, Gfx1250ScalarSpillRestoresInInstrumentationBank) {
  ...
  std::vector<uint8_t> bytes = make_gfx1250_code_object(
      text_words, "gfx1250_scalar_spill_bank_owner", kRdna4Wave64AllVgprsGranulated,
      /*wave32=*/true);
  mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
  ...
  ASSERT_TRUE(patch_succeeded(result)) << testing::PrintToString(result.errors);
  ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
  ASSERT_EQ(test_transient_sgpr_assignments(result).size(), 1u);
  EXPECT_TRUE(test_transient_sgpr_assignments(result).front().spill_backed);
  const auto patch = std::ranges::find_if(result.patches, [&](const PatchInfo &item) {
    return item.kind == PatchKind::TrampolineWatchpointStore && item.anchor_offset == second_anchor;
  });
...
TEST(ConSan, Gfx1250CapturesHighBankLdsAddressBeforeSelectingScratchBank) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA5;
  ...
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(1u);
  options.runtime_sample_stride = 1u;
  options.track_barriers = false;
  options.track_atomics = false;
  ...
  const TransformArtifacts result =
      test_lower_consan(make_gfx1250_code_object(text_words, "gfx1250_high_bank_address"), options);
  ...
  const uint16_t captured_address_vgpr =
      static_cast<uint16_t>(kScratchVgpr + (options.device_conflict_check ? 7u : 5u));
...
TEST(ConSan, Cdna4DirectProbeEmitsNativePublicationRecipes) {
  ...
  TestOptions options = test_options();
  options.scratch_vgpr = 8;
  options.set_owner_epoch_vgprs(14, 15);
  options.report_buffer_address = 0x123456780000ull;
  options.report_generation = 7;
  options.report_dispatch_id = 0x1122334455667788ull;
  options.report_buffer_size = direct_report_bytes(2);
  ...
  for (const bool high_word : {false, true}) {
    ...
    const uint16_t source_sgpr =
        static_cast<uint16_t>(*test_dispatch_id_sgpr(result) + (high_word ? 1u : 0u));
...
TEST(ConSan, DirectProbeAddsNativeLdsImmediateOffsets) {
  ...
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(3);
  ...
  const auto zero_shift = build_v_lshlrev_b32_e32(
      high_vgpr, scalar_positive_inline_u32(watchpoint::start_byte_shift - 32u),
  ...
  for (uint32_t byte_offset : std::array<uint32_t, 2>{4u, 8u}) {
    ...
    const auto shift = build_v_lshlrev_b32_e32(
        high_vgpr, scalar_positive_inline_u32(watchpoint::start_byte_shift - 32u), tmp_vgpr,
        ROCJITSU_CODE_ARCH_RDNA4);
...
TEST(ConSan, DirectProbeSnapshotsOverlappingLoadAddress) {
  std::array<uint32_t, 420> text_words{};
  ...
  const auto start_byte = build_v_lshlrev_b32_e32(
      /*vdst=*/23, scalar_positive_inline_u32(watchpoint::start_byte_shift - 32u),
...
TEST(ConSan, DirectProbePublishesMultipleLdsAccessRanges) {
  ...
  EXPECT_EQ(
      committed_semantic_sites(result, result.coverage_ledger.lowering_commits().front()).size(),
      2u);
  EXPECT_EQ(result.coverage_ledger.lowering_commits().front().locations.size(), 1u);
  AmdGpuCodeObject patched(result.replacement.data(), result.replacement.size());
  ASSERT_TRUE(patched.is_valid());
...
TEST(ConSan, DirectProbeRequiresCapacityForEveryLdsAccessRange) {
  std::vector<uint32_t> text_words(420, build_s_nop(0, ROCJITSU_CODE_ARCH_RDNA4));
  text_words[0] = 0xD8380201u;
  text_words[1] = 0x00000000u; // ds_store_2addr_b32 v0, v0, v0 offset0:1 offset1:2
  text_words.back() = build_s_endpgm(ROCJITSU_CODE_ARCH_RDNA4);
  ...
  EXPECT_TRUE(patch_succeeded(result));
  EXPECT_FALSE(result.modified());
...
TEST(ConSan, DirectSplitsLargeGfx1250TwoAddressGuestAccess) {
  constexpr auto store =
  ...
  const std::vector<uint8_t> bytes = make_gfx1250_code_object(text_words, "large_two_address");
  TestOptions options = test_options();
  options.scratch_vgpr = 20;
  options.exec_save_sgpr = 60;
  options.set_owner_epoch_vgprs(50, 51);
  options.max_patches = 2;
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(2);
  options.track_barriers = false;
  ...
  ASSERT_TRUE(patch_succeeded(result)) << testing::PrintToString(result.errors);
  ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
  const auto access = std::ranges::find_if(result.patches, [](const PatchInfo &patch) {
    return patch.kind == PatchKind::TrampolineWatchpointStore;
  });
  ASSERT_NE(access, result.patches.end());
  ASSERT_TRUE(access->scratch_vgpr);
  const auto plan = std::ranges::find_if(result.resource_plans, [](const auto &resource_plan) {
    return resource_plan.site_kind == ResourceSiteKind::Access;
  });
...
TEST(ConSan, AtomicTrackingPublishesQualifiedTypedMetadata) {
  const std::vector<uint8_t> bytes = make_rdna4_lds_and_ordered_flat_atomic_code_object();
  ASSERT_FALSE(bytes.empty());
  TestOptions options = test_options();
  options.track_atomics = true;
  options.scratch_vgpr = 8;
  options.set_owner_epoch_vgprs(20, 21);
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(2);
  options.max_patches = 2;
  ...
  EXPECT_EQ(inventory.range_bank_count, 8u);
  EXPECT_EQ(inventory.sync_slot_count, 9u);
  EXPECT_EQ(inventory.watchpoint_count, 9u);
  const AutoReportPlan auto_plan = plan_auto_report(inventory);
  ASSERT_TRUE(auto_plan.complete());
  EXPECT_EQ(auto_plan.layout.watchpoint_capacity, 9u);
  ...
  const CommittedLowering *atomic_commit =
      committed_lowering_for_intent_kind(result, ProbeIntentKind::AtomicOrdering);
  ASSERT_NE(atomic_commit, nullptr);
  EXPECT_TRUE(committed_lowering_has_intent_kind(result, *atomic_commit,
  ...
  const ReportBufferLayout layout =
      direct_report_buffer_layout_for_bytes(options.report_buffer_size);
  const uint64_t metadata = *options.report_buffer_address + layout.sync_metadata_offset;
  const auto encoded = encode_sync_metadata({
      .address = 1,
      .byte_count = 4,
  ...
  ASSERT_TRUE(detail::append_indexed_address(
      metadata_address,
  ...
  EXPECT_EQ(count_subsequence(trampoline, make_expected_literal_offset_store_words(
                                              offsetof(SyncMetadataPacked, byte_count), 4u,
                                              /*address_vgpr=*/8u, /*temporary_vgpr=*/10u)),
            1u);
  EXPECT_TRUE(contains_subsequence(
      trampoline, make_expected_offset_store_words(offsetof(SyncMetadataPacked, address),
                                                   /*value_vgpr=*/17u,
                                                   /*address_vgpr=*/8u)));
  EXPECT_TRUE(contains_subsequence(
      trampoline,
      make_expected_offset_store_words(offsetof(SyncMetadataPacked, address) + sizeof(uint32_t),
                                       /*value_vgpr=*/18u, /*address_vgpr=*/8u)));
  ASSERT_TRUE(test_exec_save_sgpr(result));
  const uint16_t state = *test_exec_save_sgpr(result);
  const auto save_vcc =
...
TEST(ConSan, PatchesAliasedAtomicOnceForEveryOwner) {
...
TEST(ConSan, Gfx1250OrderedLdsAtomicComposesAccessAndOrderingMetadata) {
  ...
  TransformArtifacts nested_inventory = result;
  auto nested_atomic = std::ranges::find(nested_inventory.patches,
                                         PatchKind::TrampolineSyncMetadata, &PatchInfo::kind);
  ASSERT_NE(nested_atomic, nested_inventory.patches.end());
  nested_atomic->trampoline_offset += 88u;
  nested_atomic->trampoline_size -= 88u;
  const std::vector<std::string> nested_errors = validate_modified_elf(bytes, nested_inventory);
  ...
  TransformArtifacts unrelated_inventory = nested_inventory;
  auto unrelated_atomic = std::ranges::find(unrelated_inventory.patches,
                                            PatchKind::TrampolineSyncMetadata, &PatchInfo::kind);
  ...
  const std::vector<std::string> unrelated_errors =
      validate_modified_elf(bytes, unrelated_inventory);
  ...
  const auto mappings = static_access_mappings(result);
  ASSERT_EQ(mappings.size(), 1u);
  ASSERT_GT(mappings.front().bank_count, 1u);
  ASSERT_TRUE(atomic->scratch_vgpr);
  ASSERT_TRUE(test_owner_vgpr(result));
  ...
  // causal windows. AtomicScratchLayout::kBank is 8.
  const uint16_t atomic_bank = static_cast<uint16_t>(*atomic->scratch_vgpr + 8u);
  const auto owner_bank_mix =
      instrumentation::build_v_xor_b32(atomic_bank, vector_source_vgpr(*test_owner_vgpr(result)),
                                       atomic_bank, ROCJITSU_CODE_ARCH_CDNA5);
  ASSERT_TRUE(owner_bank_mix);
  EXPECT_NE(std::ranges::find(atomic_body, *owner_bank_mix), atomic_body.end());
  EXPECT_EQ(decision_lowering_count(result, result.observation_plan().atomic_site_decisions,
                                    LoweringOutcomeKind::Instrumented),
            1u);
}

TEST(ConSan, Gfx1250PublishesIsolatedLdsReleaseOrdering) {
  ...
  const TransformArtifacts result =
      test_lower_consan(make_gfx1250_code_object(words, "gfx1250_isolated_lds_release"), options);

  ASSERT_TRUE(patch_succeeded(result)) << testing::PrintToString(result.errors);
  EXPECT_EQ(
      decision_count(result.observation_plan().atomic_site_decisions, SiteDecisionKind::Admitted),
      1u);
  EXPECT_EQ(std::ranges::count_if(result.patches,
                                  [](const PatchInfo &patch) {
                                    return patch.kind == PatchKind::InlineWatchpointStore ||
...
TEST(ConSan, Cdna4AtomicTracksCacheAssociatedOrdering) {
  ...
  options.exec_save_sgpr = 80;
  options.set_owner_epoch_vgprs(20, 21);
  options.exact_workgroup_vgprs = PersistentWorkgroupRegisters{30u, 31u, 32u};
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(2);
  options.max_patches = 2;
  ...
  EXPECT_NE(std::ranges::find(result.patches, PatchKind::TrampolineSyncMetadata, &PatchInfo::kind),
            result.patches.end());
  const auto sync =
      std::ranges::find(result.patches, PatchKind::TrampolineSyncMetadata, &PatchInfo::kind);
...
TEST(ConSan, Cdna4RetainsSingleRoleAtomicOverRedundantReleaseHalf) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  const auto release = cdna4::build_mubuf(cdna4::kBufferWbl2Mubuf, {.sc1 = 1});
  const auto acquire = cdna4::build_mubuf(cdna4::kBufferInvMubuf, {.sc1 = 1});
  const auto atomic = build_cdna4_flat_atomic_add_u32(
  ...
  *position++ = *wait;
  ...
  const TransformArtifacts result =
      test_lower_consan(make_cdna4_lds_code_object(text_words, "atomic_role_priority"), options);
...
TEST(ConSan, Cdna4RelocatesOrdinaryAtomicAcquireSequence) {
  const Cdna4CasOrdinaryAcquireFixture fixture =
      make_cdna4_cas_ordinary_acquire_fixture("cas_ordinary_acquire");
  TestOptions options = test_options();
  options.track_atomics = true;
  options.scratch_vgpr = 8;
  options.exec_save_sgpr = 80;
  ...
  ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
  EXPECT_EQ(result.outcome, TransformOutcome::ModifiedValid);
  const auto acquire = std::ranges::find_if(result.patches, [&](const PatchInfo &patch) {
  ...
  const auto metadata = encode_sync_metadata({
      .address = 1u,
      .byte_count = 4u,
      .kind = SyncMetadataKind::Atomic,
  ...
  ASSERT_EQ(metadata.classification, SyncClassification::Valid);
  EXPECT_NE(std::ranges::find(cave, metadata.packed.descriptor), cave.end());
}

TEST(ConSan, Gfx1250AssignsDistinctWindowsToGeneratedBufferPollingLoops) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA5;
  ...
  for (size_t index = 0; index < 2u; ++index) {
    ...
    const SyncSequence &sequence = sequence_view[index];
  ...
  std::vector<const PatchInfo *> sync_patches;
  for (const PatchInfo &patch : result.patches) {
    if (patch.kind == PatchKind::TrampolineSyncMetadata)
      sync_patches.push_back(&patch);
  }
  ASSERT_EQ(sync_patches.size(), 2u)
      << testing::PrintToString(result.patches) << testing::PrintToString(result.warnings);
  std::ranges::sort(sync_patches, {}, [](const PatchInfo *patch) { return patch->anchor_offset; });
  AmdGpuCodeObject patched(result.replacement.data(), result.replacement.size());
  ASSERT_TRUE(patched.is_valid());
  for (size_t index = 0; index < sync_patches.size(); ++index) {
    const PatchInfo &sync = *sync_patches[index];
...
TEST(ConSan, Gfx1250PublishesWideBufferReleaseRange) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA5;
  ...
  const TransformArtifacts result = test_lower_consan(
      make_gfx1250_code_object(text_words, "gfx1250_wide_buffer_release"), options);
...
TEST(ConSan, Gfx1250RelocatesBankedPollingLoopWithScalarSpill) {
  ...
  std::vector<uint8_t> bytes = make_gfx1250_code_object(text_words, "gfx1250_banked_buffer_poll",
  ...
  options.exact_workgroup_vgprs = PersistentWorkgroupRegisters{62u, 63u, 64u};
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(4u);
  options.report_dispatch_id = 0x1122334455667788ull;
  options.track_barriers = false;
  options.track_atomics = true;
  options.test_force_vgpr_spill = true;
...
TEST(ConSan, RoutesOnlyNearestAcquireToEachPayloadWindow) {
  ...
  for (const Target &target : kCdnaTargets) {
    ...
    std::vector<uint8_t> bytes = target.arch == ROCJITSU_CODE_ARCH_CDNA3
                                     ? make_cdna3_lds_code_object(text_words, "nearest_acquire")
                                     : make_cdna4_lds_code_object(text_words, "nearest_acquire");
    TestOptions options = test_options();
    options.track_barriers = false;
    options.track_atomics = true;
    options.scratch_vgpr = 8u;
    options.exec_save_sgpr = 80u;
    options.set_owner_epoch_vgprs(20u, 21u);
    options.exact_workgroup_vgprs = PersistentWorkgroupRegisters{30u, 31u, 32u};
    options.report_buffer_address = 0x123456780000ull;
    options.report_buffer_size = direct_report_bytes(4u);
    options.runtime_sample_stride = 1u;
...
TEST(ConSan, Cdna4AtomicReportsGuestOperandOverlapFallback) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  const auto release = cdna4::build_mubuf(cdna4::kBufferWbl2Mubuf, {.sc1 = 1});
  const auto acquire = cdna4::build_mubuf(cdna4::kBufferInvMubuf, {.sc1 = 1});
  const auto atomic = build_cdna4_flat_atomic_add_u32(
  ...
  text_words.back() = build_s_endpgm(kArch);
  std::vector<uint8_t> bytes =
      make_cdna4_lds_code_object(text_words, "atomic_overlap_fallback", /*vgpr_granulated=*/7u);
  mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
  ...
  options.set_owner_epoch_vgprs(29, 30);
  options.exact_workgroup_vgprs = PersistentWorkgroupRegisters{26u, 27u, 28u};
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(2);
  options.max_patches = 2;
  ...
  const auto atomic_plan =
      std::ranges::find_if(result.resource_plans, [](const CandidateResourcePlan &plan) {
        return plan.site_kind == ResourceSiteKind::Atomic;
      });
  ASSERT_NE(atomic_plan, result.resource_plans.end());
  ASSERT_EQ(atomic_plan->alternatives.size(), 1u);
  const ResourcePlanAlternative &alternative = atomic_plan->alternatives.front();
  EXPECT_EQ(alternative.kind, ResourcePlanAlternativeKind::GuestOperandOverlapSpill);
  EXPECT_EQ(alternative.source, RegisterAllocationSource::SpillRequired);
  EXPECT_EQ(alternative.scratch_vgpr_count, 11u);
  EXPECT_EQ(resource_plan_alternative_outcome(*atomic_plan, alternative),
            ResourcePlanAlternativeOutcome::Selected);
  const ResourcePlanSummary summary = test_resource_plan_summary(result);
...
TEST(ConSan, Cdna4BarrierOnlyObjectAdvancesEpochWithoutCausalWindows) {
  ...
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(0);
  const auto result = test_lower_consan(make_cdna4_lds_code_object(words, "barrier_only"), options);
  ASSERT_TRUE(patch_succeeded(result)) << testing::PrintToString(result.errors);
...
TEST(ConSan, Cdna4BarrierPublishesSelectedEpochTransition) {
  const auto barrier = build_cdna4_s_barrier(ROCJITSU_CODE_ARCH_CDNA4);
  ASSERT_TRUE(barrier);
  ...
  std::vector<uint8_t> bytes =
      make_cdna4_lds_code_object(text_words, "barrier", /*vgpr_granulated=*/3u);
  mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
    AMDHSA_BITS_SET(descriptor.compute_pgm_rsrc3, kd::COMPUTE_PGM_RSRC3_GFX90A_ACCUM_OFFSET, 3u);
  });
  const TransformArtifacts result = test_lower_consan(bytes, options);
  ...
  // synchronization point; the long ConSan probe and restoration follow.
...
TEST(ConSan, CdnaAtomicUsesPrivatePersistentStateAtAccvgprBoundary) {
      ...
      for (bool deferred_acquire : {false, true}) {
        ...
        std::vector<uint8_t> bytes =
            target.arch == ROCJITSU_CODE_ARCH_CDNA3
                ? make_cdna3_lds_code_object(text_words, "atomic_accvgpr_boundary",
                                             /*vgpr_granulated=*/3u)
                : make_cdna4_lds_code_object(text_words, "atomic_accvgpr_boundary",
                                             /*vgpr_granulated=*/3u);
        mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
        ...
        ASSERT_TRUE(patch_succeeded(result)) << testing::PrintToString(result.errors);
        ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
        EXPECT_TRUE(test_persistent_sgpr_state(result).exact_workgroup.empty())
            << testing::PrintToString(result.warnings);
        EXPECT_TRUE(test_exact_workgroup_vgprs(result).empty())
            << testing::PrintToString(result.warnings);
        const auto access = std::ranges::find_if(result.patches, [](const PatchInfo &patch) {
          return patch.kind == PatchKind::InlineWatchpointStore ||
                 patch.kind == PatchKind::TrampolineWatchpointStore;
        ...
        const auto prologue = std::ranges::find(
            result.patches, PatchKind::KernelEntryPrivateEpochPrologue, &PatchInfo::kind);
        ASSERT_NE(access, result.patches.end());
        ASSERT_NE(atomic, result.patches.end()) << testing::PrintToString(result.warnings);
        ASSERT_NE(prologue, result.patches.end());
...
TEST(ConSan, SignExtendsCdnaVglobalOffsetAt13BitBoundaries) {
...
TEST(ConSan, CdnaMaterializesFlatAddressBeforeAtomicResultOverwritesIt) {
  AtomicAddressPlan plan;
  plan.kind = AtomicAddressKind::FlatGuestPairMaterialized;
  ...
  plan.resource_source = RegisterAllocationSource::SpillRequired;
...
TEST(ConSan, CdnaVglobalMaterializationSelectsSafeSignScratch) {
  AtomicAddressPlan plan;
  ...
  for (const Target &target : kCdnaTargets) {
    SCOPED_TRACE(target.label);
    const auto expect_sign_scratch = [&](const AtomicAddressPlan &candidate, uint16_t sign_vgpr) {
      const auto materialization = build_atomic_address_materialization(
    ...
    AtomicAddressPlan boundary = plan;
    ...
    AtomicAddressPlan under_reserved = plan;
    under_reserved.result_address_vgpr = 5u;
    EXPECT_FALSE(build_atomic_address_materialization(under_reserved, /*vcc_save_sgpr=*/82u,
                                                      /*scc_save_sgpr=*/84u, target.arch));
  }
}

TEST(ConSan, AddressMaterializationRejectsMalformedScratchWindows) {
  constexpr std::array<Target, 4> kMaterializationTargets = {{
  ...
  for (const Target &target : kMaterializationTargets) {
    SCOPED_TRACE(target.label);
    // Positive control: the fixture is valid for each distinct emitter.
    ASSERT_TRUE(build_atomic_address_materialization(plan, /*vcc_save_sgpr=*/82u,
                                                     /*scc_save_sgpr=*/84u, target.arch));
    ...
    for (const MalformedWindow &window : kMalformedWindows) {
      ...
      AtomicAddressPlan malformed = plan;
      ...
      EXPECT_FALSE(build_atomic_address_materialization(malformed, /*vcc_save_sgpr=*/82u,
                                                        /*scc_save_sgpr=*/84u, target.arch));
    }
  }
}

TEST(ConSan, CdnaVglobalMaterializesVectorAndScalarAddressesInScratchTail) {
  struct VglobalCase {
    std::string_view label;
    std::array<uint32_t, 2> words;
    AtomicAddressKind expected_kind;
  ...
  constexpr std::array<VglobalCase, 2> kVglobalCases = {{
      {
          ...
          AtomicAddressKind::VglobalGuestPairMaterialized,
          2u,
          std::nullopt,
          -20,
      },
      {
          ...
          AtomicAddressKind::VglobalMaterialized,
          1u,
          20u,
          20,
      },
  ...
  for (const Target &target : kCdnaTargets) {
    ...
    EXPECT_EQ(arch_for_target(target_id), target.arch);
    EXPECT_EQ(
        capability_disposition(target_id, Mode::Default, CapabilityForm::OrderedVglobalAtomic),
    ...
    for (const VglobalCase &test_case : kVglobalCases) {
      ...
      std::vector<uint8_t> bytes =
          target.arch == ROCJITSU_CODE_ARCH_CDNA3
              ? make_cdna3_lds_code_object(text_words, "vglobal_materialized_spill",
                                           /*vgpr_granulated=*/3u)
              : make_cdna4_lds_code_object(text_words, "vglobal_materialized_spill",
                                           /*vgpr_granulated=*/3u);
      mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
        AMDHSA_BITS_SET(descriptor.compute_pgm_rsrc3, kd::COMPUTE_PGM_RSRC3_GFX90A_ACCUM_OFFSET,
                        2u);
      ...
      ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
      ASSERT_EQ(result.program_inventory.kernels().size(), 1u);
      const auto atomic_sites = test_decoded_sites<AtomicSite>(
          result.program_inventory, result.program_inventory.kernels().front());
      const auto site = std::ranges::find_if(atomic_sites, [](const AtomicSite &item) {
      ...
      const AtomicAddressPlan address_plan =
          plan_atomic_address(*site, *patch->scratch_vgpr, patch->spilled_vgpr_count,
                              RegisterAllocationSource::SpillRequired, target.arch,
      ...
      for (int32_t boundary : {-(1 << 12), (1 << 12) - 1}) {
        AtomicSite boundary_site = *site;
        boundary_site.raw_ioffset = boundary;
        EXPECT_TRUE(plan_atomic_address(boundary_site, *patch->scratch_vgpr,
                                        patch->spilled_vgpr_count,
                                        RegisterAllocationSource::SpillRequired, target.arch,
                                        /*allow_post_guest_spill_operand_overlap=*/true)
                        .supported());
      }
      for (int32_t out_of_range : {-(1 << 12) - 1, 1 << 12}) {
        ...
        EXPECT_EQ(plan_atomic_address(boundary_site, *patch->scratch_vgpr,
                                      patch->spilled_vgpr_count,
                                      RegisterAllocationSource::SpillRequired, target.arch,
                                      /*allow_post_guest_spill_operand_overlap=*/true)
                      .support,
                  AtomicAddressSupport::UnsupportedOffset);
      }
...
TEST(ConSan, Cdna4SharedAtomicSeparatesPersistentStateFromSpills) {
  const std::vector<uint8_t> bytes = make_cdna4_two_kernel_shared_atomic_code_object(
...
TEST(ConSan, Cdna4AtomicMaterializesAddressWithDynamicScalarState) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  const auto access =
  ...
  text_words.back() = build_s_endpgm(kArch);
  std::vector<uint8_t> bytes = make_cdna4_lds_code_object(text_words, "atomic_result_address_alias",
                                                          /*vgpr_granulated=*/7u,
  ...
  ASSERT_TRUE(patch_succeeded(result)) << testing::PrintToString(result.errors);
  ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
  EXPECT_TRUE(test_persistent_sgpr_state(result).owner());
  EXPECT_TRUE(test_persistent_sgpr_state(result).epoch());
  const auto atomic_patch =
      std::ranges::find(result.patches, PatchKind::TrampolineSyncMetadata, &PatchInfo::kind);
  ...
  // AtomicScratchLayout::kOwner and ::kEpoch constants.
  ...
  EXPECT_NE(std::ranges::find(
                cave, build_v_mov_b32_e32(static_cast<uint16_t>(*atomic_patch->scratch_vgpr +
                                          ...
                                          *test_persistent_sgpr_state(result).owner(), kArch)),
            cave.end());
  EXPECT_NE(std::ranges::find(
                cave, build_v_mov_b32_e32(static_cast<uint16_t>(*atomic_patch->scratch_vgpr +
                                                                kAtomicEpochScratchOffset),
                                          *test_persistent_sgpr_state(result).epoch(), kArch)),
            cave.end());
  EXPECT_NE(
...
TEST(ConSan, Cdna4AtomicRejectsFlatScratchScalarAlias) {
  const auto atomic = build_cdna4_flat_atomic_add_u32(
      /*vaddr=*/4, /*vsrc=*/6, /*vdst=*/7, /*return_old_value=*/true,
      /*scope=*/2, ROCJITSU_CODE_ARCH_CDNA4);
  const auto wait = build_cdna4_s_wait_flat0(ROCJITSU_CODE_ARCH_CDNA4);
  ...
  text_words.back() = build_s_endpgm(ROCJITSU_CODE_ARCH_CDNA4);
  const std::vector<uint8_t> bytes =
      make_cdna4_lds_code_object(text_words, "atomic_special_sgpr_alias");
  TestOptions options = test_options();
  options.track_atomics = true;
  options.scratch_vgpr = 8;
  options.exec_save_sgpr = 96;
  options.set_owner_epoch_vgprs(20, 21);
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(2);
  options.max_patches = 2;
...
TEST(ConSan, AtomicTrackingAdmitsWorkgroupScope) {
  const std::vector<uint8_t> bytes = make_rdna4_lds_and_ordered_flat_atomic_code_object(
...
TEST(ConSan, AtomicCasPublishesOnlyEvidenceDerivedOutcomes) {
  const std::vector<uint8_t> bytes =
      make_rdna4_lds_and_ordered_flat_atomic_code_object(/*compare_exchange=*/true);
  ASSERT_FALSE(bytes.empty());
  TestOptions options = test_options();
  options.track_atomics = true;
  options.scratch_vgpr = 8;
  ...
  const std::vector<uint32_t> trampoline =
      text_words_at_offset(patched, patch->trampoline_offset, patch->trampoline_size);
  const auto descriptor = [&](SyncOutcome outcome) {
    return encode_sync_metadata({
        .address = 1,
        .byte_count = 4,
        .kind = SyncMetadataKind::Atomic,
        .role = SyncRole::RmwRelease,
  ...
  const auto success = descriptor(SyncOutcome::CasSuccess);
  const auto failure = descriptor(SyncOutcome::CasFailure);
  ASSERT_EQ(success.classification, SyncClassification::Valid);
  ASSERT_EQ(failure.classification, SyncClassification::Valid);
...
TEST(ConSan, AtomicCasWithoutReturnedOldValueFailsClosed) {
...
TEST(ConSan, AtomicForcedSpillPreservesElevenVgprWindow) {
  const std::vector<uint8_t> bytes = make_rdna4_lds_and_ordered_flat_atomic_code_object();
  TestOptions options = test_options();
  options.track_atomics = true;
  options.test_force_vgpr_spill = true;
...
TEST(ConSan, Rdna4DynamicStackAtomicUsesSiteLocalSpillFrames) {
  ...
  const std::vector<uint8_t> bytes =
      make_rdna4_lds_code_object(words, "dynamic_atomic_spill", kRdna4Wave64AllVgprsGranulated,
...
TEST(ConSan, AtomicEdgesAssociateWithTheirOrderedAccessWindows) {
  const std::vector<uint8_t> bytes = make_rdna4_lds_and_ordered_flat_atomic_handoff_code_object();
  TestOptions options = test_options();
  options.track_atomics = true;
  options.scratch_vgpr = 24;
  options.set_owner_epoch_vgprs(20, 21);
  options.dispatch_sgpr.set(22);
  options.report_buffer_address = 0x123456780000ull;
  options.report_dispatch_id = 0x1122334455667788ull;
  options.report_buffer_size = direct_report_bytes(32u);
  options.max_patches = 4;
  ...
  for (uint32_t slot = 0; slot < sync_patches.size(); ++slot) {
    const PatchInfo &patch = *sync_patches[slot];
    ASSERT_TRUE(patch.scratch_vgpr);
    ...
    const uint64_t materialized_address =
        *options.report_buffer_address +
        (slot == 1 ? layout.pending_acquires_offset : layout.sync_metadata_offset);
    const TargetProfile *target = target_profile(ROCJITSU_CODE_ARCH_RDNA4);
    ASSERT_NE(target, nullptr);
    ...
    ASSERT_TRUE(detail::append_indexed_address(
        ...
        {.table_address = materialized_address,
         .stride_bytes = static_cast<uint32_t>(slot == 1 ? sizeof(PendingAcquireSlot)
    ...
    if (slot == 0) {
      ...
      for (const uint16_t dispatch_sgpr :
           {*options.dispatch_sgpr.base(),
            static_cast<uint16_t>(*options.dispatch_sgpr.base() + 1u)}) {
        const auto compare =
      ...
      EXPECT_TRUE(contains_subsequence(trampoline,
                                       make_expected_scalar_offset_store_words(
                                           offsetof(PendingAcquireSlot, dispatch_id),
                                           *options.dispatch_sgpr.base(), *patch.scratch_vgpr)));
      EXPECT_TRUE(contains_subsequence(
          trampoline,
          make_expected_scalar_offset_store_words(
              offsetof(PendingAcquireSlot, dispatch_id) + sizeof(uint32_t),
              static_cast<uint16_t>(*options.dispatch_sgpr.base() + 1u), *patch.scratch_vgpr)));
      EXPECT_TRUE(contains_subsequence(
          trampoline,
          make_expected_offset_store_words(
              offsetof(PendingAcquireSlot, metadata) + offsetof(SyncMetadataPacked, descriptor),
              static_cast<uint16_t>(*patch.scratch_vgpr + 2u), *patch.scratch_vgpr)));
      const uint64_t contention_address =
          *options.report_buffer_address + offsetof(ReportHeader, pending_acquire_contention_count);
...
TEST(ConSan, AtomicWeakenedReleaseIsReinventoriedBeforeInstrumentation) {
  const std::vector<uint8_t> bytes = make_rdna4_lds_and_ordered_flat_atomic_handoff_code_object();
  TestOptions options = test_options();
  options.track_atomics = true;
  options.scratch_vgpr = 24;
  ...
  const TransformArtifacts clean = test_lower_consan(bytes, options);
  ASSERT_TRUE(clean.errors.empty()) << testing::PrintToString(clean.errors);
  EXPECT_EQ(std::ranges::count(clean.patches, PatchKind::TrampolineSyncMetadata, &PatchInfo::kind),
            2u);
  ...
  const TransformArtifacts fault = test_lower_consan(bytes, options);

  ASSERT_TRUE(fault.errors.empty()) << testing::PrintToString(fault.errors);
  EXPECT_EQ(fault.outcome, TransformOutcome::ModifiedValid);
  EXPECT_EQ(fault.mutation.fault.applied, 1u);
  EXPECT_TRUE(std::ranges::any_of(fault.patches, [](const PatchInfo &patch) {
    return patch.phase == PatchPhase::Mutation && patch.kind == PatchKind::InlineAtomicOrderRewrite;
  }));
  std::vector<const PatchInfo *> fault_sync;
  for (const PatchInfo &patch : fault.patches) {
    if (patch.kind == PatchKind::TrampolineSyncMetadata)
...
TEST(ConSan, DirectProbeAutomaticallyUsesDeadVgprs) {
  std::array<uint32_t, 420> text_words{};
  text_words[0] = 0xD8340000u;
  text_words[1] = 0x00000000u; // ds_store_b32 v0, v0
  for (size_t i = 2; i + 1 < text_words.size(); ++i)
  ...
  ASSERT_EQ(result.resource_plans.size(), 1u);
  EXPECT_EQ(result.resource_plans.front().source, RegisterAllocationSource::LivenessDead);
...
TEST(ConSan, DirectProbeDoesNotTrustLivenessWithRelativeVgprAccess) {
  std::array<uint32_t, 420> text_words{};
  text_words[0] = 0xD8340000u;
  text_words[1] = 0x00000000u; // ds_store_b32 v0, v0
  ...
  ASSERT_TRUE(result.modified());
  ASSERT_EQ(result.resource_plans.size(), 1u);
  EXPECT_EQ(result.resource_plans.front().source, RegisterAllocationSource::DescriptorGrowth);
...
TEST(ConSan, Gfx1250AutomaticExecSaveUsesOwnerLocalWindow) {
  ...
  EXPECT_TRUE(std::ranges::all_of(result.resource_plans, [](const auto &plan) {
    return plan.site_kind != ResourceSiteKind::Access ||
           plan.source != RegisterAllocationSource::Unsupported;
  }));
}

TEST(ConSan, Cdna4DispatchOverrideRetainsLivenessDeadGlobalExecWindow) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  ...
  TestOptions options = test_options();
  options.scratch_vgpr = 8u;
  options.exec_save_sgpr = kGlobalExecSaveSgpr;
  options.dispatch_sgpr.set(96u, true);
  options.set_owner_epoch_vgprs(40u, 41u);
  options.runtime_sample_stride = 2u;
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(2u);
  ...
  ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
  ASSERT_TRUE(test_exec_save_sgpr(result));
  EXPECT_EQ(*test_exec_save_sgpr(result), kGlobalExecSaveSgpr);
  EXPECT_EQ(std::ranges::count_if(result.patches,
                                  [](const PatchInfo &patch) {
                                    return patch.kind == PatchKind::InlineWatchpointStore ||
                                           patch.kind == PatchKind::TrampolineWatchpointStore;
            ...
            2u)
      << testing::PrintToString(result.warnings);
  EXPECT_EQ(access_lowering_count(result, LoweringOutcomeKind::Instrumented), 2u)
      << testing::PrintToString(result.warnings);
  EXPECT_EQ(result.outcome, TransformOutcome::ModifiedValid);
}

TEST(ConSan, Cdna4DispatchOverridePreservesPriorOwnerLocalExecWindow) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  constexpr uint16_t kGlobalExecSaveSgpr = 80u;
  ...
  options.automatic_partial_exec_save_sgprs = true;
  const TransientSgprAssignment seed_assignment{
  ...
  const TransformArtifacts result =
      test_lower_with_owner_transient_sgpr_assignment(bytes, options, seed_assignment);
  ...
  const auto assignment =
      test_transient_sgpr_assignment(result, high_kernel->descriptor_file_offset);
  ...
  EXPECT_EQ(std::ranges::count_if(result.patches,
      ...
      << testing::PrintToString(result.warnings);
  for (const CandidateResourcePlan &plan : result.resource_plans) {
    if (plan.site_kind == ResourceSiteKind::Access) {
      EXPECT_NE(plan.source, RegisterAllocationSource::Unsupported)
          << register_plan_reason_name(plan.reason) << " at " << plan.text_offset << " owner_id "
          << (plan.owner_kernel_ids.empty() ? ProgramContainerId::kInvalidOrdinal
...
TEST(ConSan, Gfx1250SpillsExecVccStateWithSeparateDeadDenseRouter) {
  ...
  for (const bool uses_dynamic_stack : {false, true}) {
    ...
    const std::vector<uint8_t> bytes =
        make_gfx1250_code_object(words, "scalar_spill", kRdna4Wave64AllVgprsGranulated,
    ...
    options.set_owner_epoch_vgprs(40, 41);
    options.report_buffer_address = 0x123456780000ull;
    options.report_buffer_size = direct_report_bytes(9);
    options.track_barriers = false;
    options.track_atomics = false;
    ...
    ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
    ASSERT_EQ(test_transient_sgpr_assignments(result).size(), 1u);
    const TransientSgprAssignment assignment = test_transient_sgpr_assignments(result).front();
...
TEST(ConSan, Gfx1250DenseBarrierUsesCloneInvariantEntryKeys) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA5;
  ...
  TestOptions options = test_options();
  options.scratch_vgpr = 8u;
  options.exec_save_sgpr = kExecSaveSgpr;
  options.set_owner_epoch_vgprs(40u, 41u);
  options.report_buffer_address = 0x123456780000ull;
...
void expect_dense_barrier_routes_through_dead_pair_under_scalar_spill(rj_code_arch_t arch) {
  SCOPED_TRACE(arch);
  ...
  const std::vector<uint8_t> bytes =
      arch == ROCJITSU_CODE_ARCH_CDNA5
          ? make_gfx1250_code_object(words, "scalar_spill_barriers_gfx1250",
                                     kRdna4Wave64AllVgprsGranulated, /*wave32=*/true)
          : make_rdna4_lds_code_object(words, "scalar_spill_barriers_rdna4");
...
TEST(ConSan, Gfx1250DenseBarrierUsesCloneInvariantEntryKeysUnderScalarSpill) {
  expect_dense_barrier_routes_through_dead_pair_under_scalar_spill(ROCJITSU_CODE_ARCH_CDNA5);
}

TEST(ConSan, Rdna4DenseBarrierRoutesThroughDeadPairUnderScalarSpill) {
  expect_dense_barrier_routes_through_dead_pair_under_scalar_spill(ROCJITSU_CODE_ARCH_RDNA4);
}

TEST(ConSan, Rdna4SpillBackedRelocationKeepsCompleteCoverage) {
  ...
  EXPECT_EQ(
      std::ranges::count(result.patches, PatchKind::TrampolineWatchpointStore, &PatchInfo::kind),
      kAccessCount);
  EXPECT_EQ(access_lowering_count(result, LoweringOutcomeKind::Instrumented), kAccessCount);
  EXPECT_EQ(result.outcome, TransformOutcome::ModifiedValid);
}

TEST(ConSan, Gfx1250SpillBackedRelocationKeepsCompleteCoverage) {
  constexpr uint32_t kAccessCount = 9u;
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA5;
  const std::array<uint16_t, 4> dead = {0u, 1u, 4u, 6u};
  ...
  const TransformArtifacts result =
      test_lower_consan(make_gfx1250_code_object(words, "gfx1250_spill_dense_call_key"), options);
...
TEST(ConSan, Cdna4SpillsFullPressureStateThroughDynamicStackFrame) {
  ...
  std::vector<uint8_t> bytes = make_cdna4_lds_code_object(text_words, "full_pressure_dynamic_stack",
  ...
  AmdGpuCodeObject patched(result.replacement.data(), result.replacement.size());
  ASSERT_TRUE(patched.is_valid());
  for (const PatchInfo *patch : {&*access, &*sync}) {
...
TEST(ConSan, Cdna4MovesEmptyAccumulatorBoundaryForDynamicStackState) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  const auto store =
  ...
  std::vector<uint8_t> bytes = make_cdna4_lds_code_object(
      text_words, "dynamic_stack_empty_accumulator_boundary",
  ...
  ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
  EXPECT_EQ(result.outcome, TransformOutcome::ModifiedValid);
  const auto persistent_assignments = test_persistent_vgpr_assignments(result);
  ASSERT_EQ(persistent_assignments.size(), 1u);
  const PersistentVgprAssignment &assignment = persistent_assignments.front();
...
TEST(ConSan, Cdna4MixesPrivateAndEmptyAccumulatorBoundaryState) {
  ...
  const ProgramContainer *dynamic_owner =
  ...
  // store needs only the ConSan transaction registers and identity bank. The
  ...
  const uint16_t dynamic_access_scratch_count =
      static_cast<uint16_t>((options.device_conflict_check ? 7u : 5u) + 1u);
  EXPECT_EQ(dynamic_plan->scratch_vgpr_count, dynamic_access_scratch_count);

  const auto dynamic_barrier_plan =
      std::ranges::find_if(result.resource_plans, [&](const CandidateResourcePlan &plan) {
        return plan.site_kind == ResourceSiteKind::Barrier &&
               std::ranges::find(plan.owner_kernel_ids, dynamic_owner->id) !=
                   plan.owner_kernel_ids.end();
      });
...
TEST(ConSan, Cdna4RecoversInitiallyResourceFailedOwnerComponent) {
  const std::vector<uint8_t> bytes = make_cdna4_mixed_accvgpr_persistence_code_object(
      /*dynamic_flat_group_load=*/true,
  ...
  const auto persistent_assignments = test_persistent_vgpr_assignments(result);
  ASSERT_EQ(persistent_assignments.size(), 1u);
  const PersistentVgprAssignment &persistent = persistent_assignments.front();
  ...
  ASSERT_TRUE(test_exec_save_sgpr(result));
  EXPECT_GE(*test_exec_save_sgpr(result), 66u);
  ASSERT_EQ(test_transient_sgpr_assignments(result).size(), 1u)
      << testing::PrintToString(test_transient_sgpr_assignments(result));
  const auto dynamic_transient =
      test_transient_sgpr_assignment(result, dynamic->descriptor_file_offset);
  ASSERT_TRUE(dynamic_transient);
  EXPECT_NE(*test_exec_save_sgpr(result), dynamic_transient->exec_save_sgpr);
  ...
  ASSERT_NE(dynamic_plan, result.resource_plans.end());
  EXPECT_NE(dynamic_plan->source, RegisterAllocationSource::Unsupported);
  ...
  ASSERT_NE(dynamic_barrier_plan, result.resource_plans.end());
  EXPECT_NE(dynamic_barrier_plan->source, RegisterAllocationSource::Unsupported);
  EXPECT_EQ(dynamic_barrier_plan->scratch_vgpr_count, 7u);
...
TEST(ConSan, CdnaStrideOnePrivateStateMirrorsKernargPreloadEntry) {
  ...
  for (const rj_code_arch_t arch : {ROCJITSU_CODE_ARCH_CDNA3, ROCJITSU_CODE_ARCH_CDNA4}) {
    ...
    std::vector<uint8_t> bytes =
        arch == ROCJITSU_CODE_ARCH_CDNA3
            ? make_cdna3_lds_code_object(text_words, "private_kernarg_preload",
                                         /*vgpr_granulated=*/3u,
                                         /*uses_dynamic_stack=*/false,
                                         /*workgroup_id_dimension_mask=*/1u)
            : make_cdna4_lds_code_object(text_words, "private_kernarg_preload",
                                         /*vgpr_granulated=*/3u,
                                         /*uses_dynamic_stack=*/false,
                                         /*workgroup_id_dimension_mask=*/1u);
    mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
    ...
    ASSERT_TRUE(patch_succeeded(result)) << "warnings=" << testing::PrintToString(result.warnings)
                                         << " errors=" << testing::PrintToString(result.errors);
    ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
    ASSERT_TRUE(test_dispatch_id_sgpr(result));
    const auto prologue = std::ranges::find(
    ...
    EXPECT_EQ(prologue->dispatch_id_prologue->capture.sgpr(), test_dispatch_id_sgpr(result));
...
TEST(ConSan, CdnaStrideOneDynamicStackRedirectsBothKernargPreloadEntries) {
  constexpr uint32_t kFirmwareEntryOffset = 256u;
  for (const rj_code_arch_t arch : {ROCJITSU_CODE_ARCH_CDNA3, ROCJITSU_CODE_ARCH_CDNA4}) {
    SCOPED_TRACE(arch == ROCJITSU_CODE_ARCH_CDNA3 ? "gfx942" : "gfx950");
    const auto guest = arch == ROCJITSU_CODE_ARCH_CDNA3
    ...
    std::ranges::copy(*guest, text_words.begin() + kFirmwareEntryOffset / sizeof(uint32_t));
    text_words.back() = build_s_endpgm(arch);
    constexpr std::string_view kKernelName = "dynamic_stack_kernarg_preload";
    std::vector<uint8_t> bytes =
        arch == ROCJITSU_CODE_ARCH_CDNA3
    ...
    ASSERT_TRUE(test_dispatch_id_sgpr(result));
    const auto prologue = std::ranges::find(
        result.patches, PatchKind::KernelEntryOwnerEpochPrologue, &PatchInfo::kind);
    ASSERT_NE(prologue, result.patches.end());
    ASSERT_TRUE(prologue->dispatch_id_prologue);
    EXPECT_EQ(prologue->dispatch_id_prologue->capture.sgpr(), test_dispatch_id_sgpr(result));
    EXPECT_TRUE(prologue->dispatch_id_prologue->preload.identity_salt_sgpr.has_value());
...
TEST(ConSan, Cdna4Wave64AccvgprBoundaryUsesPrivatePersistentState) {
  const auto guest = build_cdna4_ds_store_b32(
  ...
  text_words.back() = build_s_endpgm(ROCJITSU_CODE_ARCH_CDNA4);
  std::vector<uint8_t> bytes =
      make_cdna4_lds_code_object(text_words, "accvgpr_boundary", /*vgpr_granulated=*/3u);
  mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
  ...
  const auto patch =
      std::ranges::find(result.patches, PatchKind::TrampolineWatchpointStore, &PatchInfo::kind);
  ASSERT_NE(patch, result.patches.end());
  EXPECT_TRUE(patch->private_state_layout);
  EXPECT_TRUE(patch->private_state_layout->owner_offset);
  EXPECT_EQ(std::ranges::count(result.patches, PatchKind::KernelEntryPrivateEpochPrologue,
                               &PatchInfo::kind),
            1u);
  AmdGpuCodeObject original(bytes.data(), bytes.size());
...
TEST(ConSan, Cdna4PrivateWorkgroupStateUsesSpillSafeBodyGate) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  constexpr uint32_t kAccessCount = 9u;
  ...
  text_words.back() = build_s_endpgm(kArch);
  std::vector<uint8_t> bytes =
      make_cdna4_lds_code_object(text_words, "private_workgroup_fast_gate", /*vgpr_granulated=*/3u);
  mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
    // v12 is the first accumulator.  ConSan state and the entry-captured
  ...
  options.runtime_sample_stride = 2u;
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(2u * kAccessCount);
  options.track_barriers = false;
  options.track_atomics = false;
...
TEST(ConSan, Cdna4FullPressureStoreRecoversSpilledAddressAfterGuest) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  const auto guest =
      build_cdna4_ds_store_b32(/*vaddr=*/7u, /*vdata=*/8u, /*byte_offset=*/0u, kArch);
  ASSERT_TRUE(guest);
  const std::vector<uint8_t> bytes =
      make_cdna4_full_pressure_access(*guest, "store_spill_recovery");
  TestOptions options = test_options();
  options.runtime_sample_stride = 2u;
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(2);
  ...
  ASSERT_TRUE(patch_succeeded(result)) << "errors=" << testing::PrintToString(result.errors)
                                       << " warnings=" << testing::PrintToString(result.warnings);
  ASSERT_TRUE(result.modified());
  ASSERT_EQ(result.resource_plans.size(), 1u);
...
TEST(ConSan, Cdna4FullPressureLoadKeepsResultOutsideRecoveredSpillWindow) {
  constexpr auto guest = cdna4::build_ds(cdna4::kDsReadB32Ds, {.addr = 7u, .vdst = 8u});
  const std::vector<uint8_t> bytes = make_cdna4_full_pressure_access(guest, "load_spill_recovery");
  TestOptions options = test_options();
  options.runtime_sample_stride = 2u;
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(2);
...
TEST(ConSan, Cdna4FullPressureRead2B64KeepsFourRegisterResult) {
  constexpr auto guest =
      cdna4::build_ds(cdna4::kDsRead2B64Ds, {.offset1 = 4u, .addr = 7u, .vdst = 8u});
  const std::vector<uint8_t> bytes =
      make_cdna4_full_pressure_access(guest, "read2_b64_spill_recovery");
  TestOptions options = test_options();
  options.runtime_sample_stride = 2u;
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(64);
...
TEST(ConSan, Cdna4FullPressureLoadRejectsDestinationOverlap) {
  constexpr auto guest = cdna4::build_ds(cdna4::kDsReadB128Ds, {.addr = 5u, .vdst = 6u});
  const std::vector<uint8_t> bytes = make_cdna4_full_pressure_access(guest, "load_result_pressure");
  TestOptions options = test_options();
  options.runtime_sample_stride = 2u;
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(2);
  ...
  EXPECT_TRUE(std::ranges::any_of(plan.alternatives, [](const auto &alternative) {
    return alternative.kind == ResourcePlanAlternativeKind::SpillBackedOperandRecovery &&
           alternative.outcome == ResourcePlanAlternativeOutcome::Rejected;
  }));
}

TEST(ConSan, CdnaBarrierUsesPrivatePersistentStateAtAccvgprBoundary) {
  for (const Target &target : kCdnaTargets) {
    for (uint32_t sample_stride : {1u, 2u}) {
      ...
      std::vector<uint8_t> bytes =
          target.arch == ROCJITSU_CODE_ARCH_CDNA3
              ? make_cdna3_lds_code_object(text_words, "barrier_accvgpr_boundary",
                                           /*vgpr_granulated=*/3u)
              : make_cdna4_lds_code_object(text_words, "barrier_accvgpr_boundary",
                                           /*vgpr_granulated=*/3u);
      mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
      ...
      TestOptions options = test_options();
      options.owner_source = OwnerSource::Automatic;
      options.runtime_sample_stride = sample_stride;
      options.report_buffer_address = 0x123456780000ull;
      options.report_buffer_size = direct_report_bytes(2);
      options.track_barriers = true;
      options.track_atomics = false;
...
TEST(ConSan, CdnaSynchronizationSpillsThroughDynamicStackFrame) {
  for (const Target &target : kCdnaTargets) {
    SCOPED_TRACE(target.label);
    const auto guest =
        target.arch == ROCJITSU_CODE_ARCH_CDNA3
    ...
    text_words.back() = build_s_endpgm(target.arch);
    std::vector<uint8_t> bytes = target.arch == ROCJITSU_CODE_ARCH_CDNA3
                                     ? make_cdna3_lds_code_object(text_words, "dynamic_stack",
                                                                  /*vgpr_granulated=*/3u,
                                                                  /*uses_dynamic_stack=*/true)
                                     : make_cdna4_lds_code_object(text_words, "dynamic_stack",
                                                                  /*vgpr_granulated=*/3u,
                                                                  /*uses_dynamic_stack=*/true);
    mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
    ...
    EXPECT_TRUE(test_persistent_sgpr_state(result).owner());
    EXPECT_TRUE(test_persistent_sgpr_state(result).epoch());
    EXPECT_GE(*test_persistent_sgpr_state(result).owner(), 40u);
    ASSERT_FALSE(result.resource_plans.empty());
    EXPECT_FALSE(test_owner_vgpr(result));
    EXPECT_FALSE(test_epoch_vgpr(result));
    EXPECT_TRUE(std::ranges::all_of(result.resource_plans, [](const CandidateResourcePlan &plan) {
      return plan.source == RegisterAllocationSource::SpillRequired;
    }));
    ...
    EXPECT_NE(std::ranges::find(access_cave,
                                build_v_mov_b32_e32(access_owner_vgpr,
                                                    *test_persistent_sgpr_state(result).owner(),
                                                    target.arch)),
              access_cave.end());
    EXPECT_NE(std::ranges::find(access_cave,
                                build_v_mov_b32_e32(access_epoch_vgpr,
                                                    *test_persistent_sgpr_state(result).epoch(),
                                                    target.arch)),
              access_cave.end());
    ASSERT_TRUE(test_exec_save_sgpr(result));
    const uint16_t saved_frame_sgpr = static_cast<uint16_t>(*test_exec_save_sgpr(result) + 8u);
    for (const PatchInfo *patch : {&*access, &*sync}) {
      EXPECT_GT(patch->spilled_vgpr_count, 0u);
      EXPECT_GT(patch->required_private_segment_size, 0u);
      const std::vector<uint32_t> cave =
    ...
    const auto persist_epoch = instrumentation::build_v_readfirstlane_b32(
        *test_persistent_sgpr_state(result).epoch(), epoch_vgpr, target.arch);
...
TEST(ConSan, CdnaBarrierMaterializesNonzeroAbsoluteSlot) {
  for (const Target &target : kCdnaTargets) {
    SCOPED_TRACE(target.label);
    ...
    const std::vector<uint8_t> bytes = target.arch == ROCJITSU_CODE_ARCH_CDNA3
                                           ? make_cdna3_lds_code_object(text_words, "nonzero_slot")
                                           : make_cdna4_lds_code_object(text_words, "nonzero_slot");
    const TransformArtifacts result = test_lower_consan(bytes, options);

    ASSERT_TRUE(patch_succeeded(result)) << testing::PrintToString(result.errors);
    ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
    std::vector<const PatchInfo *> accesses;
    for (const PatchInfo &patch : result.patches) {
      if (patch.kind == PatchKind::InlineWatchpointStore ||
          patch.kind == PatchKind::TrampolineWatchpointStore) {
        accesses.push_back(&patch);
      }
    }
    ASSERT_EQ(accesses.size(), 2u);
    auto mappings = static_access_mappings(result);
    ASSERT_EQ(mappings.size(), 2u);
    std::ranges::sort(mappings, {}, &StaticAccessMapping::first_slot);
    EXPECT_EQ(mappings.front().first_slot, 0u);
    ASSERT_GT(mappings.back().first_slot, 0u);
    const auto barrier_patch =
        std::ranges::find(result.patches, PatchKind::TrampolineSyncMetadata, &PatchInfo::kind);
    ASSERT_NE(barrier_patch, result.patches.end());
    ASSERT_TRUE(barrier_patch->scratch_vgpr);
    expect_barrier_exact_existing_metadata_path(result, *barrier_patch, target.arch);
    ...
    const auto absolute_slot = instrumentation::build_v_add_u32_literal(
        bank_vgpr, value_vgpr, mappings.back().first_slot, bank_vgpr, target.arch);
...
TEST(ConSan, CdnaUsesPerOwnerPersistentTuplesAcrossAccvgprBoundaries) {
  for (const Target &target : kCdnaTargets) {
    SCOPED_TRACE(target.label);
    ...
    const auto probe_assignment =
        std::ranges::find(persistent_assignments, probe_kernel->descriptor_file_offset,
                          &PersistentVgprAssignment::descriptor_file_offset);
...
TEST(ConSan, DirectProbeSpillsIdentityAwareWindowInAppendedCave) {
  const std::array<uint32_t, 3> text_words = {
      0xD8340000u,
      0x00000000u, // ds_store_b32 v0, v0
  ...
  TransformArtifacts incomplete_payload = result;
  ...
  const std::vector<std::string> validation_errors =
      validate_modified_elf(bytes, incomplete_payload);
...
TEST(ConSan, Gfx1250DynamicStackStoreSpillsAcrossGuestOperands) {
  ...
  std::vector<uint8_t> bytes = make_gfx1250_code_object(words, "dynamic_store_operand_spill",
  ...
  options.runtime_sample_stride = 16384u;
  options.test_force_vgpr_spill = true;
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(1);
  options.max_patches = 1u;
  ...
  EXPECT_EQ(inventory.access_range_count, 1u);
  EXPECT_EQ(inventory.range_bank_count, 8u);
  EXPECT_EQ(inventory.watchpoint_count, 8u);
  EXPECT_GT(plan_auto_report(inventory).required_bytes, sizeof(ReportHeader));
}

TEST(ConSan, Rdna4DynamicStackSpillPlansScalarStateBeforeVgprGrowth) {
  ...
  words.back() = build_s_endpgm(ROCJITSU_CODE_ARCH_RDNA4);
  std::vector<uint8_t> bytes = make_rdna4_lds_code_object(words, "dynamic_low_vgpr_store",
  ...
  ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
  ASSERT_TRUE(test_exec_save_sgpr(result));
  EXPECT_FALSE(test_dispatch_id_sgpr(result));
  ASSERT_EQ(test_transient_sgpr_assignments(result).size(), 1u);
  EXPECT_TRUE(test_transient_sgpr_assignments(result).front().spill_backed);
  const auto patch =
      std::ranges::find(result.patches, PatchKind::TrampolineWatchpointStore, &PatchInfo::kind);
...
TEST(ConSan, Gfx1250DynamicStackLoadUsesDisjointSpillWindow) {
  std::vector<uint32_t> words(128u, build_s_nop(0, ROCJITSU_CODE_ARCH_CDNA5));
  ...
  words[96] = build_v_mov_b32_e32(/*vdst=*/0u, vector_source_vgpr(8u), ROCJITSU_CODE_ARCH_CDNA5);
  words.back() = build_s_endpgm(ROCJITSU_CODE_ARCH_CDNA5);
  std::vector<uint8_t> bytes = make_gfx1250_code_object(words, "dynamic_load_operand_spill",
                                                        /*vgpr_granulated=*/0u, /*wave32=*/true,
                                                        /*uses_dynamic_stack=*/true);
  mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
    descriptor.private_segment_fixed_size = 16u;
...
TEST(ConSan, DirectProbeCanUseSleepDelay) {
  std::array<uint32_t, 420> text_words{};
  text_words[0] = 0xD8340000u;
  text_words[1] = 0x00000000u; // ds_store_b32 v0, v0
  for (size_t i = 2; i + 1 < text_words.size(); ++i)
  ...
  const auto patch = std::ranges::find_if(result.patches, [](const PatchInfo &item) {
    return item.kind == PatchKind::InlineWatchpointStore ||
           item.kind == PatchKind::TrampolineWatchpointStore;
  });
  ASSERT_NE(patch, result.patches.end());
  AmdGpuCodeObject patched(result.replacement.data(), result.replacement.size());
  ASSERT_TRUE(patched.is_valid());
  const size_t patch_offset = patch->kind == PatchKind::InlineWatchpointStore
                                  ? patch->anchor_offset
                                  : patch->trampoline_offset;
  const size_t patch_size = patch->kind == PatchKind::InlineWatchpointStore
                                ? patch->original_size
...
TEST(ConSan, DirectProbeCanUseSleepVarDelay) {
  std::array<uint32_t, 420> text_words{};
  text_words[0] = 0xD8340000u;
  text_words[1] = 0x00000000u; // ds_store_b32 v0, v0
  for (size_t i = 2; i + 1 < text_words.size(); ++i)
...
TEST(ConSan, DirectProbeRejectsOversizedSleepVarSource) {
  std::array<uint32_t, 420> text_words{};
  text_words[0] = 0xD8340000u;
  text_words[1] = 0x00000000u; // ds_store_b32 v0, v0
  for (size_t i = 2; i + 1 < text_words.size(); ++i)
  ...
  options.supercollider_delay_mode = SuperColliderDelayMode::SleepVar;
  options.supercollider_delay_nops = 1;
  options.supercollider_delay_var_ssrc = 300;
...
TEST(ConSan, DirectProbeCanStrideCandidateSelection) {
...
TEST(ConSan, DirectProbeRuntimeAddressSelectionKeepsAllSitesPatchable) {
  constexpr uint32_t kSecondSiteWord = 350;
  std::vector<uint32_t> text_words(800, build_s_nop(0, ROCJITSU_CODE_ARCH_RDNA4));
  text_words[0] = 0xD8340000u;
  text_words[1] = 0x00000000u; // ds_store_b32 v0, v0
  ...
  TestOptions options = test_options();
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(16);
  options.max_patches = 16;
  options.runtime_sample_stride = 4;
  options.runtime_sample_offset = 1;
  ...
  ASSERT_EQ(mappings.size(), 2u);
  std::ranges::sort(mappings, {}, &StaticAccessMapping::first_slot);
  EXPECT_EQ(mappings[0].first_slot, 0u);
  EXPECT_EQ(mappings[0].bank_count, 8u);
  EXPECT_EQ(mappings[1].first_slot, 8u);
  EXPECT_EQ(mappings[1].bank_count, 8u);
  ASSERT_TRUE(test_exec_save_sgpr(result));
  EXPECT_TRUE(test_dispatch_id_sgpr(result).has_value());
  EXPECT_EQ(std::ranges::count(result.patches, PatchKind::KernelEntryOwnerEpochPrologue,
                               &PatchInfo::kind),
            1u);
  ...
  const uint16_t save_sgpr = static_cast<uint16_t>(*test_exec_save_sgpr(result) + 2u);
  ...
  const uint16_t scc_save_sgpr = static_cast<uint16_t>(*test_exec_save_sgpr(result) + 6u);
  const auto save_scc =
  ...
  const uint16_t publication_exec_save_sgpr =
      static_cast<uint16_t>(*test_exec_save_sgpr(result) + 4u);
  ...
  for (const PatchInfo &patch : std::span<const PatchInfo>(result.patches).first(2u)) {
    ASSERT_TRUE(patch.scratch_vgpr);
    ...
    const auto select_cell =
        build_v_lshrrev_b32_e32(low_vgpr, scalar_positive_inline_u32(shadow_cell::granule_shift),
...
TEST(ConSan, IndependentCellSelectionPreservesLegacyCodeAndBankGeometry) {
  constexpr auto arch = ROCJITSU_CODE_ARCH_RDNA4;
  std::vector<uint32_t> text_words(800, build_s_nop(0, arch));
  text_words[0] = 0xD8340000u;
  text_words[1] = 0x00000000u; // ds_store_b32 v0, v0
  text_words.back() = build_s_endpgm(arch);
  const auto bytes = make_rdna4_lds_code_object(text_words, "independent",
  ...
  for (uint32_t stride : {1u, 256u}) {
    options.runtime_sample_stride = stride;
    options.cell_selection.reset();
    const auto legacy = test_lower_consan(bytes, options);
    ASSERT_TRUE(patch_succeeded(legacy)) << testing::PrintToString(legacy.errors);
    options.cell_selection = SampleSelector{stride, 0};
    const auto explicit_axes = test_lower_consan(bytes, options);
    ASSERT_TRUE(patch_succeeded(explicit_axes)) << testing::PrintToString(explicit_axes.errors);
  ...
  for (uint32_t workgroup_stride : {1u, 256u}) {
    for (uint32_t cell_stride : {1u, 16u}) {
      for (uint32_t banks : {1u, 2u, 4u, 8u}) {
        ...
        options.runtime_sample_stride = workgroup_stride;
        options.cell_selection = SampleSelector{cell_stride, cell_stride - 1};
        options.watchpoint_banks = banks;
        const auto result = test_lower_consan(bytes, options);
        ASSERT_TRUE(patch_succeeded(result)) << testing::PrintToString(result.errors);
        ASSERT_TRUE(result.modified());
        const auto mappings = static_access_mappings(result);
        ...
        const auto access = std::ranges::find(result.patches, PatchKind::TrampolineWatchpointStore,
                                              &PatchInfo::kind);
        ASSERT_NE(access, result.patches.end());
        ASSERT_TRUE(access->scratch_vgpr);
...
TEST(ConSan, Rdna3HighTransientStateUsesFixedSgprPool) {
  ...
  // Keep s0:s97 live across every access. The only legal automatic ConSan
  ...
  std::vector<uint8_t> bytes = make_rdna3_lds_code_object(
      text_words, "rdna3_high_transient_state", kRdna4Wave64AllVgprsGranulated, /*wave32=*/true);

  TestOptions options = test_options();
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(80u);
  options.runtime_sample_stride = 2u;
  options.track_barriers = false;
  options.track_atomics = false;
  ...
  ASSERT_TRUE(test_exec_save_sgpr(result));
  EXPECT_EQ(*test_exec_save_sgpr(result), 98u);
  AmdGpuCodeObject patched(result.replacement.data(), result.replacement.size());
  ASSERT_TRUE(patched.is_valid());
  ASSERT_EQ(patched.kernels().size(), 1u);
  const uint64_t descriptor_offset = patched.kernels().front().descriptor_file_offset;
...
TEST(ConSan, Gfx1250RuntimeSamplingUsesLiteralDispatchIdAtFullScalarPressure) {
  ...
  TestOptions options = test_options();
  options.report_buffer_address = 0x123456780000ull;
  options.report_generation = 7u;
  options.report_dispatch_id = 0x1122334455667788ull;
  options.report_buffer_size = direct_report_bytes(16u);
  options.runtime_sample_stride = 4u;
  ...
  const auto patch = std::ranges::find_if(result.patches, [](const PatchInfo &candidate) {
    return candidate.kind == PatchKind::InlineWatchpointStore ||
           candidate.kind == PatchKind::TrampolineWatchpointStore;
  });
  ASSERT_NE(patch, result.patches.end());
  ASSERT_TRUE(patch->scratch_vgpr);
  AmdGpuCodeObject patched(result.replacement.data(), result.replacement.size());
  ASSERT_TRUE(patched.is_valid());
  const uint64_t patch_offset = patch->kind == PatchKind::InlineWatchpointStore
                                    ? patch->anchor_offset
                                    : patch->trampoline_offset;
  const uint32_t patch_size = patch->kind == PatchKind::InlineWatchpointStore
                                  ? patch->original_size
                                  : patch->trampoline_size;
  ...
  for (const bool high_word : {false, true}) {
    const uint32_t offset =
        offsetof(CausalWindow, dispatch_id) + (high_word ? sizeof(uint32_t) : 0u);
    const uint32_t literal = high_word ? static_cast<uint32_t>(options.report_dispatch_id >> 32u)
                                       : static_cast<uint32_t>(options.report_dispatch_id);
    EXPECT_TRUE(contains_subsequence(
...
TEST(ConSan, CdnaRuntimeSamplingUsesLiteralDispatchIdAtFullScalarPressure) {
  for (const Target &target : kCdnaTargets) {
    SCOPED_TRACE(target.label);
    ...
    std::vector<uint8_t> bytes =
        target.arch == ROCJITSU_CODE_ARCH_CDNA3
            ? make_cdna3_lds_code_object(text_words, "literal_dispatch_full_sgpr")
            : make_cdna4_lds_code_object(text_words, "literal_dispatch_full_sgpr");
    mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
      AMDHSA_BITS_SET(descriptor.compute_pgm_rsrc1,
                      kd::COMPUTE_PGM_RSRC1_GRANULATED_WAVEFRONT_SGPR_COUNT, 13u);
    });
...
TEST(ConSan, DirectProbeCanCheckPriorSlotInKernel) {
  ...
  const auto overwrite_guest_exec_snapshot =
      build_s_mov_b64(*test_exec_save_sgpr(result), kAmdGpuVccLo, ROCJITSU_CODE_ARCH_RDNA4);
  ASSERT_TRUE(overwrite_guest_exec_snapshot);
  EXPECT_EQ(std::ranges::count(patched_words, *overwrite_guest_exec_snapshot), 0u);
}

TEST(ConSan, Gfx1250FastGatesKeepThousandPackedSitesReachable) {
  ...
  options.set_owner_epoch_vgprs(11, 12);
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(kSiteCount);
  options.max_patches = kSiteCount;
  options.runtime_sample_stride = 16384;
  options.runtime_sample_offset = 0;

  const TransformArtifacts result =
      test_lower_consan(make_gfx1250_code_object(text_words, "gfx1250_thousand_sites"), options);
...
TEST(ConSan, DirectProbeCanCheckCorrespondingPriorBank) {
  constexpr uint32_t kSecondSiteWord = 500;
  std::vector<uint32_t> text_words(1100, build_s_nop(0, ROCJITSU_CODE_ARCH_RDNA4));
  text_words[0] = 0xD8340000u;
  text_words[1] = 0x00000000u; // ds_store_b32 v0, v0
  ...
  ASSERT_TRUE(detail::append_indexed_address(
      prior_bank_address,
      {.table_address = *options.report_buffer_address + layout.watchpoints_offset,
...
TEST(ConSan, DirectProbeChecksEveryPriorMultiAddressRange) {
  constexpr uint32_t kSecondSiteWord = 500;
  ...
  ASSERT_EQ(mappings.size(), 2u);
  std::ranges::sort(mappings, {}, &StaticAccessMapping::first_slot);
  EXPECT_EQ(mappings[0].range_count, 2u);
  EXPECT_EQ(mappings[1].range_count, 2u);
  EXPECT_EQ(mappings[0].first_slot, 0u);
  EXPECT_EQ(mappings[1].first_slot, 16u);
...
TEST(ConSan, MultiRangeReportSlotsDoNotConsumePatchBudgetAcrossTargets) {
  ...
  for (rj_code_arch_t arch : targets) {
    ...
    switch (arch) {
    case ROCJITSU_CODE_ARCH_CDNA3:
      bytes = make_cdna3_lds_code_object(words, "multirange_budget_cdna3");
      break;
    case ROCJITSU_CODE_ARCH_CDNA4:
      bytes = make_cdna4_lds_code_object(words, "multirange_budget_cdna4");
      break;
    case ROCJITSU_CODE_ARCH_CDNA5:
      bytes = make_gfx1250_code_object(words, "multirange_budget_cdna5");
      break;
    case ROCJITSU_CODE_ARCH_RDNA3:
      bytes = make_rdna3_lds_code_object(words, "multirange_budget_rdna3");
      break;
    case ROCJITSU_CODE_ARCH_RDNA4:
      bytes = make_rdna4_lds_code_object(words, "multirange_budget_rdna4");
      break;
    default:
      FAIL() << "unexpected target";
    }
    ...
    ASSERT_TRUE(patch_succeeded(result))
        << testing::PrintToString(result.errors) << testing::PrintToString(result.warnings);
    auto mappings = static_access_mappings(result);
    ASSERT_EQ(mappings.size(), 2u) << testing::PrintToString(result.warnings);
    std::ranges::sort(mappings, {}, &StaticAccessMapping::first_slot);
    EXPECT_EQ(mappings[0].range_count, 2u);
    EXPECT_EQ(mappings[1].range_count, 2u);
    EXPECT_EQ(mappings[0].bank_count, 8u);
...
TEST(ConSan, DirectProbeWarnsWhenReportCapacityLimitsPatches) {
  std::vector<uint32_t> text_words(800, build_s_nop(0, ROCJITSU_CODE_ARCH_RDNA4));
  text_words[0] = 0xD8340000u;
  text_words[1] = 0x00000000u; // ds_store_b32 v0, v0
...
TEST(ConSan, RuntimeGateUsesWholeTextRelocation) {
  std::vector<uint32_t> text_words;
  ...
  options.report_buffer_size = direct_report_bytes(18);
  options.runtime_sample_stride = 16384;
  options.track_barriers = false;
  options.track_atomics = false;
  ...
  EXPECT_EQ(mappings.front().bank_count, 2u);
  for (const PatchInfo &patch : result.patches) {
    if (patch.kind != PatchKind::TrampolineWatchpointStore)
      continue;
    ASSERT_TRUE(patch.relocated_guest_instruction_offset);
  ...
  EXPECT_NE(
      std::ranges::find(patched_words, build_v_mov_b32_e32(bank_vgpr, *options.dispatch_sgpr.base(),
                                                           ROCJITSU_CODE_ARCH_RDNA4)),
      patched_words.end());
  const auto prologue =
      std::ranges::find(result.patches, PatchKind::KernelEntryOwnerEpochPrologue, &PatchInfo::kind);
  ASSERT_NE(prologue, result.patches.end());
  ASSERT_TRUE(prologue->vgpr_state);
  ASSERT_TRUE(prologue->vgpr_state->exact_workgroup.complete());
  const auto dependency_wait =
      instrumentation::build_valu_to_salu_dependency_wait(ROCJITSU_CODE_ARCH_RDNA4);
  ASSERT_TRUE(dependency_wait);
  for (uint16_t coordinate :
       {*prologue->vgpr_state->exact_workgroup.x(), *prologue->vgpr_state->exact_workgroup.y(),
        *prologue->vgpr_state->exact_workgroup.z()}) {
...
TEST(ConSan, SyncMetadataAbiHasStablePackedLayout) {
  static_assert(sizeof(SyncMetadataPacked) == 24);
  static_assert(alignof(SyncMetadataPacked) == 8);
  EXPECT_EQ(offsetof(SyncMetadataPacked, address), 0u);
  EXPECT_EQ(offsetof(SyncMetadataPacked, byte_count), 8u);
  EXPECT_EQ(offsetof(SyncMetadataPacked, descriptor), 12u);
  EXPECT_EQ(offsetof(SyncMetadataPacked, epoch_before), 16u);
  EXPECT_EQ(offsetof(SyncMetadataPacked, epoch_after), 20u);
  EXPECT_EQ(sync_abi::version, 1u);
  EXPECT_EQ(sync_abi::reserved_mask, 0xff000000u);
}

TEST(ConSan, SyncMetadataRoundTripsTypedAtomicRolesAndOutcomes) {
  constexpr std::array cases = {
      std::pair{SyncRole::Release, SyncOutcome::NotApplicable},
      std::pair{SyncRole::Acquire, SyncOutcome::NotApplicable},
      std::pair{SyncRole::Rmw, SyncOutcome::RmwNoReturn},
      std::pair{SyncRole::RmwRelease, SyncOutcome::RmwReturnsOld},
      std::pair{SyncRole::RmwAcquire, SyncOutcome::CasFailure},
      std::pair{SyncRole::RmwAcquireRelease, SyncOutcome::CasSuccess},
  };
  for (const auto &[role, outcome] : cases) {
    ...
    EXPECT_EQ(encoded.classification, SyncClassification::Valid);
    EXPECT_EQ(encoded.packed.address, metadata.address);
    EXPECT_EQ(encoded.packed.byte_count, metadata.byte_count);
    const SyncDecodeResult decoded = decode_sync_metadata(encoded.packed);
    EXPECT_EQ(decoded.classification, SyncClassification::Valid);
    EXPECT_EQ(decoded.metadata, metadata);
  }
}

TEST(ConSan, SyncMetadataRoundTripsBarrierEpochTransition) {
  const SyncMetadata metadata{
      .kind = SyncMetadataKind::Barrier,
      .role = SyncRole::AcquireRelease,
...
TEST(ConSan, SyncMetadataRejectsVersionKindRoleScopeAndOutcome) {
  const SyncMetadata valid{
      .address = 0x1000,
      .byte_count = 4,
      .kind = SyncMetadataKind::Atomic,
  ...
  const auto classify = [](SyncMetadata metadata) {
    const SyncEncodeResult encoded = encode_sync_metadata(metadata);
    EXPECT_EQ(encoded.packed, SyncMetadataPacked{});
    return encoded.classification;
  };
  auto candidate = valid;
  candidate.version = 2;
  EXPECT_EQ(classify(candidate), SyncClassification::UnsupportedVersion);
  candidate = valid;
  candidate.kind = static_cast<SyncMetadataKind>(15);
  EXPECT_EQ(classify(candidate), SyncClassification::UnsupportedKind);
  candidate = valid;
  candidate.role = static_cast<SyncRole>(15);
  EXPECT_EQ(classify(candidate), SyncClassification::UnsupportedRole);
  candidate = valid;
  candidate.scope = static_cast<SyncScope>(15);
  EXPECT_EQ(classify(candidate), SyncClassification::UnsupportedScope);
  candidate = valid;
  candidate.outcome = static_cast<SyncOutcome>(15);
  EXPECT_EQ(classify(candidate), SyncClassification::UnsupportedOutcome);
}

TEST(ConSan, SyncMetadataRejectsInvalidAndOverflowingRanges) {
  ...
  metadata.address = 0;
  EXPECT_EQ(classify_sync_metadata(metadata), SyncClassification::InvalidRange);
  metadata.address = 0x1000;
  metadata.byte_count = 0;
  EXPECT_EQ(classify_sync_metadata(metadata), SyncClassification::InvalidRange);
  metadata.address = std::numeric_limits<uint64_t>::max() - 2u;
  metadata.byte_count = 4;
  EXPECT_EQ(classify_sync_metadata(metadata), SyncClassification::RangeOverflow);
}

TEST(ConSan, SyncMetadataRejectsUnsupportedAtomicAndBarrierSequences) {
  SyncMetadata atomic{
      .address = 0x1000,
      .byte_count = 4,
      .kind = SyncMetadataKind::Atomic,
  ...
  atomic.outcome = SyncOutcome::NotApplicable;
  EXPECT_EQ(classify_sync_metadata(atomic), SyncClassification::UnsupportedSequence);
  atomic.role = SyncRole::Acquire;
  atomic.outcome = SyncOutcome::CasSuccess;
  EXPECT_EQ(classify_sync_metadata(atomic), SyncClassification::UnsupportedSequence);
  atomic.role = SyncRole::AcquireRelease;
  atomic.outcome = SyncOutcome::NotApplicable;
  EXPECT_EQ(classify_sync_metadata(atomic), SyncClassification::UnsupportedSequence);
  atomic.role = SyncRole::Acquire;
  ...
  EXPECT_EQ(classify_sync_metadata(barrier), SyncClassification::InvalidRange);
  barrier.address = 0;
  barrier.byte_count = 0;
  barrier.role = SyncRole::Release;
  EXPECT_EQ(classify_sync_metadata(barrier), SyncClassification::UnsupportedSequence);
  barrier.role = SyncRole::AcquireRelease;
  barrier.scope = SyncScope::Agent;
  EXPECT_EQ(classify_sync_metadata(barrier), SyncClassification::UnsupportedSequence);
  barrier.scope = SyncScope::Workgroup;
  barrier.epoch_after = 12;
  EXPECT_EQ(classify_sync_metadata(barrier), SyncClassification::UnsupportedSequence);
  barrier.epoch_before = std::numeric_limits<uint32_t>::max();
  barrier.epoch_after = 0;
  EXPECT_EQ(classify_sync_metadata(barrier), SyncClassification::EpochOverflow);
}

TEST(ConSan, SyncMetadataDecodeFailsClosedOnEmptyAndMalformedWords) {
  EXPECT_EQ(decode_sync_metadata({}).classification, SyncClassification::Empty);
  SyncMetadataPacked publishing{};
  publishing.descriptor = kSyncPublishingDescriptor;
  EXPECT_EQ(decode_sync_metadata(publishing).classification, SyncClassification::Publishing);
  SyncMetadataPacked malformed{};
  malformed.descriptor = sync_abi::reserved_mask;
  EXPECT_EQ(decode_sync_metadata(malformed).classification, SyncClassification::Malformed);
  malformed = {};
  malformed.descriptor = 2u << sync_abi::version_shift;
  EXPECT_EQ(decode_sync_metadata(malformed).classification, SyncClassification::UnsupportedVersion);
  malformed = {};
  malformed.descriptor = sync_abi::version | (15u << sync_abi::kind_shift);
  EXPECT_EQ(decode_sync_metadata(malformed).classification, SyncClassification::UnsupportedKind);
}

TEST(ConSan, PendingAcquireRequiresStableExactIdentityAndPastEpoch) {
  ...
  const auto classify = [&](const PendingAcquireSlot &value, uint32_t before = 2,
                            uint32_t after = 2, uint32_t window_epoch = 10) {
    return classify_pending_acquire({before, value, after}, key, window_epoch);
  };
  EXPECT_EQ(classify(slot), PendingAcquireState::Ready);
  EXPECT_EQ(classify(slot, 2, 4), PendingAcquireState::ChangedDuringRead);
  EXPECT_EQ(classify(slot, 1, 1), PendingAcquireState::Publishing);
  EXPECT_EQ(classify(slot, 2, 2, 8), PendingAcquireState::FutureEpoch);

  const auto reject_identity = [&](auto mutate) {
    auto changed = slot;
    mutate(changed);
    EXPECT_EQ(classify(changed), PendingAcquireState::IdentityMismatch);
  };
  ...
  EXPECT_EQ(classify(malformed), PendingAcquireState::Ready);
  malformed = slot;
  malformed.metadata.descriptor = sync_abi::reserved_mask;
  EXPECT_EQ(classify(malformed), PendingAcquireState::Malformed);
  malformed = slot;
  malformed.metadata = encode_sync_metadata({
                                                .address = 0x123456780000ull,
                                                .byte_count = 4,
                                                .kind = SyncMetadataKind::Atomic,
                                                .role = SyncRole::RmwRelease,
...
TEST(ConSan, DeferredAcquireJoinsOnlyItsExactWaveWindow) {
  ...
  PendingAcquireView pending{2, slot, 2};
  ...
  const auto watchpoint = [&](uint32_t wave_owner) {
    return pack_watchpoint_entry(ShadowAccessKind::Read, wave_owner, window_epoch,
                                 static_cast<uint32_t>(generation), 12, 1);
  };

  const auto joined = join_pending_acquire(pending, window, watchpoint(owner), slot_index);
  ASSERT_EQ(joined.state, State::Ready);
  ASSERT_EQ(joined.sync.classification, SyncClassification::Valid);
  ...
  EXPECT_EQ(join_pending_acquire(pending, window, watchpoint(owner + 1u), slot_index).state,
            State::IdentityMismatch);
  pending.slot.reserved = 1;
  EXPECT_EQ(join_pending_acquire(pending, window, watchpoint(owner), slot_index).state,
            State::Ready);
...
TEST(ConSan, DeferredAcquireReleaseRoutesBothExactAccessWindows) {
  using State = PendingAcquireState;
  constexpr uint64_t generation = 17;
  constexpr uint64_t dispatch = 0x1122334455667788ull;
  ...
  const PendingAcquireView pending{2, slot, 2};
  const CausalWindow release_window{
      .generation = generation,
      .dispatch_id = dispatch,
      .workgroup_x = 7,
      .workgroup_y = 8,
  ...
  const uint64_t write = pack_watchpoint_entry(ShadowAccessKind::Write, owner, epoch,
                                               static_cast<uint32_t>(generation), 12, 1);

  const auto joined =
      join_pending_release(pending, release_window, write, release_slot, acquire_slot);
  ASSERT_EQ(joined.state, State::Ready);
  EXPECT_EQ(joined.sync.classification, SyncClassification::Valid);
  EXPECT_EQ(joined.sync.metadata.role, SyncRole::RmwAcquireRelease);
  EXPECT_EQ(joined.sync.metadata.epoch_before, epoch);
  EXPECT_EQ(joined.sync.metadata.epoch_after, epoch);
  EXPECT_EQ(
      join_pending_release(pending, release_window, write, release_slot + 1u, acquire_slot).state,
      State::IdentityMismatch);
  EXPECT_EQ(
      join_pending_release(pending, release_window, write, release_slot, acquire_slot + 1u).state,
      State::IdentityMismatch);
}

TEST(ConSan, SyncSnapshotRejectsConcurrentHybridAndWrongPairedEpoch) {
  const auto encoded = encode_sync_metadata({
      .address = 0x123456780000ull,
      .byte_count = 4,
      .kind = SyncMetadataKind::Atomic,
  ...
  ASSERT_EQ(encoded.classification, SyncClassification::Valid);
  EXPECT_EQ(
      classify_sync_snapshot({0u, encoded.packed, encoded.packed.descriptor}, 7).classification,
      SyncClassification::ChangedDuringRead);
  auto publishing = encoded.packed;
  publishing.descriptor = kSyncPublishingDescriptor;
  EXPECT_EQ(
      classify_sync_snapshot({kSyncPublishingDescriptor, publishing, kSyncPublishingDescriptor}, 7)
          .classification,
      SyncClassification::Publishing);
  EXPECT_EQ(classify_sync_snapshot(
                {encoded.packed.descriptor, encoded.packed, encoded.packed.descriptor}, 8)
                .classification,
            SyncClassification::UnsupportedSequence);
  EXPECT_EQ(classify_sync_snapshot(
                {encoded.packed.descriptor, encoded.packed, encoded.packed.descriptor}, 7)
                .classification,
            SyncClassification::Valid);
}

TEST(ConSan, SyncMetadataPublicationIsBoundedAndCollisionSafe) {
  const SyncMetadata first{
      .address = 0x1000,
      .byte_count = 4,
      .kind = SyncMetadataKind::Atomic,
  ...
  std::array<SyncMetadataPacked, 2> slots{};

  auto result = publish_sync_metadata(slots, 0, first);
  EXPECT_EQ(result.outcome, SyncPublishOutcome::Published);
  EXPECT_EQ(result.classification, SyncClassification::Valid);
  const auto retained = slots[0];
  result = publish_sync_metadata(slots, 0, first);
  EXPECT_EQ(result.outcome, SyncPublishOutcome::Existing);
  result = publish_sync_metadata(slots, 0, second);
  EXPECT_EQ(result.outcome, SyncPublishOutcome::Collision);
  EXPECT_EQ(slots[0], retained);
  EXPECT_EQ(slots[1], SyncMetadataPacked{});

  result = publish_sync_metadata(slots, 2, second);
  EXPECT_EQ(result.outcome, SyncPublishOutcome::CapacityExhausted);
  result = publish_sync_metadata({}, 0, second);
  EXPECT_EQ(result.outcome, SyncPublishOutcome::CapacityExhausted);

  slots[1].descriptor = sync_abi::reserved_mask;
  result = publish_sync_metadata(slots, 1, second);
  EXPECT_EQ(result.outcome, SyncPublishOutcome::MalformedSlot);
  EXPECT_EQ(result.classification, SyncClassification::Malformed);
  EXPECT_EQ(slots[1].descriptor, sync_abi::reserved_mask);

  auto invalid = second;
  invalid.byte_count = 0;
  result = publish_sync_metadata(slots, 1, invalid);
  EXPECT_EQ(result.outcome, SyncPublishOutcome::Rejected);
  EXPECT_EQ(result.classification, SyncClassification::InvalidRange);
  EXPECT_EQ(slots[1].descriptor, sync_abi::reserved_mask);
}

TEST(ConSan, BarrierQualificationAcceptsCompleteStaticOwnedSequence) {
  ...
  EXPECT_TRUE(qualifies_barrier_sequence(sequence, true));

  const auto rejects = [&](auto mutate) {
    SyncSequence candidate = sequence;
    mutate(candidate);
    EXPECT_FALSE(qualifies_barrier_sequence(candidate, true));
  };
  rejects([](auto &item) { item.operation = SyncOperation::BarrierSignal; });
  rejects([](auto &item) { item.operation = SyncOperation::BarrierWait; });
  rejects([](auto &item) { item.barrier_operand_source = BarrierSite::OperandSource::DynamicM0; });
  rejects([](auto &item) { item.barrier_id.reset(); });
  rejects([](auto &item) { item.barrier_scope = BarrierSite::Scope::Unknown; });
  rejects([](auto &item) { item.confidence = SemanticConfidence::Ambiguous; });
  ...
  EXPECT_FALSE(qualifies_barrier_sequence(sequence, false));
  rejects([](auto &item) { item.member_event_ids.emplace_back(); });
}

TEST(ConSan, BarrierSnapshotMustStartAtPairedSelectedEpoch) {
  const auto encoded = encode_sync_metadata({
      .kind = SyncMetadataKind::Barrier,
      .role = SyncRole::AcquireRelease,
      .scope = SyncScope::Workgroup,
...
TEST(ConSan, ClusterBarrierMetadataRoundTrips) {
  const auto encoded = encode_sync_metadata({
      .kind = SyncMetadataKind::Barrier,
      .role = SyncRole::AcquireRelease,
  ...
  ASSERT_EQ(encoded.classification, SyncClassification::Valid);
  const auto decoded = decode_sync_metadata(encoded.packed);
  EXPECT_EQ(decoded.classification, SyncClassification::Valid);
  EXPECT_EQ(decoded.metadata.scope, SyncScope::Cluster);
}

TEST(ConSan, QualifiedBarrierPublishesSelectedEpochTransition) {
  ...
  words.back() = build_s_endpgm(ROCJITSU_CODE_ARCH_RDNA4);
  const std::vector<uint8_t> bytes = make_rdna4_lds_code_object(words, "barrier");
  TestOptions options = test_options();
  options.track_barriers = true;
  options.scratch_vgpr = 8;
  options.exec_save_sgpr = 80;
  ...
  const CommittedLowering *barrier_commit =
      committed_lowering_for_intent_kind(result, ProbeIntentKind::BarrierEpoch);
  ASSERT_NE(barrier_commit, nullptr);
  EXPECT_EQ(barrier_commit->intent_ids.size(), 1u);
  EXPECT_EQ(committed_semantic_sites(result, *barrier_commit).size(), 2u);
  ...
  EXPECT_TRUE(std::ranges::all_of(
      result.observation_plan().barrier_site_decisions, [&](const BarrierSiteDecision &decision) {
        return decision.kind == SiteDecisionKind::Admitted &&
               decision.reason == BarrierPolicyReason::None &&
               decision_has_lowering(result, decision, LoweringOutcomeKind::Instrumented);
      }));
  ...
  const auto advance =
      build_v_add_nc_u32_e32(options.owner_epoch_vgprs->epoch, scalar_positive_inline_u32(1),
                             options.owner_epoch_vgprs->epoch, ROCJITSU_CODE_ARCH_RDNA4);
  ASSERT_TRUE(advance);
  EXPECT_NE(std::ranges::find(trampoline, *advance), trampoline.end());
  const auto descriptor = encode_sync_metadata({
      .kind = SyncMetadataKind::Barrier,
      .role = SyncRole::AcquireRelease,
      .scope = SyncScope::Workgroup,
      .epoch_before = 0,
  ...
  ASSERT_EQ(descriptor.classification, SyncClassification::Valid);
  ...
  expect_barrier_exact_existing_metadata_path(result, *patch, ROCJITSU_CODE_ARCH_RDNA4);

  const auto select_owner_zero = build_v_cmp_eq_u32_e32_vcc(
      scalar_positive_inline_u32(0), options.owner_epoch_vgprs->owner, ROCJITSU_CODE_ARCH_RDNA4);
  const auto narrow_owner_wave =
      build_s_and_saveexec_b64(*options.exec_save_sgpr, kAmdGpuVccLo, ROCJITSU_CODE_ARCH_RDNA4);
...
TEST(ConSan, Gfx1201BarrierSuspendsDistantGuestExpertScheduling) {
  ...
  const TransformArtifacts result =
      test_lower_consan(make_rdna4_lds_code_object(words, "expert_scheduled_barrier"), options);

  ASSERT_TRUE(patch_succeeded(result)) << testing::PrintToString(result.errors);
  const auto patch = std::ranges::find_if(result.patches, [&](const PatchInfo &candidate) {
    return candidate.kind == PatchKind::TrampolineSyncMetadata &&
...
TEST(ConSan, Gfx1201AccessSuspendsDistantGuestExpertScheduling) {
  ...
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(2);
  options.runtime_sample_stride = 2;

  const TransformArtifacts result = test_lower_consan(
      make_rdna4_lds_code_object(words, "expert_scheduled_access", kRdna4Wave64AllVgprsGranulated),
      options);

  ASSERT_TRUE(patch_succeeded(result)) << testing::PrintToString(result.errors);
  const auto patch = std::ranges::find_if(result.patches, [&](const PatchInfo &candidate) {
    return candidate.kind == PatchKind::TrampolineWatchpointStore &&
...
TEST(ConSan, StraightLineSeparatedBarriersPublishTwoMemberSequences) {
  std::vector<uint32_t> words(540, build_s_nop(0, ROCJITSU_CODE_ARCH_RDNA4));
  words[0] = 0xD8340000u;
  words[1] = 0x00000000u; // ds_store_b32 v0, v0
  ...
  const TransformArtifacts result =
      test_lower_consan(make_rdna4_lds_code_object(words, "separated_barrier"), options);
  ...
  ASSERT_NE(patch, result.patches.end()) << testing::PrintToString(result.warnings);
  EXPECT_EQ(patch->anchor_offset, kWait * sizeof(uint32_t));
  EXPECT_EQ(committed_semantic_site_count_for_source_offset(result, ProbeIntentKind::BarrierEpoch,
                                                            patch->anchor_offset),
            2u);
  EXPECT_EQ(std::ranges::count(result.patches, PatchKind::TrampolineSyncMetadata, &PatchInfo::kind),
            1u);
  ...
  const TransformArtifacts farther =
      test_lower_consan(make_rdna4_lds_code_object(beyond, "overlong_barrier"), options);
  ASSERT_TRUE(patch_succeeded(farther));
  const auto farther_patch =
      std::ranges::find(farther.patches, PatchKind::TrampolineSyncMetadata, &PatchInfo::kind);
  ...
  EXPECT_EQ(committed_semantic_site_count_for_source_offset(farther, ProbeIntentKind::BarrierEpoch,
                                                            farther_patch->anchor_offset),
            2u);
}

TEST(ConSan, IncompleteAndDynamicBarriersCannotAdvanceEpoch) {
  for (const std::vector<uint32_t> &barrier_words :
    ...
    ASSERT_TRUE(patch_succeeded(result));
    EXPECT_EQ(
        std::ranges::count(result.patches, PatchKind::TrampolineSyncMetadata, &PatchInfo::kind), 0u)
        << testing::PrintToString(result.warnings);
    ...
    EXPECT_TRUE(std::ranges::all_of(
        result.observation_plan().barrier_site_decisions, [](const BarrierSiteDecision &decision) {
          return decision.kind == SiteDecisionKind::Unsupported &&
                 decision.reason == BarrierPolicyReason::UnqualifiedSyncSequence;
        }));
  }
}

TEST(ConSan, BarrierBeforeFirstSelectedWindowAdvancesEpoch) {
  std::vector<uint32_t> words(540, build_s_nop(0, ROCJITSU_CODE_ARCH_RDNA4));
  ...
  const TransformArtifacts result =
      test_lower_consan(make_rdna4_lds_code_object(words, "barrier_before_window"), options);
...
TEST(ConSan, RejectedBarriersStillPreserveAccessOwnerAtEntry) {
  std::vector<uint32_t> words(540, build_s_nop(0, ROCJITSU_CODE_ARCH_RDNA4));
  words[0] = 0xD8340000u;
  words[1] = 0x00000000u;   // ds_store_b32 v0, v0
...
TEST(ConSan, QualifiedBarrierPersistsOwnerAcrossAccessAndSync) {
  std::vector<uint32_t> words(540, build_s_nop(0, ROCJITSU_CODE_ARCH_RDNA4));
  words[0] = 0xD8340000u;
  words[1] = 0x00000000u;   // ds_store_b32 v0, v0
  ...
  const TransformArtifacts result =
      test_lower_consan(make_rdna4_lds_code_object(words, "barrier_persistent_owner"), options);

  ASSERT_TRUE(patch_succeeded(result));
  EXPECT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
  ASSERT_TRUE(test_owner_vgpr(result));
  ASSERT_TRUE(test_epoch_vgpr(result));
  EXPECT_NE(
      std::ranges::find(result.patches, PatchKind::KernelEntryOwnerEpochPrologue, &PatchInfo::kind),
      result.patches.end());
  EXPECT_NE(std::ranges::find(result.patches, PatchKind::TrampolineSyncMetadata, &PatchInfo::kind),
  ...
  const auto access_owner_mix = instrumentation::build_v_xor_b32(
      ...
      vector_source_vgpr(*test_owner_vgpr(result)),
...
TEST(ConSan, FarBarrierUsesWholeTextRelocation) {
  ...
  ASSERT_TRUE(patch_succeeded(result)) << testing::PrintToString(result.errors);
  EXPECT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
  const auto sync_patch = std::ranges::find_if(result.patches, [&](const PatchInfo &patch) {
    return patch.kind == PatchKind::TrampolineSyncMetadata &&
...
TEST(ConSan, Cdna4AtomicAndBarrierShareWholeTextTransaction) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  const auto release = cdna4::build_mubuf(cdna4::kBufferWbl2Mubuf, {.sc1 = 1});
  const auto acquire = cdna4::build_mubuf(cdna4::kBufferInvMubuf, {.sc1 = 1});
  const auto atomic = build_cdna4_flat_atomic_add_u32(
  ...
  words.push_back(build_s_endpgm(kArch));
  const std::vector<uint8_t> bytes =
      make_cdna4_lds_code_object(words, "atomic_reserved_barrier_island");
  ...
  options.set_owner_epoch_vgprs(40u, 41u);
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(64u);
  options.runtime_sample_stride = 64u;
  options.max_patches = 64u;
  ...
  ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
  EXPECT_EQ(result.outcome, TransformOutcome::ModifiedValid);
  const auto atomic_sync = std::ranges::find_if(result.patches, [&](const PatchInfo &patch) {
    return patch.kind == PatchKind::TrampolineSyncMetadata && patch.anchor_offset == atomic_offset;
  });
  const auto barrier_sync = std::ranges::find_if(result.patches, [&](const PatchInfo &patch) {
    return patch.kind == PatchKind::TrampolineSyncMetadata && patch.anchor_offset == barrier_offset;
  });
...
TEST(ConSan, Cdna4OrdinaryAtomicNeedsNoSpillBootstrapRoute) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  const auto release = cdna4::build_mubuf(cdna4::kBufferWbl2Mubuf, {.sc1 = 1});
  const auto wait = build_cdna4_s_wait_flat0(kArch);
  const auto atomic = build_cdna4_flat_atomic_add_u32(
  ...
  constexpr uint32_t kCdna4Wave64AllVgprsGranulated = 31u;
  std::vector<uint8_t> bytes =
      make_cdna4_lds_code_object(words, "ordinary_atomic_live_bootstrap",
...
TEST(ConSan, WholeTextRelocationPreservesReconvergenceEntry) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  ...
  // eight-word window that ConSan would use as an at-anchor indirect entry.
  ...
  const std::vector<uint8_t> bytes = make_cdna4_lds_code_object(words, "at_anchor_reconvergence");
  ...
  options.set_owner_epoch_vgprs(40u, 41u);
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(9u);
  options.track_barriers = false;
  options.track_atomics = false;
  options.max_patches = 9u;
...
TEST(ConSan, Cdna4WholeTextRelocationKeepsFarOrderedEdgeComplete) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  constexpr uint32_t kAccessCount = 9u;
  const auto release = cdna4::build_mubuf(cdna4::kBufferWbl2Mubuf, {.sc1 = 1});
  const auto acquire = cdna4::build_mubuf(cdna4::kBufferInvMubuf, {.sc1 = 1});
  ...
  options.set_owner_epoch_vgprs(40u, 41u);
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(4u * kAccessCount);
  options.runtime_sample_stride = 64u;
  options.track_barriers = false;
  options.track_atomics = true;
  ...
  const TransformArtifacts result =
      test_lower_consan(make_cdna4_lds_code_object(words, "dense_far_atomic"), options);
...
TEST(ConSan, Rdna4DenseAccessesUseWholeTextRelocation) {
  constexpr uint32_t kAccessCount = 9u;
  std::vector<uint32_t> text_words(
  ...
  const TransformArtifacts result =
      test_lower_consan(make_rdna4_lds_code_object(text_words, "rdna4_dense"), options);
...
TEST(ConSan, Cdna4DenseAccessesUseWholeTextRelocation) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  constexpr uint32_t kAccessCount = 9u;
  ...
  const TransformArtifacts result =
      test_lower_consan(make_cdna4_lds_code_object(text_words, "cdna4_dense"), options);
...
TEST(ConSan, Cdna4DenseRelocationPreservesEveryAccess) {
  ...
  const TransformArtifacts result =
      test_lower_consan(make_cdna4_lds_code_object(text_words, "cdna4_dense_live_scc"), options);
...
TEST(ConSan, Cdna4LargeDenseDispatchReservesFarTargetRoutes) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  ...
  const TransformArtifacts result =
      test_lower_consan(make_cdna4_lds_code_object(text_words, "cdna4_large_dense"), options);
...
TEST(ConSan, Cdna4WholeTextRelocationPreservesLaterGroup) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  ...
  const TransformArtifacts result =
      test_lower_consan(make_cdna4_lds_code_object(text_words, "cdna4_cross_group_host"), options);
...
TEST(ConSan, Cdna4SpillBackedRelocationPreservesEveryAccess) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  constexpr uint32_t kAccessCount = 9u;
  const std::array<uint16_t, 4> dead_router_sgprs = {0u, 1u, 4u, 6u};
  std::vector<uint32_t> text_words(8u, build_s_mov_b32(/*sdst=*/0u, /*ssrc0=*/0u, kArch));
  ...
  text_words.push_back(build_s_endpgm(kArch));
  constexpr uint32_t kCdna4Wave64AllVgprsGranulated = 31u;
  std::vector<uint8_t> bytes = make_cdna4_lds_code_object(text_words, "spill_backed_dense_router",
                                                          kCdna4Wave64AllVgprsGranulated,
                                                          /*uses_dynamic_stack=*/true);
  mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
    AMDHSA_BITS_SET(descriptor.compute_pgm_rsrc1,
...
TEST(ConSan, Cdna4BranchOnlyScalarSpillGuardsEmptyExecBeforePerLaneSave) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  const auto guest =
      build_cdna4_ds_store_b32(/*vaddr=*/0u, /*vdata=*/0u, /*byte_offset=*/0u, kArch);
  ASSERT_TRUE(guest);

  // Keep every allocated ordinary SGPR live across the access. ConSan must
  ...
  constexpr uint32_t kCdna4Wave64AllVgprsGranulated = 31u;
  std::vector<uint8_t> bytes = make_cdna4_lds_code_object(
      text_words, "empty_exec_branch_only_scalar_spill", kCdna4Wave64AllVgprsGranulated);
  mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
    AMDHSA_BITS_SET(descriptor.compute_pgm_rsrc1,
                    kd::COMPUTE_PGM_RSRC1_GRANULATED_WAVEFRONT_SGPR_COUNT, 13u);
  });
...
TEST(ConSan, Cdna4SpillSafeRuntimeSelectionUsesBodyGate) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  const auto guest =
      build_cdna4_ds_store_b32(/*vaddr=*/0u, /*vdata=*/0u, /*byte_offset=*/0u, kArch);
  ASSERT_TRUE(guest);
  ...
  std::vector<uint8_t> bytes = make_cdna4_lds_code_object(
      text_words, "branch_only_runtime_body_gate", /*vgpr_granulated=*/1u);
  mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
    AMDHSA_BITS_SET(descriptor.compute_pgm_rsrc1,
                    kd::COMPUTE_PGM_RSRC1_GRANULATED_WAVEFRONT_SGPR_COUNT, 13u);
  });
  ...
  ASSERT_TRUE(prologue->vgpr_state);
  EXPECT_TRUE(prologue->vgpr_state->exact_workgroup.complete());
  EXPECT_TRUE(std::ranges::any_of(result.warnings, [](const std::string &warning) {
    return warning == "ConSan runtime workgroup selection uses the spill-safe body gate";
  })) << testing::PrintToString(result.warnings);
}

TEST(ConSan, Cdna4WholeTextRelocationCoversEarliestSource) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  ...
  constexpr uint32_t kCdna4Wave64AllVgprsGranulated = 31u;
  std::vector<uint8_t> bytes = make_cdna4_lds_code_object(
      text_words, "branch_only_earliest_reservoir_frontier", kCdna4Wave64AllVgprsGranulated);
  mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
    AMDHSA_BITS_SET(descriptor.compute_pgm_rsrc1,
                    kd::COMPUTE_PGM_RSRC1_GRANULATED_WAVEFRONT_SGPR_COUNT, 13u);
  });
  ...
  ASSERT_EQ(result.outcome, TransformOutcome::ModifiedValid);
  ASSERT_EQ(test_transient_sgpr_assignments(result).size(), 1u);
  EXPECT_TRUE(test_transient_sgpr_assignments(result).front().branch_only_spill);
  EXPECT_EQ(
      std::ranges::count(result.patches, PatchKind::TrampolineWatchpointStore, &PatchInfo::kind),
      2u)
      << testing::PrintToString(result.warnings);
  for (const uint64_t anchor :
    ...
    EXPECT_TRUE(std::ranges::any_of(result.patches, [&](const PatchInfo &patch) {
      return patch.kind == PatchKind::TrampolineWatchpointStore && patch.anchor_offset == anchor &&
             patch.relocated_guest_instruction_offset.has_value();
    }));
  }
}

TEST(ConSan, Rdna3PrivateOwnerCapturePrecedesEntryScalarSpillScratchClobber) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_RDNA3;
  ...
  std::vector<uint8_t> bytes =
      make_rdna3_lds_code_object(text_words, "private_owner_before_scalar_spill",
                                 kRdna4Wave64AllVgprsGranulated, /*wave32=*/true);
  mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
    AMDHSA_BITS_SET(descriptor.compute_pgm_rsrc1,
                    kd::COMPUTE_PGM_RSRC1_GRANULATED_WAVEFRONT_SGPR_COUNT, 13u);
...
TEST(ConSan, Cdna4DenseBarrierKeepsFarSynchronizationComplete) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  ...
  options.set_owner_epoch_vgprs(40u, 41u);
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(4u * kBarrierCount);
  options.runtime_sample_stride = 64u;
  options.track_barriers = true;
  options.track_atomics = false;
  options.max_patches = 4u * kBarrierCount;
  ...
  const TransformArtifacts result =
      test_lower_consan(make_cdna4_lds_code_object(words, "dense_far_barriers"), options);
...
TEST(ConSan, Cdna4FarBarrierUsesOwnerLocalScalarRoute) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  ...
  std::vector<uint8_t> bytes = make_cdna4_lds_code_object(words, "far_owner_local_barrier");
  AmdGpuCodeObject original(bytes.data(), bytes.size());
  ASSERT_TRUE(original.is_valid());
  ASSERT_EQ(original.kernels().size(), 1u);
  ...
  const TransientSgprAssignment seed_assignment{
      ...
      .scalar_spill_setup =
          ScalarSpillSetup{
              .temporaries = ScalarSpillTemporaries{kLocalIndirectPcSgpr, kLocalIndirectSccSgpr},
          },
      .branch_only_spill = std::nullopt,
  };
  ...
  ASSERT_TRUE(patch_succeeded(result)) << testing::PrintToString(result.errors);
  ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
  const auto sync = std::ranges::find_if(result.patches, [&](const PatchInfo &patch) {
    return patch.kind == PatchKind::TrampolineSyncMetadata && patch.anchor_offset == barrier_offset;
  });
  ASSERT_NE(sync, result.patches.end()) << testing::PrintToString(result.warnings);
  ASSERT_TRUE(sync->relocated_guest_instruction_offset);
...
TEST(ConSan, Rdna4DenseAccessesRelocateInsideCalledFunction) {
  constexpr uint32_t kAccessCount = 9u;
...
TEST(ConSan, Gfx1250RelocatesAllAccessesWithTransientSgprReferences) {
  constexpr uint32_t kAccessCount = 9u;
  ...
  const TransformArtifacts result = test_lower_consan(
      make_gfx1250_code_object(words, "gfx1250_dense_liveness_fallback"), options);
...
TEST(ConSan, WholeTextTransactionPreservesFarBarrierWithManyAccesses) {
  std::vector<uint32_t> kernel_words;
  for (uint32_t i = 0; i < 9u; ++i) {
    kernel_words.push_back(0xD8340000u); // ds_store_b32 v0, v0
    kernel_words.push_back(0x00000000u);
  ...
  EXPECT_EQ(std::ranges::count(result.patches, PatchKind::TrampolineSyncMetadata, &PatchInfo::kind),
            1u);
  const BarrierSiteDecision *barrier =
      barrier_decision_at(result, barrier_offset + sizeof(uint32_t));
  ASSERT_NE(barrier, nullptr);
  EXPECT_TRUE(decision_has_lowering(result, *barrier, LoweringOutcomeKind::Instrumented));
  EXPECT_EQ(result.outcome, TransformOutcome::ModifiedValid);
}

TEST(ConSan, Rdna4DenseBarrierRelayKeepsManyFarPairsReachable) {
  ...
  const TransformArtifacts result =
      test_lower_consan(make_rdna4_lds_code_object(words, "rdna4_dense_barriers"), options);
...
TEST(ConSan, Rdna4SingleStrandedBarrierUsesWholeTextRelocation) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_RDNA4;
  ...
  options.set_owner_epoch_vgprs(40u, 41u);
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(18u);
  options.runtime_sample_stride = 2u;
  options.max_patches = 12u;

  const TransformArtifacts result = test_lower_consan(
      make_rdna4_lds_code_object(words, "rdna4_single_stranded_barrier"), options);

  ASSERT_TRUE(patch_succeeded(result)) << testing::PrintToString(result.errors);
  ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
  EXPECT_EQ(std::ranges::count(result.patches, PatchKind::TrampolineSyncMetadata, &PatchInfo::kind),
            1u);
  EXPECT_EQ(decision_lowering_count(result, result.observation_plan().barrier_site_decisions,
                                    LoweringOutcomeKind::Instrumented),
            result.observation_plan().barrier_site_decisions.size());
  EXPECT_EQ(result.outcome, TransformOutcome::ModifiedValid);
}

TEST(ConSan, Gfx1250RelocatesBarriersAcrossKernels) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA5;
  ...
  options.set_owner_epoch_vgprs(40u, 41u);
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(20u);
  options.runtime_sample_stride = 2u;
  options.max_patches = 16u;
...
TEST(ConSan, Rdna4PatchesDenseCompatibleAliasedOwnersWithFullHardwareGridIdentity) {
  ...
  options.init_owner_epoch = true;
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(2u * kSiteCount);
  options.runtime_sample_stride = 16384u;
  options.track_barriers = true;
  options.track_atomics = false;
  ...
  const auto is_access_patch = [](const PatchInfo &patch) {
    return patch.kind == PatchKind::InlineWatchpointStore ||
           patch.kind == PatchKind::TrampolineWatchpointStore;
  };
  ...
  EXPECT_EQ(std::ranges::count(result.patches, PatchKind::TrampolineSyncMetadata, &PatchInfo::kind),
            kSiteCount);
  EXPECT_EQ(access_decision_count(result, SiteDecisionKind::Admitted), kSiteCount);
  EXPECT_EQ(decision_lowering_count(result, result.observation_plan().barrier_site_decisions,
                                    LoweringOutcomeKind::Instrumented),
            2u * kSiteCount);
  EXPECT_TRUE(std::ranges::all_of(
  ...
  ASSERT_EQ(std::ranges::count(result.observation_plan().site_decisions, SiteDecisionKind::Admitted,
                               &SiteDecision::kind),
            kSiteCount);
  ...
  EXPECT_TRUE(std::ranges::all_of(result.observation_plan().barrier_site_decisions,
                                  [](const BarrierSiteDecision &decision) {
                                    return decision.kind == SiteDecisionKind::Admitted &&
                                           decision.reason == BarrierPolicyReason::None;
                                  }));
  ...
  EXPECT_EQ(std::ranges::count(result.observation_plan().probe_intents,
                               ProbeIntentKind::BarrierEpoch, &ProbeIntent::kind),
            kSiteCount);
  EXPECT_TRUE(std::ranges::all_of(
      result.observation_plan().site_decisions, [&](const SiteDecision &decision) {
        return decision.kind != SiteDecisionKind::Admitted ||
  ...
  const std::array<uint16_t, 3> persistent_grid = {
      *prologue->vgpr_state->exact_workgroup.x(),
      *prologue->vgpr_state->exact_workgroup.y(),
      *prologue->vgpr_state->exact_workgroup.z(),
  };
  const std::vector<uint32_t> prologue_words =
      text_words_at_offset(patched, prologue->trampoline_offset, prologue->trampoline_size);
  ...
  for (const PatchInfo &patch : result.patches) {
    if (!is_access_patch(patch) && patch.kind != PatchKind::TrampolineSyncMetadata)
      continue;
    EXPECT_EQ(patch.owner_descriptor_file_offsets.size(), 2u);
  }
}

TEST(ConSan, OwnerAbiFailureRemainsApplicableAndFailsLowering) {
  TwoKernelSharedFixtureOptions fixture;
  ...
  TestOptions options = test_options();
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(6u);
  options.track_barriers = true;
  options.track_atomics = false;
  ...
  EXPECT_TRUE(decision_has_lowering(result,
                                    result.observation_plan().barrier_site_decisions.front(),
                                    LoweringOutcomeKind::ResourceRejected));
  const CommittedLowering *rejection =
      committed_lowering_for_intent_kind(result, ProbeIntentKind::BarrierEpoch);
  ASSERT_NE(rejection, nullptr);
  EXPECT_EQ(rejection->outcome, LoweringOutcomeKind::ResourceRejected);
  ...
  EXPECT_TRUE(std::ranges::any_of(result.warnings, [](const std::string &warning) {
    return warning.find("barrier sync cannot lower every owner") != std::string::npos &&
           warning.find("incompatible descriptor ABI inputs") != std::string::npos;
  })) << testing::PrintToString(result.warnings);
}

TEST(ConSan, QualifiedBarrierAdmitsLongStraightLinePair) {
  ...
  const TransformArtifacts result =
      test_lower_consan(make_gfx1250_code_object(words, "long_straight_line_barrier"), options);

  ASSERT_TRUE(patch_succeeded(result)) << testing::PrintToString(result.errors);
  const auto patch = std::ranges::find_if(result.patches, [](const PatchInfo &item) {
    return item.kind == PatchKind::TrampolineSyncMetadata &&
...
TEST(ConSan, QualifiedBarrierForcedSpillPreservesCompleteVgprWindow) {
  std::vector<uint32_t> words(540, build_s_nop(0, ROCJITSU_CODE_ARCH_RDNA4));
  words[1] = 0xD8340000u;
  words[2] = 0x00000000u;
  ...
  const TransformArtifacts result =
      test_lower_consan(make_rdna4_lds_code_object(words, "barrier_spill"), options);
...
TEST(ConSan, Rdna4DynamicStackBarrierUsesSiteLocalSpillFrames) {
  std::vector<uint32_t> words(540, build_s_nop(0, ROCJITSU_CODE_ARCH_RDNA4));
  words[0] = 0xD8340000u;
  words[1] = 0x00000000u;
  words[400] = 0xBE804EC1u;
  ...
  options.max_patches = 3;
  const std::vector<uint8_t> bytes =
      make_rdna4_lds_code_object(words, "dynamic_barrier_spill", kRdna4Wave64AllVgprsGranulated,
                                 /*wave32=*/false, /*uses_dynamic_stack=*/true);
...
TEST(ConSan, Rdna4DynamicStackRejectsMissingEntryPrologueScratch) {
  std::vector<uint32_t> words(540, build_s_nop(0, ROCJITSU_CODE_ARCH_RDNA4));
  ...
  words.back() = build_s_endpgm(ROCJITSU_CODE_ARCH_RDNA4);
  const std::vector<uint8_t> bytes =
      make_rdna4_lds_code_object(words, "dynamic_no_entry_scratch", kRdna4Wave64AllVgprsGranulated,
                                 /*wave32=*/false, /*uses_dynamic_stack=*/true);
  TestOptions options = test_options();
  options.track_barriers = true;
  options.test_force_vgpr_spill = true;
...
TEST(ConSan, Gfx1250QualifiedBarrierUsesSpill) {
  std::vector<uint32_t> words(540, build_s_nop(0, ROCJITSU_CODE_ARCH_CDNA5));
  constexpr auto store = cdna5::build_vds(cdna5::kDsStoreB32Vds, {.addr = 0, .data0 = 0});
  words[0] = store[0];
  words[1] = store[1];
  ...
  const TransformArtifacts result =
      test_lower_consan(make_gfx1250_code_object(words, "gfx1250_barrier_spill"), options);
...
TEST(ConSan, Gfx1250BarrierDoesNotGateWorkgroupsForAddressSampling) {
  std::vector<uint32_t> words(540, build_s_nop(0, ROCJITSU_CODE_ARCH_CDNA5));
  constexpr auto store = cdna5::build_vds(cdna5::kDsStoreB32Vds, {.addr = 0, .data0 = 0});
  words[0] = store[0];
  words[1] = store[1];
  ...
  const TransformArtifacts result =
      test_lower_consan(make_gfx1250_code_object(words, "gfx1250_barrier_runtime_gate"), options);
  ...
  expect_barrier_exact_existing_metadata_path(result, *patch, ROCJITSU_CODE_ARCH_CDNA5);
  AmdGpuCodeObject patched(result.replacement.data(), result.replacement.size());
  ASSERT_TRUE(patched.is_valid());
  const std::vector<uint32_t> trampoline =
      text_words_at_offset(patched, patch->trampoline_offset, patch->trampoline_size);
  ...
  const auto owner_election = build_v_cmp_eq_u32_e32_vcc(
      scalar_positive_inline_u32(0), options.owner_epoch_vgprs->owner, ROCJITSU_CODE_ARCH_CDNA5);
...
TEST(ConSan, Gfx1250BarriersPartitionRelayWindowsAcrossLargeKernel) {
  ...
  options.exec_save_sgpr = 80;
  options.set_owner_epoch_vgprs(20, 21);
  options.runtime_sample_stride = 1;
  options.report_buffer_address = 0x123456780000ull;
  options.report_buffer_size = direct_report_bytes(2u * kAccessesPerWindow);
  options.max_patches = 2u * kAccessesPerWindow + 4u;

  const TransformArtifacts result =
      test_lower_consan(make_gfx1250_code_object(words, "gfx1250_partitioned_barriers"), options);
...
TEST(ConSan, Gfx1250ClusterBarrierPublishesClusterScope) {
  std::vector<uint32_t> words(540, build_s_nop(0, ROCJITSU_CODE_ARCH_CDNA5));
  constexpr auto store = cdna5::build_vds(cdna5::kDsStoreB32Vds, {.addr = 0, .data0 = 0});
  words[0] = store[0];
  words[1] = store[1];
  ...
  const TransformArtifacts result =
      test_lower_consan(make_gfx1250_code_object(words, "gfx1250_cluster_barrier"), options);
...
TEST(ConSan, Gfx1250ComposesWithAdjacentClusterBarrierDrop) {
  ...
  const std::vector<uint8_t> bytes =
      make_gfx1250_code_object(words, "gfx1250_cluster_drop_composition");
  ...
  const TransformArtifacts inventory = test_lower_consan(bytes, options);
  const auto cluster_sequence = std::ranges::find_if(
      inventory.program_inventory.sync().sync_sequences, [](const SyncSequence &sequence) {
        return sequence.operation == SyncOperation::BarrierFull &&
               sequence.barrier_scope == BarrierSite::Scope::Cluster;
      });
  ...
  for (const PatchInfo &mutation : result.patches) {
    if (mutation.phase != PatchPhase::Mutation)
      continue;
    EXPECT_TRUE(std::ranges::none_of(result.patches, [&](const PatchInfo &instrumentation) {
      return instrumentation.phase == PatchPhase::Instrumentation &&
...
TEST(ConSan, QualifiedBarrierUsesSpillBackedPersistentEpoch) {
  std::vector<uint32_t> words(540, build_s_nop(0, ROCJITSU_CODE_ARCH_RDNA4));
  words[0] = 0xD8340000u;
  words[1] = 0x00000000u;   // ds_store_b32 v0, v0
  words[400] = 0xBE804EC1u; // s_barrier_signal -1
  ...
  const TransformArtifacts result =
      test_lower_consan(make_rdna4_lds_code_object(words, "barrier_private_epoch"), options);

  ASSERT_TRUE(patch_succeeded(result));
  EXPECT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
  EXPECT_FALSE(test_owner_vgpr(result));
  EXPECT_FALSE(test_epoch_vgpr(result));
  const auto access = std::ranges::find_if(result.patches, [](const PatchInfo &item) {
    return item.kind == PatchKind::TrampolineWatchpointStore ||
           item.kind == PatchKind::InlineWatchpointStore;
  });
  const auto barrier =
      std::ranges::find(result.patches, PatchKind::TrampolineSyncMetadata, &PatchInfo::kind);
  const auto prologue = std::ranges::find(
...
TEST(ConSan, ConditionallyExecutedBarrierPublishesOnlyWhenExecuted) {
  std::vector<uint32_t> words(540, build_s_nop(0, ROCJITSU_CODE_ARCH_RDNA4));
  words[0] = 0xD8340000u;
  words[1] = 0x00000000u;
...
TEST(ConSan, BarrierInConditionallyExitingLoopPublishesEveryIteration) {
  std::vector<uint32_t> words(540, build_s_nop(0, ROCJITSU_CODE_ARCH_RDNA4));
  words[0] = 0xD8340000u;
  words[1] = 0x00000000u;
  words[400] = 0xBE804EC1u;
...
TEST(ConSan, WatchpointRoundTripsRangeFields) {
  constexpr uint64_t packed = pack_watchpoint_entry(ShadowAccessKind::Read,
                                                    /*owner_id=*/0x512,
                                                    /*epoch=*/0x4aa,
                                                    /*generation=*/0x1abcde,
                                                    /*start_byte=*/0x4567,
                                                    /*byte_count=*/32,
                                                    /*consumed=*/true);
  constexpr WatchpointEntry decoded = decode_watchpoint_entry(packed);
...
TEST(ConSan, AtomicPublicationHasNoStableHybridSnapshot) {
  constexpr uint64_t old_entry =
      pack_watchpoint_entry(ShadowAccessKind::Write, /*owner_id=*/1, /*epoch=*/2,
                            /*generation=*/6, /*start_cell=*/10, /*cell_count=*/1);
  constexpr uint64_t new_entry =
      pack_watchpoint_entry(ShadowAccessKind::Read, /*owner_id=*/7, /*epoch=*/9,
    ...
    for (size_t high_stage = low_before_stage; high_stage < high_by_stage.size(); ++high_stage) {
      for (size_t low_after_stage = high_stage; low_after_stage < low_by_stage.size();
        ...
        const Snapshot snapshot =
            classify_snapshot({low_by_stage[low_before_stage], high_by_stage[high_stage],
                               low_by_stage[low_after_stage]},
                              /*active_generation=*/7);
        if (snapshot.state == SnapshotState::Stable) {
          ...
          EXPECT_EQ(snapshot.state, SnapshotState::StaleGeneration);
        } else {
          EXPECT_EQ(snapshot.state, SnapshotState::ChangedDuringRead);
        }
      }
    }
  }
...
TEST(ConSan, AtomicWatchpointIsStableAndConflictsWithOrdinaryAccess) {
  constexpr uint64_t atomic =
      pack_watchpoint_entry(ShadowAccessKind::Atomic, /*owner_id=*/3, /*epoch=*/2,
  ...
  constexpr Snapshot snapshot =
      classify_snapshot({atomic_low, static_cast<uint32_t>(atomic >> 32u), atomic_low},
                        /*active_generation=*/7);
  static_assert(snapshot.state == SnapshotState::Stable);
  static_assert(snapshot.entry.kind == ShadowAccessKind::Atomic);

  constexpr uint64_t ordinary =
      pack_watchpoint_entry(ShadowAccessKind::Read, /*owner_id=*/4, /*epoch=*/2,
                            /*generation=*/7, /*start_cell=*/10, /*cell_count=*/1);
  constexpr WatchpointEntry ordinary_entry = decode_watchpoint_entry(ordinary);
  EXPECT_TRUE(watchpoints_conflict(snapshot.entry, ordinary_entry));

  constexpr uint64_t other_atomic =
      pack_watchpoint_entry(ShadowAccessKind::Atomic, /*owner_id=*/4, /*epoch=*/2,
                            /*generation=*/7, /*start_cell=*/10, /*cell_count=*/1);
  constexpr WatchpointEntry other_atomic_entry = decode_watchpoint_entry(other_atomic);
  EXPECT_FALSE(watchpoints_conflict(snapshot.entry, other_atomic_entry));
}

TEST(ConSan, ClassifiesEverySnapshotPublicationState) {
  constexpr uint64_t stale = pack_watchpoint_entry(ShadowAccessKind::Write, 1, 2, 6, 10, 1);
  constexpr uint64_t incomplete =
      pack_watchpoint_entry(ShadowAccessKind::Write, 2, 2, 7, 10, 1) & ~watchpoint::valid_mask;
  constexpr uint64_t malformed =
      pack_watchpoint_entry(static_cast<ShadowAccessKind>(7), 3, 2, 7, 10, 1);
  constexpr uint64_t stable = pack_watchpoint_entry(ShadowAccessKind::Read, 4, 2, 7, 20, 1);
  const auto words = [](uint64_t packed) {
    const uint32_t low = static_cast<uint32_t>(packed);
    return SnapshotWords{low, static_cast<uint32_t>(packed >> 32u), low};
  };
  std::array<SnapshotWords, 6> snapshots = {words(0),
                                            words(stale),
                                            words(incomplete),
  ...
  const std::array expected = {
      SnapshotState::Empty,
      SnapshotState::StaleGeneration,
      SnapshotState::IncompletePublication,
      SnapshotState::ChangedDuringRead,
      SnapshotState::Malformed,
      SnapshotState::Stable,
  };
  for (size_t i = 0; i < snapshots.size(); ++i) {
    SCOPED_TRACE(i);
    EXPECT_EQ(classify_snapshot(snapshots[i], 7u).state, expected[i]);
  }
}

TEST(ConSan, PackedWatchpointRetainsConflictingOwnersAndKinds) {
  std::array<uint64_t, 2> sampled = {
      pack_watchpoint_entry(ShadowAccessKind::Write,
                            /*owner_id=*/1, /*epoch=*/3, /*generation=*/7,
                            /*start_cell=*/2, /*cell_count=*/1),
      pack_watchpoint_entry(ShadowAccessKind::Read,
                            /*owner_id=*/2, /*epoch=*/3, /*generation=*/7,
                            /*start_cell=*/2, /*cell_count=*/1),
  };
  const auto first = decode_watchpoint_entry(sampled[0]);
  const auto second = decode_watchpoint_entry(sampled[1]);
  ASSERT_TRUE(first.valid);
  ASSERT_TRUE(second.valid);
  EXPECT_TRUE(watchpoints_conflict(first, second));
  EXPECT_EQ(first.owner_id, 1u);
  EXPECT_EQ(second.owner_id, 2u);
  EXPECT_EQ(first.kind, ShadowAccessKind::Write);
  EXPECT_EQ(second.kind, ShadowAccessKind::Read);
  EXPECT_EQ(first.generation, 7u);
  EXPECT_EQ(second.epoch, 3u);
}

TEST(ConSan, PackedWatchpointsRequireOverlappingBytes) {
  std::array<uint64_t, 2> sampled = {
      pack_watchpoint_entry(ShadowAccessKind::Write,
                            /*owner_id=*/1, /*epoch=*/3, /*generation=*/7,
                            /*start_cell=*/2, /*cell_count=*/1),
  ...
  ASSERT_TRUE(first.valid);
  ASSERT_TRUE(second.valid);
  EXPECT_FALSE(watchpoints_conflict(first, second));
}

TEST(ConSan, PackedWatchpointsRejectStaleGenerations) {
  std::array<uint64_t, 2> sampled = {
      pack_watchpoint_entry(ShadowAccessKind::Write,
                            /*owner_id=*/1, /*epoch=*/3, /*generation=*/7,
                            /*start_cell=*/2, /*cell_count=*/1),
  ...
  for (uint64_t packed : sampled) {
    const uint32_t low = static_cast<uint32_t>(packed);
    EXPECT_EQ(classify_snapshot({low, static_cast<uint32_t>(packed >> 32u), low}, 8u).state,
              SnapshotState::StaleGeneration);
  }
}

TEST(ConSan, WatchpointConflictRequiresExactRangeOverlap) {
  constexpr WatchpointEntry current{
      /*valid=*/true,
      /*consumed=*/false, ShadowAccessKind::Write,
      /*owner_id=*/3,
      /*epoch=*/11,
      /*generation=*/42,
      /*start_byte=*/20,
  ...
  constexpr WatchpointEntry overlapping_read{
      /*valid=*/true,
      /*consumed=*/false, ShadowAccessKind::Read,
      /*owner_id=*/1,
      /*epoch=*/11,
      /*generation=*/42,
      /*start_byte=*/21,
  ...
  constexpr WatchpointEntry adjacent_read{
      /*valid=*/true,
      /*consumed=*/false, ShadowAccessKind::Read,
      /*owner_id=*/1,
      /*epoch=*/11,
  ...
  constexpr WatchpointEntry consumed_read{
      /*valid=*/true,
      /*consumed=*/true, ShadowAccessKind::Read,
      /*owner_id=*/1,
      /*epoch=*/11,
      /*generation=*/42,
      /*start_byte=*/21,
  ...
  constexpr WatchpointEntry old_generation{
      /*valid=*/true,
      /*consumed=*/false, ShadowAccessKind::Read,
      /*owner_id=*/1,
      /*epoch=*/11,
  ...
  EXPECT_TRUE(watchpoints_conflict(current, overlapping_read));
  EXPECT_FALSE(watchpoints_conflict(current, adjacent_read));
  EXPECT_FALSE(watchpoints_conflict(current, consumed_read));
  EXPECT_FALSE(watchpoints_conflict(current, old_generation));
}
