...
TEST(ConSanMoi, FirstLightProbeWritesOneLikelyGroupFlatAccessRecord) {
  ...
  EXPECT_EQ(rewritten_words[rewritten_words.size() - 3u], 0xEC05007Fu);
...
TEST(ConSanMoi, Gfx1250IsolatedLdsReleaseIsNotApplicableWithAccessReplay) {
  ...
  EXPECT_TRUE(std::ranges::any_of(result.site_dispositions, [](const auto &disposition) {
    return disposition.site_kind == ConSanResourceSiteKind::Atomic &&
           ...
           disposition.reason == ConSanSiteDispositionReason::NoAtomicAcquireConsumer;
  }));
}
