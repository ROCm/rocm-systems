...
TEST(LegacyOptionsAdapterTest, ProjectsRequestAndDerivesInlineModeWithoutMutatingInput) {
...
TEST(LegacyOptionsAdapterTest, ProjectsTransformDebugAndRuntimeCapabilityFields) {
  TransformPolicy transform;
...
TEST(LegacyOptionsAdapterTest, ProjectsEveryMutationAndBoundResourceFamily) {
  ...
  EXPECT_TRUE(options.fault_allow_completing_conditional_barrier_move);
  EXPECT_TRUE(options.fault_allow_destructive_divergent_barrier_move);
  EXPECT_TRUE(options.fault_mutate_barrier_id_scope);
  EXPECT_TRUE(options.fault_mutate_barrier_participants);
  EXPECT_TRUE(options.fault_atomic_wrong_address);
  EXPECT_TRUE(options.fault_atomic_weaken_order);
  EXPECT_EQ(options.fault_atomic_order_edge, ConSanAtomicOrderEdge::Acquire);
  EXPECT_TRUE(options.fault_atomic_weaken_scope);
  EXPECT_TRUE(options.fault_lds_wrong_address);
  EXPECT_TRUE(options.fault_ordinary_wrong_address);
  EXPECT_TRUE(options.fault_ordinary_weaken_order);
  EXPECT_TRUE(options.fault_ordinary_weaken_scope);
  EXPECT_EQ(options.fault_atomic_address_delta, 8u);
  EXPECT_EQ(options.fault_lds_address_vgpr, 9);
  EXPECT_EQ(options.fault_ordinary_address_delta, 12u);
  EXPECT_TRUE(options.fault_dry_run);
  EXPECT_TRUE(options.fault_require_exactly_one);
  EXPECT_EQ(options.fault_barrier_index, 1u);
  EXPECT_EQ(options.fault_atomic_index, 2u);
  EXPECT_EQ(options.fault_lds_index, 3u);
  EXPECT_EQ(options.fault_ordinary_index, 4u);
  EXPECT_EQ(options.fault_site_identity, "site");
  EXPECT_EQ(options.fault_barrier_destination_identity, "destination");
  EXPECT_EQ(options.fault_barrier_sequence_identity, "sequence");
  EXPECT_EQ(options.fault_barrier_companion_site_identity, "companion-site");
  EXPECT_EQ(options.fault_barrier_companion_sequence_identity, "companion-sequence");
  EXPECT_EQ(options.fault_barrier_target_id, 5);
  EXPECT_EQ(options.fault_barrier_target_participant_count, 6u);
  EXPECT_EQ(options.fault_barrier_target_participant_mask, 7u);
  EXPECT_EQ(options.fault_barrier_move_direction, ConSanBarrierMoveDirection::Later);
  EXPECT_EQ(options.sc_perturb_kind, ConSanPerturbationKind::Atomic);
  EXPECT_EQ(options.sc_perturb_edge, ConSanPerturbationEdge::Acquire);
  EXPECT_EQ(options.sc_perturb_identity, "perturb");
  EXPECT_EQ(options.sc_perturb_index, 8u);
  EXPECT_EQ(options.sc_perturb_max, 2u);
  EXPECT_EQ(options.sc_perturb_sleep, 9u);
  EXPECT_EQ(options.sc_perturb_required_count, 1u);
