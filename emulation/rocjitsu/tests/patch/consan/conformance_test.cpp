...
Options conformance_options(uint32_t access_count) {
  Options options = test_options();
  options.scratch_vgpr = 8;
  options.requested_exec_save_sgpr = 80u;
  options.requested_owner_vgpr = 40;
  options.requested_epoch_vgpr = 41;
  options.report_buffer_address = 0x100000000ull;
  options.report_buffer_size = direct_report_bytes(access_count);
...
TEST(ConSanConformance, InstrumentsManyLargeAccessBodies) {
  ...
  const auto result =
      test_lower_consan(make_rdna4_lds_code_object(text_words), conformance_options(kAccessCount));
  ...
  for (const CommittedLowering &commit : result.coverage_ledger.lowering_commits()) {
    EXPECT_EQ(commit.outcome, LoweringOutcomeKind::Instrumented);
    EXPECT_EQ(commit.intent_ids.size(), 1u);
    EXPECT_EQ(commit.locations.size(), 1u);
  }
}

TEST(ConSanConformance, InstrumentsGfx1100NativeLdsAccess) {
  ...
  for (bool wave32 : std::array{false, true}) {
    ...
    const TransformArtifacts result =
        test_lower_consan(make_rdna3_lds_code_object(text_words, "gfx1100_native_lds",
                          ...
                          conformance_options(/*access_count=*/1u));
...
TEST(ConSanConformance, InstrumentsGfx1100SingletonWorkgroupBarrier) {
  ...
  for (bool wave32 : std::array{false, true}) {
    ...
    EXPECT_EQ(result.program_inventory.target(), ROCJITSU_CODE_TARGET_GFX1100);
    EXPECT_EQ(result.program_inventory.arch(), ROCJITSU_CODE_ARCH_RDNA3);
    EXPECT_EQ(std::ranges::count(result.patches, expected_kind, &PatchInfo::kind), 1u)
        << testing::PrintToString(result.warnings) << testing::PrintToString(result.patches);
  }
}

TEST(ConSanConformance, MaterializesRdna4Cdna5ScalarVectorGroupFlatAddress) {
  for (const rj_code_arch_t arch : {ROCJITSU_CODE_ARCH_RDNA4, ROCJITSU_CODE_ARCH_CDNA5}) {
    ...
    const TransformArtifacts result =
        test_lower_consan(bytes, conformance_options(/*access_count=*/1u));
...
TEST(ConSanConformance, RelocatesStraightLinePrefixWhenNoEntryIslandIsReachable) {
  ...
  const uint32_t displaced_scalar_count = 7u - 3;
  ...
  const auto result = test_lower_consan(bytes, conformance_options(1));
...
TEST(ConSanConformance, InstrumentsEveryAdjacentAccessInsideRelocationRange) {
  const std::array<uint32_t, 2> kernel_words = {
      pack_sopk(/*s_call_b64=*/0x14, /*sdst=*/30, /*simm16=*/1),
      build_s_endpgm(ROCJITSU_CODE_ARCH_RDNA4),
  };
  ...
  const auto result = test_lower_consan(bytes, conformance_options(2));
...
TEST(ConSanConformance, Gfx1250RelocatesSparseAccessesAcrossLargeText) {
  constexpr uint32_t kAccessCount = 9;
  ...
  Options options = conformance_options(kAccessCount);
