...
TEST(ConSanMoi, Gfx1250SampledSpillsExecVccStateWithSeparateDeadDenseRouter) {
  ...
  for (const bool uses_dynamic_stack : {false, true}) {
    ...
    const auto restore_return_scc = instrumentation::build_s_cmp_lg_u32(
        *assignment.indirect_scc_sgpr, scalar_positive_inline_u32(0), ROCJITSU_CODE_ARCH_CDNA5);
...
TEST(ConSanMoi, Gfx1250SampledSpillBackedDenseDispatcherMatchesCapturedCallKey) {
  ...
  ASSERT_TRUE(assignment.indirect_pc_sgpr);
  ASSERT_TRUE(assignment.router_call);
  ASSERT_EQ(assignment.router_call->call_return_sgpr, *assignment.indirect_pc_sgpr);
  const auto dense_host = std::ranges::find_if(result.patches, [](const ConSanPatchInfo &patch) {
    return patch.kind == ConSanPatchKind::TrampolineMoiIndirectBranchIsland &&
           patch.original_size != 0u;
  });
  ...
  const auto captured_key_compare = instrumentation::build_s_cmp_eq_u32(
      assignment.router_call->dispatch_key_sgpr, *assignment.indirect_pc_sgpr, kArch);
