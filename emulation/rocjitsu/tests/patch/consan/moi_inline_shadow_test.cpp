...
TEST(ConSanMoi, InlineShadowProbePublishesNativeLdsStoreToExactShadow) {
  ...
  const uint32_t copy_dispatch_id =
      build_v_mov_b32_e32(12, *test_moi_dispatch_id_sgpr(result), ROCJITSU_CODE_ARCH_RDNA4);
...
TEST(ConSanMoi, Cdna4InlineShadowProbeEmitsNativeTransactions) {
  ...
  EXPECT_GE(result.coverage_ledger.lowering_commits().front().locations.size(), 1u);
  ASSERT_TRUE(result.patches.front().scratch_vgpr);
  EXPECT_EQ(*result.patches.front().scratch_vgpr, 8u);
  ...
  ASSERT_TRUE(patched.is_valid());
  ASSERT_EQ(patched.text_sections().size(), 1u);
  ASSERT_TRUE(result.patches.front().relocated_guest_instruction_offset);
  const std::vector<uint32_t> patched_words = text_words_at_offset(
      patched, result.patches.front().trampoline_offset, result.patches.front().trampoline_size);
...
TEST(ConSanMoi, CdnaInlineShadowUsesTrustedMetadataToMoveRoundedEmptyAccumulatorTail) {
  for (const rj_code_arch_t arch : {ROCJITSU_CODE_ARCH_CDNA3, ROCJITSU_CODE_ARCH_CDNA4}) {
    for (const uint8_t agpr_count : {0u, 1u}) {
      ...
      const std::string_view kernel_name = "rounded_accumulator_tail";
      std::vector<uint32_t> text_words(1200u, build_s_nop(0, arch));
      const auto store = arch == ROCJITSU_CODE_ARCH_CDNA3
                             ? build_cdna3_ds_store_b32(/*vaddr=*/2u, /*vdata=*/3u,
                                                        /*byte_offset=*/0u, arch)
      ...
      std::vector<uint8_t> bytes = arch == ROCJITSU_CODE_ARCH_CDNA3
                                       ? make_cdna3_lds_code_object(text_words, kernel_name,
                                                                    /*vgpr_granulated=*/1u)
                                       : make_cdna4_lds_code_object(text_words, kernel_name,
                                                                    /*vgpr_granulated=*/1u);
      mutate_first_kernel_descriptor(bytes, [](KD &descriptor) {
        // The unified allocation is v0:v15, while ACCUM_OFFSET places the
        // accumulator boundary at v12. Metadata is the only authoritative
        // distinction between an empty rounded tail and live accumulator
        // storage in v12:v15.
        AMDHSA_BITS_SET(descriptor.compute_pgm_rsrc3, kd::COMPUTE_PGM_RSRC3_GFX90A_ACCUM_OFFSET,
                        2u);
      });
      append_kernel_metadata_note(
          bytes, kernel_name, /*uses_dynamic_stack=*/false, /*sgpr_count=*/0u,
          /*private_segment_fixed_size=*/std::nullopt,
          /*required_workgroup_size=*/std::nullopt, /*has_dynamic_lds=*/true,
          /*additional_kernel_names=*/{}, agpr_count, /*vgpr_count=*/9u);
      ...
      if (agpr_count == 0u) {
        ...
        EXPECT_EQ(result.resource_plans.front().source,
                  ConSanRegisterAllocationSource::DescriptorGrowth);
        ASSERT_EQ(result.resource_plans.front().alternatives.size(), 1u);
        EXPECT_EQ(result.resource_plans.front().alternatives.front().kind,
                  ConSanResourcePlanAlternativeKind::EmptyAccumulatorDescriptorGrowth);
        EXPECT_EQ(result.resource_plans.front().alternatives.front().outcome,
                  ConSanResourcePlanAlternativeOutcome::Selected);
        ASSERT_TRUE(result.resource_plans.front().scratch_vgpr);
        EXPECT_EQ(result.resource_plans.front().scratch_vgpr_count, 16u);
        EXPECT_EQ(result.resource_plans.front().required_vgpr_count,
                  *result.resource_plans.front().scratch_vgpr +
                      result.resource_plans.front().scratch_vgpr_count);
        EXPECT_GT(result.resource_plans.front().required_vgpr_count, 12u);
        AmdGpuCodeObject patched(result.replacement.data(), result.replacement.size());
        ASSERT_TRUE(patched.is_valid());
        KD descriptor{};
        std::memcpy(&descriptor,
                    result.replacement.data() + patched.kernels().front().descriptor_file_offset,
                    sizeof(descriptor));
        const uint32_t moved_accumulator_offset = AMDHSA_BITS_GET(
...
TEST(ConSanMoi, Cdna4InlineShadowPreservesDsWorkgroupKeyFromKernelEntry) {
  ...
  EXPECT_FALSE(prologue->workgroup_shadow->lazy_initialization);
  EXPECT_EQ(prologue->workgroup_shadow->validity_size, 0u);
...
TEST(ConSanMoi, Cdna4InlineShadowAvoidsOriginalPhysicalVccPair) {
  ...
  EXPECT_EQ(*test_moi_dispatch_id_sgpr(result), 72u);
...
TEST(ConSanMoi, Cdna4PrivateEpochWholeObjectEntryDoesNotClobberEntryAbiSgprs) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  // Keep the access more than one signed SOPP branch range from the source
  // entry. The whole-object transaction must still put the private prefix in
  // the translated launch stub, where its translator-owned continuation does
  // not borrow the restored s0:s4 entry-ABI window as a PC temporary.
  ...
  EXPECT_EQ(std::ranges::find(words, build_s_getpc_b64(vcc_lo, kArch)), words.end());
  EXPECT_EQ(std::ranges::find(words, build_s_setpc_b64(vcc_lo, kArch)), words.end());
  ...
  const std::vector<uint32_t> continuation = text_words_at_offset(
      patched, prologue->trampoline_offset + prologue->trampoline_size, sizeof(uint32_t));
  ASSERT_EQ(continuation.size(), 1u);
  EXPECT_EQ(continuation.front() & 0xffff0000u, build_s_branch(/*simm16=*/0, kArch) & 0xffff0000u);
}
...
TEST(ConSanMoi, Cdna4InlineShadowUsesScalarEpochForFullOrdinaryVgprBank) {
  ...
  text_words[0] = 0xd81a0004u;
  text_words[1] = 0x00000302u; // ds_write_b32 v2, v3 offset:4
  text_words[2] = *barrier;
  size_t cursor = 3u;
  for (uint16_t vgpr = 0u; vgpr < 64u; ++vgpr)
    text_words[cursor++] = build_v_mov_b32_e32(/*vdst=*/0u, vector_source_vgpr(vgpr), kArch);
  text_words.back() = build_s_endpgm(kArch);
  std::vector<uint8_t> bytes =
      make_cdna4_lds_code_object(text_words, "scalar_epoch_full_vgpr_bank", /*vgpr_granulated=*/7u,
                                 /*uses_dynamic_stack=*/false, /*workgroup_id_dimension_mask=*/0u,
                                 /*group_segment_fixed_size=*/4u);
  mutate_kernel_descriptor(bytes, "scalar_epoch_full_vgpr_bank", [](KD &descriptor) {
  ...
  append_kernel_metadata_note(bytes, "scalar_epoch_full_vgpr_bank",
                              /*uses_dynamic_stack=*/false, /*sgpr_count=*/16u,
                              /*private_segment_fixed_size=*/std::nullopt,
  ...
  EXPECT_EQ(std::ranges::find(result.patches, ConSanPatchKind::KernelEntryMoiPrivateEpochPrologue,
                              &ConSanPatchInfo::kind),
            result.patches.end());
  AmdGpuCodeObject patched(result.replacement.data(), result.replacement.size());
  ASSERT_TRUE(patched.is_valid());
...
TEST(ConSanMoi, Cdna4InlineShadowCapturesDispatchIdPrivatelyForFullPressureOwner) {
  ...
  std::copy(release.begin(), release.end(), full_pressure_words.begin() + cursor);
  cursor += release.size();
  ...
  // dead spill-setup pair. This models the component-local assignment selected for
  ...
  options.moi_scalar_spill_setup = ConSanMoiScalarSpillSetup{
      .temporaries = ConSanMoiScalarSpillTemporaries{48u, 50u},
  };
  ...
  ASSERT_TRUE(full_assignment->scalar_spill_setup);
  ...
  ASSERT_EQ(empty_exec_continuation_word, relocated_guest_word)
      << "an empty wave must bypass every nested probe and resume at the guest instruction";
  const uint64_t empty_exec_guard_distance = relocated_guest_word - 1u;
  ...
  EXPECT_EQ(access_words.front(), *empty_exec_guard)
      << "an empty wave must skip all per-lane instrumentation before the inert guest LDS "
         "operation";
  ...
  const auto read_low = instrumentation::build_v_readfirstlane_b32(
      full_assignment->scalar_spill_setup->temporaries.frame_base_sgpr, *full_access->scratch_vgpr,
      kArch);
  const auto read_high = instrumentation::build_v_readfirstlane_b32(
      static_cast<uint16_t>(full_assignment->scalar_spill_setup->temporaries.frame_base_sgpr + 1u),
...
TEST(ConSanMoi, Cdna4InlineShadowKeepsDispatchIdInVgprsForDynamicStackOwner) {
  ...
  ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
  EXPECT_EQ(result.outcome, ConSanTransformOutcome::ModifiedValid);
  ASSERT_TRUE(test_moi_dispatch_id_vgpr(result));
  EXPECT_FALSE(test_moi_dispatch_id_sgpr(result));
...
TEST(ConSanMoi, CdnaInlineShadowClobberingLoadUsesDisjointCompactSpillWindow) {
  for (const rj_code_arch_t arch : {ROCJITSU_CODE_ARCH_CDNA3, ROCJITSU_CODE_ARCH_CDNA4}) {
    ...
    MoiOptions options = moi_options(ConSanMoiEngine::InlineShadow);
    options.test_force_vgpr_spill = true;
    options.set_moi_owner_epoch_vgprs(16u, 17u);
...
TEST(ConSanMoi, Cdna4InlineShadowRecordsEveryRejectedFallbackAttempt) {
  ...
  options.set_moi_owner_epoch_vgprs(7u, 8u);
...
TEST(ConSanMoi, Cdna4InlineShadowReloadsOverlappingDynamicStackAddress) {
  ...
  options.set_moi_owner_epoch_vgprs(24u, 25u);
  ...
  EXPECT_EQ(count_subsequence(cave_words, reload_sequence), 2u)
      << "the local cell must recover the spilled address before publication and diagnosis";
  EXPECT_EQ(result.outcome, ConSanTransformOutcome::ModifiedValid);
}
...
TEST(ConSanMoi, CdnaInlineShadowAtomicTrackingFitsSpillBackedTransactionWindow) {
  for (const rj_code_arch_t arch : {ROCJITSU_CODE_ARCH_CDNA3, ROCJITSU_CODE_ARCH_CDNA4}) {
    ...
    options.set_moi_owner_epoch_vgprs(25u, 26u);
...
TEST(ConSanMoi, CdnaInlineShadowKeepsDisjointClobberedAddressInPlace) {
  for (const rj_code_arch_t arch : {ROCJITSU_CODE_ARCH_CDNA3, ROCJITSU_CODE_ARCH_CDNA4}) {
    ...
    MoiOptions options = moi_options(ConSanMoiEngine::InlineShadow);
    options.test_force_vgpr_spill = true;
    options.set_moi_owner_epoch_vgprs(28u, 29u);
...
TEST(ConSanMoi, Rdna4AccessOnlyInlineShadowUsesInitializedWorkgroupLocalLdsMirror) {
  ...
  for (const ConSanPatchInfo *patch : {&*access, &*prologue}) {
    ASSERT_TRUE(patch->workgroup_shadow);
    EXPECT_EQ(patch->workgroup_shadow->base, 4352u);
    EXPECT_EQ(patch->workgroup_shadow->size, 8704u);
    EXPECT_EQ(patch->workgroup_shadow->validity_base, 13056u);
  ...
  ASSERT_LE(descriptor_offset + sizeof(KD), result.replacement.size());
  KD descriptor{};
  ...
  const auto save_original_exec =
      build_s_mov_b64(static_cast<uint16_t>(*test_moi_exec_save_sgpr(result) +
                                            kConSanMoiInlineOriginalExecSaveOffset),
                      /*exec=*/126u, ROCJITSU_CODE_ARCH_RDNA4);
  const auto restore_exec =
      build_s_mov_b64(/*sdst=*/126u, *test_moi_exec_save_sgpr(result), ROCJITSU_CODE_ARCH_RDNA4);
  const auto restore_bounded_exec = build_s_mov_b64(
      /*sdst=*/126u, static_cast<uint16_t>(*test_moi_exec_save_sgpr(result) + 16u),
      ROCJITSU_CODE_ARCH_RDNA4);
...
TEST(ConSanMoi, Rdna4AtomicTrackingUsesInitializedWorkgroupLocalLdsMirror) {
  ...
  EXPECT_EQ(access->workgroup_shadow->size, 8704u);
  EXPECT_EQ(access->workgroup_shadow->validity_base, 13056u);
  ...
  const auto mix_dispatch_low =
      ib::build_v_xor_b32(generation_vgpr, *test_moi_dispatch_id_sgpr(result), generation_vgpr,
                          ROCJITSU_CODE_ARCH_RDNA4);
  const auto mix_dispatch_high = ib::build_v_xor_b32(
      generation_vgpr, static_cast<uint16_t>(*test_moi_dispatch_id_sgpr(result) + 1u),
...
TEST(ConSanMoi, Rdna4InlineShadowEagerlyClearsExactMirrorWhenValidityBitmapDoesNotFit) {
  ...
  for (const ConSanPatchInfo *patch : {&*access, &*prologue}) {
    ASSERT_TRUE(patch->workgroup_shadow);
    EXPECT_EQ(patch->workgroup_shadow->base, 21840u);
    EXPECT_EQ(patch->workgroup_shadow->size, 43680u);
    EXPECT_EQ(patch->workgroup_shadow->validity_size, 0u);
...
TEST(ConSanMoi, InlineShadowFallsBackToExternalMirrorWhenLocalMirrorDoesNotFit) {
  ...
  const auto prologue = std::ranges::find(
      result.patches, ConSanPatchKind::KernelEntryMoiOwnerEpochPrologue, &ConSanPatchInfo::kind);
  ASSERT_NE(prologue, result.patches.end());
  EXPECT_FALSE(prologue->workgroup_shadow);
...
TEST(ConSanMoi, InlineShadowLoopsOverEveryWideWorkgroupLocalCellCompactly) {
  ...
  const auto masked_predicate =
      build_s_and_saveexec_b64(static_cast<uint16_t>(*test_moi_exec_save_sgpr(result) + 14u),
  ...
  ConSanTransformArtifacts broken_loop = result;
  ...
  std::memcpy(broken_loop.replacement.data() +
...
TEST(ConSanMoi, Gfx1250InlineWorkgroupShadowCachesEvidenceInFreshScalarState) {
  ...
  ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
  ASSERT_TRUE(test_moi_exec_save_sgpr(result));
  const uint16_t evidence_latch = static_cast<uint16_t>(*test_moi_exec_save_sgpr(result) + 24u);
  const auto prologue = std::ranges::find_if(result.patches, [](const ConSanPatchInfo &patch) {
    return patch.kind == ConSanPatchKind::KernelEntryMoiOwnerEpochPrologue;
  });
  ASSERT_NE(prologue, result.patches.end());
  ...
  ASSERT_TRUE(prologue->workgroup_shadow);
  EXPECT_EQ(prologue->workgroup_shadow->validity_base, 13080u);
  EXPECT_EQ(prologue->workgroup_shadow->validity_size, 288u);
  ...
  const auto advance_row = ib::build_v_add_u32(
      initializer_address, static_cast<uint16_t>(*test_moi_exec_save_sgpr(result) + 4u),
  ...
  const auto count_lanes =
      ib::build_s_bcnt1_i32_b64(static_cast<uint16_t>(*test_moi_exec_save_sgpr(result) + 4u),
...
TEST(ConSanMoi, Gfx1250InlineWorkgroupShadowValidatesAtomicAccessCandidate) {
  ...
  ASSERT_TRUE(exact_patch->workgroup_shadow);
  EXPECT_NE(exact_patch->workgroup_shadow->size, 0u);
  EXPECT_TRUE(validate_consan_modified_elf(bytes, result).empty());
}
...
TEST(ConSanMoi, Rdna4InlineGlobalShadowSpillsFullScalarPressure) {
  ...
  for (const bool uses_dynamic_stack : {false, true}) {
    ...
    EXPECT_TRUE(test_moi_dispatch_id_sgpr(result));
...
TEST(ConSanMoi, Rdna4InlineBranchOnlyDynamicStackPreservesEntryScalarInputs) {
  ...
  corrupted.replacement[patched.text_sections().front()->sectionOffset() + backup_body_offset] ^=
      1u;
  ...
  ConSanTransformArtifacts invalid_resources = result;
...
void check_inline_fixed_stack_prefers_branch_only_over_available_scalar_spill_setup(
    rj_code_arch_t arch) {
  constexpr uint16_t kInitializedEntrySgprCount = 8u;
  ...
  // leaves s48:s55 available for probe-local setup. Fixed-stack owners must
  // nevertheless choose entry-time branch-only backup because an empty EXEC
  // cannot preserve borrowed scalar values through per-lane private storage.
...
TEST(ConSanMoi, Rdna4InlineFixedStackPrefersBranchOnlyOverAvailableSpillSetup) {
  check_inline_fixed_stack_prefers_branch_only_over_available_scalar_spill_setup(
      ROCJITSU_CODE_ARCH_RDNA4);
}

TEST(ConSanMoi, Gfx1250InlineFixedStackPrefersBranchOnlyOverAvailableSpillSetup) {
  check_inline_fixed_stack_prefers_branch_only_over_available_scalar_spill_setup(
      ROCJITSU_CODE_ARCH_CDNA5);
}
...
TEST(ConSanMoi, Rdna4DynamicStackTransactionDoesNotConsumeIsolatedNopWords) {
  ...
  for (uint64_t candidate : relay_candidates) {
    EXPECT_EQ(text_words_at_offset(patched, candidate, sizeof(uint32_t)).front(),
              build_s_nop(0u, ROCJITSU_CODE_ARCH_RDNA4));
  }
}

ConSanTransformArtifacts run_rdna4_branch_only_instruction_reservoir(uint32_t filler,
...
TEST(ConSanMoi, Rdna4DynamicStackRelocatesLargeOwnerWithoutInstructionReservoirs) {
  const ConSanTransformArtifacts relocated = run_rdna4_branch_only_instruction_reservoir(
  ...
  ASSERT_TRUE(relocated.text_relocation);
  const auto access = std::ranges::find(
...
TEST(ConSanMoi, Rdna4DynamicStackDoesNotRequireAdmissibleInstructionReservoir) {
  // s_delay_alu is intentionally excluded from legacy donor discovery. The
  // whole-object transaction must remain independent of that local geometry.
  const ConSanTransformArtifacts result = run_rdna4_branch_only_instruction_reservoir(
...
TEST(ConSanMoi, Rdna4DynamicStackDoesNotConsumeSelectedAnchorTails) {
  ...
  ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
  ASSERT_EQ(result.outcome, ConSanTransformOutcome::ModifiedValid);
  const auto access_patch_at = [&](size_t word) {
    return std::ranges::find_if(result.patches, [&](const ConSanPatchInfo &patch) {
      return patch.kind == ConSanPatchKind::TrampolineMoiExactShadowStore &&
             patch.anchor_offset == word * sizeof(uint32_t);
    });
  };
  const auto early = access_patch_at(kEarlyAccessWord);
  const auto middle = access_patch_at(kMiddleAccessWord);
  const auto late = access_patch_at(kLateAccessWord);
  ...
  ASSERT_TRUE(patched.is_valid());
  EXPECT_EQ(
      text_words_at_offset(patched, (kMiddleAccessWord + 1u) * sizeof(uint32_t), sizeof(uint32_t))
          .front(),
      0x00000000u);
  EXPECT_EQ(
      text_words_at_offset(patched, (kLateAccessWord + 1u) * sizeof(uint32_t), sizeof(uint32_t))
          .front(),
      0x00000000u);
}
...
TEST(ConSanMoi, Gfx1250InlineOddShadowSlotCountClearsOnlyItsValidityState) {
  ...
  ASSERT_NE(prologue, result.patches.end());
  ASSERT_TRUE(prologue->workgroup_shadow);
  EXPECT_EQ(prologue->workgroup_shadow->size, 8u);
  EXPECT_EQ(prologue->workgroup_shadow->validity_base, 16u);
  EXPECT_EQ(prologue->workgroup_shadow->validity_size, 16u);
  EXPECT_TRUE(prologue->workgroup_shadow->lazy_initialization);
...
TEST(ConSanMoi, Gfx1250InlineLargeLocalMirrorUsesFullExactCellsAndValidityState) {
  ...
  ASSERT_TRUE(access->scratch_vgpr);
  ASSERT_TRUE(access->workgroup_shadow);
  EXPECT_EQ(access->workgroup_shadow->base, 21120u);
  EXPECT_EQ(access->workgroup_shadow->size, 42240u);
...
TEST(ConSanMoi, Gfx1250FullExactShadowValidatesAtomicTokenWithWorkgroupKey) {
  ...
  ASSERT_TRUE(test_moi_exec_save_sgpr(result));
  const uint16_t exec_base = *test_moi_exec_save_sgpr(result);
...
TEST(ConSanMoi, InlineShadowEntryPrologueRelocatesCompleteScalarClause) {
  ...
  std::vector<uint32_t> patched_words(text_word_count);
  ...
  const auto relocated = std::search(
      patched_words.begin() +
          static_cast<ptrdiff_t>(result.text_relocation->source_text_size / sizeof(uint32_t)),
      patched_words.end(), clause_run.begin(), clause_run.end());
  ASSERT_NE(relocated, patched_words.end());

  EXPECT_TRUE(std::equal(clause_run.begin(), clause_run.end(), patched_words.begin()))
      << "the whole-object transaction must preserve the source clause as inert provenance";
}
...
TEST(ConSanMoi, Cdna4InlineValidatorResolvesDerivedPrivateDispatchReloadsPerOwner) {
  ...
  ASSERT_TRUE(test_moi_dispatch_id_sgpr(result));
  ASSERT_GE(*test_moi_dispatch_id_sgpr(result), 12u);
  ConSanTransformArtifacts with_component_assignment = result;
  ...
  fixed_assignment->scalar_spill_setup.reset();
  ...
  auto &unrelated_assignments = with_unrelated_assignment.moi_operating_point.owner_transient_sgprs;
...
TEST(ConSanMoi, Cdna4InlineShadowCapturesComponentDispatchWithPersistentOwnerVgprs) {
  ...
  const auto transient =
      test_moi_transient_sgpr_assignment(result, full_pressure->descriptor_file_offset);
...
TEST(ConSanMoi, InlineShadowSpillsThroughSiteLocalDynamicStackFrame) {
  ...
  EXPECT_EQ(first_note_segment_bytes(result.replacement), original_note)
...
TEST(ConSanMoi, InlineShadowAutomaticallyAllocatesHwIdOwnerAndSpecialStateSgprs) {
  ...
  EXPECT_EQ(test_moi_dispatch_id_sgpr(result), 20);
  EXPECT_EQ(test_moi_exec_save_sgpr(result), 22);
...
TEST(ConSanMoi, InlineShadowSubwordObjectPublishesIndependentByteSlots) {
  ...
  EXPECT_TRUE(std::ranges::all_of(
      result.patches, [](const ConSanPatchInfo &patch) { return !patch.workgroup_shadow; }));
  EXPECT_TRUE(std::ranges::any_of(result.warnings, [](const std::string &warning) {
...
TEST(ConSanMoi, InlineShadowProbeCanPatchTwoAppendedCaveSites) {
  ...
  MoiOptions options = moi_options(ConSanMoiEngine::InlineShadow);
  options.scratch_vgpr = 8;
  options.set_moi_owner_epoch_vgprs(25, 26);
  options.moi_report_buffer_address = 0x100000000ull;
  options.moi_report_buffer_size = kInlineShadowFullLdsReportBufferSize;
  options.max_patches = 2;
...
TEST(ConSanMoi, Cdna4InlineShadowPackedTokenOwnerPreservesClobberedLoadAddress) {
  constexpr auto load = cdna4::build_ds(cdna4::kDsReadB32Ds, {.addr = 0u, .vdst = 0u});
  const auto release = cdna4::build_mubuf(cdna4::kBufferWbl2Mubuf, {.sc1 = 1});
  ...
  EXPECT_EQ(plan->scratch_vgpr_count, 25u)
      << "the packed token owner should avoid reserving a redundant scalar-owner copy";
  const auto patch = std::ranges::find(
      result.patches, ConSanPatchKind::TrampolineMoiExactShadowStore, &ConSanPatchInfo::kind);
  ASSERT_NE(patch, result.patches.end());
  AmdGpuCodeObject patched(result.replacement.data(), result.replacement.size());
  ...
  const std::vector<uint32_t> cave =
      text_words_at_offset(patched, patch->trampoline_offset, patch->trampoline_size);
  const uint16_t redundant_consumer_owner = static_cast<uint16_t>(*options.scratch_vgpr + 24u);
  const uint16_t saved_load_address = redundant_consumer_owner;
  ...
  EXPECT_EQ(std::ranges::find(cave, redundant_owner_materialization), cave.end());
  EXPECT_TRUE(contains_subsequence(cave, std::array<uint32_t, 1>{*index_saved_address}));
}

TEST(ConSanMoi, Gfx1250InlineShadowRelocatesLoadWithoutOwningItsConsumer) {
  ...
  const auto patch = std::ranges::find(
      result.patches, ConSanPatchKind::TrampolineMoiExactShadowStore, &ConSanPatchInfo::kind);
  ASSERT_NE(patch, result.patches.end());
  ASSERT_EQ(patch->original_size, load.size() * sizeof(uint32_t));
  ...
  EXPECT_EQ(std::find(cave.begin(), cave.end(), address_consumer), cave.end());
  EXPECT_EQ(static_cast<size_t>(load_position - cave.begin()),
...
TEST(ConSanMoi, Rdna4DenseInlineShadowAccessesRelocateWithoutKeyRelay) {
  ...
  ASSERT_TRUE(first_access->relocated_guest_instruction_offset);
  ASSERT_TRUE(result.text_relocation);
  ...
  const size_t guest_word =
      (*first_access->relocated_guest_instruction_offset - first_access->trampoline_offset) /
      sizeof(uint32_t);
  ASSERT_LE(guest_word + 2u, body_words.size());
  EXPECT_EQ(body_words[guest_word], text_words[8u]);
  EXPECT_EQ(body_words[guest_word + 1u], text_words[9u]);
}

TEST(ConSanMoi, Cdna4FarInlineShadowAccessesShareOneTextTransaction) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  constexpr uint32_t kAccessCount = 64u;
  ...
  // Put the appended island bank outside direct SOPP reach of the early
  // accesses, matching large generated CDNA kernels with no usable NOP cave.
  ...
  const ConSanTransformArtifacts result =
      test_lower_consan(make_cdna4_lds_code_object(text_words, "cdna4_far_inline_shadow"), options);
...
TEST(ConSanMoi, Cdna4DenseInlineShadowAccessDoesNotIntroduceSccMutatingRelay) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  ...
  std::vector<uint8_t> bytes =
      make_cdna4_lds_code_object(text_words, "cdna4_dense_alias_key_scc", /*vgpr_granulated=*/7u,
                                 /*uses_dynamic_stack=*/false);
  AmdGpuCodeObject original(bytes.data(), bytes.size());
  ASSERT_TRUE(original.is_valid());
  ASSERT_EQ(original.kernels().size(), 1u);
  ...
  EXPECT_TRUE(std::ranges::any_of(result.patches, [&](const ConSanPatchInfo &patch) {
    return patch.kind == ConSanPatchKind::TrampolineMoiExactShadowStore &&
           patch.anchor_offset == last_access_offset &&
           patch.relocated_guest_instruction_offset.has_value();
  }));
  EXPECT_TRUE(std::ranges::any_of(result.patches, [&](const ConSanPatchInfo &patch) {
    return patch.kind == ConSanPatchKind::TrampolineMoiInlineEpochBarrier &&
...
TEST(ConSanMoi, Cdna4DenseInlineShadowRelocatesLiveGuestSccWithoutRoute) {
  constexpr uint32_t kAccessCount = 18u;
  constexpr size_t kLargeTextWords = 33'000u;
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  constexpr uint16_t kExecSaveSgpr = 60u;
  ...
  const ConSanTransformArtifacts result = test_lower_consan(
      make_cdna4_lds_code_object(text_words, "cdna4_dense_inline_live_scc"), options);
...
TEST(ConSanMoi, Rdna4DenseCalledFunctionAccessesShareTextTransaction) {
  constexpr uint32_t kAccessCount = 9u;
...
TEST(ConSanMoi, Cdna4FarEntryPrefixAndAccessShareTextTransaction) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  constexpr size_t kLargeTextWords = 33'000u;
...
TEST(ConSanMoi, Cdna4FarEntryPrefixNeedsNoOverlappingIndirectIsland) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA4;
  constexpr size_t kLargeTextWords = 33'000u;
...
TEST(ConSanMoi, Cdna4FarInlineShadowBarrierUsesDenseRoute) {
  ...
  std::vector<uint32_t> text_words(33'000u, build_s_mov_b32(/*sdst=*/0, /*ssrc0=*/0, kArch));
  const auto barrier = build_cdna4_s_barrier(kArch);
  ASSERT_TRUE(barrier);
  text_words[kBarrierWord] = *barrier;
  text_words.back() = build_s_endpgm(kArch);
  ...
  const ConSanTransformArtifacts result = test_lower_consan(
      make_cdna4_lds_code_object(text_words, "cdna4_far_inline_barrier"), options);
...
TEST(ConSanMoi, Gfx1250DenseInlineShadowBarriersShareTextTransaction) {
  ...
  options.automatic_moi_scalar_spill_layout = ConSanMoiScalarSpillLayout::Inline;
  options.moi_scalar_spill_setup = ConSanMoiScalarSpillSetup{
      .temporaries = ConSanMoiScalarSpillTemporaries{30, 29},
...
TEST(ConSanMoi, Gfx1250FarInlineShadowBarrierNeedsNoReturnRoute) {
  constexpr rj_code_arch_t kArch = ROCJITSU_CODE_ARCH_CDNA5;
  constexpr size_t kLargeTextWords = 33'000u;
...
TEST(ConSanMoi, Gfx1250DenseInlineShadowAccessAndBarrierShareTextTransaction) {
  constexpr uint32_t kAccessCount = 9u;
  constexpr size_t kLargeTextWords = 33'000u;
  const uint32_t filler = build_s_mov_b32(100, 100, ROCJITSU_CODE_ARCH_CDNA5);
  std::vector<uint32_t> text_words(kLargeTextWords, filler);
  ...
  EXPECT_EQ(anchor.front(), *build_s_barrier_wait_all(ROCJITSU_CODE_ARCH_CDNA5));
}

TEST(ConSanMoi, Rdna4DenseInlineShadowBarriersShareTextTransaction) {
  constexpr uint32_t kAccessCount = 9u;
  constexpr size_t kLargeTextWords = 33'000u;
  const uint32_t filler = build_s_mov_b32(0, 0, ROCJITSU_CODE_ARCH_RDNA4);
  std::vector<uint32_t> text_words(kLargeTextWords, filler);
...
TEST(ConSanMoi, Gfx1250DenseBarrierNeedsNoDispatcherReservation) {
  constexpr uint32_t kAccessCount = 9u;
...
TEST(ConSanMoi, Cdna4InlinePrefersOwnerWideFreshWindowToObjectWideSiteDeadWindow) {
  ...
  ASSERT_TRUE(result.modified()) << testing::PrintToString(result.warnings);
  ASSERT_TRUE(test_moi_exec_save_sgpr(result));
  EXPECT_EQ(*test_moi_exec_save_sgpr(result), 68u) << testing::PrintToString(result.warnings);
  EXPECT_EQ(test_moi_dispatch_id_sgpr(result), 64u) << testing::PrintToString(result.warnings);
  ...
  const auto assignment = test_moi_transient_sgpr_assignment(result, high->descriptor_file_offset);
  ASSERT_TRUE(assignment) << testing::PrintToString(result.warnings);
  EXPECT_TRUE(assignment->spill_backed);
  EXPECT_FALSE(test_moi_transient_sgpr_assignment(result, low->descriptor_file_offset));
  const auto persistent_assignments = test_moi_persistent_vgpr_assignments(result);
...
TEST(ConSanMoi, Cdna4InlineUsesComponentLocalScalarSpillOutsidePreloadsAndPhysicalVcc) {
  ...
  for (uint16_t sgpr = 8u; sgpr <= 85u; ++sgpr) {
    ...
    // spill window. Fresh s86:s89 carry spill-setup and evidence state.
  ...
  ASSERT_TRUE(assignment.scalar_spill_setup->visible_evidence_sgpr);
  ...
  const std::array ranges = {
      ...
      std::pair{*assignment.scalar_spill_setup->visible_evidence_sgpr, uint16_t{1u}},
      std::pair{assignment.scalar_spill_setup->temporaries.frame_base_sgpr, uint16_t{2u}},
      std::pair{assignment.scalar_spill_setup->temporaries.scc_save_sgpr, uint16_t{1u}},
  };
...
TEST(ConSanMoi, Cdna4InlineSpillsMixedVgprSourcesThroughDynamicStackFrames) {
  ...
  ASSERT_TRUE(assignment.spill_backed);
  ASSERT_TRUE(assignment.scalar_spill_setup);
  EXPECT_FALSE(assignment.scalar_spill_setup->visible_evidence_sgpr);
  ...
  const std::vector<uint32_t> access_cave =
      text_words_at_offset(patched, access->trampoline_offset, access->trampoline_size);
  EXPECT_NE(
      std::ranges::find(access_cave,
                        build_s_mov_b32(assignment.scalar_spill_setup->temporaries.frame_base_sgpr,
...
TEST(ConSanMoi, InlineShadowPreservesTwoAddressLoadAddressAliasedBySecondResult) {
  ...
  options.set_moi_owner_epoch_vgprs(160, 161);
  options.moi_report_buffer_address = 0x100000000ull;
  options.moi_report_buffer_size = kInlineShadowFullLdsReportBufferSize;
...
TEST(ConSanMoi, InlineShadowLoadPreservesConditionStateBeforeMetadataSetup) {
  ...
  options.set_moi_owner_epoch_vgprs(41, 42);
  options.moi_exec_save_sgpr = 60;
  options.moi_report_buffer_address = 0x100000000ull;
  options.moi_report_buffer_size = kInlineShadowFullLdsReportBufferSize;
...
TEST(ConSanMoi, SharedInlineShadowUsesOnePersistentPairForEveryOwner) {
  ...
  EXPECT_EQ(std::count_if(result.patches.begin(), result.patches.end(),
            ...
            2);
  std::vector<uint64_t> access_placements;
  for (const ConSanPatchInfo &patch : result.patches) {
    if (patch.kind != ConSanPatchKind::TrampolineMoiExactShadowStore)
      continue;
    EXPECT_EQ(patch.owner_descriptor_file_offsets.size(), 2u);
    EXPECT_TRUE(patch.relocated_guest_instruction_offset);
    access_placements.push_back(patch.trampoline_offset);
  }
  ASSERT_EQ(access_placements.size(), 2u);
  EXPECT_NE(access_placements[0], access_placements[1]);
  ASSERT_TRUE(result.text_relocation);
...
TEST(ConSanMoi, FinalValidationPinsVersionedExactShadowPublication) {
  ...
  ASSERT_TRUE(test_moi_dispatch_id_sgpr(valid));
  ...
  ConSanTransformArtifacts wrong_atomic = valid;
  ...
  std::memcpy(wrong_atomic.replacement.data() + body_file_offset + cas_byte_offset,
...
TEST(ConSanMoi, InlineShadowBarrierEpochPatchTrampolinesBarrierAndSaturatesEpoch) {
  ...
  EXPECT_EQ(epoch_patch_it->trampoline_size, 4u * sizeof(uint32_t));
  ASSERT_TRUE(epoch_patch_it->relocated_guest_instruction_offset);
  EXPECT_EQ(*epoch_patch_it->relocated_guest_instruction_offset, epoch_patch_it->trampoline_offset);
  ASSERT_TRUE(result.text_relocation);
  ...
  EXPECT_EQ(rewritten_barrier, kBarrierWait)
      << "the source image remains intact; the relocated owner executes the composed body";

  std::array<uint32_t, 4> trampoline_words{};
