...
struct TranslatedPipelineContext {
  Gfx1250Sim sim;
...
TEST(GpuVmPipeline, TranslatedScalarLoadAndStoreUseExternalBacking) {
  ...
  context.sim.memory->write32(kAddress, kLegacyValue, 77);
  ...
  EXPECT_EQ(context.sim.memory->read32(kAddress, 77), kLegacyValue);
...
TEST(GpuVmPipeline, CompletionRetryOnOneQueueDoesNotStarveAnotherQueue) {
  ...
  std::array<std::byte, sizeof(rocr::llvm::amdhsa::kernel_descriptor_t) + sizeof(kEndpgm)>
      kernel_image{};
  ASSERT_EQ(sim.memory->read_block(
                kKernelAddress, std::span<uint8_t>(reinterpret_cast<uint8_t *>(kernel_image.data()),
                                                   kernel_image.size())),
...
TEST(GpuVmPipeline, UnreadyGartRemainsOnTranslatedPathUntilPublication) {
  ...
  EXPECT_TRUE(initial_info->external);
  ...
  sim.memory->write32(kAperture, kLegacyValue, 0);
  ...
  EXPECT_EQ(sim.memory->read32(kAperture, 0), kLegacyValue);
}
...
TEST(GpuVmPipeline, TranslatedTensorLoadRetriesBeforeCommittingLdsOrBarrier) {
  ...
  for (uint32_t index = 0; index < kExternal.size(); ++index) {
    ...
    context.sim.memory->write32(kGlobalAddress + index * sizeof(uint32_t), kLegacy[index], 77);
  ...
  EXPECT_EQ(context.sim.memory->read32(kGlobalAddress, 77), kLegacy[0]);
