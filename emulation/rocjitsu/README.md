...
The AMDGPU instruction files under
`lib/rocjitsu/src/rocjitsu/isa/arch/amdgpu/` are autogenerated from the MR ISA
XML specification and checked in. To regenerate with shared execute templates
across all supported ISAs, build the CMake codegen target:

```bash
cmake --build build --target rocjitsu_generate_amdgpu_isa
```

The normal rocjitsu build depends on this target before compiling the AMDGPU ISA
object libraries, and the target refreshes and clang-formats the generated files in place. By default, the target reads MR ISA XMLs from
`../../shared/machine-readable-isa/isa` relative to the rocjitsu source directory
(`/home/arosa/rocm-systems/shared/machine-readable-isa/isa` in this workspace).
Override the input directory with `-DRJ_AMDGPU_ISA_DIR=/path/to/isa` at configure time.
Configure with `-DRJ_AMDGPU_ISA_CODEGEN_FAIL_ON_CHANGES=ON` to make the codegen
target fail if regeneration changes any checked-in AMDGPU ISA generated file.

The CMake target runs the equivalent of:

```bash
PYTHONPATH=$PWD/lib/python python3 -m amdisa --gen-all --gen-shared-execute \
  -o lib/rocjitsu/src/rocjitsu/isa/arch/amdgpu \
  --multi \
    cdna1:../../shared/machine-readable-isa/isa/amdgpu_isa_cdna1.xml \
    cdna2:../../shared/machine-readable-isa/isa/amdgpu_isa_cdna2.xml \
    cdna3:../../shared/machine-readable-isa/isa/amdgpu_isa_cdna3.xml \
    cdna4:../../shared/machine-readable-isa/isa/amdgpu_isa_cdna4.xml \
    rdna1:../../shared/machine-readable-isa/isa/amdgpu_isa_rdna1.xml \
    rdna2:../../shared/machine-readable-isa/isa/amdgpu_isa_rdna2.xml \
    rdna3:../../shared/machine-readable-isa/isa/amdgpu_isa_rdna3.xml \
    rdna3_5:../../shared/machine-readable-isa/isa/amdgpu_isa_rdna3_5.xml \
    rdna4:../../shared/machine-readable-isa/isa/amdgpu_isa_rdna4.xml
```

To generate an easy-to-read Markdown report comparing generated output against a base ref:
...
The PR workflow `.github/workflows/rocjitsu-amdisa-codegen-report.yml` runs this
report when the MR ISA XMLs, amdisa generator, or checked-in generated AMDGPU ISA files change.
...
Hand-written files (`isa.h`, `mfma_exec.h`, `addr_calc.h/.cpp`) are
not overwritten by the generator.

The shared MR ISA checkout lives under `shared/machine-readable-isa/isa` in the
top-level rocm-systems repository.
