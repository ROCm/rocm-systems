...
The AMDGPU instruction files are autogenerated from the MR ISA XML specification
into the CMake build tree. To regenerate with shared execute templates across
all supported ISAs, build the CMake codegen target:
...
object libraries. Generated files are written under
`build/generated/rocjitsu/isa/arch/amdgpu/`; handwritten AMDGPU support files
remain under `lib/rocjitsu/src/rocjitsu/isa/arch/amdgpu/`. By default, the target reads MR ISA XMLs from
...
PYTHONPATH=$PWD/lib/python python3 -m amdisa --gen-all --gen-shared-execute \
  -o build/generated/rocjitsu/isa/arch/amdgpu \
  --multi \
...
report when the MR ISA XMLs or amdisa generator change.
