...
[[nodiscard]] CodeObjectInspection inspect_code_object(const AmdGpuCodeObject &obj,
  ...
  for (const auto *section : obj.code_sections()) {
    ...
    // The section filter is intentionally only a presentation filter. The
    // report still summarizes every code section so validation and debugging
    // do not silently ignore translated text.
    const bool print_section =
        include_disassembly && (section_filter.empty() || section->name() == section_filter);
    if (print_section)
...
[[nodiscard]] SelectedInput select_input(const TranslateOptions &options, std::string &error) {
  ...
  if (options.input_kind == InputKind::Auto || options.input_kind == InputKind::Executable) {
    ...
    if (options.input_kind == InputKind::Executable) {
      error = "failed to select code object from executable: " + options.input_path;
      selected.executable.reset();
      return selected;
    }
  }

  // Auto mode falls back to a standalone AMDGPU code-object ELF. This makes the
  // CLI convenient for both HIP object files and already-extracted DBT outputs.
  selected.executable.reset();
...
ToolResult<TranslateOutput> translate_code_object(const TranslateOptions &options) {
  ...
  output.value.target_mach = options.target_mach;
  ...
  if (need_report || need_source_disassembly) {
    auto source_inspection = inspect_code_object(*input.code_object, options.guest_arch,
                                                 "source", options.disassembly_section,
  ...
  if (need_report || need_translated_disassembly || options.validate_host_decode) {
    auto translated_inspection = inspect_code_object(translated_obj, options.host_arch,
                                                     "translated", options.disassembly_section,
  ...
  if (options.validate_host_decode) {
  ...
  if (!options.output_path.empty()) {
    if (!write_file(options.output_path, output.value.elf_bytes, error)) {
      add_error(output, kOutputError, error);
      return output;
    }
  }

  if (options.fail_on_warning && !output.warnings.empty()) {
