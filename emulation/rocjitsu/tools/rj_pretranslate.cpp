...
void usage(const char *program) {
  std::fprintf(stderr,
               "usage: %s [--store-root DIR] [--force] FILE...\n"
               "\n"
...
Outcome pretranslate_one(const char *path, rocjitsu::TranslationStore &store, bool force) {
  ...
  const rj_status_t status =
      rj_gfx1250_b0_to_a0_translate(source.data(), source.size(), &translated, &translated_size);
...
int main(int argc, char **argv) {
  ...
  rocjitsu::TranslationStore store(rocjitsu::kGfx1250B0A0Domain,
                                   rocjitsu::gfx1250_b0_a0_translator_anchor(), root,
