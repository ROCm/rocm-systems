...
pub async fn run_cmd(a: RunArgs) -> anyhow::Result<ExitCode> {
    ...
    warn_if_the_runtime_cannot_see_the_device(&profile, &a.envs);
...
/// Say so when the ROCm runtime this session will load has never heard
/// of the GPU the profile emulates.
///
/// The emulated device is presented correctly and then skipped:
/// `libhsa-runtime64.so` enumerates agents by looking their ISA up in
/// the table it was compiled with, and an agent whose ISA is not in that
/// table is not an agent it reports. The session still starts, the
/// emulator daemon still logs the lines it logs for a target that works,
/// the workload still runs, and it exits 0 having seen a CPU and no GPU.
/// Every visible signal is the one a successful run gives, which is why
/// this was diagnosed as an emulator bug for a day before it was
/// diagnosed as a ROCm too old (issue #11361).
///
/// A warning and not a refusal. The verdict is read out of the runtime's
/// own binary rather than asked of an API — there is no API to ask; see
/// [`mirage_core::rocr`] — so it can be wrong, and the direction it must
/// never be wrong in is blocking a run that would have worked. It stays
/// quiet unless it found a real target table without this target in it.
///
/// Not asked at all for a containerised session: the workload there
/// loads the image's ROCm, and nothing on this filesystem says anything
/// about that one. Running the unsupported target under a newer ROCm
/// image is in fact the fix the message suggests, so warning about it
/// would be advice against taking its own advice.
fn warn_if_the_runtime_cannot_see_the_device(
    profile: &mirage_core::profile::ProfileDef,
    envs: &[String],
) {
    let backend = mirage_core::emulator::get_emulator_backend(&profile.emulator.emulator);
    // Malformed `--env` is `check_run_args`'s to refuse, and it already
    // has by here; a diagnostic is not the place to report it a second
    // time, and an empty map only ever makes this quieter.
    let env = parse_envs(envs).unwrap_or_default();
    if let Some(notice) =
        unsupported_target_notice(profile, &env, backend, mirage_core::rocr::check_target)
    {
        eprintln!("mirage: {notice}");
    }
}

/// What [`warn_if_the_runtime_cannot_see_the_device`] should print, if
/// anything.
///
/// The backend and the runtime check are parameters rather than looked
/// up here, because neither is a test's to choose: which backends are
/// linked into a binary is decided at link time, and which ROCm is
/// installed is a fact about the machine. Passing them in is what lets
/// the rule — who is asked, who is exempt, and what is said — be
/// exercised on any host.
fn unsupported_target_notice(
    profile: &mirage_core::profile::ProfileDef,
    env: &std::collections::BTreeMap<String, String>,
    backend: Option<&dyn mirage_core::emulator::EmulatorBackend>,
    support: impl FnOnce(&str) -> mirage_core::rocr::TargetSupport,
) -> Option<String> {
    if profile.containerize.is_some() {
        return None;
    }
    // `--env` outranks what mirage inherited, so a run that sets one of
    // the variables the ROCm search consults is pointing the workload's
    // loader at a runtime this process is not the one to read. Saying
    // "your ROCm does not support this" about a library the workload was
    // never going to load is the one failure mode worth this much care:
    // the user has already told mirage where to look, and would be being
    // corrected with the answer to a question they did not ask.
    if mirage_core::rocr::search_vars()
        .iter()
        .any(|key| env.contains_key(key))
    {
        return None;
    }
    // Asked of the backend, because whether the profile's target is the
    // one ROCr will be shown is the backend's own business: `hotswap`
    // and `rocjitsu-dbt` retarget code onto the host's real GPU, so
    // their sessions turn on an ISA the profile does not name at all.
    // The trait's default answer is "nothing emulated to check".
    let isa = backend?.emulated_isa(profile)?;
    match support(&isa) {
        mirage_core::rocr::TargetSupport::Unsupported(problem) => Some(problem.explain()),
        _ => None,
    }
}
...
mod tests {
    ...
    /// A backend that emulates `isa`, or nothing when `isa` is `None`.
    ///
    /// Not registered: the population of backends in a binary is decided
    /// at link time, and `mirage_ctl` links none — so the only way to
    /// exercise the rule below at all is to hand the backend in, which
    /// is what [`unsupported_target_notice`] takes it as a parameter
    /// for. Everything but `emulated_isa` is the minimum the trait
    /// demands.
    #[derive(Debug)]
    struct Emulating(Option<&'static str>);

    impl mirage_core::emulator::EmulatorBackend for Emulating {
        fn description(&self) -> mirage_core::emulator::EmulatorDescription {
            mirage_core::emulator::EmulatorDescription {
                name: "emulating".to_string(),
                version: "0".to_string(),
                description: "test-only backend".to_string(),
                options_schema: Vec::new(),
            }
        }

        fn boot(&self, _def: &mirage_core::profile::ProfileDef) -> Result<(), String> {
            Ok(())
        }

        fn options(&self) -> Vec<mirage_core::config::OptionDef> {
            Vec::new()
        }

        fn shutdown(&self, _ctx: &mirage_core::session::SessionContext) {}

        fn validate_profile(&self, _def: &mirage_core::profile::ProfileDef) -> Result<(), String> {
            Ok(())
        }

        fn runtime(&self) -> mirage_core::emulator::RuntimeStatus {
            mirage_core::emulator::RuntimeStatus::new(
                true,
                mirage_core::discovery::RuntimeLocation::Unknown,
            )
        }

        fn supported(&self) -> mirage_core::emulator::SupportStatus {
            mirage_core::emulator::SupportStatus::supported("nothing required")
        }

        fn discover_plugins(&self) -> Vec<mirage_core::plugin::PluginsDef> {
            Vec::new()
        }

        fn health(
            &self,
            _ctx: &mirage_core::session::SessionContext,
        ) -> mirage_core::session::SessionHealth {
            mirage_core::session::SessionHealth::phase(
                true,
                mirage_core::session::state::READY,
                None,
            )
        }
        ...
        fn injection_def(
            &self,
            _ctx: &mirage_core::session::SessionContext,
        ) -> mirage_core::error::Result<mirage_core::exec::InjectionDef> {
            Ok(mirage_core::exec::InjectionDef::default())
        }

        fn emulated_isa(&self, _def: &mirage_core::profile::ProfileDef) -> Option<String> {
            self.0.map(str::to_string)
        }
    }

    /// A minimal profile, containerised or not.
    fn profile_for(image: Option<&str>) -> mirage_core::profile::ProfileDef {
        mirage_core::profile::ProfileDef {
            name: "p".to_string(),
            description: None,
            emulator: mirage_core::emulator::EmulatorDef {
                emulator: "emulating".to_string(),
                plugins: Default::default(),
                exec_mode: Default::default(),
                options: Default::default(),
                topology: mirage_core::common::MaybeRef::Ref("t".to_string()),
            },
            containerize: image.map(|image| mirage_core::profile::ContainerizedDef {
                provider: None,
    ...
    /// A run that redirects nothing, which is almost every run.
    fn no_env() -> std::collections::BTreeMap<String, String> {
        std::collections::BTreeMap::new()
    }

    /// A ROCm that knows nothing, which is the verdict the notice is
    /// built from.
    ...
    /// The whole point of issue #11361: a target the host's ROCm cannot
    /// enumerate used to produce a session that started, a workload that
    /// ran, an exit status of 0 and no GPU — with nothing said about any
    /// of it.
    #[test]
    fn a_target_this_rocm_cannot_see_is_reported_before_the_session_starts() {
        let notice = unsupported_target_notice(
            &profile_for(None),
            &no_env(),
            Some(&Emulating(Some("gfx1250"))),
            |isa| {
                assert_eq!(isa, "gfx1250", "the backend's target is what gets checked");
                unsupported(isa)
            },
        )
        .expect("an unsupported target must be reported");
        assert!(notice.contains("gfx1250"), "{notice}");
        assert!(notice.contains("no GPU"), "{notice}");
    }

    /// And it stays quiet about everything else.
    #[test]
    fn nothing_is_said_about_a_target_the_check_does_not_condemn() {
        for verdict in [
            mirage_core::rocr::TargetSupport::Supported,
            // The reading a heuristic gets wrong: no ROCm found, or one
            // whose target table could not be read. Silence, because a
            // warning drawn from a failed scan would send a user to fix
            // something that was never wrong.
            mirage_core::rocr::TargetSupport::Unknown,
        ] {
            assert_eq!(
                unsupported_target_notice(
                    &profile_for(None),
                    &no_env(),
                    Some(&Emulating(Some("gfx1250"))),
                    |_| verdict.clone(),
                ...
                None,
                "{verdict:?} is not something to warn about"
            );
        }
    }

    /// A backend that emulates no device of its own — `hotswap` and
    /// `rocjitsu-dbt` both run on the host's real GPU — has no target
    /// for the host's ROCm to fail to recognise, and neither has a
    /// profile naming a backend this binary was not built with.
    #[test]
    fn a_backend_that_emulates_no_device_is_not_asked() {
        ...
        assert_eq!(
            unsupported_target_notice(&profile_for(None), &no_env(), None, |_| unsupported(
                "gfx1250"
            )),
            None
        );
        assert!(
            !checked.get(),
            "the host's ROCm is nobody's business when nothing is being emulated"
        );
    }

    /// A containerised session loads the image's ROCm, not this
    /// machine's, so nothing here has anything to say about it — and
    /// running under a newer ROCm image is the very fix the warning
    /// recommends, so warning about it would be advice against taking
    /// its own advice.
    #[test]
    fn a_containerised_session_is_judged_by_its_image_and_not_by_this_host() {
        let checked = std::cell::Cell::new(false);
        let notice = unsupported_target_notice(
            &profile_for(Some("rocm/pytorch:rocm10.0")),
            &no_env(),
            Some(&Emulating(Some("gfx1250"))),
            |_| {
                checked.set(true);
        ...
        assert!(
            !checked.get(),
            "this host's ROCm must not even be looked at"
        );
    }

    /// A run that redirects the loader is asking about a ROCm this
    /// process cannot read, so it is not told about the one it can.
    ///
    /// `--env` outranks mirage's own environment, so
    /// `--env LD_LIBRARY_PATH=<a newer ROCm>/lib` is a user who has
    /// already answered this question. Warning them off a run that is
    /// about to work — with the version of a library the workload will
    /// never load — is the worst thing a best-effort check can do, and
    /// the reason it is a warning rather than a refusal.
    #[test]
    fn a_run_that_redirects_the_loader_is_not_second_guessed() {
        let vars = mirage_core::rocr::search_vars();
        assert!(
            !vars.is_empty(),
        ...
        for key in vars {
            ...
            let notice = unsupported_target_notice(
                &profile_for(None),
                &[(key.clone(), "/opt/rocm-10.0/lib".to_string())]
                    .into_iter()
                    .collect(),
                Some(&Emulating(Some("gfx1250"))),
                |_| {
            ...
            assert_eq!(notice, None, "--env {key} must silence the check");
            assert!(!checked.get(), "--env {key} redirects what would be read");
        }

        // An unrelated `--env` changes nothing about which ROCm is
        // loaded, so it must not become a way to lose the warning.
        assert!(
            unsupported_target_notice(
                &profile_for(None),
                &[("HSA_ENABLE_SDMA".to_string(), "0".to_string())]
                    .into_iter()
                    .collect(),
                Some(&Emulating(Some("gfx1250"))),
                |isa| unsupported(isa),
            ...
            .is_some(),
            "only the variables the search consults may silence it"
        );
    }
