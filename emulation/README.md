...
- [Rocjitsu](emulation/rocjitsu/) — JIT-based AMD GPU emulator. The core engine that interprets/translates AMDGPU ISA and models GPU blocks so unmodified ROCm apps can run without a physical GPU.
  ...
  - [simdojo](emulation/rocjitsu/lib/simdojo/) — Decoupled parallel discrete-event simulation engine that orchestrates composable models. See [src/](emulation/rocjitsu/lib/simdojo/src/) ([simulation.cpp](emulation/rocjitsu/lib/simdojo/src/simulation.cpp), [topology.cpp](emulation/rocjitsu/lib/simdojo/src/topology.cpp), [component.cpp](emulation/rocjitsu/lib/simdojo/src/component.cpp)).
...
- [CLI](emulation/rocjitsu/cli/) — iOS-simulator-inspired UX that drives the emulator (named profiles, shareable sessions, containers). A Rust workspace whose top-level [`rocjitsu`](emulation/rocjitsu/cli/src/) binary composes the crates below. See the [CLI](#cli) section for the architecture. A session lives in the `rocjitsu run` that owns it and goes away with it — nothing is kept on disk — while a second terminal reaches it over that run's socket with `rocjitsu exec --session <id>`.
  - [core](emulation/rocjitsu/cli/core/) (`rj_core`) — Shared library: profile/session/topology/agent stores, the control-plane wire protocol, KMD/library discovery, and container env wiring. See [discovery.rs](emulation/rocjitsu/cli/core/src/discovery.rs), [profile.rs](emulation/rocjitsu/cli/core/src/profile.rs), [proto.rs](emulation/rocjitsu/cli/core/src/proto.rs), [paths.rs](emulation/rocjitsu/cli/core/src/paths.rs), [container.rs](emulation/rocjitsu/cli/core/src/container.rs).
