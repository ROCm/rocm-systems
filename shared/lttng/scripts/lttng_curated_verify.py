...
- One argument that cannot fit into an LTTng-UST event (§4.4). High-arity
  APIs are emitted as ordered event chunks by lttng_curated_codegen.py.
...
from lttng_curated_lib import (parse_yaml_file, ParseError, BudgetError,
                                validate_api)
...
HANDLE_TYPE_PATTERNS = (
    ...
    'hipUserObject_t',
    'hsa_signal_t', 'hsa_queue_t', 'hsa_agent_t', 'hsa_cache_t',
    'hsa_region_t', 'hsa_signal_group_t', 'hsa_isa_t', 'hsa_wavefront_t',
    'hsa_code_object_t', 'hsa_callback_data_t', 'hsa_code_symbol_t',
    'hsa_code_object_reader_t', 'hsa_executable_t',
...
HANDLE_NATURAL_DEPTH = {
    ...
    'hsa_cache_t': 0, 'hsa_region_t': 0, 'hsa_signal_group_t': 0,
    'hsa_isa_t': 0, 'hsa_wavefront_t': 0, 'hsa_code_object_t': 0,
    'hsa_callback_data_t': 0, 'hsa_code_symbol_t': 0,
    'hsa_code_object_reader_t': 0, 'hsa_executable_t': 0,
    'hsa_executable_symbol_t': 0, 'hsa_amd_vmem_alloc_handle_t': 0,
    'hsa_amd_external_semaphore_t': 0, 'hsa_queue_t': 1,
}
...
class UnsupportedReturnTypeError(Exception):
    """Raised when a curated wrapper return type has no safe macro kind."""
...
def infer_dsl_type(c_type, canonical, is_enum, dir_):
    ...
    # A typedef can hide the pointer spelling (e.g. hipArray_t is spelled
    # without `*`, but canonicalizes to `hipArray *`). That is still a safe
    # address-only capture; use the same raw-pointer fallback rather than
    # treating a conclusively pointer-shaped value as ambiguous.
    if depth >= 1 or canon_depth >= 1:
        return 'ptr'
...
def expand_compact_apis(raw_apis, header_decls):
    ...
    or lttng_curated_lib.BudgetError (one arg exceeds the per-event field
    budget)."""
    out = []
    for api in raw_apis:
...
def _function_params(node, cindex):
    """Return the declaration tuple used throughout this module for `node`."""
    params = []
...
def _function_return_type(node):
    """Return the result-type tuple retained with a parsed declaration."""
    result = node.result_type
    canonical = result.get_canonical()
    return (result.spelling, canonical.spelling, result.kind.name, canonical.kind.name)


def parse_headers(header_paths, extra_args):
    """Parse one or more headers; return RAW (unresolved) declarations:
    {api_name: [(is_static, (param, ...), return_type), ...]}, where each
    param is (name, c_type, canonical_type, is_enum) and return_type is
    (c_type, canonical_type, type_kind, canonical_type_kind).
    ...
    for hp in header_paths:
        ...
        for n in tu.cursor.walk_preorder():
            # Newer libclang versions can expose cursor kinds unknown to an
            # older Python binding while parsing implementation TUs. They are
            # irrelevant here; keep scanning the remaining AST for function
            # declarations rather than failing the entire curated gate.
            try:
                kind = n.kind
            except ValueError:
                continue
            ...
            entry = (is_static, _function_params(n, cindex), _function_return_type(n))
...
def parse_function_definitions(source_paths, extra_args):
    """Parse implementation TUs and return only their own function definitions.

    Most APIs are declared by public headers. Some exported compatibility and
    compiler-support wrappers are intentionally implementation-only, though,
    so their real signatures are only available from the wrapper definition.
    Those definitions are more authoritative than a similarly-named C++
    convenience overload in a public header.
    """
    try:
        from clang import cindex
    except ImportError:
    ...
    for source_path in source_paths:
        ...
        for n in tu.cursor.walk_preorder():
            try:
                ...
                continue
            if kind != cindex.CursorKind.FUNCTION_DECL or not n.is_definition():
                continue
            if n.location.file is None or os.path.realpath(n.location.file.name) != real_source_path:
                continue
            is_static = n.storage_class == cindex.StorageClass.STATIC
            entry = (is_static, _function_params(n, cindex), _function_return_type(n))
            lst = candidates.setdefault(n.spelling, [])
...
def parse_declarations(header_paths, source_paths, extra_args):
    """Return header declarations, with implementation-only definitions preferred.

    `source_paths` is deliberately separate from `header_paths`: parsing an
    implementation TU pulls in declarations from many headers, but only a
    definition physically located in that TU should override the public-header
    candidate set.
    """
    declarations = parse_headers(header_paths, extra_args)
    source_names = set()
    for source_path in source_paths:
        for name, definitions in parse_function_definitions([source_path], extra_args).items():
            # A compatibility wrapper can coexist with a direct internal
            # implementation of the same exported symbol. The caller lists
            # wrapper TUs first, so retain the first definition's parameter
            # names: those are the identifiers the migration must capture.
            if name not in source_names:
                declarations[name] = definitions
...
def _select_declaration_candidate(name, candidates, yaml_arg_names):
    """Return the one authoritative candidate (see resolve_declaration())."""
    if not candidates:
        raise AmbiguousDeclarationError(f"{name}: no declarations found")
    if len(candidates) == 1:
        return candidates[0]
    ...
    for candidate in candidates:
        lines.append(f"  {_candidate_sig_str(name, candidate[0], candidate[1])}")
...
def resolve_declaration(name, candidates, yaml_arg_names):
    ...
    Resolution order:
      ...
      4. If exactly one candidate remains after that narrowing, it
         ...
         to walk last.
    """
    return list(_select_declaration_candidate(name, candidates, yaml_arg_names)[1])


def resolve_return_type(name, candidates, yaml_arg_names):
    """Resolve ``name`` using the parameter-name policy, then return its
    libclang result-type tuple.

    The generic migration inventories already classified all wrappers by
    result kind. Curated wrappers need the more precise result type too: the
    generic STATUS bucket deliberately includes ordinary 32-bit values such
    as HIP's ``int hipGetStreamDeviceId`` and HSA's ``uint32_t`` signal waits.
    """
    candidate = _select_declaration_candidate(name, candidates, yaml_arg_names)
    if len(candidate) < 3:
        raise UnsupportedReturnTypeError(
            f"{name}: declaration has no libclang result type; cannot select curated return macro")
    return candidate[2]


def classify_curated_return_kind(provider, return_type):
    """Map a libclang result type to the provider's curated return macro kind.

    This is the type-aware extension of the original generic tiers. Their
    STATUS exit event represented both status enums and ordinary 32-bit
    scalar results; curated helpers require a real success status for their
    optional OUT captures, so scalar values must not be treated as errors.
    Unknown kinds are rejected instead of silently falling back to STATUS.
    """
    c_type, canonical, type_kind, canonical_kind = return_type
    c_type = c_type.strip()
    canonical = canonical.strip()

    if c_type == 'void' or canonical == 'void':
        return 'VOID'
    if type_kind == 'POINTER' or canonical_kind == 'POINTER' or '*' in canonical:
        return 'PTR'
    if type_kind == 'RECORD' or canonical_kind == 'RECORD':
        return 'STRUCT'

    if provider == 'hip':
        if c_type == 'hipError_t':
            return 'STATUS'
        if c_type == 'int' or canonical == 'int':
            return 'I32'
    elif provider == 'hsa':
        if c_type == 'hsa_status_t':
            return 'STATUS'
        if c_type == 'uint32_t' or canonical == 'unsigned int':
            return 'U32'
        if c_type == 'uint64_t' or canonical in ('unsigned long', 'unsigned long long'):
            return 'U64'
        if c_type in ('hsa_signal_value_t', 'int64_t') or canonical in ('long', 'long long'):
            return 'I64'

    raise UnsupportedReturnTypeError(
        f"{provider} return type {c_type!r} (canonical {canonical!r}, "
        f"kinds {type_kind}/{canonical_kind}) has no curated return macro")


def resolve_curated_return_kind(provider, name, candidates, yaml_arg_names):
    """Resolve and classify one API result type through the shared libclang path."""
    return classify_curated_return_kind(
        provider, resolve_return_type(name, candidates, yaml_arg_names))
...
def verify(yaml_path, header_paths, extra_args, out_sidecar=None, source_paths=()):
...
def main():
    ...
    ap.add_argument('--source', action='append', default=[],
                    help='Wrapper implementation TU to use for exported APIs '
                    'that have no public-header declaration. May be repeated.')
    ...
    sys.exit(verify(args.yaml, args.header, args.extra_arg,
                    out_sidecar=args.out_sidecar, source_paths=args.source))
