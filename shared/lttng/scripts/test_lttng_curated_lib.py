...
from lttng_curated_lib import (
    event_field_chunks, parse_yaml_text, validate_api, expanded_field_count,
    DSL_TYPES, ALLOWED_DIRS, ParseError, IN_ARG_KIND,
)
...
# ---- Per-event budget chunking (validate_api) ----
...
def test_validate_over_budget_splits_events():
    """High-arity APIs preserve all arguments in ordered event chunks."""
    api = {'api': 'foo', 'category': 'kernel_launch', 'args': [
        {'name': f'a{i}', 'type': 'uint32', 'dir': 'IN'} for i in range(11)
    ]}
    validate_api(api)
    chunks = event_field_chunks(api)
    assert [[arg['name'] for arg in chunk] for chunk in chunks] == [
        [f'a{i}' for i in range(10)], ['a10']]
