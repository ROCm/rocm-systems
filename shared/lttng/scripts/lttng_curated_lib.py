...
The LTTng-UST ten-field budget applies to each generated event, not an API
as a whole. High-arity APIs are emitted as ordered `_args`, `_args_2`, ...
event chunks; event_field_chunks() makes that partitioning available to the
generator. `category` (removed) and `dir: INOUT` (never implemented, removed
...
class BudgetError(Exception):
    """One argument expands beyond a single tracepoint event's field budget."""
...
def event_field_chunks(api):
    """Partition fully-resolved API args into ordered event-size chunks.

    LTTng-UST accepts at most PAYLOAD_BUDGET fields per tracepoint event.
    Keep each argument intact (notably an unpacked dim3's three fields) and
    start a new event only between arguments. The first chunk becomes the
    API's `<api>_args` event; each later chunk becomes `<api>_args_N`.
    """
    chunks = []
    current = []
    current_fields = 0
    for arg in api['args']:
        fields = _type_expand(arg) * _dir_expand(arg)
        if fields > PAYLOAD_BUDGET:
            raise BudgetError(
                f"{api['api']} arg {arg['name']}: expands to {fields} fields, exceeds "
                f"the per-event budget of {PAYLOAD_BUDGET}")
...
def parse_api_entry(entry):
    ...
    of `args`:
      - list of {name, type, dir} mappings -> explicit shape, returned
        unchanged (already fully resolved and event-budget-checked).
