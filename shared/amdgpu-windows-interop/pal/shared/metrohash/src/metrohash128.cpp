...
const char *MetroHash128::test_string =
...
MetroHash128::MetroHash128(const uint64_t seed) { Initialize(seed); }
...
void MetroHash128::Finalize(uint8_t *const hash) {
  ...
  if (bytes >= 32) {
    state.v[2] ^=
        rotate_right(((state.v[0] + state.v[3]) * k0) + state.v[1], 21) * k1;
    state.v[3] ^=
        rotate_right(((state.v[1] + state.v[2]) * k1) + state.v[0], 21) * k0;
    state.v[0] ^=
        rotate_right(((state.v[0] + state.v[2]) * k0) + state.v[3], 21) * k1;
    state.v[1] ^=
        rotate_right(((state.v[1] + state.v[3]) * k1) + state.v[2], 21) * k0;
  }
  ...
  if ((end - ptr) >= 16) {
    state.v[0] += read_u64(ptr) * k2;
    ptr += 8;
    state.v[0] = rotate_right(state.v[0], 33) * k3;
    state.v[1] += read_u64(ptr) * k2;
    ptr += 8;
    state.v[1] = rotate_right(state.v[1], 33) * k3;
...
void MetroHash128::Hash(const uint8_t *buffer, const uint64_t length,
                        uint8_t *const hash, const uint64_t seed) {
  const uint8_t *ptr = reinterpret_cast<const uint8_t *>(buffer);
  const uint8_t *const end = ptr + length;
...
bool MetroHash128::ImplementationVerified() {
  ...
  metro.Initialize(0);
  metro.Update(reinterpret_cast<const uint8_t *>(MetroHash128::test_string),
               strlen(MetroHash128::test_string));
  metro.Finalize(hash);
  if (memcmp(hash, MetroHash128::test_seed_0, 16) != 0)
    return false;
