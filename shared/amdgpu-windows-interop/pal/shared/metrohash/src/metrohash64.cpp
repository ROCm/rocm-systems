...
const char *MetroHash64::test_string =
...
MetroHash64::MetroHash64(const uint64_t seed) { Initialize(seed); }
...
void MetroHash64::Finalize(uint8_t *const hash) {
  ...
  if (bytes >= 32) {
    state.v[2] ^=
        rotate_right(((state.v[0] + state.v[3]) * k0) + state.v[1], 37) * k1;
    state.v[3] ^=
        rotate_right(((state.v[1] + state.v[2]) * k1) + state.v[0], 37) * k0;
    state.v[0] ^=
        rotate_right(((state.v[0] + state.v[2]) * k0) + state.v[3], 37) * k1;
    state.v[1] ^=
        rotate_right(((state.v[1] + state.v[3]) * k1) + state.v[2], 37) * k0;
...
void MetroHash64::Hash(const uint8_t *buffer, const uint64_t length,
...
bool MetroHash64::ImplementationVerified() {
  ...
  metro.Initialize(0);
  metro.Update(reinterpret_cast<const uint8_t *>(MetroHash64::test_string),
               strlen(MetroHash64::test_string));
  metro.Finalize(hash);
  if (memcmp(hash, MetroHash64::test_seed_0, 8) != 0)
    return false;
