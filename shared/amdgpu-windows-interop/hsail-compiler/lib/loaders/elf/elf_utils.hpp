...
public:
  ...
  //
  // Save the error string in ErrorMessage.  If it is built without NDEBUG, the
 ...
 */
off_t xlseek(OclElfErr &err, const char *fname, const int fd, const off_t o,
             const int whence);

/*
   Return the number of bytes that are read on success; return -1 on error.
 */
ssize_t xread(OclElfErr &err,
