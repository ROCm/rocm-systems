...
public:
  /*
     OclElf object can be created for reading or writing (it could be created
     for both reading and writing, which is not supported yet at this time).
     Currently, it has two forms:

      1)  OclElf(eclass, rawElfBytes, rawElfSize, 0, ELF_C_READ)

          To load ELF from raw bytes in memory and generate OclElf object. And
     this object is for reading only.

      2)  OclElf(eclass,  NULL, 0, elfFileName|NULL, ELF_C_WRITE)

          To create an ELF for writing and save it into a file 'elfFileName' (if
     it is NULL, the OclElf will create a temporary file and set it to
     'elfFileName'.

          Since we need to read the ELF into memory, this file 'elfFileName' is
     created with both read and write, so that the runtime can use dumpImage()
     to get ELF raw bytes by reading this file.
      ...
      Return values of all public APIs with bool return type
         ...
         false : on error.
   */
  OclElf(unsigned char eclass,    // eclass for this ELF
  ...
  /*
     addSection() is used to create a single ELF section with data <d_buf,
     d_size>. If do_copy is true, the OclElf object will make a copy of d_buf
     and uses that copy to create an ELF section.

     When setting do_copy = false,  the caller should make sure that <d_buf,
     d_size> will be unchanged and available during the lifetime of this OclElf
     object; ie before calling dumpImage().
   */
  bool addSection(oclElfSections id, const void *d_buf, size_t d_size,
                  bool do_copy = true);
   ...
   */
  bool getSymbol(oclElfSections id,      // Section in which symbol is in
  ...
  */
  bool addNote(const char *noteName, const char *noteDesc, size_t nameSize,
               size_t descSize);
  ...
  */
  ...
  bool isHsaCo() const {
    return (_eclass == ELFCLASS32)
               ? (elf32_getehdr(_e)->e_machine == EM_AMDGPU)
               : (elf64_getehdr(_e)->e_machine == EM_AMDGPU);
  }
...
private:
  ...
  bool createShdr(oclElfSections id, Elf_Scn *&scn, Elf64_Word shname,
                  Elf64_Word shlink = 0);

  Elf_Data *createElfData(Elf_Scn *&scn, oclElfSections id, void *d_buf,
  ...
  /*
     ...
     Note that newSection() uses Section Header's size, so make sure
     elf_update() is invoked properly before invoking newSection().
   */
  Elf_Scn *newSection(oclElfSections id, const void *d_buf, size_t d_size,
   ...
   */
  bool addSectionData(Elf64_Xword &outOffset, oclElfSections id,
