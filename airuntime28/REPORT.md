...
The ticket assumed the hint required dropping the access width from 128-bit to 64-bit, because
`__builtin_nontemporal_store` rejects HIP's `ulong2`. That is true in HIP but not in OpenCL,
and the blit kernel is OpenCL, where `ulong2` is a native vector the builtin accepts. So the
width sacrifice was never required — which matters, because width turns out to dominate
...
Commit `81e65d6bbb` on `users/victzhan/AIRUNTIME-28-nt-blit`, based on `8a43c008e9`
(`develop` head at the time of writing). 59 insertions, 4 deletions, 6 files.
`airuntime28-nt-blit.patch` is generated from that commit by `remote/clr_patch.sh`, which also
verifies that applying it to the base reproduces the commit exactly, so the branch and the
patch cannot drift.
