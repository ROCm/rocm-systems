...
- **`(noise)` means do not quote the number.** Two bars have to be cleared for a result to
  count: its 95% interval must exclude zero, *and* its size must exceed the run's resolution
  limit. The resolution limit is the widest difference the rig reports between two slots running
  the *same* arm, where the true answer is zero — so an effect no bigger than that is
  indistinguishable from the rig measuring nothing, however tight its interval looks. It is a
  magnitude threshold, not a plus-or-minus band.
