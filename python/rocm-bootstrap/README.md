...
The variant provider reports package owners in its `gfx_arch` values. Both
`gfx1250` and `gfx1250-strict` select the shared `gfx1250` package, matching the
variant metadata and dependency markers generated by kpack's wheel splitter.
...
For `gfx1250` and `gfx1250-strict`, the shared `gfx1250` package may contain
payloads for either target or both. The build and publication must supply the
appropriate contents: selecting this shared owner does not guarantee that a
particular target's payload is present. This shared-owner mapping applies
specifically to these two targets; other targets retain their existing package
ownership. Kpack archive names and runtime payload lookup retain their target
identities.
