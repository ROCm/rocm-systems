...
template <>
NCCL_DEVICE_INLINE EltPack<__hip_bfloat16, 2> load<EltPack<__hip_bfloat16, 2>, true, OpSum<__hip_bfloat16>>(
  const EltPack<__hip_bfloat16, 2>* addr) {
  EltPack<__hip_bfloat16, 2> result;
  uint32_t raw;
...
template <>
NCCL_DEVICE_INLINE EltPack<__hip_bfloat16, 1> load<EltPack<__hip_bfloat16, 1>, true, OpSum<__hip_bfloat16>>(
  const EltPack<__hip_bfloat16, 1>* addr) {
#ifndef NCCL_DEVICE_PERMIT_EXPERIMENTAL_CODE
  assert(false && "Experimental NCCL device code detected; you may accept the risk "
                  "of this not being available in the future. If you accept that risk, "
                  "set NCCL_DEVICE_PERMIT_EXPERIMENTAL_CODE during compilation or "
                  "refactor the code. More details "
                  "https://docs.nvidia.com/cuda/parallel-thread-execution/#addresses-as-operands");
  return EltPack<__hip_bfloat16, 1>{};
#else
  // Align address to 4 bytes for bf16x2 load
  const char* charAddr = reinterpret_cast<const char*>(addr);
  const size_t offset = reinterpret_cast<size_t>(addr) & 3;
  const char* alignedAddr = charAddr - offset;
  // Load as EltPack<__hip_bfloat16, 2> and extract the correct bf16
  EltPack<__hip_bfloat16, 2> loaded = load<EltPack<__hip_bfloat16, 2>, true, OpSum<__hip_bfloat16>>(
    reinterpret_cast<const EltPack<__hip_bfloat16, 2>*>(alignedAddr));
  EltPack<__hip_bfloat16, 1> result;
  const __hip_bfloat16* loadedElts = loaded.elts();
  // Extract the correct bf16 based on address offset
  result.elts()[0] = loadedElts[offset / sizeof(__hip_bfloat16)];
  return result;
#endif
}
...
template <>
NCCL_DEVICE_INLINE EltPack<__hip_bfloat16, 8> load<EltPack<__hip_bfloat16, 8>, true, OpSum<__hip_bfloat16>>(
  const EltPack<__hip_bfloat16, 8>* addr) {
  // Use v4.bf16x2 instruction - bf16 requires its own instruction format
  EltPack<__hip_bfloat16, 8> result;
  uint32_t raw[4];
  ...
  union {
    uint32_t raw[4];
    __hip_bfloat16 elts[8];
  } packed{{raw[0], raw[1], raw[2], raw[3]}};
  __hip_bfloat16* out = result.elts();
  NVCC_PRAGMA_UNROLL(8)
  for (int i = 0; i < 8; i++) out[i] = packed.elts[i];
  return result;
}
...
template <>
NCCL_DEVICE_INLINE EltPack<__hip_fp8_e4m3_fnuz, 4> load<EltPack<__hip_fp8_e4m3_fnuz, 4>, true, OpSum<__hip_fp8_e4m3_fnuz>>(
  const EltPack<__hip_fp8_e4m3_fnuz, 4>* addr) {
  ...
  EltPack<__hip_fp8_e4m3_fnuz, 4> result;
  uint32_t raw;
  ...
  union {
    uint32_t raw;
    __hip_fp8_e4m3_fnuz elts[4];
  } packed{raw};
  __hip_fp8_e4m3_fnuz* out = result.elts();
  NVCC_PRAGMA_UNROLL_AUTO
  for (int i = 0; i < 4; i++) out[i] = packed.elts[i];
  return result;
#else
  assert(false && "FP8 multimem is not supported on this architecture.");
  return EltPack<__hip_fp8_e4m3_fnuz, 4>{};
#endif
}

// FP8 E4M3 precision - 2 elements (trick: load as e4m3x4, extract two fp8s)
template <>
NCCL_DEVICE_INLINE EltPack<__hip_fp8_e4m3_fnuz, 2> load<EltPack<__hip_fp8_e4m3_fnuz, 2>, true, OpSum<__hip_fp8_e4m3_fnuz>>(
  const EltPack<__hip_fp8_e4m3_fnuz, 2>* addr) {
#ifndef NCCL_DEVICE_PERMIT_EXPERIMENTAL_CODE
  assert(false && "Experimental NCCL device code detected; you may accept the risk "
                  "of this not being available in the future. If you accept that risk, "
                  "set NCCL_DEVICE_PERMIT_EXPERIMENTAL_CODE during compilation or "
                  "refactor the code. More details "
                  "https://docs.nvidia.com/cuda/parallel-thread-execution/#addresses-as-operands");
  return EltPack<__hip_fp8_e4m3_fnuz, 2>{};
#else
  // Align address to 4 bytes for e4m3x4 load
  const char* charAddr = reinterpret_cast<const char*>(addr);
  const size_t offset = reinterpret_cast<size_t>(addr) & 3;
  const char* alignedAddr = charAddr - offset;
  // Load as EltPack<__hip_fp8_e4m3_fnuz, 4> and extract the correct two fp8s
  EltPack<__hip_fp8_e4m3_fnuz, 4> loaded = load<EltPack<__hip_fp8_e4m3_fnuz, 4>, true, OpSum<__hip_fp8_e4m3_fnuz>>(
    reinterpret_cast<const EltPack<__hip_fp8_e4m3_fnuz, 4>*>(alignedAddr));
  EltPack<__hip_fp8_e4m3_fnuz, 2> result;
  // Extract the correct two fp8s based on address offset
  const __hip_fp8_e4m3_fnuz* loadedElts = loaded.elts();
  const int startIdx = offset / sizeof(__hip_fp8_e4m3_fnuz);
  __hip_fp8_e4m3_fnuz* resultElts = result.elts();
...
template <>
NCCL_DEVICE_INLINE EltPack<__hip_fp8_e4m3_fnuz, 1> load<EltPack<__hip_fp8_e4m3_fnuz, 1>, true, OpSum<__hip_fp8_e4m3_fnuz>>(
  const EltPack<__hip_fp8_e4m3_fnuz, 1>* addr) {
#ifndef NCCL_DEVICE_PERMIT_EXPERIMENTAL_CODE
  assert(false && "Experimental NCCL device code detected; you may accept the risk "
                  "of this not being available in the future. If you accept that risk, "
                  "set NCCL_DEVICE_PERMIT_EXPERIMENTAL_CODE during compilation or "
                  "refactor the code. More details "
                  "https://docs.nvidia.com/cuda/parallel-thread-execution/#addresses-as-operands");
  return EltPack<__hip_fp8_e4m3_fnuz, 1>{};
#else
  // Align address to 4 bytes for e4m3x4 load
  const char* charAddr = reinterpret_cast<const char*>(addr);
  const size_t offset = reinterpret_cast<size_t>(addr) & 3;
  const char* alignedAddr = charAddr - offset;
  // Load as EltPack<__hip_fp8_e4m3_fnuz, 4> and extract the correct fp8
  EltPack<__hip_fp8_e4m3_fnuz, 4> loaded = load<EltPack<__hip_fp8_e4m3_fnuz, 4>, true, OpSum<__hip_fp8_e4m3_fnuz>>(
    reinterpret_cast<const EltPack<__hip_fp8_e4m3_fnuz, 4>*>(alignedAddr));
  EltPack<__hip_fp8_e4m3_fnuz, 1> result;
  // Extract the correct fp8 based on address offset
  const __hip_fp8_e4m3_fnuz* loadedElts = loaded.elts();
  result.elts()[0] = loadedElts[offset / sizeof(__hip_fp8_e4m3_fnuz)];
  return result;
#endif
}
...
// Only define FP8 multimem specializations for supported architectures - otherwise fallback to generic template
template <>
NCCL_DEVICE_INLINE EltPack<__hip_fp8_e4m3_fnuz, 16> load<EltPack<__hip_fp8_e4m3_fnuz, 16>, true, OpSum<__hip_fp8_e4m3_fnuz>>(
  const EltPack<__hip_fp8_e4m3_fnuz, 16>* addr) {
#if (defined(__CUDA_ARCH_SPECIFIC__) && (__CUDA_ARCH_SPECIFIC__ == 1000 || __CUDA_ARCH_SPECIFIC__ == 1010 || \
                                         __CUDA_ARCH_SPECIFIC__ == 1200 || __CUDA_ARCH_SPECIFIC__ == 1210)) || \
  (defined(__CUDA_ARCH_FAMILY_SPECIFIC__) && \
   (__CUDA_ARCH_FAMILY_SPECIFIC__ == 1000 || __CUDA_ARCH_FAMILY_SPECIFIC__ == 1010))
  EltPack<__hip_fp8_e4m3_fnuz, 16> result;
  uint32_t raw[4];
  ...
  return EltPack<__hip_fp8_e4m3_fnuz, 16>{};
#endif
}
...
// Uses .acc::f16 accumulation (accumulates to half precision)
template <>
NCCL_DEVICE_INLINE EltPack<__hip_fp8_e5m2_fnuz, 4> load<EltPack<__hip_fp8_e5m2_fnuz, 4>, true, OpSum<__hip_fp8_e5m2_fnuz>>(
  const EltPack<__hip_fp8_e5m2_fnuz, 4>* addr) {
#if (defined(__CUDA_ARCH_SPECIFIC__) && (__CUDA_ARCH_SPECIFIC__ == 1000 || __CUDA_ARCH_SPECIFIC__ == 1010 || \
                                         __CUDA_ARCH_SPECIFIC__ == 1200 || __CUDA_ARCH_SPECIFIC__ == 1210)) || \
  (defined(__CUDA_ARCH_FAMILY_SPECIFIC__) && \
   (__CUDA_ARCH_FAMILY_SPECIFIC__ == 1000 || __CUDA_ARCH_FAMILY_SPECIFIC__ == 1010))
  EltPack<__hip_fp8_e5m2_fnuz, 4> result;
  uint32_t raw;
  ...
  union {
    uint32_t raw;
    __hip_fp8_e5m2_fnuz elts[4];
  } packed{raw};
  __hip_fp8_e5m2_fnuz* out = result.elts();
  NVCC_PRAGMA_UNROLL_AUTO
  for (int i = 0; i < 4; i++) out[i] = packed.elts[i];
  return result;
#else
  assert(false && "FP8 multimem with is not supported on this architecture.");
  return EltPack<__hip_fp8_e5m2_fnuz, 4>{};
#endif
}

// FP8 E5M2 precision - 2 elements (trick: load as e5m2x4, extract two fp8s)
template <>
NCCL_DEVICE_INLINE EltPack<__hip_fp8_e5m2_fnuz, 2> load<EltPack<__hip_fp8_e5m2_fnuz, 2>, true, OpSum<__hip_fp8_e5m2_fnuz>>(
  const EltPack<__hip_fp8_e5m2_fnuz, 2>* addr) {
#ifndef NCCL_DEVICE_PERMIT_EXPERIMENTAL_CODE
  assert(false && "Experimental NCCL device code detected; you may accept the risk "
                  "of this not being available in the future. If you accept that risk, "
                  "set NCCL_DEVICE_PERMIT_EXPERIMENTAL_CODE during compilation or "
                  "refactor the code. More details "
                  "https://docs.nvidia.com/cuda/parallel-thread-execution/#addresses-as-operands");
  return EltPack<__hip_fp8_e5m2_fnuz, 2>{};
#else
  // Align address to 4 bytes for e5m2x4 load
  const char* charAddr = reinterpret_cast<const char*>(addr);
  const size_t offset = reinterpret_cast<size_t>(addr) & 3;
  const char* alignedAddr = charAddr - offset;
  // Load as EltPack<__hip_fp8_e5m2_fnuz, 4> and extract the correct two fp8s
  EltPack<__hip_fp8_e5m2_fnuz, 4> loaded = load<EltPack<__hip_fp8_e5m2_fnuz, 4>, true, OpSum<__hip_fp8_e5m2_fnuz>>(
    reinterpret_cast<const EltPack<__hip_fp8_e5m2_fnuz, 4>*>(alignedAddr));
  EltPack<__hip_fp8_e5m2_fnuz, 2> result;
  // Extract the correct two fp8s based on address offset
  const __hip_fp8_e5m2_fnuz* loadedElts = loaded.elts();
  const int startIdx = offset / sizeof(__hip_fp8_e5m2_fnuz);
  __hip_fp8_e5m2_fnuz* resultElts = result.elts();
  resultElts[0] = loadedElts[startIdx];
  resultElts[1] = loadedElts[startIdx + 1];
  return result;
#endif
...
template <>
NCCL_DEVICE_INLINE EltPack<__hip_fp8_e5m2_fnuz, 1> load<EltPack<__hip_fp8_e5m2_fnuz, 1>, true, OpSum<__hip_fp8_e5m2_fnuz>>(
  const EltPack<__hip_fp8_e5m2_fnuz, 1>* addr) {
#ifndef NCCL_DEVICE_PERMIT_EXPERIMENTAL_CODE
  assert(false && "Experimental NCCL device code detected; you may accept the risk "
                  "of this not being available in the future. If you accept that risk, "
                  "set NCCL_DEVICE_PERMIT_EXPERIMENTAL_CODE during compilation or "
                  "refactor the code. More details "
                  "https://docs.nvidia.com/cuda/parallel-thread-execution/#addresses-as-operands");
  return EltPack<__hip_fp8_e5m2_fnuz, 1>{};
#else
  // Align address to 4 bytes for e5m2x4 load
  const char* charAddr = reinterpret_cast<const char*>(addr);
  const size_t offset = reinterpret_cast<size_t>(addr) & 3;
  const char* alignedAddr = charAddr - offset;
  // Load as EltPack<__hip_fp8_e5m2_fnuz, 4> and extract the correct fp8
  EltPack<__hip_fp8_e5m2_fnuz, 4> loaded = load<EltPack<__hip_fp8_e5m2_fnuz, 4>, true, OpSum<__hip_fp8_e5m2_fnuz>>(
    reinterpret_cast<const EltPack<__hip_fp8_e5m2_fnuz, 4>*>(alignedAddr));
  EltPack<__hip_fp8_e5m2_fnuz, 1> result;
  // Extract the correct fp8 based on address offset
  const __hip_fp8_e5m2_fnuz* loadedElts = loaded.elts();
  result.elts()[0] = loadedElts[offset / sizeof(__hip_fp8_e5m2_fnuz)];
  return result;
#endif
}
...
template <>
NCCL_DEVICE_INLINE EltPack<__hip_fp8_e5m2_fnuz, 16> load<EltPack<__hip_fp8_e5m2_fnuz, 16>, true, OpSum<__hip_fp8_e5m2_fnuz>>(
  const EltPack<__hip_fp8_e5m2_fnuz, 16>* addr) {
#if (defined(__CUDA_ARCH_SPECIFIC__) && (__CUDA_ARCH_SPECIFIC__ == 1000 || __CUDA_ARCH_SPECIFIC__ == 1010 || \
                                         __CUDA_ARCH_SPECIFIC__ == 1200 || __CUDA_ARCH_SPECIFIC__ == 1210)) || \
  (defined(__CUDA_ARCH_FAMILY_SPECIFIC__) && \
   (__CUDA_ARCH_FAMILY_SPECIFIC__ == 1000 || __CUDA_ARCH_FAMILY_SPECIFIC__ == 1010))
  EltPack<__hip_fp8_e5m2_fnuz, 16> result;
  uint32_t raw[4];
...
#else
  assert(false && "FP8 multimem with is not supported on this architecture.");
  return EltPack<__hip_fp8_e5m2_fnuz, 16>{};
#endif
}
