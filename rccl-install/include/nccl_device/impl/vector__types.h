...
template <>
struct AccumulateType<OpSum<__hip_bfloat16>> {
...
template <>
struct AccumulateType<OpSum<__hip_fp8_e4m3_fnuz>> {
  using Type = half;  // fp8 accumulates into half precision (matches .acc::f16 in multimem)
};
template <>
struct AccumulateType<OpSum<__hip_fp8_e5m2_fnuz>> {
  using Type = half;  // fp8 accumulates into half precision (matches .acc::f16 in multimem)
};
#endif
...
#if defined(__CUDA_BF16_TYPES_EXIST__)
template <>
struct MinMultimemType<__hip_bfloat16> {
  using Type = EltPack<__hip_bfloat16, 2>; // Minimum multimem type for bfloat16 is EltPack<__hip_bfloat16, 2> (32 bits)
};
#endif

#if defined(__CUDA_FP8_TYPES_EXIST__)
template <>
struct MinMultimemType<__hip_fp8_e4m3_fnuz> {
  using Type = EltPack<__hip_fp8_e4m3_fnuz, 4>; // Minimum multimem type for fp8_e4m3 is EltPack<__hip_fp8_e4m3_fnuz, 4> (32 bits)
};
template <>
struct MinMultimemType<__hip_fp8_e5m2_fnuz> {
  using Type = EltPack<__hip_fp8_e5m2_fnuz, 4>; // Minimum multimem type for fp8_e5m2 is EltPack<__hip_fp8_e5m2_fnuz, 4> (32 bits)
};
#endif
