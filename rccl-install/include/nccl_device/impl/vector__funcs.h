...
NCCL_DEVICE_INLINE bool tryComplexStridedAlignmentForPackSize(
  ...
  // Strided offsets must preserve relative alignment for all src/dst pairs.
  // This requires both displacements to be multiples of PackBytes.
...
// Specialization for __hip_bfloat16 -> float conversion (upcast to accumulation type)
template <>
NCCL_DEVICE_INLINE EltPack<float, 1> castPack(EltPack<__hip_bfloat16, 1> x) {
  EltPack<float, 1> out{};
...
template <>
NCCL_DEVICE_INLINE EltPack<float, 2> castPack(EltPack<__hip_bfloat16, 2> x) {
  union Bf162PackAccess {
    EltPack<__hip_bfloat16, 2> pack;
...
// Specialization for float -> __hip_bfloat16 conversion (downcast from accumulation type)
template <>
NCCL_DEVICE_INLINE EltPack<__hip_bfloat16, 1> castPack(EltPack<float, 1> x) {
  EltPack<__hip_bfloat16, 1> out{};
  out.elts()[0] = __float2bfloat16_rn(x.elts()[0]);  // Round to nearest
  return out;
}

template <>
NCCL_DEVICE_INLINE EltPack<__hip_bfloat16, 2> castPack(EltPack<float, 2> x) {
  union Bf162PackAccess {
    EltPack<__hip_bfloat16, 2> pack;
    __hip_bfloat162 pair;
  };
...
// Specialization for __hip_fp8_e4m3_fnuz -> half conversion (upcast to accumulation type)
// Uses vectorized fp8x2 -> half2 when available for SIMD performance
template <>
NCCL_DEVICE_INLINE EltPack<half, 1> castPack(EltPack<__hip_fp8_e4m3_fnuz, 1> x) {
  union HalfRawAccess {
    __half_raw raw;
    half val;
  };
  union Fp8E4m3Access1 {
    EltPack<__hip_fp8_e4m3_fnuz, 1> pack;
    __hip_fp8_storage_t storage;
  };
  Fp8E4m3Access1 in;
  EltPack<half, 1> out{};
  in.pack = x;
  HalfRawAccess h;
  h.raw = __hip_cvt_fp8_to_halfraw(in.storage, __HIP_E4M3_FNUZ);
  out.elts()[0] = h.val;
  return out;
}

template <>
NCCL_DEVICE_INLINE EltPack<half, 2> castPack(EltPack<__hip_fp8_e4m3_fnuz, 2> x) {
  ...
  union Fp8E4m3Access2 {
    EltPack<__hip_fp8_e4m3_fnuz, 2> pack;
    __hip_fp8x2_storage_t storage2;
  };
  Fp8E4m3Access2 in;
  Half2PackAccess out;
  in.pack = x;
  Half2RawAccess h2;
  h2.raw = __hip_cvt_fp8x2_to_halfraw2(in.storage2, __HIP_E4M3_FNUZ);
  out.pair = h2.val;
  return out.pack;
#else
  PackAccess<__hip_fp8_e4m3_fnuz, 2> in;
...
// Specialization for __hip_fp8_e5m2_fnuz -> half conversion (upcast to accumulation type)
// Uses vectorized fp8x2 -> half2 when available for SIMD performance
template <>
NCCL_DEVICE_INLINE EltPack<half, 1> castPack(EltPack<__hip_fp8_e5m2_fnuz, 1> x) {
  union HalfRawAccess {
    __half_raw raw;
    half val;
  };
  union Fp8E5m2Access1 {
    EltPack<__hip_fp8_e5m2_fnuz, 1> pack;
    __hip_fp8_storage_t storage;
  };
  Fp8E5m2Access1 in;
  EltPack<half, 1> out{};
  in.pack = x;
  HalfRawAccess h;
  h.raw = __hip_cvt_fp8_to_halfraw(in.storage, __HIP_E5M2_FNUZ);
  out.elts()[0] = h.val;
  return out;
}

template <>
NCCL_DEVICE_INLINE EltPack<half, 2> castPack(EltPack<__hip_fp8_e5m2_fnuz, 2> x) {
#if __CUDA_ARCH__ >= 900
  union Half2RawAccess {
    __half2_raw raw;
    half2 val;
  ...
  union Fp8E5m2Access2 {
    EltPack<__hip_fp8_e5m2_fnuz, 2> pack;
    __hip_fp8x2_storage_t storage2;
  };
  Fp8E5m2Access2 in;
  Half2PackAccess out;
  in.pack = x;
  Half2RawAccess h2;
  h2.raw = __hip_cvt_fp8x2_to_halfraw2(in.storage2, __HIP_E5M2_FNUZ);
  out.pair = h2.val;
  return out.pack;
#else
  PackAccess<__hip_fp8_e5m2_fnuz, 2> in;
  PackAccess<half, 2> out;
  in.pack = x;
  out.lo = castPack<half>(in.lo);
  out.hi = castPack<half>(in.hi);
...
// Specialization for half -> __hip_fp8_e4m3_fnuz conversion (downcast from accumulation type)
// Uses vectorized half2 -> fp8x2 when available for SIMD performance
template <>
NCCL_DEVICE_INLINE EltPack<__hip_fp8_e4m3_fnuz, 1> castPack(EltPack<half, 1> x) {
  union HalfRawAccess {
    __half_raw raw;
    half val;
  };
  ...
  HalfRawAccess h;
  h.val = x.elts()[0];
  out.storage = __hip_cvt_halfraw_to_fp8(h.raw, __HIP_SATFINITE, __HIP_E4M3_FNUZ);
  return out.pack;
}

template <>
NCCL_DEVICE_INLINE EltPack<__hip_fp8_e4m3_fnuz, 2> castPack(EltPack<half, 2> x) {
#if __CUDA_ARCH__ >= 900
  union Half2RawAccess {
    __half2_raw raw;
    half2 val;
  ...
  Half2RawAccess h2;
  h2.val = in.pair;
  out.storage2 = __hip_cvt_halfraw2_to_fp8x2(h2.raw, __HIP_SATFINITE, __HIP_E4M3_FNUZ);
  return out.pack;
#else
  PackAccess<half, 2> in;
  PackAccess<__hip_fp8_e4m3_fnuz, 2> out;
  in.pack = x;
  out.lo = castPack<__hip_fp8_e4m3_fnuz>(in.lo);
  out.hi = castPack<__hip_fp8_e4m3_fnuz>(in.hi);
  return out.pack;
#endif
}

// Specialization for half -> __hip_fp8_e5m2_fnuz conversion (downcast from accumulation type)
// Uses vectorized half2 -> fp8x2 when available for SIMD performance
template <>
NCCL_DEVICE_INLINE EltPack<__hip_fp8_e5m2_fnuz, 1> castPack(EltPack<half, 1> x) {
  union HalfRawAccess {
    __half_raw raw;
    half val;
  };
  ...
  HalfRawAccess h;
  h.val = x.elts()[0];
  out.storage = __hip_cvt_halfraw_to_fp8(h.raw, __HIP_SATFINITE, __HIP_E5M2_FNUZ);
  return out.pack;
}

template <>
NCCL_DEVICE_INLINE EltPack<__hip_fp8_e5m2_fnuz, 2> castPack(EltPack<half, 2> x) {
#if __CUDA_ARCH__ >= 900
  union Half2RawAccess {
    __half2_raw raw;
    half2 val;
  ...
  Half2RawAccess h2;
  h2.val = in.pair;
  out.storage2 = __hip_cvt_halfraw2_to_fp8x2(h2.raw, __HIP_SATFINITE, __HIP_E5M2_FNUZ);
  return out.pack;
#else
  PackAccess<half, 2> in;
  PackAccess<__hip_fp8_e5m2_fnuz, 2> out;
  in.pack = x;
  out.lo = castPack<__hip_fp8_e5m2_fnuz>(in.lo);
  out.hi = castPack<__hip_fp8_e5m2_fnuz>(in.hi);
  return out.pack;
#endif
}
...
#if defined(__CUDA_BF16_TYPES_EXIST__)
// Specialization for __hip_bfloat16 with OpSum - uses __hadd2 SIMD intrinsic
// Architecture check: __CUDA_ARCH__ >= 530 && __CUDA_ARCH__ != 610
template <>
NCCL_DEVICE_INLINE EltPack<__hip_bfloat16, 2> reducePack(OpSum<__hip_bfloat16> const& /* red */,
                                                        EltPack<__hip_bfloat16, 2> a, EltPack<__hip_bfloat16, 2> b) {
#if __CUDA_ARCH__ >= 530 && __CUDA_ARCH__ != 610
  ...
  return out.pack;
#else
  EltPack<__hip_bfloat16, 2> out{};
  OpSum<__hip_bfloat16> red{};
  out.elts()[0] = red(a.elts()[0], b.elts()[0]);
  out.elts()[1] = red(a.elts()[1], b.elts()[1]);
  return out;
#endif
