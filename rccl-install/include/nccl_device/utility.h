...
// compiler specific check for __HIPCC__
// We define NCCL_CHECK_CUDACC as a textual alias of __HIPCC__ (not a
// "snapshot" 0/1 value) so each `#if NCCL_CHECK_CUDACC` re-evaluates
// __HIPCC__ at the point of use. This matters for the AMD path because
// some host-API headers (e.g. nccl_device/lsa_barrier.h) do
// `#undef __HIPCC__` followed by `#define __HIPCC__ 0` (which hipify
// rewrites to __HIPCC__) to hide device-side declarations from host TUs.
// With a snapshot value, NCCL_CHECK_CUDACC and __HIPCC__ would diverge
// after that point and we'd compile references to types that were never
// declared in this TU.
#ifndef NCCL_CHECK_CUDACC
#define NCCL_CHECK_CUDACC __HIPCC__
#endif
