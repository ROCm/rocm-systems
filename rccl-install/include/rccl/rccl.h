...
#define NCCL_MAJOR 2
#define NCCL_MINOR 31
#define NCCL_PATCH 2
#define NCCL_SUFFIX ""

#define NCCL_VERSION_CODE 23102
...
/*! @brief      Result type
  ...
  typedef enum {
    ...
    ncclNumResults              =  9  /*!< Number of result types */
  } ncclResult_t;
/*! @} */
...
#define NCCL_REVOKE_DEFAULT 0x00 /* default revoke behavior: quiesce in-flight work, reject new ops */
...
/*! @brief      Host-side collective fault tolerance mode */
typedef enum {
  ncclHostCftDefault = NCCL_CONFIG_UNDEF_INT, /*!< Use the library default */
  ncclHostCftEnable = 1,                      /*!< Enable host CFT */
  ncclHostCftDisable = 2,                     /*!< Disable host CFT */
  ncclHostCftFallback = 3                     /*!< Enable host CFT with fallback */
} ncclHostCftMode_t;
...
typedef struct ncclConfig_v23100 {
  ...
  int launchOrderImplicit;     /*!< Implicit launch order between communicators*/
  int numRmaSig;               /*!< Number of RMA signals*/
  int rmaEagerInit;            /*!< Eagerly initialize RMA resources*/
  int hostCftMode;             /*!< Host collective fault tolerance mode, see ncclHostCftMode_t*/
} ncclConfig_t;
...
typedef struct ncclCollConfig_v23100 {
  /* attributes that users should never touch. */
  size_t size;
  unsigned int magic;
...
ncclResult_t  ncclAllReduceConfig(const void* sendbuff, void* recvbuff, size_t count, ncclDataType_t datatype,
    ncclRedOp_t op, ncclComm_t comm, hipStream_t stream, const ncclCollConfig_t* config);
...
ncclResult_t  ncclBroadcastConfig(const void* sendbuff, void* recvbuff, size_t count, ncclDataType_t datatype,
    int root, ncclComm_t comm, hipStream_t stream, const ncclCollConfig_t* config);
...
ncclResult_t  ncclReduceConfig(const void* sendbuff, void* recvbuff, size_t count, ncclDataType_t datatype,
    ncclRedOp_t op, int root, ncclComm_t comm, hipStream_t stream, const ncclCollConfig_t* config);
...
ncclResult_t  ncclAllGatherConfig(const void* sendbuff, void* recvbuff, size_t sendcount, ncclDataType_t datatype,
    ncclComm_t comm, hipStream_t stream, const ncclCollConfig_t* config);
...
ncclResult_t pncclSignal(int peer, int sigIdx, int ctx, unsigned int flags, ncclComm_t comm, hipStream_t stream);
...
ncclResult_t pncclWaitSignal(int nDesc, ncclWaitSignalDesc_t* signalDescs, ncclComm_t comm, hipStream_t stream);
